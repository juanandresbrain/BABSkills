# Tax Transaction Details Report

**Workspace:** Enterprise Analytics QA  
**Report ID:** 27734c3a-310e-43cd-a015-66e18b2af581  
**Dataset ID:** 4a998365-013a-45f6-8e8d-7d5b7881b943  
**Web URL:** https://app.powerbi.com/groups/00856575-de9c-435b-ad8d-2ac43b338e3d/reports/27734c3a-310e-43cd-a015-66e18b2af581  
...
```mermaid
flowchart LR
    REPORT["Tax Transaction Details Report"]
    ...
    Min_Tax_Lines__JumpMind__ISO_Currency_Code_(["Min(Tax Lines (JumpMind).ISO Currency Code)"]) --> REPORT
    CountNonNull_Tax_Lines__JumpMind__Tax_Percentage_(["CountNonNull(Tax Lines (JumpMind).Tax Percentage)"]) --> REPORT
...
| Min(Tax Lines (JumpMind).ISO Currency Code) |
| CountNonNull(Tax Lines (JumpMind).Tax Percentage) |
...
| add10538a1e00e82da3d | pivotTable | Measure Table.Money Tax Amount (Native), Min(Tax Lines (JumpMind).ISO Currency Code), CountNonNull(Tax Lines (JumpMind).Tax Percentage), Measure Table.Taxable Amount (Native) \| Merchandise, Measure Table.Taxable Amount (Native) \| Fees, Measure Table.Non-Taxable Amount (Native) \| Merchandise, Measure Table.Non-Taxable Amount (Native) \| Fees, Measure Table.Tax Amount (Native), Transactions (JumpMind).Transaction Key, Retail Lines (JumpMind).Item Line, Business Units (JumpMind).Location Line, Locations (Store MDM).Country name, Locations (Store MDM).State/Province name |
