...
**Report ID:** 34955618-98ab-49c8-855d-aa365604f528  
**Dataset ID:** 4a998365-013a-45f6-8e8d-7d5b7881b943  
**Web URL:** https://app.powerbi.com/groups/00856575-de9c-435b-ad8d-2ac43b338e3d/reports/34955618-98ab-49c8-855d-aa365604f528  
...
```mermaid
flowchart LR
    REPORT["Serialized Coupon Report"]
    ...
    Retail_Line_Discounts__JumpMind__Loyalty_Promotion_Id(["Retail Line Discounts (JumpMind).Loyalty Promotion Id"]) --> REPORT
    Sum_Retail_Line_Discounts__JumpMind__Modification_Total__Native_Currency__(["Sum(Retail Line Discounts (JumpMind).Modification Total (Native Currency))"]) --> REPORT
    ...
    Retail_Lines__JumpMind__Transaction_Key(["Retail Lines (JumpMind).Transaction Key"]) --> REPORT
```
...
| Retail Line Discounts (JumpMind).Loyalty Promotion Id |
...
| Retail Lines (JumpMind).Transaction Key |
...
| cc69e56a98cd90093c6a | tableEx | Calendar.Actual Date, Transactions (JumpMind).Business Unit Id, Transactions (JumpMind).Till Id, Transactions (JumpMind).Username Id, Retail Line Discounts (JumpMind).Loyalty Promotion Id, Sum(Retail Line Discounts (JumpMind).Modification Total (Native Currency)), Retail Line Discounts (JumpMind).ISO Currency Code, Sum(Retail Lines (JumpMind).Total Tender Amount (Native Currency)), Retail Lines (JumpMind).ISO Currency Code, Transactions (JumpMind).Transaction Key, Retail Lines (JumpMind).Transaction Key |
