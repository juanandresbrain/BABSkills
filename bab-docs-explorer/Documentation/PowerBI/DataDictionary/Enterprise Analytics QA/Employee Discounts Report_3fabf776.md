...
**Report ID:** 3fabf776-7984-4b9a-a7c3-132002f77a92  
**Dataset ID:** 4a998365-013a-45f6-8e8d-7d5b7881b943  
**Web URL:** https://app.powerbi.com/groups/00856575-de9c-435b-ad8d-2ac43b338e3d/reports/3fabf776-7984-4b9a-a7c3-132002f77a92  
...
```mermaid
flowchart LR
    REPORT["Employee Discounts Report"]
    ...
    Retail_Transactions__JumpMind__Employee_Id_for_Discount(["Retail Transactions (JumpMind).Employee Id for Discount"]) --> REPORT
    Retail_Transactions__JumpMind__Employee_Name_for_Discount(["Retail Transactions (JumpMind).Employee Name for Discount"]) --> REPORT
    Sum_Retail_Transactions__JumpMind__Discount_Total_(["Sum(Retail Transactions (JumpMind).Discount Total)"]) --> REPORT
...
| Retail Transactions (JumpMind).Employee Id for Discount |
| Retail Transactions (JumpMind).Employee Name for Discount |
| Sum(Retail Transactions (JumpMind).Discount Total) |
...
| 561beea1bdc77c6ab964 | tableEx | Calendar.Actual Date, Transactions (JumpMind).Business Unit Id, Transactions (JumpMind).Username Id, Retail Transactions (JumpMind).Employee Id for Discount, Retail Transactions (JumpMind).Employee Name for Discount, Sum(Retail Transactions (JumpMind).Discount Total), Retail Transactions (JumpMind).Loyalty Card Number, Retail Transactions (JumpMind).ISO Currency Code, Retail Line Discounts (JumpMind).Promotion Type, Transactions (JumpMind).Transaction Key |
