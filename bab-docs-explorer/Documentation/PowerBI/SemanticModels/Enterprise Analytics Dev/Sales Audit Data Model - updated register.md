# Sales Audit Data Model - updated register

**Workspace:** Enterprise Analytics Dev  
**Dataset ID:** 00583f56-7b56-400f-9364-a2f60cc9e65d  
...
CASE WHEN EmployeeID IS NOT NULL THEN 'Employee Discount' ELSE 'Transaction' END                                    AS promotion_type,
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_tx AS (
  SELECT
      CONVERT(varchar(128),
        CONCAT(
          st.device_id, '-',
          st.business_date, '-',
          CONVERT(varchar(50), st.sequence_number)
        )
      ) AS TransactionKey,
      st.device_id,
      st.business_date,
      st.sequence_number,
      st.total,
      st.pre_tender_balance_due,
      st.subtotal,
      st.tax_total,
      st.tax_total_for_display,
      st.discount_total,
      st.customer_id,
      st.selling_channel_code,
      st.loyalty_card_number,
      st.tax_exempt_customer_id,
      st.tax_exempt_certificate,
      st.tax_exempt_code,
      st.employee_id_for_discount,
      st.iso_currency_code,
      st.line_item_count,
      st.age_restricted_date_of_birth,
      st.item_count,
      st.customer_name,
      st.tender_type_codes,
      st.voidable_flag,
      st.tax_geo_code_origin,
      st.rcpt_rtn_total,
      st.non_rcpt_rtn_total,
      st.customer_entry_method_code,
      st.cust_other_id,
      st.rcpt_rtn_count,
      st.non_rcpt_rtn_count,
      st.ring_elapsed_time_in_secs,
      st.tender_elapsed_time_in_secs,
      st.idle_elapsed_time_in_secs,
      st.lock_elapsed_time_in_secs,
      st.entry_mode_code,
      st.suspended_reason_code,
      st.suspended_note,
      st.order_id,
      st.loyalty_points_earned,
      st.customer_callout,
      st.fiscal_control_number,
      st.gift_receipt_print_type,
      st.fiscal_processor_code,
      st.create_time,
      st.create_by,
      st.last_update_time,
      st.last_update_by,
      st.party_id,
      st.employee_name_for_discount,
      st.event_id,
      st.event_invoice,
      st.extended_subtotal,
      st.parent_order_id,
      st.additional_attributes,
      'POS' as Source
  FROM dbo.jumpmind_sls_retail_trans AS st
  WHERE st.business_date >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS
),
hs AS (
  SELECT
      ...
      COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS TransDateDT,
      CONVERT(varchar(64), r.OrderNumber) AS OrderNumber,
      CONVERT(varchar(64), r.OrderID)     AS OrderID,
      r.Custom3 AS LoyaltyNumber,
      r.Custom3 AS EmployeeID,
      TRY_CONVERT(decimal(17,2), r.Discount) AS Discount,
      TRY_CONVERT(decimal(17,2), r.SubTotal) AS SubTotal_Dec,
      TRY_CONVERT(decimal(17,2), r.Tax)      AS Tax_Dec,
      TRY_CONVERT(decimal(17,2), r.Total)    AS Total_Dec,
      CONVERT(varchar(8000), r.OrderSource)  AS OrderSource,
      COALESCE(r.UpdateDate, r.OrderDateUTC, r.DateCreatedUTC) AS LastUpdateTime,
      r.ExportCreatedUTC AS ExportCreatedUTC
  FROM dbo.mulesoft_deckjsonraw_root r
  LEFT JOIN dbo.mulesoft_dynamicstargettrans dtt
    ON CONVERT(varchar(64), dtt.OrderId) = CONVERT(varchar(64), r.OrderID)
  WHERE CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS date)
        >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS
),
hs_norm AS (
  SELECT
    CASE
...
oms_tx AS (
  SELECT
      CONVERT(varchar(128),
  CONCAT(
    hn.InventLocationIdMapped, '-','052','-',
    CONVERT(varchar(8), CAST(hn.TransDateDT AS date), 112),
    '-',
    COALESCE(
      CONVERT(varchar(50), TRY_CONVERT(bigint, hn.OrderID)),  -- sequence_number
      CONVERT(varchar(64), hn.OrderID)                        -- fallback if not numeric
    )
  )
) AS TransactionKey,
      CONVERT(varchar(8000), CONCAT(hn.InventLocationIdMapped, '-', '052')) AS device_id,
      CONVERT(varchar(10), CAST(hn.TransDateDT AS date), 120)         AS business_date,
      TRY_CONVERT(bigint, hn.OrderID)                                 AS sequence_number,
      hn.Total_Dec                                                     AS total,
      CAST(NULL AS decimal(17,2))                                      AS pre_tender_balance_due,
      hn.SubTotal_Dec                                                  AS subtotal,          -- << fixed casing
      hn.Tax_Dec                                                       AS tax_total,
      hn.Tax_Dec                                                       AS tax_total_for_display,
      CAST(hn.Discount AS decimal(17,2))                               AS discount_total,
      CAST(NULL AS varchar(8000))                                      AS customer_id,
      hn.OrderSource                                                   AS selling_channel_code,
      CAST(hn.LoyaltyNumber AS varchar(8000))                                      AS loyalty_card_number,
      CAST(NULL AS varchar(8000))                                      AS tax_exempt_customer_id,
      CAST(NULL AS varchar(8000))                                      AS tax_exempt_certificate,
      CAST(NULL AS varchar(8000))                                      AS tax_exempt_code,
      CAST(hn.EmployeeID AS varchar(8000))                                      AS employee_id_for_discount,
      CAST(NULL AS varchar(8000))                                      AS iso_currency_code,
      CAST(NULL AS int)                                                AS line_item_count,
      CAST(NULL AS datetime2(6))                                       AS age_restricted_date_of_birth,
      CAST(NULL AS int)                                                AS item_count,
      CAST(NULL AS varchar(8000))                                      AS customer_name,
      CAST(NULL AS varchar(8000))                                      AS tender_type_codes,
      CAST(0 AS int)                                                   AS voidable_flag,
      CAST(NULL AS varchar(8000))                                      AS tax_geo_code_origin,
      CAST(NULL AS decimal(17,2))                                      AS rcpt_rtn_total,
      CAST(NULL AS decimal(17,2))                                      AS non_rcpt_rtn_total,
      CAST(NULL AS varchar(8000))                                      AS customer_entry_method_code,
      CAST(NULL AS varchar(8000))                                      AS cust_other_id,
      CAST(NULL AS int)                                                AS rcpt_rtn_count,
      CAST(NULL AS int)                                                AS non_rcpt_rtn_count,
      CAST(NULL AS int)                                                AS ring_elapsed_time_in_secs,
      CAST(NULL AS int)                                                AS tender_elapsed_time_in_secs,
      CAST(NULL AS int)                                                AS idle_elapsed_time_in_secs,
      CAST(NULL AS int)                                                AS lock_elapsed_time_in_secs,
      CAST(NULL AS varchar(8000))                                      AS entry_mode_code,
      CAST(NULL AS varchar(8000))                                      AS suspended_reason_code,
      CAST(NULL AS varchar(8000))                                      AS suspended_note,
      CONVERT(varchar(8000), hn.OrderID)                               AS order_id,
      CAST(NULL AS decimal(17,2))                                      AS loyalty_points_earned,
      CAST(NULL AS varchar(8000))                                      AS customer_callout,
      CAST(NULL AS varchar(8000))                                      AS fiscal_control_number,
      CAST(NULL AS varchar(8000))                                      AS gift_receipt_print_type,
      CAST(NULL AS varchar(8000))                                      AS fiscal_processor_code,
      CAST(hn.ExportCreatedUTC AS datetime2(6))                        AS create_time,
      CAST('WEB' AS varchar(8000))                                     AS create_by,
      CAST(hn.LastUpdateTime AS datetime2(6))                          AS last_update_time,
      CAST(NULL AS varchar(8000))                                      AS last_update_by,
      CAST(NULL AS varchar(8000))                                      AS party_id,
      CAST(NULL AS varchar(8000))                                      AS employee_name_for_discount,
      CAST(NULL AS varchar(8000))                                      AS event_id,
      CAST(NULL AS varchar(8000))                                      AS event_invoice,
      CAST(NULL AS decimal(17,2))                                      AS extended_subtotal,
      CAST(NULL AS varchar(8000))                                      AS parent_order_id,
      CAST(NULL AS varchar(8000))                                      AS additional_attributes,
      'OMS'                                                            AS Source
  FROM hs_norm hn
)
SELECT *
...
UNION ALL
SELECT *
FROM oms_tx", CreateNavigationProperties=false]),
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_raw AS (#(lf)  SELECT *#(lf)  FROM dbo.jumpmind_sls_card_line_item#(lf)  WHERE TRY_CONVERT(date, business_date, 112) >= DATEADD(month, -2, CAST(GETDATE() AS date))#(lf)),#(lf)pos_shaped AS (#(lf)  SELECT#(lf)      device_id AS device_id,#(lf)      CONVERT(varchar(8), TRY_CONVERT(date, business_date, 112), 112) AS business_date,#(lf)      sequence_number AS sequence_number,#(lf)      line_sequence_number AS line_sequence_number,#(lf)      CAST(brand AS varchar(200)) AS brand,#(lf)      CAST(card_name AS varchar(200)) AS card_name,#(lf)      CAST(code AS varchar(200)) AS code,#(lf)      CAST(type_code AS varchar(200)) AS type_code,#(lf)      CAST(payment_provider_code AS varchar(200)) AS payment_provider_code,#(lf)      CAST(masked_card_number AS varchar(64)) AS masked_card_number,#(lf)      CAST(entry_mode AS varchar(64)) AS entry_mode,#(lf)      CAST(service_code AS varchar(64)) AS service_code,#(lf)      CAST(expiration_date AS varchar(10)) AS expiration_date,#(lf)      ref_line_sequence_number AS ref_line_sequence_number,#(lf)      CAST(card_number AS varchar(64)) AS card_number,#(lf)      create_time AS create_time,#(lf)      create_by AS create_by,#(lf)      last_update_time AS last_update_time,#(lf)      last_update_by AS last_update_by,#(lf)      CAST(gift_card_action_code AS varchar(64)) AS gift_card_action_code,#(lf)      'POS' AS source#(lf)  FROM pos_raw#(lf)),#(lf)op AS (#(lf)  SELECT#(lf)      TRY_CONVERT(int, op._ParentKeyField) AS ParentOrderID,#(lf)      CAST(op.ID AS bigint) AS OP_ID,#(lf)      op.InsertDate AS OP_InsertDate,#(lf)      op.UpdateDate AS OP_UpdateDate,#(lf)      CAST(op.PaymentProcessor AS varchar(200)) AS OP_PaymentProcessor,#(lf)      CAST(op.PaymentSubType  AS varchar(200)) AS OP_PaymentSubType,#(lf)      CAST(op.Generic1 AS varchar(200)) AS OP_CardType,#(lf)      CAST(op.Generic2 AS varchar(32)) AS OP_Last4,#(lf)      TRY_CONVERT(int, op.ExpirationMonth) AS OP_ExpMonth,#(lf)      TRY_CONVERT(int, op.ExpirationYear) AS OP_ExpYear#(lf)  FROM dbo.mulesoft_deckjsonraw_orderpayments op#(lf)  WHERE TRY_CONVERT(int, op._ParentKeyField) IS NOT NULL#(lf)    AND TRY_CONVERT(date, COALESCE(op.UpdateDate, op.InsertDate)) >= DATEADD(month, -2, CAST(GETDATE() AS date))#(lf)),#(lf)pt AS (#(lf)  SELECT#(lf)      CAST(pt.OrderPaymentId AS bigint) AS PT_OrderPaymentId,#(lf)      CAST(pt.PaymentTransactionTypeId AS int) AS PT_TypeId,#(lf)      CAST(pt.Amount AS decimal(18,6)) AS PT_Amount,#(lf)      CAST(pt.IsDecline AS bit) AS PT_IsDecline,#(lf)      pt.TransactionDateUTC AS PT_TransUTC,#(lf)      pt.InsertDate AS PT_InsertDate,#(lf)      pt.UpdateDate AS PT_UpdateDate#(lf)  FROM dbo.mulesoft_deckjsonraw_paymenttransactions pt#(lf)  WHERE TRY_CONVERT(date, COALESCE(pt.UpdateDate, pt.InsertDate, pt.TransactionDateUTC)) >= DATEADD(month, -2, CAST(GETDATE() AS date))#(lf)),#(lf)root AS (#(lf)  SELECT#(lf)      r.OrderID,#(lf)      r.OrderNumber,#(lf)      r.SiteCode#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)),#(lf)op_ranked AS (#(lf)  SELECT#(lf)      o.*,#(lf)      ROW_NUMBER() OVER (#(lf)        PARTITION BY o.ParentOrderID#(lf)        ORDER BY COALESCE(p.PT_UpdateDate, p.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate) ASC, o.OP_ID#(lf)      ) AS rn#(lf)  FROM op o#(lf)  LEFT JOIN pt p#(lf)    ON p.PT_OrderPaymentId = o.OP_ID#(lf)),#(lf)oms_enriched AS (#(lf)  SELECT#(lf)      CAST((#(lf)        CASE #(lf)          WHEN rt.SiteCode = 'BAB' THEN '1013'#(lf)          WHEN rt.SiteCode = 'BABUK' THEN '2013'#(lf)          ELSE CAST(rt.SiteCode AS varchar(64))#(lf)        END#(lf)      ) + '-052' AS varchar(64)) AS device_id,#(lf)      CONVERT(varchar(8), CAST(COALESCE(p.PT_UpdateDate, p.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate) AS date), 112) AS business_date,#(lf)      CAST(rt.OrderID AS bigint) AS sequence_number,#(lf)      CAST(orx.rn AS int) AS line_sequence_number,#(lf)      CAST(o.OP_CardType AS varchar(200)) AS brand,#(lf)      CAST(NULL AS varchar(200)) AS card_name,#(lf)      COALESCE(NULLIF(o.OP_PaymentProcessor, ''), NULLIF(o.OP_PaymentSubType, ''), NULLIF(o.OP_CardType, '')) AS code,#(lf)      CASE#(lf)        WHEN UPPER(ISNULL(o.OP_PaymentSubType,'')) IN ('CASH','GIFTCARD') THEN o.OP_PaymentSubType#(lf)        ELSE 'Credit'#(lf)      END AS type_code,#(lf)      CAST(NULL AS varchar(200)) AS payment_provider_code,#(lf)      CASE WHEN NULLIF(o.OP_Last4, '') IS NOT NULL THEN CONCAT(REPLICATE('*', 12), o.OP_Last4) END AS masked_card_number,#(lf)      CAST(NULL AS varchar(64)) AS entry_mode,#(lf)      CAST(NULL AS varchar(64)) AS service_code,#(lf)      CASE#(lf)        WHEN ISNULL(o.OP_ExpMonth,0) BETWEEN 1 AND 12 AND ISNULL(o.OP_ExpYear,0) > 0#(lf)          THEN RIGHT('00'+CAST(o.OP_ExpMonth AS varchar(2)),2) + RIGHT(RIGHT('0000'+CAST(o.OP_ExpYear AS varchar(4)),4),2)#(lf)      END AS expiration_date,#(lf)      CAST(orx.rn AS int) AS ref_line_sequence_number,#(lf)      CAST(o.OP_Last4 AS varchar(64)) AS card_number,#(lf)      COALESCE(p.PT_InsertDate, o.OP_InsertDate) AS create_time,#(lf)      'sp_bab_pos_merge_webreturns' AS create_by,#(lf)      COALESCE(p.PT_UpdateDate, p.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate) AS last_update_time,#(lf)      'sp_bab_pos_merge_webreturns' AS last_update_by,#(lf)      CAST(NULL AS varchar(64)) AS gift_card_action_code,#(lf)      'OMS' AS source#(lf)  FROM op_ranked orx#(lf)  JOIN op o#(lf)    ON o.OP_ID = orx.OP_ID#(lf)  LEFT JOIN pt p#(lf)    ON p.PT_OrderPaymentId = o.OP_ID#(lf)  JOIN root rt#(lf)    ON rt.OrderID = orx.ParentOrderID#(lf)  WHERE TRY_CONVERT(date, COALESCE(p.PT_UpdateDate, p.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate)) >= DATEADD(month, -2, CAST(GETDATE() AS date))#(lf))#(lf)SELECT *#(lf)FROM pos_shaped#(lf)UNION ALL#(lf)SELECT *#(lf)FROM oms_enriched;"]),
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_base AS (#(lf)  SELECT#(lf)      CAST(device_id AS varchar(64)) AS device_id,#(lf)      CAST(business_date AS varchar(16)) AS business_date,#(lf)      CAST(sequence_number AS varchar(50)) AS sequence_number,#(lf)      CAST(barcode AS varchar(100)) AS barcode,#(lf)      CAST(begin_time AS datetime2) AS begin_time,#(lf)      CAST(business_unit_id AS varchar(32)) AS business_unit_id,#(lf)      CAST(client_offset AS varchar(32)) AS client_offset,#(lf)      CAST(create_by AS varchar(128)) AS create_by,#(lf)      CAST(create_time AS datetime2) AS create_time,#(lf)      CAST(customer_id AS varchar(64)) AS customer_id,#(lf)      CAST(customer_name AS varchar(256)) AS customer_name,#(lf)      CAST(device_type AS varchar(64)) AS device_type,#(lf)      CAST(discount_total AS decimal(18,6)) AS discount_total,#(lf)      CAST(employee_id_for_discount AS varchar(64)) AS employee_id_for_discount,#(lf)      CAST(end_time AS datetime2) AS end_time,#(lf)      CAST(iso_currency_code AS varchar(16)) AS iso_currency_code,#(lf)      CAST(item_count AS int) AS item_count,#(lf)      CAST(last_update_by AS varchar(128)) AS last_update_by,#(lf)      CAST(last_update_time AS datetime2) AS last_update_time,#(lf)      CAST(local_offset AS varchar(32)) AS local_offset,#(lf)      CAST(loyalty_card_number AS varchar(64)) AS loyalty_card_number,#(lf)      CAST(loyalty_rewards_count AS int) AS loyalty_rewards_count,#(lf)      CAST(loyalty_rewards_total AS decimal(18,6)) AS loyalty_rewards_total,#(lf)      CAST(mfr_coupons_count AS int) AS mfr_coupons_count,#(lf)      CAST(mfr_coupons_non_taxable_count AS int) AS mfr_coupons_non_taxable_count,#(lf)      CAST(mfr_coupons_non_taxable_total AS decimal(18,6)) AS mfr_coupons_non_taxable_total,#(lf)      CAST(mfr_coupons_taxable_count AS int) AS mfr_coupons_taxable_count,#(lf)      CAST(mfr_coupons_taxable_total AS decimal(18,6)) AS mfr_coupons_taxable_total,#(lf)      CAST(mfr_coupons_total AS decimal(18,6)) AS mfr_coupons_total,#(lf)      CAST(non_rcpt_rtn_count AS int) AS non_rcpt_rtn_count,#(lf)      CAST(paid_to AS varchar(64)) AS paid_to,#(lf)      CAST(pre_tender_balance_due AS decimal(18,6)) AS pre_tender_balance_due,#(lf)      CAST(rcpt_rtn_count AS int) AS rcpt_rtn_count,#(lf)      CAST(resumed_device_id AS varchar(64)) AS resumed_device_id,#(lf)      CAST(resumed_sequence_number AS varchar(50)) AS resumed_sequence_number,#(lf)      CAST(session_id AS varchar(64)) AS session_id,#(lf)      CAST(store_bank_id AS varchar(64)) AS store_bank_id,#(lf)      CAST(store_electronic_promos_count AS int) AS store_electronic_promos_count,#(lf)      CAST(store_electronic_promos_total AS decimal(18,6)) AS store_electronic_promos_total,#(lf)      CAST(store_physical_coupons_count AS int) AS store_physical_coupons_count,#(lf)      CAST(store_physical_coupons_total AS decimal(18,6)) AS store_physical_coupons_total,#(lf)      CAST(store_promos_count AS int) AS store_promos_count,#(lf)      CAST(store_promos_non_taxable_count AS int) AS store_promos_non_taxable_count,#(lf)      CAST(store_promos_non_taxable_total AS decimal(18,6)) AS store_promos_non_taxable_total,#(lf)      CAST(store_promos_taxable_count AS int) AS store_promos_taxable_count,#(lf)      CAST(store_promos_taxable_total AS decimal(18,6)) AS store_promos_taxable_total,#(lf)      CAST(store_promos_total AS decimal(18,6)) AS store_promos_total,#(lf)      CAST(suspended_device_id AS varchar(64)) AS suspended_device_id,#(lf)      CAST(suspended_sequence_number AS varchar(50)) AS suspended_sequence_number,#(lf)      CAST(tax_total AS decimal(18,6)) AS tax_total,#(lf)      CAST(tender_type_codes AS varchar(200)) AS tender_type_codes,#(lf)      CAST(tender1_amount AS decimal(18,6)) AS tender1_amount,#(lf)      CAST(tender1_auth_code AS varchar(64)) AS tender1_auth_code,#(lf)      CAST(tender1_card_type_code AS varchar(64)) AS tender1_card_type_code,#(lf)      CAST(tender1_masked_card_number AS varchar(64)) AS tender1_masked_card_number,#(lf)      CAST(tender1_type_code AS varchar(64)) AS tender1_type_code,#(lf)      CAST(tender2_amount AS decimal(18,6)) AS tender2_amount,#(lf)      CAST(tender2_auth_code AS varchar(64)) AS tender2_auth_code,#(lf)      CAST(tender2_card_type_code AS varchar(64)) AS tender2_card_type_code,#(lf)      CAST(tender2_masked_card_number AS varchar(64)) AS tender2_masked_card_number,#(lf)      CAST(tender2_type_code AS varchar(64)) AS tender2_type_code,#(lf)      CAST(tender3_amount AS decimal(18,6)) AS tender3_amount,#(lf)      CAST(tender3_auth_code AS varchar(64)) AS tender3_auth_code,#(lf)      CAST(tender3_card_type_code AS varchar(64)) AS tender3_card_type_code,#(lf)      CAST(tender3_masked_card_number AS varchar(64)) AS tender3_masked_card_number,#(lf)      CAST(tender3_type_code AS varchar(64)) AS tender3_type_code,#(lf)      CAST(tender4_amount AS decimal(18,6)) AS tender4_amount,#(lf)      CAST(tender4_auth_code AS varchar(64)) AS tender4_auth_code,#(lf)      CAST(tender4_card_type_code AS varchar(64)) AS tender4_card_type_code,#(lf)      CAST(tender4_masked_card_number AS varchar(64)) AS tender4_masked_card_number,#(lf)      CAST(tender4_type_code AS varchar(64)) AS tender4_type_code,#(lf)      CAST(tender5_amount AS decimal(18,6)) AS tender5_amount,#(lf)      CAST(tender5_auth_code AS varchar(64)) AS tender5_auth_code,#(lf)      CAST(tender5_card_type_code AS varchar(64)) AS tender5_card_type_code,#(lf)      CAST(tender5_masked_card_number AS varchar(64)) AS tender5_masked_card_number,#(lf)      CAST(tender5_type_code AS varchar(64)) AS tender5_type_code,#(lf)      CAST(till_id AS varchar(64)) AS till_id,#(lf)      CAST(total AS decimal(18,6)) AS total,#(lf)      CAST(total_physical_coupons_count AS int) AS total_physical_coupons_count,#(lf)      CAST(training_mode AS bit) AS training_mode,#(lf)      CAST(trans_status_code AS varchar(64)) AS trans_status_code,#(lf)      CAST(transaction_duration_in_sec AS int) AS transaction_duration_in_sec,#(lf)      CAST(username AS varchar(64)) AS username,#(lf)      CAST(voidable_flag AS bit) AS voidable_flag,#(lf)      CAST(voided_sequence_number AS varchar(50)) AS voided_sequence_number,#(lf)      CAST(trans_type_code AS varchar(64)) AS trans_type_code,#(lf)      CAST(reason_code AS varchar(64)) AS reason_code,#(lf)      CAST('POS' AS varchar(8)) AS source#(lf)  FROM dbo.jumpmind_sls_trans_summary s#(lf)  WHERE TRY_CONVERT(date, s.business_date) >= DATEADD(MONTH, -2, CAST(GETDATE() AS date))#(lf)),#(lf)oms_root AS (#(lf)  SELECT#(lf)      r.OrderID,#(lf)      r.OrderNumber,#(lf)      r.SiteCode,#(lf)      COALESCE(#(lf)        TRY_CONVERT(datetime2(7), r.OrderDateUTC),#(lf)        TRY_CONVERT(datetime2(7), r.DateCreatedUTC),#(lf)        TRY_CONVERT(datetime2(7), r.UpdateDate)#(lf)      ) AS biz_dt_raw,#(lf)      COALESCE(#(lf)        TRY_CONVERT(datetime2(7), r.UpdateDate),#(lf)        TRY_CONVERT(datetime2(7), r.LastUpdateUTC),#(lf)        TRY_CONVERT(datetime2(7), r.OrderDateUTC),#(lf)        TRY_CONVERT(datetime2(7), r.DateCreatedUTC)#(lf)      ) AS last_upd_raw#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)  WHERE TRY_CONVERT(date, COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.UpdateDate)) >= DATEADD(MONTH, -2, CAST(GETDATE() AS date))#(lf)),#(lf)oms_header AS (#(lf)  SELECT#(lf)      rf.OrderID,#(lf)      rf.OrderNumber,#(lf)      rf.SiteCode,#(lf)      CAST(CAST(rf.biz_dt_raw AS date) AS date) AS business_date,#(lf)      (#(lf)        CASE #(lf)          WHEN rf.SiteCode = 'BAB'   THEN '1013'#(lf)          WHEN rf.SiteCode = 'BABUK' THEN '2013'#(lf)          ELSE CAST(rf.SiteCode AS varchar(64))#(lf)        END#(lf)      ) + '-052' AS device_id,#(lf)      CASE WHEN rf.SiteCode = 'BABUK' THEN 'GBP'#(lf)           WHEN rf.SiteCode = 'BAB'   THEN 'USD'#(lf)           ELSE '*' END AS iso_currency_code,#(lf)      rf.last_upd_raw AS last_update_time#(lf)  FROM oms_root rf#(lf)),#(lf)oms_payments AS (#(lf)  SELECT#(lf)      TRY_CONVERT(int, pt._ParentKeyField) AS OrderID,#(lf)      pt.Amount AS tender1_amount,#(lf)      pt.Generic1 AS tender1_auth_code,#(lf)      pt.PaymentTransactionTypeId AS tender1_type_code,#(lf)      CASE WHEN NULLIF(pt.Generic1, '') IS NOT NULL THEN '****' + RIGHT(pt.Generic1, 4) END AS tender1_masked_card_number#(lf)  FROM dbo.mulesoft_deckjsonraw_paymenttransactions pt#(lf)  WHERE TRY_CONVERT(date, pt.TransactionDateUTC) >= DATEADD(MONTH, -2, CAST(GETDATE() AS date))#(lf)    AND TRY_CONVERT(int, pt._ParentKeyField) IS NOT NULL#(lf)),#(lf)oms_base AS (#(lf)  SELECT#(lf)      CAST(oh.device_id AS varchar(64)) AS device_id,#(lf)      CAST(FORMAT(oh.business_date, 'yyyyMMdd') AS varchar(16)) AS business_date,#(lf)      CAST(oh.OrderNumber AS varchar(50)) AS sequence_number,#(lf)      CAST(NULL AS varchar(100)) AS barcode,#(lf)      CAST(NULL AS datetime2) AS begin_time,#(lf)      CAST(NULL AS varchar(32)) AS business_unit_id,#(lf)      CAST(NULL AS varchar(32)) AS client_offset,#(lf)      CAST(NULL AS varchar(128)) AS create_by,#(lf)      CAST(NULL AS datetime2) AS create_time,#(lf)      CAST(NULL AS varchar(64)) AS customer_id,#(lf)      CAST(NULL AS varchar(256)) AS customer_name,#(lf)      CAST(NULL AS varchar(64)) AS device_type,#(lf)      CAST(NULL AS decimal(18,6)) AS discount_total,#(lf)      CAST(NULL AS varchar(64)) AS employee_id_for_discount,#(lf)      CAST(NULL AS datetime2) AS end_time,#(lf)      CAST(oh.iso_currency_code AS varchar(16)) AS iso_currency_code,#(lf)      CAST(NULL AS int) AS item_count,#(lf)      CAST(NULL AS varchar(128)) AS last_update_by,#(lf)      CAST(oh.last_update_time AS datetime2) AS last_update_time,#(lf)      CAST(NULL AS varchar(32)) AS local_offset,#(lf)      CAST(NULL AS varchar(64)) AS loyalty_card_number,#(lf)      CAST(NULL AS int) AS loyalty_rewards_count,#(lf)      CAST(NULL AS decimal(18,6)) AS loyalty_rewards_total,#(lf)      CAST(NULL AS int) AS mfr_coupons_count,#(lf)      CAST(NULL AS int) AS mfr_coupons_non_taxable_count,#(lf)      CAST(NULL AS decimal(18,6)) AS mfr_coupons_non_taxable_total,#(lf)      CAST(NULL AS int) AS mfr_coupons_taxable_count,#(lf)      CAST(NULL AS decimal(18,6)) AS mfr_coupons_taxable_total,#(lf)      CAST(NULL AS decimal(18,6)) AS mfr_coupons_total,#(lf)      CAST(NULL AS int) AS non_rcpt_rtn_count,#(lf)      CAST(NULL AS varchar(64)) AS paid_to,#(lf)      CAST(NULL AS decimal(18,6)) AS pre_tender_balance_due,#(lf)      CAST(NULL AS int) AS rcpt_rtn_count,#(lf)      CAST(NULL AS varchar(64)) AS resumed_device_id,#(lf)      CAST(NULL AS varchar(50)) AS resumed_sequence_number,#(lf)      CAST(NULL AS varchar(64)) AS session_id,#(lf)      CAST(NULL AS varchar(64)) AS store_bank_id,#(lf)      CAST(NULL AS int) AS store_electronic_promos_count,#(lf)      CAST(NULL AS decimal(18,6)) AS store_electronic_promos_total,#(lf)      CAST(NULL AS int) AS store_physical_coupons_count,#(lf)      CAST(NULL AS decimal(18,6)) AS store_physical_coupons_total,#(lf)      CAST(NULL AS int) AS store_promos_count,#(lf)      CAST(NULL AS int) AS store_promos_non_taxable_count,#(lf)      CAST(NULL AS decimal(18,6)) AS store_promos_non_taxable_total,#(lf)      CAST(NULL AS int) AS store_promos_taxable_count,#(lf)      CAST(NULL AS decimal(18,6)) AS store_promos_taxable_total,#(lf)      CAST(NULL AS decimal(18,6)) AS store_promos_total,#(lf)      CAST(NULL AS varchar(64)) AS suspended_device_id,#(lf)      CAST(NULL AS varchar(50)) AS suspended_sequence_number,#(lf)      CAST(NULL AS decimal(18,6)) AS tax_total,#(lf)      CAST(NULL AS varchar(200)) AS tender_type_codes,#(lf)      CAST(pt.tender1_amount AS decimal(18,6)) AS tender1_amount,#(lf)      CAST(pt.tender1_auth_code AS varchar(64)) AS tender1_auth_code,#(lf)      CAST(NULL AS varchar(64)) AS tender1_card_type_code,#(lf)      CAST(pt.tender1_masked_card_number AS varchar(64)) AS tender1_masked_card_number,#(lf)      CAST(pt.tender1_type_code AS varchar(64)) AS tender1_type_code,#(lf)      CAST(NULL AS decimal(18,6)) AS tender2_amount,#(lf)      CAST(NULL AS varchar(64)) AS tender2_auth_code,#(lf)      CAST(NULL AS varchar(64)) AS tender2_card_type_code,#(lf)      CAST(NULL AS varchar(64)) AS tender2_masked_card_number,#(lf)      CAST(NULL AS varchar(64)) AS tender2_type_code,#(lf)      CAST(NULL AS decimal(18,6)) AS tender3_amount,#(lf)      CAST(NULL AS varchar(64)) AS tender3_auth_code,#(lf)      CAST(NULL AS varchar(64)) AS tender3_card_type_code,#(lf)      CAST(NULL AS varchar(64)) AS tender3_masked_card_number,#(lf)      CAST(NULL AS varchar(64)) AS tender3_type_code,#(lf)      CAST(NULL AS decimal(18,6)) AS tender4_amount,#(lf)      CAST(NULL AS varchar(64)) AS tender4_auth_code,#(lf)      CAST(NULL AS varchar(64)) AS tender4_card_type_code,#(lf)      CAST(NULL AS varchar(64)) AS tender4_masked_card_number,#(lf)      CAST(NULL AS varchar(64)) AS tender4_type_code,#(lf)      CAST(NULL AS decimal(18,6)) AS tender5_amount,#(lf)      CAST(NULL AS varchar(64)) AS tender5_auth_code,#(lf)      CAST(NULL AS varchar(64)) AS tender5_card_type_code,#(lf)      CAST(NULL AS varchar(64)) AS tender5_masked_card_number,#(lf)      CAST(NULL AS varchar(64)) AS tender5_type_code,#(lf)      CAST(NULL AS varchar(64)) AS till_id,#(lf)      CAST(NULL AS decimal(18,6)) AS total,#(lf)      CAST(NULL AS int) AS total_physical_coupons_count,#(lf)      CAST(NULL AS bit) AS training_mode,#(lf)      CAST(NULL AS varchar(64)) AS trans_status_code,#(lf)      CAST(NULL AS int) AS transaction_duration_in_sec,#(lf)      CAST(NULL AS varchar(64)) AS username,#(lf)      CAST(NULL AS bit) AS voidable_flag,#(lf)      CAST(NULL AS varchar(50)) AS voided_sequence_number,#(lf)      CAST(NULL AS varchar(64)) AS trans_type_code,#(lf)      CAST(NULL AS varchar(64)) AS reason_code,#(lf)      CAST('OMS' AS varchar(8)) AS source#(lf)  FROM oms_header oh#(lf)  LEFT JOIN oms_payments pt ON pt.OrderID = oh.OrderID#(lf))#(lf)SELECT * FROM pos_base#(lf)UNION ALL#(lf)SELECT * FROM oms_base"]),
#"Merged Columns | Transaction Key" =
    Table.CombineColumns(
        Table.TransformColumns(
            Source,
