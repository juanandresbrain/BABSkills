# Credit Card Balancing Report

**Workspace:** Enterprise Analytics Dev  
**Dataset ID:** a20f8a14-b3be-41dd-8610-7eb680b1b033  

## Tables

| Table | Columns | Measures | Hidden |
|---|---|---|---|
| jumpmind_sls_trans | 32 | 8 |  |
...
| LocalDateTable_c4e9d20d-7863-44dd-9bf6-b5613efe3c8a | 8 | 0 | Yes |

## Measures

### jumpmind_sls_trans.Total Visa Payment Amount (Native)

```sql

CALCULATE(
    SUM([tender_amount]),
    'jumpmind_sls_trans'[brand] = "VISA",
    'jumpmind_sls_trans'[training_mode] = 0,
    'jumpmind_sls_trans'[trans_status] = "COMPLETED"
)
```

### jumpmind_sls_trans.Total MasterCard Payment Amount (Native)

```sql

CALCULATE(
    SUM([tender_amount]),
    'jumpmind_sls_trans'[brand] IN {"MASTERCARD", "MAESTRO", "MC"},
    'jumpmind_sls_trans'[training_mode] = 0,
    'jumpmind_sls_trans'[trans_status] = "COMPLETED"
)
```

### jumpmind_sls_trans.Total Visa/MasterCard (Native)

```sql
CALCULATE(
    SUM([tender_amount]),
    'jumpmind_sls_trans'[brand] IN {"MASTERCARD", "MAESTRO", "MC", "VISA"},
    'jumpmind_sls_trans'[training_mode] = 0,
    'jumpmind_sls_trans'[trans_status] = "COMPLETED"
)
```

### jumpmind_sls_trans.Total Discover (Native)

```sql
CALCULATE(
    SUM([tender_amount]),
    'jumpmind_sls_trans'[brand] = "DISCOVER",
    'jumpmind_sls_trans'[training_mode] = 0,
    'jumpmind_sls_trans'[trans_status] = "COMPLETED"
)
```

### jumpmind_sls_trans.Total American Express (Native)

```sql
CALCULATE(
    SUM([tender_amount]),
    'jumpmind_sls_trans'[brand] IN {"AMERICAN EXPRESS", "AMEX"},
    'jumpmind_sls_trans'[training_mode] = 0,
    'jumpmind_sls_trans'[trans_status] = "COMPLETED"
)
```

### jumpmind_sls_trans.Total Debit/Credit (Native)

```sql
CALCULATE(
    SUM([tender_amount]),
    'jumpmind_sls_trans'[tender_type_code] IN {"DEBIT_CARD","CREDIT_CARD"},
    'jumpmind_sls_trans'[training_mode] = 0,
    'jumpmind_sls_trans'[trans_status] = "COMPLETED"
)
```

### jumpmind_sls_trans.Total E-Wallet (Native)

```sql
CALCULATE(
    SUM([tender_amount]),
    'jumpmind_sls_trans'[tender_type_code] = "E-WALLET",
    'jumpmind_sls_trans'[training_mode] = 0,
    'jumpmind_sls_trans'[trans_status] = "COMPLETED"
)
```

### jumpmind_sls_trans.Total Electronic Payments (Native)

```sql
CALCULATE(
    SUM([tender_amount]),
    'jumpmind_sls_trans'[tender_type_code] IN {"DEBIT_CARD","CREDIT_CARD","CREDITCARD","E-WALLET","E_WALLET"},
    'jumpmind_sls_trans'[training_mode] = 0,
    'jumpmind_sls_trans'[trans_status] = "COMPLETED"
)
```
...
let
    ...
    Query="WITH date_filter AS (SELECT CAST(DATETRUNC(YEAR, DATEADD(YEAR, -1, GETDATE())) AS DATE) AS target_date)
, stores AS (
	SELECT
	CAST(CASE 
		WHEN CAST(store_id AS INT) < 1000 THEN 1000 + CAST(store_id AS INT) 
		ELSE CAST(store_id AS INT) END AS VARCHAR(4)) AS business_unit_id
	, CASE WHEN country = 'US' THEN 1100
		WHEN country = 'CA' THEN 1700
		WHEN country = 'IE' THEN 2110
		WHEN country = 'UK' THEN 2110
		ELSE NULL END AS gl_company
	, store_key
	, store_id 
	, store_name
	, bearritory 
...
inventory_locations AS (
	SELECT RetailReceiptId
	, MAX(InventLocationId) AS InventLocationId 
	FROM [LH_Source].[dbo].[mulesoft_dynamicssaleslineoms]
	GROUP BY RetailReceiptId
),
pos AS (
	SELECT 'POS' AS source_system
	, COALESCE(CAST(s.gl_company AS VARCHAR(6)), '1100') AS gl_company
	--, s.store_id as store_number
	--, s.store_name
	, COALESCE(t.business_unit_id, LEFT(t.device_id, 4)) AS business_unit_id
	, t.device_id
	, t.business_date
	, t.sequence_number
	, t.trans_status
	...
	, NULL AS eCommOrderType
	, CAST(t.create_time AS DATE) AS transaction_date
	, tl.iso_currency_code
	, tl.tender_code
	, tl.tender_type_code
	, UPPER(cl.brand) AS brand
	, tl.tender_amount
	FROM [LH_Source].dbo.jumpmind_sls_trans t
	LEFT JOIN [LH_Source].[dbo].[jumpmind_ctx_business_unit] bu ON t.business_unit_id = bu.business_unit_id
	LEFT JOIN stores s ON t.business_unit_id = s.business_unit_id
	LEFT JOIN [LH_Source].[dbo].jumpmind_sls_tender_line_item tl 
		ON t.device_id = tl.device_id 
		AND t.business_date = tl.business_date 
		AND t.sequence_number = tl.sequence_number
	LEFT JOIN [LH_Source].[dbo].jumpmind_sls_card_line_item cl
		ON t.device_id = cl.device_id
		AND t.business_date = cl.business_date
		AND t.sequence_number = cl.sequence_number
		AND tl.line_sequence_number = cl.ref_line_sequence_number
	WHERE t.create_by = 'openpos-sls'
	AND t.create_time >= (SELECT target_date FROM date_filter)
),
oms AS (
	SELECT 'OMS' as source_system
	, CAST(COALESCE(dh.Entity, CASE WHEN r.SiteCode = 'BAB' THEN 1100 WHEN r.SiteCode = 'BABUK' THEN 2110 END) AS VARCHAR(6)) AS gl_company
	, CAST(COALESCE(il.InventLocationId, CASE WHEN r.SiteCode = 'BAB' THEN 1013 WHEN r.SiteCode = 'BABUK' THEN 2013 END) AS VARCHAR(8)) AS business_unit_id
	, CAST(COALESCE(il.InventLocationId, CASE WHEN r.SiteCode = 'BAB' THEN 1013 WHEN r.SiteCode = 'BABUK' THEN 2013 END) AS VARCHAR(8)) AS device_id
	, CAST(CAST(op.DateCreated AS DATE) AS VARCHAR(10)) AS business_date
	, r._RowIndex AS sequence_number
	, 'COMPLETED' AS trans_status --this is what the code for Sales Audit data model was doing
	, 0 AS training_mode
	, r._RowIndex AS line_sequence_number
	, dh.eCommOrderType
	, CAST(op.DateCreated AS DATE) AS transaction_date
	, CASE WHEN r.SiteCode = 'BAB' THEN 'USD' WHEN r.SiteCode = 'BABUK' THEN 'GBP' END AS iso_currency_code
	, CASE
        WHEN UPPER(PaymentSubType) LIKE '%APPLEPAY%'  OR UPPER(PaymentProcessor) LIKE 'ADYEN_APPLEPAY%'  THEN 'APPLEPAY'
        WHEN UPPER(PaymentSubType) LIKE '%GOOGLEPAY%' OR UPPER(PaymentProcessor) LIKE 'ADYEN_GOOGLEPAY%' THEN 'GOOGLE PAY'
        WHEN UPPER(PaymentSubType) LIKE '%PAYPAL%'    OR UPPER(PaymentProcessor) LIKE 'ADYEN_PAYPAL%'    THEN 'PAYPAL'
        WHEN UPPER(PaymentSubType) LIKE '%GIFTCARD%'  OR UPPER(PaymentProcessor) LIKE 'ADYEN_GIFTCARD%'  THEN 'GIFTCARD'
        WHEN UPPER(PaymentSubType) LIKE '%KLARNA%'    OR UPPER(PaymentProcessor) LIKE 'ADYEN_KLARNA%'    THEN 'KLARNA'
		WHEN UPPER(PaymentSubType) = 'ADYENV2'															THEN 'ADYEN'
        WHEN UPPER(PaymentSubType) LIKE '%AFTERPAY%'  OR UPPER(PaymentProcessor) LIKE 'ADYEN_AFTERPAY%'  THEN 'AFTERPAY'
        WHEN UPPER(CardType) IN ('VISA','VISA CREDIT')             THEN 'VISA_CREDIT'
		WHEN UPPER(CardType) = 'VISA DEBIT'							THEN 'VISA_DEBIT'
        WHEN UPPER(CardType) IN ('MASTERCARD','MC','MASTERCARD CREDIT') THEN 'MASTERCARD_CREDIT'
		WHEN UPPER(CardType) = 'MASTERCARD DEBIT' THEN 'MASTERCARD_DEBIT'
        WHEN UPPER(CardType) IN ('DISCOVER','DISCOVER CREDIT','DISCOVER DEBIT') THEN 'DISCOVER'
        WHEN UPPER(CardType) IN ('AMEX','AMERICAN EXPRESS','AMERICANEXPRESS')   THEN 'AMEX'
        WHEN UPPER(PaymentProcessor) LIKE 'GLOBAL%E' OR UPPER(PaymentProcessor)='GLOBALE'                 THEN 'GLOBALE'
        WHEN UPPER(PaymentProcessor) = 'CASH'                                           THEN 'CASH_' + 
							CASE WHEN r.SiteCode = 'BAB' THEN 'USD' WHEN r.SiteCode = 'BABUK' THEN 'GBP' END
        WHEN UPPER(PaymentProcessor) <> ''                                              THEN UPPER(PaymentProcessor)
        ELSE 'OTHER'
      END AS tender_code
	, UPPER(COALESCE(op.PaymentSubType, op.PaymentProcessor, op.CardType)) AS tender_type_code
	, UPPER(op.Generic1) AS brand
	, op.AuthorizedAmount AS tender_amount
	FROM [LH_Source].[dbo].[mulesoft_deckjsonraw_root] r
	LEFT JOIN [LH_Source].[dbo].[mulesoft_deckjsonraw_orderpayments] op 
		ON r.OrderID = op._ParentKeyField
		AND r._RowIndex = op._RowIndex
	LEFT JOIN [LH_Source].[dbo].[mulesoft_dynamicsheaderoms] dh 
		ON r.OrderNumber = dh.RetailReceiptId
	LEFT JOIN inventory_locations il
		ON r.OrderNumber = il.RetailReceiptId
	WHERE r.OrderID IS NOT NULL
	AND op.DateCreated >= (SELECT target_date FROM date_filter)
),
total_company AS (
	SELECT *
	FROM pos
...
, tc.business_date
, tc.sequence_number
, tc.line_sequence_number
, tc.trans_status
, tc.training_mode
, CASE 
	WHEN tc.eCommOrderType IN ('BOPIS', 'BOSFS') THEN 52 
	ELSE NULL 
	END AS register_no
, tc.eCommOrderType
, tc.transaction_date
, tc.iso_currency_code
, tc.tender_code
, CASE
	WHEN tc.tender_type_code LIKE '%APPLEPAY%' OR tc.tender_code LIKE '%APPLEPAY%'			THEN 'E-WALLET'
	WHEN tc.tender_type_code LIKE '%GOOGLEPAY%' OR tc.tender_code LIKE '%GOOGLEPAY%'		THEN 'E-WALLET'
	WHEN tc.tender_type_code LIKE '%PAYPAL%' OR tc.tender_code LIKE '%PAYPAL%'				THEN 'E-WALLET'
	WHEN tc.tender_type_code LIKE '%GIFT%CARD%' OR tc.tender_code LIKE '%GIFT%CARD%'		THEN 'E-WALLET'
	WHEN tc.tender_type_code LIKE '%KLARNA%' OR tc.tender_code LIKE '%KLARNA%'				THEN 'E-WALLET'
	WHEN tc.tender_type_code LIKE '%AFTERPAY%' OR tc.tender_code LIKE '%AFTERPAY%'			THEN 'E-WALLET'
	WHEN tc.tender_type_code = 'GLOBALE' OR  tc.tender_code = 'GLOBALE'						THEN 'GLOBALE'
	WHEN tc.tender_type_code LIKE 'CASH%' OR tc.tender_code LIKE 'CASH%'					THEN 'CASH'
	WHEN tc.tender_type_code = 'CYBERSOURCE'												THEN 'CYBERSOURCE'
	WHEN tc.tender_type_code LIKE 'ADYEN%'													THEN 'E-WALLET'
	WHEN tc.tender_type_code IN ('VISA','MASTERCARD','MC','DISCOVER','AMEX',
								'AMERICAN EXPRESS','AMERICANEXPRESS')
		OR tc.tender_code IN ('VISA','MASTERCARD','MC','DISCOVER','AMEX',
								'AMERICAN EXPRESS','AMERICANEXPRESS')						THEN 'CREDIT_CARD'
	ELSE COALESCE(NULLIF(tc.tender_type_code,''),'OTHER')
	END AS tender_type_code
, tc.brand
, tc.tender_amount
FROM total_company tc
LEFT JOIN stores s ON tc.business_unit_id = s.business_unit_id"
]),
    #"Added Custom" = Table.AddColumn(Source, "sales_date", each [transaction_date]),
    #"Changed Type" = Table.TransformColumnTypes(#"Added Custom",{{"sales_date", type date}})
in
    #"Changed Type"
```
...
### LocalDateTable_c4e9d20d-7863-44dd-9bf6-b5613efe3c8a

```sql
Calendar(Date(Year(MIN('jumpmind_sls_trans'[sales_date])), 1, 1), Date(Year(MAX('jumpmind_sls_trans'[sales_date])), 12, 31))
```
