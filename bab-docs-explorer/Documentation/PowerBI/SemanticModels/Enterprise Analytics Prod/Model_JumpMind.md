...
**Dataset ID:** 5cac5262-3665-4304-922c-bcc860541c3c  
...
VAR Percentage = 
    ...
    RETURN
        IF(
            ISBLANK(s) || s = 0 || ISBLANK(sh) || sh = 0, 
            0, 
            IF (
                DIVIDE(sh, s, 0) > 1,
                1,
                ROUND(DIVIDE(sh, s, 0),2)
            )
        )
...
VAR SummaryTable =
    ADDCOLUMNS(
        SUMMARIZE(
            gaap_salesreport,
            gaap_salesreport[TransactionHour]
        ),
        "RowPercentage",
            VAR s = CALCULATE([Skins])
            VAR sh = CALCULATE([ShoesPerHour])
            RETURN
                IF(
                    ISBLANK(s) || s = 0 || ISBLANK(sh) || sh = 0, 
                    0,
...
VAR TotalAverage = 
    AVERAGEX(SummaryTable, [RowPercentage])

RETURN
    IF(
        ISINSCOPE(gaap_salesreport[TransactionHour]),
        Percentage,
        TotalAverage
    )
...
VAR SummaryTable =
    ADDCOLUMNS(
        ...
        "RowPercentage",
            VAR s = CALCULATE([Skins])
            VAR sh = CALCULATE([StuffersPerHour])
            RETURN
                IF(
                    ISBLANK(s) || s = 0 || ISBLANK(sh) || sh = 0, 
                    0,
