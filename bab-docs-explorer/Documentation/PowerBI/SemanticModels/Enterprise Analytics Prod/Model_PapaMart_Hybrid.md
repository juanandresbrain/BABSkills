# Model_PapaMart_Hybrid

**Workspace:** Enterprise Analytics Prod  
**Dataset ID:** fd16b1c8-15ee-468a-b6e4-dc44fd7beec9  
...
| Birthday Data Status | 3 | 4 |  |
| Email Address Status | 3 | 0 |  |
| Exchange Rates | 13 | 7 |  |
| Partial Transaction Fact | 4 | 1 |  |
| Giftcard Activated Fact | 10 | 11 |  |
| Gender | 4 | 0 |  |
| Giftcard Type | 2 | 0 |  |
| Is Gift | 4 | 0 |  |
...
| Labor Cube | 16 | 2 |  |
| Time | 15 | 0 |  |
| Transactions Cube Fanchisees Additional | 14 | 4 |  |
| Shopper Trak | 14 | 8 |  |
| Registrations | 32 | 0 |  |
| Transactions | 124 | 199 |  |
| Has Traffic | 2 | 0 |  |
| currency | 8 | 0 |  |
...
| DateTableTemplate_7706691f-73d1-4ee6-a5e8-0e27485542c3 | 8 | 0 | Yes |
| LocalDateTable_8598204c-0716-4f34-8ede-32d1aecad88c | 8 | 0 | Yes |
| LocalDateTable_b7e9b2dc-a64c-4578-916d-6ff0ec61bd54 | 8 | 0 | Yes |
| LocalDateTable_faded909-a647-419f-806f-21b3f369dbcc | 8 | 0 | Yes |
| LocalDateTable_d0d82034-ad9c-421c-b885-fc26c75ad4be | 8 | 0 | Yes |
| LocalDateTable_55d71ee0-15ed-44c2-839b-0198087b3dd1 | 8 | 0 | Yes |
...
### Exchange Rates.Actual Rate

```sql
SUM('Exchange Rates'[actual_rate])
```

### Exchange Rates.Bbw Rate
...
### Exchange Rates.Calendar Month Ave Rate

```sql
SUM('Exchange Rates'[calendar_month_ave_rate])
```

### Exchange Rates.Calendar Month End Rate

```sql
SUM('Exchange Rates'[calendar_month_end_rate])
```

### Exchange Rates.Fiscal Month Ave Rate

```sql
SUM('Exchange Rates'[fiscal_month_ave_rate])
```

### Exchange Rates.Fiscal Month End Rate

```sql
SUM('Exchange Rates'[fiscal_month_end_rate])
```

### Partial Transaction Fact.Partial Transaction Count
...
### Giftcard Activated Fact.GC Activated Amount

```sql
SUM('Giftcard Activated Fact'[activated_amount])
```

### Giftcard Activated Fact.GC Discount Amount

```sql
SUM('Giftcard Activated Fact'[discount_amount])
```

### Giftcard Activated Fact.Native GC Activated Amount

```sql
SUM('Giftcard Activated Fact'[activated_amount])
```

### Giftcard Activated Fact.Native GC Discount Amount
...
### Labor Cube.Mins Recorded

```sql
SUM('Labor Cube'[minsWorked])
```

### Transactions Cube Fanchisees Additional.Coupons And Discounts

```sql
SUM('Transactions Cube Fanchisees Additional'[Coupons_And_Discounts])/[Bbw Rate]
```

### Transactions Cube Fanchisees Additional.Franch GAAP Transactions

```sql
SUM('Transactions Cube Fanchisees Additional'[Transaction_Count])
```

### Transactions Cube Fanchisees Additional.Franch party Count

```sql
SUM('Transactions Cube Fanchisees Additional'[Party_Count])
```

### Transactions Cube Fanchisees Additional.Franch party Sales
...
```sql
CALCULATE([Animal Units], 'Is Comp LY'[isComp]=1, DATEADD('Date'[Actual Date], -52*7, DAY))
```
...
```sql
CALCULATE([Native GAAP Sales], DATEADD('Date'[Actual Date], -52*7, DAY))
```
...
```sql
CALCULATE([Native Store Sales],  DATEADD('Date'[Actual Date], -52*7, DAY))
```
...
```sql
CALCULATE([Store Transactions], DATEADD('Date'[Actual Date], -52*7, DAY))
```
...
```sql
CALCULATE([Store Transactions LY For Comp], 'Is Shopper Trak Comp LY'[isComp] =1
...
```sql
CALCULATE([GAAP Transactions], 'Is Comp LY'[isComp] = 1, DATEADD('Date'[Actual Date], -52*7, DAY))
```
...
// IF(SELECTEDVALUE(Transactions[IsComp]) =1, "Y", "N")
if([Comp Store Transactions] > 0,"Y","N" )
...
### Transactions.InStoreNativeSales

```sql
[Native Store Sales] - ([Native Curbside Sales]+[Native Pickup From Store Sales]+[Native SameDayShipt Sales]+[Native Ship From Store Sales])
```

### Transactions.InStoreTransactions

```sql
[Store Transactions] - ([Curbside Transaction Count]+[Pickup From Store Transaction Count]+[SameDayShipt Transaction Count]+[Ship From Store Transaction Count])
```
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-m2o53thjetderkgqw4nc6a676e.datawarehouse.fabric.microsoft.com", "LH_Reporting"),
    dbo_table = Source{[Schema="dbo",Item="employee_type_dim"]}[Data]
in
    dbo_table
```
...
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-m2o53thjetderkgqw4nc6a676e.datawarehouse.fabric.microsoft.com", "LH_Reporting"),
    dbo_table = Source{[Schema="dbo",Item="time_dim"]}[Data]
in
    dbo_table
```
...
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-m2o53thjetderkgqw4nc6a676e.datawarehouse.fabric.microsoft.com", "LH_Reporting"),
    dbo_table = Source{[Schema="dbo",Item="dim_store"]}[Data]
in
    dbo_table
```
...
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-m2o53thjetderkgqw4nc6a676e.datawarehouse.fabric.microsoft.com", "LH_Reporting"),
    dbo_table = Source{[Schema="dbo",Item="product_dim"]}[Data]
in
    dbo_table
```
...
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-m2o53thjetderkgqw4nc6a676e.datawarehouse.fabric.microsoft.com", "LH_Reporting"),
    dbo_table = Source{[Schema="dbo",Item="dim_date"]}[Data]
in
    dbo_table
```
...
### DateTableTemplate_7706691f-73d1-4ee6-a5e8-0e27485542c3

```sql
Calendar(Date(2015,1,1), Date(2015,1,1))
```

### LocalDateTable_8598204c-0716-4f34-8ede-32d1aecad88c

```sql
Calendar(Date(Year(MIN('Store'[closing_date])), 1, 1), Date(Year(MAX('Store'[closing_date])), 12, 31))
```

### LocalDateTable_b7e9b2dc-a64c-4578-916d-6ff0ec61bd54

```sql
Calendar(Date(Year(MIN('Store'[closing_max_comp_date])), 1, 1), Date(Year(MAX('Store'[closing_max_comp_date])), 12, 31))
```

### LocalDateTable_faded909-a647-419f-806f-21b3f369dbcc

```sql
Calendar(Date(Year(MIN('Store'[closing_max_ly_comp_date])), 1, 1), Date(Year(MAX('Store'[closing_max_ly_comp_date])), 12, 31))
```

### LocalDateTable_d0d82034-ad9c-421c-b885-fc26c75ad4be

```sql
Calendar(Date(Year(MIN('Store'[opening_date])), 1, 1), Date(Year(MAX('Store'[opening_date])), 12, 31))
```

### LocalDateTable_55d71ee0-15ed-44c2-839b-0198087b3dd1

```sql
Calendar(Date(Year(MIN('Product'[activation_date])), 1, 1), Date(Year(MAX('Product'[activation_date])), 12, 31))
```

## Shared Expressions

### DirectLake - LH_Reporting (0)

```sql
let
    Source = AzureStorage.DataLake("https://eastus-onelake.dfs.fabric.microsoft.com/ccdd9d66-24e9-48c6-a8d0-b71a2f03dff1/0e9deb9f-101d-4eff-8f95-09ca1cf6b851", [HierarchicalNavigation=true])
in
    Source
```

### DirectLake - LH_Mart (0)

```sql
let
    Source = AzureStorage.DataLake("https://eastus-onelake.dfs.fabric.microsoft.com/ccdd9d66-24e9-48c6-a8d0-b71a2f03dff1/38a14342-3e7a-4ae0-aab2-ab30265d4bad", [HierarchicalNavigation=true])
in
    Source
```
...
|---|---|---|
| 4db76rlxaxcuvmuh5kw37wbnqq-m2o53thjetderkgqw4nc6a676e.datawarehouse.fabric.microsoft.com | LH_Reporting | _(not found in SQL documentation)_ |
