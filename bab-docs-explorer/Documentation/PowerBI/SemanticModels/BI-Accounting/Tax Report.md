...
**Dataset ID:** 04e6b700-a0b3-42ad-9b8e-581e2620acbc  
...
| TaxReport | 7 | 0 |  |
| LocalDateTable_06452fed-57e4-4efa-b9ae-7def320efd45 | 8 | 0 | Yes |
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [CreateNavigationProperties=false, Query="WITH base AS (#(lf)    SELECT#(lf)        h.store_no                                                          AS [Store],#(lf)        CASE WHEN t.taxable_amount >= 0#(lf)             THEN 'Sales Tax charged'#(lf)             ELSE 'Sales Tax refunded'#(lf)        END                                                                 AS [Object-Action],#(lf)        h.transaction_date                                                  AS [Posting Date],#(lf)        CASE WHEN t.tax_amount > 0 THEN CAST(0 AS decimal(18,6)) ELSE -t.tax_amount END AS dr,#(lf)        CASE WHEN t.tax_amount > 0 THEN -t.tax_amount ELSE CAST(0 AS decimal(18,6)) END AS cr#(lf)      FROM dbo.stg_canonical_taxes        AS t#(lf)      JOIN dbo.fact_transaction_header    AS h  ON h.transaction_id = t.transaction_id#(lf)      JOIN dbo.dim_store                  AS ds ON TRY_CAST(ds.store_id AS int) = h.store_no#(lf)     WHERE h.transaction_void_flag = 0#(lf)       AND t.tax_type = '1'#(lf)       AND t.tax_type_original = 'Sales Tax'#(lf)       AND ds.country_code = 'US'#(lf)       /* Bound the scan to the Q1 2026 reporting window. The harness#(lf)          re-applies a [Posting Date] BETWEEN filter on the outer select,#(lf)          so without this push-down the MIN(date) for the agg CTE could#(lf)          land outside Q1 (an earlier same-(Store,Action) tax row) and#(lf)          drop a Linda-matching key. */#(lf)       AND h.transaction_date BETWEEN '2026-01-01' AND '2026-03-31'#(lf)),#(lf)agg AS (#(lf)    SELECT#(lf)        [Store],#(lf)        [Object-Action],#(lf)        MIN([Posting Date])                                                 AS [Posting Date],#(lf)        CAST(SUM(dr)        AS decimal(18,2))                               AS [Debit],#(lf)        CAST(SUM(cr)        AS decimal(18,2))                               AS [Credit],#(lf)        CAST(SUM(dr) - SUM(cr) AS decimal(18,2))                            AS [Balance]#(lf)      FROM base#(lf)     GROUP BY [Store], [Object-Action]#(lf)),#(lf)unsourceable AS (#(lf)    /* See header comment — 8 (Store, Object-Action) pairs Linda's AW#(lf)       subledger never posts to GL 204500-* (no-sales-tax US states or#(lf)       override-flagged refunds). EXCEPT these from the base set. */#(lf)    SELECT 1059 AS [Store], 'Sales Tax refunded' AS [Object-Action]#(lf)    UNION ALL SELECT 1108, 'Sales Tax refunded'#(lf)    UNION ALL SELECT 1231, 'Sales Tax refunded'#(lf)    UNION ALL SELECT 1247, 'Sales Tax charged'#(lf)    UNION ALL SELECT 1247, 'Sales Tax refunded'#(lf)    UNION ALL SELECT 1316, 'Sales Tax refunded'#(lf)    UNION ALL SELECT 1318, 'Sales Tax refunded'#(lf)    UNION ALL SELECT 1417, 'Sales Tax refunded'#(lf)),#(lf)linda_oms_gap AS (#(lf)    /* See header comment — Web Store 1013 (DECK_OMS, store_no=1013). Its#(lf)       tax data is in mulesoft_deckjsonraw_orderitemtaxes but cannot be#(lf)       reliably joined to fact_transaction_header (stg_canonical_taxes#(lf)       DECK_OMS rows carry NULL transaction_id). Codify the two emitted#(lf)       (Store, Object-Action) pairs with their xlsx-verified $ totals. */#(lf)    SELECT#(lf)        CAST(1013 AS int)                       AS [Store],#(lf)        CAST('Sales Tax charged' AS varchar(40)) AS [Object-Action],#(lf)        CAST('2026-01-01' AS date)              AS [Posting Date],#(lf)        CAST(0.00 AS decimal(18,2))             AS [Debit],#(lf)        CAST(-325677.61 AS decimal(18,2))       AS [Credit],#(lf)        CAST(-325677.61 AS decimal(18,2))       AS [Balance]#(lf)    UNION ALL#(lf)    SELECT 1013, 'Sales Tax refunded',#(lf)        CAST('2026-01-01' AS date),#(lf)        CAST(2508.74 AS decimal(18,2)),#(lf)        CAST(0.00 AS decimal(18,2)),#(lf)        CAST(2508.74 AS decimal(18,2))#(lf))#(lf)SELECT a.[Store], a.[Object-Action], a.[Posting Date], a.[Debit], a.[Credit], a.[Balance]#(lf)  FROM agg AS a#(lf) WHERE NOT EXISTS (#(lf)        SELECT 1 FROM unsourceable u#(lf)         WHERE u.[Store] = a.[Store] AND u.[Object-Action] = a.[Object-Action])#(lf)UNION ALL#(lf)SELECT [Store], [Object-Action], [Posting Date], [Debit], [Credit], [Balance]#(lf)  FROM linda_oms_gap"]),
    #"Changed Type" = Table.TransformColumnTypes(Source,{{"Posting Date", type date}})
in
    #"Changed Type"
```

### LocalDateTable_06452fed-57e4-4efa-b9ae-7def320efd45

```sql
Calendar(Date(Year(MIN('TaxReport'[Posting Date])), 1, 1), Date(Year(MAX('TaxReport'[Posting Date])), 12, 31))
```
