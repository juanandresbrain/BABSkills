...
**Dataset ID:** e0667f76-3a80-4a27-9128-fb9999bc3ef6  
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH #(lf)-- 1. Prepare Transaction Header Data#(lf)Transactions_Base AS (#(lf)    SELECT #(lf)        CONCAT(device_id, '-', business_date, '-', sequence_number) AS Transaction_Key,#(lf)        trans_status,#(lf)        training_mode,#(lf)        business_unit_id,#(lf)        TRY_CONVERT(date, business_date, 112) AS Business_Date_Clean#(lf)    FROM dbo.jumpmind_sls_trans#(lf)    WHERE create_by = 'openpos-sls'#(lf)      AND TRY_CONVERT(date, business_date, 112) >= DATEADD(month, -4, CAST(GETDATE() AS date))#(lf)),#(lf)#(lf)-- 2. Prepare Settlement Line Data#(lf)Settlement_Lines AS (#(lf)    SELECT #(lf)        CONCAT(device_id, '-', business_date, '-', sequence_number) AS Transaction_Key,#(lf)        store_bank_id,#(lf)        till_id,#(lf)        open_session_amount,#(lf)        close_session_amount,#(lf)        reason_code,#(lf)        last_update_time,#(lf)        ISNULL(voided, 0) AS is_voided#(lf)    FROM dbo.jumpmind_sls_tender_settlement_line_itm#(lf)    WHERE TRY_CONVERT(date, business_date, 112) >= DATEADD(month, -4, CAST(GETDATE() AS date))#(lf)),#(lf)#(lf)-- 3a. Latest Open Store Bank row per day#(lf)#(lf)Filtered_StoreBank AS (#(lf)    SELECT *#(lf)    FROM (#(lf)        SELECT#(lf)            tr.business_unit_id,#(lf)            tr.Business_Date_Clean AS Transaction_Date,#(lf)            sl.store_bank_id,#(lf)            sl.till_id,#(lf)            sl.open_session_amount AS Bank_Amount,#(lf)            sl.last_update_time,#(lf)            sl.Transaction_Key,#(lf)            ROW_NUMBER() OVER (#(lf)                PARTITION BY#(lf)                    sl.store_bank_id,#(lf)                    tr.Business_Date_Clean#(lf)                ORDER BY#(lf)                    sl.last_update_time ASC#(lf)            ) AS rn#(lf)        FROM Settlement_Lines sl#(lf)        INNER JOIN Transactions_Base tr#(lf)            ON sl.Transaction_Key = tr.Transaction_Key#(lf)        WHERE sl.reason_code = 'OpenStoreBank'#(lf)          AND sl.is_voided = 0#(lf)          AND tr.training_mode = 0#(lf)          AND UPPER(tr.trans_status) = 'COMPLETED'#(lf)    ) x#(lf)    WHERE rn = 1#(lf)),#(lf)#(lf)-- 3b. Latest ""Open Till"" event per Store Bank and Till#(lf)Filtered_Till AS (#(lf)    SELECT#(lf)        tr.business_unit_id,#(lf)        tr.Business_Date_Clean,#(lf)        sl.store_bank_id,#(lf)        sl.till_id,#(lf)        sl.open_session_amount,#(lf)        sl.close_session_amount,#(lf)        sl.last_update_time,#(lf)        ROW_NUMBER() OVER (#(lf)            PARTITION BY#(lf)                sl.store_bank_id,#(lf)                sl.till_id,#(lf)                tr.Business_Date_Clean#(lf)            ORDER BY#(lf)                sl.last_update_time DESC#(lf)        ) AS rn#(lf)    FROM Settlement_Lines sl#(lf)    INNER JOIN Transactions_Base tr#(lf)        ON sl.Transaction_Key = tr.Transaction_Key#(lf)    WHERE sl.reason_code = 'OpenTill'#(lf)      AND tr.training_mode = 0#(lf)      AND UPPER(tr.trans_status) = 'COMPLETED'#(lf)      AND sl.is_voided = 0#(lf)),#(lf)#(lf)-- 4b. Aggregate Till amounts per store/date#(lf)Final_Aggregation_Till AS (#(lf)    SELECT #(lf)        business_unit_id,#(lf)        Business_Date_Clean AS Transaction_Date,#(lf)        SUM(open_session_amount) AS Till_Amount ---first open_session_amount for each till per day summed#(lf)    FROM Filtered_Till#(lf)    WHERE rn = 1#(lf)    GROUP BY #(lf)        business_unit_id, #(lf)        Business_Date_Clean#(lf)),#(lf)#(lf)-- 5. Get one currency per store to avoid massive joins#(lf)Store_Currency AS (#(lf)    SELECT #(lf)        t.business_unit_id,#(lf)        MAX(rt.iso_currency_code) AS Currency#(lf)    FROM dbo.jumpmind_sls_trans t#(lf)    JOIN dbo.jumpmind_sls_retail_trans rt #(lf)        ON t.device_id = rt.device_id #(lf)       AND t.business_date = rt.business_date #(lf)       AND t.sequence_number = rt.sequence_number#(lf)    WHERE t.create_by = 'openpos-sls'#(lf)      AND TRY_CONVERT(date, t.business_date, 112) >= DATEADD(month, -4, CAST(GETDATE() AS date))#(lf)    GROUP BY t.business_unit_id#(lf)),#(lf)#(lf)-- 6. Prepare Store Dimensions#(lf)Store_Dim_Standard AS (#(lf)    SELECT *#(lf)    FROM (#(lf)        SELECT #(lf)            *,#(lf)            CASE #(lf)                WHEN LEN(CAST(store_id AS VARCHAR(20))) >= 4 #(lf)                    THEN CAST(store_id AS VARCHAR(20)) #(lf)                ELSE #(lf)                    '1' + RIGHT('000' + CAST(store_id AS VARCHAR(20)), 3) #(lf)            END AS location_number,#(lf)            ROW_NUMBER() OVER (#(lf)                PARTITION BY CASE #(lf)                    WHEN LEN(CAST(store_id AS VARCHAR(20))) >= 4 #(lf)                        THEN CAST(store_id AS VARCHAR(20)) #(lf)                    ELSE #(lf)                        '1' + RIGHT('000' + CAST(store_id AS VARCHAR(20)), 3) #(lf)                END#(lf)                ORDER BY CASE WHEN country IS NOT NULL THEN 1 ELSE 2 END#(lf)            ) as rn#(lf)        FROM LH_Mart.dbo.store_dim#(lf)    ) sub#(lf)    WHERE rn = 1#(lf))#(lf)#(lf)-- Final output: one row per OpenStoreBank event#(lf)SELECT #(lf)    TRY_CONVERT(int, fsb.business_unit_id) AS [Store Number],#(lf)    CAST(CASE #(lf)        WHEN sd.country = 'US' THEN '1100'#(lf)        WHEN sd.country = 'CA' THEN '1700'#(lf)        WHEN sd.country = 'IE' THEN '2110'#(lf)        WHEN sd.country = 'UK' THEN '2110'#(lf)        ELSE NULL #(lf)    END AS varchar(10)) AS [Legal Entity],#(lf)    fsb.Transaction_Date AS [Transaction Date],#(lf)    MAX(sc.Currency) AS [Currency],#(lf)    ISNULL(fsb.Bank_Amount, 0) AS [Safe Amount (Native)],#(lf)    ISNULL(MAX(fat.Till_Amount), 0) AS [Till Amount (Native)],#(lf)    ISNULL(fsb.Bank_Amount, 0) + ISNULL(MAX(fat.Till_Amount), 0) AS [Store Funds Amount (Native)]#(lf)FROM Filtered_StoreBank fsb#(lf)LEFT JOIN Store_Currency sc #(lf)    ON fsb.business_unit_id = sc.business_unit_id#(lf)LEFT JOIN Store_Dim_Standard sd #(lf)    ON CAST(fsb.business_unit_id AS VARCHAR(20)) = sd.location_number#(lf)LEFT JOIN Final_Aggregation_Till fat #(lf)    ON fat.business_unit_id = fsb.business_unit_id #(lf)   AND fat.Transaction_Date = fsb.Transaction_Date#(lf)GROUP BY #(lf)    fsb.business_unit_id,#(lf)    fsb.Transaction_Date,#(lf)    sd.country,#(lf)    fsb.Bank_Amount,#(lf)    fsb.last_update_time#(lf) ", CreateNavigationProperties=false]),
