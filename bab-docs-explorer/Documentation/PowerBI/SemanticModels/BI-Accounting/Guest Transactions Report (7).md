# Guest Transactions Report (7)

**Workspace:** BI-Accounting  
**Dataset ID:** 9bd4c5e3-f11e-4b27-a347-ab407f3322dd  
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH jm_transactions AS (#(lf)#(tab)select TRY_CONVERT(INT, t.business_unit_id) as store_number#(lf)#(tab), t.device_id#(lf)#(tab), t.business_date#(lf)#(tab), t.sequence_number#(lf)#(tab), bu.business_unit_name#(lf)#(tab), CAST(t.last_update_time AS DATE) AS transaction_date#(lf)#(tab), t.username as cashier_number#(lf)#(tab), CONCAT_WS(' ', u.first_name, u.last_name) AS associate_name#(lf)#(tab), concat(t.device_id, '-', t.business_date, '-', t.sequence_number) AS transaction_key#(lf)#(tab), CAST(dh.RetailTransactionId AS varchar(64)) AS transaction_id#(lf)#(tab), rt.loyalty_card_number#(lf)#(tab), NULLIF(rt.customer_name, '') AS guest_name#(lf)#(tab), c.address_1#(lf)#(tab), c.address_2#(lf)#(tab), c.address_3 AS city#(lf)#(tab), c.address_4 AS state#(lf)#(tab), c.post_code AS postal_code#(lf)#(tab)from dbo.jumpmind_sls_trans t#(lf)#(tab)left join dbo.jumpmind_sls_retail_trans rt#(lf)#(tab)#(tab)ON t.device_id = rt.device_id#(lf)#(tab)#(tab)AND t.business_date = rt.business_date#(lf)#(tab)#(tab)AND t.sequence_number = rt.sequence_number#(lf)#(tab)left join dbo.jumpmind_ctx_business_unit bu#(lf)#(tab)#(tab)ON t.business_unit_id = bu.business_unit_id#(lf)#(tab)left join dbo.jumpmind_bab_active_user u#(lf)#(tab)#(tab)ON t.business_unit_id = u.business_unit_id#(lf)#(tab)#(tab)AND t.username = u.username#(lf)#(tab)left join dbo.mulesoft_dynamicsheader dh#(lf)#(tab)#(tab)ON dh.TransactionKey = concat(t.device_id, '-', t.business_date, '-', t.sequence_number)#(lf)#(tab)left join dbo.inboundloyalty_customermasterde c #(lf)#(tab)#(tab)#(tab)ON rt.loyalty_card_number = c.customerNumber#(lf)#(tab)WHERE t.trans_status = 'COMPLETED'#(lf)#(tab)AND CAST(t.last_update_time AS DATE) >= DATEADD(MONTH, -6, GETDATE())#(lf)), jm_tender_lines as (#(lf)#(tab)SELECT device_id#(lf)#(tab), business_date#(lf)#(tab), sequence_number#(lf)#(tab), iso_currency_code AS currency#(lf)#(tab), CAST(SUM(tl.tender_amount) AS DECIMAL(18,6)) AS tender_amount_native_currency#(lf)#(tab), CAST(SUM(tl.tender_amount) AS DECIMAL(18,6)) AS tender_total_with_currency_rounding#(lf)#(tab), SUM(CASE WHEN tl.tender_type_code = 'ROUNDING' THEN tl.tender_amount ELSE 0 END) AS currency_rounding_adjustment#(lf)#(tab), CAST(SUM(tl.tender_amount) AS DECIMAL(18,6)) + SUM(CASE WHEN tl.tender_type_code = 'ROUNDING' THEN tl.tender_amount ELSE 0 END) AS tender_amount_with_currency_rounding#(lf)#(tab)FROM dbo.jumpmind_sls_tender_line_item tl #(lf)#(tab)WHERE tl.voided = 0#(lf)#(tab)AND tl.create_time >= DATEADD(MONTH, -6, GETDATE())#(lf)#(tab)GROUP BY device_id#(lf)#(tab), business_date#(lf)#(tab), sequence_number#(lf)#(tab), iso_currency_code#(lf)), order_payments AS (#(lf)#(tab)SELECT dh.InventLocationId AS store_number#(lf)#(tab), CASE WHEN dh.InventLocationId = 1013 THEN 'BABW Web Store (1013)'#(lf)#(tab)#(tab)WHEN dh.InventLocationId = 2013 THEN 'BABW Web Store (2013)'#(lf)#(tab)#(tab)ELSE NULL END AS business_unit_name#(lf)#(tab), CAST(TRY_CONVERT(DATETIME2(6), op.UpdateDate) AT TIME ZONE 'Central Standard Time' AS DATE) AS transaction_date#(lf)#(tab), '0' AS cashier_number#(lf)#(tab), 'None' AS associate_name#(lf)#(tab), CONCAT_WS('-', dh.InventLocationId, '052', CONVERT(varchar(8), CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.InsertDate) AS date), 112), r.OrderNumber)#(lf)#(tab)#(tab)AS transaction_key#(lf)#(tab), dh.RetailTransactionId AS transaction_id#(lf)#(tab), r.OrderNumber#(lf)#(tab), op.ID AS OrderPaymentID#(lf)#(tab), CASE WHEN dh.InventLocationId = 2013 THEN 'GBP' ELSE 'USD' END AS currency#(lf)#(tab), r.Custom3 AS loyalty_card_number#(lf)#(tab), TRIM(CONCAT(ISNULL(r.FirstName1, ''), ' ', ISNULL(r.LastName1, ''))) AS guest_name#(lf)#(tab), r.Address1 AS address1#(lf)#(tab), r.Address2 AS address2#(lf)#(tab), r.City AS city#(lf)#(tab), r.Province AS state#(lf)#(tab), r.PostalCode AS postal_code#(lf)#(tab), op.EarlyCaptureAmount AS currency_rounding_adjustment#(lf)#(tab), op.CreditedAmount AS credited_amount#(lf)#(tab)FROM dbo.mulesoft_deckjsonraw_root r#(lf)#(tab)LEFT JOIN dbo.mulesoft_dynamicsheaderoms dh#(lf)#(tab)#(tab)ON r.OrderNumber = dh.RetailReceiptId#(lf)#(tab)LEFT JOIN dbo.mulesoft_deckjsonraw_orderpayments op#(lf)#(tab)#(tab)ON r.OrderID = op._ParentKeyField#(lf)#(tab)#(tab)AND r._RowIndex = op._RowIndex#(lf)#(tab)WHERE op.UpdateDate >= DATEADD(MONTH, -6, GETDATE())#(lf)), payment_transactions as (#(lf)#(tab)SELECT OrderPaymentId#(lf)#(tab), CAST(SUM(CASE #(lf)#(tab)#(tab)WHEN pt.Amount IS NULL OR pt.Amount = 0 THEN NULL#(lf)#(tab)#(tab)WHEN pt.PaymentTransactionTypeId IN (3,4,11) THEN -ABS(pt.Amount)#(lf)#(tab)#(tab)WHEN pt.PaymentTransactionTypeId IN (1,2,10,14) THEN ABS(pt.Amount)#(lf)#(tab)#(tab)ELSE pt.Amount END) AS DECIMAL(18,6)) AS tender_amount#(lf)#(tab)FROM  dbo.mulesoft_deckjsonraw_paymenttransactions pt#(lf)#(tab)WHERE pt.UpdateDate >= DATEADD(MONTH, -6, GETDATE())#(lf)#(tab)GROUP BY OrderPaymentId#(lf)), all_transactions AS (#(lf)SELECT t.store_number#(lf), t.business_unit_name#(lf), t.transaction_date#(lf), t.cashier_number#(lf), t.associate_name#(lf), t.transaction_key#(lf), t.transaction_id#(lf), loyalty_card_number#(lf), guest_name#(lf), address_1#(lf), address_2#(lf), city#(lf), state#(lf), postal_code#(lf), tl.currency#(lf), tl.tender_amount_native_currency#(lf), tl.currency_rounding_adjustment#(lf), tl.tender_amount_with_currency_rounding#(lf), tl.tender_total_with_currency_rounding#(lf)FROM jm_transactions t#(lf)LEFT JOIN jm_tender_lines tl#(lf)#(tab)ON t.device_id = tl.device_id#(lf)#(tab)AND t.business_date = tl.business_date#(lf)#(tab)AND t.sequence_number = tl.sequence_number#(lf)UNION#(lf)SELECT op.store_number#(lf), op.business_unit_name#(lf), op.transaction_date#(lf), op.cashier_number#(lf), op.associate_name#(lf), op.transaction_key#(lf), op.transaction_id#(lf), loyalty_card_number#(lf), guest_name#(lf), address1#(lf), address2#(lf), city#(lf), state#(lf), postal_code#(lf), op.currency#(lf), pt.tender_amount - op.credited_amount AS tender_amount_native_currency#(lf), op.currency_rounding_adjustment#(lf), pt.tender_amount AS tender_amount_with_currency_rounding#(lf), pt.tender_amount AS tender_total_with_currency_rounding#(lf)FROM order_payments op#(lf)LEFT JOIN payment_transactions pt#(lf)#(tab)ON op.OrderPaymentID = pt.OrderPaymentId#(lf))#(lf)SELECT *#(lf)FROM all_transactions #(lf)#(lf)", CreateNavigationProperties=false])
in
    Source
```
