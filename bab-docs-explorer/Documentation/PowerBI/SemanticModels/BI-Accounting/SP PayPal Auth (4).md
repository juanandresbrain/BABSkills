...
**Dataset ID:** dc02d393-f53f-49fa-8f69-0f3c75ee9531  
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH #(lf)pt_transdate AS (#(lf)    SELECT #(lf)        CONVERT(varchar(64), pt.OrderPaymentId) AS OrderPaymentId,#(lf)        pt._ParentKeyField AS OrderID,#(lf)        MAX(CASE #(lf)            WHEN pt.PaymentTransactionTypeId = 13 #(lf)                THEN TRY_CONVERT(datetime2(6), td.TransactionDate) #(lf)        END) AS Capture_Date,#(lf)        MAX(CASE #(lf)            WHEN pt.PaymentTransactionTypeId = 11 #(lf)                THEN TRY_CONVERT(datetime2(6), td.TransactionDate) #(lf)        END) AS Refund_Date#(lf)    FROM dbo.mulesoft_deckjsonraw_paymenttransactions pt #(lf) INNER JOIN [LH_Source].[dbo].[vwDynamicsOrderTransDate_All_OrderStatusCodes] td ON pt._ParentKeyField = td.OrderID and td.Shipped  = 1#(lf)    WHERE TRY_CAST(pt.TransactionDateUTC AS date) >= DATEADD(MONTH, -4, CAST(GETDATE() AS date))#(lf)      AND pt.PaymentTransactionTypeId IN (11, 13) --AND td.OrderNumber = 'W9267852'#(lf)    GROUP BY #(lf)        pt.OrderPaymentId,#(lf)        pt._ParentKeyField#(lf)),#(lf)#(lf)pt AS (#(lf)    SELECT #(lf)        DISTINCT CONVERT(varchar(64), pt.OrderPaymentId) AS OrderPaymentId,#(lf)        pt._ParentKeyField AS OrderID,#(lf)        ptt.Capture_Date AS Capture_Date,#(lf)        ptt.Refund_Date AS Refund_Date,#(lf)        pt.Generic1 AS Generic1,#(lf)        CASE #(lf)            WHEN pt.PaymentTransactionTypeId = 11 THEN -1 * pt.Amount#(lf)            ELSE pt.Amount#(lf)        END AS gross_line_amount,#(lf)        pt.PaymentTransactionTypeId#(lf)    FROM dbo.mulesoft_deckjsonraw_paymenttransactions pt #(lf)    INNER JOIN pt_transdate ptt #(lf)        ON pt.OrderPaymentId = ptt.OrderPaymentId #(lf)        AND pt._ParentKeyField = ptt.OrderID #(lf)    WHERE pt.PaymentTransactionTypeId IN (11, 13)#(lf)),#(lf)#(lf)oms_root AS (#(lf)    SELECT#(lf)        r.OrderID,#(lf)        r.OrderNumber,#(lf)        r.OrderDateUTC,#(lf)        pt.Capture_Date,#(lf)        pt.Refund_Date,#(lf)        r.DateCreatedUTC,#(lf)        r.InsertDate,#(lf)        CAST(op.PaymentProcessor AS varchar(30)) AS TenderTypeCode,#(lf)        CAST(COALESCE(NULLIF(pt.Generic1, ''), 'NONE') AS varchar(20)) AS reference_no,#(lf)        oi.WarehouseCode,#(lf)        CASE #(lf)            WHEN pt.PaymentTransactionTypeId = 11 THEN -1 * r.TotalGrossTotal#(lf)            ELSE r.TotalGrossTotal#(lf)        END AS Tender_Total_Transaction_Level,#(lf)        pt.PaymentTransactionTypeId,#(lf)  CASE #(lf)            WHEN pt.PaymentTransactionTypeId = 11 THEN pt.gross_line_amount#(lf)            ELSE r.TotalGrossTotal#(lf)        END AS tender_total,#(lf)        pt.gross_line_amount#(lf)    FROM dbo.mulesoft_deckjsonraw_root r#(lf)    LEFT JOIN (#(lf)        SELECT _ParentKeyField, MAX(WarehouseCode) as WarehouseCode #(lf)        FROM dbo.mulesoft_deckjsonraw_orderitems #(lf)        GROUP BY _ParentKeyField#(lf)    ) oi #(lf)        ON oi._ParentKeyField = r.OrderID#(lf)    LEFT JOIN dbo.mulesoft_deckjsonraw_orderpayments op #(lf)        ON op._ParentKeyField = r.OrderID#(lf)    INNER JOIN pt #(lf)        ON CONVERT(varchar(64), op.ID) = pt.OrderPaymentId #(lf)    WHERE TRY_CONVERT(date, COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.InsertDate)) >= DATEADD(month, -4, CAST(GETDATE() AS date))#(lf)      AND op.PaymentSubType = 'Adyen_PayPal'#(lf))#(lf)#(lf)SELECT#(lf)    sl.InventLocationId AS [Store Number],#(lf)    -- DATE CONVERSION LOGIC: 2013 to GMT, others to CST#(lf)    CAST(#(lf)        CASE #(lf)            WHEN sl.InventLocationId = '2013' THEN#(lf)                TRY_CONVERT(datetime2(6), #(lf)                    CASE #(lf)                        WHEN orm.PaymentTransactionTypeId = 11 THEN orm.Refund_Date#(lf)                        ELSE orm.Capture_Date#(lf)                    END#(lf)                ) AT TIME ZONE 'UTC' AT TIME ZONE 'GMT Standard Time'#(lf)            ELSE#(lf)                TRY_CONVERT(datetime2(6), #(lf)                    CASE #(lf)                        WHEN orm.PaymentTransactionTypeId = 11 THEN orm.Refund_Date#(lf)                        ELSE orm.Capture_Date#(lf)                    END#(lf)                ) AT TIME ZONE 'UTC' AT TIME ZONE 'Central Standard Time'#(lf)        END AS datetime2(6)#(lf)    ) AS [Transaction Date],#(lf)    CAST(sl.RetailTransactionid AS varchar(64)) AS [Transaction ID],#(lf)    COALESCE(CAST(TRY_CONVERT(BIGINT, orm.OrderNumber) AS VARCHAR(50)), orm.OrderNumber) AS [Transaction No],#(lf)    dh.ECommOrderType AS [Order Type],#(lf)    orm.tender_total AS [Tender Total],#(lf)    orm.Tender_Total_Transaction_Level AS [Tender Total Transaction Level],#(lf)    orm.reference_no AS [Reference No],#(lf)    orm.gross_line_amount AS [PayPal Total],#(lf)    orm.TenderTypeCode, #(lf)    orm.PaymentTransactionTypeId AS [Payment Transaction Type ID],#(lf)    'OMS' AS Source#(lf)FROM oms_root orm#(lf)LEFT JOIN dbo.vwDynamicsOrderTransDate_All_OrderStatusCodes dh #(lf)    ON orm.OrderNumber = dh.OrderNumber#(lf)    AND dh.Shipped = 1#(lf)LEFT JOIN (#(lf)    SELECT DISTINCT#(lf)        RetailReceiptId,#(lf)        RetailTransactionid,#(lf)        InventLocationId#(lf)    FROM dbo.mulesoft_dynamicssaleslineoms#(lf)    --GROUP BY RetailReceiptId, RetailTransactionid#(lf)) sl #(lf)    ON dh.OrderNumber = sl.RetailReceiptId AND dh.InventLocationId = sl.InventLocationId", CreateNavigationProperties=false]),
