...
**Dataset ID:** db8ecbb0-7e5b-45b3-b712-d02e850b81e3  
...
| Activated Gift Cards (JumpMind) | 17 | 0 |  |
| Measure Table | 2 | 85 |  |
...
VAR FilteredData =
    FILTER (
        ...
        'Tender Settlement Lines (JumpMind)'[Reason Code] = "Open Store Bank"
            && RELATED ( 'Transactions (JumpMind)'[Training Mode] ) = FALSE()
            && RELATED ( 'Transactions (JumpMind)'[Transaction Status] ) = "Completed"
    )
VAR WithDateOnly =
    ADDCOLUMNS (
        FilteredData,
        "@DateOnly", DATEVALUE ( 'Tender Settlement Lines (JumpMind)'[Last Updated Datetime] )
    )
VAR LatestPerDate =
    ADDCOLUMNS (
        SUMMARIZE ( WithDateOnly, [@DateOnly] ),
        "LatestDatetime",
            CALCULATE (
                MAX ( 'Tender Settlement Lines (JumpMind)'[Last Updated Datetime] ),
                FILTER ( WithDateOnly, [@DateOnly] = EARLIER ( [@DateOnly] ) )
            )
    )
VAR Result =
    SUMX (
        ...
        CALCULATE (
            ...
            FILTER (
                WithDateOnly,
                'Tender Settlement Lines (JumpMind)'[Last Updated Datetime] = [LatestDatetime]
                    && [@DateOnly] = EARLIER ( [@DateOnly] )
            )
        )
    )
RETURN
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="
WITH pos_raw AS (
#(lf)  SELECT *#(lf)  
FROM dbo.jumpmind_sls_card_line_item#(lf)  
WHERE TRY_CONVERT(date, business_date, 112) >= DATEADD(month, -2, CAST(GETDATE() AS date))  
-- 2 MONTHS#(lf))
,#(lf)pos_shaped AS (
#(lf)  SELECT#(lf)      device_id AS device_id
,#(lf)      CONVERT(varchar(8)
, TRY_CONVERT(date, business_date, 112), 112) AS business_date
,#(lf)      sequence_number AS sequence_number
,#(lf)      line_sequence_number AS line_sequence_number
,#(lf)      CAST(brand AS varchar(200)) AS brand
,#(lf)      CAST(card_name AS varchar(200)) AS card_name
,#(lf)      CAST(code AS varchar(200)) AS code
,#(lf)      CAST(type_code AS varchar(200)) AS type_code
,#(lf)      CAST(payment_provider_code AS varchar(200)) AS payment_provider_code
,#(lf)      CAST(masked_card_number AS varchar(64)) AS masked_card_number
,#(lf)      CAST(entry_mode AS varchar(64)) AS entry_mode
,#(lf)      CAST(service_code AS varchar(64)) AS service_code
,#(lf)      CAST(expiration_date AS varchar(10)) AS expiration_date
,#(lf)      ref_line_sequence_number AS ref_line_sequence_number
,#(lf)      CAST(card_number AS varchar(64)) AS card_number
,#(lf)      create_time AS create_time
,#(lf)      create_by AS create_by
,#(lf)      last_update_time AS last_update_time
,#(lf)      last_update_by AS last_update_by
,#(lf)      CAST(gift_card_action_code AS varchar(64)) AS gift_card_action_code
,#(lf)      'POS' AS source#(lf)  FROM pos_raw#(lf))
,#(lf)op AS (
#(lf)  SELECT#(lf)      TRY_CONVERT(int, op._ParentKeyField) AS ParentOrderID
,#(lf)      CAST(op.ID AS bigint)                AS OP_ID
,#(lf)      op.InsertDate                        AS OP_InsertDate
,#(lf)      op.UpdateDate                        AS OP_UpdateDate
,#(lf)      CAST(op.PaymentProcessor AS varchar(200)) AS OP_PaymentProcessor
,#(lf)      CAST(op.PaymentSubType  AS varchar(200)) AS OP_PaymentSubType
,#(lf)      CAST(op.Generic1        AS varchar(200)) AS OP_CardType
,#(lf)      CAST(op.Generic2      AS varchar(32))  AS OP_Last4
,#(lf)      TRY_CONVERT(int, op.ExpirationMonth)     AS OP_ExpMonth
,#(lf)      TRY_CONVERT(int, op.ExpirationYear)      AS OP_ExpYear#(lf)  
FROM dbo.mulesoft_deckjsonraw_orderpayments op#(lf)  
WHERE TRY_CONVERT(int, op._ParentKeyField) IS NOT NULL#(lf)    
AND TRY_CONVERT(date, COALESCE(op.UpdateDate, op.InsertDate)) >= DATEADD(month, -2, CAST(GETDATE() AS date)) 
-- 2 MONTHS#(lf))
,#(lf)pt AS (
#(lf)  SELECT#(lf)      CAST(pt.OrderPaymentId AS bigint)        AS PT_OrderPaymentId
,#(lf)      CAST(pt.PaymentTransactionTypeId AS int) AS PT_TypeId
,#(lf)      CAST(pt.Amount AS decimal(18,6))         AS PT_Amount
,#(lf)      CAST(pt.IsDecline AS bit)                AS PT_IsDecline
,#(lf)      pt.TransactionDateUTC                    AS PT_TransUTC
,#(lf)      pt.InsertDate                            AS PT_InsertDate
,#(lf)      pt.UpdateDate                            AS PT_UpdateDate#(lf)  
FROM dbo.mulesoft_deckjsonraw_paymenttransactions pt#(lf)  
WHERE TRY_CONVERT(date, COALESCE(pt.UpdateDate, pt.InsertDate, pt.TransactionDateUTC))#(lf)        >= DATEADD(month, -2, CAST(GETDATE() AS date))  
-- 2 MONTHS#(lf))
,#(lf)pt_best AS (
#(lf)  SELECT *#(lf)  
FROM (
#(lf)    SELECT#(lf)        p.*
,#(lf)        ROW_NUMBER() OVER (#(lf)          PARTITION BY p.PT_OrderPaymentId#(lf)          ORDER BY#(lf)            CASE WHEN p.PT_IsDecline = 0 AND p.PT_TypeId IN (14) THEN 1#(lf)                 WHEN p.PT_IsDecline = 0 THEN 2#(lf)                 ELSE 3 END,#(lf)            COALESCE(p.PT_UpdateDate, p.PT_InsertDate, p.PT_TransUTC) DESC#(lf)        ) AS rn#(lf)    
FROM pt p#(lf)  ) x#(lf)  
WHERE x.rn = 1#(lf))
,#(lf)root AS (#(lf)  
SELECT#(lf)      r.OrderID
,#(lf)      r.OrderNumber
,#(lf)      r.SiteCode#(lf)  
FROM dbo.mulesoft_deckjsonraw_root r#(lf))
,#(lf)op_ranked AS (
#(lf)  SELECT#(lf)      o.*
,#(lf)      ROW_NUMBER() OVER (#(lf)        PARTITION BY o.ParentOrderID#(lf)        ORDER BY COALESCE(pb.PT_UpdateDate, pb.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate) ASC, o.OP_ID#(lf)      ) AS rn#(lf)  FROM op o#(lf)  
LEFT JOIN pt_best pb#(lf)    ON pb.PT_OrderPaymentId = o.OP_ID#(lf))
,#(lf)oms_enriched AS (
#(lf)  SELECT#(lf)      CAST((#(lf)        CASE #(lf)          WHEN NULLIF(CAST(dtt.SiteWarehouseCode AS varchar(64)), '') IS NOT NULL#(lf)            THEN CAST(dtt.SiteWarehouseCode AS varchar(64))#(lf)          WHEN rt.SiteCode = 'BAB'   THEN '1013'#(lf)          WHEN rt.SiteCode = 'BABUK' THEN '2013'#(lf)          ELSE CAST(rt.SiteCode AS varchar(64))#(lf)        END#(lf)      ) + '-052' AS varchar(64)) AS device_id
,#(lf)      CONVERT(varchar(8),#(lf)              CAST(COALESCE(pb.PT_UpdateDate, pb.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate) AS date), 112) AS business_date
,#(lf)      CAST(rt.OrderID AS bigint) AS sequence_number,#(lf)      CAST(orx.rn AS int) AS line_sequence_number
,#(lf)      COALESCE(NULLIF(o.OP_CardType, ''), NULLIF(o.OP_PaymentSubType, ''), NULLIF(o.OP_PaymentProcessor, '')) AS brand
,#(lf)      CAST(NULL AS varchar(200)) AS card_name
,#(lf)      COALESCE(NULLIF(o.OP_PaymentProcessor, ''), NULLIF(o.OP_PaymentSubType, ''), NULLIF(o.OP_CardType, '')) AS code
,#(lf)      NULLIF(o.OP_CardType, '') AS type_code
,#(lf)      CAST(NULL AS varchar(200)) AS payment_provider_code
,#(lf)      CASE WHEN NULLIF(o.OP_Last4, '') IS NOT NULL THEN CONCAT(REPLICATE('*', 12), o.OP_Last4) END AS masked_card_number
,#(lf)      CAST(NULL AS varchar(64)) AS entry_mode,#(lf)      CAST(NULL AS varchar(64)) AS service_code
,#(lf)      CASE#(lf)        WHEN ISNULL(o.OP_ExpMonth,0) BETWEEN 1 AND 12 AND ISNULL(o.OP_ExpYear,0) > 0#(lf)          THEN RIGHT('00'+CAST(o.OP_ExpMonth AS varchar(2)),2) + RIGHT(RIGHT('0000'+CAST(o.OP_ExpYear AS varchar(4)),4),2)#(lf)      END AS expiration_date
,#(lf)      CAST(orx.rn AS int) AS ref_line_sequence_number
,#(lf)      CAST(o.OP_Last4 AS varchar(64)) AS card_number
,#(lf)      COALESCE(pb.PT_InsertDate, o.OP_InsertDate) AS create_time
,#(lf)      'sp_bab_pos_merge_webreturns' AS create_by
,#(lf)      COALESCE(pb.PT_UpdateDate, pb.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate) AS last_update_time
,#(lf)      'sp_bab_pos_merge_webreturns' AS last_update_by
,#(lf)      CAST(NULL AS varchar(64)) AS gift_card_action_code
,#(lf)      'OMS' AS source#(lf)  FROM op_ranked orx#(lf)  JOIN op o#(lf)    ON o.OP_ID = orx.OP_ID#(lf)  
LEFT JOIN pt_best pb#(lf)    ON pb.PT_OrderPaymentId = o.OP_ID#(lf)  
JOIN root rt#(lf)    ON rt.OrderID = orx.ParentOrderID#(lf)  
OUTER APPLY (#(lf)    
SELECT TOP (1) dtt.SiteWarehouseCode#(lf)    
FROM dbo.mulesoft_dynamicstargettrans dtt#(lf)    
WHERE dtt.OrderId = rt.OrderID#(lf)    
ORDER BY TRY_CONVERT(datetime2(7), dtt.ExportCreatedUTC) DESC#(lf)  ) dtt#(lf)  
-- Safety net: ensure the derived OMS business_date also falls in the last 2 months
#(lf)  WHERE TRY_CONVERT(date, COALESCE(pb.PT_UpdateDate, pb.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate))
#(lf)        >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf))#(lf)SELECT *#(lf)FROM pos_shaped#(lf)UNION ALL#(lf)SELECT *#(lf)FROM oms_enriched;#(lf)"]),
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_giftcards AS (#(lf)    SELECT#(lf)        CONVERT(varchar(32),  j.device_id)                                         AS device_id,#(lf)        CONVERT(varchar(8),   j.business_date)                                     AS business_date,#(lf)        CONVERT(varchar(50),  j.sequence_number)                                   AS sequence_number,#(lf)        CONVERT(varchar(50),  j.line_sequence_number)                              AS line_sequence_number,#(lf)        CONVERT(varchar(200), j.brand)                                             AS brand,#(lf)        CONVERT(varchar(200), j.card_name)                                         AS card_name,#(lf)        CONVERT(varchar(200), j.code)                                              AS code,#(lf)        CONVERT(varchar(200), j.type_code)                                         AS type_code,#(lf)        CONVERT(varchar(200), j.payment_provider_code)                             AS payment_provider_code,#(lf)        CONVERT(varchar(64),  j.masked_card_number)                                AS masked_card_number,#(lf)        CONVERT(varchar(64),  j.entry_mode)                                        AS entry_mode,#(lf)        CONVERT(varchar(64),  j.service_code)                                      AS service_code,#(lf)        CONVERT(varchar(16),  j.expiration_date)                                   AS expiration_date,#(lf)        CONVERT(varchar(50),  j.ref_line_sequence_number)                          AS ref_line_sequence_number,#(lf)        CONVERT(varchar(64),  j.card_number)                                       AS card_number,#(lf)        CONVERT(varchar(64),  j.gift_card_action_code)                             AS gift_card_action_code,#(lf)        CAST(j.last_update_time AS datetime2(6))                                   AS last_update_time,#(lf)        'POS'                                                                      AS source#(lf)    FROM dbo.jumpmind_sls_card_line_item AS j#(lf)    WHERE j.type_code = 'GIFTCARD'#(lf)      AND TRY_CONVERT(date, j.business_date, 112) >= DATEADD(month, -2, CAST(GETDATE() AS date))#(lf)),#(lf)hs AS (#(lf)    SELECT#(lf)        NULLIF(CONVERT(varchar(64), r.SiteCode), '')                               AS InventLocationId,#(lf)        CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS date) AS TransDate,#(lf)        CONVERT(varchar(64), r.OrderNumber)  AS OrderNumber,#(lf)        r.OrderID,#(lf)        r._RowIndex#(lf)    FROM dbo.mulesoft_deckjsonraw_root r#(lf)    WHERE CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS date)#(lf)          >= DATEADD(month, -2, CAST(GETDATE() AS date))#(lf)),#(lf)hs_norm AS (#(lf)  SELECT#(lf)    CASE#(lf)      WHEN hs.InventLocationId = 'BAB'   THEN '1013'#(lf)      WHEN hs.InventLocationId = 'BABUK' THEN '2013'#(lf)      WHEN NULLIF(hs.InventLocationId,'') IS NULL THEN '9999'#(lf)      ELSE hs.InventLocationId#(lf)    END AS InventLocationIdMapped,#(lf)    hs.*#(lf)  FROM hs#(lf)),#(lf)oms_giftcards AS (#(lf)    SELECT#(lf)        CONCAT(COALESCE(hn.InventLocationIdMapped,'9999'), '-052')                  AS device_id,#(lf)        CONVERT(varchar(8), hn.TransDate, 112)                                      AS business_date,#(lf)        CONVERT(varchar(50),#(lf)            COALESCE(#(lf)                TRY_CONVERT(bigint, hn.OrderNumber),#(lf)                TRY_CONVERT(bigint, hn.OrderID),#(lf)                ABS(CHECKSUM(CAST(hn.OrderNumber AS nvarchar(4000))))#(lf)            )#(lf)        )                                                                           AS sequence_number,#(lf)        CONVERT(varchar(50), g.OrderItemID)                                         AS line_sequence_number,   -- always use OrderItemID#(lf)        CAST('Build-A-Bear' AS varchar(200))                                        AS brand,#(lf)        CAST('Gift Card' AS varchar(200))                                           AS card_name,#(lf)        'GIFTCARD'                                                                  AS code,#(lf)        'GIFTCARD'                                                                  AS type_code,#(lf)        CAST(NULL AS varchar(200))                                                  AS payment_provider_code,#(lf)        CASE#(lf)          WHEN g.GiftCardNumber IS NOT NULL#(lf)            THEN REPLICATE('*', 12) + RIGHT(CONVERT(varchar(32), TRY_CONVERT(bigint, g.GiftCardNumber)), 4)#(lf)          ELSE NULL#(lf)        END                                                                         AS masked_card_number,#(lf)        CAST(NULL AS varchar(64))                                                   AS entry_mode,#(lf)        CAST(NULL AS varchar(64))                                                   AS service_code,#(lf)        CAST(NULL AS varchar(16))                                                   AS expiration_date,#(lf)        CAST(NULL AS varchar(50))                                                   AS ref_line_sequence_number,#(lf)        CONVERT(varchar(64), TRY_CONVERT(bigint, g.GiftCardNumber))                 AS card_number,#(lf)        CASE WHEN TRY_CONVERT(int, g.Processed) = 1 THEN 'Issue' ELSE NULL END      AS gift_card_action_code,#(lf)        CAST(COALESCE(TRY_CONVERT(datetime2(6), g.UpdateDate),#(lf)                      TRY_CONVERT(datetime2(6), g.InsertDate)) AS datetime2(6))     AS last_update_time,#(lf)        'OMS'                                                                      AS source#(lf)    FROM dbo.mulesoft_deckjsonraw_giftcards g#(lf)    JOIN hs_norm hn#(lf)      ON CONVERT(varchar(64), g.OrderTransactionIdentifier) = CONVERT(varchar(64), hn._RowIndex)#(lf)    WHERE hn.TransDate >= DATEADD(month, -2, CAST(GETDATE() AS date))#(lf))#(lf)SELECT * FROM pos_giftcards#(lf)UNION ALL#(lf)SELECT * FROM oms_giftcards;", CreateNavigationProperties=false]),
