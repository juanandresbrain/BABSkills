...
| Product Images | 5 | 0 |  |
| Products (PLM) | 147 | 0 |  |
| Retail Line Discounts (JumpMind) | 39 | 3 |  |
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_tx_header AS (
	SELECT
	st.device_id,
	st.business_date,
	st.sequence_number
	FROM dbo.jumpmind_sls_retail_trans st
	WHERE st.business_date >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS
),
pos_line_discounts AS (
	SELECT
	CONVERT(varchar(10), d.business_date, 120)        AS business_date,
	CASE
		WHEN TRY_CONVERT(decimal(18,6), d.mod_by_percentage) IS NOT NULL
		AND TRY_CONVERT(decimal(18,6), d.mod_by_percentage) <> 0
		THEN 'PERCENT' ELSE 'AMOUNT'
	END                                               AS calc_method,
	d.device_id,
	d.entry_method_code,
	d.iso_currency_code,
	d.last_update_time,
	d.line_sequence_number,
	d.mod_line_sequence_number,
	d.override_user_id,
	d.price_mod_source_type_code,
	d.price_mod_type_code,
	d.reason_code,
	CAST(NULL AS numeric(18,6))                       AS rounding_amount,
	d.sequence_number,
	d.username,
	ISNULL(d.voided, 0)                               AS voided,
	d.applied_coupon_item_ids,
	d.create_by,
	d.create_time,
	ISNULL(d.[description], '')                       AS [description],
	d.last_update_by,
	d.loyalty_promotion_id,
	d.mod_by_amount,
	d.mod_by_percentage,
	d.modification_total,                             -- existing
	d.modification_total AS GrossLineAmt,             -- new to match OMS GrossLineAmt
	d.modification_total AS CouponValue,              -- new to match OMS CouponValue
	d.price_mod_source_sub_type_code,
	ISNULL(d.promotion_id, '')                        AS promotion_id,
	d.promotion_reward_quantity,
	ISNULL(d.promotion_type, '')                      AS promotion_type,
	d.ref_line_sequence_number,
	d.promo_code_id,
	d.reward_base_price_type_code,
	ISNULL(d.vendor_funded_flag, 0)                   AS vendor_funded_flag,
	d.quantity_index,
	d.rtn_device_id,
	CONVERT(varchar(10), TRY_CONVERT(date, d.rtn_business_date), 120) AS rtn_business_date,
	d.rtn_sequence_number,
	ISNULL(d.returned_flag, 0)                        AS returned_flag,
	d.external_id
	FROM dbo.jumpmind_sls_retail_line_item_price_mod d
	WHERE d.business_date >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS
),
hs AS (
	SELECT
	COALESCE(
	NULLIF(CONVERT(varchar(64), dtt.MaxWarehouseCode), ''),
	NULLIF(CONVERT(varchar(64), dtt.SiteWarehouseCode), ''),
	NULLIF(CONVERT(varchar(64), r.SiteCode), '')
	)                                                AS InventLocationId,
	CAST(COALESCE(
		r.OrderDateUTC,
		r.DateCreatedUTC,
		r.OrderStatusChangeDateUTC,
		r.ExportCreatedUTC
	) AS date)                                       AS TransDate,
	CONVERT(varchar(64), r.OrderNumber)              AS Barcode,
	CONVERT(varchar(64), r.OrderID)                  AS OrderID,
	r._RowIndex
	FROM dbo.mulesoft_deckjsonraw_root r
	LEFT JOIN dbo.mulesoft_dynamicstargettrans dtt
	ON CONVERT(varchar(64), dtt.OrderId) = CONVERT(varchar(64), r.OrderID)
	WHERE CAST(COALESCE(
		r.OrderDateUTC,
		r.DateCreatedUTC,
		r.OrderStatusChangeDateUTC,
		r.ExportCreatedUTC
	) AS date) >= DATEADD(month, -2, CAST(GETDATE() AS date))           -- 2 MONTHS
),
hs_norm AS (
SELECT
CASE
	WHEN hs.InventLocationId = 'BAB'   THEN '1013'
	WHEN hs.InventLocationId = 'BABUK' THEN '2013'
	WHEN NULLIF(hs.InventLocationId,'') IS NULL THEN '9999'
	ELSE '9999'
	END                                              AS InventLocationIdMapped,
	hs.*
	FROM hs
),
deck_adjustments AS (
	SELECT
	CONVERT(varchar(64), a._ParentKeyField)          AS OrderID_key,
	a.OrderTransactionIdentifier                    AS RootRowIndex,
	a.AdjustmentDate,
	a.AdjustmentTypeValue,
	a.CouponCode                                    AS CouponCode,
	a.GrossPrice                                    AS GrossPrice,      -- GrossLineAmt for OMS
	a.NetPrice                                      AS NetPrice         -- CouponValue for OMS
	FROM dbo.mulesoft_deckjsonraw_orderadjustments a
	WHERE a.AdjustmentDate >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS
)
SELECT
'POS' AS SourceSystem,
CONCAT(
CAST(h.device_id AS varchar(64)), '-',
CONVERT(varchar(10), h.business_date, 120), '-',
CONVERT(varchar(50), h.sequence_number), '-',
CONVERT(varchar(50), d.line_sequence_number)
) AS TransactionKey,
d.*
FROM pos_line_discounts d
JOIN pos_tx_header h
ON h.device_id       = d.device_id
AND h.business_date   = CAST(d.business_date AS date)
AND h.sequence_number = d.sequence_number
UNION ALL
SELECT
'OMS' AS SourceSystem,
CONCAT(
hn.InventLocationIdMapped, '-', '052', '-', CONVERT(varchar, hn.TransDate, 112), '-', hn.Barcode
) AS TransactionKey,
CONVERT(varchar(10), hn.TransDate, 112)            AS business_date,
'AMOUNT'                                          AS calc_method,
CAST(COALESCE(hn.InventLocationIdMapped, '9999') + '-052' AS VARCHAR(8000)) AS device_id,
CAST(NULL AS varchar(64))                         AS entry_method_code,
CASE WHEN hn.InventLocationIdMapped IN ('2013') THEN 'GBP' ELSE 'USD' END AS iso_currency_code,
da.AdjustmentDate                                 AS last_update_time,
CAST(hn._RowIndex AS VARCHAR(64))				  AS line_sequence_number,
CAST(1 AS int)                                    AS mod_line_sequence_number,
CAST(NULL AS varchar(64))                         AS override_user_id,
CASE WHEN da.AdjustmentTypeValue LIKE '%Manual%' THEN 'MANUAL' ELSE NULL END AS price_mod_source_type_code,
'TRANS'                                           AS price_mod_type_code,
CAST(NULL AS varchar(64))                         AS reason_code,
CAST(NULL AS numeric(18,6))                       AS rounding_amount,
TRY_CONVERT(bigint, da.OrderID_key)               AS sequence_number,
CAST(NULL AS varchar(128))                        AS username,
CAST(0 AS int)                                    AS voided,
CAST(NULL AS varchar(4000))                       AS applied_coupon_item_ids,
'WEB'                                             AS create_by,
CAST(NULL AS datetime2(6))                        AS create_time,
ISNULL(da.AdjustmentTypeValue, '')                AS [description],
CAST(NULL AS varchar(128))                        AS last_update_by,
CAST(NULL AS varchar(64))                         AS loyalty_promotion_id,
CAST(NULL AS decimal(18,6))                       AS mod_by_amount,
CAST(NULL AS decimal(18,6))                       AS mod_by_percentage,
CAST(NULL AS decimal(18,6))                       AS modification_total,  -- existing
da.GrossPrice                                     AS GrossLineAmt,         -- new
da.NetPrice                                       AS CouponValue,          -- new
CAST(NULL AS varchar(64))                         AS price_mod_source_sub_type_code,
''                                                AS promotion_id,
CAST(NULL AS decimal(18,6))                       AS promotion_reward_quantity,
'Transaction'                                     AS promotion_type,
CAST(NULL AS int)                                 AS ref_line_sequence_number,
da.CouponCode                                     AS promo_code_id,
CAST(NULL AS varchar(64))                         AS reward_base_price_type_code,
CAST(0 AS int)                                    AS vendor_funded_flag,
CAST(NULL AS int)                                 AS quantity_index,
CAST(NULL AS varchar(64))                         AS rtn_device_id,
CAST(NULL AS varchar(10))                         AS rtn_business_date,
CAST(NULL AS bigint)                              AS rtn_sequence_number,
CAST(0 AS int)                                    AS returned_flag,
CAST(NULL AS varchar(128))                        AS external_id
FROM deck_adjustments da
JOIN hs_norm hn
ON CONVERT(varchar(64), hn.OrderID) = CONVERT(varchar(64), da.OrderID_key)
AND hn._RowIndex = da.RootRowIndex
;"]),
