# Sales Audit Data Model - Transactions Table has No Gift Cards

**Workspace:** Enterprise Analytics QA  
**Dataset ID:** 7b0816df-8899-4875-ad6d-43f1ffc0c8d1  
...
| Product Images | 5 | 0 |  |
| Products (PLM) | 146 | 0 |  |
| Retail Line Discounts (JumpMind) | 37 | 3 |  |
| Global Products (JumpMind) | 9 | 0 |  |
| Retail Lines (JumpMind) | 97 | 2 |  |
| Retail Return Lines (JumpMind) | 19 | 0 |  |
| Retail Transaction Discounts (JumpMind) | 17 | 0 |  |
| Retail Transactions (JumpMind) | 51 | 0 |  |
| Tax Authorities (JumpMind) | 7 | 0 |  |
| Tax Groups (JumpMind) | 5 | 0 |  |
| Tax Lines (JumpMind) | 34 | 0 |  |
| Tender Card Lines (JumpMind) | 14 | 0 |  |
| Tender Lines (JumpMind) | 27 | 3 |  |
| Tender Settlement Lines (JumpMind) | 27 | 4 |  |
| Tenders (JumpMind) | 10 | 0 |  |
| Transaction Summaries (JumpMind) | 59 | 0 |  |
| Users (JumpMind) | 14 | 0 |  |
| Transactions (JumpMind) | 37 | 0 |  |
| Activated Gift Cards (JumpMind) | 16 | 1 |  |
| Measure Table | 2 | 85 |  |
...
CALCULATE(
    ...
    'Retail Lines (JumpMind)'[Line Item Type] = "Web Sale",
    'Transactions (JumpMind)'[Training Mode] = FALSE()
)
```
...
### Activated Gift Cards (JumpMind).TEST
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_tx_header AS (#(lf)  SELECT#(lf)    st.device_id,#(lf)    st.business_date,#(lf)    st.sequence_number#(lf)  FROM dbo.jumpmind_sls_retail_trans st#(lf)  WHERE st.business_date >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf)),#(lf)pos_line_discounts AS (#(lf)  SELECT#(lf)    CONVERT(varchar(10), d.business_date, 120)        AS business_date,#(lf)    CASE#(lf)      WHEN TRY_CONVERT(decimal(18,6), d.mod_by_percentage) IS NOT NULL#(lf)        AND TRY_CONVERT(decimal(18,6), d.mod_by_percentage) <> 0#(lf)      THEN 'PERCENT' ELSE 'AMOUNT'#(lf)    END                                               AS calc_method,#(lf)    d.device_id,#(lf)    d.entry_method_code,#(lf)    d.iso_currency_code,#(lf)    d.last_update_time,#(lf)    d.line_sequence_number,#(lf)    d.mod_line_sequence_number,#(lf)    d.override_user_id,#(lf)    d.price_mod_source_type_code,#(lf)    d.price_mod_type_code,#(lf)    d.reason_code,#(lf)    CAST(NULL AS numeric(18,6))                       AS rounding_amount,#(lf)    d.sequence_number,#(lf)    d.username,#(lf)    ISNULL(d.voided, 0)                               AS voided,#(lf)    d.applied_coupon_item_ids,#(lf)    d.create_by,#(lf)    d.create_time,#(lf)    ISNULL(d.[description], '')                       AS [description],#(lf)    d.last_update_by,#(lf)    d.loyalty_promotion_id,#(lf)    d.mod_by_amount,#(lf)    d.mod_by_percentage,#(lf)    d.modification_total,#(lf)    d.price_mod_source_sub_type_code,#(lf)    ISNULL(d.promotion_id, '')                        AS promotion_id,#(lf)    d.promotion_reward_quantity,#(lf)    ISNULL(d.promotion_type, '')                      AS promotion_type,#(lf)    d.ref_line_sequence_number,#(lf)    d.promo_code_id,#(lf)    d.reward_base_price_type_code,#(lf)    ISNULL(d.vendor_funded_flag, 0)                   AS vendor_funded_flag,#(lf)    d.quantity_index,#(lf)    d.rtn_device_id,#(lf)    CONVERT(varchar(10), TRY_CONVERT(date, d.rtn_business_date), 120) AS rtn_business_date,#(lf)    d.rtn_sequence_number,#(lf)    ISNULL(d.returned_flag, 0)                        AS returned_flag,#(lf)    d.external_id#(lf)  FROM dbo.jumpmind_sls_retail_line_item_price_mod d#(lf)  WHERE d.business_date >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf)),#(lf)hs AS (#(lf)  SELECT#(lf)    COALESCE(#(lf)      NULLIF(CONVERT(varchar(64), dtt.MaxWarehouseCode), ''),#(lf)      NULLIF(CONVERT(varchar(64), dtt.SiteWarehouseCode), ''),#(lf)      NULLIF(CONVERT(varchar(64), r.SiteCode), '')#(lf)    )                                                AS InventLocationId,#(lf)    CAST(COALESCE(#(lf)      r.OrderDateUTC,#(lf)      r.DateCreatedUTC,#(lf)      r.OrderStatusChangeDateUTC,#(lf)      r.ExportCreatedUTC#(lf)    ) AS date)                                       AS TransDate,#(lf)    CONVERT(varchar(64), r.OrderNumber)              AS Barcode,#(lf)    CONVERT(varchar(64), r.OrderID)                  AS OrderID,#(lf)    r._RowIndex#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)  LEFT JOIN dbo.mulesoft_dynamicstargettrans dtt#(lf)    ON CONVERT(varchar(64), dtt.OrderId) = CONVERT(varchar(64), r.OrderID)#(lf)  WHERE CAST(COALESCE(#(lf)      r.OrderDateUTC,#(lf)      r.DateCreatedUTC,#(lf)      r.OrderStatusChangeDateUTC,#(lf)      r.ExportCreatedUTC#(lf)    ) AS date) >= DATEADD(month, -2, CAST(GETDATE() AS date))           -- 2 MONTHS#(lf)),#(lf)hs_norm AS (#(lf)  SELECT#(lf)    CASE#(lf)      WHEN hs.InventLocationId = 'BAB'   THEN '1013'#(lf)      WHEN hs.InventLocationId = 'BABUK' THEN '2013'#(lf)      WHEN NULLIF(hs.InventLocationId,'') IS NULL THEN '9999'#(lf)      ELSE '9999'#(lf)    END                                              AS InventLocationIdMapped,#(lf)    hs.*#(lf)  FROM hs#(lf)),#(lf)deck_adjustments AS (#(lf)  SELECT#(lf)    CONVERT(varchar(64), a._ParentKeyField)          AS OrderID_key,#(lf)    a.OrderTransactionIdentifier                      AS RootRowIndex,#(lf)    a.AdjustmentDate,#(lf)    a.AdjustmentTypeValue#(lf)  FROM dbo.mulesoft_deckjsonraw_orderadjustments a#(lf)  WHERE a.AdjustmentDate >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf))#(lf)#(lf)SELECT#(lf)  'POS' AS SourceSystem,#(lf)  CONCAT(#(lf)    CAST(h.device_id AS varchar(64)), '-',#(lf)    CONVERT(varchar(10), h.business_date, 120), '-',#(lf)    CONVERT(varchar(50), h.sequence_number), '-',#(lf)    CONVERT(varchar(50), d.line_sequence_number)#(lf)  ) AS TransactionKey,#(lf)  d.*#(lf)FROM pos_line_discounts d#(lf)JOIN pos_tx_header h#(lf)  ON h.device_id       = d.device_id#(lf) AND h.business_date   = CAST(d.business_date AS date)#(lf) AND h.sequence_number = d.sequence_number#(lf)#(lf)UNION ALL#(lf)#(lf)SELECT#(lf)  'OMS' AS SourceSystem,#(lf)  CONCAT(#(lf)    hn.InventLocationIdMapped, '-', '052', '-', CONVERT(varchar, hn.TransDate, 112), '-', hn.Barcode#(lf)  ) AS TransactionKey,#(lf)  CONVERT(varchar(10), hn.TransDate, 120)            AS business_date,#(lf)  'AMOUNT'                                          AS calc_method,#(lf)  'WEB'                                             AS device_id,#(lf)  CAST(NULL AS varchar(64))                         AS entry_method_code,#(lf)  CASE WHEN hn.InventLocationIdMapped IN ('2013') THEN 'GBP' ELSE 'USD' END AS iso_currency_code,#(lf)  da.AdjustmentDate                                   AS last_update_time,#(lf)  CAST(0 AS int)                                      AS line_sequence_number,#(lf)  CAST(1 AS int)                                      AS mod_line_sequence_number,#(lf)  CAST(NULL AS varchar(64))                           AS override_user_id,#(lf)  CASE WHEN da.AdjustmentTypeValue LIKE '%Manual%' THEN 'MANUAL' ELSE NULL END AS price_mod_source_type_code,#(lf)  'TRANS'                                           AS price_mod_type_code,#(lf)  CAST(NULL AS varchar(64))                         AS reason_code,#(lf)  CAST(NULL AS numeric(18,6))                       AS rounding_amount,#(lf)  TRY_CONVERT(bigint, da.OrderID_key)               AS sequence_number,#(lf)  CAST(NULL AS varchar(128))                        AS username,#(lf)  CAST(0 AS int)                                    AS voided,#(lf)  CAST(NULL AS varchar(4000))                       AS applied_coupon_item_ids,#(lf)  'WEB'                                             AS create_by,#(lf)  CAST(NULL AS datetime2(6))                        AS create_time,#(lf)  ISNULL(da.AdjustmentTypeValue, '')                AS [description],#(lf)  CAST(NULL AS varchar(128))                        AS last_update_by,#(lf)  CAST(NULL AS varchar(64))                         AS loyalty_promotion_id,#(lf)  CAST(NULL AS decimal(18,6))                       AS mod_by_amount,#(lf)  CAST(NULL AS decimal(18,6))                       AS mod_by_percentage,#(lf)  CAST(NULL AS decimal(18,6))                       AS modification_total,#(lf)  CAST(NULL AS varchar(64))                         AS price_mod_source_sub_type_code,#(lf)  ''                                                AS promotion_id,#(lf)  CAST(NULL AS decimal(18,6))                       AS promotion_reward_quantity,#(lf)  'Transaction'                                     AS promotion_type,#(lf)  CAST(NULL AS int)                                  AS ref_line_sequence_number,#(lf)  CAST(NULL AS varchar(64))                          AS promo_code_id,#(lf)  CAST(NULL AS varchar(64))                          AS reward_base_price_type_code,#(lf)  CAST(0 AS int)                                    AS vendor_funded_flag,#(lf)  CAST(NULL AS int)                                  AS quantity_index,#(lf)  CAST(NULL AS varchar(64))                          AS rtn_device_id,#(lf)  CAST(NULL AS varchar(10))                          AS rtn_business_date,#(lf)  CAST(NULL AS bigint)                               AS rtn_sequence_number,#(lf)  CAST(0 AS int)                                    AS returned_flag,#(lf)  CAST(NULL AS varchar(128))                        AS external_id#(lf)FROM deck_adjustments da#(lf)JOIN hs_norm hn#(lf)  ON CONVERT(varchar(64), hn.OrderID) = CONVERT(varchar(64), da.OrderID_key)#(lf) AND hn._RowIndex = da.RootRowIndex;"]),
...
let
    ...
    #"Removed Duplicates" = Table.Distinct(#"Reordered Columns", {"Transaction Key"})
in
    #"Removed Duplicates"
```
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_tx AS (#(lf)  SELECT#(lf)      CONVERT(varchar(128),#(lf)        CONCAT(#(lf)          st.device_id, '-',#(lf)          st.business_date, '-',#(lf)          CONVERT(varchar(50), st.sequence_number)#(lf)        )#(lf)      ) AS TransactionKey,#(lf)      st.device_id,#(lf)      st.business_date,#(lf)      st.sequence_number,#(lf)      st.total,#(lf)      st.pre_tender_balance_due,#(lf)      st.subtotal,#(lf)      st.tax_total,#(lf)      st.tax_total_for_display,#(lf)      st.discount_total,#(lf)      st.customer_id,#(lf)      st.selling_channel_code,#(lf)      st.loyalty_card_number,#(lf)      st.tax_exempt_customer_id,#(lf)      st.tax_exempt_certificate,#(lf)      st.tax_exempt_code,#(lf)      st.employee_id_for_discount,#(lf)      st.iso_currency_code,#(lf)      st.line_item_count,#(lf)      st.age_restricted_date_of_birth,#(lf)      st.item_count,#(lf)      st.customer_name,#(lf)      st.tender_type_codes,#(lf)      st.voidable_flag,#(lf)      st.tax_geo_code_origin,#(lf)      st.rcpt_rtn_total,#(lf)      st.non_rcpt_rtn_total,#(lf)      st.customer_entry_method_code,#(lf)      st.cust_other_id,#(lf)      st.rcpt_rtn_count,#(lf)      st.non_rcpt_rtn_count,#(lf)      st.ring_elapsed_time_in_secs,#(lf)      st.tender_elapsed_time_in_secs,#(lf)      st.idle_elapsed_time_in_secs,#(lf)      st.lock_elapsed_time_in_secs,#(lf)      st.entry_mode_code,#(lf)      st.suspended_reason_code,#(lf)      st.suspended_note,#(lf)      st.order_id,#(lf)      st.loyalty_points_earned,#(lf)      st.customer_callout,#(lf)      st.fiscal_control_number,#(lf)      st.gift_receipt_print_type,#(lf)      st.fiscal_processor_code,#(lf)      st.create_time,#(lf)      st.create_by,#(lf)      st.last_update_time,#(lf)      st.last_update_by,#(lf)      st.party_id,#(lf)      st.employee_name_for_discount,#(lf)      st.event_id,#(lf)      st.event_invoice,#(lf)      st.extended_subtotal,#(lf)      st.parent_order_id,#(lf)      st.additional_attributes,#(lf)      'POS' as Source#(lf)  FROM dbo.jumpmind_sls_retail_trans AS st#(lf)  WHERE st.business_date >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf)),#(lf)hs AS (#(lf)  SELECT#(lf)      COALESCE(#(lf)        NULLIF(CONVERT(varchar(64), dtt.MaxWarehouseCode), ''),#(lf)        NULLIF(CONVERT(varchar(64), dtt.SiteWarehouseCode), ''),#(lf)        NULLIF(CONVERT(varchar(64), r.SiteCode), '')#(lf)      ) AS InventLocationId,#(lf)      COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS TransDateDT,#(lf)      CONVERT(varchar(64), r.OrderNumber) AS OrderNumber,#(lf)      CONVERT(varchar(64), r.OrderID)     AS OrderID,#(lf)      TRY_CONVERT(decimal(17,2), r.SubTotal) AS SubTotal_Dec,#(lf)      TRY_CONVERT(decimal(17,2), r.Tax)      AS Tax_Dec,#(lf)      TRY_CONVERT(decimal(17,2), r.Total)    AS Total_Dec,#(lf)      CONVERT(varchar(8000), r.OrderSource)  AS OrderSource,#(lf)      COALESCE(r.UpdateDate, r.OrderDateUTC, r.DateCreatedUTC) AS LastUpdateTime,#(lf)      r.ExportCreatedUTC AS ExportCreatedUTC#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)  LEFT JOIN dbo.mulesoft_dynamicstargettrans dtt#(lf)    ON CONVERT(varchar(64), dtt.OrderId) = CONVERT(varchar(64), r.OrderID)#(lf)  WHERE CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS date)#(lf)        >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf)),#(lf)hs_norm AS (#(lf)  SELECT#(lf)    CASE#(lf)      WHEN hs.InventLocationId = 'BAB'   THEN '1013'#(lf)      WHEN hs.InventLocationId = 'BABUK' THEN '2013'#(lf)      WHEN NULLIF(hs.InventLocationId,'') IS NULL THEN '9999'#(lf)      ELSE '9999'#(lf)    END AS InventLocationIdMapped,#(lf)    hs.*#(lf)  FROM hs#(lf)),#(lf)oms_tx AS (#(lf)  SELECT#(lf)      CONVERT(varchar(128),#(lf)        CONCAT(#(lf)          hn.InventLocationIdMapped, '-','052','-',#(lf)          CONVERT(varchar(8), CAST(hn.TransDateDT AS date), 112),#(lf)          '-',#(lf)          hn.OrderNumber#(lf)        )#(lf)      ) AS TransactionKey,#(lf)      CONVERT(varchar(8000), CONCAT(hn.InventLocationIdMapped, '-', '052')) AS device_id,#(lf)      CONVERT(varchar(10), CAST(hn.TransDateDT AS date), 120)         AS business_date,#(lf)      TRY_CONVERT(bigint, hn.OrderID)                                 AS sequence_number,#(lf)      hn.Total_Dec                                                     AS total,#(lf)      CAST(NULL AS decimal(17,2))                                      AS pre_tender_balance_due,#(lf)      hn.SubTotal_Dec                                                  AS subtotal,          -- << fixed casing#(lf)      hn.Tax_Dec                                                       AS tax_total,#(lf)      hn.Tax_Dec                                                       AS tax_total_for_display,#(lf)      CAST(NULL AS decimal(17,2))                                      AS discount_total,#(lf)      CAST(NULL AS varchar(8000))                                      AS customer_id,#(lf)      hn.OrderSource                                                   AS selling_channel_code,#(lf)      CAST(NULL AS varchar(8000))                                      AS loyalty_card_number,#(lf)      CAST(NULL AS varchar(8000))                                      AS tax_exempt_customer_id,#(lf)      CAST(NULL AS varchar(8000))                                      AS tax_exempt_certificate,#(lf)      CAST(NULL AS varchar(8000))                                      AS tax_exempt_code,#(lf)      CAST(NULL AS varchar(8000))                                      AS employee_id_for_discount,#(lf)      CAST(NULL AS varchar(8000))                                      AS iso_currency_code,#(lf)      CAST(NULL AS int)                                                AS line_item_count,#(lf)      CAST(NULL AS datetime2(6))                                       AS age_restricted_date_of_birth,#(lf)      CAST(NULL AS int)                                                AS item_count,#(lf)      CAST(NULL AS varchar(8000))                                      AS customer_name,#(lf)      CAST(NULL AS varchar(8000))                                      AS tender_type_codes,#(lf)      CAST(0 AS int)                                                   AS voidable_flag,#(lf)      CAST(NULL AS varchar(8000))                                      AS tax_geo_code_origin,#(lf)      CAST(NULL AS decimal(17,2))                                      AS rcpt_rtn_total,#(lf)      CAST(NULL AS decimal(17,2))                                      AS non_rcpt_rtn_total,#(lf)      CAST(NULL AS varchar(8000))                                      AS customer_entry_method_code,#(lf)      CAST(NULL AS varchar(8000))                                      AS cust_other_id,#(lf)      CAST(NULL AS int)                                                AS rcpt_rtn_count,#(lf)      CAST(NULL AS int)                                                AS non_rcpt_rtn_count,#(lf)      CAST(NULL AS int)                                                AS ring_elapsed_time_in_secs,#(lf)      CAST(NULL AS int)                                                AS tender_elapsed_time_in_secs,#(lf)      CAST(NULL AS int)                                                AS idle_elapsed_time_in_secs,#(lf)      CAST(NULL AS int)                                                AS lock_elapsed_time_in_secs,#(lf)      CAST(NULL AS varchar(8000))                                      AS entry_mode_code,#(lf)      CAST(NULL AS varchar(8000))                                      AS suspended_reason_code,#(lf)      CAST(NULL AS varchar(8000))                                      AS suspended_note,#(lf)      CONVERT(varchar(8000), hn.OrderID)                               AS order_id,#(lf)      CAST(NULL AS decimal(17,2))                                      AS loyalty_points_earned,#(lf)      CAST(NULL AS varchar(8000))                                      AS customer_callout,#(lf)      CAST(NULL AS varchar(8000))                                      AS fiscal_control_number,#(lf)      CAST(NULL AS varchar(8000))                                      AS gift_receipt_print_type,#(lf)      CAST(NULL AS varchar(8000))                                      AS fiscal_processor_code,#(lf)      CAST(hn.ExportCreatedUTC AS datetime2(6))                        AS create_time,#(lf)      CAST('WEB' AS varchar(8000))                                     AS create_by,#(lf)      CAST(hn.LastUpdateTime AS datetime2(6))                          AS last_update_time,#(lf)      CAST(NULL AS varchar(8000))                                      AS last_update_by,#(lf)      CAST(NULL AS varchar(8000))                                      AS party_id,#(lf)      CAST(NULL AS varchar(8000))                                      AS employee_name_for_discount,#(lf)      CAST(NULL AS varchar(8000))                                      AS event_id,#(lf)      CAST(NULL AS varchar(8000))                                      AS event_invoice,#(lf)      CAST(NULL AS decimal(17,2))                                      AS extended_subtotal,#(lf)      CAST(NULL AS varchar(8000))                                      AS parent_order_id,#(lf)      CAST(NULL AS varchar(8000))                                      AS additional_attributes,#(lf)      'OMS'                                                            AS Source#(lf)  FROM hs_norm hn#(lf))#(lf)SELECT *#(lf)FROM pos_tx#(lf)UNION ALL#(lf)SELECT *#(lf)FROM oms_tx;", CreateNavigationProperties=false]),
    ...
    #"Removed Duplicates" = Table.Distinct(#"Removed Columns | Empty Columns", {"TransactionKey"})
in
    #"Removed Duplicates"
```
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_raw AS (#(lf)  SELECT *#(lf)  FROM dbo.jumpmind_sls_card_line_item#(lf)  WHERE TRY_CONVERT(date, business_date, 112) >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf)),#(lf)pos_shaped AS (#(lf)  SELECT#(lf)      device_id AS device_id,#(lf)      CONVERT(varchar(8), TRY_CONVERT(date, business_date, 112), 112) AS business_date,#(lf)      sequence_number AS sequence_number,#(lf)      line_sequence_number AS line_sequence_number,#(lf)      CAST(brand AS varchar(200)) AS brand,#(lf)      CAST(card_name AS varchar(200)) AS card_name,#(lf)      CAST(code AS varchar(200)) AS code,#(lf)      CAST(type_code AS varchar(200)) AS type_code,#(lf)      CAST(payment_provider_code AS varchar(200)) AS payment_provider_code,#(lf)      CAST(masked_card_number AS varchar(64)) AS masked_card_number,#(lf)      CAST(entry_mode AS varchar(64)) AS entry_mode,#(lf)      CAST(service_code AS varchar(64)) AS service_code,#(lf)      CAST(expiration_date AS varchar(10)) AS expiration_date,#(lf)      ref_line_sequence_number AS ref_line_sequence_number,#(lf)      CAST(card_number AS varchar(64)) AS card_number,#(lf)      create_time AS create_time,#(lf)      create_by AS create_by,#(lf)      last_update_time AS last_update_time,#(lf)      last_update_by AS last_update_by,#(lf)      CAST(gift_card_action_code AS varchar(64)) AS gift_card_action_code,#(lf)      'POS' AS source#(lf)  FROM pos_raw#(lf)),#(lf)op AS (#(lf)  SELECT#(lf)      TRY_CONVERT(int, op._ParentKeyField) AS ParentOrderID,#(lf)      CAST(op.ID AS bigint)                AS OP_ID,#(lf)      op.InsertDate                        AS OP_InsertDate,#(lf)      op.UpdateDate                        AS OP_UpdateDate,#(lf)      CAST(op.PaymentProcessor AS varchar(200)) AS OP_PaymentProcessor,#(lf)      CAST(op.PaymentSubType  AS varchar(200)) AS OP_PaymentSubType,#(lf)      CAST(op.CardType        AS varchar(200)) AS OP_CardType,#(lf)      CAST(op.CardNumber      AS varchar(32))  AS OP_Last4,#(lf)      TRY_CONVERT(int, op.ExpirationMonth)     AS OP_ExpMonth,#(lf)      TRY_CONVERT(int, op.ExpirationYear)      AS OP_ExpYear#(lf)  FROM dbo.mulesoft_deckjsonraw_orderpayments op#(lf)  WHERE TRY_CONVERT(int, op._ParentKeyField) IS NOT NULL#(lf)    AND TRY_CONVERT(date, COALESCE(op.UpdateDate, op.InsertDate)) >= DATEADD(month, -2, CAST(GETDATE() AS date)) -- 2 MONTHS#(lf)),#(lf)pt AS (#(lf)  SELECT#(lf)      CAST(pt.OrderPaymentId AS bigint)        AS PT_OrderPaymentId,#(lf)      CAST(pt.PaymentTransactionTypeId AS int) AS PT_TypeId,#(lf)      CAST(pt.Amount AS decimal(18,6))         AS PT_Amount,#(lf)      CAST(pt.IsDecline AS bit)                AS PT_IsDecline,#(lf)      pt.TransactionDateUTC                    AS PT_TransUTC,#(lf)      pt.InsertDate                            AS PT_InsertDate,#(lf)      pt.UpdateDate                            AS PT_UpdateDate#(lf)  FROM dbo.mulesoft_deckjsonraw_paymenttransactions pt#(lf)  WHERE TRY_CONVERT(date, COALESCE(pt.UpdateDate, pt.InsertDate, pt.TransactionDateUTC))#(lf)        >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf)),#(lf)pt_best AS (#(lf)  SELECT *#(lf)  FROM (#(lf)    SELECT#(lf)        p.*,#(lf)        ROW_NUMBER() OVER (#(lf)          PARTITION BY p.PT_OrderPaymentId#(lf)          ORDER BY#(lf)            CASE WHEN p.PT_IsDecline = 0 AND p.PT_TypeId IN (14) THEN 1#(lf)                 WHEN p.PT_IsDecline = 0 THEN 2#(lf)                 ELSE 3 END,#(lf)            COALESCE(p.PT_UpdateDate, p.PT_InsertDate, p.PT_TransUTC) DESC#(lf)        ) AS rn#(lf)    FROM pt p#(lf)  ) x#(lf)  WHERE x.rn = 1#(lf)),#(lf)root AS (#(lf)  SELECT#(lf)      r.OrderID,#(lf)      r.OrderNumber,#(lf)      r.SiteCode#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)),#(lf)op_ranked AS (#(lf)  SELECT#(lf)      o.*,#(lf)      ROW_NUMBER() OVER (#(lf)        PARTITION BY o.ParentOrderID#(lf)        ORDER BY COALESCE(pb.PT_UpdateDate, pb.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate) ASC, o.OP_ID#(lf)      ) AS rn#(lf)  FROM op o#(lf)  LEFT JOIN pt_best pb#(lf)    ON pb.PT_OrderPaymentId = o.OP_ID#(lf)),#(lf)oms_enriched AS (#(lf)  SELECT#(lf)      CAST((#(lf)        CASE #(lf)          WHEN NULLIF(CAST(dtt.SiteWarehouseCode AS varchar(64)), '') IS NOT NULL#(lf)            THEN CAST(dtt.SiteWarehouseCode AS varchar(64))#(lf)          WHEN rt.SiteCode = 'BAB'   THEN '1013'#(lf)          WHEN rt.SiteCode = 'BABUK' THEN '2013'#(lf)          ELSE CAST(rt.SiteCode AS varchar(64))#(lf)        END#(lf)      ) + '-052' AS varchar(64)) AS device_id,#(lf)      CONVERT(varchar(8),#(lf)              CAST(COALESCE(pb.PT_UpdateDate, pb.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate) AS date), 112) AS business_date,#(lf)      CAST(rt.OrderID AS bigint) AS sequence_number,#(lf)      CAST(orx.rn AS int) AS line_sequence_number,#(lf)      COALESCE(NULLIF(o.OP_CardType, ''), NULLIF(o.OP_PaymentSubType, ''), NULLIF(o.OP_PaymentProcessor, '')) AS brand,#(lf)      CAST(NULL AS varchar(200)) AS card_name,#(lf)      COALESCE(NULLIF(o.OP_PaymentProcessor, ''), NULLIF(o.OP_PaymentSubType, ''), NULLIF(o.OP_CardType, '')) AS code,#(lf)      COALESCE(NULLIF(o.OP_PaymentSubType, ''),  NULLIF(o.OP_PaymentProcessor, ''), NULLIF(o.OP_CardType, '')) AS type_code,#(lf)      CAST(NULL AS varchar(200)) AS payment_provider_code,#(lf)      CASE WHEN NULLIF(o.OP_Last4, '') IS NOT NULL THEN CONCAT(REPLICATE('*', 12), o.OP_Last4) END AS masked_card_number,#(lf)      CAST(NULL AS varchar(64)) AS entry_mode,#(lf)      CAST(NULL AS varchar(64)) AS service_code,#(lf)      CASE#(lf)        WHEN ISNULL(o.OP_ExpMonth,0) BETWEEN 1 AND 12 AND ISNULL(o.OP_ExpYear,0) > 0#(lf)          THEN RIGHT('00'+CAST(o.OP_ExpMonth AS varchar(2)),2) + RIGHT(RIGHT('0000'+CAST(o.OP_ExpYear AS varchar(4)),4),2)#(lf)      END AS expiration_date,#(lf)      CAST(orx.rn AS int) AS ref_line_sequence_number,#(lf)      CAST(NULL AS varchar(64)) AS card_number,#(lf)      COALESCE(pb.PT_InsertDate, o.OP_InsertDate) AS create_time,#(lf)      'sp_bab_pos_merge_webreturns' AS create_by,#(lf)      COALESCE(pb.PT_UpdateDate, pb.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate) AS last_update_time,#(lf)      'sp_bab_pos_merge_webreturns' AS last_update_by,#(lf)      CAST(NULL AS varchar(64)) AS gift_card_action_code,#(lf)      'OMS' AS source#(lf)  FROM op_ranked orx#(lf)  JOIN op o#(lf)    ON o.OP_ID = orx.OP_ID#(lf)  LEFT JOIN pt_best pb#(lf)    ON pb.PT_OrderPaymentId = o.OP_ID#(lf)  JOIN root rt#(lf)    ON rt.OrderID = orx.ParentOrderID#(lf)  OUTER APPLY (#(lf)    SELECT TOP (1) dtt.SiteWarehouseCode#(lf)    FROM dbo.mulesoft_dynamicstargettrans dtt#(lf)    WHERE dtt.OrderId = rt.OrderID#(lf)    ORDER BY TRY_CONVERT(datetime2(7), dtt.ExportCreatedUTC) DESC#(lf)  ) dtt#(lf)  -- Safety net: ensure the derived OMS business_date also falls in the last 2 months#(lf)  WHERE TRY_CONVERT(date, COALESCE(pb.PT_UpdateDate, pb.PT_InsertDate, o.OP_UpdateDate, o.OP_InsertDate))#(lf)        >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf))#(lf)SELECT *#(lf)FROM pos_shaped#(lf)UNION ALL#(lf)SELECT *#(lf)FROM oms_enriched;#(lf)"]),
    #"Filtered Rows | Remove Web Merge" = Table.SelectRows(Source, each [create_by] = "openpos-sls"),
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_raw AS (#(lf)  SELECT *#(lf)  FROM dbo.jumpmind_sls_tender_line_item#(lf)  WHERE create_by = 'openpos-sls'#(lf)    AND ISNULL(voided,0) = 0#(lf)    AND TRY_CONVERT(date, business_date, 112) >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf)),#(lf)pos_base AS (#(lf)  SELECT#(lf)      CAST(device_id AS varchar(64))                                   AS device_id,#(lf)      CONVERT(varchar(8), TRY_CONVERT(date, business_date, 112), 112)  AS business_date,#(lf)      CAST(sequence_number AS bigint)                                   AS sequence_number,#(lf)      CAST(line_sequence_number AS int)                                 AS line_sequence_number,#(lf)      CAST(tender_code AS varchar(200))                                 AS tender_code,#(lf)      CAST(tender_type_code AS varchar(200))                            AS tender_type_code,#(lf)      CAST(change_flag AS bit)                                          AS change_flag,#(lf)      CAST(customer_account_number AS varchar(128))                     AS customer_account_number,#(lf)      CAST(tender_account_number AS varchar(64))                        AS tender_account_number,#(lf)      CAST(iso_currency_code AS varchar(10))                            AS iso_currency_code,#(lf)      CAST(tender_amount AS decimal(18,6))                              AS tender_amount,#(lf)      CAST(cash_back_amount AS decimal(18,6))                           AS cash_back_amount,#(lf)      CAST(iso_foreign_currency_code AS varchar(10))                    AS iso_foreign_currency_code,#(lf)      CAST(foreign_currency_amount AS decimal(18,6))                    AS foreign_currency_amount,#(lf)      CAST(exchange_rate AS decimal(18,6))                              AS exchange_rate,#(lf)      CAST(overtendered AS bit)                                         AS overtendered,#(lf)      CAST(partially_approved AS bit)                                   AS partially_approved,#(lf)      CAST(tender_finance_id AS varchar(64))                            AS tender_finance_id,#(lf)      CAST(certificate_number AS varchar(128))                          AS certificate_number,#(lf)      CAST(post_void AS bit)                                            AS post_void,#(lf)      CAST(voided AS bit)                                               AS voided,#(lf)      CAST(override_user_id AS varchar(64))                             AS override_user_id,#(lf)      CAST(entry_method_code AS varchar(64))                            AS entry_method_code,#(lf)      CAST(create_time AS datetime2)                                    AS create_time,#(lf)      CAST(create_by AS varchar(128))                                   AS create_by,#(lf)      CAST(last_update_time AS datetime2)                               AS last_update_time,#(lf)      CAST(last_update_by AS varchar(128))                              AS last_update_by,#(lf)      CAST(tender_auth_method_code AS varchar(64))                      AS tender_auth_method_code,#(lf)      CAST(tender_group AS varchar(64))                                 AS tender_group,#(lf)      CAST(tender_id AS varchar(64))                                    AS tender_id,#(lf)      CAST(voucher_id AS varchar(64))                                   AS voucher_id,#(lf)      CAST('POS' AS varchar(8))                                         AS source#(lf)  FROM pos_raw#(lf)),#(lf)root AS (#(lf)  SELECT#(lf)      r.OrderID,#(lf)      r.OrderNumber,#(lf)      r.SiteCode,#(lf)      CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS date) AS TransDate,#(lf)      r.OrderDateUTC,#(lf)      r.DateCreatedUTC,#(lf)      r.OrderStatusChangeDateUTC,#(lf)      r.ExportCreatedUTC#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)  WHERE CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS date)#(lf)        >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 MONTHS#(lf)),#(lf)hs AS (#(lf)  SELECT#(lf)      COALESCE(#(lf)        NULLIF(CONVERT(varchar(64), dtt.MaxWarehouseCode), ''),#(lf)        NULLIF(CONVERT(varchar(64), dtt.SiteWarehouseCode), ''),#(lf)        NULLIF(CONVERT(varchar(64), r.SiteCode), '')#(lf)      ) AS InventLocationId,#(lf)      r.*#(lf)  FROM root r#(lf)  LEFT JOIN dbo.mulesoft_dynamicstargettrans dtt#(lf)    ON CONVERT(varchar(64), dtt.OrderId) = CONVERT(varchar(64), r.OrderID)#(lf)),#(lf)hs_norm AS (#(lf)  SELECT#(lf)    CASE#(lf)      WHEN hs.InventLocationId = 'BAB'   THEN '1013'#(lf)      WHEN hs.InventLocationId = 'BABUK' THEN '2013'#(lf)      WHEN NULLIF(hs.InventLocationId,'') IS NULL THEN '9999'#(lf)      ELSE '9999'#(lf)    END AS InventLocationIdMapped,#(lf)    hs.*#(lf)  FROM hs#(lf)),#(lf)op AS (#(lf)  SELECT#(lf)      TRY_CONVERT(int, op._ParentKeyField)           AS ParentOrderID,#(lf)      CONVERT(varchar(64), op.ID)                    AS OrderPaymentId,#(lf)      CAST(op._RowIndex AS int)                      AS OP_RowIndex,#(lf)      CAST(op.PaymentProcessor AS varchar(200))      AS PaymentProcessor,#(lf)      CAST(op.PaymentSubType  AS varchar(200))       AS PaymentSubType,#(lf)      CAST(op.CardType        AS varchar(200))       AS CardType,#(lf)      CAST(op.AuthorizedAmount AS decimal(18,6))     AS AuthorizedAmount,#(lf)      CAST(op.CapturedAmount   AS decimal(18,6))     AS CapturedAmount,#(lf)      CAST(op.CreditedAmount   AS decimal(18,6))     AS CreditedAmount,#(lf)      op.InsertDate                                    AS OP_InsertDate,#(lf)      op.UpdateDate                                    AS OP_UpdateDate#(lf)  FROM dbo.mulesoft_deckjsonraw_orderpayments op#(lf)  WHERE TRY_CONVERT(int, op._ParentKeyField) IS NOT NULL#(lf)),#(lf)pt AS (#(lf)  SELECT#(lf)      CONVERT(varchar(64), pt.OrderPaymentId)        AS OrderPaymentId,#(lf)      CAST(pt.PaymentTransactionTypeId AS int)       AS PaymentTransactionTypeId,#(lf)      CAST(pt.Amount AS decimal(18,6))               AS Amount,#(lf)      pt.TransactionDateUTC                          AS TransactionDateUTC,#(lf)      pt.InsertDate                                  AS PT_InsertDate,#(lf)      pt.UpdateDate                                  AS PT_UpdateDate#(lf)  FROM dbo.mulesoft_deckjsonraw_paymenttransactions pt#(lf)),#(lf)pt_best AS (#(lf)  SELECT *#(lf)  FROM (#(lf)    SELECT#(lf)      p.*,#(lf)      ROW_NUMBER() OVER (#(lf)        PARTITION BY p.OrderPaymentId#(lf)        ORDER BY#(lf)          CASE WHEN p.PaymentTransactionTypeId IN (14,1,2) THEN 1 ELSE 2 END,#(lf)          COALESCE(p.PT_UpdateDate, p.PT_InsertDate, p.TransactionDateUTC) DESC#(lf)      ) AS rn#(lf)    FROM pt p#(lf)  ) x#(lf)  WHERE x.rn = 1#(lf)),#(lf)oms_base AS (#(lf)  SELECT#(lf)      CAST(COALESCE(hn.InventLocationIdMapped, '9999') + '-052' AS varchar(64)) AS device_id,#(lf)      CONVERT(varchar(8), h.TransDate, 112)                  AS business_date,#(lf)      CAST(h.OrderID AS bigint)                              AS sequence_number,#(lf)      ROW_NUMBER() OVER (#(lf)        PARTITION BY h.OrderID#(lf)        ORDER BY COALESCE(pb.TransactionDateUTC, o.OP_UpdateDate, o.OP_InsertDate)#(lf)      )                                                      AS line_sequence_number,#(lf)      COALESCE(o.CardType, o.PaymentSubType, o.PaymentProcessor) AS tender_code,#(lf)      COALESCE(o.PaymentSubType, o.PaymentProcessor, o.CardType) AS tender_type_code,#(lf)      CAST(0 AS bit)                                         AS change_flag,#(lf)      CAST(NULL AS varchar(128))                              AS customer_account_number,#(lf)      CAST(NULL AS varchar(64))                               AS tender_account_number,#(lf)      CASE WHEN hn.InventLocationIdMapped = '2013' THEN 'GBP' ELSE 'USD' END AS iso_currency_code,#(lf)      COALESCE(#(lf)        CASE#(lf)          WHEN pb.PaymentTransactionTypeId IN (3,4) THEN -ABS(pb.Amount)#(lf)          WHEN pb.PaymentTransactionTypeId IN (1,2,14) THEN  ABS(pb.Amount)#(lf)          ELSE pb.Amount#(lf)        END,#(lf)        NULLIF(o.CapturedAmount, 0),#(lf)        NULLIF(o.AuthorizedAmount, 0),#(lf)        -1 * NULLIF(o.CreditedAmount, 0),#(lf)        0#(lf)      )                                                       AS tender_amount,#(lf)      CAST(NULL AS decimal(18,6))                             AS cash_back_amount,#(lf)      CAST(NULL AS varchar(10))                               AS iso_foreign_currency_code,#(lf)      CAST(NULL AS decimal(18,6))                             AS foreign_currency_amount,#(lf)      CAST(NULL AS decimal(18,6))                             AS exchange_rate,#(lf)      CAST(0 AS bit)                                          AS overtendered,#(lf)      CAST(0 AS bit)                                          AS partially_approved,#(lf)      CAST(NULL AS varchar(64))                               AS tender_finance_id,#(lf)      CAST(NULL AS varchar(128))                              AS certificate_number,#(lf)      CAST(0 AS bit)                                          AS post_void,#(lf)      CAST(0 AS bit)                                          AS voided,#(lf)      CAST(NULL AS varchar(64))                               AS override_user_id,#(lf)      CAST(NULL AS varchar(64))                               AS entry_method_code,#(lf)      COALESCE(h.OrderDateUTC, h.DateCreatedUTC)              AS create_time,#(lf)      'sp_bab_pos_merge_webreturns'                           AS create_by,#(lf)      COALESCE(pb.PT_UpdateDate, pb.PT_InsertDate, h.OrderStatusChangeDateUTC, h.OrderDateUTC, h.DateCreatedUTC) AS last_update_time,#(lf)      'sp_bab_pos_merge_webreturns'                           AS last_update_by,#(lf)      CAST(NULL AS varchar(64))                               AS tender_auth_method_code,#(lf)      'WEB'                                                   AS tender_group,#(lf)      CAST(NULL AS varchar(64))                               AS tender_id,#(lf)      CAST(NULL AS varchar(64))                               AS voucher_id,#(lf)      'OMS'                                                   AS source#(lf)  FROM op o#(lf)  JOIN hs_norm hn#(lf)    ON hn.OrderID = o.ParentOrderID#(lf)  LEFT JOIN pt_best pb#(lf)    ON pb.OrderPaymentId = o.OrderPaymentId#(lf)  JOIN root h#(lf)    ON h.OrderID = o.ParentOrderID#(lf))#(lf)SELECT *#(lf)FROM pos_base#(lf)UNION ALL#(lf)SELECT *#(lf)FROM oms_base;#(lf)"]),
    #"Filtered Rows | Remove Web Merge" = Table.SelectRows(Source, each [create_by] = "openpos-sls"),
    #"Filtered Rows | Remove Voided Lines" = Table.SelectRows(#"Filtered Rows | Remove Web Merge", each [voided] = false),
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH cutoff AS (#(lf)  SELECT CONVERT(int, CONVERT(varchar(8), CAST(DATEADD(month, -2, GETDATE()) AS date), 112)) AS cutoff_yyyymmdd#(lf)),#(lf)pos_base AS (#(lf)  SELECT#(lf)      device_id,#(lf)      business_date,#(lf)      sequence_number,#(lf)      barcode,#(lf)      begin_time,#(lf)      business_unit_id,#(lf)      client_offset,#(lf)      create_by,#(lf)      create_time,#(lf)      customer_id,#(lf)      customer_name,#(lf)      device_type,#(lf)      discount_total,#(lf)      employee_id_for_discount,#(lf)      end_time,#(lf)      iso_currency_code,#(lf)      item_count,#(lf)      last_update_by,#(lf)      last_update_time,#(lf)      local_offset,#(lf)      loyalty_card_number,#(lf)      loyalty_rewards_count,#(lf)      loyalty_rewards_total,#(lf)      mfr_coupons_count,#(lf)      mfr_coupons_non_taxable_count,#(lf)      mfr_coupons_non_taxable_total,#(lf)      mfr_coupons_taxable_count,#(lf)      mfr_coupons_taxable_total,#(lf)      mfr_coupons_total,#(lf)      non_rcpt_rtn_count,#(lf)      paid_to,#(lf)      pre_tender_balance_due,#(lf)      rcpt_rtn_count,#(lf)      resumed_device_id,#(lf)      resumed_sequence_number,#(lf)      session_id,#(lf)      store_bank_id,#(lf)      store_electronic_promos_count,#(lf)      store_electronic_promos_total,#(lf)      store_physical_coupons_count,#(lf)      store_physical_coupons_total,#(lf)      store_promos_count,#(lf)      store_promos_non_taxable_count,#(lf)      store_promos_non_taxable_total,#(lf)      store_promos_taxable_count,#(lf)      store_promos_taxable_total,#(lf)      store_promos_total,#(lf)      suspended_device_id,#(lf)      suspended_sequence_number,#(lf)      tax_total,#(lf)      tender_type_codes,#(lf)      tender1_amount,#(lf)      tender1_auth_code,#(lf)      tender1_card_type_code,#(lf)      tender1_masked_card_number,#(lf)      tender1_type_code,#(lf)      tender2_amount,#(lf)      tender2_auth_code,#(lf)      tender2_card_type_code,#(lf)      tender2_masked_card_number,#(lf)      tender2_type_code,#(lf)      tender3_amount,#(lf)      tender3_auth_code,#(lf)      tender3_card_type_code,#(lf)      tender3_masked_card_number,#(lf)      tender3_type_code,#(lf)      tender4_amount,#(lf)      tender4_auth_code,#(lf)      tender4_card_type_code,#(lf)      tender4_masked_card_number,#(lf)      tender4_type_code,#(lf)      tender5_amount,#(lf)      tender5_auth_code,#(lf)      tender5_card_type_code,#(lf)      tender5_masked_card_number,#(lf)      tender5_type_code,#(lf)      till_id,#(lf)      total,#(lf)      total_physical_coupons_count,#(lf)      training_mode,#(lf)      trans_status_code,#(lf)      transaction_duration_in_sec,#(lf)      username,#(lf)      voidable_flag,#(lf)      voided_sequence_number,#(lf)      trans_type_code,#(lf)      reason_code,#(lf)      CAST('POS' AS varchar(8)) AS source#(lf)  FROM dbo.jumpmind_sls_trans_summary s#(lf)  CROSS JOIN cutoff k#(lf)  WHERE COALESCE(#(lf)          TRY_CONVERT(int, CONVERT(varchar(8), TRY_CONVERT(datetime2, s.business_date), 112)),#(lf)          TRY_CONVERT(int, s.business_date)#(lf)        ) >= k.cutoff_yyyymmdd#(lf)),#(lf)oms_root AS (#(lf)  SELECT#(lf)      r.OrderID,#(lf)      r.OrderNumber,#(lf)      r.SiteCode,#(lf)      COALESCE(#(lf)        TRY_CONVERT(datetime2(7), r.OrderDateUTC),#(lf)        TRY_CONVERT(datetime2(7), r.DateCreatedUTC),#(lf)        TRY_CONVERT(datetime2(7), r.UpdateDate)#(lf)      ) AS biz_dt_raw,#(lf)      COALESCE(#(lf)        TRY_CONVERT(datetime2(7), r.UpdateDate),#(lf)        TRY_CONVERT(datetime2(7), r.LastUpdateUTC),#(lf)        TRY_CONVERT(datetime2(7), r.OrderDateUTC),#(lf)        TRY_CONVERT(datetime2(7), r.DateCreatedUTC)#(lf)      ) AS last_upd_raw#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)),#(lf)oms_root_filt AS (#(lf)  SELECT r.*#(lf)  FROM oms_root r#(lf)  CROSS JOIN cutoff k#(lf)  WHERE TRY_CONVERT(int, CONVERT(varchar(8), r.biz_dt_raw, 112)) >= k.cutoff_yyyymmdd#(lf)),#(lf)oms_header AS (#(lf)  SELECT#(lf)      rf.OrderID,#(lf)      rf.OrderNumber,#(lf)      rf.SiteCode,#(lf)      CAST(CAST(rf.biz_dt_raw AS date) AS date) AS business_date,#(lf)      (#(lf)        CASE #(lf)          WHEN NULLIF(CAST(dtt.SiteWarehouseCode AS varchar(64)), '') IS NOT NULL#(lf)            THEN CAST(dtt.SiteWarehouseCode AS varchar(64))#(lf)          WHEN rf.SiteCode = 'BAB'   THEN '1013'#(lf)          WHEN rf.SiteCode = 'BABUK' THEN '2013'#(lf)          ELSE CAST(rf.SiteCode AS varchar(64))#(lf)        END#(lf)      ) + '-052' AS device_id,#(lf)      CASE WHEN rf.SiteCode = 'BABUK' THEN 'GBP'#(lf)           WHEN rf.SiteCode = 'BAB'   THEN 'USD'#(lf)           ELSE '*' END AS iso_currency_code,#(lf)      rf.last_upd_raw AS last_update_time#(lf)  FROM oms_root_filt rf#(lf)  OUTER APPLY (#(lf)    SELECT TOP (1) dtt.SiteWarehouseCode#(lf)    FROM dbo.mulesoft_dynamicstargettrans dtt#(lf)    WHERE dtt.OrderId = rf.OrderID#(lf)    ORDER BY TRY_CONVERT(datetime2(7), dtt.ExportCreatedUTC) DESC#(lf)  ) dtt#(lf)),#(lf)oms_payments AS (#(lf)  SELECT#(lf)      TRY_CONVERT(int, op._ParentKeyField) AS OrderID,#(lf)      COALESCE(NULLIF(op.CapturedAmount, 0), NULLIF(op.AuthorizedAmount, 0), 0)#(lf)        - COALESCE(NULLIF(op.CreditedAmount, 0), 0) AS net_amount,#(lf)      COALESCE(NULLIF(op.CardType, ''), NULLIF(op.PaymentSubType, ''), NULLIF(op.PaymentProcessor, '')) AS type_code,#(lf)      CASE WHEN NULLIF(op.CardNumber, '') IS NOT NULL#(lf)           THEN '****' + RIGHT(op.CardNumber, 4) END AS masked_num,#(lf)      NULLIF(op.Generic1, '') AS auth_code#(lf)  FROM dbo.mulesoft_deckjsonraw_orderpayments op#(lf)  WHERE TRY_CONVERT(int, op._ParentKeyField) IS NOT NULL#(lf)),#(lf)oms_tenders AS (#(lf)  SELECT OrderID,#(lf)         MAX(CASE WHEN rn = 1 THEN net_amount END)   AS t1_amount,#(lf)         MAX(CASE WHEN rn = 1 THEN type_code  END)   AS t1_type_code,#(lf)         MAX(CASE WHEN rn = 1 THEN masked_num END)   AS t1_masked,#(lf)         MAX(CASE WHEN rn = 1 THEN auth_code  END)   AS t1_auth,#(lf)         MAX(CASE WHEN rn = 2 THEN net_amount END)   AS t2_amount,#(lf)         MAX(CASE WHEN rn = 2 THEN type_code  END)   AS t2_type_code,#(lf)         MAX(CASE WHEN rn = 2 THEN masked_num END)   AS t2_masked,#(lf)         MAX(CASE WHEN rn = 2 THEN auth_code  END)   AS t2_auth#(lf)  FROM (#(lf)    SELECT#(lf)      p.*,#(lf)      ROW_NUMBER() OVER (#(lf)        PARTITION BY p.OrderID#(lf)        ORDER BY ABS(TRY_CONVERT(decimal(18,6), p.net_amount)) DESC, type_code#(lf)      ) AS rn#(lf)    FROM oms_payments p#(lf)  ) x#(lf)  WHERE rn <= 2#(lf)  GROUP BY OrderID#(lf)),#(lf)hs AS (#(lf)  SELECT#(lf)      CASE #(lf)        WHEN NULLIF(CONVERT(varchar(64), dtt.MaxWarehouseCode), '') IS NOT NULL#(lf)          THEN CONVERT(varchar(64), dtt.MaxWarehouseCode)#(lf)        WHEN NULLIF(CONVERT(varchar(64), dtt.SiteWarehouseCode), '') IS NOT NULL#(lf)          THEN CONVERT(varchar(64), dtt.SiteWarehouseCode)#(lf)        WHEN r.SiteCode = 'BAB'   THEN '1013'#(lf)        WHEN r.SiteCode = 'BABUK' THEN '2013'#(lf)        ELSE CONVERT(varchar(64), r.SiteCode)#(lf)      END AS InventLocationId,#(lf)      CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS date) AS TransDate,#(lf)      CONVERT(varchar(64), r.OrderNumber) AS Barcode,#(lf)      CONVERT(varchar(64), r.OrderID)     AS OrderID,#(lf)      r._RowIndex#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)  LEFT JOIN dbo.mulesoft_dynamicstargettrans dtt#(lf)    ON CONVERT(varchar(64), dtt.OrderId) = CONVERT(varchar(64), r.OrderID)#(lf)),#(lf)oms_base AS (#(lf)  SELECT#(lf)      oh.device_id,#(lf)      CONCAT(YEAR(oh.business_date), RIGHT(CONCAT('0', MONTH(oh.business_date)), 2), RIGHT(CONCAT('0', DAY(oh.business_date)), 2)) as business_date,#(lf)      TRY_CONVERT(bigint, oh.OrderNumber) AS sequence_number,#(lf)      CAST(NULL AS varchar(100)) AS barcode,#(lf)      CAST(NULL AS datetime2(7)) AS begin_time,#(lf)      CAST(NULL AS int) AS business_unit_id,#(lf)      CAST(NULL AS int) AS client_offset,#(lf)      CAST(NULL AS varchar(128)) AS create_by,#(lf)      CAST(NULL AS datetime2(7)) AS create_time,#(lf)      CAST(NULL AS varchar(64)) AS customer_id,#(lf)      CAST(NULL AS varchar(256)) AS customer_name,#(lf)      CAST(NULL AS varchar(64)) AS device_type,#(lf)      CAST(NULL AS decimal(18,6)) AS discount_total,#(lf)      CAST(NULL AS varchar(64)) AS employee_id_for_discount,#(lf)      CAST(NULL AS datetime2(7)) AS end_time,#(lf)      oh.iso_currency_code,#(lf)      CAST(NULL AS int) AS item_count,#(lf)      CAST(NULL AS varchar(128)) AS last_update_by,#(lf)      oh.last_update_time AS last_update_time,#(lf)      CAST(NULL AS int) AS local_offset,#(lf)      CAST(NULL AS varchar(64)) AS loyalty_card_number,#(lf)      CAST(NULL AS int) AS loyalty_rewards_count,#(lf)      CAST(NULL AS decimal(18,6)) AS loyalty_rewards_total,#(lf)      CAST(NULL AS int) AS mfr_coupons_count,#(lf)      CAST(NULL AS int) AS mfr_coupons_non_taxable_count,#(lf)      CAST(NULL AS decimal(18,6)) AS mfr_coupons_non_taxable_total,#(lf)      CAST(NULL AS int) AS mfr_coupons_taxable_count,#(lf)      CAST(NULL AS decimal(18,6)) AS mfr_coupons_taxable_total,#(lf)      CAST(NULL AS decimal(18,6)) AS mfr_coupons_total,#(lf)      CAST(NULL AS int) AS non_rcpt_rtn_count,#(lf)      CAST(NULL AS varchar(64)) AS paid_to,#(lf)      CAST(NULL AS decimal(18,6)) AS pre_tender_balance_due,#(lf)      CAST(NULL AS int) AS rcpt_rtn_count,#(lf)      CAST(NULL AS varchar(64)) AS resumed_device_id,#(lf)      CAST(NULL AS bigint) AS resumed_sequence_number,#(lf)      CAST(NULL AS varchar(64)) AS session_id,#(lf)      CAST(NULL AS varchar(64)) AS store_bank_id,#(lf)      CAST(NULL AS int) AS store_electronic_promos_count,#(lf)      CAST(NULL AS decimal(18,6)) AS store_electronic_promos_total,#(lf)      CAST(NULL AS int) AS store_physical_coupons_count,#(lf)      CAST(NULL AS decimal(18,6)) AS store_physical_coupons_total,#(lf)      CAST(NULL AS int) AS store_promos_count,#(lf)      CAST(NULL AS int) AS store_promos_non_taxable_count,#(lf)      CAST(NULL AS decimal(18,6)) AS store_promos_non_taxable_total,#(lf)      CAST(NULL AS int) AS store_promos_taxable_count,#(lf)      CAST(NULL AS decimal(18,6)) AS store_promos_taxable_total,#(lf)      CAST(NULL AS decimal(18,6)) AS store_promos_total,#(lf)      CAST(NULL AS varchar(64)) AS suspended_device_id,#(lf)      CAST(NULL AS bigint) AS suspended_sequence_number,#(lf)      CAST(NULL AS decimal(18,6)) AS tax_total,#(lf)      LTRIM(RTRIM(COALESCE(ot.t1_type_code,'') + CASE WHEN ot.t1_type_code IS NOT NULL AND ot.t2_type_code IS NOT NULL THEN ' ' ELSE '' END + COALESCE(ot.t2_type_code,''))) AS tender_type_codes,#(lf)      TRY_CONVERT(decimal(18,6), ot.t1_amount) AS tender1_amount,#(lf)      ot.t1_auth AS tender1_auth_code,#(lf)      ot.t1_type_code AS tender1_card_type_code,#(lf)      ot.t1_masked AS tender1_masked_card_number,#(lf)      ot.t1_type_code AS tender1_type_code,#(lf)      TRY_CONVERT(decimal(18,6), ot.t2_amount) AS tender2_amount,#(lf)      ot.t2_auth AS tender2_auth_code,#(lf)      ot.t2_type_code AS tender2_card_type_code,#(lf)      ot.t2_masked AS tender2_masked_card_number,#(lf)      ot.t2_type_code AS tender2_type_code,#(lf)      CAST(NULL AS decimal(18,6)) AS tender3_amount,#(lf)      CAST(NULL AS varchar(64)) AS tender3_auth_code,#(lf)      CAST(NULL AS varchar(64)) AS tender3_card_type_code,#(lf)      CAST(NULL AS varchar(64)) AS tender3_masked_card_number,#(lf)      CAST(NULL AS varchar(64)) AS tender3_type_code,#(lf)      CAST(NULL AS decimal(18,6)) AS tender4_amount,#(lf)      CAST(NULL AS varchar(64)) AS tender4_auth_code,#(lf)      CAST(NULL AS varchar(64)) AS tender4_card_type_code,#(lf)      CAST(NULL AS varchar(64)) AS tender4_masked_card_number,#(lf)      CAST(NULL AS varchar(64)) AS tender4_type_code,#(lf)      CAST(NULL AS decimal(18,6)) AS tender5_amount,#(lf)      CAST(NULL AS varchar(64)) AS tender5_auth_code,#(lf)      CAST(NULL AS varchar(64)) AS tender5_card_type_code,#(lf)      CAST(NULL AS varchar(64)) AS tender5_masked_card_number,#(lf)      CAST(NULL AS varchar(64)) AS tender5_type_code,#(lf)      CAST(NULL AS varchar(64)) AS till_id,#(lf)      CAST(NULL AS decimal(18,6)) AS total,#(lf)      CAST(NULL AS int) AS total_physical_coupons_count,#(lf)      CAST(NULL AS bit) AS training_mode,#(lf)      CAST(NULL AS varchar(64)) AS trans_status_code,#(lf)      CAST(NULL AS int) AS transaction_duration_in_sec,#(lf)      CAST(NULL AS varchar(64)) AS username,#(lf)      CAST(NULL AS bit) AS voidable_flag,#(lf)      CAST(NULL AS bigint) AS voided_sequence_number,#(lf)      CAST(NULL AS varchar(64)) AS trans_type_code,#(lf)      CAST(NULL AS varchar(64)) AS reason_code,#(lf)      CAST('OMS' AS varchar(8)) AS source#(lf)  FROM oms_header oh#(lf)  LEFT JOIN oms_tenders ot#(lf)    ON ot.OrderID = oh.OrderID#(lf))#(lf)SELECT * FROM pos_base#(lf)UNION ALL#(lf)SELECT * FROM oms_base;"]),
#"Merged Columns | Transaction Key" =
    Table.CombineColumns(
        Table.TransformColumns(
            Source,
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos AS (#(lf)  SELECT#(lf)      CAST(device_id AS varchar(64))                                        AS device_id,#(lf)      CAST(business_date AS varchar(8))                                     AS business_date,#(lf)      CAST(sequence_number AS varchar(50))                                  AS sequence_number,#(lf)      CAST(trans_type AS varchar(64))                                       AS trans_type,#(lf)      CAST(trans_status AS varchar(64))                                     AS trans_status,#(lf)      TRY_CONVERT(int, business_unit_id)                                    AS business_unit_id,#(lf)      CAST(username AS varchar(64))                                         AS username,#(lf)      TRY_CONVERT(datetime2(6), begin_time)                                 AS begin_time,#(lf)      TRY_CONVERT(datetime2(6), end_time)                                   AS end_time,#(lf)      TRY_CONVERT(int, local_offset)                                        AS local_offset,#(lf)      TRY_CONVERT(int, client_offset)                                       AS client_offset,#(lf)      TRY_CONVERT(bit, keyed_offline)                                       AS keyed_offline,#(lf)      CAST(override_user_id AS varchar(64))                                 AS override_user_id,#(lf)      CAST(barcode AS varchar(64))                                          AS barcode,#(lf)      TRY_CONVERT(bit, training_mode)                                       AS training_mode,#(lf)      CAST(session_id AS varchar(64))                                       AS session_id,#(lf)      CAST(trans_pin AS varchar(64))                                        AS trans_pin,#(lf)      CAST(till_id AS varchar(64))                                          AS till_id,#(lf)      CAST(app_id AS varchar(64))                                           AS app_id,#(lf)      CAST(app_version AS varchar(64))                                      AS app_version,#(lf)      TRY_CONVERT(datetime2(6), create_time)                                AS create_time,#(lf)      CAST(create_by AS varchar(128))                                       AS create_by,#(lf)      TRY_CONVERT(datetime2(6), last_update_time)                           AS last_update_time,#(lf)      CAST(last_update_by AS varchar(128))                                  AS last_update_by,#(lf)      CAST(suspended_trans_data AS varchar(max))                            AS suspended_trans_data,#(lf)      CAST(bank_bag_number AS varchar(64))                                  AS bank_bag_number,#(lf)      'POS'                                                                 AS source#(lf)  FROM dbo.jumpmind_sls_trans#(lf)  WHERE create_by = 'openpos-sls'#(lf)    AND TRY_CONVERT(date, business_date, 112) >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 months#(lf)),#(lf)oms_root AS (#(lf)  SELECT#(lf)      r.OrderID,#(lf)      r.OrderNumber,#(lf)      r.SiteCode,#(lf)      r.OrderDateUTC,#(lf)      r.DateCreatedUTC,#(lf)      r.DeliveryDate,#(lf)      r.InsertDate,#(lf)      r.UpdateDate,#(lf)      dtt.SiteWarehouseCode#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)  OUTER APPLY (#(lf)    SELECT TOP (1) dtt.SiteWarehouseCode#(lf)    FROM dbo.mulesoft_dynamicstargettrans dtt#(lf)    WHERE dtt.OrderId = r.OrderID#(lf)    ORDER BY TRY_CONVERT(datetime2(7), dtt.ExportCreatedUTC) DESC#(lf)  ) dtt#(lf)  WHERE TRY_CONVERT(date, COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.InsertDate))#(lf)        >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 months#(lf)),#(lf)-- Map site -> InventLocationId#(lf)oms_norm AS (#(lf)  SELECT#(lf)    CASE#(lf)      WHEN COALESCE(NULLIF(CAST(orm.SiteWarehouseCode AS varchar(64)),''), NULLIF(CAST(orm.SiteCode AS varchar(64)),'')) = 'BAB'   THEN '1013'#(lf)      WHEN COALESCE(NULLIF(CAST(orm.SiteWarehouseCode AS varchar(64)),''), NULLIF(CAST(orm.SiteCode AS varchar(64)),'')) = 'BABUK' THEN '2013'#(lf)      WHEN COALESCE(NULLIF(CAST(orm.SiteWarehouseCode AS varchar(64)),''), NULLIF(CAST(orm.SiteCode AS varchar(64)),'')) IS NULL THEN '9999'#(lf)      ELSE '9999'#(lf)    END AS InventLocationIdMapped,#(lf)    orm.*#(lf)  FROM oms_root orm#(lf)),#(lf)/* ==== NEW: Pull item-level status and normalize to POS trans_status ==== */#(lf)-- Scope order items to orders in-range to keep it lean#(lf)oi_scoped AS (#(lf)  SELECT oi.*#(lf)  FROM dbo.mulesoft_deckjsonraw_orderitems oi#(lf)  INNER JOIN oms_root r#(lf)    ON oi.OrderID = r.OrderID      -- <== change if your key differs#(lf)),#(lf)-- Normalize ItemStatusName to POS status set#(lf)oi_norm AS (#(lf)  SELECT#(lf)    oi.OrderID,#(lf)    UPPER(LTRIM(RTRIM(CAST(oi.ItemStatusName AS varchar(128))))) AS ItemStatusName,#(lf)    CASE#(lf)      /* COMPLETED-like */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('COMPLETED','FULFILLED','SHIPPED','DELIVERED','CLOSED') THEN 'COMPLETED'#(lf)#(lf)      /* IN_PROGRESS-like */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('IN_PROGRESS','PROCESSING','PICKING','PACKING','AWAITINGPAYMENT','AWAITINGCAPTURE','PENDING','ORDERCREATED','PAYMENTAUTHORIZED') THEN 'IN_PROGRESS'#(lf)#(lf)      /* SUSPENDED / HOLD */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('SUSPENDED','ONHOLD','PAYMENTREVIEW','FRAUDREVIEW','HOLD') THEN 'SUSPENDED'#(lf)#(lf)      /* SUSPEND_RETRIEVED */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('SUSPEND_RETRIEVED','SUSPENDED_RETRIEVED','HOLDRELEASED') THEN 'SUSPEND_RETRIEVED'#(lf)#(lf)      /* ORPHANED */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('ORPHANED') THEN 'ORPHANED'#(lf)#(lf)      /* CANCELLED */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('CANCELLED','CANCELED','VOIDED') THEN 'CANCELLED'#(lf)#(lf)      /* Fallback */#(lf)      ELSE 'IN_PROGRESS'#(lf)    END AS normalized_status#(lf)  FROM oi_scoped oi#(lf)),#(lf)-- Collapse to an order-level trans_status with precedence#(lf)oms_order_status AS (#(lf)  SELECT#(lf)    OrderID,#(lf)    -- precedence: IN_PROGRESS > SUSPENDED > ORPHANED > SUSPEND_RETRIEVED > CANCELLED > COMPLETED#(lf)    CASE#(lf)      WHEN MAX(CASE WHEN normalized_status = 'IN_PROGRESS'       THEN 1 ELSE 0 END) = 1 THEN 'IN_PROGRESS'#(lf)      WHEN MAX(CASE WHEN normalized_status = 'SUSPENDED'          THEN 1 ELSE 0 END) = 1 THEN 'SUSPENDED'#(lf)      WHEN MAX(CASE WHEN normalized_status = 'ORPHANED'           THEN 1 ELSE 0 END) = 1 THEN 'ORPHANED'#(lf)      WHEN MAX(CASE WHEN normalized_status = 'SUSPEND_RETRIEVED'  THEN 1 ELSE 0 END) = 1 THEN 'SUSPEND_RETRIEVED'#(lf)      WHEN SUM(CASE WHEN normalized_status = 'COMPLETED' THEN 1 ELSE 0 END)#(lf)         = COUNT(*)                                                    THEN 'COMPLETED'#(lf)      WHEN MAX(CASE WHEN normalized_status = 'CANCELLED'          THEN 1 ELSE 0 END) = 1 THEN 'CANCELLED'#(lf)      ELSE 'IN_PROGRESS'#(lf)    END AS trans_status#(lf)  FROM oi_norm#(lf)  GROUP BY OrderID#(lf)),#(lf)oms AS (#(lf)  SELECT#(lf)      CONCAT(InventLocationIdMapped,'-052')                                   AS device_id,#(lf)      CONVERT(varchar(8), TRY_CONVERT(date, COALESCE(orm.OrderDateUTC, orm.DateCreatedUTC, orm.InsertDate)), 112)#(lf)                                                                              AS business_date,#(lf)      CAST(orm.OrderNumber AS varchar(50))                                    AS sequence_number,#(lf)      CAST(NULL AS varchar(64))                                              AS trans_type,#(lf)      CAST(os.trans_status AS varchar(64))                                    AS trans_status,       -- <== was 'COMPLETED'#(lf)      TRY_CONVERT(int, InventLocationIdMapped)                                AS business_unit_id,#(lf)      CAST(NULL AS varchar(64))                                              AS username,#(lf)      TRY_CONVERT(datetime2(6), COALESCE(orm.OrderDateUTC, orm.DateCreatedUTC, orm.InsertDate))#(lf)                                                                              AS begin_time,#(lf)      TRY_CONVERT(datetime2(6), COALESCE(orm.DeliveryDate, orm.UpdateDate, orm.OrderDateUTC, orm.DateCreatedUTC))#(lf)                                                                              AS end_time,#(lf)      CAST(0 AS int)                                                         AS local_offset,#(lf)      CAST(0 AS int)                                                         AS client_offset,#(lf)      CAST(0 AS bit)                                                         AS keyed_offline,#(lf)      CAST(NULL AS varchar(64))                                              AS override_user_id,#(lf)      CAST(NULL AS varchar(64))                                              AS barcode,#(lf)      CAST(0 AS bit)                                                         AS training_mode,#(lf)      CAST(NULL AS varchar(64))                                              AS session_id,#(lf)      CAST(NULL AS varchar(64))                                              AS trans_pin,#(lf)      CAST('052' AS varchar(64))                                             AS till_id,#(lf)      CAST(NULL AS varchar(64))                                              AS app_id,#(lf)      CAST(NULL AS varchar(64))                                              AS app_version,#(lf)      TRY_CONVERT(datetime2(6), COALESCE(orm.DateCreatedUTC, orm.InsertDate)) AS create_time,#(lf)      CAST(orm.SiteCode AS varchar(128))                                     AS create_by,#(lf)      TRY_CONVERT(datetime2(6), COALESCE(orm.UpdateDate, orm.DateCreatedUTC, orm.OrderDateUTC, orm.InsertDate))#(lf)                                                                              AS last_update_time,#(lf)      CAST(NULL AS varchar(128))                                             AS last_update_by,#(lf)      CAST(NULL AS varchar(max))                                             AS suspended_trans_data,#(lf)      CAST(NULL AS varchar(64))                                              AS bank_bag_number,#(lf)      'OMS'                                                                  AS source#(lf)  FROM oms_norm orm#(lf)  LEFT JOIN oms_order_status os#(lf)    ON os.OrderID = orm.OrderID#(lf))#(lf)SELECT * FROM pos#(lf)UNION ALL#(lf)SELECT * FROM oms;#(lf)WITH pos AS (#(lf)  SELECT#(lf)      CAST(device_id AS varchar(64))                                        AS device_id,#(lf)      CAST(business_date AS varchar(8))                                     AS business_date,#(lf)      CAST(sequence_number AS varchar(50))                                  AS sequence_number,#(lf)      CAST(trans_type AS varchar(64))                                       AS trans_type,#(lf)      CAST(trans_status AS varchar(64))                                     AS trans_status,#(lf)      TRY_CONVERT(int, business_unit_id)                                    AS business_unit_id,#(lf)      CAST(username AS varchar(64))                                         AS username,#(lf)      TRY_CONVERT(datetime2(6), begin_time)                                 AS begin_time,#(lf)      TRY_CONVERT(datetime2(6), end_time)                                   AS end_time,#(lf)      TRY_CONVERT(int, local_offset)                                        AS local_offset,#(lf)      TRY_CONVERT(int, client_offset)                                       AS client_offset,#(lf)      TRY_CONVERT(bit, keyed_offline)                                       AS keyed_offline,#(lf)      CAST(override_user_id AS varchar(64))                                 AS override_user_id,#(lf)      CAST(barcode AS varchar(64))                                          AS barcode,#(lf)      TRY_CONVERT(bit, training_mode)                                       AS training_mode,#(lf)      CAST(session_id AS varchar(64))                                       AS session_id,#(lf)      CAST(trans_pin AS varchar(64))                                        AS trans_pin,#(lf)      CAST(till_id AS varchar(64))                                          AS till_id,#(lf)      CAST(app_id AS varchar(64))                                           AS app_id,#(lf)      CAST(app_version AS varchar(64))                                      AS app_version,#(lf)      TRY_CONVERT(datetime2(6), create_time)                                AS create_time,#(lf)      CAST(create_by AS varchar(128))                                       AS create_by,#(lf)      TRY_CONVERT(datetime2(6), last_update_time)                           AS last_update_time,#(lf)      CAST(last_update_by AS varchar(128))                                  AS last_update_by,#(lf)      CAST(suspended_trans_data AS varchar(max))                            AS suspended_trans_data,#(lf)      CAST(bank_bag_number AS varchar(64))                                  AS bank_bag_number,#(lf)      'POS'                                                                 AS source#(lf)  FROM dbo.jumpmind_sls_trans#(lf)  WHERE create_by = 'openpos-sls'#(lf)    AND TRY_CONVERT(date, business_date, 112) >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 months#(lf)),#(lf)oms_root AS (#(lf)  SELECT#(lf)      r.OrderID,#(lf)      r.OrderNumber,#(lf)      r.SiteCode,#(lf)      r.OrderDateUTC,#(lf)      r.DateCreatedUTC,#(lf)      r.DeliveryDate,#(lf)      r.InsertDate,#(lf)      r.UpdateDate,#(lf)      dtt.SiteWarehouseCode#(lf)  FROM dbo.mulesoft_deckjsonraw_root r#(lf)  OUTER APPLY (#(lf)    SELECT TOP (1) dtt.SiteWarehouseCode#(lf)    FROM dbo.mulesoft_dynamicstargettrans dtt#(lf)    WHERE dtt.OrderId = r.OrderID#(lf)    ORDER BY TRY_CONVERT(datetime2(7), dtt.ExportCreatedUTC) DESC#(lf)  ) dtt#(lf)  WHERE TRY_CONVERT(date, COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.InsertDate))#(lf)        >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 months#(lf)),#(lf)-- Map site -> InventLocationId#(lf)oms_norm AS (#(lf)  SELECT#(lf)    CASE#(lf)      WHEN COALESCE(NULLIF(CAST(orm.SiteWarehouseCode AS varchar(64)),''), NULLIF(CAST(orm.SiteCode AS varchar(64)),'')) = 'BAB'   THEN '1013'#(lf)      WHEN COALESCE(NULLIF(CAST(orm.SiteWarehouseCode AS varchar(64)),''), NULLIF(CAST(orm.SiteCode AS varchar(64)),'')) = 'BABUK' THEN '2013'#(lf)      WHEN COALESCE(NULLIF(CAST(orm.SiteWarehouseCode AS varchar(64)),''), NULLIF(CAST(orm.SiteCode AS varchar(64)),'')) IS NULL THEN '9999'#(lf)      ELSE '9999'#(lf)    END AS InventLocationIdMapped,#(lf)    orm.*#(lf)  FROM oms_root orm#(lf)),#(lf)/* ==== NEW: Pull item-level status and normalize to POS trans_status ==== */#(lf)-- Scope order items to orders in-range to keep it lean#(lf)oi_scoped AS (#(lf)  SELECT oi.*#(lf)  FROM dbo.mulesoft_deckjsonraw_orderitems oi#(lf)  INNER JOIN oms_root r#(lf)    ON oi.OrderID = r.OrderID      -- <== change if your key differs#(lf)),#(lf)-- Normalize ItemStatusName to POS status set#(lf)oi_norm AS (#(lf)  SELECT#(lf)    oi.OrderID,#(lf)    UPPER(LTRIM(RTRIM(CAST(oi.ItemStatusName AS varchar(128))))) AS ItemStatusName,#(lf)    CASE#(lf)      /* COMPLETED-like */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('COMPLETED','FULFILLED','SHIPPED','DELIVERED','CLOSED') THEN 'COMPLETED'#(lf)#(lf)      /* IN_PROGRESS-like */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('IN_PROGRESS','PROCESSING','PICKING','PACKING','AWAITINGPAYMENT','AWAITINGCAPTURE','PENDING','ORDERCREATED','PAYMENTAUTHORIZED') THEN 'IN_PROGRESS'#(lf)#(lf)      /* SUSPENDED / HOLD */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('SUSPENDED','ONHOLD','PAYMENTREVIEW','FRAUDREVIEW','HOLD') THEN 'SUSPENDED'#(lf)#(lf)      /* SUSPEND_RETRIEVED */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('SUSPEND_RETRIEVED','SUSPENDED_RETRIEVED','HOLDRELEASED') THEN 'SUSPEND_RETRIEVED'#(lf)#(lf)      /* ORPHANED */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('ORPHANED') THEN 'ORPHANED'#(lf)#(lf)      /* CANCELLED */#(lf)      WHEN UPPER(oi.ItemStatusName) IN ('CANCELLED','CANCELED','VOIDED') THEN 'CANCELLED'#(lf)#(lf)      /* Fallback */#(lf)      ELSE 'IN_PROGRESS'#(lf)    END AS normalized_status#(lf)  FROM oi_scoped oi#(lf)),#(lf)-- Collapse to an order-level trans_status with precedence#(lf)oms_order_status AS (#(lf)  SELECT#(lf)    OrderID,#(lf)    -- precedence: IN_PROGRESS > SUSPENDED > ORPHANED > SUSPEND_RETRIEVED > CANCELLED > COMPLETED#(lf)    CASE#(lf)      WHEN MAX(CASE WHEN normalized_status = 'IN_PROGRESS'       THEN 1 ELSE 0 END) = 1 THEN 'IN_PROGRESS'#(lf)      WHEN MAX(CASE WHEN normalized_status = 'SUSPENDED'          THEN 1 ELSE 0 END) = 1 THEN 'SUSPENDED'#(lf)      WHEN MAX(CASE WHEN normalized_status = 'ORPHANED'           THEN 1 ELSE 0 END) = 1 THEN 'ORPHANED'#(lf)      WHEN MAX(CASE WHEN normalized_status = 'SUSPEND_RETRIEVED'  THEN 1 ELSE 0 END) = 1 THEN 'SUSPEND_RETRIEVED'#(lf)      WHEN SUM(CASE WHEN normalized_status = 'COMPLETED' THEN 1 ELSE 0 END)#(lf)         = COUNT(*)                                                    THEN 'COMPLETED'#(lf)      WHEN MAX(CASE WHEN normalized_status = 'CANCELLED'          THEN 1 ELSE 0 END) = 1 THEN 'CANCELLED'#(lf)      ELSE 'IN_PROGRESS'#(lf)    END AS trans_status#(lf)  FROM oi_norm#(lf)  GROUP BY OrderID#(lf)),#(lf)oms AS (#(lf)  SELECT#(lf)      CONCAT(InventLocationIdMapped,'-052')                                   AS device_id,#(lf)      CONVERT(varchar(8), TRY_CONVERT(date, COALESCE(orm.OrderDateUTC, orm.DateCreatedUTC, orm.InsertDate)), 112)#(lf)                                                                              AS business_date,#(lf)      CAST(orm.OrderNumber AS varchar(50))                                    AS sequence_number,#(lf)      CAST(NULL AS varchar(64))                                              AS trans_type,#(lf)      CAST(os.trans_status AS varchar(64))                                    AS trans_status,       -- <== was 'COMPLETED'#(lf)      TRY_CONVERT(int, InventLocationIdMapped)                                AS business_unit_id,#(lf)      CAST(NULL AS varchar(64))                                              AS username,#(lf)      TRY_CONVERT(datetime2(6), COALESCE(orm.OrderDateUTC, orm.DateCreatedUTC, orm.InsertDate))#(lf)                                                                              AS begin_time,#(lf)      TRY_CONVERT(datetime2(6), COALESCE(orm.DeliveryDate, orm.UpdateDate, orm.OrderDateUTC, orm.DateCreatedUTC))#(lf)                                                                              AS end_time,#(lf)      CAST(0 AS int)                                                         AS local_offset,#(lf)      CAST(0 AS int)                                                         AS client_offset,#(lf)      CAST(0 AS bit)                                                         AS keyed_offline,#(lf)      CAST(NULL AS varchar(64))                                              AS override_user_id,#(lf)      CAST(NULL AS varchar(64))                                              AS barcode,#(lf)      CAST(0 AS bit)                                                         AS training_mode,#(lf)      CAST(NULL AS varchar(64))                                              AS session_id,#(lf)      CAST(NULL AS varchar(64))                                              AS trans_pin,#(lf)      CAST('052' AS varchar(64))                                             AS till_id,#(lf)      CAST(NULL AS varchar(64))                                              AS app_id,#(lf)      CAST(NULL AS varchar(64))                                              AS app_version,#(lf)      TRY_CONVERT(datetime2(6), COALESCE(orm.DateCreatedUTC, orm.InsertDate)) AS create_time,#(lf)      CAST(orm.SiteCode AS varchar(128))                                     AS create_by,#(lf)      TRY_CONVERT(datetime2(6), COALESCE(orm.UpdateDate, orm.DateCreatedUTC, orm.OrderDateUTC, orm.InsertDate))#(lf)                                                                              AS last_update_time,#(lf)      CAST(NULL AS varchar(128))                                             AS last_update_by,#(lf)      CAST(NULL AS varchar(max))                                             AS suspended_trans_data,#(lf)      CAST(NULL AS varchar(64))                                              AS bank_bag_number,#(lf)      'OMS'                                                                  AS source#(lf)  FROM oms_norm orm#(lf)  LEFT JOIN oms_order_status os#(lf)    ON os.OrderID = orm.OrderID#(lf))#(lf)SELECT * FROM pos#(lf)UNION ALL#(lf)SELECT * FROM oms;#(lf)"]),
    ...
    #"Merged Columns | Transaction Key" =
  Table.CombineColumns(
    Table.TransformColumnTypes(#"Duplicated Column | Sequence Number", {{"sequence_number", type text}}, "en-US"),
    {"device_id","business_date","sequence_number"},
    Combiner.CombineTextByDelimiter("-", QuoteStyle.None),
    "Transaction Key"
),
...
```sql
let
    Source = Sql.Database("4db76rlxaxcuvmuh5kw37wbnqq-ovsykae43znuhlmnflcdwm4ohu.datawarehouse.fabric.microsoft.com", "LH_Source", [Query="WITH pos_giftcards AS (#(lf)    SELECT#(lf)        CONVERT(varchar(32),  j.device_id)                                         AS device_id,#(lf)        CONVERT(varchar(8),   j.business_date)                                     AS business_date,#(lf)        CONVERT(varchar(50),  j.sequence_number)                                   AS sequence_number,#(lf)        CONVERT(varchar(50),  j.line_sequence_number)                              AS line_sequence_number,#(lf)        CONVERT(varchar(200), j.brand)                                             AS brand,#(lf)        CONVERT(varchar(200), j.card_name)                                         AS card_name,#(lf)        CONVERT(varchar(200), j.code)                                              AS code,#(lf)        CONVERT(varchar(200), j.type_code)                                         AS type_code,#(lf)        CONVERT(varchar(200), j.payment_provider_code)                             AS payment_provider_code,#(lf)        CONVERT(varchar(64),  j.masked_card_number)                                AS masked_card_number,#(lf)        CONVERT(varchar(64),  j.entry_mode)                                        AS entry_mode,#(lf)        CONVERT(varchar(64),  j.service_code)                                      AS service_code,#(lf)        CONVERT(varchar(16),  j.expiration_date)                                   AS expiration_date,#(lf)        CONVERT(varchar(50),  j.ref_line_sequence_number)                          AS ref_line_sequence_number,#(lf)        CONVERT(varchar(64),  j.card_number)                                       AS card_number,#(lf)        CONVERT(varchar(64),  j.gift_card_action_code)                             AS gift_card_action_code,#(lf)        CAST(j.last_update_time AS datetime2(6))                                   AS last_update_time#(lf)    FROM dbo.jumpmind_sls_card_line_item AS j#(lf)    WHERE j.type_code = 'GIFTCARD'#(lf)      AND TRY_CONVERT(date, j.business_date, 112) >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 months#(lf)),#(lf)hs AS (#(lf)    SELECT#(lf)        COALESCE(#(lf)            NULLIF(CONVERT(varchar(64), dtt.SiteWarehouseCode), ''),#(lf)            NULLIF(CONVERT(varchar(64), dtt.MaxWarehouseCode), ''),#(lf)            NULLIF(CONVERT(varchar(64), r.SiteCode), '')#(lf)        )                                                AS InventLocationId,#(lf)        CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS date) AS TransDate,#(lf)        CONVERT(varchar(64), r.OrderNumber)              AS OrderNumber,#(lf)        r.OrderID,#(lf)        r._RowIndex#(lf)    FROM dbo.mulesoft_deckjsonraw_root r#(lf)    LEFT JOIN dbo.mulesoft_dynamicstargettrans dtt#(lf)      ON CONVERT(varchar(64), dtt.OrderId) = CONVERT(varchar(64), r.OrderID)#(lf)    WHERE CAST(COALESCE(r.OrderDateUTC, r.DateCreatedUTC, r.OrderStatusChangeDateUTC, r.ExportCreatedUTC) AS date)#(lf)          >= DATEADD(month, -2, CAST(GETDATE() AS date))                                                   -- 2 months#(lf)),#(lf)hs_norm AS (#(lf)  SELECT#(lf)    CASE#(lf)      WHEN hs.InventLocationId = 'BAB'   THEN '1013'#(lf)      WHEN hs.InventLocationId = 'BABUK' THEN '2013'#(lf)      WHEN NULLIF(hs.InventLocationId,'') IS NULL THEN '9999'#(lf)      ELSE '9999'#(lf)    END AS InventLocationIdMapped,#(lf)    hs.*#(lf)  FROM hs#(lf)),#(lf)oms_giftcards AS (#(lf)    SELECT#(lf)        CONCAT(COALESCE(hn.InventLocationIdMapped,'9999'), '-052')                  AS device_id,#(lf)        CONVERT(varchar(8), hn.TransDate, 112)                                      AS business_date,#(lf)        CONVERT(varchar(50),#(lf)            COALESCE(#(lf)                TRY_CONVERT(bigint, hn.OrderNumber),#(lf)                TRY_CONVERT(bigint, hn.OrderID),#(lf)                ABS(CHECKSUM(CAST(hn.OrderNumber AS nvarchar(4000))))#(lf)            )#(lf)        )                                                                             AS sequence_number,#(lf)        CONVERT(varchar(50), 0)                                                      AS line_sequence_number,#(lf)        CAST(NULL AS varchar(200))                                                   AS brand,#(lf)        CAST(NULL AS varchar(200))                                                   AS card_name,#(lf)        'GIFTCARD'                                                                   AS code,#(lf)        'GIFTCARD'                                                                   AS type_code,#(lf)        CAST(NULL AS varchar(200))                                                   AS payment_provider_code,#(lf)        CASE#(lf)          WHEN g.GiftCardNumber IS NOT NULL#(lf)            THEN REPLICATE('*', 12) + RIGHT(CONVERT(varchar(32), g.GiftCardNumber), 4)#(lf)          ELSE NULL#(lf)        END                                                                          AS masked_card_number,#(lf)        CAST(NULL AS varchar(64))                                                    AS entry_mode,#(lf)        CAST(NULL AS varchar(64))                                                    AS service_code,#(lf)        CAST(NULL AS varchar(16))                                                    AS expiration_date,#(lf)        CAST(NULL AS varchar(50))                                                    AS ref_line_sequence_number,#(lf)        CONVERT(varchar(64), g.GiftCardNumber)                                       AS card_number,#(lf)        CASE WHEN TRY_CONVERT(int, g.Processed) = 1 THEN 'Issue' ELSE NULL END       AS gift_card_action_code,#(lf)        CAST(COALESCE(TRY_CONVERT(datetime2(6), g.UpdateDate),#(lf)                      TRY_CONVERT(datetime2(6), g.InsertDate)) AS datetime2(6))      AS last_update_time#(lf)    FROM dbo.mulesoft_deckjsonraw_giftcards g#(lf)    JOIN hs_norm hn#(lf)      ON CONVERT(varchar(64), g.OrderTransactionIdentifier) = CONVERT(varchar(64), hn._RowIndex)#(lf)    WHERE TRY_CONVERT(date, COALESCE(g.UpdateDate, g.InsertDate)) >= DATEADD(month, -2, CAST(GETDATE() AS date))  -- 2 months#(lf))#(lf)SELECT * FROM pos_giftcards#(lf)UNION ALL#(lf)SELECT * FROM oms_giftcards;", CreateNavigationProperties=false]),
    #"Filtered Rows | Type Code = GIFTCARD" = Table.SelectRows(Source, each ([type_code] = "GIFTCARD")),
    #"Merged Columns | Transaction Line Key" = Table.CombineColumns(Table.TransformColumnTypes(#"Filtered Rows | Type Code = GIFTCARD", {{"sequence_number", type text}, {"ref_line_sequence_number", type text}}, "en-US"),{"device_id", "business_date", "sequence_number"},Combiner.CombineTextByDelimiter("-", QuoteStyle.None),"Transaction Line Key"),
