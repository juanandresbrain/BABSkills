...
FROM [ReportServerBIRPT02TempDB].dbo.SessionData
WHERE 0 = 1;
...
begin
  ...
  from [ReportServerBIRPT02TempDB].dbo.SessionLock sl with(rowlock, readpast)
  join [ReportServerBIRPT02TempDB].dbo.SessionData s with(readpast) on sl.SessionID = s.SessionID
  ...
  delete top(20) l
  from [ReportServerBIRPT02TempDB].dbo.SessionData l
...
begin
  ...
  from [ReportServerBIRPT02TempDB].dbo.SessionData s with(readpast)
  left join [ReportServerBIRPT02TempDB].dbo.SessionLock sl with(nolock) on sl.SessionID = s.SessionID
...
delete top(20) ps
from [ReportServerBIRPT02TempDB].dbo.PersistedStream as ps
