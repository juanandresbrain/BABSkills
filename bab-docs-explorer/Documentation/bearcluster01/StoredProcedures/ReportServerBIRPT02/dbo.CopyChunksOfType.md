...
IF @FromIsPermanent != 0 AND @ToIsPermanent = 0 BEGIN
    ...
    INSERT INTO [ReportServerBIRPT02TempDB].dbo.ChunkData
    ...
    NOT EXISTS(
        SELECT T.ChunkName
        FROM [ReportServerBIRPT02TempDB].dbo.ChunkData AS T -- exclude the ones in the target
        WHERE
    ...
    select @Machine = Machine from [ReportServerBIRPT02TempDB].dbo.SnapshotData SD where SD.SnapshotDataID = @ToSnapshotID

    INSERT INTO [ReportServerBIRPT02TempDB].dbo.SegmentedChunk
    ...
    WHERE
        ...
        NOT EXISTS(
            ...
            FROM [ReportServerBIRPT02TempDB].dbo.SegmentedChunk TSC
            -- JOIN [ReportServerBIRPT02TempDB].dbo.SnapshotChunkMapping TSCM ON (TSC.ChunkId = TSCM.ChunkId)
            WHERE
 ...
 END ELSE IF @FromIsPermanent = 0 AND @ToIsPermanent = 0 BEGIN
    ...
    select @Machine = Machine from [ReportServerBIRPT02TempDB].dbo.SnapshotData SD where SD.SnapshotDataID = @FromSnapshotID
    ...
    SELECT
        newid(), @ToSnapshotID, COALESCE(@TargetChunkName, S.ChunkName), S.ChunkType, S.MimeType, S.Version, S.ChunkFlags, S.Content
    FROM
        [ReportServerBIRPT02TempDB].dbo.ChunkData AS S
    WHERE
        S.SnapshotDataID = @FromSnapshotID AND
        (S.ChunkType = @ChunkType OR @ChunkType IS NULL) AND
        (S.ChunkName = @ChunkName OR @ChunkName IS NULL) AND
    ...
    FROM [ReportServerBIRPT02TempDB].dbo.SegmentedChunk C WITH(INDEX (UNIQ_SnapshotChunkMapping))
    WHERE	C.SnapshotDataId = @FromSnapshotID AND
            ...
            NOT EXISTS(
                ...
                SELECT T.ChunkId
                FROM [ReportServerBIRPT02TempDB].dbo.SegmentedChunk T
...
END ELSE IF @FromIsPermanent = 0 AND @ToIsPermanent != 0 BEGIN
    ...
    WHERE
     ...
     FROM [ReportServerBIRPT02TempDB].dbo.ChunkSegmentMapping CM
     ...
     FROM [ReportServerBIRPT02TempDB].dbo.Segment CS
