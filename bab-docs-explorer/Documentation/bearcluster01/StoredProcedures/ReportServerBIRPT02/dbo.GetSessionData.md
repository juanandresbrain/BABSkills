...
FROM
    [ReportServerBIRPT02TempDB].dbo.SessionData AS SE WITH (XLOCK)
WHERE
...
FROM
    [ReportServerBIRPT02TempDB].dbo.SessionData AS SE
    INNER JOIN [ReportServerBIRPT02TempDB].dbo.SnapshotData AS SN ON SN.SnapshotDataID = SE.SnapshotDataID
    LEFT OUTER JOIN [ReportServerBIRPT02TempDB].dbo.SnapshotData AS COMP ON SE.CompiledDefinition = COMP.SnapshotDataID
WHERE
   SE.SessionID = @DBSessionID
