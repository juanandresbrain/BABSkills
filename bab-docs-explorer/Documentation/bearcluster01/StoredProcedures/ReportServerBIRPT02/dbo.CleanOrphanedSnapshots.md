...
AS
    ...
    begin
        ...
        and not exists (
            select 1 from [ReportServerBIRPT02TempDB].dbo.SegmentedChunk TSC with(nolock)
    ...
    begin transaction
    DELETE TOP (@TemporarySnapshotCount) [ReportServerBIRPT02TempDB].dbo.SnapshotData
    output deleted.SnapshotDataID into @cleanedSnapshots(SnapshotDataId)
    FROM [ReportServerBIRPT02TempDB].dbo.SnapshotData with(readpast)
    WHERE   [ReportServerBIRPT02TempDB].dbo.SnapshotData.PermanentRefCount <= 0 AND
            [ReportServerBIRPT02TempDB].dbo.SnapshotData.TransientRefCount <= 0 AND
            [ReportServerBIRPT02TempDB].dbo.SnapshotData.Machine = @Machine ;
    SET @SnapshotsCleaned = @SnapshotsCleaned + @@ROWCOUNT ;

    DELETE [ReportServerBIRPT02TempDB].dbo.ChunkData FROM [ReportServerBIRPT02TempDB].dbo.ChunkData with (readpast)
    INNER JOIN @cleanedSnapshots cs
    ON [ReportServerBIRPT02TempDB].dbo.ChunkData.SnapshotDataID = cs.SnapshotDataId
    ...
    begin
        delete SC
        output deleted.ChunkId into @cleanedChunks(ChunkId)
        from [ReportServerBIRPT02TempDB].dbo.SegmentedChunk SC with (readpast)
        join @cleanedSnapshots cs on SC.SnapshotDataId = cs.SnapshotDataId ;
        set @deleteCount = @@ROWCOUNT ;
        set @ChunksCleaned =  @ChunksCleaned + @deleteCount ;
    end ;
    ...
    begin
        ...
        output deleted.ChunkId, deleted.SegmentId into @cleanedSegments (ChunkId, SegmentId)
        from [ReportServerBIRPT02TempDB].dbo.ChunkSegmentMapping CSM with (readpast)
    ...
    begin
        ...
        from [ReportServerBIRPT02TempDB].dbo.Segment S with (readpast)
        join @cleanedSegments cs on S.SegmentId = cs.SegmentId
        where not exists (
