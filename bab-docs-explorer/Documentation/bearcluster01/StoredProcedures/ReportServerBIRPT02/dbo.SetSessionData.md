...
FROM [ReportServerBIRPT02TempDB].dbo.SessionData WITH (XLOCK)
...
BEGIN -- Yes, update it
   ...
   UPDATE
      [ReportServerBIRPT02TempDB].dbo.SessionData
   SET
   ...
   END ELSE BEGIN
      UPDATE
         [ReportServerBIRPT02TempDB].dbo.SnapshotData
      SET
         ExpirationDate = DATEADD(n, @SnapshotTimeoutSeconds, @now)
      WHERE
         SnapshotDataID = @SnapshotDataID
...
BEGIN -- no, insert it
   UPDATE PS
    ...
    INSERT INTO [ReportServerBIRPT02TempDB].dbo.SessionData
