...
flowchart LR
    ...
    dbo_vwCurrentForgetMeStatus_V1_1(["dbo.vwCurrentForgetMeStatus_V1_1"]) --> SP
```
...
If (0 < (Select Count(*) FROM [BABWForgetMe].[dbo].[ActionStatus] A		
			...
			AND (SELECT DATEDIFF(d, getutcdate(), DATEADD(DAY, 30, ForgetMeAdminValidationDate))) = 29))
	   ...
	   + SUBSTRING([EmailAddress], CHARINDEX('@',[EmailAddress])+1,(LEN([EmailAddress]) - CHARINDEX('@',[EmailAddress])+1))
		...
		FROM 
		(SELECT *  
		FROM BABWForgetMe.dbo.vwCurrentForgetMeStatus_V1_1 where CompletionDate IS NULL
		AND (SELECT DATEDIFF(d, getutcdate(), DATEADD(DAY, 30, ForgetMeAdminValidationDate))) = 29) tab
