# WM.spRptWebOrderLookup_ForStorForce
...
```mermaid
flowchart LR
    SP["WM.spRptWebOrderLookup_ForStorForce"]
    dbo_a(["dbo.a"]) --> SP
...
```sql
CREATE proc [WM].[spRptWebOrderLookup_ForStorForce]
...
select	
	...
	(select sum(st1.Price)
		...
		--and st.ShipmentNumber=st1.ShipmentNumber
		and st.OrderNumber=st1.OrderNumber
		and st1.sku not in (select StyleCode from #TaxExempt)
	) as PriceTotalForTax,
	(select sum(st1.SubTotal) 
		from #SubTotalsPre st1 
		where st.TransactionID=st1.TransactionID
...
select
	...
	--	END AS ItemLevelTax
	CASE
			WHEN SKU in (select StyleCode from #TaxExempt) then 0.00
			WHEN (OrderTax = 0) THEN 0.00
			WHEN (SubTotalTotalForOrder = 0) THEN 0.00
				ELSE CAST(ROUND(((OrderTax / nullif(SubTotalTotalForTax,0)) * SubTotal),4) as Decimal(9,4))
		END AS ItemLevelTax
...
select DISTINCT 
	...
	--ws.Tax, 
	--ws.SubTotal, 
	round(ws.ItemLevelTax, 2) ItemLevelTax,
...
PM as 
	(
		select 
			...
			pm.CardNum,
			sum(pm.Tax) Tax,
			sum(pm.SubTotal) SubTotal,
			sum(pm.Shipping) Shipping,
			sum(pm.TotalCharges) TotalCharges,
...
select DISTINCT 
	...
	pm.CardNum,
	pm.Tax Tax,
	pm.SubTotal SubTotal,
	pm.Shipping Shipping,
	pm.TotalCharges TotalCharges,
	pm.TransactionAmount TransactionAmount,
	(
				select 
...
--where OrderNumber in ('W0469114','W0469113')
--order by OrderNumber

--select *
--from tmpWebSalesForPOSReturnsDetails
--where OrderNumber in ('W0469114','W0469113')
...
dupes as
	(
		...
		from tmpWebSalesForPOSReturnsHeader
		group by OrderNumber, PaymentMethod
		having count(*) >1
	)
--select *
--from tmpWebSalesForPOSReturnsHeader
--where OrderNumber in (select OrderNumber from Dupes)
--order by OrderNumber 
delete dd
from tmpWebSalesForPOSReturnsHeader dd
join dupes d on dd.OrderNumber=d.OrderNumber;


with 
dupes as
	(
		select TransactionID, OrderNumber, OrderItemID 
		from tmpWebSalesForPOSReturnsDetails 
		group by TransactionID, OrderNumber, OrderItemID
		having count(*) >1
	)
--select *
--from tmpWebSalesForPOSReturnsDetails
--where OrderNumber in (select OrderNumber from Dupes)
--order by TransactionID, OrderNumber, OrderItemID
delete dd
from tmpWebSalesForPOSReturnsDetails dd
join dupes d on dd.OrderNumber=d.OrderNumber;
```
