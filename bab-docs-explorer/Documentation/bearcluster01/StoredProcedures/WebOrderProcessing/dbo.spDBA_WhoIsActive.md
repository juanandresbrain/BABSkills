...
```mermaid
flowchart LR
    SP["dbo.spDBA_WhoIsActive"]
    agent_node_value(["agent_node.value"]) --> SP
    dbo_s(["dbo.s"]) --> SP
    dbo_sysjobs(["dbo.sysjobs"]) --> SP
    dbo_sysjobsteps(["dbo.sysjobsteps"]) --> SP
    trans_node_value(["trans_node.value"]) --> SP
```
...
```sql
create PROC [dbo].[spDBA_WhoIsActive]
...
dbo,spFlashGapWeb,CREATE proc spFlashGapWeb 
@StartDate date, 
@EndDate date
...
set nocount on
			...
			where 1=1
			and b.bSentToAW = 1 
			and t.dTimeStamp between @StartDate and @EndDate
		...
		SELECT 
		  ...
		  into #TransactionDetailPayments
...
IF (Object_ID('WebOrderProcessing..tmpFGW') IS NOT NULL) DROP TABLE tmpFGW;
select 
...
dbo,spGetDuplicateSATransactionsByDate,CREATE PROC [dbo].[spGetDuplicateSATransactionsByDate]  

AS

-- Name: spGetDuplicateSATransactionsByDate
--
-- Description:	Emails Sales Audit
-- 
-- Output: email
-- 
-- Available actions: 
...
-- Revision History
--		Name:			Date:			Comments:
--		Ben Barud		10/02/2018		Creation
--		Ben Barud		06/05/2020		Added register number
--		Ben Barud		06/12/2020		Added store number
...
SELECT @recipients = 'benb@buildabear.com; lindak@buildabear.com',
--SELECT @recipients = 'benb@buildabear.com',
@copy_recipients = '',
@subject = 'Duplicate Transactions in Sales Audit',
@MessageTxt = 'The following transactions are duplicates and can be deleted.'
		...
		Set @TableTail = '</table><br><br><br>This email has been generated from [bearcluster01.sql.buildabear.com].[WebOrderProcessing].[dbo].[spGetDuplicateSATransactionsByDate]</body></html>';
		Set @TableHead = '<html><head>' +
						  ...
						  '</head>' +
						  '<body>
						  Please delete these transactions in Sales Audit.
						  <br><br>
						  <table cellpadding=0 cellspacing=0 border=0>' +
						  '<tr bgcolor=#FFEFD8>' +
						  '<td align=center><b>WebOrder</b></td>' +
						  '<td align=center><b>StoreNo</b></td>' +
						  '<td align=center><b>RegisterNo</b></td>' +
						  '<td align=center><b>SATransactionNo</b></td>' +
						  '<td align=center><b>Transaction Date</b></td></tr>';

		WITH SAOrdersByDate (line_note, store_no, register_no, transaction_no, transaction_date)
		AS
		(
		SELECT DISTINCT n.line_note, store_no, register_no, transaction_no, transaction_date 
		FROM [bedrockdb01].auditworks.dbo.uvw_transaction_line l
			,[bedrockdb01].auditworks.dbo.uvw_transaction_header h
			,[bedrockdb01].auditworks.dbo.uvw_line_note n
		WHERE l.transaction_id = h.transaction_id
		AND l.transaction_id = n.transaction_id
		AND h.transaction_date > DATEADD(DAY, -21, CAST(GETDATE() AS DATE))
		AND line_note LIKE 'Web Order: %'
		)
		,
		WebOrderCounts (WebOrderCount, store_no, register_no, WebOrder, TransactionDate)
		AS(
		SELECT COUNT(line_note) AS 'WebOrderCount', store_no, register_no, LTRIM(RIGHT(line_note, 11)) AS 'WebOrder', transaction_date AS TransactionDate 
		FROM SAOrdersByDate
		GROUP BY line_note, store_no, register_no, transaction_date
		)
		,
		SAOrders (WebOrder, SATransactionNo)
		AS
		(
		SELECT LTRIM(RIGHT(line_note, 11)) AS 'WebOrder', transaction_no
		FROM SAOrdersByDate 
		)
		,Keepers (WebOrder, store_no, register_no, SATransactionNo, TransactionDate)
		AS
		(
		SELECT c.WebOrder, store_no, register_no, MIN(s.SATransactionNo) AS 'SATransactionNo', c.TransactionDate
		FROM WebOrderCounts c
		LEFT JOIN SAOrders s ON c.WebOrder = s.WebOrder
		WHERE WebOrderCount > 1
		GROUP BY c.WebOrder, store_no, register_no, c.TransactionDate
		)
		SELECT c.WebOrder, store_no, register_no, s.SATransactionNo, CAST(c.TransactionDate AS DATE) AS 'TransactionDate'
		INTO #tmpTransactions
		FROM WebOrderCounts c
		LEFT JOIN SAOrders s ON c.WebOrder = s.WebOrder
		WHERE WebOrderCount > 1 
		AND s.SATransactionNo NOT IN (SELECT SATransactionNo FROM Keepers)
		ORDER BY c.TransactionDate ASC, SATransactionNo DESC

		WHILE ((SELECT COUNT(*) FROM #tmpTransactions) > 0)
		BEGIN
			Select @Body = (SELECT TOP 100 Row_Number() OVER(ORDER BY WebOrder) % 2 AS [TRRow], WebOrder AS 'TD', store_no AS 'TD', register_no AS 'TD', SATransactionNo AS 'TD', CAST(TransactionDate AS DATE) AS 'TD'
			...
			DELETE FROM #tmpTransactions
			WHERE SATransactionNo IN (SELECT TOP 100 SATransactionNo FROM #tmpTransactions ORDER BY WebOrder)
		END
dbo,spGetDuplicateSATransactionsByDate_All,CREATE PROC [dbo].[spGetDuplicateSATransactionsByDate_All]  
...
--		Ben Barud		06/05/2020		Added register number
--		Ben Barud		06/12/2020		Added store number
--		Ben Barud		09/26/2025		Modified to send all records regarding duplicates
...
--SELECT @recipients = 'benb@buildabear.com',
@copy_recipients = '',
@subject = 'Duplicate Transactions(All) in Sales Audit',
@MessageTxt = 'The following transactions are duplicates and need to be analyzed.'
		...
		Set @TableTail = '</table><br><br><br>This email has been generated from [bearcluster01.sql.buildabear.com].[WebOrderProcessing].[dbo].[spGetDuplicateSATransactionsByDate_All]</body></html>';
		Set @TableHead = '<html><head>' +
						  '<style>' +
						  'td {border: solid black 1px;padding-left:5px;padding-right:5px;padding-top:1px;padding-bottom:1px;font-size:11pt;} ' +
						  '</style>' +
						  '</head>' +
						  '<body>
						  Please analyze these transactions in Sales Audit.
						  <br><br>
						  <table cellpadding=0 cellspacing=0 border=0>' +
						  '<tr bgcolor=#FFEFD8>' +
						  '<td align=center><b>WebOrder</b></td>' +
		...
		BEGIN
			...
			WHERE SATransactionNo IN (SELECT TOP 100 SATransactionNo FROM #tmpTransactions ORDER BY WebOrder)
		END
dbo,spMergeDeckNightlyWaveStatus,CREATE proc [dbo].[spMergeDeckNightlyWaveStatus]

as 

set nocount on 

merge into DeckNightlyWaveStatus as target
using 
	(
		select 
			OrderNumber,
			DeckStatus,
			min(DeckFileName) DeckFileName,
			Country,
			min(StatusDateTime) StatusDateTime
		from DeckNightlyWaveStatusStage
		group by 
			OrderNumber,
			DeckStatus,
			Country
...
on 
	(	
		...
		and
		target.DeckStatus=source.DeckStatus
	)
