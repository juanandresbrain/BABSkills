...
```sql
CREATE PROCEDURE [WM].[spGetShippedWMOrders_V2]
...
BEGIN
	...
	SET NOCOUNT ON;
		...
		SELECT DISTINCT OrderNumber
		  ...
		  ,CASE
		     WHEN SourceSite IS NULL AND LEFT(OrderNumber, 1) = 'W' THEN 'BABW-US'
			 WHEN SourceSite IS NULL AND LEFT(OrderNumber, 1) = '1' AND ShipToCountry = 'GBR' THEN 'BABW-UK'
			 WHEN SourceSite IS NULL AND LEFT(OrderNumber, 1) = '1' THEN 'BABW-US'
			 WHEN SourceSite IS NULL AND LEFT(OrderNumber, 1) = '7' THEN 'BABW-US'
			 WHEN SourceSite IS NULL AND LEFT(OrderNumber, 1) = 'U' THEN 'BABW-UK'
			 ELSE SourceSite
		   END AS 'SourceSite'
		  ,ShippingAmount
		  ,PreviousShipping
		  ,LoyaltyNumber
		  ,CASE
		     WHEN PickupStore IS NULL AND LEFT(OrderNumber, 1) = 'W' THEN '0013'
			 WHEN PickupStore IS NULL AND LEFT(OrderNumber, 1) = '1' AND ShipToCountry = 'GBR' THEN '2013'
			 WHEN PickupStore IS NULL AND LEFT(OrderNumber, 1) = '1' THEN '0013'
			 WHEN PickupStore IS NULL AND LEFT(OrderNumber, 1) = '7' THEN '0013'
			 WHEN PickupStore IS NULL AND LEFT(OrderNumber, 1) = 'U' THEN '2013'
			 ELSE PickupStore
		   END AS 'PickupStore'
	FROM(
	...
	LEFT JOIN [WebOrderProcessing].[WM].[Orders] o ON v.TransactionID = o.TransactionID AND o.OrderStatus IN ('Complete', 'Shipped')
	...
	GROUP BY OrderNumber, SourceSite, ShippingAmount, PreviousShipping, LoyaltyNumber, PickupStore, ShipToCountry
