...
```mermaid
flowchart LR
    SP["WM.spGetShippedWMOrders_V3_1"]
...
```sql
CREATE PROCEDURE [WM].[spGetShippedWMOrders_V3_1]
...
BEGIN
	...
	SELECT o.[OrderNumber]
		  ...
		  ,CASE
		    WHEN v.PaymentTransactionType IN ('sales', 'credit') THEN 0
			ELSE ISNULL(v.[Shipping], 0) 
	...
	SELECT o.[OrderNumber]
		  ...
		  ,v.ShipToCountry
	FROM [WebOrderProcessing].[WM].vwTransactionDetailPayments_V2 v 
	INNER JOIN [WebOrderProcessing].[WM].[tmpOrderOrderTransactionIdentifier] o ON v.TransactionID = o.TransactionID AND o.OrderTransactionIdentifier = -1
	LEFT JOIN [WebOrderProcessing].[WM].[vwSalesAuditShippingForReturn] sv ON v.TransactionID = sv.TransactionID AND sv.OrderTransactionIdentifier = v.TansactionDetailID
	LEFT JOIN [WebOrderProcessing].[WM].[Transactions] t ON v.TransactionID = t.TransactionID
	WHERE v.OmsTransactionType = 'ShippingManualCredit') AS innerQry
