# WM.spUpdateWMItemStatus_to_SalesAuditComplete
...
```mermaid
flowchart LR
    SP["WM.spUpdateWMItemStatus_to_SalesAuditComplete"]
...
```sql
CREATE PROCEDURE [WM].[spUpdateWMItemStatus_to_SalesAuditComplete] 
...
BEGIN
 ...
 --   --INTO #tmpSalesAuditCompleteItems
	...
	UPDATE [WebOrderProcessing].[WM].OMSTransactionDetails 
	SET isSAProcessed = 1
	WHERE TransactionID IN (SELECT DISTINCT TransactionID FROM [WebOrderProcessing].[WM].[vwOrderNumPickupStore] v INNER JOIN [BABWeCommerce].[dbo].[NSBTranslate_LogTrans] l WITH(NOLOCK) ON v.OrderNumber = sOrderNumber) 
