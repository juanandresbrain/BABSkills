...
```mermaid
flowchart LR
    SP["WM.spGetReturnWMOrderPayments"]
    WM_Orders(["WM.Orders"]) --> SP
    WM_vwTransactionDetail(["WM.vwTransactionDetail"]) --> SP
    WM_vwTransactionDetailAll(["WM.vwTransactionDetailAll"]) --> SP
```
...
```sql
CREATE PROCEDURE [WM].[spGetReturnWMOrderPayments] 
...
BEGIN
	...
	(
		...
		FROM [WebOrderProcessing].[WM].[vwTransactionDetailAll]
