# WM.spGetShippedWMOrdersTaxes_V2
...
```mermaid
flowchart LR
    SP["WM.spGetShippedWMOrdersTaxes_V2"]
...
```sql
CREATE PROCEDURE [WM].[spGetShippedWMOrdersTaxes_V2]
...
BEGIN
	...
	SELECT DISTINCT v.[OrderNumber]
	      ,[TansactionDetailID] AS 'OrderTransactionIdentifier'
          ,[Tax] AS 'TaxAmount'
          ,CASE
	...
	(
	SELECT DISTINCT v.[OrderNumber]
	      ,[TansactionDetailID] 'OrderTransactionIdentifier'
          ,[Tax] AS 'TaxAmount'
          ,CASE
		    WHEN [TaxJurisdiction] = 'AP' THEN 'APO'
			ELSE [TaxJurisdiction]
