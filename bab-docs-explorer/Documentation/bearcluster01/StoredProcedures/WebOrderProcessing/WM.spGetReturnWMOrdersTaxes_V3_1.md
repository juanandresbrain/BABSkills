# WM.spGetReturnWMOrdersTaxes_V3_1
...
```mermaid
flowchart LR
    SP["WM.spGetReturnWMOrdersTaxes_V3_1"]
...
```sql
CREATE PROCEDURE [WM].[spGetReturnWMOrdersTaxes_V3_1]
...
BEGIN
  ...
  LEFT JOIN [WebOrderProcessing].[WM].[Transactions] t ON td.TransactionID = t.TransactionID
	...
	SELECT DISTINCT MAX(o.[OrderNumber]) AS 'OrderNumber'
	...
	GROUP BY o.PickupStore, v.TransactionID, v.PaymentTransactionType
