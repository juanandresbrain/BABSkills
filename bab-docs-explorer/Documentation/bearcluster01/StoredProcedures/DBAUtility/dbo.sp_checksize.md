...
```sql
CREATE proc [dbo].[sp_checksize] @dbname sysname = null
as
if @dbname is not null and @dbname not in (select name from master..sysdatabases)
		begin
...
print '' print ''
if @dbname is null
		declare dbname cursor for select name from master..sysdatabases order by name asc
else if @dbname is not null
begin
		delete from #sizeinfo where db_name <> @dbname
		declare dbname cursor for select name from master..sysdatabases where name = @dbname	
end
open dbname
