# dbo.CreateBackUpFromJob_DELETE20141203
...
```mermaid
flowchart LR
    SP["dbo.CreateBackUpFromJob_DELETE20141203"]
    ...
    dbo_sysnotifications(["dbo.sysnotifications"]) --> SP
...
--* , 
    ...
    FROM    msdbMIKE.dbo.sysjobs AS Job
...
--Declare @additional_parameters [ntext] 
    ...
    FOR
        SELECT  JobSteps.*
        FROM    msdbMIKE.dbo.sysjobsteps JobSteps
                INNER JOIN msdbMIKE.dbo.sysjobs Job ON Job.job_id = JobSteps.job_id
        --WHERE   Job.name = @JobName
    OPEN Cur
...
---------------------------------------------------------------------------------------
    ...
    FOR
        ...
        FROM    msdbMIKE.dbo.sysjobschedules AS JobSch
                INNER JOIN msdbMIKE.dbo.sysjobs AS Job ON Job.job_id = JobSch.job_id
                INNER JOIN msdbMIKE.dbo.sysschedules AS Sch ON JobSch.schedule_id =     Sch.schedule_id
        --WHERE   Job.name = @JobName
...
---------------------------------------------------------------------------------------
    ...
    FOR
        SELECT  Command_notifications = ' INSERT INTO [msdb].[dbo].[sysnotifications] '
                ...
                + +') Set @Al = @@IDENTITY '
        FROM    msdbMIKE.dbo.sysalerts AS Ale
                INNER JOIN msdbMIKE.dbo.sysnotifications AS Noti ON ( Ale.id =     Noti.alert_id )
                INNER JOIN msdbMIKE.dbo.sysoperators AS Ope ON ( Noti.operator_id = ope.id     )
                INNER JOIN msdbMIKE.dbo.sysjobs AS Job ON ( Job.job_id = Ale.job_id )
...
END
dbo,DatabaseBackup,CREATE PROCEDURE [dbo].[DatabaseBackup]
...
dbo,DatabaseIntegrityCheck,CREATE PROCEDURE [dbo].[DatabaseIntegrityCheck]
...
dbo,IndexOptimize,CREATE PROCEDURE [dbo].[IndexOptimize]
