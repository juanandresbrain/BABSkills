...
BEGIN
	...
	DECLARE @CurrentCommand05 varchar(max)
		...
		BEGIN
			...
			BEGIN									
				...
				(
					...
					LEFT JOIN tblDBA_FileSize fs ON cf.CurrentFilePath = fs.Directory
					WHERE fs.Directory IS NULL AND fs.Process = @@SPID AND fs.UserName = SYSTEM_USER
				)> 0
				BEGIN
					...
					SET @sbjt = 'ERROR: Job failure of backups on ' + @@SERVERNAME
					EXEC DBAUtility.dbo.spDBA_SendEmail @recipients = 'Databears@buildabear.com', @subject = @sbjt, @MessageTxt = 'The SQL backup job proceedure had an error.  The While Loop in [spDBA_DatabaseBackup] broke.'
					GOTO Logging
