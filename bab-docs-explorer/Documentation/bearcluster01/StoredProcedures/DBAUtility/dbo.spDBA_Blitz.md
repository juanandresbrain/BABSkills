...
AS
	...
	SELECT @Version = '20150216', @VersionDate = '20150216'
		...
		/*Check for collation conflicts between user databases and tempdb */
			...
			END /* IF ( ( SERVERPROPERTY('ServerName') NOT IN ( SELECT ServerName */
				...
				--	BEGIN
						...
						--EXEC(@StringToExecute);
						 INSERT COREDB01_MAINT.[DBAUtilityMaster].[dbo].[tblDBA_SQLBlitzResults] (ServerName, CheckDate, BlitzVersion, CheckID, DatabaseName, Priority, FindingsGroup,
							Finding, URL, Details, QueryPlan, QueryPlanFiltered) 
							SELECT CAST(SERVERPROPERTY('ServerName') AS NVARCHAR(128)), GETDATE(), 39, CheckID, DatabaseName, Priority, FindingsGroup, Finding, URL, Details, 
							QueryPlan, QueryPlanFiltered FROM #BlitzResults 
...
dbo,spDBA_CheckBackups,CREATE PROCEDURE [dbo].[spDBA_CheckBackups]
AS	
