...
```mermaid
flowchart LR
    SP["dbo.sp_agent_get_jobstep"]
    dbo_sp_help_jobstep(["dbo.sp_help_jobstep"]) --> SP
```
...
dbo,sp_agent_log_job_history,CREATE PROCEDURE sp_agent_log_job_history
    @job_id               UNIQUEIDENTIFIER,
    @is_system            TINYINT = 0,
    @step_id              INT,
