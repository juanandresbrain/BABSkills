...
```mermaid
flowchart LR
    VIEW["dbo.vwUpdateWavedOMS_ErrorLog"]
...
```sql
CREATE view [dbo].[vwUpdateWavedOMS_ErrorLog]

as
------------------------------------------------------------------------------------------------------------------------------------
--Dan Tweedie - 2017-09-14 - Returns OrderNumber, ErrorMessage, LogDateTime for orders which errored during update OMS to waved
------------------------------------------------------------------------------------------------------------------------------------

--select 
--	cast(substring(FunctionName,charindex('UpdateWavedOrders|',FunctionName)+18,10) as varchar(10)) as OrderNumber, 
--	Message as ErrorMessage,
--	max(LogCreatedDate) as LogDateTime
--from ServiceLoggingGeneralUsage with (nolock)
--where left(FunctionName,18) = 'UpdateWavedOrders|'
--group by 
--	substring(FunctionName,charindex('UpdateWavedOrders|',FunctionName)+18,10), 
--	Message
...
WITH 
	...
	where datediff(dd, ml.LogDateTime, getdate()) <= 90
```
