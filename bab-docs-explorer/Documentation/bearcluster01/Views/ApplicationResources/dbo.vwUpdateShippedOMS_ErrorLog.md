# dbo.vwUpdateShippedOMS_ErrorLog
...
```mermaid
flowchart LR
    VIEW["dbo.vwUpdateShippedOMS_ErrorLog"]
...
```sql
CREATE view [dbo].[vwUpdateShippedOMS_ErrorLog]

as
------------------------------------------------------------------------------------------------------------------------------------
--Dan Tweedie - 2017-09-14 - Returns OrderNumber, ErrorMessage, LogDateTime for orders which errored during update OMS to shipped
------------------------------------------------------------------------------------------------------------------------------------
WITH 
	ErrorLog as
		(
			select 
				cast(substring(FunctionName,charindex('UpdateShippedOrders|',FunctionName)+20,10) as varchar(10)) as OrderNumber, 
			...
			where left(FunctionName,20) = 'UpdateShippedOrders|'
			--group by 
