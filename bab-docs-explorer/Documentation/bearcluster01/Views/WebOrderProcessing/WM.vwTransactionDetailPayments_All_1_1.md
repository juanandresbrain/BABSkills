...
```mermaid
flowchart LR
    VIEW["WM.vwTransactionDetailPayments_All_1_1"]
    ...
    WM_vwTransactionDetailPayments_All_V2(["WM.vwTransactionDetailPayments_All_V2"]) --> VIEW
```
...
| WM.vwTransactionDetailPayments_All_V2 |

## View Code

```sql
CREATE VIEW [WM].[vwTransactionDetailPayments_All_1_1]
AS
  ...
  AS(SELECT td.TansactionDetailID
           ,ctd.TansactionDetailID
					...
					,CASE 
					  WHEN DATEDIFF(s, ctd.TransactionDate, td.TransactionDate) BETWEEN -1 AND 1 THEN (DATEDIFF(ms, ctd.TransactionDate, td.TransactionDate) / 1000)
					  ELSE DATEDIFF(s, ctd.TransactionDate, td.TransactionDate)
					 END AS 'DateDiff'
                    ,td.[TransactionDate]
					,ctd.[TransactionDate]
                    ,td.[SubTotal]
...
FROM
(
  SELECT DISTINCT TansactionDetailID
                    ,TransactionNum
                    ,CASE
  ...
  WHERE (OrderItemCount > 0 AND PaymentTransactionType IN ('sales', 'return')) OR PaymentTransactionType IN ('credit')
  AND TransactionDate < '2020-07-15 09:00:00'
  ...
  SELECT [TansactionDetailID]
      ...
      ,[isSAProcessed]
	  ,-1 AS 'OrderItemCount'
  FROM [WebOrderProcessing].[WM].[vwTransactionDetailPayments_All_V2] v1
  INNER JOIN WebOrderProcessing.WM.vwOrderOrderTransactionIdentifier v2 ON v2.TransactionID = v1.TransactionID AND v2.OrderTransactionIdentifier = v1.OrderTransactionIdentifier
  WHERE TransactionDate >= '2020-07-15 09:00:00'
  )AS innerQry
  ORDER BY TransactionDate, OrderNumber
```
