# WM.vwTransactionDetailPayments_DMT
...
```mermaid
flowchart LR
    VIEW["WM.vwTransactionDetailPayments_DMT"]
...
```sql
CREATE VIEW [WM].[vwTransactionDetailPayments_DMT]
AS
WITH OrderItemsCount([TransactionID]
  ...
  AND isSAProcessed = 1 
  and datediff(dd, TransactionDate, getdate()) <= 60
  ...
  where td.isSAProcessed = 1
  and datediff(dd, td.TransactionDate, getdate()) <= 60
  )
