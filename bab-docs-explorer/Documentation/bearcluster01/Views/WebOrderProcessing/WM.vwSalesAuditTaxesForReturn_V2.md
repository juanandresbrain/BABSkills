# WM.vwSalesAuditTaxesForReturn_V2
...
```mermaid
flowchart LR
    VIEW["WM.vwSalesAuditTaxesForReturn_V2"]
    WM_Transactions(["WM.Transactions"]) --> VIEW
    WM_vwTransactionDetail_V2(["WM.vwTransactionDetail_V2"]) --> VIEW
```
...
```sql
CREATE VIEW [WM].[vwSalesAuditTaxesForReturn_V2]
AS
  ...
  FROM [WebOrderProcessing].[WM].[vwTransactionDetail_V2] td
  LEFT JOIN [WebOrderProcessing].[WM].[vwTransactionDetail_V2] ptd ON td.TransactionID = ptd.TransactionID AND ptd.TansactionDetailID < td.TansactionDetailID
  ...
  LEFT JOIN [WebOrderProcessing].[WM].[vwTransactionDetail_V2]  td ON td.TransactionID = cte.TransactionID AND td.TansactionDetailID = cte.OrderTransactionIdentifier
  LEFT JOIN [WebOrderProcessing].[WM].[vwTransactionDetail_V2]  ptd ON ptd.TransactionID = cte.TransactionID AND ptd.TansactionDetailID = cte.PreviousOrderTransactionIdentifier
  LEFT JOIN [WebOrderProcessing].[WM].[Transactions] t ON td.TransactionID = t.TransactionID
