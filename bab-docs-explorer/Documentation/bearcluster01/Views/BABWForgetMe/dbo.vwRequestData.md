# dbo.vwRequestData

**Database:** BABWForgetMe  
**Server:** bearcluster01  

## Architecture Diagram

```mermaid
flowchart LR
    VIEW["dbo.vwRequestData"]
    dbo_ActionLog(["dbo.ActionLog"]) --> VIEW
    dbo_ActionQuery(["dbo.ActionQuery"]) --> VIEW
    dbo_ActionStatus(["dbo.ActionStatus"]) --> VIEW
    dbo_ActionTables(["dbo.ActionTables"]) --> VIEW
    dbo_ApplicationDim(["dbo.ApplicationDim"]) --> VIEW
    dbo_OutputData(["dbo.OutputData"]) --> VIEW
    dbo_TablesToAppsXref(["dbo.TablesToAppsXref"]) --> VIEW
```

## Table Dependencies

| Referenced Table |
|---|
| dbo.ActionLog |
| dbo.ActionQuery |
| dbo.ActionStatus |
| dbo.ActionTables |
| dbo.ApplicationDim |
| dbo.OutputData |
| dbo.TablesToAppsXref |

## View Code

```sql
CREATE VIEW [dbo].[vwRequestData]
AS
SELECT        s.RecordKey, OD.FirstName, OD.LastName, OD.Phone1, OD.Phone2, OD.EmailAddress, OD.Address1, OD.Address2, OD.Address3, OD.City, OD.State, OD.PostalCode, OD.Country, A.AppName, OD.OuputKey, 
                         OD.LogKey, l.RemoveData, l.RemoveDate
FROM            dbo.OutputData AS OD INNER JOIN
                         dbo.ActionStatus AS s ON OD.RecordKey = s.RecordKey INNER JOIN
                         dbo.ActionQuery AS Q ON OD.AQKey = Q.AQKey INNER JOIN
                         dbo.ActionTables AS T ON Q.ATKey = T.ATKey INNER JOIN
                         dbo.TablesToAppsXref AS X ON T.ATKey = X.ATKey INNER JOIN
                         dbo.ApplicationDim AS A ON X.AppKey = A.AppKey LEFT OUTER JOIN
                         dbo.ActionLog AS l ON OD.LogKey = l.LogKey
WHERE        (OD.LogKey IS NOT NULL)
```

