...
flowchart LR
    ...
    dbo_PrivacyPolicy(["dbo.PrivacyPolicy"]) --> VIEW
...
WITH RequestsNeedingManualData(RecordKey, ATKey) AS (SELECT        s.RecordKey, Q.ATKey
    ...
    SELECT        ROW_NUMBER() OVER(ORDER BY ast.ValidationDate ASC) AS ID, r.RecordKey, ast.EmailAddress, ast.FirstName, ast.LastName, ast.PhoneNumber, ast.Address1, ast.Address2, ast.City, ast.State, ast.PostalCode, c.CountryAbbr AS Country, pp.Policy, apd.AppName, apd.AppTeam, apd.TeamEmailAddress, ar.ActionRequestName, aq.AQKey, ast.ValidationDate
     FROM            RequestsNeedingManualData AS r LEFT OUTER JOIN
							  ...
							  dbo.ActionQuery AS aq ON aq.ATKey = at.ATKey LEFT OUTER JOIN
							  dbo.Country AS c ON ast.CountryID = c.CountryID LEFT OUTER JOIN
							  dbo.PrivacyPolicy AS pp ON ast.PrivacyPolicyID = pp.PrivacyPolicyID
```
