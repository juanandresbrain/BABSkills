...
```mermaid
flowchart LR
    VIEW["dbo.vwStoreForcePartiesBookedEvery30Minutes"]
...
```sql
CREATE view [dbo].[vwStoreForcePartiesBookedEvery30Minutes] 
...
PartyStage as
	(
		SELECT 
			...
			format(cast(CreatedDate as date),'d/M/yyyy') as PartyBookDate,
		...
		WHERE e.EventType = 1
		and datediff(dd, e.createdDate, getdate()) <= 2
		and e.StoreID <4000
		group by 
			e.StoreID,
