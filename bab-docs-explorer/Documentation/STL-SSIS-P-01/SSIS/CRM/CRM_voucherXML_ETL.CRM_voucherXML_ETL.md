...
flowchart TD
    ...
    subgraph ControlFlow
        ...
        count_task["count"]
        CRM_voucherXML_ETL_task --> count_task
        count_task["count"]
        count_task --> count_task
        create_zip_and_move_file_task["create zip and move file"]
        count_task --> create_zip_and_move_file_task
        ...
        delete_voucher_XML_task["delete voucher XML"]
        move_file_to_FTP_folder_task --> delete_voucher_XML_task
        placeholder_task["placeholder"]
        delete_voucher_XML_task --> placeholder_task
        fix_any_countries_task["fix any countries"]
        placeholder_task --> fix_any_countries_task
        UK_task["UK"]
...
| Task | Type |
|---|---|
| CRM_voucherXML_ETL | Microsoft.Package |
...
| delete voucher XML | STOCK:SEQUENCE |
| placeholder | Microsoft.ExecuteSQLTask |
...
|---|---|
|  | select  CouponID,  CouponID as DiscountID,  case when Country in ('US','CA','MX','','AU','CH','NZ') then 'US'  when Country in ('UK' ,'GB') then 'UK' else 'US' end as Country,  count(CouponID) as totalCoupons  from SerializedVoucher  where 1=1  --cast(ExportedDate as date) = cast(getdate() as date)  and ExportedDateXML is null --and SerializedNumber = '52979899535755281' --and cast(ExportedDate as |
