...
| Process | SEQUENCE |
| populate tender_dim_stage | Pipeline |
| spMergeTenderDim | ExecuteSQLTask |
...
flowchart TD
    ...
    n_Package_Process_populate_tender_dim_stage["populate tender_dim_stage"]
    n_Package_Process_spMergeTenderDim["spMergeTenderDim"]
    ...
    n_Package_Process_truncate_stage --> n_Package_Process_populate_tender_dim_stage
    n_Package_Process_populate_tender_dim_stage --> n_Package_Process_spMergeTenderDim
...
**Path:** `Package\Process\spMergeTenderDim`  
**Connection:** DWStaging (papamarttest/DWStaging)  

```sql
exec [dbo].[spMergeTenderDim]
```
...
```sql
truncate table [dbo].[tender_dim_stage]
...
```sql
SELECT COUNT(*) AS auditDestCount FROM dbo.tender_dim WHERE tender_code > 0 AND tender_code NOT IN (641,643)
```
...
```sql
SELECT COUNT(*) AS auditSrcCount FROM dbo.vwDW_Tender_Dim WHERE tender_code > 0 AND tender_code NOT IN (641,643)
```
...
|---|---|---|---|---|---|
| auditworks vwDW_Tender_Dim |  | OLEDBSource | populate tender_dim_stage | Auditworks | SqlCommand |

#### auditworks vwDW_Tender_Dim — SqlCommand

```sql
SELECT tender_code
      ,tender_desc
  FROM dbo.vwDW_Tender_Dim with (nolock)
...
|---|---|---|---|---|---|
| tender_dim_stage |  | OLEDBDestination | populate tender_dim_stage | DWStaging |  |
