...
```mermaid
flowchart LR
    VIEW["POS.vwPOSOutbound_Products"]
...
```sql
CREATE VIEW  [POS].[vwPOSOutbound_Products] AS
...
SELECT p.[AccessoryType]
     ...
     --,p.[QuantityRestriction]
	  ...
	  ,case when p.[ProductCountry] not in ('IE','UK') then 
		 ...
		 -- and tg.Id <> 'R-B-D-37-01-01' then 1
		 --and Class in ('Donations','Transaction Flags', 'Virtual Gift Card', 'Gift Card', 'Gift Cards', 'Non-Tabable Fees', 'US State Mandated Fees Bag Fees', 'Misc.Fees Miscellaneous Fees') then 1 
		 else 0 end 
	  ...
	  ,p.[isCouponEligible]      -- replaced with case statement below IDW 04032025
	    --,case when p.StyleCode in 
	 -- (
	 --   '080744',
		--'080742'
		--) THEN '1' ELSE p.[isCouponEligible] END AS isCouponEligible
