...
```mermaid
flowchart LR
    VIEW["ERP.vwVendorFactoryAddress"]
...
```sql
CREATE view [ERP].[vwVendorFactoryAddress] 
as
...
(
	...
	and substring(vm.OrganizationPhoneticName, charindex('-',vm.OrganizationPhoneticName)+1,10 ) not in ('USFACT','UKFACT','CHFACT','MXFACT','KRFACT','ITFACT','SVFACT')
	UNION
	select --vendors setup in Dynamics generic country factory code, joins only on the factory code, not on vendor code
		vm.VENDORACCOUNTNUMBER,
		substring(vm.OrganizationPhoneticName, 1, charindex('-',vm.OrganizationPhoneticName)-1) as VendorCode,
		fa.FactoryCode,
		vm.Entity,
		fa.FactoryName,
		fa.Port,
	...
	from erp.VendorMaster vm with (nolock)
	join erp.FactoryAddress fa with (nolock) 
		on substring(vm.OrganizationPhoneticName, charindex('-',vm.OrganizationPhoneticName)+1,10 ) = fa.FactoryCode 
	where vm.OrganizationPhoneticName like '%-%'
