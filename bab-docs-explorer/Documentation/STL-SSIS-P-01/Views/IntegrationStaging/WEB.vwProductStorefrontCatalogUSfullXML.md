# WEB.vwProductStorefrontCatalogUSfullXML
...
```mermaid
flowchart LR
    VIEW["WEB.vwProductStorefrontCatalogUSfullXML"]
...
```sql
CREATE view [WEB].[vwProductStorefrontCatalogUSfullXML]

as

--------------------------------------------------------------------------------------------------
-- vwProductStorefrontCatalogUSfullXML - Outputs XML for eCommerce Product Storefront Catalog XML 
	...
	(
		...
		from 
			(
				...
				where left(CategoryID, 2) = 'US'
				and CategoryID <> 'US' --excluding root category
		)x
...
XMLStage (XML) as
	(
		select
			'buildabear-storefront-us' as '@catalog-id',
			--(
			--	select *
			--	from Header
			--),
...
select 
cast(
	replace(cast(XML as nvarchar(max)), '<catalog catalog-id="buildabear-storefront-us">', '<catalog catalog-id="buildabear-storefront-us" xmlns="http://www.demandware.com/xml/impex/catalog/2006-10-31">') 
as xml) as XMLData
from XMLStage
```
