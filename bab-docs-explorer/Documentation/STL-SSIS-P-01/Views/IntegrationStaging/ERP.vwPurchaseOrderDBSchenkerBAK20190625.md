# ERP.vwPurchaseOrderDBSchenkerBAK20190625
...
```mermaid
flowchart LR
    VIEW["ERP.vwPurchaseOrderDBSchenkerBAK20190625"]
...
```sql
CREATE view [ERP].[vwPurchaseOrderDBSchenkerBAK20190625]
...
join ERP.ItemMaster im with (nolock)  on l.ItemID = im.ProductNumber and l.Entity = im.Entity
join ERP.ItemMasterProducts p with (nolock) on l.ItemID = p.ProductNumber
...
WHERE 1=1
and lc.LocationCode in ('0980','0960','0013','9999','9975','2970','2999','2013','1971','1972','3970','3980','9991')
and h.IsCurrent = 1
and l.IsCurrent = 1
and (
		(
			(h.PurchaseOrderNumber in (select distinct PurchaseOrderNumber 
										...
										where IsCurrent = 1
										and datediff(dd, getdate()+60, l.StartShipDate) = 0
										) --
		...
		AND ((getdate() < l.StartShipDate and datediff(dd, getdate(), l.StartShipDate) <= 60)
				OR (getdate() > l.StartShipDate and datediff(dd, l.StartShipDate, getdate()) <= 60)) 
		)

	 OR ---THIS BLOCK SHOULD ENABLE PO'S TO EXPORT IF THEY DIDN'T PREVIOUSLY DUE TO MISSING DATA THAT IS NOW PRESENT
		( 
			dateadd(dd, -60, l.StartShipDate) < getdate()
			AND ((getdate() < l.StartShipDate and datediff(dd, getdate(), l.StartShipDate) <= 60)
				OR (getdate() > l.StartShipDate and datediff(dd, l.StartShipDate, getdate()) <= 60)) 
...
--and h.PurchaseOrderNumber in ('PO110002946','PO210000784')
```
