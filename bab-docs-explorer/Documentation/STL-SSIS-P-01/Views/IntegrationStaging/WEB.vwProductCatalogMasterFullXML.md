# WEB.vwProductCatalogMasterFullXML
...
```mermaid
flowchart LR
    VIEW["WEB.vwProductCatalogMasterFullXML"]
...
```sql
CREATE view [WEB].[vwProductCatalogMasterFullXML]
...
--	2022-11-01	Dan Tweedie	Added logic to only include the sportsTeam xml element if isFirstTransmit=1
--------------------------------------------------------------------------------------------------
...
ProductStage as
	(
		...
		--and style_code in ('026706','025012')
		--UNION
		--select 
		--	Style_Code,
		--	DisplayName,
			...
			(
				...
				( 
					select
						...
						case 
							when IsFirstTransmit = 1 --or babwproductid='027318'
								then
									( 
										select							
											'sportsTeam' --as 'custom-attribute/@attribute-id'
										--from productstage ps
										--where ps.BABWProductID=pa.BABWProductID
									)
							end as 'custom-attribute/@attribute-id',
						case 
							when IsFirstTransmit = 1 --or babwproductid='027318'
								then
									( 
