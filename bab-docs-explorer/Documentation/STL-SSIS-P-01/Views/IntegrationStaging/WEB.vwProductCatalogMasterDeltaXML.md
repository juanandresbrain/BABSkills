# WEB.vwProductCatalogMasterDeltaXML
...
```mermaid
flowchart LR
    VIEW["WEB.vwProductCatalogMasterDeltaXML"]
    WEB_AlternateImages(["WEB.AlternateImages"]) --> VIEW
    WEB_AlternateImagesArchive(["WEB.AlternateImagesArchive"]) --> VIEW
...
```sql
CREATE view [WEB].[vwProductCatalogMasterDeltaXML]

as

--------------------------------------------------------------------------------------------------
-- vwProductCatalogMasterDeltaXML - Outputs XML for eCommerce Product Catalog XML 
...
ViewTypes as
	(
		...
		select
			BABWProductID,
			case 
				when left(BABWProductID,1) = 4 and BABWProductID not in ('424925','424974','490501')
					then '/' + cast(cast(right(BABWProductID,5) as int) as varchar)+'x.jpg' 
				when BABWProductID in ('424925','424974','490501')
...
Products (XML) as
	(
			...
			(
				...
				( 
					select
						...
						NULL,
						'PackageOption' as 'custom-attribute/@attribute-id',
						PackageOption as 'custom-attribute'
		...
		where pa.SendData = 1 
		or exists (select BABWProductID from WEB.AlternateImages where SendData = 1)
		or exists (select BABWProductID from WEB.AlternateImagesArchive where CurrentBatch = 1)
