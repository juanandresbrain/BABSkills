...
```sql
CREATE view [WEB].[vwPricebookSaleUKXML]

as

--------------------------------------------------------------------------------------------------
-- vwPricebookSaleUKXML - Outputs XML for eCommerce Pricebook-list XML - Integrates with Salesforce
...
XMLStage (XML) as
	(
		select
			(
				select
					'buildabear-gbp-sale-prices' as '@pricebook-id',
					'GBP' as 'currency',
