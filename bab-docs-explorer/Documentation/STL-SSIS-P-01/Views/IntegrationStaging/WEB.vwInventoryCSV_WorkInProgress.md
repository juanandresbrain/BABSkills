# WEB.vwInventoryCSV_WorkInProgress
...
```mermaid
flowchart LR
    VIEW["WEB.vwInventoryCSV_WorkInProgress"]
    WEB_InventoryFact(["WEB.InventoryFact"]) --> VIEW
    web_LocationStage(["web.LocationStage"]) --> VIEW
    web_PreOrderBackOrderInventory(["web.PreOrderBackOrderInventory"]) --> VIEW
...
```sql
CREATE view [WEB].[vwInventoryCSV_WorkInProgress]
...
select
	ps.GTIN,
	ps.TotalQuantity,
	ps.ProtectedQuantity,
	ps.WarehouseCode,
	ps.CustomerSKU,
	ps.ProductCode,
	ps.Attribute1,
	cast(isnull(po.NonInStockQty,ps.PreBackOrderQuantity) as nvarchar) as PreBackOrderQuantity,
	cast(isnull(po.InStockDate,ps.InStockDateUTC) as nvarchar) as InStockDateUTC,
	cast(isnull(po.Inventorytype,ps.InventoryType) as nvarchar) as InventoryType
from PreStage ps
left join web.PreOrderBackOrderInventory po with (nolock) 
	on ps.WarehouseCode in ('0013','2013')
	and cast(ps.ProductCode as int)=cast(po.SKU as int)
```
