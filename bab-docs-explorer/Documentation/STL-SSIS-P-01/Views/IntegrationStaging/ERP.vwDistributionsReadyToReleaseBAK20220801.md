# ERP.vwDistributionsReadyToReleaseBAK20220801
...
```mermaid
flowchart LR
    VIEW["ERP.vwDistributionsReadyToReleaseBAK20220801"]
...
```sql
CREATE view [ERP].[vwDistributionsReadyToReleaseBAK20220801]
...
--2021-02-02 - Tim Callahan	 - Added 9941 aka 3980 to warehouse exclusion list as there is no existing integration with this 3PL warehouse. 
------------------------------------------------------------------------------------------------------------------------------------------------
With 
DistroData as 
	(
			select DISTINCT 
				...
				rt.ReasonCode,
				rt.Priority,
				--case when h.ORDERID in ('TO0000110619')
				--	then  upper(datename(dw,getdate()-1)) 
				--	else  upper(datename(dw,getdate()))
				--	end as current_day, -- Use this when you need to export specific TOs and their export day is not today 
			...
			AND
							(
								...
								(
									...
									and convert(varchar, getdate(), 108) >= 
																			case 
																				when dd.FROMWAREHOUSE in ('3970','3980','8505','8502') then '15:00:00' 
																				when dd.Entity=2110 then '18:00:00'
																					else '16:30:00' 
																			end
								)
				
							)
			and isnull(dd.FROMWAREHOUSE,'x') not in ('9980','1013') --not shipping to 3980 until 6/28/21
		)

	--OR dd.OrderID in ('TO0000110619') -- Use this when you need to export specific TOs and their export day is not today , in conjuction with the current_day hack above as well
)
```
