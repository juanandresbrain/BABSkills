...
```sql
CREATE view [WEB].[vwInventoryCSV]
...
--	2018-03-27	- Dan Tweedie	-	Added SingleSet bundles for new Disney Princess sets that need to have the 'bundle sku' have the same inventory as the Merch Sku
									...
									*/
-- 2018-10-18	Dan Tweedie	- Removed XML so view can be used for CSV
--	2021-09-09	Dan Tweedie	Updated to left join from the Master Catalog so it will include new items with 0 inventory 
---------------------------------------------------------------------------------------------------
...
Inv as
	(
		...
		from Inventory
		where LocationCode in ('2013','0013')
		and isnull(GTIN,'') <> ''
		group by GTIN,
			LocationCode,
			StyleCode,
...
MasterCatalog as
	(
		select	
			cast(UPC as nvarchar) as GTIN,
			cast(0 as int) as TotalQuantity,
			cast(0 as int) as ProtectedQuantity,
			cast(case when ProductSellingGeography = 'US' then '0013' else '2013' end as nvarchar) as WarehouseCode,
			cast(BABWProductID as nvarchar) as CustomerSKU,
			cast(BABWProductID as nvarchar) as ProductCode,
			cast(left(DisplayName, 50) as nvarchar) as 'Attribute1',
			cast(0 as nvarchar) as PreBackOrderQuantity, 
			convert(nvarchar, getdate(), 121) as InStockDateUTC, 
			cast('' as nvarchar) as InventoryType
...
select
	isnull(i.GTIN,mc.GTIN) as GTIN,
	isnull(i.TotalQuantity,mc.TotalQuantity) as TotalQuantity,
	isnull(i.ProtectedQuantity,mc.ProtectedQuantity) as ProtectedQuantity,
	isnull(i.WarehouseCode,mc.WarehouseCode) as WarehouseCode,
	isnull(i.CustomerSku,mc.CustomerSKU) as CustomerSKU,
	isnull(i.ProductCode,mc.ProductCode) as ProductCode,
	isnull(i.Attribute1,mc.Attribute1) as Attribute1,
	isnull(i.PreBackOrderQuantity,mc.PreBackOrderQuantity) as PreBackOrderQuantity,
	isnull(i.InStockDateUTC,mc.InStockDateUTC) as InStockDateUTC,
	isnull(i.InventoryType,mc.InventoryType) as InventoryType
from MasterCatalog mc
left join Inv i on mc.CustomerSKU=i.CustomerSKU
where mc.ProductCode not in ('089173', '080187','081678')--gala tickets
and mc.ProductCode not in ('080188', '080189')--documentary tickets
and mc.ProductCode not in ('080759')--from Bryce 8/23/24
--and mc.ProductCode not in ('032871')--from Bryce 12/1/24
```
