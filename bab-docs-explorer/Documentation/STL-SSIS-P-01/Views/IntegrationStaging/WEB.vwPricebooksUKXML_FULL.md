...
```mermaid
flowchart LR
    VIEW["WEB.vwPricebooksUKXML_FULL"]
    WEB_vwPricebookListUKXML_FULL(["WEB.vwPricebookListUKXML_FULL"]) --> VIEW
```
...
| Referenced Table |
|---|
| WEB.vwPricebookListUKXML_FULL |

## View Code

```sql
CREATE view [WEB].[vwPricebooksUKXML_FULL]
...
Stage1 (XML) as
	(
		...
		from WEB.vwPricebookListUKXML_FULL
		--UNION
		--select cast(XMLData as nvarchar(max)) as XMLData
