...
```mermaid
flowchart LR
    VIEW["WEB.vwProductCatalogMasterFullXMLBackup20220818"]
...
```sql
CREATE view [WEB].[vwProductCatalogMasterFullXMLBackup20220818]
...
--	219-08-26 -	Dan Tweedie	- Updated view to add new alt and title image tags
--	2020-11-02	Dan Tweedie	Updated to allow image tags for new sac / snc styles that start with 2,3,5,6
--------------------------------------------------------------------------------------------------
...
ViewTypes as
	(
		...
		--	BABWProductID,
		--	case 
		--		when left(BABWProductID,1) = 4 and BABWProductID not in ('424925','424974','490501','090502', '490502')
		--			then '/' + cast(cast(right(BABWProductID,5) as int) as varchar)+'x.jpg' 
		--		when BABWProductID in ('424925','424974','490501','090502', '490502')
		--			then '/' + cast(cast(BABWProductID as int) as varchar)+'x.jpg' 
		...
		select
			m.BABWProductID,
			case 
				when left(m.BABWProductID,1) = 4 and m.BABWProductID not in ('424925','424974','490501','090502', '490502','028284','028285','028287','028288','428284','428285','428287','428288','487179','487180','430376','030376')
					then '/' + cast(cast(right(m.BABWProductID,5) as int) as varchar)+'x.jpg' 
				when m.BABWProductID in ('424925','424974','490501','090502', '490502','028284','028285','028287','028288','428284','428285','428287','428288','487179','487180','430376','030376')
...
ProductStage as
	(
		select 
			...
			giftCardType
		from WEB.ProductCatalogMasterAttributes
		where style_code not in ('080088','028175','028176','028815','428815','057700','091450','491450','030142')
		UNION
		select 
			Style_Code,
			DisplayName,
		...
		AND CurrentBatch = 1
		and style_code not in ('080088','028175','028176','028815','428815','057700','091450','491450','030142')
	),
Products (XML) as
	(
		select
					...
					'hi-res' as '@view-type',
						(
							select
								...
								'x-default' as 'alt/@xml:lang',
								vt.AltText as 'alt',
								'x-default' as 'title/@xml:lang',
								vt.TitleText as 'title'
							from ViewTypes vt
			...
			(
				...
				( 
					select
						...
						--NULL,
						--'stuffedNotClosed' as 'custom-attribute/@attribute-id',
						--SNC as 'custom-attribute',
						--NULL,
						...
						NULL,
						'MSTAT' as 'custom-attribute/@attribute-id',
						MSTAT as 'custom-attribute',
						NULL, 
