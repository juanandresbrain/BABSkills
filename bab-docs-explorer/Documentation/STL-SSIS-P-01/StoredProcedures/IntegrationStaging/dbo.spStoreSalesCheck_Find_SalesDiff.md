# dbo.spStoreSalesCheck_Find_SalesDiff
...
```mermaid
flowchart LR
    SP["dbo.spStoreSalesCheck_Find_SalesDiff"]
    dbo_POLLING_STORES(["dbo.POLLING_STORES"]) --> SP
    dbo_POSSalesTotalByDay(["dbo.POSSalesTotalByDay"]) --> SP
    dbo_spPOSSalesTotalByDay(["dbo.spPOSSalesTotalByDay"]) --> SP
    dbo_spPOSSalesTotalByDayByTransaction(["dbo.spPOSSalesTotalByDayByTransaction"]) --> SP
...
|---|
| dbo.POLLING_STORES |
| dbo.POSSalesTotalByDay |
| dbo.spPOSSalesTotalByDay |
...
```sql
CREATE PROCEDURE [dbo].[spStoreSalesCheck_Find_SalesDiff]
AS
-- =============================================================================================================
...
WHERE 1=1
	...
	--and th.transaction_date between CONVERT(char, DATEADD(day,-2,GETDATE()), 101) and CONVERT(char, DATEADD(day,-2,GETDATE()), 101)
	AND (th.store_no IN (
...
--insert Jump Mind POS Sales
-- Added Stored Proc 8/15/223
exec papamart.dw.dbo.spPOSSalesTotalByDay -- This is for the automated reporting 
exec papamart.dw.dbo.spPOSSalesTotalByDayByTransaction -- This is for manual check process that gary h uses 
-- 
insert StoreSalesCheck_StoreSales
select 
	StoreID,
	cast(BusinessDate as datetime) as SalesDate,
	Units,
	Sales,
	getdate() as datestamp
--from papamart.dw.dbo.vwPOSSalesTotalByDay
from papamart.dw.dbo.POSSalesTotalByDay -- Replaced above on 8/15/2023 as this table is now loaded by spPOSSalesTotalByDay
where BusinessDate = 
	case when datepart(hh, getdate()) <=20 --up until 8pm, the job will look for previous day, else same day
