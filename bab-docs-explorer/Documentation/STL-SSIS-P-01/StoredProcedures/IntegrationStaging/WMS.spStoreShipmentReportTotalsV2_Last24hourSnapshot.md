# WMS.spStoreShipmentReportTotalsV2_Last24hourSnapshot
...
```mermaid
flowchart LR
    SP["WMS.spStoreShipmentReportTotalsV2_Last24hourSnapshot"]
...
CREATE proc [WMS].[spStoreShipmentReportTotalsV2_Last24hourSnapshot]
...
set nocount on
	    ...
	    and DATEDIFF(hh, s.ShipConfirmDateTime, getdate()) <= 24 
		...
		and NOT EXISTS (
		 ...
		 and DATEDIFF(hh, i.ShipDate, getdate()) <= 24 
