# WMS.spMergeDynamicsWMSInventory
...
```mermaid
flowchart LR
    SP["WMS.spMergeDynamicsWMSInventory"]
    dbo_DynamicsWMSInventoryStage(["dbo.DynamicsWMSInventoryStage"]) --> SP
    WMS_DynamicsWMSInventory(["WMS.DynamicsWMSInventory"]) --> SP
```
...
```sql
CREATE proc WMS.[spMergeDynamicsWMSInventory] -- Update to Proper Name 
...
merge into IntegrationStaging.WMS.DynamicsWMSInventory as target
using	( 
		select 
		s.location_code, 
		s.style_code, 
