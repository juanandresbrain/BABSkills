# WEB.spFTPukWarehouseOrderStatusUpdatesTest
...
```mermaid
flowchart LR
    SP["WEB.spFTPukWarehouseOrderStatusUpdatesTest"]
...
```sql
CREATE proc [WEB].[spFTPukWarehouseOrderStatusUpdatesTest]
...
insert #DEL exec master..xp_cmdshell 'dir \\kermode\FileRepository\WarehouseTestOrderStatusUpdates\log\Download.log /B'
insert #DEL exec master..xp_cmdshell 'dir \\kermode\FileRepository\WarehouseTestOrderStatusUpdates\log\FTPLog_UKWeb.txt /B'

delete from #DEL where output is null or output = 'File Not Found'

IF (select count(*) from #DEL where output = 'Download.log') > 0
	begin
		exec master..xp_cmdshell 'del \\kermode\FileRepository\WarehouseTestOrderStatusUpdates\log\Download.log'
	end
IF (select count(*) from #DEL where output = 'FTPLog_UKWeb.txt') > 0
	begin
		exec master..xp_cmdshell 'del \\kermode\FileRepository\WarehouseTestOrderStatusUpdates\log\FTPLog_UKWeb.txt'
	end
			...
			-----ftp download
					...
					select 
							...
							@script = ' /script=\\stl-ssis-p-01\Integrationstaging\wm\Test\WinSCPGET.txt',
							@log = ' /log=\\kermode\FileRepository\WarehouseTestOrderStatusUpdates\log\Download.log',
					...
					----connect to ftp server, if connection unsuccessful, send email
							...
							if (select count(*) from WEB.tmpFTGetWeb where ftplog like '%.xml%100[%]') < 1
								begin
									...
									set @Log_file_location = '\\kermode\FileRepository\WarehouseTestOrderStatusUpdates\log\'
