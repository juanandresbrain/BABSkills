# NOCDev.spGetSQLJobStatus_Shipped
...
```mermaid
flowchart LR
    SP["NOCDev.spGetSQLJobStatus_Shipped"]
...
```sql
CREATE proc [NOCDev].[spGetSQLJobStatus_Shipped]
...
AND j.name IN ('WMS_Manage D365 Ship Messages') --Uncomment to search for a single job
