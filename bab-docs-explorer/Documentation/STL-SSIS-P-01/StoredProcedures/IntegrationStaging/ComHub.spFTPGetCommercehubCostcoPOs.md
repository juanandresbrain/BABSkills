# ComHub.spFTPGetCommercehubCostcoPOs
...
```mermaid
flowchart LR
    SP["ComHub.spFTPGetCommercehubCostcoPOs"]
...
```sql
CREATE proc [ComHub].[spFTPGetCommercehubCostcoPOs]

as

-- =====================================================================================================
-- Name: spFTPGetCommercehubCostcoPO's
--
-- Description:	
--
-- Revision History
--		Name:			Date:			Comments:
--		Ben Barud		2020-10-19		Initial Creation
-- =====================================================================================================
...
insert #DEL exec master..xp_cmdshell 'dir \\kermode\Filerepository\CommerceHubCostcoPOs\log\Upload.log /B'
insert #DEL exec master..xp_cmdshell 'dir \\kermode\Filerepository\CommerceHubCostcoPOs\log\FTPLog_CommercehubCostcoPOs.txt /B'

delete from #DEL where output is null or output = 'File Not Found'

IF (select count(*) from #DEL where output = 'Upload.log') > 0
	begin
		exec master..xp_cmdshell 'del \\kermode\Filerepository\CommerceHubCostcoPOs\log\Upload.log'
	end
IF (select count(*) from #DEL where output = 'FTPLog_CommercehubCostcoPOs.txt') > 0
	begin
		exec master..xp_cmdshell 'del \\kermode\Filerepository\CommerceHubCostcoPOs\log\FTPLog_CommercehubCostcoPOs.txt'
	end
...
--insert #DIR exec master..xp_cmdshell 'dir \\kermode\Filerepository\CommerceHubCostcoPOs\*.xml /B'
...
--from #DIR
			...
			-----ftp upload
					...
					select 
							...
							@script = ' /script=\\stl-ssis-p-01\Integrationstaging\Commercehub\Prod\WinSCPGetCostcoPOs.txt',
							@log = ' /log=\\kermode\Filerepository\CommerceHubCostcoPOs\log\Upload.log',
					...
					----connect to ftp server, if connection unsuccessful, send email
							...
							if (select count(*) from ComHub.tmpComhubFTP where ftplog like '%.xml%100[%]') < 1
								begin
									...
									set @Log_filename = 'FTPLog_CommercehubCostcoPOs.txt'
									set @Log_file_location = '\\kermode\Filerepository\CommerceHubCostcoPOs\log\'
									...
									--@body = @body,
									--@file_attachments = '\\kermode\Filerepository\CommerceHubCostcoPOs\log\FTPLog_UKWeb.txt;\\kermode\Filerepository\CommerceHubCostcoPOs\log\Upload.log',
									--@importance = 'HIGH'
									------------------------------------------------------------------------------------------------------------------------
									--EXEC master..xp_cmdshell 'move \\kermode\Filerepository\CommerceHubCostcoPOs\*.xml \\kermode\Filerepository\CommerceHubCostcoPOs\FAILED\'
									--update ComHub.ComhubFTPLog   set Success = 0 where Success is NULL
								end
							--else
								--begin
									--EXEC master..xp_cmdshell 'move \\kermode\Filerepository\CommerceHubCostcoPOs\Temp\* \\kermode\Filerepository\CommerceHubCostcoPOs\Success\'
									--EXEC master..xp_cmdshell 'move \\kermode\Filerepository\CommerceHubCostcoPOs\*.xml \\kermode\Filerepository\CommerceHubCostcoPOs\Archive\'
