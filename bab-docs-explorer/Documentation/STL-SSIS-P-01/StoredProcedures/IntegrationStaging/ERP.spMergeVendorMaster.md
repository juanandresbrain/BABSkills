...
```sql
CREATE proc [ERP].[spMergeVendorMaster] 
...
when matched and 
	(
		...
		isnull(target.ZAKATREGISTRATIONNUMBER  ,'xxx')<>isnull(source.ZAKATREGISTRATIONNUMBER,'xxx')  OR
		isnull(target.BABFactoryCode,'x')<>isnull(source.BABFactoryCode,'x') OR
		isnull(target.BABFOBPort,'x')<>isnull(source.BABFOBPort,'x') OR
		isnull(target.BABVendorCode,'x')<>isnull(source.BABVendorCode,'x') 
	)
then update 
	set 
		...
		target.BABFactoryCode=source.BABFactoryCode,
		target.BABFOBPort=source.BABFOBPort,
		target.BABVendorCode=source.BABVendorCode,
	...
	then insert
		...
		values
			(
				...
				source.Entity,
				source.BABFactoryCode,
				source.BABFOBPort,
				source.BABVendorCode,
				getdate(),
				NULL
			)
;
