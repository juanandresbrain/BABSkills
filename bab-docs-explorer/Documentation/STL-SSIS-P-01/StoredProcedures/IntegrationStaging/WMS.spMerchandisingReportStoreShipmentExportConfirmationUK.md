# WMS.spMerchandisingReportStoreShipmentExportConfirmationUK
...
```mermaid
flowchart LR
    SP["WMS.spMerchandisingReportStoreShipmentExportConfirmationUK"]
    dbo_sp_send_dbmail(["dbo.sp_send_dbmail"]) --> SP
    dbo_ukExport(["dbo.ukExport"]) --> SP
...
```sql
CREATE proc [WMS].[spMerchandisingReportStoreShipmentExportConfirmationUK]
...
IF (Object_ID('IntegrationStaging..ukExport') IS NOT NULL) DROP TABLE ukExport
--select	
...
--and document_number = '2900257734' -- Testing Purposes Only 
order by 1
...
begin
	...
	SET @1file_location = '\\stl-ssis-p-01\IntegrationStaging\3PW\UK_Distro\OUTBOUND\StoreShipmentConfirmation\'
	...
	EXEC [stl-ssis-p-01].msdb.dbo.sp_send_dbmail
		...
		@file_attachments ='\\stl-ssis-p-01\IntegrationStaging\3PW\UK_Distro\OUTBOUND\StoreShipmentConfirmation\UK_store_shipments.csv'
