# WMS.spSelectWMSPurchaseOrderReceiptsBACKUP20210628
...
```mermaid
flowchart LR
    SP["WMS.spSelectWMSPurchaseOrderReceiptsBACKUP20210628"]
...
```sql
CREATE proc [WMS].[spSelectWMSPurchaseOrderReceiptsBACKUP20210628] 
...
select	convert(varchar, cast(MessageQueueDateUTC as smalldatetime), 101) as receipt_date,
		AptosPONumber as PO, 
		ASN,
		...
		'' as BOL
from #stage
group by convert(varchar, cast(MessageQueueDateUTC as smalldatetime), 101), AptosPONumber, ASN, ItemID
...
---prepare data for printing
	...
	declare header cursor for
		select distinct convert(varchar, getdate(), 112) + convert(varchar, datepart(hh, getdate())) + convert(varchar, datepart(mi, getdate())) + convert(varchar, datepart(ss, getdate())) + convert(varchar, max(id)) doc_nbr, isnull(bol, 'NO_BOL') bol, po, asn 
	...
	while @counterH <= @totalH
		begin
			fetch next from header into @docnbr, @bol, @po, @asn
			print 'H' + '	' + 'A' + '	' + @docnbr + '	' + @bol + '	' + @date + '	' + '0980' + '	' + @po + '	' + 'Administrator' + '	' + '	' + '	' + '	' + '	' + @asn + '	' + '	' + '	' + '	' + '	' + '	' + '	' + '	' + '	' + '	' + '	' + 'N' + '	'
				--detail cursor
					set @counterD = 1
