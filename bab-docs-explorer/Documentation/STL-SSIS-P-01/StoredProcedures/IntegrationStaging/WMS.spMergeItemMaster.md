...
```mermaid
flowchart LR
    SP["WMS.spMergeItemMaster"]
    WMS_ItemMaster(["WMS.ItemMaster"]) --> SP
    WMS_ItemMasterStage(["WMS.ItemMasterStage"]) --> SP
```
...
```sql
CREATE proc [WMS].[spMergeItemMaster]
...
merge into WMS.ItemMaster as target 
Using WMS.ItemMasterStage as source
on (
