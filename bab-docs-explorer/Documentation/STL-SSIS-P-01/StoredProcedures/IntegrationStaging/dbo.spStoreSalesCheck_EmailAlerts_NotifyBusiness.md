# dbo.spStoreSalesCheck_EmailAlerts_NotifyBusiness
...
```mermaid
flowchart LR
    SP["dbo.spStoreSalesCheck_EmailAlerts_NotifyBusiness"]
    dbo_sp_send_dbmail(["dbo.sp_send_dbmail"]) --> SP
    dbo_StoreSalesCheck_Diff(["dbo.StoreSalesCheck_Diff"]) --> SP
    dbo_tmpStoresZeroSales(["dbo.tmpStoresZeroSales"]) --> SP
```
...
| dbo.tmpStoresZeroSales |

## Stored Procedure Code

```sql
CREATE PROCEDURE [dbo].[spStoreSalesCheck_EmailAlerts_NotifyBusiness]
AS
BEGIN
...
set @subject = 'Flash GAAP Sales from AW  ** PLEASE READ **'

set @recipients = 'northamericanflashreport@buildabear.com;databears@buildabear.com;entsyssupport@buildabear.com'
...
set @body = 
	'Please note, the POS team is investigating to verify the following possible variances between Stores and Sales Audit.
	<br><br>
	<html>
	<body>
	<STYLE TYPE="text/css">
	...
	'</table>
	<br><br>
	Additionally, the following stores are reporting with 0 sales in Sales Audit for yesterday. 
	<br><br>
	...
	FOR
		select STORE_NUM 
		from bedrockdb01.auditworks.dbo.tmpStoresZeroSales
		order by STORE_NUM
	
	OPEN curStores
	FETCH NEXT FROM curStores INTO @store_id
	...
	BEGIN
		...
		begin
			...
			begin
				set @body = @body + '<tr><td>' + @store_id + '</td></tr>'
			end
		end
	...
	set @body = @body +
	'</table>
	<font face =arial size = 1><i>This was run from stl-ssis-p-01.IntegrationStaging.dbo.spStoreSalesCheck_EmailAlerts_NotifyBusiness.</i></font>
	</body>
	<html>
	'
