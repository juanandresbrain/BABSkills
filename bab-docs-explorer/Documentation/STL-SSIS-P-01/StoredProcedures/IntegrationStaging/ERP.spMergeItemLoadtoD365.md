...
```sql
CREATE proc [ERP].[spMergeItemLoadtoD365] 
 @LoadType varchar(5)
...
-- 2023-11-13	Lizzy Timm	Updated to accommodate new data for item create changes; Jira BIB-647 with BAB Design Document - Retail Item Create - Merch to D365 V2 PO Writing
/*
...
when matched 
	and (
			...
			or
			isnull(target.ColorDescription,'x')<>isnull(source.ColorDescription,'x')
			or
			isnull(target.VendorProductDescription,'x')<>isnull(source.VendorProductDescription,'x')
		)
	then 
		UPDATE
			SET
				...
				target.ColorDescription=source.ColorDescription,
				target.VendorProductDescription=source.VendorProductDescription,
...
When Not Matched By Target 
	Then 
		...
		Values (	
					...
					source.VendorProductDescription,
					source.Entity,
					1,
					getdate()
				)
