# ERP.spDistributionsReadyToRelease
...
```mermaid
flowchart LR
    SP["ERP.spDistributionsReadyToRelease"]
...
```sql
CREATE  proc [ERP].[spDistributionsReadyToRelease]
as
...
--		Tim Callahan	12/05/2023		Added New fields to data flow per JIRA BIB 704 and peformance enhancments 
-- =====================================================================================================
set nocount on
...
;
	...
	select DISTINCT 
		...
		h.OrderCreateSource,
		left (rt.message,20) as RecTypeMessage, -- Changed to Left 20 on May 27 2025
		d.BABAptosDistroNumber,  -- Added 12/05/2023
		d.BABAptosDistroLineNumber  -- Added 12/05/2023
	into ##DistroData
	from 
	...
	and DATEDIFF(dd,h.TRANSACTIONDATETIME,getdate()) <= 90  -- Added 12/05/2023
	and h.TRANSACTIONDATETIME > '06-27-2022' -- Existed priorty to 11/20/2023 - So Leaviing it in there 
	--and h.OrderID in ('TO0000222255') -- For Testing Only 
	


-- Capture Max SquenceNumber 
-- New Max Sequence as Previous Performance was terrible 
-- added 11/20/2023
;
with NewSeqSource as 
(
select 
dd.Entity, 
dd.orderid, 
row_number() over(partition by dd.Entity, dd.OrderID order by dd.OrderID, dd.ToWarehouse, dd.ItemNumber, dd.ModeOfDelivery) as SequenceNumber 
from ##DistroData dd
) 
...
r.ORDERID, 
r.entity, 
max (r.SequenceNumber) as MaxSequence 
into ##MaxSequence
from NewSeqSource r
where 1=1
group by r.ORDERID, 
...
--	Old MaxSequence Code 
--	First Tried doing a join below but still poor performance 
--select 
--	OrderID,
...
select  
	...
	dd.RecTypeMessage,
	dd.BABAptosDistroNumber, -- Added 11/20/2023
	dd.BABAptosDistroLineNumber -- Added 11/20/2023

from ##DistroData dd
left join ##MaxSequence ms on dd.OrderID = ms.OrderID  and dd.Entity = ms.Entity
...
AND (
	...
	--OR dd.OrderID in ('TO0000046639') -- Ad Hoc Let Order Export 
)
