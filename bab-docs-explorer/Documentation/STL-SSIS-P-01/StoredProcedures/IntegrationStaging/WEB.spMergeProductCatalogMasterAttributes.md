# WEB.spMergeProductCatalogMasterAttributes
...
```mermaid
flowchart LR
    SP["WEB.spMergeProductCatalogMasterAttributes"]
...
```sql
CREATE proc [WEB].[spMergeProductCatalogMasterAttributes]
@LoadType varchar(5)
...
When Matched 
	AND 
		(
			...
			or  isnull(target.BaseID,'x')<>isnull(source.BaseID,'x')
