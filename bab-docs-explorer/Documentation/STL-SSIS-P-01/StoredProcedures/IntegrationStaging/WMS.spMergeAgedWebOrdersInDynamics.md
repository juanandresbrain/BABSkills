# WMS.spMergeAgedWebOrdersInDynamics
...
```mermaid
flowchart LR
    SP["WMS.spMergeAgedWebOrdersInDynamics"]
...
```sql
CREATE proc [WMS].[spMergeAgedWebOrdersInDynamics]
...
select 
			...
			case 
				when exists (select s.SalesOrderNumber from WMS.AgedWebOrdersInDynamicsStage s where s.ItemNumber in ('000015', '022610', '023019') and s.SalesOrderNumber = x.SalesOrderNumber)
					then 
						case when datediff(dd, x.OrderCreationDateTime, getdate()) < 1 then 0 
							when datediff(dd, x.OrderCreationDateTime, getdate()) >= 1 then datediff(dd, x.OrderCreationDateTime, getdate()) -1 else 0 end 
				else datediff(dd, x.OrderCreationDateTime, getdate())
			end as ProcessingAge,
...
using 
	(
		select 
			...
			WebOrderNumber,	
			WaveID,	
			max(ReleasedDateAndTime) ReleasedDateAndTime,
			ContainerID,	
			WorkID,	
			DeckOrderDate,
			DynamicsOrderAge,
