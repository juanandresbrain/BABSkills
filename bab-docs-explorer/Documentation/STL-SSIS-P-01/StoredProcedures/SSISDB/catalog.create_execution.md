...
```mermaid
flowchart LR
    SP["catalog.create_execution"]
    catalog_catalog_properties(["catalog.catalog_properties"]) --> SP
    catalog_environment_references(["catalog.environment_references"]) --> SP
    catalog_folders(["catalog.folders"]) --> SP
    catalog_object_parameters(["catalog.object_parameters"]) --> SP
    catalog_packages(["catalog.packages"]) --> SP
    ...
    internal_environment_variables(["internal.environment_variables"]) --> SP
    ...
    internal_folders(["internal.folders"]) --> SP
    ...
    internal_object_parameters(["internal.object_parameters"]) --> SP
...
catalog,create_execution_dump,CREATE PROCEDURE [catalog].[create_execution_dump]
    @execution_id       bigint          
AS
    SET NOCOUNT ON;
    DECLARE @return_value int
