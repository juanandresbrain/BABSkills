...
```mermaid
flowchart LR
    SP["catalog.set_object_parameter_value"]
    ...
    internal_check_data_type_value(["internal.check_data_type_value"]) --> SP
    internal_check_permission(["internal.check_permission"]) --> SP
    internal_environment_references(["internal.environment_references"]) --> SP
...
| internal.adt_sid |
| internal.adt_sname |
| internal.check_data_type_value |
...
catalog,start_execution,CREATE PROCEDURE [catalog].[start_execution]
        @execution_id       bigint   
AS 
    SET NOCOUNT ON
