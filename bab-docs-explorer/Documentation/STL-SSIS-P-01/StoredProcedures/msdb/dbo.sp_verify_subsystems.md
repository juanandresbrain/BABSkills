...
dbo,spPurgeSSISLogData,-- =============================================
-- Author:		Tim Bytnar
-- Create date: 6/2/2019
-- Description:	This procedure is used to delete SSISLog data older than 90 days
-- =============================================
CREATE PROCEDURE [dbo].[spPurgeSSISLogData] 
...
BEGIN
    ...
    DECLARE @NextIDs TABLE(ID int primary key)
    DECLARE @PurgeDate datetime
    SELECT @PurgeDate = DATEADD(d, -30, GetDate())

    WHILE EXISTS(SELECT 1 FROM msdb.dbo.sysssislog WHERE starttime < @PurgeDate)
    BEGIN 
	   BEGIN TRAN 

		  INSERT INTO @NextIDs(ID)
		  SELECT TOP 1000 ID FROM msdb.dbo.sysssislog WHERE starttime < @PurgeDate

		  DELETE msdb.dbo.sysssislog
		  FROM  msdb.dbo.sysssislog AS a
		  INNER JOIN @NextIDs AS b ON a.ID = b.ID 
...
dbo,StartAgentJob,CREATE PROCEDURE dbo.StartAgentJob
