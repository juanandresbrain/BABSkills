...
,InventoryJournalComponents AS (
    ...
    SELECT
        ...
        SUM(CASE WHEN t.[journaltype] = 1
                 ...
                 AND transdescription NOT LIKE '%damage%'
                 AND transdescription NOT LIKE 'gxo po%'
            THEN t.costamount ELSE 0 END) AS STSAdjCost,
        SUM(CASE WHEN t.journalnameid = 'IMOV' THEN t.costamount * -1 ELSE 0 END) AS MovementCost
    FROM
        InventoryJournalComponentBase t
   WHERE t.journaltype in (1,4)
    GROUP BY
        WeekEndingDate,
        L
```
