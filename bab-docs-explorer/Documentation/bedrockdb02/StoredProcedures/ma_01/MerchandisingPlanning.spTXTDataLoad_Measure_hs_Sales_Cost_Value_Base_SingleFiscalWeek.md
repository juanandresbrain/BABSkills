# MerchandisingPlanning.spTXTDataLoad_Measure_hs Sales Cost Value Base_SingleFiscalWeek

**Database:** ma_01  
**Server:** bedrockdb02  

## Architecture Diagram

```mermaid
flowchart LR
    SP["MerchandisingPlanning.spTXTDataLoad_Measure_hs Sales Cost Value Base_SingleFiscalWeek"]
    MerchandisingPlanning_spTXTDataLoad_ETLBatchDetailLog_UpdateBatchStatus(["MerchandisingPlanning.spTXTDataLoad_ETLBatchDetailLog_UpdateBatchStatus"]) --> SP
```

## Table Dependencies

| Referenced Table |
|---|
| MerchandisingPlanning.spTXTDataLoad_ETLBatchDetailLog_UpdateBatchStatus |

## Stored Procedure Code

```sql

```

