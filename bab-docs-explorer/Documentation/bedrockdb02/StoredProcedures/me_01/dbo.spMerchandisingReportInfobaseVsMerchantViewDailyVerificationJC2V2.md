...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingReportInfobaseVsMerchantViewDailyVerificationJC2V2"]
...
```sql
CREATE proc [dbo].[spMerchandisingReportInfobaseVsMerchantViewDailyVerificationJC2V2]
...
BEGIN
    EXEC msdb.dbo.sp_send_dbmail
        ...
        @subject = @subj,
        @body = 'There are differences between IB and MA. MERCHANDISING - Process - Pipeline Sales Posting & MERCHANDISING - Report - InfoBase-MerchantviewDailyVerification_CA Will run again once more.',
        @body_format = 'HTML'
END
...
BEGIN
    ...
    EXEC msdb.dbo.sp_start_job @job_name = 'MERCHANDISING -  Validation - AutoRerun'
END
```
