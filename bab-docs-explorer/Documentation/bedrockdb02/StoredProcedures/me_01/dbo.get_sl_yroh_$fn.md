...
```sql
create proc dbo.get_sl_yroh_$fn  @no_pds int, @no_yrs int
...
select @year_code = @period_code / 100

select @i = 1

while @i <= @no_yrs
begin
	SELECT @year_code = max (merch_year)
	FROM calendar_date
	where (merch_year)< @year_code
	select @i = @i + 1
end
