# dbo.sp_smartlook_WH_AVAILABLE_OH_RPT
...
```mermaid
flowchart LR
    SP["dbo.sp_smartlook_WH_AVAILABLE_OH_RPT"]
...
```sql
CREATE proc [dbo].[sp_smartlook_WH_AVAILABLE_OH_RPT] (@location_code varchar(4))
...
and l.location_code in (@location_code)
...
where l.location_code in (@location_code)
...
into #keith_temp_3
from #keith_temp
group by location_code, dept,style_code,long_desc,next_receipt_date
order by 3
...
select location_code, dept, style_code, long_desc, available,allocated,0 as available_to_distribute,next_receipt_date, on_order_units from #keith_temp_2

select location_code, dept, style_code, long_desc, available,allocated,available_to_distribute, on_order_units,next_receipt_date  
from #keith_temp_3
