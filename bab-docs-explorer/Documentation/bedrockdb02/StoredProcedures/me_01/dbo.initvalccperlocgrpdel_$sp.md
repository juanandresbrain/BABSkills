...
```mermaid
flowchart LR
    SP["dbo.initvalccperlocgrpdel_$sp"]
...
and lp.hierarchy_level_id = @LocLevelId /*curr loc level*/
and lp.parent_hierarchy_group_id in (select lp2.parent_hierarchy_group_id from location_parent lp2 where 
lp2.location_id = cvh.location_hierarchy_group_id )
...
select @MerchGroupId, @StartPerId, c.parent_hierarchy_group_id, 
(sum(cost))/(sum(retail)), 1, 1, 
sum(cost_local)/sum(retail_local), 1, a.jurisdiction_id
from initial_cum_val_post a, merch_group_parent b, location_parent c 
where a.merch_hierarchy_group_id = b.hierarchy_group_id and 
b.hierarchy_level_id = @MerchLevelId and 
(b.parent_hierarchy_group_id = @MerchGroupId or b.hierarchy_group_id=@MerchGroupId)and 
a.location_id = c.location_id and 
c.hierarchy_level_id = @LocLevelId
group by b.parent_hierarchy_group_id, c.parent_hierarchy_group_id, a.jurisdiction_id;
...
where parent_hierarchy_group_id = @MerchGroupId or hierarchy_group_id = @MerchGroupId);

delete from initial_val_list 
where merch_hierarchy_group_id in (select 
hierarchy_group_id from 
merch_group_parent where 
parent_hierarchy_group_id = @MerchGroupId or hierarchy_group_id = @MerchGroupId) and 
