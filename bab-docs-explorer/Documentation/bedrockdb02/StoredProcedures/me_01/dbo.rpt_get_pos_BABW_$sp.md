...
```sql
CREATE PROCEDURE [dbo].[rpt_get_pos_BABW_$sp] @report_no TINYINT
...
Updated on 8/15/2018 to flag POs with franchisee styles (9######) to use franchisee/MX PO print version.  China POs were overriding this so changes needed to be added. - KL
...
case when location_code in ('3970','8502','8505') then 'CH1'  -- included location 8505 per Rachel Stack Request - 02/27/2018 - TimC
when location_code in ('3980','3981','9942') then 'CH2' -- Added 3981 per Carolina Ramos Noldin Fett  on 10/1/2024 - Tim C -- Added 2970 per Carolina Ramos Noldin Fett on 10/23/2024 removed 2970 11/12/2024 PaulK
--when location_code in ('3980','3981','9942') then 'CH2' -- Added 3981 per Carolina Ramos Noldin Fett  on 10/1/2024 - Tim C -- Added 2970 per Carolina Ramos Noldin Fett on 10/23/2024
end  as china

into #temp_china_po
from #temp_po po join po_location pl on po.po_id=pl.po_id
join location l on pl.location_id=l.location_id
join  po_detail pd on po.po_id=pd.po_id --- * included style range so that China ware can be used on MX/franchise PO print 08/13/2018 - KL
join sku k on k.sku_id=pd.sku_id ---*
join style s on s.style_id=k.style_id ---*
where 1=1
and l.location_code in ('3970','8502','3980','8505','3981','9942') -- included location 8505 per Rachel Stack Request - 02/27/2018 - TimC -- Added 3981 per Carolina Ramos Noldin Fett  on 10/1/2024 - Tim C added 2970 10/23/2024 per Carolina Ramos Noldin Fett - Paul K removed 2970 11/12/2024 PaulK
--and l.location_code in ('3970','8502','3980','8505','3981','9942') -- included location 8505 per Rachel Stack Request - 02/27/2018 - TimC -- Added 3981 per Carolina Ramos Noldin Fett  on 10/1/2024 - Tim C 
and len(style_code) = 6 ---*
--and style_code not between '900000' and '999999'---* -- Remarked out on 9/18/2024 to allow 9 styles to populate UPC fields for CN POs
