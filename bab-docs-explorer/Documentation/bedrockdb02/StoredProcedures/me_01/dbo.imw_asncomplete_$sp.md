...
```sql
create proc [dbo].[imw_asncomplete_$sp]
	( @asn_id AS DECIMAL(12), @data_set_type TINYINT )
	
AS

/*
Proc name: imw_asncomplete_$sp

Description: 

Called when printing ASN in CR
...
WHERE
	h.advance_shipping_notice_id = @asn_id	
...
/*ELSE IF @data_set_type = 1
	...
	BEGIN
		...
		SELECT 
			...
			, s.style_code styleCode, s.long_desc styleLongDesc, sv.vendor_style vendorStyle
			, c.color_code colorCode, sc.long_desc styleColorLongDesc
			, p.pack_code packCode, p.pack_description packDescription, p.vendor_pack_code vendorPackCode
			, SUM(ad.units_sent) shippedUnits
		FROM
			advance_shipping_notice h
		INNER JOIN asn_po_location al ON h.advance_shipping_notice_id = al.advance_shipping_notice_id
		INNER JOIN asn_po_location_detail ad ON ad.asn_po_location_id = al.asn_po_location_id AND ad.advance_shipping_notice_id = al.advance_shipping_notice_id
		INNER JOIN po ON al.po_id = po.po_id
		INNER JOIN vendor v ON h.vendor_id = v.vendor_id
		LEFT OUTER JOIN ( style s 
							INNER JOIN style_vendor sv ON s.style_id = sv.style_id ) ON ad.style_id = s.style_id AND h.vendor_id = sv.vendor_id
		...
		GROUP BY 
			po.po_no
			, ad.carton_no
			, s.style_code, s.long_desc, sv.vendor_style
			, c.color_code, sc.long_desc
