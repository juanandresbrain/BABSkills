...
```sql
CREATE PROCEDURE [dbo].[plu_pricing_group_$sp]
AS
...
/*
	...
	Description	: Procedure called by Segment 1038 -- EDM & PROD to Price Look-Up File Generate (CRS)
				  Populate a temp table of pricing groups using locations in #location table
	...
	The #pricing_group temp table will be used in future procedures when retrieving permanent/promo prices.  
	This work is done by pricing group which is why we have an identity column on this table
	for the purposes of looping.
		
	-- NOTE: The temp tables #pricing_group and #location exist	
...
BEGIN TRY
	...
	-- Get distinct pricing groups from #location table
		
	SET @line_id = 10
	
	INSERT INTO #pricing_group
