...
```mermaid
flowchart LR
    SP["dbo.get_avg_cost_style_chain_$sp"]
...
CREATE PROCEDURE dbo.get_avg_cost_style_chain_$sp
...
/*
	Get dynamic average cost by style/chain
...
CREATE TABLE dbo.#temp_style_chain
	(
		style_id DECIMAL (12, 0) NULL
	)

INSERT INTO dbo.#temp_style_chain
	(
		style_id
	)
SELECT DISTINCT
	...
	INNER JOIN
		...
		(
			...
			FROM
				...
				INNER JOIN dbo.sku K ON IBIT.sku_id = K.sku_id
				INNER JOIN dbo.#temp_style_chain TSL ON K.style_id = TSL.style_id
				LEFT JOIN
		...
		) SQAV ON TWACL.style_id = SQAV.style_id
