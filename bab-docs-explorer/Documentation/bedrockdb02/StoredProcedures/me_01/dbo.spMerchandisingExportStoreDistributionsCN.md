# dbo.spMerchandisingExportStoreDistributionsCN
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingExportStoreDistributionsCN"]
...
```sql
CREATE proc [dbo].[spMerchandisingExportStoreDistributionsCN]
...
--===============================================================================================
		...
		(
			...
			join ERP_DistributionDataLookup ddl with (nolock) 
				...
				and case 
					...
					when ddas.sourceid in ('3980') then '1200'
					else '3001'
				end = ddl.Entity
		...
		select 
			...
			ref_field_1,
			short_desc,
			@A_seed3970 + DENSE_RANK() OVER (ORDER BY destid, rec_type) as document_number,
			handling_days,
			case 
				when 
...
--===============================================================================================
		...
		select 
			...
			ref_field_1,
			short_desc,
			@C_seed8502 + DENSE_RANK() OVER (ORDER BY destid, rec_type) as document_number,
			handling_days,
			case 
				when 
					(datepart(dw, release_date) = 1 and handling_days >= 7)
		...
		BEGIN
				...
				while @C_counter > 0
					...
					begin
						...
						set @C_query = 'set nocount on select document_number, sourceid, destid, rec_type, message, style_code, quantity, convert(varchar, getdate(), 101) release_date, distribution_number, ref_field_1, convert(varchar, expected_ship_date, 101) expected_ship_date from ##CNDistros8502 where document_number = ' + @C_shipment + 'order by style_code'
...
--===============================================================================================
		...
		select 
			...
			ref_field_1,
			short_desc,
			@D_seed8505 + DENSE_RANK() OVER (ORDER BY destid, rec_type) as document_number,
			handling_days,
			case 
				when 
					(datepart(dw, release_date) = 1 and handling_days >= 7)
...
--===============================================================================================
		declare @X_seed3980 bigint
		select @X_seed3980 = max(document_number) from store_shipment_export
		...
		select 
			...
			ref_field_1,
			short_desc,
			@X_seed3980 + DENSE_RANK() OVER (ORDER BY destid, rec_type) as document_number,
			handling_days,
			case 
				when 
					(datepart(dw, release_date) = 1 and handling_days >= 7)
		...
		BEGIN
				...
				(
					ID is NULL 
					and distribution_number in  (select distribution_number from ##CNDistros3980) 
					and sourceid = '3980'
				)

				--OUTPUT CSV FILE 
				declare @X_counter int,
						@X_shipment varchar(20),
						@X_location varchar(4),
						@X_rectype int,
						@X_query varchar(1000),
						@X_date varchar(52),
						@X_file_name varchar(100),
						@X_file_location varchar(100),
						@X_server varchar(20),
						@X_database varchar(20),
						@X_bcp varchar(1000)

				select @X_counter = count(distinct document_number) from ##CNDistros3980

				while @X_counter > 0

					begin
						select @x_shipment = max(document_number) from ##CNDistros3980
						select @x_location = max(destid) from ##CNDistros3980 where document_number = @X_shipment
						select @X_rectype = max(rec_type) from ##CNDistros3980 where document_number = @X_shipment

						set @X_query = 'set nocount on select document_number, sourceid, destid, rec_type, message, style_code, quantity, convert(varchar, getdate(), 101) release_date, distribution_number, ref_field_1, convert(varchar, expected_ship_date, 101) expected_ship_date from ##CNDistros3980 where document_number = ' + @X_shipment + 'order by style_code'
						select @X_date = replace(replace(replace(replace(convert(varchar, getdate(), 121), ' ', ''), '-', ''), ':', ''), '.', '')
						set @X_file_location = '\\kermode\FileRepository\MERCHANDISING\CN_DISTRO\OUTBOUND\Distros\'
						set @X_file_name = 'DISTRIBUTION_CN_3980' + cast(@X_rectype as varchar) + '-' + @X_location + '.' + @X_date + '.csv'
						set @X_server = 'bedrockdb02'
						set @X_database = 'me_01'
						set @X_bcp = 'bcp "' + @X_query + '" queryout "' + @X_file_location + @X_file_name + '"  -T -t, -c -S' + @X_server 

						exec master..xp_cmdshell @X_bcp

						delete from ##CNDistros3980 where document_number = @X_shipment
						select @X_counter = count(distinct document_number) from ##CNDistros3980
