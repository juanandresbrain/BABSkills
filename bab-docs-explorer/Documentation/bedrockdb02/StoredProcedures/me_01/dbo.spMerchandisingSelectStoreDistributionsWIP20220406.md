...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingSelectStoreDistributionsWIP20220406"]
    ...
    dbo_SkuReticketingListByCountry(["dbo.SkuReticketingListByCountry"]) --> SP
...
| dbo.location |
| dbo.rec_type |
| dbo.SkuReticketingListByCountry |
| dbo.sp_send_dbmail |
| dbo.STR_DIM |
| dbo.style |
| dbo.style_group |
...
```sql
CREATE proc [dbo].[spMerchandisingSelectStoreDistributionsWIP20220406]
as
-- =====================================================================================================
...
--		Dan Tweedie		2022-04-06		Added handling for reticketing skus for the email that is sent at the end
-- =====================================================================================================
set nocount on
			...
			--email report
				...
				set @text = '
					<font face =arial size = 2> ' +
						...
						'<br><br><b>**** PLEASE PAY EXTRA ATTENTION TO THE ITEMS WHICH NEED TO BE RE-STICKERED ****</b> ' +
						'<br><br><b>**** ITEMS TO BE RE-STICKERED ARE **** SURROUNDED  **** BY ADDTIONAL CHARACTERS ****</b> ' +
						'<br><br>' +
						'<table border="1">' +
						...
						CAST ( ( SELECT td = sd.Store,'',
										td = sd.RecTypeLabel, '',
										--td = case when StyleCode in ('057840') then '<span style="font-weight:bold">'+ StyleCode +'</span>' else StyleCode end, '',
										td = case when rt.StyleCode is not null then '****'+ sd.StyleCode +'****' else sd.StyleCode end, '',
										td = sd.StyleShortdescription, '',
										td = sum(sd.Quantity), '',
										td = sd.Category, ''
										/*,td = Type, ''*/ -- Business agreed that misc carton/type info is no longer needed. LT 03/04/2020
								 from me_01.dbo.TMP_US_Store_Distribution_Data sd
								 join location l with (nolock) on sd.store=l.location_code
								 join address a with (nolock) on l.location_id=a.parent_id and a.parent_type=2
								 join country c with (nolock) on a.country_id=c.country_id
								 --left join SkuReticketingListByCountry rt on sd.StyleCode=rt.StyleCode and c.country_code=rt.country
								 left join SkuReticketingListByCountry rt on cast(sd.StyleCode as int)=cast(rt.StyleCode as int) and case when c.country_code='USA' then 'US' else c.country_code end=rt.country
								 where sd.store = @store_nbr
								 group by 
									sd.Store,
									sd.RecTypeLabel,
									case when rt.StyleCode is not null then '****'+ sd.StyleCode +'****' else sd.StyleCode end,
									sd.StyleShortDescription,
									sd.Category
								 order by 
									case when rt.StyleCode is not null then '****'+ sd.StyleCode +'****' else sd.StyleCode end, 
									case sd.rectypelabel 
										when 'Ground Shipping' then 1 
										when 'Regular Truck' then 2 
										when 'Supply 2nd delivery' then 3 
										when 'SUPPLY 3RD DELIVERY' then 4 
										else 5 
									end
