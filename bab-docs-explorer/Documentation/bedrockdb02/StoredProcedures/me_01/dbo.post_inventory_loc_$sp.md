...
```sql
CREATE PROCEDURE [dbo].[post_inventory_loc_$sp] 
...
/* 
Proc name: post_inventory_loc_$sp 
Description: 
	For the given document and location:
		1. Posts the actual shrink (500) records into ib_inventory and pack actual shrink (1500) records into ib_pack_inventory
...
/*--------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------*/
-- First deal with packs

	/*--------------------------------------------------------------------------------------------------------------*/
	-- Inserting of records into ib_inventory for packs

	INSERT INTO
		ib_pack_inventory (pack_id, location_id, transaction_date, transaction_type_code, document_number, transaction_units)
	SELECT
		inventory_count_detail.pack_id,
		@LocId location_id,
		@DocDate transaction_date,
		N'1500' transaction_type_code,
		@DocNo document_number,
		inventory_count_detail.units_counted - inventory_count_detail.book_pack_units transaction_units
	FROM 
	...
	WHERE 
		...
		AND inventory_count_detail.units_counted - ISNULL(inventory_count_detail.book_pack_units, 0) <> 0 
		AND inventory_count_detail.pack_id IS NOT NULL

	/*--------------------------------------------------------------------------------------------------------------*/
	-- Update inventory_count_detail table with shrink numbers for reporting purposes

	UPDATE
		inventory_count_detail
	SET
		inventory_count_detail.pack_shrink_units = A.pack_shrink_units
	FROM
		inventory_count_detail,
		(
			SELECT
				ib_pack_inventory.pack_id,
				SUM(ib_pack_inventory.transaction_units) pack_shrink_units
			FROM
				ib_pack_inventory
			WHERE
				ib_pack_inventory.location_id = @LocId
				AND ib_pack_inventory.document_number = @DocNo
				AND ib_pack_inventory.transaction_type_code = N'1500'
				AND ib_pack_inventory.transaction_date = @DocDate
			GROUP BY
				ib_pack_inventory.pack_id
		) A
	WHERE
		inventory_count_detail.pack_id = A.pack_id
...
/*--------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------*/
-- Finally, handle skus (pseudo-styles and skus can be handled at the same time)
	...
	/*--------------------------------------------------------------------------------------------------------------*/
	-- Post actual shrink records (Pseudo-styles)
	...
	WHERE 
		inventory_count_detail.inventory_control_loc_id = @IclId 
		AND inventory_count_detail.inventory_control_id = @DocId  
		AND (inventory_count_detail.extended_units_counted - inventory_count_detail.total_oh_book_units <> 0)
		AND inventory_count_detail.total_retail IS NOT NULL
	...
	/*--------------------------------------------------------------------------------------------------------------*/
	-- Post actual shrink records (Regular skus)
	...
	SELECT 	
		...
		(inventory_count_detail.extended_units_counted * inventory_count_detail.selling_unit_retail) - (inventory_count_detail.total_oh_book_units * inventory_count_detail.selling_unit_retail) transaction_valuation_retail
	FROM 
		inventory_count_detail WITH (NOLOCK)
	WHERE 
		inventory_count_detail.inventory_control_loc_id = @IclId 
		...
		AND inventory_count_detail.average_cost IS NOT NULL
		AND inventory_count_detail.pack_id IS NULL
		AND (inventory_count_detail.extended_units_counted - inventory_count_detail.total_oh_book_units <> 0 OR (inventory_count_detail.extended_units_counted * inventory_count_detail.average_cost - inventory_count_detail.total_oh_book_cost) <> 0)

	/*--------------------------------------------------------------------------------------------------------------*/
	-- Update the shrink units column in the inventory_count_detail table with the transaction_units of the 500 records just inserted

	UPDATE
		inventory_count_detail
	SET
		inventory_count_detail.shrink_units = A.shrink_units,
		inventory_count_detail.shrink_cost = A.shrink_cost,
		inventory_count_detail.shrink_valuation_retail = A.shrink_valuation_retail,
		inventory_count_detail.shrink_selling_retail = A.shrink_selling_retail
	FROM
		inventory_count_detail,
		(
			SELECT
				#ib_inventory.sku_id,
				SUM(#ib_inventory.transaction_units) shrink_units,
				SUM(#ib_inventory.transaction_cost) shrink_cost,
				SUM(#ib_inventory.transaction_valuation_retail) shrink_valuation_retail,
				SUM(#ib_inventory.transaction_selling_retail) shrink_selling_retail
			FROM
				#ib_inventory
			WHERE
				#ib_inventory.location_id = @LocId
				AND #ib_inventory.document_number = @DocNo
				AND #ib_inventory.transaction_type_code = N'500'
				AND #ib_inventory.transaction_date = @DocDate
			GROUP BY
				#ib_inventory.sku_id
