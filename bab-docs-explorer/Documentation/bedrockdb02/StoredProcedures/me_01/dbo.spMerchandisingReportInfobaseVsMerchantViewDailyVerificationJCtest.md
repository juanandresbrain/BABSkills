...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingReportInfobaseVsMerchantViewDailyVerificationJCtest"]
...
```sql
CREATE proc [dbo].[spMerchandisingReportInfobaseVsMerchantViewDailyVerificationJCtest]
...
--		Justin Cross	01/22/2024		Altered filters for yesterday 
-- =====================================================================================================
...
WHERE a.document_number IN (SELECT DISTINCT po_no FROM po WHERE create_date <= CONCAT(CONVERT(varchar, DATEADD(day,-1,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00')), 23),' 00:00:00'))
	...
	(select   style_code, color_code, isnull(sum(on_order_units),0) as current_on_order
		...
		where   merch_period <= 
			(select merch_period 
				...
				where convert (char,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'),111) = convert (char,calendar_date,111)) 
				or convert (char,receipt_date,111) <= convert (char,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'),111) 
				and merch_year <= datepart(yy,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'))
...
left join 
	(select style_code, color_code, isnull(sum(on_order_units),0) as on_order_1
		...
		where	merch_period =	
			(select merch_period
				from 	calendar_date  with (nolock)
				where 	convert (char,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'),111) = convert (char,calendar_date,111))+1
				and merch_year = datepart(yy,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'))
...
left join
	(select style_code, color_code, isnull(sum(on_order_units),0) as on_order_2
		...
		where	merch_period = 
			(select merch_period
				from 	calendar_date  with (nolock)
				where 	convert (char,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'),111) = convert (char,calendar_date,111))+2
				and merch_year = datepart(yy,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'))
...
left join
	(select style_code, color_code, isnull(sum(on_order_units),0) as on_order_3
		...
		where	merch_period = 
			(select merch_period
				from 	calendar_date  with (nolock)
				where 	convert (char,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'),111) = convert (char,calendar_date,111))+3
				and merch_year = datepart(yy,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'))
...
exec msdb.dbo.sp_send_dbmail
	 ...
	 @recipients = 'justincr@buildabear.com',
	 @body = @text,
	 @subject= @subj,
	 @body_format = 'HTML'
```
