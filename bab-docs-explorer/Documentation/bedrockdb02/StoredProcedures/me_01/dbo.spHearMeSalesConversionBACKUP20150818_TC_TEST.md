# dbo.spHearMeSalesConversionBACKUP20150818_TC_TEST
...
```mermaid
flowchart LR
    SP["dbo.spHearMeSalesConversionBACKUP20150818_TC_TEST"]
...
```sql
create procedure [dbo].[spHearMeSalesConversionBACKUP20150818_TC_TEST]
...
--		Dan Tweedie		08/07/2015		Added code to stage data for insert into ib_inventory_total, and call Aptos proc. Code is commented out.
-- =====================================================================================================
...
begin
	...
	-- Create sales under Master/Blank sound instead
	insert into ib_inventory
	select  sk_m.sku_id, 
			t.location_id, 
			t.price_status_id, 
	...
	-- Update ib_inventory_id so we capture any new transctions, even if they are backdated.
	update  ib_inventory_id_hearme
	set ib_inventory_id = (select max(ib_inventory_id) as ib_inventory_id from ib_inventory)
	--------------------------------------------------------------------------------------------------
	...
	-------STAGE DATA AND CALL PROC TO INSERT INTO IB_INVENTORY_TOTAL
	/*
