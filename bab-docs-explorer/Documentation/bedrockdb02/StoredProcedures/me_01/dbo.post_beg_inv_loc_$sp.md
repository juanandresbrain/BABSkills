...
```sql
CREATE PROCEDURE [dbo].[post_beg_inv_loc_$sp] 
...
AS
/* 
Proc name: post_beg_inv_loc_$sp 
Description: 
	For the given document and location:
		1. Posts the beginning inventory (590) records into ib_inventory and pack beginning inventory (1590) records into ib_pack_inventory
...
-- First deal with packs
	...
	SELECT
		...
		N'1590' transaction_type_code,
		@DocNo document_number,
		inventory_count_detail.units_counted transaction_units
	FROM 
...
-- Finally, handle skus (pseudo-styles and skus can be handled at the same time)
	...
	/*--------------------------------------------------------------------------------------------------------------*/
	-- Post beginning inventory records (pseudo-styles)
	...
	SELECT 
		...
		N'590' transaction_type_code, 
		@DocNo document_number, 
		@DocDate transaction_date, 
		inventory_count_detail.extended_units_counted transaction_units, 
		inventory_count_detail.cost transaction_cost, 
		inventory_count_detail.total_valuation_retail transaction_valuation_retail,
		inventory_count_detail.total_retail transaction_selling_retail
	FROM 
		inventory_count_detail WITH (NOLOCK)
	WHERE 
		inventory_count_detail.inventory_control_loc_id = @IclId 
		AND inventory_count_detail.inventory_control_id = @DocId  
		AND inventory_count_detail.extended_units_counted <> 0 
		AND inventory_count_detail.pack_id IS NULL
		AND inventory_count_detail.total_retail IS NOT NULL

	/*--------------------------------------------------------------------------------------------------------------*/
	-- Post beginning inventory records (regular skus)
	...
	SELECT 
		...
		@DocDate transaction_date, 
		inventory_count_detail.extended_units_counted transaction_units, 
		inventory_count_detail.extended_units_counted * inventory_count_detail.average_cost transaction_cost, 
		inventory_count_detail.extended_units_counted * inventory_count_detail.valuation_unit_retail transaction_valuation_retail,
		inventory_count_detail.extended_units_counted * inventory_count_detail.selling_unit_retail transaction_selling_retail
	FROM 
		inventory_count_detail WITH (NOLOCK)
	WHERE 
		inventory_count_detail.inventory_control_loc_id = @IclId 
...
/*--------------------------------------------------------------------------------------------------------------*/
	...
	DROP TABLE #ib_inv_retro_post
	DROP TABLE #ib_inv_retro_post_work
	DROP TABLE #MAX_ID
