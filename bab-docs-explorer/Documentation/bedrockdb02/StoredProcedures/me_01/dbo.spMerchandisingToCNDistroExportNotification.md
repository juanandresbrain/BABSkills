# dbo.spMerchandisingToCNDistroExportNotification
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingToCNDistroExportNotification"]
...
```sql
CREATE proc [dbo].[spMerchandisingToCNDistroExportNotification]

as

-- =====================================================================================================
-- Name: spMerchandisingToCNDistroExportNotification
--
...
--		Tim Callahan	04/11/2017		Remarked out location 3980 on 4/11/2017 due to peformance issues 
--		Tim Callahan	06/08/2018		Added locations 8502 and 8505 to job logic
-- =====================================================================================================
...
select @distro_transfers = count(*)
		...
		and	l1.location_code in ('3970','8502','8505', '3980')
...
select @distros = count(*)
					...
					and		l1.location_code in ('3970','8502','8505') --'3980' Remarked out on 4/11/2017
					and		dd.quantity > 0
					...
					and l2.location_code in (select distinct idestid
							...
							where iSourceID in ('3970','8502','8505','3980') 
...
select @distro_split = count(*)
		...
		where ds.sourceid in ('3970','8502','8505','3980')
...
select @distro_after_split = count(*)
		...
		where ddas.sourceid in ('3970','8502','8505','3980')
...
--output summary of distros exported to wm
	...
	select @subj = 'Merch-to-CN Distro Export: ' + cast(getdate() as varchar)
	
	set @text = '<font face =arial size = 2></font>' + 
		'<font face =arial size = 2>' + 
		'The Merch-to-CN Distro Export Process Has Completed.' +
		'<br>'+
	...
	if datepart(hh, getdate()) >= 18 and (@distro_transfers > 0 or @distros > 0 or @distro_split > 0 or @distro_after_split > 0)
	begin
	select @recip = 'EnterpriseSystemsAlerts@buildabear.com'
	select @subj = 'CN Distro Export Error'
