# dbo.spMerchandisingFTPUKDistro_WinSCP
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingFTPUKDistro_WinSCP"]
...
```sql
CREATE proc [dbo].[spMerchandisingFTPUKDistro_WinSCP]
...
insert #DEL exec master..xp_cmdshell 'dir \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Outbound\DistroUpload.log /B'
insert #DEL exec master..xp_cmdshell 'dir \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Outbound\SFTP_upload_UK_MonitorLog.txt /B'
delete from #DEL where output is null or output = 'File Not Found'


IF (select count(*) from #DEL where output = 'DistroUpload.log') > 0
	begin
		exec master..xp_cmdshell 'del \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Outbound\DistroUpload.log'
	end
IF (select count(*) from #DEL where output = 'SFTP_upload_UK_MonitorLog.txt') > 0
	begin
		exec master..xp_cmdshell 'del \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Outbound\SFTP_upload_UK_MonitorLog.txt'
	end
...
BEGIN
			-----ftp upload
					...
					select 
							...
							@script = ' /script=\\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Scripts\Distros\DistroUpload.txt',
							@log = ' /log=\\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Outbound\DistroUpload.log',
							@SFTP = concat(@winSCP, @ini, @script, @log)

					--create temp tables for ftp logs
					IF (Object_ID('me_01..SFTP_upload_UK_Distro') IS NOT NULL) DROP TABLE SFTP_upload_UK_Distro
					...
					-- This step may ultimately be disabled once we are comfortbale with the new SFTP process, this was put in place when we had routine issues with uploading via FTP   						
									...
									set @Log_query = 'select * from bedrockdb02.me_01.dbo.SFTP_upload_UK_Distro'
									...
									@body= @body, 
									@file_attachments = '\\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Outbound\SFTP_upload_UK_MonitorLog.txt'
									*/

					--if connection unsuccessful, send email
							if (select count(*) from SFTP_upload_UK_Distro where ftplog like 'DISTRIBUTION%.csv%100[%]') < 1
								begin																	
									set @Log_query2 = 'select * from bedrockdb02.me_01.dbo.SFTP_upload_UK_Distro'
									...
									set @body2 =	'An attempt to FTP a UK Distro to Clipper failed.' 
												...
												+ char(10) + char(13) + 
												+ char(10) + char(13) + 
												'This process is managed by bedrockdb02.me_01.dbo.spMerchandisingFTPUKDistro_WinSCP'
									...
									@body = @body2,
									@file_attachments = '\\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Outbound\SFTP_upload_UK_DistroLog.txt;\\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Outbound\DistroUpload.log',
									@importance = 'HIGH'
