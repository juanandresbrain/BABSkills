# dbo.spWEBPricebookStage_TCOnDemand
...
```mermaid
flowchart LR
    SP["dbo.spWEBPricebookStage_TCOnDemand"]
...
```sql
CREATE proc [dbo].[spWEBPricebookStage_TCOnDemand]
@BryceDate date = NULL
...
--	2022-07-06 - Made changes as related to  JIRA BIB402, specificaly changed some comparisons from less than to not equal 
--------------------------------------------------------------------------------------------------
set nocount on
			...
			(
					(
						...
						isnull(cast(pricechange.effective_from_date as date), '3030-12-31') 
							<= 
								case when datepart(hh, @RunDate) >= 20 -- Temp change to 17 from 23
