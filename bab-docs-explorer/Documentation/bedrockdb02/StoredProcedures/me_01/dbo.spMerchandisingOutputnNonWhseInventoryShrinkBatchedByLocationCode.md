# dbo.spMerchandisingOutputnNonWhseInventoryShrinkBatchedByLocationCode
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingOutputnNonWhseInventoryShrinkBatchedByLocationCode"]
...
```sql
CREATE proc [dbo].[spMerchandisingOutputnNonWhseInventoryShrinkBatchedByLocationCode]
@LocationCode int
as
...
select @whses = count(distinct location_code) from tmpNightlyNonWhseInventoryShrink where location_code = @LocationCode 
while @whses > 0
begin
	select @whse = min(location_code) from tmpNightlyNonWhseInventoryShrink where location_code = @LocationCode 
