...
```sql
create proc [dbo].[insert_pseudo_$sp] 
(
			...
			FROM
				(
					...
					SELECT
						...
						inventory_count_detail.cost cost_local,
...
-- PSEUDO-STYLE / INTERNATIONATIONALIZATION CHANGES
	...
	UPDATE
		inventory_count_detail
	SET
		cost = (total_oh_book_cost / total_oh_book_val_retail) / (total_retail * @ExchangeRate),
		total_cost = (total_oh_book_cost / total_oh_book_val_retail) / (total_retail * @ExchangeRate)
	WHERE
		inventory_count_detail.total_retail IS NOT NULL
		...
		AND inventory_count_detail.total_oh_book_val_retail <> 0


	--Multi currency mod. For those pseudo-styles that don't have a cost_local, calculate cost_local based on total_oh_book_cost_local
	UPDATE
		inventory_count_detail
	SET
		cost_local = (total_oh_book_cost_local / total_oh_book_sell_retail) / (total_retail),
		total_cost_local = (total_oh_book_cost_local / total_oh_book_sell_retail) / (total_retail)
	WHERE
		inventory_count_detail.total_retail IS NOT NULL
		...
		AND inventory_count_detail.pack_id IS NULL
		AND inventory_count_detail.total_retail <> 0
		AND inventory_count_detail.total_oh_book_sell_retail <> 0
