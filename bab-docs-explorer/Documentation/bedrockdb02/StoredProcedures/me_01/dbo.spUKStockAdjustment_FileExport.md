# dbo.spUKStockAdjustment_FileExport
...
```mermaid
flowchart LR
    SP["dbo.spUKStockAdjustment_FileExport"]
...
```sql
CREATE proc [dbo].[spUKStockAdjustment_FileExport]
-- =====================================================================================================
-- Name: spUKStockAdjustment_FileExport
--
-- Description:	Returns data in format to meet Epicor requirements for Stock Adjustment file.
--				This proc is executed from bedrockdb02.me_01.spUKStockAdjustment into a text file.
--
-- Input:	NA
--			
--
...
--			
--
-- Dependencies: bedrockdb02.me_01.spUKStockAdjustment
--
-- Revision History
--		Name:			Date:			Comments:
...
declare header cursor for
		select distinct 'H', 'A', 
			((convert(varchar, datepart(yyyy, getdate())) + convert(varchar, datepart(mm, getdate())) + convert(varchar, datepart(dd, getdate())) + convert(varchar, datepart(hh, getdate())) + convert(varchar, datepart(mi, getdate()))) + convert(varchar, @counter_h)), 
				@date, '1', 'UKStockAdjFile', description, '3', 'UKClipper'
		from qty_convert
...
while @counter_h <= @total_headers
	begin
		...
		declare detail cursor for
			select 'D', 'A', @documentno_h, '2970', ('000000' + style), converted_qty
			from qty_convert
