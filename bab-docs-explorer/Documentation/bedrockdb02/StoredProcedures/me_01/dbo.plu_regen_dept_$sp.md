...
```sql
CREATE PROCEDURE [dbo].[plu_regen_dept_$sp]
AS
...
/*
	...
	Description	: Procedure called by Segment 1038 -- EDM & PROD to Price Look-Up File Generate (CRS)
				  Gets all department and department class records
...
BEGIN TRY
	...
	INNER JOIN ( merch_group_parent_full DeptMerchGroupParent
					...
					INNER JOIN hierarchy Hierarchy ON HierarchyCharLevel.hierarchy_id = Hierarchy.hierarchy_id
														AND Hierarchy.hierarchy_type = 1 AND Hierarchy.alternate_flag = 0 ) ON HierarchyGroup.hierarchy_group_id = DeptMerchGroupParent.parent_hierarchy_group_id
	...
	INNER JOIN ( merch_group_parent_full DeptClassMerchGroupParent
					...
					INNER JOIN hierarchy Hierarchy ON HierarchyCharLevel.hierarchy_id = Hierarchy.hierarchy_id
														AND Hierarchy.hierarchy_type = 1 AND Hierarchy.alternate_flag = 0 ) ON HierarchyGroup.hierarchy_group_id = DeptClassMerchGroupParent.parent_hierarchy_group_id
	INNER JOIN ( merch_group_parent_full DeptMerchGroupParent 
					INNER JOIN #dept TempDept ON TempDept.dept_id = DeptMerchGroupParent.parent_hierarchy_group_id
													AND TempDept.hierarchy_level_id = DeptMerchGroupParent.parent_hierarchy_level_id ) ON HierarchyGroup.hierarchy_group_id = DeptMerchGroupParent.hierarchy_group_id
