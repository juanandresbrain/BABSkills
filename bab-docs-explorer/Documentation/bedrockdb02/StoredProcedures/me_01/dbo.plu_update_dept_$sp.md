...
```sql
CREATE PROCEDURE [dbo].[plu_update_dept_$sp]
AS
...
/*
	...
	Call from C++ code:
		...
		-- Function: LoadDepartmentFileDefs
...
BEGIN TRY
	...
	INNER JOIN ( merch_group_parent_full DeptMerchGroupParent
					...
					INNER JOIN hierarchy Hierarchy ON HierarchyCharLevel.hierarchy_id = Hierarchy.hierarchy_id
														AND Hierarchy.hierarchy_type = 1 AND Hierarchy.alternate_flag = 0 ) ON Department.hierarchy_group_id = DeptMerchGroupParent.hierarchy_group_id
	...
	INNER JOIN ( merch_group_parent_full DeptClassMerchGroupParent
					...
					INNER JOIN hierarchy Hierarchy ON HierarchyCharLevel.hierarchy_id = Hierarchy.hierarchy_id
														AND Hierarchy.hierarchy_type = 1 AND Hierarchy.alternate_flag = 0 ) ON Department.hierarchy_group_id = DeptClassMerchGroupParent.hierarchy_group_id
	INNER JOIN ( merch_group_parent_full DeptMerchGroupParent 
					INNER JOIN #dept TempDept ON TempDept.dept_id = DeptMerchGroupParent.parent_hierarchy_group_id
													AND TempDept.hierarchy_level_id = DeptMerchGroupParent.parent_hierarchy_level_id ) ON Department.hierarchy_group_id = DeptMerchGroupParent.hierarchy_group_id
