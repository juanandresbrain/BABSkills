...
```mermaid
flowchart LR
    SP["dbo.get_avg_cost_sku_location_$sp"]
...
CREATE PROCEDURE dbo.get_avg_cost_sku_location_$sp
...
/*
	Get dynamic average cost by sku/location
...
FROM
	...
	INNER JOIN (SELECT sku_id, location_id, SUM(total_on_hand_units) as total_on_hand_units,
						...
						GROUP BY sku_id, location_id) IBIT ON IBIT.sku_id = TWACL.sku_id AND IBIT.location_id = TWACL.location_id
	LEFT JOIN
...
EXEC dbo.get_avg_cost_style_location_$sp
