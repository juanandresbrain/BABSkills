# dbo.pop_25Nov25_temp_ib_inv_$sp
...
```mermaid
flowchart LR
    SP["dbo.pop_25Nov25_temp_ib_inv_$sp"]
...
```sql
CREATE PROCEDURE [dbo].[pop_25Nov25_temp_ib_inv_$sp]
...
-----------------------------------------------------------------------------------------------------------------------------
	...
	BEGIN TRANSACTION
		...
		INSERT INTO temp_25Nov25_ib_inv
		...
		FROM
			...
			(
				...
				FROM
					...
					,dbo.temp_25Nov25_sale_master m
				WHERE
...
-----------------------------------------------------------------------------------------------------------------------------
	...
	-- difference will be calculated then we are going to insert these transactions back to temp_25Nov25_ib_inv
...
-----------------------------------------------------------------------------------------------------------------------------
	...
	SELECT @job_id
		...
		,i.register
	FROM im_sale_25nov25 i, transaction_reason r , temp_25Nov25_sale_master m
	WHERE
		i.im_sale_number BETWEEN @min_im_sale_number AND @max_im_sale_number
	AND i.location_id BETWEEN @min_location_id AND @max_location_id
	AND i.aw_transaction_type = @c_discount_type
	...
	SELECT @job_id
		...
		FROM
			dbo.im_sale_25nov25 s
			INNER JOIN dbo.temp_25Nov25_sale_master m ON m.transaction_date = s.transaction_date
	...
	-- we could insert back these transactions into temp_25Nov25_ib_inv
	BEGIN TRAN
	INSERT INTO temp_25Nov25_ib_inv
		( job_id
		,sku_id
	...
	BEGIN
		...
		SELECT t.job_id, t.sku_id, t.location_id, t.inventory_status_id, t.price_status_id, t.transaction_date,
			...
			,0 transaction_cost_local
			,t.register_no
		FROM #ib_exchange_rate t, temp_25Nov25_sale_master m
	...
	BEGIN TRAN
		...
		SELECT   @job_id
				...
				,i.register
			FROM im_sale_25nov25 i, style s, temp_25Nov25_sale_master m
			WHERE i.im_sale_number BETWEEN @min_im_sale_number AND @max_im_sale_number
			AND   i.location_id    BETWEEN @min_location_id AND @max_location_id
	...
	IF EXISTS (SELECT 1 FROM im_sale_25nov25 i, style s
				WHERE i.im_sale_number BETWEEN @min_im_sale_number AND @max_im_sale_number
				AND i.location_id BETWEEN @min_location_id AND @max_location_id
				AND i.aw_transaction_type = @c_layaway_deposit_type
				AND i.style_id = s.style_id
	...
	-- we are going to insert these transactions back to temp_25Nov25_ib_inv.
	...
	SELECT   @job_id
			...
			,- SUM(s.units * m.average_cost_local) transaction_cost_local
			,s.register
		FROM dbo.im_sale_25nov25 s, dbo.temp_25Nov25_sale_master m
	...
	SELECT   @job_id
			...
			,0 transaction_cost_local
			,s.register
		FROM dbo.im_sale_25nov25 s, dbo.temp_25Nov25_sale_master m, transaction_reason r
		WHERE s.im_sale_number BETWEEN @min_im_sale_number AND @max_im_sale_number
		AND s.location_id BETWEEN @min_location_id AND @max_location_id
	...
	BEGIN
		...
		FROM ib_inventory i
		INNER JOIN temp_25Nov25_sale_master m
			ON m.job_id = @job_id
	...
	-- we could insert then back into temp_25Nov25_ib_inv
	BEGIN TRAN
	INSERT INTO temp_25Nov25_ib_inv
		( job_id
		,sku_id
...
BEGIN CATCH
	...
	SELECT    @table_name		= N'temp_25Nov25_ib_inv'
