# dbo.spMerchandisingExportWebDistros_bak_20191023
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingExportWebDistros_bak_20191023"]
...
```sql
CREATE proc [dbo].[spMerchandisingExportWebDistros_bak_20191023]
	...
	IF (select count(*) from WMDB01.WMPROD.DBO.INPT_MASS_PULL where PROC_STAT_CODE = 0) = 0 
			BEGIN
				begin
					...
					and		ddas.distribution_number not in (select distribution_number from WEBSTORE_PROCESS_CONTROL) --updated at the end of the procedure, to prevent us from sending the same distro twice
					order by ddas.Id
				end
