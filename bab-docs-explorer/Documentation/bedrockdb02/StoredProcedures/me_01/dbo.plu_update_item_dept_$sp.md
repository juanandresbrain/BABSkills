...
```mermaid
flowchart LR
    SP["dbo.plu_update_item_dept_$sp"]
...
```sql
CREATE PROCEDURE [dbo].[plu_update_item_dept_$sp]
AS
...
/*
	...
	Description	: Procedure called by Segment 1038 -- EDM & PROD to Price Look-Up File Generate (CRS)
				  Gets all department and department class records for upcs requiring an update/resend
...
BEGIN TRY
	...
	FROM
		#all_upc Upc
	INNER JOIN style_group StyleGroup ON Upc.style_id = StyleGroup.style_id
