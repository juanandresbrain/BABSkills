...
```mermaid
flowchart LR
    SP["dbo.spMerchandising_980NightlySync_OnDemand"]
...
```sql
CREATE proc [dbo].[spMerchandising_980NightlySync_OnDemand]
as
-- =====================================================================================================
-- Name: spMerchandising_980NightlySync_OnDemand
--
-- Description:	Runs nightly sync for only warehouse 0980.
--
-- Input: N/A
--
-- Output: 
...
from 
(
select M.style_code, M.short_desc, M.style_type, M.distmult, sum(M.loc_0980) M0980
from 
	(select a.style_code,
		   a.short_desc, 
		   a.style_type,
...
join 
(
select M.style_code, M.short_desc, M.style_type, M.distmult, sum(M.loc_0980) W0980
from 
	(select a.style_code,
		   a.short_desc,
		   a.style_type,
...
-- Step 8b
Delete from tmpWhseInventoryShrink where location_code <> '0980'
...
begin
	...
	set @file_name = 'STSIMSA.WhseSync.0980' + @date + '.GO'
