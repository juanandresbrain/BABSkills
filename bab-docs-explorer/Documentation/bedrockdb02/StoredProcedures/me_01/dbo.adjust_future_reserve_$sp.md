...
```mermaid
flowchart LR
    SP["dbo.adjust_future_reserve_$sp"]
    ...
    dbo_im_reserve_receipt(["dbo.im_reserve_receipt"]) --> SP
...
```sql
CREATE PROCEDURE dbo.adjust_future_reserve_$sp
AS
BEGIN
...
-----------------------------------------------------------------------------------------------------------------------------
--	Exit procedure if nothing is on reservation
-----------------------------------------------------------------------------------------------------------------------------

IF NOT EXISTS (SELECT 1 FROM ib_future_inventory_reserve_total)
BEGIN

  IF NOT EXISTS (SELECT 1 FROM ib_inventory WHERE inventory_status_id = @Reserved_Status_Id)
  BEGIN
...
-----------------------------------------------------------------------------------------------------------------------------
-- Prepare data for ib_inventory and ib_future_inventory_reserve
-----------------------------------------------------------------------------------------------------------------------------
...
SELECT
  ...
  ,TAUA.location_id
  ,CASE
    WHEN TAUA.available_units_adjusted > 0-- AND COALESCE(AIIT.total_on_hand_units, 0) > 0
      THEN
        (CASE WHEN TAUA.available_units_adjusted < COALESCE(IFIRT.reserved_quantity, 0) THEN TAUA.available_units_adjusted ELSE COALESCE(IFIRT.reserved_quantity, 0) END)
    WHEN TAUA.available_units_adjusted < 0
      THEN
        (
          CASE WHEN 0 > COALESCE(AIIT.total_on_hand_units, 0) AND COALESCE(RIIT.total_on_hand_units, 0) > 0
            THEN
              (
                CASE WHEN -1 * COALESCE(AIIT.total_on_hand_units, 0) <= COALESCE(RIIT.total_on_hand_units, 0) THEN COALESCE(AIIT.total_on_hand_units, 0) ELSE -1 * COALESCE(RIIT.total_on_hand_units, 0) END
              )
            ELSE
              0
          END
  ...
  ,CASE
    WHEN PS.use_future_inventory_flag = 1 THEN
      (
         CASE
          WHEN TAUA.available_units_adjusted > 0-- AND COALESCE(AIIT.total_on_hand_units, 0) > 0
            THEN
              -1 * (CASE WHEN TAUA.available_units_adjusted < COALESCE(IFIRT.reserved_quantity, 0) THEN TAUA.available_units_adjusted ELSE COALESCE(IFIRT.reserved_quantity, 0) END)
          WHEN TAUA.available_units_adjusted < 0
            THEN
              (
                CASE WHEN 0 > COALESCE(AIIT.total_on_hand_units, 0) AND COALESCE(RIIT.total_on_hand_units, 0) > 0
                  THEN
                    (
                      CASE WHEN -1 * COALESCE(AIIT.total_on_hand_units, 0) <= COALESCE(RIIT.total_on_hand_units, 0) THEN -1 * COALESCE(AIIT.total_on_hand_units, 0) ELSE COALESCE(RIIT.total_on_hand_units, 0) END
                    )
                  ELSE
                    0
                END
...
BEGIN
  ...
  SELECT
    ...
    ,transaction_units
  FROM
    dbo.#temp_ib_inventory_total_update_values T
  WHERE
    inventory_status_id = @Reserved_Status_Id
    AND NOT EXISTS
      (
        SELECT 1
        FROM
...
INSERT INTO dbo.im_reserve_receipt
  (
    sku_id
    ,location_id
    ,reserved_quantity
...
SELECT
  sku_id
  ,location_id
  ,-1 * reserved_quantity AS reserved_quantity
  ,date_reserved
FROM
