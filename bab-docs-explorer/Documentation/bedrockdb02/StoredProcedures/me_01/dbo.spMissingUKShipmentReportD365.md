# dbo.spMissingUKShipmentReportD365
...
```mermaid
flowchart LR
    SP["dbo.spMissingUKShipmentReportD365"]
...
```sql
CREATE PROC [dbo].[spMissingUKShipmentReportD365]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- PARAMETERS
    ------------------------------------------------------------
    DECLARE @SourceId     varchar(4)   = '2970';
    DECLARE @FileRoot     varchar(500) = '\\kermode\filerepository\MERCHANDISING\UK_Distro\SHIPMENTS';
    ...
    DECLARE @Subject      varchar(255) = 'Missing UK Shipment Report - THIS IS A TEST PLEASE DO NOT TAKE ACTION '
                                         + CONVERT(varchar(10), GETDATE(), 120);
    DECLARE @D365BaseUrl  varchar(500) = 'https://buildabear.operations.dynamics.com';
    DECLARE @D365Company  varchar(10)  = '2110';
    ...
    DECLARE @GlobalCsvTable sysname      = '##MissingUKShipmentEmail_' + CAST(@@SPID AS varchar(20));
    DECLARE @Sql            nvarchar(max);
    DECLARE @NoIssueSubject varchar(255) = '';
    ...
    --    ADD WAREHOUSE FLAG FOR FILTERING ONLY
    ------------------------------------------------------------
    SELECT DISTINCT
        sse.DynamicsOrderId,
        ...
        sse.ExportDate,
        CASE 
            WHEN wm.[AreAdvancedWarehouseManagementProcessesEnabled] = 'Yes' THEN 1
            ELSE 0
        END AS IsAdvancedWH
    INTO #Base
    ...
    LEFT JOIN [STL-SSIS-P-01].[IntegrationStaging].[ERP].[WarehouseMaster] wm
        ON LTRIM(RTRIM(wm.[warehouseId])) = LTRIM(RTRIM(sse.destid))
    ...
    (
        SELECT
            ...
            MIN(IsAdvancedWH) AS IsAdvancedWH
        FROM #Base
        GROUP BY DynamicsOrderId
    ) t
    ...
    ------------------------------------------------------------
    -- APPLY UK FILTER RULE
    -- If file exists and warehouse is NOT advanced-enabled, do not include
    ------------------------------------------------------------
    DELETE
    FROM #TOSummary
    WHERE TotalFileHits > 0
      AND IsAdvancedWH = 0;
    ...
    <p class="summary"><strong>2110 Missing Inbound Shipment Report</strong><br/>
    ...
    BEGIN
        EXEC msdb.dbo.sp_send_dbmail
            ...
            @query_attachment_filename   = 'Missing_UK_Shipments_2110.csv',
    ...
    ELSE
    BEGIN
        SET @NoIssueSubject = 'No Missing UK Shipments - ' + CONVERT(varchar(10), GETDATE(), 120);
