...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingReportInfobaseVsMerchantViewDailyVerificationJC2"]
...
```sql
CREATE proc [dbo].[spMerchandisingReportInfobaseVsMerchantViewDailyVerificationJC2]
...
--		Justin Cross	02/07/2024		Added Text Alert for IF PROBLEM 
-- =====================================================================================================
...
where c2.calendar_date = cast(cast(Month(CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00')) as varchar(2)) + '/' + cast(Day(CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00')) as varchar(2)) + '/' + cast(Year(CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00')) as varchar(4)) as datetime)
...
where c.calendar_date =  cast(convert(varchar,CONCAT(CONVERT(varchar, DATEADD(day,-1,getdate()), 23),' 00:00:00'),1) as smalldatetime)
...
'<br>' +
'<b>Technical Details:' +
'<br>SQL Agent Job on Kermode:</b> MERCHANDISING - Report - Infobase-MerchantViewDailyVerification_CA'+
'<br><b>SQL Stored Procedure on Bedrockdb02.me_01:</b> spMerchandisingReportInfobaseVsMerchantViewDailyVerificationJC2' +
...
set @file_name = 'InfobaseVsMerchantviewJC2.TXT'
...
SELECT @subj = CASE
        WHEN @maTotalOnOrder - @ibTotalOnOrder <> 0
             OR (@iohunits_mev_group - @iohunits_ib > 1
			 OR @iohunits_mev_group - @iohunits_ib < 0)
             OR (@iOnhand_MeV_StyleClr - @iohunits_ib > 1
            ...
            THEN 'Infobase-MerchantView Daily Verification Results - PROBLEM'
        ELSE 'Infobase-MerchantView Daily Verification Results - NO PROBLEM'
    END

-- Send email with body to regular recipients
IF @subj = 'Infobase-MerchantView Daily Verification Results - PROBLEM'
BEGIN
    SET @recipients = 'EnterpriseSystemsAlerts@buildabear.com; JeffK@buildabear.com'
END
ELSE
BEGIN
    SET @recipients = 'EntSysSupport@buildabear.com; JeffK@buildabear.com'
END
...
-- Send email with only subject to enterprisesystemsalerts@buildabear.com
IF @subj = 'Infobase-MerchantView Daily Verification Results - PROBLEM'
BEGIN
    EXEC msdb.dbo.sp_send_dbmail
        @profile_name = 'merchadmin',
        ...
        @subject = @subj,
        @body = 'There are differences between IB and MA. Please re run MERCHANDISING - Process - Pipeline Sales Posting once that completes please run MERCHANDISING - Report - InfoBase-MerchantviewDailyVerification_CA on bedrockdb02',
        @body_format = 'HTML'
END
```
