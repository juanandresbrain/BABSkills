...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingDBSchenkerPOExport_3_SelectPoData"]
...
```sql
CREATE proc [dbo].[spMerchandisingDBSchenkerPOExport_3_SelectPoData]
as
-- =====================================================================================================
...
--		Dan Tweedie		2018-07-02		Added substring(hg.hierarchy_group_code,7,2)<> '60' to exclude supplies
--		Lizzy Timm		06/30/2021		Updated filter in PART THREE from 60 days to 75 days per SR 30894
...
BEGIN
	...
	and	  l.location_code in ('0980','0960','0013','9999','0975','2970','2999', '2013','1971','1972','3970','3980','9942','8502','8505') -- Added More China Warehouses 5/25/2018; Added 9942 LT 01/10/25
	...
	*/
	...
	and location_code in ('0980','0960','0013','9999','0975','2970','2999', '2013','1971','1972','3970','3980','9942','8502','8505') -- Location Sepcification added 09/08/2015 -- Added China Warehouses on 2/10/2016 -- Added More China Warehouses 5/25/2018; Added 9942 LT 01/10/25
	...
	case 
		...
		then 
			...
			case 
				when l1.location_code in ('3970','3980','9942','8502','8505') -- Changed from = '0975' aka Canada on 2/10/2016 -- Added 8502 and 8505 on 5/25/2018; Added 9942 LT 01/10/25
				then 
					case 
						when substring(hg.hierarchy_group_code,7,2)= '60'
	...
	WHERE substring(hg.hierarchy_group_code,7,2) <> '60' --EXCLUDES SUPPLIES 2018-07-02
	and e.total_line_loc_ordered_units <> 0
	-- and i.upc_number < '000001000000' -- Removed on 3/22/2016 due to conflict with "true" UPC's
		...
		*/
		and (	a.import_flag = 1 --ensures we only capture non-domestic vendors
				and l1.location_code in ('0980','0960','0013','9999','0975','2970','2999','2013','1971','1972','3970','3980','9942','8502','8505') -- Added China Warrhouses on 5/25/2018; Added 9942 LT 01/10/25
		...
		and (
			(
				(d.po_id in (select distinct po_id from view_po_shipment_rep (nolock) where date_type_code = 100 and datediff(dd, getdate()+75, user_defined_date) = 0) -- 06/30/21 Changed from 60 to 75;
			...
			AND ((getdate() < f.user_defined_date and datediff(dd, getdate(), f.user_defined_date) <= 75) -- 06/30/21 Changed from 60 to 75
				 OR (getdate() > f.user_defined_date and datediff(dd, f.user_defined_date, getdate()) <= 75)) --05/29/2014 - Changed from 30 to 60; 06/30/21 Changed from 60 to 75
			)
			)
							...
							(SELECT distinct e.style_code, e.long_desc, left(hg.hierarchy_group_code,8) department,
								...
								case 
									...
									then 
										...
										case 
											when l1.location_code in ('3970','3980','9942','8502','8505') -- Changed from 0975 aka Canada on 2/11/2016 -- Added 8502 and 8505 on 5/25/2018; Added 9942 LT 01/10/25
											then 
												case 
													when substring(hg.hierarchy_group_code,7,2)= '60'
								...
								case 
										...
										case when l1.location_code in ('3970','3980','9942','8502','8505') -- Changed from = '0975'  on 2/10/2016 --Added 8502 and 8505 on 5/25/2018; Added 9942 LT 01/10/25
								...
								AND (a.import_flag = 1 
									...
									-- and i.upc_number < '000001000000' -- Removed on 3/22/2016 due to conflict with "true" UPC's
									and s.active_flag = 1
									and l1.location_code in ('0980','0960','0013','9999', '0975', '2970','2999','2013','1971','1972','3970','3980','9942','8502','8505') -- Added 8502 and 8505 on 5/25/2018; Added 9942 LT 01/10/25
