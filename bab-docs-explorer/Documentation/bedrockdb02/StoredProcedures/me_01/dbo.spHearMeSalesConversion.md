...
```mermaid
flowchart LR
    SP["dbo.spHearMeSalesConversion"]
...
```sql
CREATE procedure [dbo].[spHearMeSalesConversion]
...
--		Lizzy Timm		08/26/2019		Added code to remove records with null values
--		Lizzy Timm		01/28/2020		Added code to archive records with null values
--		Lizzy Timm		03/04/2021		Commented out email summary sent to MerchAdmin as a part of Jira ESS-2627 - MerchAdmin Alert Redesign
--		Lizzy Timm		03/29/2021		Added logic to set cost to $1 if it is null
-- =====================================================================================================
...
begin
	...
	-- temporarly added for conversion issues with location id 999
	/* INSERT tblHearMeSalesConversionArchive_original_01232020LT
	select * 
	from tblHearMeSalesConversionArchive_original
    ...
    select  t.ib_inventory_id,  --Added for failed archives
			t.sku_id, 
			t.location_id, 
			t.price_status_id, 
			t.transaction_date, 
	...
	SELECT ib_inventory_id, --Added for failed archives
			sku_id, 
		   location_id, 
		   price_status_id,
		   transaction_date,
	...
	SELECT @nullCheck = COUNT(*) FROM tmpStageIb_Inventory_Total
		WHERE [sku_id] IS NULL
			...
			OR transaction_cost_local IS NULL

	--Check for NULLs in manditory fields, excluding [transaction_cost]
	SELECT @nullCheckB = COUNT(*) FROM tmpStageIb_Inventory_Total
		WHERE [sku_id] IS NULL
			OR [location_id] IS NULL
			OR [price_status_id] IS NULL
			OR [transaction_date] IS NULL
	...
	BEGIN
		...
		SELECT [ib_inventory_id]
		  ...
		  ,[register_no]
		 ,''
		FROM tblHearMeSalesConversionArchive
		WHERE ib_inventory_id IN (SELECT DISTINCT ib_inventory_id 
		...
		--Set Universal Sounds to default cost of $1 if null (update is made here to ensure the record populates tblHearMeSalesConversionArchive)
		UPDATE tmpStageIb_Inventory_Total
		SET [transaction_cost] = cast ('1' as decimal(10,2))
		WHERE [transaction_cost] IS NULL

		IF(@nullCheckB > 0)
		--IF(@nullCheck > 0) -- temporary to ensure $1 update works
			BEGIN
				--Send alert
				...
				SET @xml = CAST(( SELECT ISNULL(CAST([ib_inventory_id] AS varchar), '-') AS 'td',''
								,ISNULL(CAST([sku_id] AS varchar), 'MISSING') AS 'td',''
								,ISNULL(CAST([location_id] AS varchar), 'MISSING') AS 'td',''
								,ISNULL(CAST([price_status_id] AS varchar), 'MISSING') AS 'td',''
								,ISNULL(CONVERT(nvarchar(30),[transaction_date],101), 'MISSING') AS 'td',''
								,ISNULL(CAST([transaction_type_code] AS varchar), 'MISSING') AS 'td',''
								,ISNULL(CAST([inventory_status_id] AS varchar), 'MISSING') AS 'td',''
								...
								,ISNULL(CAST([transaction_units] AS varchar), 'MISSING') AS 'td',''
								,ISNULL(CAST([transaction_cost] AS varchar), 'MISSING and is updated to 1') AS 'td',''
								,ISNULL(CAST([transaction_cost_local] AS varchar), '-') AS 'td',''
								,ISNULL(CAST([transaction_valuation_retail] AS varchar), 'MISSING') AS 'td',''
								,ISNULL(CAST([transaction_selling_retail] AS varchar), 'MISSING') AS 'td',''
							...
							<div>The records listed below were not converted during the HearMe Sales Conversion Process:</div> 
								<table border = 1 style="border-collapse: collapse;"> 
									<tr style="background-color: #0056a2; color: #ffffff;">
										<th> IB Inventory ID* </th> <th> SKU ID* </th> <th> Location ID* </th> <th> Price Status ID* </th> <th> Transaction Date* </th> <th> Transaction Type Code* </th> <th> Inventory Status ID* </th> <th> Other Location ID </th> <th> Transaction Reason ID </th> <th> Previous Style Code </th> <th> Transaction Units* </th> <th> Transaction Cost* </th> <th> Transaction Cost Local* </th> <th> Transaction Valuation Retail* </th> <th> Transaction Selling Retail* </th> <th> Price Change Type </th> <th> Units Affected </th> <th> Transaction No. </th> <th> Batch No. </th> <th> Register No </th> 
									</tr>' 
				SET @btext = @body + @xml + '</table><p style="font-size:12px;">* denotes required fields.<br><br>This alert was generated from BEDROCKDB02.me_01.dbo.spHearMeSalesConversion.</p></body></html>'
				SET @sub = 'HearMe Sales Conversion Process - Skipped '  + @nullCheckB + ' Records'
		...
		IF (@nullCheckB > 99)
			BEGIN
				...
				SET @sub = 'HearMe Sales Conversion - ' + @nullCheckB + ' Failures'
