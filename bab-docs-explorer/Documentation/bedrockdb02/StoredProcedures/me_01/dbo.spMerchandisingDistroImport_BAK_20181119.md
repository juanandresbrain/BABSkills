# dbo.spMerchandisingDistroImport_BAK_20181119
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingDistroImport_BAK_20181119"]
...
```sql
CREATE proc [dbo].[spMerchandisingDistroImport_BAK_20181119]
...
--NEW CODE TO REMOVE JUNK DATA - ADDED 07.17.2013
		...
		if (select count(*) from #JUNKDetail 
				where sourceid in (960,980,975,2970,9913,9914,9915,9916,9917,9918,9919,9920,9921,9922,3970,3980,8502,8505)
			...
			begin
				...
				<font face =arial size = 2> '  +
					...
					CAST ( ( SELECT td = SOURCEID,'',
							  ...
							  where sourceid in (960,980,975,2970, 9913,9914,9915,9916,9917,9918,9919,9920,9921,9922,3970,3980,8502,8505) --we don't want to report on these since we're going to create transfers with these instead
							  or right('0000' + cast(sourceid as varchar), 4) not in (select location_code from location (nolock) where active_flag = 1) --added 07/16/2014
							  FOR XML PATH('tr'), TYPE 
					) AS NVARCHAR(MAX) ) +
					'</font></table></font></p></p><br>'
