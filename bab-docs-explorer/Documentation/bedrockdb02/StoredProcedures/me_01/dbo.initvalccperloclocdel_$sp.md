...
```mermaid
flowchart LR
    SP["dbo.initvalccperloclocdel_$sp"]
...
```sql
CREATE proc [dbo].[initvalccperloclocdel_$sp] 
...
and lp.location_id = cvh.location_hierarchy_group_id
...
(b.parent_hierarchy_group_id = @MerchGroupId or b.hierarchy_group_id = @MerchGroupId)
