# dbo.spMerchandisingDistroImportBACKUP20150909
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingDistroImportBACKUP20150909"]
...
```sql
create proc [dbo].[spMerchandisingDistroImportBACKUP20150909]
...
---------Bulk Insert Loop
		while @files > 0
			begin
				...
				select @move = 'move ' + @filepath + @filename + '.' + @timestamp + '.csv' + ' \\kermode\FileRepository\MERCHANDISING\DistroExport\History\'
...
--NEW CODE TO REMOVE DUPLICATES - ADDED 07.17.2013
		...
		if (select count(*) from #dupesDetail) > 0
			...
			<font face =arial size = 2> '  +
				...
				@recipients = 'distrobears@buildabear.com;merchadmin@buildabear.com',
				@body = @text2,
...
--NEW CODE TO REMOVE JUNK DATA - ADDED 07.17.2013
		...
		or quantity < 1
		or (sourceid = destid)
		or sourceid not in (960,980,975,2970,9913,9914,9915,9916,9917,9918,9919,9920,9921,9922)
		...
		if (select count(*) from #JUNKDetail
			where sourceid not in (960,980,975,2970, 9913,9914,9915,9916,9917,9918,9919,9920,9921,9922)
		...
		if (select count(*) from #JUNKDetail 
				where sourceid in (960,980,975,2970,9913,9914,9915,9916,9917,9918,9919,9920,9921,9922)
			...
			begin
				...
				<font face =arial size = 2> '  +
					...
					CAST ( ( SELECT td = SOURCEID,'',
							  ...
							  where sourceid in (960,980,975,2970, 9913,9914,9915,9916,9917,9918,9919,9920,9921,9922) --we don't want to report on these since we're going to create transfers with these instead
							  or right('0000' + cast(sourceid as varchar), 4) not in (select location_code from location (nolock) where active_flag = 1) --added 07/16/2014
							  FOR XML PATH('tr'), TYPE 
					) AS NVARCHAR(MAX) ) +
					'</font></table></font></p></p><br>'
...
BEGIN
			...
			set @file_location52 = '\\pipeapp01\E$\Company01\Text File to IM Import Tables - Import Outbound Xfers\'
