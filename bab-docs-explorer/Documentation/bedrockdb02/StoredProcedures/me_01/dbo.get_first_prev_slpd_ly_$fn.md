# dbo.get_first_prev_slpd_ly_$fn
...
```mermaid
flowchart LR
    SP["dbo.get_first_prev_slpd_ly_$fn"]
...
```sql
create proc dbo.get_first_prev_slpd_ly_$fn  @dummy int, @dummy2 int
...
WHERE cal_year_period_code = (SELECT MAX (cal_year_period_code)
	  	  	FROM view_cal_year_period
			WHERE cal_year_period_code < current_year_pd -100);
