...
```sql
create proc [dbo].[insert_skus_$sp] 
(
...
-- that have inventory in ib_inventory
	...
	IF @ReplaceOrInc = 0 -- do not include query into count_import_temp
	...
	ELSE -- include query into count_import_temp
		...
		BEGIN
			...
			FROM
				(
					...
					SELECT
						sku.sku_id,
						count_import_temp.units_counted units_counted,
						count_import_temp.units_counted extended_units_counted,
						0 total_oh_book_units,
						0 total_oh_book_cost,
						0 total_oh_book_cost_local,
						0 total_oh_book_val_retail,
...
/*--------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------*/
-- Calculate the average costs in home currency
	...
	UPDATE
		inventory_count_detail
	SET
		inventory_count_detail.average_cost = (A.total_cost/total_units)
	FROM
		inventory_count_detail,
		sku,
		(
	...
	WHERE
		...
		AND inventory_count_detail.inventory_control_id = @DocId
		AND inventory_count_detail.total_retail IS NULL
		AND A.total_units > 0 AND A.total_cost >= 0

/*Multi Currency Mod ----------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------*/
-- Calculate the average costs in local currency
	...
	UPDATE
		inventory_count_detail
	SET
		inventory_count_detail.average_cost_local = (A.total_cost_local/total_units)
	FROM
		inventory_count_detail,
		sku,
		(
	...
	WHERE
		...
		AND inventory_count_detail.inventory_control_id = @DocId
		AND inventory_count_detail.total_retail IS NULL
		AND A.total_units > 0 AND A.total_cost_local >= 0
	...
	/* Multi Currency mod --------------------------------------------------------------------------------------------------------------*/
	-- For those styles that don't have a valid average cost in local currency, use the last_net_final_po_cost from the style_detail table
...
/* Multi Currency Mod --------------------------------------------------------------------------------------------------------------
	-- For those styles that don't have a cost in local currency, use the current_cost of the primary vendor from the style_vendor table
