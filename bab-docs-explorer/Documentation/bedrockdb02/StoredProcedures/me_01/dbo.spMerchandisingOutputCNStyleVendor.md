# dbo.spMerchandisingOutputCNStyleVendor
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingOutputCNStyleVendor"]
...
```sql
-- =====================================================================================================
-- Name: spMerchandisingOutputCNStyleVendor
--
...
--		Tim Callahan	07/10/2018		Updated CTC vendor for DANMTYF which is the China to China vendor for standard vendor DANDEEE and DANDEES, Previously it was DANBSTF
--										See HEAT SR# 22898 - There is an attached e-mail thread on the topic\change. 
--		Lizzy Timm		02/08/2019		Updated the conversion rate from 6.8285 to 6.8701 
--		Lizzy Timm		04/02/2019		Updated CTC vendor for DANMTYF and DANBSTF which are the China to China vendors for standard vendors DANDEEE and DANDEES, previously it was only DANMTYF
--										See JIRA Story ESS-233 - There is an attached e-mail thread on the topic\change.
--		Lizzy Timm		04/11/2019		Removed DANDEEE & DANDEES from Step #4 and moved to Step #7
-- =====================================================================================================
CREATE PROCEDURE [dbo].[spMerchandisingOutputCNStyleVendor]
...
IF (Object_ID('tempdb..#nonbondedDD') IS NOT NULL) --Added for new STEP # 7 on 04/02/2019
		DROP TABLE #nonbondedDD
...
AND	sv.vendor_id = v.vendor_id
AND s.style_code between '800000' and '899999'
AND v.vendor_code IN ('CENTHFF', 'DANUANF', 'DRMCHTF', 'DRMLISF', 'ELNTPWF', 'JINSDZF', 'VXPRTWF', 'JSHWLIF', 'RPACSUP','JSKXINF')  -- Replaced DANBSTF with DANMTYF on 7/10/2018; Removed DANMTYF on 04/02/2019 (moved to #7)
ORDER BY 1
...
SELECT 	'VS' AS Record_Type,
	...
	CASE WHEN v.vendor_code = 'CENTURY'
		THEN 'CENTHFF'
	--WHEN v.vendor_code = 'DANDEEE'
	--	THEN 'DANMTYF' -- Replaced DANBSTF on 7/10/2018; Moved to #7 on 04/02/2019
	--WHEN v.vendor_code = 'DANDEES'
	--	THEN 'DANMTYF' -- Replaced DANBSTF on 7/10/2018; Moved to #7 on 04/02/2019
	WHEN v.vendor_code = 'DANUINT'
		THEN 'DANUANF'
...
AND	sv.currency_id = cu.currency_id
AND s.style_code between '800000' and '899999'
AND	v.vendor_code IN ('CENTURY', 'DANUINT', 'DREAMAN', 'DREAMIN', 'ELANPOL', 'JINSOLL', 'VOICEEX', 'JSHOLLY', 'RPACHKL','JSKYANT')--Removed DANDEEE & DANDEES, moved to Step #7 on 04/11/2019
AND NOT EXISTS (
...
--  STEP #7 to add the appropriate non-bonded vendor codes to all CN styles where they don't exist (DANDEEE and DANDEES ONLY).
...
AND	sv.vendor_id = v.vendor_id
AND s.style_code between '800000' and '899999'
AND v.vendor_code IN ('DANMTYF', 'DANBSTF')
ORDER BY 1
...
AND	sv.currency_id = cu.currency_id
AND s.style_code between '800000' and '899999'
AND	v.vendor_code IN ('DANDEEE', 'DANDEES')
AND NOT EXISTS (
SELECT * FROM #nonbondedDD st
WHERE s.style_code = st.style_code
AND st.vendor_code = 'DANMTYF')
UNION
SELECT 	'VS' AS Record_Type,
	'A' AS Action_Type,
	s.style_code AS Style_Code,
