# dbo.spMerchandisingEmailMAvsMerchAzureAlert
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingEmailMAvsMerchAzureAlert"]
...
```sql
CREATE proc [dbo].[spMerchandisingEmailMAvsMerchAzureAlert] 
...
set @Subj = 'Azure MA data to SilverDeltaLake alert'
...
select @body = 
'MA data extracts to SilverDeltaLake has not run because spMerchVsMACompare is indicating differences. Please investigate. <br> ' +
    '<br><br>' +
    '<br>
