# dbo.spMerchandisingFTPgetUKfiles_Web_WinSCP
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingFTPgetUKfiles_Web_WinSCP"]
...
```sql
CREATE proc [dbo].[spMerchandisingFTPgetUKfiles_Web_WinSCP] -- Stored Proc name
as
-- =====================================================================================================
...
insert #DEL exec master..xp_cmdshell 'dir \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Inbound\sftp_UK_WEB_GET.log /B'
insert #DEL exec master..xp_cmdshell 'dir \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Inbound\sftpGETwebLog.txt /B'
delete from #DEL where output is null or output = 'File Not Found'

IF (select count(*) from #DEL where output = 'sftp_UK_WEB_GET.log') > 0
	begin
		exec master..xp_cmdshell 'del \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Inbound\sftp_UK_WEB_GET.log'
	end
IF (select count(*) from #DEL where output = 'sftpGETwebLog.txt') > 0
	begin
		exec master..xp_cmdshell 'del \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Inbound\sftpGETwebLog.txt'
	end
...
--declare and set ftp variables 
					...
					select
							...
							@script = ' /script=\\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Scripts\Get\sftpGETweb.txt',
							@log = ' /log=\\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Inbound\sftp_UK_WEB_GET.log',
							@SFTP = concat(@winSCP, @ini, @script, @log)

--create temp tables for ftp logs

IF (Object_ID('me_01..sftpGETLogUKWhseWeb') IS NOT NULL) DROP TABLE sftpGETLogUKWhseWeb
...
insert sftpGETLogUKWhseWeb exec master..xp_cmdshell @SFTP
	...
	--select * from sftpGETLogUKWhseWeb -- Troubleshooting Purposes 
		...
		if (select count(*) from sftpGETLogUKWhseWeb where ftplog like '%.txt%' or ftplog like '%.dat%' ) < 1
		  AND DATEPART(hh, getdate()) = 4 AND DATEPART(mm, getdate()) < 30
			...
			begin
				...
				set @Log_query = 'select * from BEDROCKDB02.me_01.dbo.sftpGETLogUKWhseWeb'
				...
				@body = @body,
				@file_attachments = '\\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Logs\Inbound\sftpGETwebLog.txt',
				@importance = 'HIGH'
...
------
	...
	select @moveInventory = 'move \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Hold\Web\INVENTORY_WEB_*.txt \\kermode\FileRepository\MERCHANDISING\UK_Distro\INVENTORY\Web'
	select @moveCBR = 'move \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Hold\Web\STSIMCTN.UKWEB.*.txt \\kermode\FileRepository\MERCHANDISING\UK_Distro\WebCBR'
	select @movereceipt = 'move \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Hold\Web\recv*.dat \\kermode\FileRepository\MERCHANDISING\UK_Distro\RECEIPTS'
	select @movestockadj = 'move \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Hold\Web\STOCKADJUSTMENT_WEB_*.txt \\kermode\FileRepository\MERCHANDISING\UK_Distro\STOCKADJ\Web'
	select @moveProdBal = 'move \\kermode\FileRepository\MERCHANDISING\UK_Distro\FTP\WinSCP\Hold\Web\PRODBAL_WEB_*.txt \\kermode\FileRepository\MERCHANDISING\UK_Distro\PRODBAL\WEB'
...
-------------------------------------------------------------------------------------------------------------------------------------
	...
	select @dirInventory = 'dir \\kermode\FileRepository\MERCHANDISING\UK_Distro\INVENTORY\Web /B'
	select @dirCBR = 'dir \\kermode\FileRepository\MERCHANDISING\UK_Distro\WebCBR /B'
	select @dirReceipt = 'dir \\kermode\FileRepository\MERCHANDISING\UK_Distro\RECEIPTS\ /B'
	select @dirStockAdj = 'dir \\kermode\FileRepository\MERCHANDISING\UK_Distro\STOCKADJ\Web /B'
	select @dirProdBal = 'dir \\kermode\FileRepository\MERCHANDISING\UK_Distro\PRODBAL\WEB /B'
...
/*	declare @inv int,			
	...
	set @text = '<font face =arial size = 2>' + 
			...
			'(files retrieved from the Clipper UK Webstore SFTP server and successfully located to our interface directories on \\kermode\FileRepository\MERCHANDISING\UK_Distro)' + 
			'<br><br>' +
			'<table border="1">' +
