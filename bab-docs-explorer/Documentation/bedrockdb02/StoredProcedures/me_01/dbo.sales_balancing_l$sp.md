...
```sql
CREATE PROC [dbo].[sales_balancing_l$sp] 
...
--		INSERT INTO #sale_temp(source, date, units, selling_retail, valuation_retail)
		SELECT	
		  a.transaction_date, COALESCE(originating_store_no, a.store_no) store_no, COALESCE(fulfillment_store_no, source_store_no) other_location_id
	...
	--		AND a.store_no = @LocationCode
			...
			GROUP BY a.transaction_date,COALESCE(originating_store_no, a.store_no), COALESCE(fulfillment_store_no, source_store_no) 
UNION
--		SELECT 'Totals in the Sales Audit current transaction tables for non rejected transactions:'
--		INSERT INTO #sale_temp (source, date, units, selling_retail, valuation_retail)
		SELECT	a.transaction_date,COALESCE(originating_store_no, a.store_no) store_no
		, COALESCE(fulfillment_store_no, source_store_no) other_location_id
		, SUM(c.units * b.db_cr_none*-1 * b.voiding_reversal_flag) AS units
		, SUM(c.units * c.sold_at_price * b.db_cr_none*-1 * b.voiding_reversal_flag) AS sold_at_price
		, SUM(c.units * c.sold_at_price * b.db_cr_none*-1 * b.voiding_reversal_flag * @exchange_rate) AS sold_v_at_price
...
INSERT INTO #sale_temp(source, date,location,other_location, units, selling_retail, valuation_retail)
SELECT 'SA'
	, transaction_date,store_no ,other_location_id
	, SUM(units) AS units
	, SUM(sold_at_price) AS sold_at_price
	, SUM(sold_v_at_price) AS sold_v_at_price
FROM #sa_sales
GROUP BY transaction_date,store_no,other_location_id
...
--		SELECT 'Totals in Merchandising ib_inventory :'
		...
		INSERT INTO #sale_temp (source, date, location ,other_location, units, selling_retail,valuation_retail)
		SELECT	'MERCH',transaction_date,l.location_code,l2.location_code,-SUM(transaction_units) AS transaction_units
		...
		FROM	me_01.dbo.ib_inventory a
left join  location l
on 	 a.location_id = l.location_id
left join location l2
on 	a.other_location_id = l2.location_id
		...
		GROUP BY transaction_date,l.location_code,l2.location_code
...
--SELECT 'Totals in MA :' 
		...
		INSERT INTO #sale_temp (source, date,location,units, selling_retail, valuation_retail)
		SELECT	'MA', sales_date,location_code,
		...
		FROM	ma_01.dbo.hist_flsh_group_loc_da a, location l
...
--									WHERE location_code = @LocationCode)
		GROUP BY sales_date,location_code

		END

--SELECT * FROM  #sale_temp where source = 'MERCH'
--SELECT * FROM  #sale_temp where source = 'SA'


SELECT COALESCE(a.date,b.date,c.date) date
, COALESCE(a.location,b.location,c.location) location
, sum(isnull(a.units,0)) 'SA Units'
, sum(isnull(b.units,0)) 'Merch Units'--, c.units 'MA Units'
, sum(isnull(c.units,0)) 'MA Units'
--, sum(isnull(a.selling_retail,0)) 'SA Retail'
--, sum(isnull(b.selling_retail,0)) 'Merch Retail'--, c.selling_retail 'MA Retail'
FROM (SELECT * FROM  #sale_temp where source = 'SA') a
FULL JOIN (SELECT * FROM  #sale_temp where source = 'MERCH') b
	on a.date = b.date
	and a.location = b.location
	and isnull(a.other_location,-99) = isnull(b.other_location ,-99)
FULL JOIN (SELECT * FROM  #sale_temp where source = 'MA') c
	on a.date = c.date
	and a.location = c.location
	and isnull(a.other_location,-99) = isnull(c.other_location ,-99)
WHERE COALESCE(a.location,b.location,c.location) = 952
group by COALESCE(a.date,b.date) 
, COALESCE(a.location,b.location)
order by COALESCE(a.date,b.date), COALESCE(a.location,b.location)
