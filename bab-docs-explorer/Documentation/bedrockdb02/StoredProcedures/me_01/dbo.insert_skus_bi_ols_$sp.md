...
```sql
create proc [dbo].[insert_skus_bi_ols_$sp] 
(
...
/*
Proc name: insert_skus_bi_ols_$sp 
...
-- If counted from the GUI, skus have already been inserted into the inventory_count_detail table
	...
	SELECT
		sku.sku_id,
		count_import_sku_temp.cost cost,
		count_import_sku_temp.cost cost_local,
...
-- update average_cost_local
	UPDATE
		inventory_count_detail
	SET
		inventory_count_detail.average_cost = style_vendor.current_cost * exchange_rate / @ExchangeRate
	FROM
		inventory_count_detail WITH (NOLOCK),
		style_vendor WITH (NOLOCK),
		sku WITH (NOLOCK),
