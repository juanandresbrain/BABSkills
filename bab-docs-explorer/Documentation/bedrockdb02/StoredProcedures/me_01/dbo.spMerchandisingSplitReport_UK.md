# dbo.spMerchandisingSplitReport_UK
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingSplitReport_UK"]
...
```sql
CREATE proc [dbo].[spMerchandisingSplitReport_UK]

as 


-- =====================================================================================================
-- Name: spMerchandisingSplitReport_UK
--
...
select s.style_code, 
case when att.attribute_set_code = 'YES' then 'Y' else 'N' end as active_pick
into #actv
from style s (nolock)
...
where a.attribute_label = 'ACTIVE PICK STYLES'
