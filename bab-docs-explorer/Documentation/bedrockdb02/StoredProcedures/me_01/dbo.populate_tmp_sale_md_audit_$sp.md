...
```mermaid
flowchart LR
    SP["dbo.populate_tmp_sale_md_audit_$sp"]
...
```sql
CREATE PROCEDURE [dbo].[populate_tmp_sale_md_audit_$sp]
	(@job_id INT
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN TRAN
		INSERT INTO temp_sales_markdown_audit 
			( job_id
			, transaction_no
			, register
			, location_id
		...
		FROM temp_ib_inventory i, temp_sale_master m, im_sale s, transaction_type t
		...
		BEGIN TRY
			...
			FROM temp_sales_markdown_audit t, 
		...
		BEGIN TRY
			...
			FROM temp_sales_markdown_audit a,
					( SELECT job_id, transaction_no, transaction_line, location_id, sku_id,
							SUM(transaction_valuation_retail) valuation_retail
					 FROM temp_ib_inventory 
					 WHERE job_id = @job_id 
