# dbo.spMerchandisingSelectStoreDistribution_UK_CN
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingSelectStoreDistribution_UK_CN"]
...
```sql
CREATE proc [dbo].[spMerchandisingSelectStoreDistribution_UK_CN]
as
...
--		Dan Tweedie		2022-04-18		Added handling for reticketing skus
--		Tim Callahan	2022-08-15		Updated Proc to point to new source table with integration of 3PW to Dynamics 
-- =====================================================================================================
...
select	cast (sse.document_number as decimal (10,0)) as [Store Shipment Number],
		sse.destid [Store],
		sse.message [Rec Type Label],
		sse.style_code [Style Code],
		replace(sse.short_desc, ',', '') [Style Short Description],
		sse.quantity [Quantity],
		sse.exportdate [Exported to Warehouse Date],
		0 [Exported]
into	TMP_Store_Distribution_Data
from [stl-ssis-p-01].[IntegrationStaging].wms.DynamicsTo3PLOrderExport sse
where datediff(dd, sse.exportdate, getdate()) = 0
and	sse.destid in (select store from #stores)
and	sse.sourceid in ('2970', '3970')
...
BEGIN
		...
		while @counter <= @total_stores 
			...
			---output report to Excel
				...
				select @query_text = 'set nocount on select sd.[Store Shipment Number], sd.[Store], sd.[Rec Type Label], case when rt.StyleCode is not null then ''****''+ sd.[Style Code] +''****'' else sd.[Style Code] end as [Style Code], [Style Short Description],[Quantity],[Exported to Warehouse Date] from me_01.dbo.TMP_Store_Distribution_Data sd join location l with (nolock) on sd.store=l.location_code join address a with (nolock) on l.location_id=a.parent_id and a.parent_type=2 join country c with (nolock) on a.country_id=c.country_id left join SkuReticketingListByCountry rt on sd.[Style Code]=rt.StyleCode and c.country_code=rt.country where store = ' + @store_nbr + ' order by [Store Shipment Number],case when rt.StyleCode is not null then ''****''+ sd.[Style Code] +''****'' else sd.[Style Code] end,[Rec Type Label]'
				...
				set @file_location = '\\kermode\FileRepository\MERCHANDISING\DBCompare\TESTDMT\'  
