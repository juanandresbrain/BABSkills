# dbo.spPOSSelectProductCatalogMasterAttributes_BACKUP20240313
...
```mermaid
flowchart LR
    SP["dbo.spPOSSelectProductCatalogMasterAttributes_BACKUP20240313"]
    ...
    dbo_vwPOSIncludedStyles(["dbo.vwPOSIncludedStyles"]) --> SP
    dbo_vwPOSProductHierarchy(["dbo.vwPOSProductHierarchy"]) --> SP
    dbo_vwPOSTaxItemGroupAssignment(["dbo.vwPOSTaxItemGroupAssignment"]) --> SP
```
...
```sql
CREATE proc [dbo].[spPOSSelectProductCatalogMasterAttributes_BACKUP20240313]

as


-- =====================================================================================================
-- Name: spPOSSelectProductCatalogMasterAttributes 
--
...
-- Revision History
--		Name:			Date:			Comments:
--		Dan Tweedie		2022-11-28		Created proc modeled after spWEBSelectProductCatalogMasterAttributes
--		Tim Callahan	2023-02-21		Updated proc isCouponEligible now dependent on Product Hierarchy Department Code as provided by Bryce Ahrens
--		Tim Callahan	2023-02-23		Updated proc isEmployeeDiscountEligible to match that of isCouponEligible
--										Added isLoyaltyRewardsDiscountEligible field per JIRA BIB-514
--		Tim Callahan	2023-02-28		Updated Proc case statement for ItemType derived value for donations per JIRA BIB-514
--		Tim Callahan	2023-03-14		Updated Proc case statement for isTaxExempt derived value per JIRA BIB-514
--		Tim Callahan	2023-03-16		Updated Proc case statement for isTaxExempt to handle transaction flag department code as tax exempt as well 
--										Updated proc case statement for ItemType to be donation if specific style codes per DanT\Mark Diehm 	
--		Tim Callahan	2024-02-19		Updated proc case statement for ItemType to be Service if 098075 (REPAIR FEE) 
--										Added due to Service Desk Tickets indicating the item was not available this is because it was STOCK per hierarchy
--										If Stock dependent on Idate and Inventory OnHand\Allocated in the Chain 
-- =====================================================================================================
...
truncate table POSProductCatalogMasterAttributes
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
...
into #Styles 
from vwPOSIncludedStyles s
join vwPOSProductHierarchy h on s.style_code=h.StyleCode
...
select 
	...
	--s.isTaxExempt, --= check the tax data set to verify the relationship to style.... join on tax item group to department... styles that are not included are assumed tax exempt.. will this be gift cards?
	-- Remarked out isTaxExempt line above and replaced with new logic below  on 3/14/2023 per JIRA BIB-514 Requirements 
	case 
		--when s.DepartmentCode in ('R-B-U-47','R-B-D-47','R-B-C-47') -- These Are Transaction Flag Departments -- Added 3/16/2023
		--	then 1 
		when isnull(tg.tax_item_group_id,0) in (10,30) -- These appear to be the only TaxExempt GroupIDs at this time. 
			then 1 
		--when tg.tax_item_group_id not in (10,30)
		--	then  0  
			else 0 -- Basically if we cannot find a Tax Item Group we will default to 0 aka that is taxable 
		end as isTaxExempt,
	...
	s.isCashierEntersPrice,
	s.isQtyRestricted
into #POSProductCatalogMasterAttributes
from #Styles s
...
insert POSProductCatalogMasterAttributes
select 
	Style_Code,	
	SKUDescription,	
	UPC,	
	...
	case 
		...
		when giftcardType is not null 
			or style_code in 
				...
				then 'Gift Card'
		when style_code = '098075' 
			then 'Service' -- Repair Fee -- Added 2/19/2024
		when 
		...
		when SKUDescription 
				like '%embroid%' then 'Service' --anything else ?? is this not a 'service'?
		else 'Stock' 
	end as ItemType
...
--where 	cast(getdate() as date) >= cast(dateadd(dd, -1, MerchInDate) as date) --product cannot go out until at no more than 1 day before the Merch In Date


----INSERT IRELAND DERIVED FROM UK, USE IE AS PRODUCTSELLINGGEOGRAPHY -- per DanT
...
--keep this commented out--and style_code in (select style_code from POSListPrices where jurisdiction='IE')
```
