# dbo.spMerchandisingFTPgetDbsASNs
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingFTPgetDbsASNs"]
...
```sql
CREATE proc [dbo].[spMerchandisingFTPgetDbsASNs]
as

-- =====================================================================================================
-- Name: spMerchandisingFTPgetDbsASNs
--
...
-- Revision History
--		Name:			Date:			Comments:
--		Dan Tweedie		09/25/2012		Created proc.	
...
if (select count(*) from ftpDIRLogASN where ftplog like '%Login successful%') < 1
	begin
		...
		set @body =	'An attempt to connect to the DB Schenker FTP server failed.' 
					...
					+ char(10) + char(13) + 
					+ char(10) + char(13) + 
					'This process is managed by bedrockdb02.me_01.dbo.spMerchandisingFTPgetDbsASNs'
...
if (select count(*) from ftpDIRLogASN where ftplog like '%%.csv%') > 0 
	...
	BEGIN
		...
		if (select count(*) from ftpGETLogASN where ftplog like '%File send OK%') < @files
			begin
				...
				@importance = 'HIGH'
						...
						if (select count(*) from ftpPUTLogASN where ftplog like '%File receive OK%') < @files
							begin 
		...
		if (select count(*) from ftpDELLogASN where ftplog like '%Delete operation successful.%') < @files
			begin
