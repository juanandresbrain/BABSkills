...
CREATE PROCEDURE dbo.on_order_reinstate_$sp
...
DECLARE @Max_IB_On_Order_Id AS DECIMAL(13, 0)
DECLARE @Receipt_Date SMALLDATETIME

SET @Max_IB_On_Order_Id = COALESCE((SELECT MAX (ib_on_order_id) FROM ib_on_order IOO WHERE IOO.document_number = @PO_Number AND IOO.transaction_type_code = 130), 0)
...
WHERE
	...
	AND IOO.transaction_type_code = 120
	AND ( @Max_IB_On_Order_Id = 0 OR IOO.ib_on_order_id > @Max_IB_On_Order_Id )
GROUP BY
	IOO.sku_id
	,S.style_id
	,SC.style_color_id
