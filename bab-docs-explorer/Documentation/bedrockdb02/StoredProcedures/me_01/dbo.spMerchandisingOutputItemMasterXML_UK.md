# dbo.spMerchandisingOutputItemMasterXML_UK
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingOutputItemMasterXML_UK"]
    dbo_VW_WMItemMaster_UK(["dbo.VW_WMItemMaster_UK"]) --> SP
```
...
```sql
CREATE proc [dbo].[spMerchandisingOutputItemMasterXML_UK]
...
-- Revision History
--		Name:			Date:			Comments:
--		Keith Lee		07/02/2017		Created proc
--		Tim Callahan	09/12/2017		Updated Directory to Drop File as we are now in Production
-- =====================================================================================================
...
select a.*, 
	   case when a.rankID between 0 and 6000 then 1
			when a.rankID between 6001 and 8000 then 2
...
BEGIN
	...
	set @file_location = '\\kermode\FileRepository\MERCHANDISING\'
--	set @file_destination = '\\kermode\FileRepository\MERCHANDISING\UK_Distro\'  -- Use for testing
	set @file_destination = '\\kermode\FileRepository\MERCHANDISING\UK_Distro\OUTBOUND\ItemMaster'  -- Use for go live.
