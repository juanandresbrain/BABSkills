...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingSelectNonWhseInventoryShrink_BAK20230829"]
...
```sql
CREATE proc [dbo].[spMerchandisingSelectNonWhseInventoryShrink_BAK20230829]
...
select d.LocationCode, d.StyleCode, d.SKUDescription, d.Qty, cast(d.LoadDate as datetime) as LoadDate
...
and ts.location_code in --select concat('''',LocationCode,''',') from [ERP].[vwWarehouseIDToLocationCodeRetailInventory]
