...
```mermaid
flowchart LR
    SP["dbo.get_avg_cost_style_location_$sp"]
...
CREATE PROCEDURE dbo.get_avg_cost_style_location_$sp
...
/*
	Get dynamic average cost by style/location
	Called by get_avg_cost_$sp and get_avg_cost_sku_location_$sp
...
CREATE TABLE dbo.#temp_style_location
	(
		location_id SMALLINT NULL
		,style_id DECIMAL (12, 0) NULL
	)

INSERT INTO dbo.#temp_style_location
	(
		location_id
		,style_id
	)
	...
	INNER JOIN
		...
		(
			...
			FROM
				...
				INNER JOIN dbo.#temp_style_location TSL ON K.style_id = TSL.style_id AND IBIT.location_id = TSL.location_id
				LEFT JOIN
		...
		) SQAV ON TWACL.style_id = SQAV.style_id AND TWACL.location_id = SQAV.location_id
