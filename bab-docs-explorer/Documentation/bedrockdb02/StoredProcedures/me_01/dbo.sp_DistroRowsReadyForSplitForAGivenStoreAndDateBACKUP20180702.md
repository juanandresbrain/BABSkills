# dbo.sp_DistroRowsReadyForSplitForAGivenStoreAndDateBACKUP20180702
...
```mermaid
flowchart LR
    SP["dbo.sp_DistroRowsReadyForSplitForAGivenStoreAndDateBACKUP20180702"]
...
```sql
CREATE PROCEDURE [dbo].[sp_DistroRowsReadyForSplitForAGivenStoreAndDateBACKUP20180702] 
	(@storeNum  int,
	 @warehouseNum int)
...
INTO #sumQuantity
FROM distribution_split (NOLOCK) WHERE destid = @storeNum AND sourceid = @warehouseNum AND released = 0 GROUP BY style_code
