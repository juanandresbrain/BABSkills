# dbo.spPosStagePOSProductCatalogWithHierarchyStage
...
```mermaid
flowchart LR
    SP["dbo.spPosStagePOSProductCatalogWithHierarchyStage"]
    dbo_POSProductCatalogMasterAttributes(["dbo.POSProductCatalogMasterAttributes"]) --> SP
    dbo_tmpPOSProductCatalogWithHierarchyStage(["dbo.tmpPOSProductCatalogWithHierarchyStage"]) --> SP
    dbo_vwPOSItemsExportEligible(["dbo.vwPOSItemsExportEligible"]) --> SP
...
```sql
CREATE proc [dbo].[spPosStagePOSProductCatalogWithHierarchyStage]

as


-- =====================================================================================================
-- Name: spPosStagePOSProductCatalogWithHierarchyStage
...
-- Revision History
--		Name:			Date:			Comments:
--		TimCallahan 	2025-06-26		Created proc modeled after vwPOSProductCatalogWithHierarchyStage - as that view took 1+ hours to query 

set nocount on


truncate table tmpPOSProductCatalogWithHierarchyStage
----------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------


--PRE STAGE 
IF (Object_ID('tempdb..#vwPOSItemsExportEligible') IS NOT null) DROP TABLE #vwPOSItemsExportEligible
select *
into #vwPOSItemsExportEligible
from vwPOSItemsExportEligible

IF (Object_ID('tempdb..#vwPOSProductHierarchy') IS NOT null) DROP TABLE #vwPOSProductHierarchy
select *
into #vwPOSProductHierarchy
from vwPOSProductHierarchy


insert into tmpPOSProductCatalogWithHierarchyStage
select 
	...
	p.ODATE,
	p.Outlet 
--into tmpPOSProductCatalogWithHierarchyStage
...
where 1=1
and left(ph.StyleCode,1) in ('0','1','2','3','4','5','6')
```
