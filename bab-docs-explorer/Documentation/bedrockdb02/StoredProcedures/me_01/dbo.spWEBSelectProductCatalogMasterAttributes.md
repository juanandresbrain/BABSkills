# dbo.spWEBSelectProductCatalogMasterAttributes
...
```mermaid
flowchart LR
    SP["dbo.spWEBSelectProductCatalogMasterAttributes"]
...
```sql
CREATE proc [dbo].[spWEBSelectProductCatalogMasterAttributes]
...
--		Dan Tweedie		2022-11-02		Additional attributes, Customer Property, columns for PIM
-- =====================================================================================================
...
group by 
	...
	--delete from #Styles  where style_code = '432070' and SellingGeography = 'US' 
...
select 
	s.style_code,
	cp.cust_prop_code,
	max(ecp.custom_property_value) CustomProperty
...
select
	s.style_code,
	max(a.attribute_set_code) as WarningLabel
into #WarningLabel
from #Styles s
join #Attributes a on s.style_code = a.style_code
...
if (select count(*) from WebProductCatalogMasterAttributes group by style_code having count(*) >1) >0
