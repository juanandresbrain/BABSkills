# dbo.spMerchandisingProcessAutoReceiveInterCompanyTransfers
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingProcessAutoReceiveInterCompanyTransfers"]
...
```sql
CREATE proc [dbo].[spMerchandisingProcessAutoReceiveInterCompanyTransfers]

as

-- =====================================================================================================
-- Name: spMerchandisingProcessAutoReceiveInterCompanyTransfers
--
-- Description:	Requested by Lisa Velikis via IT Work Request
--				Automatically receive in transfers to locations 9960, 9965 ,9970, 9975, 9980, 9985 in Merchandising 
--				Captures carton data for outbound transfers which have not been received, 
...
-- Revision History
--		Name:			Date:			Comments:
--		Keith Lee		8/12/2020		Created proc.	
...
if (select count(*) from AutoReceiveTransferCartons) > 0
	...
	begin
		...
		set @file_name = 'STSIMCTN.TRANSFERS.INTERCOMPANY.' + @date + '.GO'
