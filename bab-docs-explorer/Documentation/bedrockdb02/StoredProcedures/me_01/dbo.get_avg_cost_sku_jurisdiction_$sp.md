...
```mermaid
flowchart LR
    SP["dbo.get_avg_cost_sku_jurisdiction_$sp"]
...
CREATE PROCEDURE dbo.get_avg_cost_sku_jurisdiction_$sp
...
/*
	Get dynamic average cost by sku/jurisdiction
...
CREATE TABLE dbo.#temp_sku_jurisdiction
	(
		jurisdiction_id SMALLINT NULL
		,sku_id DECIMAL (13, 0) NULL
	)

INSERT INTO dbo.#temp_sku_jurisdiction
	(
		jurisdiction_id
		,sku_id
	)
	...
	INNER JOIN
		...
		(
			...
			FROM
				...
				INNER JOIN dbo.#temp_sku_jurisdiction TSL ON IBIT.sku_id = TSL.sku_id AND L.jurisdiction_id = TSL.jurisdiction_id
				LEFT JOIN
			...
			GROUP BY
				TSL.sku_id, TSL.jurisdiction_id
			HAVING
		...
		) SQAV ON TWACL.sku_id = SQAV.sku_id AND TWACL.jurisdiction_id = SQAV.jurisdiction_id

EXEC dbo.get_avg_cost_style_jurisdiction_$sp
