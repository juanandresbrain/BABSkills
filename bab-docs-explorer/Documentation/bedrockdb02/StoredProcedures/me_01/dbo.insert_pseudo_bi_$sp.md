...
```sql
create proc [dbo].[insert_pseudo_bi_$sp] 
...
/* 
Proc name: insert_pseudo_bi_$sp 
...
June03,05   	Sameer Patel   	51181	physical inventory:location beginning inventory report-incorrect cost value
Feb. 29 2010		Feng			Multi-currency mod. 	add/set cost_local, total_cost_local, total_valuation_retail 
*/
...
-- If counted from the GUI, skus have already been inserted into the inventory_count_detail table
	...
	SELECT
		sku.sku_id,
		SUM(count_import_temp.cost) cost,
		SUM(count_import_temp.cost_local) cost_local,
		SUM(count_import_temp.total_retail) total_retail,
		SUM(count_import_temp.total_valuation_retail) total_valuation_retail,
	...
	WHERE
		...
		AND count_import_temp.total_retail IS NOT NULL
	GROUP BY
		sku.sku_id
...
/*----Multi Currency mod ------------------------------------------------------------------------*/	
			...
			WHERE
				...
				AND inventory_count_detail.pack_id IS NULL
				AND inventory_count_detail.total_retail IS NOT NULL
				AND inventory_count_detail.cost_local IS NOT NULL
...
/*--------------Multi currency mod------------------------------------------------------------------------------*/
			...
			WHERE
				...
				AND inventory_count_detail.inventory_control_loc_id = @IclId
				AND inventory_count_detail.pack_id IS NULL
				AND #PI_SKU_OH_TEMP.cost_local IS NOT NULL
				AND inventory_count_detail.total_cost_local IS NULL
		END
...
-- PSEUDO-STYLE / INTERNATIONATIONALIZATION CHANGES
	...
	DECLARE @ExchangeRate2 AS FLOAT
	
	SELECT
		@ExchangeRate2 = exchange_rate
	FROM
		currency_conversion
	WHERE
		to_currency_id = 
	...
	UPDATE
		inventory_count_detail
	SET
		total_valuation_retail = total_retail * @ExchangeRate2
	WHERE
...
-- Determine exchange rate
	...
	-- Calculate the total_cost_local based on the total_cost for pseudo-styles
	UPDATE
		inventory_count_detail
	SET
		total_cost_local = total_cost / @ExchangeRate1
	WHERE
		inventory_count_detail.total_cost IS NOT NULL
		AND inventory_count_detail.inventory_control_id = @DocId
		AND inventory_count_detail.inventory_control_loc_id = @IclId
		AND inventory_count_detail.pack_id IS NULL
		AND inventory_count_detail.total_cost_local IS NULL
