...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingEmailNightlySyncReport"]
...
```sql
CREATE proc [dbo].[spMerchandisingEmailNightlySyncReport]
as
-- =====================================================================================================
...
--		Tim Callahan	06/14/2021		Altered proc to handle all CN Warehouses 
-- =====================================================================================================
...
begin
	...
	select @loc = '0013'
	select @recip = 'brysona@buildabear.com;BearAP@buildabear.com;sherir@buildabear.com;valeriec@buildabear.com;seanw@buildabear.com'
...
begin
	...
	select @loc = '0980'
	select @recip = 'brysona@buildabear.com;BearAP@buildabear.com;sherir@buildabear.com;valeriec@buildabear.com;seanw@buildabear.com'
	select @copy = 'larryw@buildabear.com;dennish@buildabear.com;bryanw@buildabear.com;candaces@buildabear.com'
	select @subj = 'Nightly Sync Summary for ' + @loc
...
begin
	...
	select @loc = '0960'
	select @recip = 'BearAP@buildabear.com;'
...
begin
	...
	select @loc = '2970'
	select @recip = 'UKLogistics@buildabear.com;BearAP@buildabear.com;'
	select @subj = 'Nightly Sync Summary for ' + @loc
...
begin
	...
	set @text = '<font face =arial size = 2>' + 
	'</b><H1>Nightly Sync Summary for ' + '8502' + '</H1>' +
		'<table border="1">' +
		'<tr><th>STYLE</th><th>DESCRIPTION</th><th>WHSE QTY</th><th>MERCH QTY</th><th>DIFFERENCE</th><th>CONVERTED (SUPPLIES) SHRINK</th></tr>' +
		CAST ( ( SELECT td = style_code,'',
						td = short_desc, '',
...
if (select count(*) from tmpWhseInventoryShrink where location_code = '8502' and style_code = 'n/a') <> 0
begin
set @text = 'NO FILE FOUND'
end

if (select count(*) from tmpWhseInventoryShrink where location_code = '8502' and style_code = 'n/a') = 0
and (select count(*) from tmpWhseInventoryShrink where location_code = '8502') = 0
...
begin
	...
	set @text = '<font face =arial size = 2>' + 
	'</b><H1>Nightly Sync Summary for ' + '8505' + '</H1>' +
		'<table border="1">' +
		'<tr><th>STYLE</th><th>DESCRIPTION</th><th>WHSE QTY</th><th>MERCH QTY</th><th>DIFFERENCE</th><th>CONVERTED (SUPPLIES) SHRINK</th></tr>' +
		CAST ( ( SELECT td = style_code,'',
						td = short_desc, '',
...
if (select count(*) from tmpWhseInventoryShrink where location_code = '8505' and style_code = 'n/a') <> 0
begin
set @text = 'NO FILE FOUND'
end

if (select count(*) from tmpWhseInventoryShrink where location_code = '8505' and style_code = 'n/a') = 0
and (select count(*) from tmpWhseInventoryShrink where location_code = '8505') = 0
