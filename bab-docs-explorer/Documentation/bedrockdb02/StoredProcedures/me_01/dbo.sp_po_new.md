...
```sql
CREATE procedure [dbo].[sp_po_new]
as
-- =====================================================================================================
-- Name: sp_po_new
--
...
--		Tim Callahan	01/25/2018		Added China Warehouse 8502 to be included		
--		Tim Callahan	03/13/2018		Added China Warehouse 8505 to be included		
--		Dan Tweedie		2023-03-28		Added Stores		
...
select 	po.po_no as po_no,
	...
	pl.line_no,
	po.create_date as TransactionDate,
...
and not exists (select po_no from TPM_PROCESS_CONTROL where po.po_no = TPM_PROCESS_CONTROL.po_no)
and exists (select application_identifier from ib_audit_trail with (nolock) 
		  	where application = 'POM' 
			and activity = 'Create' 
			and po.po_no = ib_audit_trail.application_identifier )
--and po.po_no='1080943'
```
