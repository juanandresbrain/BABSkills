# dbo.spMerchandisingEmail2DaysPastERDWarning
...
```mermaid
flowchart LR
    SP["dbo.spMerchandisingEmail2DaysPastERDWarning"]
...
```sql
CREATE proc [dbo].[spMerchandisingEmail2DaysPastERDWarning]
...
--		Lizzy Timm		05/14/2020		Modified proc to reference dbo.temp_ExcludeStoresBOSFS for locations that will temporarily need to be excluded for the BOSFS process; changes are marked with "05/14/2020 LT"
--		Lizzy Timm		06/25/2020		Modified proc to send seperate alerts to BOSFS locations, new sections begins at line containing "BOSFS; 05/14/2020 LT (enabled 06/25/2020)"
--		Lizzy Timm		03/21/2022		Modified proc to only send BOSFS alerts because receive as sent is temporarily disabled
-- =====================================================================================================

set nocount on 

/* -- Commented out 3/21/22, uncomment to re-enable usual receive as sent alerts; LT
...
-- BOSFS; 05/14/2020 LT (enabled 06/25/2020)
...
LEFT JOIN kodiak.BABWMstrData.dbo.CNTCT_DIM c ON c.CNTCT_ID = b.CNTCT_ID
join location l (nolock) on right('0000' + cast(s.STR_NUM as varchar(4)), 4) = l.location_code
-- where right('0000' + cast(s.STR_NUM as varchar(4)), 4) in (SELECT DISTINCT ExStore FROM dbo.temp_ExcludeStoresBOSFS) -- Commented out 3/21/22, uncomment to re-enable usual receive as sent alerts; LT
where right('0000' + cast(s.STR_NUM as varchar(4)), 4) not in ('0311','0322','0630','0631','0632','0633','0634') -- Added 3/21/22, remove to re-enable usual receive as sent alerts; LT
ORDER BY 1
...
BEGIN
	...
	from	##bosfsShipments
	 ...
	 declare @stores int,
			...
			@store_email varchar(100),
			@bl_email varchar(100),
			@query1 varchar(8000)  -- Uncomment for testing -- Uncommented 3/21/22, comment out to re-enable usual receive as sent alerts; LT
