...
```mermaid
flowchart LR
    VIEW["dbo.view_hist_style_loc_wk"]
...
```sql
CREATE VIEW [dbo].[view_hist_style_loc_wk]

AS
SELECT 
style_id, merch_year_wk, l.location_id,	
...
FROM hist_style_loc_wk
INNER JOIN location l ON l.location_id = hist_style_loc_wk.location_id
```
