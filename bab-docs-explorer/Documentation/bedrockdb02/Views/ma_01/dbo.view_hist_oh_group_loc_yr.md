...
```mermaid
flowchart LR
    VIEW["dbo.view_hist_oh_group_loc_yr"]
    dbo_hierarchy_group(["dbo.hierarchy_group"]) --> VIEW
    dbo_hist_oh_group_loc_yr(["dbo.hist_oh_group_loc_yr"]) --> VIEW
```
...
```sql
create view dbo.view_hist_oh_group_loc_yr 
as
select merch_year, location_id, inventory_status_id,
 price_status_id,
 ...
 from hist_oh_group_loc_yr h , hierarchy_group hg
 where h.hierarchy_group_id = hg.hierarchy_group_id and
 hg.hierarchy_id =1
 group by merch_year, location_id, inventory_status_id, price_status_id
```
