...
```mermaid
flowchart LR
    VIEW["dbo.view_dl_hist_group"]
    dbo_dl_hist_group(["dbo.dl_hist_group"]) --> VIEW
```
...
```sql
create view dbo.view_dl_hist_group AS
SELECT dl_hist_group_id,
   record_no,   
   ...
   shrink_actual_cost,
   shrink_provision_units,
   shrink_provision_retail,
   shrink_provision_cost,
