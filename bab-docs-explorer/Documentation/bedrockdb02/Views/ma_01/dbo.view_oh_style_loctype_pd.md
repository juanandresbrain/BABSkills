...
```mermaid
flowchart LR
    VIEW["dbo.view_oh_style_loctype_pd"]
    dbo_hist_oh_style_loc_pd(["dbo.hist_oh_style_loc_pd"]) --> VIEW
...
```sql
create view dbo.view_oh_style_loctype_pd 
AS
select h.style_id, h.merch_year_pd, h.inventory_status_id, 
...
from hist_oh_style_loc_pd h, location l
where h.location_id = l.location_id
group by  h.style_id, h.merch_year_pd, h.inventory_status_id, 
h.price_status_id , h.location_id, l.location_type
```
