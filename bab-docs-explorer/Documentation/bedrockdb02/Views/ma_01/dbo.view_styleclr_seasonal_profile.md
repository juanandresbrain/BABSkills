# dbo.view_styleclr_seasonal_profile
...
```mermaid
flowchart LR
    VIEW["dbo.view_styleclr_seasonal_profile"]
...
```sql
create view dbo.view_styleclr_seasonal_profile AS
SELECT DISTINCT
  sc.style_color_id, 
  sc.style_id,
  sc.color_id,
...
WHERE sc.style_color_id in (SELECT DISTINCT style_color_id FROM seasonal_profile_item)
```
