...
```mermaid
flowchart LR
    VIEW["dbo.view_oh_group_loctype_li"]
...
```sql
create view dbo.view_oh_group_loctype_li 
as
select h.hierarchy_group_id,  h.inventory_status_id,
...
from hist_oh_group_loc_li h, location l
where h.location_id = l.location_id
group by h.hierarchy_group_id,  h.inventory_status_id, h.price_status_id ,
