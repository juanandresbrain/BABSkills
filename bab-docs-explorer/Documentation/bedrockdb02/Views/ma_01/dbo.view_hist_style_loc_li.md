...
```mermaid
flowchart LR
    VIEW["dbo.view_hist_style_loc_li"]
    dbo_hist_style_loc_li(["dbo.hist_style_loc_li"]) --> VIEW
...
```sql
CREATE VIEW [dbo].[view_hist_style_loc_li]

AS
SELECT 
style_id, l.location_id,	
...
FROM hist_style_loc_li
INNER JOIN location l ON l.location_id = hist_style_loc_li.location_id
```
