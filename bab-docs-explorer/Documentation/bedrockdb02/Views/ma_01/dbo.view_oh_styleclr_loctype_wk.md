# dbo.view_oh_styleclr_loctype_wk
...
```mermaid
flowchart LR
    VIEW["dbo.view_oh_styleclr_loctype_wk"]
...
```sql
create view dbo.view_oh_styleclr_loctype_wk 
as
select sc.style_color_id, h.style_id,h.color_id, h.merch_year_wk,
...
from hist_oh_styleclr_loc_wk h, location l ,style_color sc
where h.location_id = l.location_id
and sc.style_id = h.style_id
and sc.color_id= h.color_id
group by sc.style_color_id,   h.style_id,h.color_id, h.merch_year_wk,
h.inventory_status_id, h.price_status_id , h.location_id, l.location_type
```
