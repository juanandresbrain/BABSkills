...
```mermaid
flowchart LR
    VIEW["dbo.view_otb_hist"]
    dbo_hist_group_chn_wk(["dbo.hist_group_chn_wk"]) --> VIEW
...
```sql
create view dbo.view_otb_hist



AS
select distinct c.hierarchy_group_id,
...
from   hist_group_chn_wk c ,post_parameter p
where
 p.parameter_id =23
and p.parameter_value = c.merch_year_wk
group by c.hierarchy_group_id
```
