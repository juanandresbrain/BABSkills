...
```mermaid
flowchart LR
    VIEW["dbo.view_otb_oh_bow"]
    dbo_hist_oh_group_chn_wk(["dbo.hist_oh_group_chn_wk"]) --> VIEW
...
```sql
create view dbo.view_otb_oh_bow

AS
select distinct b.hierarchy_group_id,
SUM((b.on_hand_units)
...
from  hist_oh_group_chn_wk b, post_parameter p ,view_calendar_merch_week_rel e,view_calendar_merch_week_rel d
where p.parameter_id = 23
and  p.parameter_value  = (e.merch_year * 100 + e.merch_week )
and e.relative_week = d.relative_week +1
group by  b.hierarchy_group_id
```
