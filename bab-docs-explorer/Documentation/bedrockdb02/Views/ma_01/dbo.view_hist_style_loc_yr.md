...
```mermaid
flowchart LR
    VIEW["dbo.view_hist_style_loc_yr"]
    dbo_hist_style_loc_yr(["dbo.hist_style_loc_yr"]) --> VIEW
...
```sql
CREATE VIEW [dbo].[view_hist_style_loc_yr]

AS
SELECT 
style_id, merch_year, l.location_id,	
...
FROM hist_style_loc_yr
INNER JOIN location l ON l.location_id = hist_style_loc_yr.location_id
```
