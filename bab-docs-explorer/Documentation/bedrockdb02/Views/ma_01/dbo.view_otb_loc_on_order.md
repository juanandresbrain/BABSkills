...
```mermaid
flowchart LR
    VIEW["dbo.view_otb_loc_on_order"]
...
```sql
create view dbo.view_otb_loc_on_order 

AS
SELECT DISTINCT  a.hierarchy_group_id,
g.merch_year_pd, a.location_id,
SUM(a.on_order_units)on_order_units ,
SUM(a.on_order_retail)on_order_retail, 
SUM(a.on_order_retail_local)on_order_retail_local,
SUM(a.on_order_cost)on_order_cost,
SUM(a.on_order_cost_local)on_order_cost_local,
