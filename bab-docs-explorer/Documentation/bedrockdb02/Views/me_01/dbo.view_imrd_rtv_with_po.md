...
```mermaid
flowchart LR
    VIEW["dbo.view_imrd_rtv_with_po"]
...
```sql
create view dbo.view_imrd_rtv_with_po 
...
FROM dbo.inv_move_req_from_loc,dbo.inventory_move_request, dbo.vendor,dbo.po
WHERE inventory_move_request.inventory_move_request_id = inv_move_req_from_loc.inventory_move_request_id
AND inventory_move_request.document_type = 1
AND inventory_move_request.vendor_id = vendor.vendor_id     
AND inventory_move_request.po_id = po.po_id
AND inventory_move_request.po_id IS NOT NULL
```
