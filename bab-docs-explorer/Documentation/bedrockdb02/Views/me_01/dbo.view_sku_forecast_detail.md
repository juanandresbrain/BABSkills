...
```mermaid
flowchart LR
    VIEW["dbo.view_sku_forecast_detail"]
...
```sql
create view dbo.view_sku_forecast_detail AS
SELECT DISTINCT
  sk.sku_id,
  sk.style_id,
  sk.style_color_id,
...
WHERE sk.sku_id in (SELECT DISTINCT sku_id FROM forecast_detail)
```
