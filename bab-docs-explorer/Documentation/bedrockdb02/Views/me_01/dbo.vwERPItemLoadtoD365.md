...
```sql
CREATE view [dbo].[vwERPItemLoadtoD365]
...
-- LT	2023-11-13	Added new data for item create changes; Jira BIB-647 with BAB Design Document - Retail Item Create - Merch to D365 V2 PO Writing
---------------------------------------------------------------------------------------------------------------
...
select	
		...
		h.HierarchyGroup,
		j.Entity,
		sc.long_desc as ColorDescription,
		sd.long_desc as VendorProductDescription
from	style s with (nolock)
join	style_group sg with (nolock) on s.style_id = sg.style_id
...
join style_color sc on s.style_id = sc.style_id and sc.reorder_flag = 1
left join style_description sd ON s.style_id = sd.style_id AND sd.language_id = 100006
where 1=1
and s.active_flag = 1
