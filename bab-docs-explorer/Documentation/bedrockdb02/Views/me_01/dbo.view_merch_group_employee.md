...
```mermaid
flowchart LR
    VIEW["dbo.view_merch_group_employee"]
...
```sql
CREATE  VIEW [dbo].[view_merch_group_employee]
AS
...
SELECT hierarchy_group_id, u.[USER_ID] AS [user_id]
 ...
 INNER JOIN hierarchy h on hg.hierarchy_id = h.hierarchy_id and h.hierarchy_type = 1 and h.alternate_flag=1
...
SELECT hierarchy_group_id, u.[USER_ID] AS [user_id]
 FROM hierarchy_group hg
 INNER JOIN hierarchy h on hg.hierarchy_id = h.hierarchy_id and h.hierarchy_type = 1 and h.alternate_flag=0
 CROSS JOIN employee_user u WITH (NOLOCK)
...
FROM
  merch_group_parent_full LGPF WITH (NOLOCK)
  INNER JOIN entity_position EP WITH (NOLOCK) on EP.parent_type=5 and EP.parent_id=LGPF.parent_hierarchy_group_id
