...
-- Name: VW_CN_PO_ASN
--
-- Description:	View called by stored proc that creates CSV file to contain PO's shipping to Shanghai warehouses
-- Revision History
--		Name:			Date:			Comments:
--		Dan Tweedie		01/20/2016		Created View.	
--		Tim Callahan	06/11/2018		Updated proc to include both 8502 and 8505 warehouses 
--		Lizzy Timm		10/17/2025		Updated proc to include 9942 warehouse
-- =====================================================================================================



CREATE view [dbo].[VW_CN_PO_ASN]
...
WITH view1 (po_id, po_line_id, line_no, expected_receipt_date, color_code, location_code, location_name, style_code, long_desc, total_line_loc_ordered_units) 
	as (	
		...
		where exists (select po_id from po where po_status IN (4,7) and approval_status in (3,7)) --and po.po_id=view_po_line_location_rep.po_id and po_no in ('1078338','1078337','1078347','1078348','1078350','1078351','1078352','1078353','1078354','1078359','1078360','1078370','1078372','1078378','1078379','1078343','1078344','1078345','1078346','1078406','1078407','1077974')) 
		and location_code in ('3970','8502','8505','3980','9942') -- Added on 06/11/2018; 9942 on 10/17/2025
	   ),
	...
	as (
		...
		where 1=1
		--po.po_id in (select po_id from po (nolock) where po_status IN (4,7) and approval_status in (3,7)) 	
		and exists (select v.po_id from view1 v where v.po_id=po.po_id)
	   ),
	...
	as (
		...
		where 1=1
		and exists (select v.po_id from view1 v where v.po_id=view_po_detail_rep.po_id)
		--po_id in (select po_id from po where po_status IN (4,7)and approval_status in (3,7))
	   ),
	...
	as (
		...
		and l1.location_code in ('3970','8502','8505','3980','9942') -- Added on 06/11/2018; 9942 on 10/17/2025
		and datediff(dd, f.expected_receipt_date, getdate()+14) = 0 
		--and d.po_no in ('1083774')
		)
