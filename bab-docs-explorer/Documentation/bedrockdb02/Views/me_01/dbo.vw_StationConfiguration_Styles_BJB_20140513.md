# dbo.vw_StationConfiguration_Styles_BJB_20140513
...
```mermaid
flowchart LR
    VIEW["dbo.vw_StationConfiguration_Styles_BJB_20140513"]
...
```sql
CREATE VIEW [dbo].[vw_StationConfiguration_Styles_BJB_20140513]
AS
...
--Mike P	20140507		Update Comments to point to new StationMaster server
SELECT TOP 100 PERCENT s.style_code AS STYLE_CD, s.short_desc AS SHORT_DESCR, s.long_desc AS LONG_DESCR, u.upc_number AS UPC, 
                      ecpan.custom_property_value AS ANIMAL_NM, REPLACE(ecpw.custom_property_value, ' oz', '') AS WEIGHT, 
                      ecph.custom_property_value AS HEIGHT, 
                      ...
                      WHEN ISDATE(REPLACE(ecpid.custom_property_value,'//','/')) = 1 THEN REPLACE(ecpid.custom_property_value,'//','/') 
                      ELSE CONVERT(VARCHAR(10), GETDATE(), 101) END AS INSTORE_DT, 
...
WHERE     (hg.hierarchy_group_code LIKE 'R-%-%-25%' OR
                      hg.hierarchy_group_code LIKE 'R-%-%-30%'
					 OR hg.hierarchy_group_code LIKE 'R-_-_-20-07%'
					 OR hg.hierarchy_group_code LIKE 'R-B-D-55%'
					 OR hg.hierarchy_group_code LIKE 'R-B-_-12%') 
