...
```mermaid
flowchart LR
    VIEW["dbo.vwItem_BABWSupplyItems_v3"]
...
```sql
CREATE VIEW [dbo].[vwItem_BABWSupplyItems_v3] AS

WITH d365Supplies_NA
AS
(
	SELECT DISTINCT TOP 100 PERCENT RIGHT('000000' + RIGHT(a.PRODUCTNUMBER, 6), 12) AS 'upc_number'
				   ...
				   ,RIGHT(a.PRODUCTNUMBER, 6) AS 'style_code'
				   ,a.UNITCOST AS 'cost'
				   ,MAX(c2.FACTOR) as 'std_pack_qty'
	--INTO #SVWORKV11_1  
	FROM [STL-SSIS-P-01].IntegrationStaging.ERP.ItemMaster a
	--LEFT JOIN IntegrationStaging.ERP.SupplyCategoryAssignments b ON a.PRODUCTNUMBER = b.ProductNumber
	LEFT JOIN [STL-SSIS-P-01].IntegrationStaging.ERP.ItemsUOM c1 ON a.PRODUCTNUMBER = c1.PRODUCTNUMBER AND c1.Entity = 1100 AND a.INVENTORYUNITSYMBOL = c1.FROMUNITSYMBOL
	LEFT JOIN [STL-SSIS-P-01].IntegrationStaging.ERP.ItemsUOM c2 ON a.PRODUCTNUMBER = c2.PRODUCTNUMBER AND c2.Entity = 1100 AND a.PURCHASEUNITSYMBOL = c2.FROMUNITSYMBOL
	LEFT JOIN [STL-SSIS-P-01].IntegrationStaging.ERP.ItemMasterProducts d ON a.PRODUCTNUMBER = d.PRODUCTNUMBER
	WHERE a.Entity = 1100 AND a.PRODUCTNUMBER LIKE 'S[0-9][0-9][0-9][0-9][0-9][0-9]' 
	GROUP BY a.PRODUCTNUMBER, d.PRODUCTNAME, a.UNITCOST
...
(
	...
	FROM [STL-SSIS-P-01].IntegrationStaging.ERP.ItemMaster a
	--LEFT JOIN IntegrationStaging.ERP.SupplyCategoryAssignments b ON a.PRODUCTNUMBER = b.ProductNumber
	LEFT JOIN [STL-SSIS-P-01].IntegrationStaging.ERP.ItemsUOM c1 ON a.PRODUCTNUMBER = c1.PRODUCTNUMBER AND c1.Entity = 2110 AND a.INVENTORYUNITSYMBOL = c1.FROMUNITSYMBOL
	LEFT JOIN [STL-SSIS-P-01].IntegrationStaging.ERP.ItemsUOM c2 ON a.PRODUCTNUMBER = c2.PRODUCTNUMBER AND c2.Entity = 2110 AND a.PURCHASEUNITSYMBOL = c2.FROMUNITSYMBOL
	LEFT JOIN [STL-SSIS-P-01].IntegrationStaging.ERP.ItemMasterProducts d ON a.PRODUCTNUMBER = d.PRODUCTNUMBER
	WHERE a.Entity = 2110 AND a.PRODUCTNUMBER LIKE 'S[0-9][0-9][0-9][0-9][0-9][0-9]' 
	GROUP BY a.PRODUCTNUMBER, d.PRODUCTNAME, a.UNITCOST
	--ORDER BY a.PRODUCTNUMBER
