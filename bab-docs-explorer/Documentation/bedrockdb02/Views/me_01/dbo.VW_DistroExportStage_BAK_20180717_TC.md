# dbo.VW_DistroExportStage_BAK_20180717_TC
...
```mermaid
flowchart LR
    VIEW["dbo.VW_DistroExportStage_BAK_20180717_TC"]
...
```sql
CREATE view [dbo].[VW_DistroExportStage_BAK_20180717_TC]
as
...
select	ddas.Id, 
		...
		ddas.style_code, 
	--case when ddl.MerchOrSupply = 'Supply'
	--	then	ddl.UnconvertedQty --need to convert cases to units --caN UPDATE ORIGINAL VIEW TO SHOW 'QTY IN CASES' AND 'QTY IN EACHES'
	--	else	ddl.UnconvertedQty ----need to convert cases to units
	--	end as quantity, 
	ddas.quantity * isnull(uom.Factor,1) as quantity, --converts from staged unit to wm eaches
	case 
		when ddas.destid in ('0247','0305') and ddas.rec_type = 1
		then '80'
		else ddas.rec_type
	end as rec_type,
	case 
	...
	case 
		when (ddas.destid in ('0247','0305') and ddas.rec_type = 1) then 'R2'
		when rt.priority in (1,5) then '101'
		when rt.priority in (2,6) then 'R2'
		when rt.priority in (4) and ddas.rec_type in (62, 63) then '091' --this is the FedEx International (Economy or Priority) label type
		else '001' 
