# dbo.vwItem_Items_WithInvOnHand
...
```mermaid
flowchart LR
    VIEW["dbo.vwItem_Items_WithInvOnHand"]
...
```sql
CREATE VIEW [dbo].[vwItem_Items_WithInvOnHand]
AS 
...
AND CAST(u.upc_number AS BIGINT) < 400000
AND st.style_id IN (
	SELECT s.style_id 
	...
	INNER JOIN me_01.dbo.entity_attribute_set eas ON s.style_id = eas.parent_id
