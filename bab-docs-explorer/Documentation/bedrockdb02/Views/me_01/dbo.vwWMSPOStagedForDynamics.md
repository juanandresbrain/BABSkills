...
```mermaid
flowchart LR
    VIEW["dbo.vwWMSPOStagedForDynamics"]
...
```sql
CREATE view  [dbo].[vwWMSPOStagedForDynamics] as
...
Locs as 
	(
		select 
			l.location_code,
			cast(l.location_code as int) as LocationInt,
			l.gl_location_number,
			a.address_name, 
			case 
				when c.country_code in ('USA','US') then '1100' 
				when c.country_code in ('CAN','CA') then '1700'
				when c.country_code in ('GB','UK') then '2110'
				when c.country_code in ('CN') then '3001' 
			end as Company
		from location l
		join address a 
			on l.location_id=a.parent_id 
			and a.parent_type=2--store
			and a.address_type_id=1 --main
		join country c on a.country_id=c.country_id
		where 
			len(case 
				when c.country_code='USA' 
					then 'US' 
				when c.country_code='CAN'
					then 'CA'
				when c.country_code='GB'
					then 'UK'
				else c.country_code 
			end) =2
			--found 3 CAF locations (central african republic??)		
	)
select distinct
	cast(case 
		when po.ShipToID in (980,960,13) 
			then '1100'
		when po.ShipToID in (2970,2991)
			then '2110'
		when po.ShipToID in (3970)
			then '3001'
		when po.ShipToID in (3980,9942)
			then '1200'
		else (select top 1 l.Company from Locs l where po.ShipToID=l.LocationInt)
	end as varchar) as Company,
	cast(case 
		po.ShipToID 
			when 980 then '9980'
			when 960 then '9960'
			...
			when 3980 then '9941'
			when 2991 then '8010'
		else (select top 1 l.gl_location_number from Locs l where po.ShipToID=l.LocationInt)
	end as varchar) as Warehouse,
...
where 1=1
and (po.ShipToID in (980,960,13,2970,3970,3980,9942,2991) or po.ShipToID between 1and 600 or po.ShipToID between 2000 and 2999) -- updated from between 1 and 500 to 1 and 600; LT 11/12/24
...
left join POLineNetFinalPrice nfp 
	...
	and po.line_no=nfp.POMainLine
where 1=1
and (po.ShipToID in (980,960,13,2970,3970,3980,9942,2991) or po.ShipToID between 1 and 600 or po.ShipToID between 2000 and 2999) -- updated from between 1 and 500 to 1 and 600; LT 11/12/24
