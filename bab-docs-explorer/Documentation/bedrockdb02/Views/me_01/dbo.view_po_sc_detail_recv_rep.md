# dbo.view_po_sc_detail_recv_rep
...
```mermaid
flowchart LR
    VIEW["dbo.view_po_sc_detail_recv_rep"]
...
```sql
create view dbo.view_po_sc_detail_recv_rep 
...
FROM	po 
		...
		(SELECT DISTINCT
				...
				sku_id
		FROM	po_detail
		WHERE	pack_id IS NULL) pd
		ON (pd.po_id = po.po_id) 
		INNER JOIN (SELECT	sku_id,   
							document_number,
