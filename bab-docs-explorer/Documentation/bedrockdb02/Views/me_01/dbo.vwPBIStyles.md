...
flowchart LR
    ...
    dbo_vwPBISingleUPC(["dbo.vwPBISingleUPC"]) --> VIEW
```
...
dbo,vwPLM_NotificationOfProductExport_Styles,CREATE VIEW vwPLM_NotificationOfProductExport_Styles
AS
SELECT s.style_code, sv.current_cost, s.short_desc 
FROM me_01.dbo.style s (NOLOCK)
INNER JOIN dbo.style_vendor sv (NOLOCK) ON s.style_id = sv.style_id
WHERE  sv.primary_vendor_flag = 1
```
