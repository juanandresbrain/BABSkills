...
```mermaid
flowchart LR
    VIEW["dbo.vwDistroExportTransferOrderCreate"]
...
```sql
CREATE view  [dbo].[vwDistroExportTransferOrderCreate] 
...
PreStage as
	(
		...
		--and ss.document_number in ('2900172122', '2900172132') 
	)
select 
	FromWarehouse,
	ToWarehouse,
	...
	cast(
		case 
			...
			and NOT (SourceCountryCode='UK' and DestinationCountryCode in ('UK', 'GB','IE') )
				then 'SalesOrder'
			else 'TransferOrder'
		end as varchar(15)
		) as OrderType
