# dbo.VW_DistroExportPreSplit_KL_TEST
...
```mermaid
flowchart LR
    VIEW["dbo.VW_DistroExportPreSplit_KL_TEST"]
...
```sql
CREATE view [dbo].[VW_DistroExportPreSplit_KL_TEST]
as 
...
and		(isnull(ats.attribute_set_code,1) >= 50
	or		(isnull(ats.attribute_set_code,1) < 50 and datepart(hh,getdate()) >= 16)
