# dbo.VW_DistroExportPreSplit_SPLIT_TOOL_TEST
...
```mermaid
flowchart LR
    VIEW["dbo.VW_DistroExportPreSplit_SPLIT_TOOL_TEST"]
...
```sql
Create view [dbo].[VW_DistroExportPreSplit_SPLIT_TOOL_TEST]
as 
...
and		(isnull(ats.attribute_set_code,1) >= 50
	or		(isnull(ats.attribute_set_code,1) < 50 and datepart(hh,getdate()) >= 0)
