# dbo.vw_StationConfiguration_Styles
...
```mermaid
flowchart LR
    VIEW["dbo.vw_StationConfiguration_Styles"]
...
```sql
CREATE VIEW [dbo].[vw_StationConfiguration_Styles]
AS
...
Ben B   20160405		Added logic to remove duplicate China product
Ben B	20190114		Updated logic to override fur color if the new fur color custom property exists
*/ SELECT
                          TOP (100) PERCENT s.style_code AS STYLE_CD, s.short_desc AS SHORT_DESCR, s.long_desc AS LONG_DESCR, u.upc_number AS UPC, 
                         ...
                         attat.attribute_set_label AS ANIMAL_TYPE, attec.attribute_set_label AS EYE_COLOR, CASE WHEN ecpcolor.custom_property_value IS NULL THEN c.color_long_description ELSE ecpcolor.custom_property_value END AS FUR_COLOR, 
                         hg.hierarchy_group_code AS HIERARCHY_GROUP_CD, CASE WHEN ISNULL(eas.parent_id, 0) > 0 THEN 2 /*Dino*/ WHEN LEFT(hg.hierarchy_group_code, 3) 
                         = 'R-B-Z' THEN 3 /*Licensed product*/ ELSE 1 END ProductTypeID
FROM            dbo.style AS s INNER JOIN
						 ...
						 dbo.entity_custom_property AS ecpcolor ON s.style_id = ecpcolor.parent_id AND ecpcolor.custom_property_id = 62 LEFT OUTER JOIN
                         ...
                         dbo.attribute_set AS attec ON easec.attribute_set_id = attec.attribute_set_id LEFT JOIN
                         me_01.dbo.entity_attribute_set eas(nolock) ON s.style_id = eas.parent_id AND eas.attribute_id = 572 AND eas.attribute_set_id = 57200008 /*DINO*/ LEFT OUTER JOIN
                         ...
                         dbo.upc AS u ON sk.sku_id = u.sku_id AND u.upc_number < '000001000000' AND u.upc_number IS NOT NULL LEFT OUTER JOIN
                             (SELECT        b.style_code
                               ...
                               WHERE        (LEFT(a.style_code, 1) = '0') AND (LEFT(b.style_code, 1) = '1' OR
                                                         LEFT(b.style_code, 1) = '4' OR
                                                         LEFT(b.style_code, 1) = '8')) AS dup1 ON s.style_code = dup1.style_code LEFT OUTER JOIN
                             (SELECT        b.style_code
                               FROM            dbo.style AS a INNER JOIN
                                                         dbo.style AS b ON RIGHT(a.style_code, 5) = RIGHT(b.style_code, 5) AND a.style_code < b.style_code
                               WHERE        (LEFT(a.style_code, 1) = '1') AND (LEFT(b.style_code, 1) = '4') OR (LEFT(b.style_code, 1) = '8')) AS dup2 ON s.style_code = dup2.style_code LEFT OUTER JOIN
                             (SELECT        b.style_code
                               FROM            dbo.style AS a INNER JOIN
                                                         dbo.style AS b ON RIGHT(a.style_code, 5) = RIGHT(b.style_code, 5) AND a.style_code < b.style_code
                               WHERE        (LEFT(a.style_code, 1) = '4') AND (LEFT(b.style_code, 1) = '8')) AS dup3 ON s.style_code = dup3.style_code LEFT OUTER JOIN
							 (SELECT        b.style_code
                               FROM            dbo.style AS a INNER JOIN
                                                         dbo.style AS b ON RIGHT(a.style_code, 5) = RIGHT(b.style_code, 5) LEFT JOIN
														 me_01.dbo.entity_attribute_set eas(nolock) ON a.style_id = eas.parent_id AND eas.attribute_id = 572 AND eas.attribute_set_id = 57200008 /*DINO*/
                               WHERE        (LEFT(a.style_code, 1) = '3') AND (LEFT(b.style_code, 1) = '1' OR
                                                         LEFT(b.style_code, 1) = '4' OR
                                                         LEFT(b.style_code, 1) = '8')) AS dup4 ON s.style_code = dup4.style_code
WHERE        (hg.hierarchy_group_code LIKE 'R-%-%-25%' OR
                         ...
                         hg.hierarchy_group_code LIKE 'W-_-_-02%' OR
                         /* Animals*/ hg.hierarchy_group_code LIKE 'W-_-_-04-02%' OR
                         /* Prestuffed*/ hg.hierarchy_group_code LIKE 'W-_-_-12-01%' OR
                         /* Sounds*/ hg.hierarchy_group_code LIKE 'W-_-_-12_02%' OR
                         /* Scents*/ hg.hierarchy_group_code LIKE 'W-_-_-06_07%'/* Sports*/ ) AND (sc.reorder_flag = 1) AND (dup1.style_code IS NULL) AND (dup2.style_code IS NULL) AND (dup3.style_code IS NULL) AND (dup4.style_code IS NULL)
UNION ALL
/*BOXED GIFT SETS*/ SELECT TOP (100) PERCENT s.style_code AS STYLE_CD, s.short_desc AS SHORT_DESCR, s.long_desc AS LONG_DESCR, u.upc_number AS UPC, 
                         ecpan.custom_property_value AS ANIMAL_NM, REPLACE(ecpw.custom_property_value, ' oz', '') AS WEIGHT, ecph.custom_property_value AS HEIGHT, 
                         CASE WHEN ecpid.custom_property_value IS NULL THEN '1/1/2000' WHEN ecpid.custom_property_value LIKE '%JANUARY%' THEN '1/1/' + CAST(DATEPART(YEAR, 
                         GETDATE()) AS VARCHAR) WHEN ecpid.custom_property_value LIKE '%FEBRUARY%' THEN '2/1/' + CAST(DATEPART(YEAR, GETDATE()) AS VARCHAR) 
                         WHEN ecpid.custom_property_value LIKE '%MARCH%' THEN '3/1/' + CAST(DATEPART(YEAR, GETDATE()) AS VARCHAR) 
                         ...
                         dbo.upc AS u ON sk.sku_id = u.sku_id AND u.upc_number < '000001000000' AND u.upc_number IS NOT NULL LEFT OUTER JOIN
							 ...
							 (SELECT        b.style_code
                               ...
                               WHERE        (LEFT(a.style_code, 1) = '3') AND (LEFT(b.style_code, 1) = '1' OR
                                                         LEFT(b.style_code, 1) = '4' OR
                                                         LEFT(b.style_code, 1) = '8')) AS dup4 ON s.style_code = dup4.style_code
WHERE        (hg.hierarchy_group_code LIKE 'R-B-D-47-01%') AND (sc.reorder_flag = 1) AND (dup1.style_code IS NULL) AND (dup2.style_code IS NULL) AND (dup3.style_code IS NULL) AND (dup4.style_code IS NULL) AND 
                         ecpan.custom_property_value IS NOT NULL
ORDER BY STYLE_CD
```
