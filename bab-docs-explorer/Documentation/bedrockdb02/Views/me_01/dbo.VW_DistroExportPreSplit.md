...
```mermaid
flowchart LR
    VIEW["dbo.VW_DistroExportPreSplit"]
...
```sql
CREATE view [dbo].[VW_DistroExportPreSplit]
as 
...
and		l1.location_code in ('0980') -- '9913','9914','9915','9916','9917','9918','9919','9920','9921','9922','9125') Remarked out 4/11/2017 due to performance issues 
and		dd.quantity > 0
and		ds.distribution_number is null
and		ds.destid is null
and		sc.reorder_flag = 1
and		(
			isnull(ats.attribute_set_code,1) >= 50
			--or		(isnull(ats.attribute_set_code,1) < 50 and datepart(hh,getdate()) >= 18) -- Change from 18\6pm to 13\1pm at Request of Distro\Bearhouse for a one time early load to MA-WMS 
			or (isnull(ats.attribute_set_code,1) < 50 and convert(varchar, getdate(), 108) >= '16:30:00') -- Updated from 18 to 14:30, SR 27924
			or			atswc.attribute_set_code ='960'
	--	or l2.location_code IN ('0453','0422') -- Only to specific store when distro specially requests
		)		
union all
select	l1.location_code as sourceid,
		l2.location_code as destid,
...
and		(
			isnull(ats.attribute_set_code,1) >= 50
			--or		(isnull(ats.attribute_set_code,1) < 50 and datepart(hh,getdate()) >= 18) -- Change from 18\6pm to 19\7pm at Request of Distro to extend distro export cutoff during 4th quarter			
			or (isnull(ats.attribute_set_code,1) < 50 and convert(varchar, getdate(), 108) >= '14:30:00') -- Updated from 18 to 16:30, SR 27924; updated from 16:30 to 14:30 in reaction to 3PW related issues	
--			or (l2.location_code in ('0011')) -- Only to specific store when distro specially requests)
	)
union all
select	l1.location_code as sourceid,
		l2.location_code as destid,
...
and		(
			isnull(ats.attribute_set_code,1) >= 50
			--or		(isnull(ats.attribute_set_code,1) < 50 and datepart(hh,getdate()) >= 18) -- Change from 18\6pm to 19\7pm at Request of Distro to extend distro export cutoff during 4th quarter; 
			or (isnull(ats.attribute_set_code,1) < 50 and convert(varchar, getdate(), 108) >= '14:30:00') -- updated from 18 to 14:30 in reaction to 3PW related issues -LT 3/22/23;	
			--or (isnull(ats.attribute_set_code,1) < 50 and convert(varchar, getdate(), 108) >= '16:30:00')  -- Updated from 18 to 16:30, SR 27924	
		) 
...
left join attribute_set ats with (nolock) on das.attribute_set_id = ats.attribute_set_id
	and		ats.attribute_id = 112
where	d.distribution_status in (6,7) -- 2 = Preliminary 5 = Open 6 = Release 9 = Cancelled
and		l1.location_code in ('3970','8502','8505')
and		dd.quantity > 0
and		ds.distribution_number is null
and		ds.destid is null
and		(
			isnull(ats.attribute_set_code,1) >= 50
			or	
			(isnull(ats.attribute_set_code,1) < 50 and datepart(hh,getdate()) >= 15)  -- Typically 15 aka 3 pm
		)
```
