# dbo.vwGiftCardDetail_International
...
```mermaid
flowchart LR
    VIEW["dbo.vwGiftCardDetail_International"]
...
```sql
create view [dbo].[vwGiftCardDetail_International] 
as
SELECT [merchant_id] as original_merchant_id
	  ...
	  ,cast(convert(varchar(10),dbo.GiftCard_Detail_International.FDMS_local_timestamp,101) 
		as datetime) TransactionDate
      ,[escheatable_transaction]
	  ,cast(convert(varchar(10),dbo.GiftCard_Detail_International.processed_date,101) 
		as datetime) [processed_date]
      ,[original_request_code]
      ,[internal_request_code]
 	  ,cast(convert(varchar(10),dbo.GiftCard_Detail_International.exported_date,101) 
		as datetime) [exported_date]
  FROM [dw].[dbo].[GiftCard_Detail_International]
