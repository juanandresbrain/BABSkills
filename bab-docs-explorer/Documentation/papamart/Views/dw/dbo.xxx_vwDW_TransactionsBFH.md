...
```mermaid
flowchart LR
    VIEW["dbo.xxx_vwDW_TransactionsBFH"]
    bf2005_transdetailfacts(["bf2005_transdetailfacts"]) --> VIEW
...
```sql
CREATE VIEW [dbo].[vwDW_TransactionsBFH]
AS
SELECT temp.*
	...
	--					THEN tdf1.unit_disc_amount END ,0)) AS GiftCardDiscount
					...
					,SUM(CASE WHEN RIGHT(p.department_code, 2) = '25' OR RIGHT(p.subclass_code, 2) = '25' THEN ISNULL(tdf1.units, 0) ELSE 0 END) AS AnimalUnits
					,SUM(CASE WHEN RIGHT(department_code, 2) = '15' THEN ISNULL(tdf1.units, 0) ELSE 0 END) AS ShoeUnits
					,SUM(CASE WHEN RIGHT(department_code, 2) = '20' THEN ISNULL(tdf1.units, 0) ELSE 0 END) AS SoundUnits
...
store_key,
transaction_id = -9999,
tender_group_key = -9999,
transaction_key = '-9999',
PartyFlag = 0,
LineCount= 0,
currency_key= 0,
GAAPTransactionFlag= 0,
unit_net_amount= 0,
Animal_UGA= 0,
Non_Animal_UGA= 0,
Footwear_UGA= 0,
...
IsCompNextYear= 0,
CompGaapSales= 0,
CompGAAPTransactionFlag= 0,
CompAnimalUnits= 0,
CompSoundUnits= 0,
CompShoeUnits= 0,
CompAnimal_UGA= 0,
CompPartyFlag= 0,
CompMerchandiseUnits= 0,
GaapSalesForCompLY= 0,
GAAPTransactionFlagForCompLY= 0,
AnimalUnitsForCompLY= 0,
SoundUnitsForCompLY= 0,
ShoeUnitsForCompLY= 0,
Animal_UGAForCompLY= 0,
PartyFlagForCompLY= 0,
MerchandiseUnitsForCompLY = 0
 from bf2005_transdetailfacts
```
