...
```sql
CREATE view [dbo].[vwCRMserviceCloudCSV] 

as 

select top 10000 [customerNumber],REPLACE([SubscriberKey],',',' ') as SubscriberKey,NULLIF([status], '') as 'status',
			...
			,NTILE(2) OVER (order by customerNumber asc ) as groupNum
			 from DW.dbo.CRMde1
```
