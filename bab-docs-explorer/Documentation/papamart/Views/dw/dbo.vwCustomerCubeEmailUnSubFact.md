# dbo.vwCustomerCubeEmailUnSubFact
...
```mermaid
flowchart LR
    VIEW["dbo.vwCustomerCubeEmailUnSubFact"]
...
```sql
CREATE view [dbo].[vwCustomerCubeEmailUnSubFact]
as
...
join date_dim dd with (nolock) on cast(e.UnSubDate as date) = cast(dd.actual_date as date)
join time_dim td with (nolock) 
	on datepart(hh,e.UnSubDate) = td.hour 
	and datepart(mi,e.UnSubDate) = td.minute
join vwCustomerCubeCustomerDim cd with (nolock) on e.SubscriberKey=cd.SubscriberKey
where e.UnSubDate is not null
```
