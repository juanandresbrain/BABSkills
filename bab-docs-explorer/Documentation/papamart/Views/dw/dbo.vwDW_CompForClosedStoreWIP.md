# dbo.vwDW_CompForClosedStoreWIP
...
```mermaid
flowchart LR
    VIEW["dbo.vwDW_CompForClosedStoreWIP"]
...
```sql
CREATE VIEW [dbo].[vwDW_CompForClosedStoreWIP]
-- =============================================================================================================
...
/* ***** Level 4 starts - determine max_ly_comp_date for closed stores *************
...
FROM 
  dw.dbo.date_dim d WITH (NOLOCK) right JOIN
...
--,d.org_fiscal_period  closing_max_comp_fiscal_period
,d.org_fiscal_week  closing_max_comp_fiscal_week
...
SELECT 	
cd1.store_key,cd1.store_id,cd1.store_name
,cd1.bearritory,cast(cd1.IsClosed as bit) IsClosed
...
/* *********************Level 1 starts - Get corresponding fiscal information for closing date  *********************/
	...
	SELECT s.store_key,cast(s.store_id as varchar(50)) store_id,s.store_name
		,s.closing_date, d.date_key closing_date_key
		, d.fiscal_year closing_fiscal_year ,d.fiscal_period closing_fiscal_period
		,s.bearritory
		, CASE WHEN s.closing_date is not null or s.bearritory like '%Closed%' 
		 THEN 1 ELSE 0 END AS IsClosed
	  FROM  dw.dbo.date_dim d  right join dbo.store_dim s  --store_franchise_dim s  
...
UNION
	SELECT s.store_key,cast(s.store_id as varchar(50)) store_id,s.store_name
		...
		, CASE WHEN s.closing_date is not null or s.bearritory like '%Closed%' 
		 THEN 1 ELSE 0 END AS IsClosed
	  FROM  dw.dbo.date_dim d  right join dbo.store_franchise_dim s   
	   ON d.actual_date = s.closing_date
...
GROUP BY 
cd1.store_key,cd1.store_id,cd1.store_name
,cd1.closing_date,cd1.closing_date_key
, cd1.closing_fiscal_year ,cd1.closing_fiscal_period
,cd1.IsClosed,cd1.Bearritory
...
/* ********************************************* Level 3 ends *************************************/
 ...
 --and d.org_fiscal_period = cd3.closing_max_comp_fiscal_period
 and d.org_fiscal_week = cd3.closing_max_comp_fiscal_week
GROUP BY 
cd3.store_key
,cd3.store_id
,cd3.store_name
