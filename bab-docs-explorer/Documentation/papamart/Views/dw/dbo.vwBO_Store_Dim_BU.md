...
```sql
--ALTER VIEW [dbo].[vwDW_Store_dev_4_RZ]
create VIEW [dbo].[vwBO_Store_Dim_BU]
...
11-14-2006 JLL remove UK Web store from its own bearitory; Change report flag and Club max flag logic*/
	...
	(
		SELECT s.store_key
				...
				,s.store_name
				,IsNull(s.store_name,'(blank)') as store_display
				,s.storeNameNum
				,s.bearea	
				--,s.bearritory
--commented out for UK alignment changes Aug 2007
				...
				,IsNull(s.dma_code,0) AS dma_code
				,CASE WHEN s.metro_name IS NULL OR s.metro_name = '' THEN 'Other' ELSE s.metro_name END AS metro_name
				,IsNull(s.metro_code,0) AS metro_code
				,CASE WHEN s.cluster_name IS NULL OR s.cluster_name = '' THEN 'Other' ELSE s.cluster_name END AS cluster_name
				,IsNull(s.cluster_code,0) AS cluster_code
				,s.opening_date
				,s.opening_date_id
				,s.closing_date
				,s.comp_week_id
...
--				END AS ReportFlag
				...
				,CASE
					WHEN s.bearritory like '%Closed%' THEN 0
					WHEN s.store_id in (480, 482, 013, 136, 473) THEN 2
					WHEN s.region IN ('Central US', 'East US', 'West US', 'North UK', 'South UK') THEN 1
					WHEN s.region IN ('US-Corporate', 'Web Stores', 'Web Store') THEN 2
					ELSE 0
				END AS ClubMaxFlag
				...
				,Case 	WHEN s.store_id in (480, 482, 013, 136, 473) then 'North America'
						...
						WHEN s.region IN ('North UK', 'South UK') then 'Europe'
				...
				, CASE	WHEN s.bearritory like '%Closed%' THEN 'Other'
						...
						WHEN s.region IN ('East US', 'Central US', 'West US', 'North UK', 'South UK') THEN 'Company'
						WHEN s.region IN ('East US', 'Central US', 'West US', 'North UK', 'South UK') THEN 'Company'
...
--			WHERE s.store_name IS NOT NULL
			...
			SELECT
				...
				,StoreType = 'N/A'
				,StoreProductType = 'N/A'
				,StoreSeasonality = 'N/A'
