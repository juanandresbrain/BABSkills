...
```mermaid
flowchart LR
    VIEW["dbo.vwDW_ShopperTrak_Cube"]
...
```sql
CREATE view [dbo].[vwDW_ShopperTrak_Cube] --WITH SCHEMABINDING
as
-- =============================================================================================================
...
--		Matt Lewis			2026-05-26		Reintroduced Dan's commented out logic from 08/08/24, but using CTEs to make it more efficient as I presume the performance is why it was commented out. 
-- =============================================================================================================
...
--CASE WHEN STXNY.isSTComp = '1' then '1' else STXC.isSTComp end as isSTComp,
STXC.isSTComp,
CASE WHEN STXNY.isCompThisYear = 1 then 1 else CASE WHEN STXNY2.isCompThisYear = 1 THEN 1 ELSE STXC.isSTCompNextYear END end as isSTCompNextYear, 
...
FROM
	...
	left join (select distinct store_key, dmonth, dyear, isCompThisYear, isSTCompNextYear from STX) as STXNY
	on STXC.store_key = STXNY.store_key  and STXC.dmonth = STXNY.dmonth and STXC.dyear + 1 = STXNY.dyear
		left join (select distinct store_key, dmonth, dyear, isCompThisYear, isSTCompNextYear from STX) as STXNY2
