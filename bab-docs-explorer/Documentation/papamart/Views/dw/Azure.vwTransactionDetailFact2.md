# Azure.vwTransactionDetailFact2
...
```mermaid
flowchart LR
    VIEW["Azure.vwTransactionDetailFact2"]
...
```sql
/* =============================================================================================================
 Name: [Azure].[vwTransactionDetailFacts2]

 Description: Brings Franchisee and Corporate records inbto one view
...
*/
CREATE VIEW [Azure].[vwTransactionDetailFact2]
AS
SELECT        tdf.product_key AS ProductKey, cd.currency_code AS CurrencyCode, Cast(tdf.transaction_id as varchar(20))  AS TransactionID, tdf.transaction_line_seq AS TransactionLineSeq, tdf.Register_Num AS RegisterNumber, CONVERT(DATE, 
                         dd.actual_date) AS TransactionDate, CAST(CONVERT(VARCHAR, CONVERT(DATE, dd.actual_date)) + ' ' + LEFT(CONVERT(TIME, CONVERT(VARCHAR, td.hour) + ':' + CONVERT(VARCHAR, td.minute)), 5) 
