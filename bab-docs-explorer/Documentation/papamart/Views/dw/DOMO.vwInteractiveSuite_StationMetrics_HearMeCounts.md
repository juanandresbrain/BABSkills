...
```mermaid
flowchart LR
    VIEW["DOMO.vwInteractiveSuite_StationMetrics_HearMeCounts"]
...
```sql
CREATE VIEW [DOMO].[vwInteractiveSuite_StationMetrics_HearMeCounts]
AS
WITH StationNameMeIPAll (StoreNumber, StationIP, MetricIDKey, MetricValueCount) AS (
		...
		CAST (MetricDateTime AS DATE ) BETWEEN CAST ( GETDATE() -2 AS DATE ) AND CAST ( GETDATE() -1 AS DATE ) AND   --Performance enhancement
		EventType = 'HEARME'
...
CAST(dd1.actual_date AS DATE ) BETWEEN CAST ( GETDATE() -2 AS DATE ) AND CAST ( GETDATE() -1 AS DATE )    --Performance enhancement
