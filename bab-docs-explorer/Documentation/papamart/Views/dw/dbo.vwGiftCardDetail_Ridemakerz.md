# dbo.vwGiftCardDetail_Ridemakerz
...
```mermaid
flowchart LR
    VIEW["dbo.vwGiftCardDetail_Ridemakerz"]
    dbo_GiftCard_Detail_Ridemakerz(["dbo.GiftCard_Detail_Ridemakerz"]) --> VIEW
```
...
| Referenced Table |
|---|
| dbo.GiftCard_Detail_Ridemakerz |

## View Code

```sql
create view [dbo].[vwGiftCardDetail_Ridemakerz] 
as
SELECT [merchant_id] as original_merchant_id
		...
		then '97153900006' else [merchant_id] end as [merchant_id]
	  ...
	  ,cast(convert(varchar(10),dbo.GiftCard_Detail_Ridemakerz.FDMS_local_timestamp,101) 
		as datetime) TransactionDate
      ,[escheatable_transaction]
	  ,cast(convert(varchar(10),dbo.GiftCard_Detail_Ridemakerz.processed_date,101) 
		as datetime) [processed_date]
      ,[original_request_code]
      ,[internal_request_code]
 	  ,cast(convert(varchar(10),dbo.GiftCard_Detail_Ridemakerz.exported_date,101) 
		as datetime) [exported_date]
  FROM [dw].[dbo].[GiftCard_Detail_Ridemakerz]
WHERE internal_request_code in (8,18,28,43,68,155,23,167,1)
```
