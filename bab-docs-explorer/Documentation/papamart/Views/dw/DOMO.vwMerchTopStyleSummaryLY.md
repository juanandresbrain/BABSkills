...
```mermaid
flowchart LR
    VIEW["DOMO.vwMerchTopStyleSummaryLY"]
    dbo_date_dim(["dbo.date_dim"]) --> VIEW
    dbo_exchange_rate_facts(["dbo.exchange_rate_facts"]) --> VIEW
    dbo_TopStyleLYDomoStage(["dbo.TopStyleLYDomoStage"]) --> VIEW
```
...
| dbo.TopStyleLYDomoStage |

## View Code

```sql
CREATE view [DOMO].[vwMerchTopStyleSummaryLY]
...
select 
	...
	dd.WeekEndingDate
from dwstaging.dbo.TopStyleLYDomoStage s
join DateDim dd 
