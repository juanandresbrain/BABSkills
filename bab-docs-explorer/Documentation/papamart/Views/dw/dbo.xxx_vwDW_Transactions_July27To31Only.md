# dbo.xxx_vwDW_Transactions_July27To31Only
...
```mermaid
flowchart LR
    VIEW["dbo.xxx_vwDW_Transactions_July27To31Only"]
...
```sql
CREATE VIEW [dbo].[vwDW_Transactions_July27To31Only]
AS
/***********************************
...
--					/*** END - Adding new keys - TMK - 2007-08-27 ***/
	...
	--					THEN tdf1.unit_disc_amount END ,0)) AS GiftCardDiscount
			...
			--SMF BEGIN 20080118
			left join (select top 100 Percent transaction_id 
							, store_key
							, date_key
		...
		LEFT JOIN	/*get the total amount of tax (-1) and redemptions (621,633,640,690) for the tender group*/
			(
				SELECT
					...
					,COUNT(CASE WHEN tg.tender_key = 22 THEN 1 ELSE NULL END) AS reward_redemption_key
...
--	WHERE date_key IN (SELECT date_key FROM date_dim WHERE fiscal_year = 2008 and fiscal_period > 7)
--	WHERE date_key IN (SELECT date_key FROM date_dim WHERE actual_date > '7/31/2008')
where date_key in
(select date_key from dbo.date_dim where actual_date in ('2008-07-27', '2008-07-28', '2008-07-29', '2008-07-30', '2008-07-31'))
...
--UNION ALL
--SELECT * from tbl_Transactions_2004
--UNION ALL
--SELECT * from tbl_Transactions_2005
--UNION ALL
--SELECT * from tbl_Transactions_2006
--UNION ALL
--SELECT * from tbl_Transactions_2007
--UNION ALL
--SELECT * from tbl_Transactions_2008
