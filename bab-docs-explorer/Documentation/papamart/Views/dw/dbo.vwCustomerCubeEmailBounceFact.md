# dbo.vwCustomerCubeEmailBounceFact
...
```mermaid
flowchart LR
    VIEW["dbo.vwCustomerCubeEmailBounceFact"]
...
```sql
CREATE view [dbo].[vwCustomerCubeEmailBounceFact]
as
...
join date_dim dd with (nolock) on cast(e.BounceDate as date) = cast(dd.actual_date as date)
join time_dim td with (nolock) 
	on datepart(hh,e.BounceDate) = td.hour 
	and datepart(mi,e.BounceDate) = td.minute
join vwCustomerCubeCustomerDim cd with (nolock) on e.SubscriberKey=cd.SubscriberKey
where e.BounceDate is not null
```
