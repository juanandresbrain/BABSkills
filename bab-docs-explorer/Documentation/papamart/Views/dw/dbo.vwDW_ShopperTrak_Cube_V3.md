...
--		Matt Lewis			2026-06-09		Updated STXNY to use row_number as column rown for better deduplication
-- =============================================================================================================
...
STXNY as (select  store_key, month as dmonth, year as dyear, isShopperTrakCompTY, row_number() over (partition by store_key, month, year order by store_key, month, year,isShopperTrakCompTY desc) rown
	 from ShopperTrackFact STTF WITH (NOLOCK)
	INNER JOIN date_dim dd WITH (NOLOCK)
		ON dd.date_key = STTF.DateKey
	LEFT JOIN StoreCompDetail_Dim cmp WITH (NOLOCK)
...
left join STXNY
	on STXC.store_key = STXNY.store_key  and STXC.dmonth = STXNY.dmonth and STXC.dyear + 1 = STXNY.dyear and STXNY.rown = 1
left join STXNY as STXNY2
	on STXC.store_key = STXNY2.store_key  and STXC.dmonth = STXNY2.dmonth and STXC.dyear + 2 = STXNY2.dyear and STXNY2.rown = 1
```
