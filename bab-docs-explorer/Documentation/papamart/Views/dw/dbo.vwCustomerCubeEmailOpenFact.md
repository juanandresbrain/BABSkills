# dbo.vwCustomerCubeEmailOpenFact
...
```mermaid
flowchart LR
    VIEW["dbo.vwCustomerCubeEmailOpenFact"]
...
```sql
CREATE view [dbo].[vwCustomerCubeEmailOpenFact]
as
...
join date_dim dd with (nolock) on cast(e.OpenDate as date) = cast(dd.actual_date as date)
join time_dim td with (nolock) 
	on datepart(hh,e.OpenDate) = td.hour 
	and datepart(mi,e.OpenDate) = td.minute
join vwCustomerCubeCustomerDim cd with (nolock) on e.SubscriberKey=cd.SubscriberKey
where e.OpenDate is not null
```
