# dbo.vwStore_dim_ForCube_WIPFinalPass
...
```mermaid
flowchart LR
    VIEW["dbo.vwStore_dim_ForCube_WIPFinalPass"]
...
```sql
CREATE VIEW [dbo].[vwStore_dim_ForCube_WIPFinalPass]
-- =============================================================================================================
...
-- Revision History
--		Name:			Date:			Comments:
--		Funmi Agbebi	02/15/2010		Added 'IsClosed' field to flag closed stores
--		Funmi Agbebi	02/15/2010		Set all stores with a closing_date to 'Closed Stores' or 'UK Closed Stores' bearritory 
...
storeNameNum = CASE
	...
	--			WHEN sd.store_id IN (13, 136, 473, - 991) THEN 'Web Stores' 
				...
				WHEN sd.country = 'UK' and closing_date is not null THEN 'UK Closed Stores' --FA 2/15/2010
				WHEN sd.country <> 'UK' and closing_date is not null THEN 'Closed Stores' --FA 2/15/2010
				ELSE sd.bearritory 
			END AS bearritory, 

/*	
select top 3 * from store_dim
select * -- bearritory 
from dbo.store_dim 
where bearritory like '%closed%'
select  bearritory from dbo.store_dim where bearritory like '%closed%'
group by bearritory
*/
...
--					set store 13 and 136 to Web Stores and FTD Sales and 991 to 'Other'
					...
					WHEN bearritory in ('Midlands', 'Scotland & North', 'Southeast UK', 'Southern UK', 'Southwest UK') then 'UK'--added 4/1/08 JLL
...
,  CASE WHEN closing_date is not null or sd.bearritory like '%Closed' 
	THEN 'Y' ELSE 'N' END AS IsClosed --FA 2/15/2010
