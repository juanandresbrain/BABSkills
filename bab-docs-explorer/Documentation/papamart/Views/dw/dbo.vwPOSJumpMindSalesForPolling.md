# dbo.vwPOSJumpMindSalesForPolling
...
```mermaid
flowchart LR
    VIEW["dbo.vwPOSJumpMindSalesForPolling"]
...
```sql
create view vwPOSJumpMindSalesForPolling
---this includes donations and giftcards, so is not about gaap
as
...
--			sum((d.actual_unit_price * cast((abs (d.quantity)) as int))) as net_sales,
		...
		and h.trans_type in ('SALE', 'RETURN', 'REDEEM')
