...
```sql
--select sku_style,sku_display from [vwBO_Product_Dim]
CREATE view [dbo].[vwBO_Product_Dim]
as
/*
select product_group, department_display,department_key, department_code, class_display, class_key, subclass_display,
subclass_key, subclass_code, sku_key, sku_display, sku
from vwBO_Product_Dim
where Department like 'Sp%rts%'
group by 
product_group, department_display,department_key, department_code, class_display, class_key, subclass_display,
subclass_key, subclass_code, sku_key, sku_display, sku
order by 
product_group, department_display,department_key, department_code, class_display, class_key, subclass_display,
subclass_key, subclass_code, sku_key, sku_display, sku
...
select max(len(sku_display)), max(len(class_display)), max(len(subclass_display)),
max(len(sku_key)), max(len(class_key)), max(len(subclass_key)) 
 from vwBO_Product_Dim2
...
,CASE WHEN Department like 'Sp%rts%' then product_group + ' ' + department + ' ' + department_code-- as department_key 
	...
	, department_code + ' ' + class as class_key
	, subclass + ' ' + subclass_code as subclass_key
	, cast(sku as varchar(50)) + ' - ' + [product_desc] as sku_key
,CASE WHEN Department like 'Sp%rts%' then product_group + ' ' + department + ' ' + department_code-- as department_key 
	 ELSE department + ' ' + department_code  END as department_display 
,CASE WHEN Department like 'Sp%rts%' then department_code + ' ' + class  + ' (Sprts Lic ' + product_group + ')'
	ELSE  department_code + ' ' + class END as class_display
,CASE WHEN Department like 'Sp%rts%' then  subclass + ' ' + subclass_code + ' (Sprts Lic ' + product_group + ')'
	ELSE subclass + ' ' + subclass_code END as subclass_display
,CASE WHEN Department like 'Sp%rts%' then cast(sku as varchar(50)) + ' - ' + [product_desc] + ' (Sprts Lic ' + product_group + ')'
ELSE cast(sku as varchar(50)) + ' - ' + [product_desc] END as sku_display
	, *
	from (
