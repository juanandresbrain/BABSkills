...
```mermaid
flowchart LR
    VIEW["dbo.vwDW_SFSCube_Transactions"]
    ...
    dbo_vwDW_SFSCube_Transactions_Base(["dbo.vwDW_SFSCube_Transactions_Base"]) --> VIEW
```
...
```sql
CREATE VIEW [dbo].[vwDW_SFSCube_Transactions]
AS SELECT
       BASE.currency_key
      ,BASE.transaction_id
      ,BASE.store_key
      ,BASE.CLNSD_GST_ID
      ,BASE.date_key
      ,BASE.MerchandiseUnits AS Units
      ,BASE.UnitDiscAmount
      ,BASE.UnitGrossAmount
      ...
      ,BASE.GaapSales
      ,BASE.numGAAPTrans
      ,BASE.numGiftCardsTrans
      ,BASE.numItems
      ,BASE.transaction_type_key
      ,BASE.[12MoVisit]
      ,BASE.[24MoVisit]
      ,BASE.Animal_UGA
      ,BASE.Non_Animal_UGA
      ,BASE.Footwear_UGA
      ,BASE.Accessories_UGA
      ,BASE.Sounds_UGA
      ,BASE.Clothing_UGA
      ,BASE.Other_UGA
      ,BASE.MerchandiseUga
      ,BASE.GiftCardsSoldUga
      ,BASE.GiftCardDiscount
      ,BASE.DonationsUga
      ,BASE.StuffingAndSuppliesUGA
      ,BASE.AnimalUnits
      ,BASE.ShoeUnits
      ,BASE.SoundUnits
	  ...
	  ,BASE.psyte_clus_id
	  ,CASE WHEN BASE.dstnc_to_str_qty >= 0 THEN BASE.dstnc_to_str_qty ELSE 0 END AS dstnc_to_str_qty
	  ,CAST(CASE
	  ...
	  ,BASE.isSFSHousehold
   FROM
       dbo.vwDW_SFSCube_Transactions_Base AS BASE
   LEFT OUTER JOIN dbo.vwDW_SFSCube_SinceLastTransaction AS SLT
   ON  BASE.DaysSinceLastTransaction BETWEEN SLT.minDays
       AND SLT.maxDays
   INNER JOIN dbo.vwDW_SFSCube_GuestSegment AS Y1GS
   ON  BASE.[12MoVisit] BETWEEN Y1GS.minVisits
       AND Y1GS.maxVisits
   INNER JOIN dbo.vwDW_SFSCube_GuestSegment AS Y2GS
   ...
   ON  BASE.lifetimeVisitNumber BETWEEN LTGS.minVisits
       AND LTGS.maxVisits
   INNER JOIN dbo.vwDW_SFSCube_GuestSegment AS GST_Y1GS
   ON  BASE.[GST_12MoVisit] BETWEEN GST_Y1GS.minVisits
       AND GST_Y1GS.maxVisits
   INNER JOIN dbo.vwDW_SFSCube_GuestSegment AS GST_Y2GS
   ON  BASE.[GST_24MoVisit] BETWEEN GST_Y2GS.minVisits
       AND GST_Y2GS.maxVisits
   INNER JOIN dbo.vwDW_SFSCube_GuestSegment AS GST_LTGS
   ON  BASE.GST_lifetimeVisitNumber BETWEEN GST_LTGS.minVisits
