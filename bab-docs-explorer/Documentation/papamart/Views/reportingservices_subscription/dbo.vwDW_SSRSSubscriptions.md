...
```mermaid
flowchart LR
    VIEW["dbo.vwDW_SSRSSubscriptions"]
...
Gary Murrish	2/27/2013		Changed the Title for the Top UK Stores Reports
...
CREATE VIEW [dbo].[vwDW_SSRSSubscriptions]
AS
...
UNION
		...
		--Top7OpsScorecard for Top 7 UK Stores in GBP in Excel Format (Sunday schedule)
			...
			,'Company Level' as Breakout
			,'EXCEL' as Format
			,'Top7OpsScorecard (Top UK Stores) (GBP) (Excel) was executed at @ExecutionTime' as [Subject]
			,'Previous' as [Week]
			FROM dbo.vwDW_WeekEnding d
			where [Week] = 'Previous'
...
UNION
		...
		--Top7StoreByStore for Top 7 UK Stores in GBP in Excel Format (Sunday schedule)
			...
			,'Week' as ReportingLevel
			,'EXCEL' as Format
			,'Top Store By Store (Fiscal Week) (Top UK Stores) (GBP) (Excel) was executed at @ExecutionTime' as [Subject]
			,'Previous' as [Week]
			FROM dbo.vwDW_WeekEnding d 
			where [Week] = 'Previous'
