# dbo.vwWebOrderInboundDemandTrackingStageForAzureV2_TEMP
...
```mermaid
flowchart LR
    VIEW["dbo.vwWebOrderInboundDemandTrackingStageForAzureV2_TEMP"]
    dbo_WebOrderDiscountsStage_TEMP(["dbo.WebOrderDiscountsStage_TEMP"]) --> VIEW
    WebOrderInboundDemandTrackingStageV2_TEMP(["WebOrderInboundDemandTrackingStageV2_TEMP"]) --> VIEW
```
...
| Referenced Table |
|---|
| dbo.WebOrderDiscountsStage_TEMP |
| WebOrderInboundDemandTrackingStageV2_TEMP |

## View Code

```sql
CREATE view [dbo].[vwWebOrderInboundDemandTrackingStageForAzureV2_TEMP]
...
--		and exists (select OrderNumber from WebOrderInboundDemandTrackingStageV2_TEMP s where s.OrderNumber=f.OrderNumber and s.Channel is NULL)
--		group by 
--			f.OrderNumber
--	),
hasGift as 
	(
		select OrderNumber
		from WebOrderInboundDemandTrackingStageV2_TEMP 
		where isGiftBox=1
		or hasGiftMessage=1
...
PreStage1 as
	(
		...
		from WebOrderInboundDemandTrackingStageV2_TEMP d
...
select 
	...
	d.InsertDate
from PreStage2 d
left join DWStaging.dbo.WebOrderDiscountsStage_TEMP od 
	on d.OrderNumber=od.OrderNumber 
	and d.DeckSKU=od.SKU
