# Job: zRetired_sysutility_mi_collect_and_upload

**Enabled:** No  
**Server:** papamart  
**Description:** Collect configuration and performance information  
...
flowchart LR
    JOB["zRetired_sysutility_mi_collect_and_upload"]
    JOB --> S1["Step 1: Record batch start time [TSQL]"]
    S1 --> S2["Step 2: Stage Data Collected from PowerShell Script [PowerShell]"]
    S2 --> S3["Step 3: Upload to Utility Control Point [TSQL]"]
```

## Steps

### Step 1: Record batch start time
**Subsystem:** TSQL  
...
### Step 2: Stage Data Collected from PowerShell Script
...
### Step 3: Upload to Utility Control Point
**Subsystem:** TSQL  

```sql
EXEC [msdb].[dbo].[sp_sysutility_mi_upload]
```
