# dbo.fnGetNearestFutureStoreFromLatLonZip
...
```mermaid
flowchart LR
    FUNC["dbo.fnGetNearestFutureStoreFromLatLonZip"]
...
```sql
CREATE function [dbo].[fnGetNearestFutureStoreFromLatLonZip](
	@Lat float(15), @Lon float(15), @Date datetime, @Zip varchar(10))
returns float
AS
BEGIN
declare @futurestoreKEY int
...
SET @futureDate = (dateadd(mm,6,@Date))

-- US 
IF ISNUMERIC(Substring(@Zip,1,3))=1 --US Zip
BEGIN
	--future store--
	select @futurestoreKEY =
	...
	where s.store_id > 0 and s.store_id < 400 and s.store_id not in (0, 8, 17, 13, 136, 155, 179, 180, 209, 212, 242, 272, 1513)
		and (s.bearea not in ('US-Corporate','Canada-Corporate') and s.bearea is not NULL)
		and s.Opening_Date <= @futureDate
		and (s.Closing_date > @futureDate or s.Closing_date is NULL)
...
ELSE 
BEGIN
	set @futurestoreKEY = null
END
	
RETURN @futurestoreKEY
END
...
IF ISNUMERIC(Substring(@Zip,1,3))=1 --US Zip
  BEGIN
	...
	(select top 1 s.store_key
	from store_dim s 
	where (s.bearea not in ('US-Corporate','Canada-Corporate') and s.bearea is not NULL)
--s.store_id > 0 and s.store_id < 2000 and s.bearritory not in ('US-Corporate','Canada-Corporate')
		and s.Opening_Date <= @futureDate
		and (s.Closing_date > @futureDate or s.Closing_date is NULL)
		and store_id in (select istore from tblziptop where szip=@Zip)
	order by dw.dbo.fnCalcDistance(@Lat, @Lon, s.latitude, s.longitude) asc
...
dbo,fnRemoveASCIIChar_test20110412,SQL_SCALAR_FUNCTION,-- =============================================
-- Author:		<Author,,Name>
-- Create date: <Create Date, ,>
-- Description:	<Description, ,>
-- =============================================
CREATE FUNCTION [dbo].[fnRemoveASCIIChar_test20110412] (
