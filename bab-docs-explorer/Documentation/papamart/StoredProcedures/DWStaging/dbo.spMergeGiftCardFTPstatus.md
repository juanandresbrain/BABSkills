...
```mermaid
flowchart LR
    SP["dbo.spMergeGiftCardFTPstatus"]
    dbo_GiftCard_FTP_Status(["dbo.GiftCard_FTP_Status"]) --> SP
    dbo_GiftCard_FTP_Status_Stage(["dbo.GiftCard_FTP_Status_Stage"]) --> SP
```
...
```sql
CREATE proc [dbo].[spMergeGiftCardFTPstatus]
...
using DWStaging.dbo.GiftCard_FTP_Status_Stage as source 
on 
	(
