...
```mermaid
flowchart LR
    SP["dbo.spMergeGiftCardDetail"]
    dbo_GiftCard_Detail(["dbo.GiftCard_Detail"]) --> SP
    dbo_GiftCard_Detail_Stage(["dbo.GiftCard_Detail_Stage"]) --> SP
```
...
```sql
CREATE proc [dbo].[spMergeGiftCardDetail]
...
using DWStaging.dbo.GiftCard_Detail_Stage as source 
on 
	(
