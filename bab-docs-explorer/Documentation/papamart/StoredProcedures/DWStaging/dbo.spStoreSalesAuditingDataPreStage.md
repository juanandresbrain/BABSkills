# dbo.spStoreSalesAuditingDataPreStage
...
```mermaid
flowchart LR
    SP["dbo.spStoreSalesAuditingDataPreStage"]
...
```sql
CREATE proc [dbo].[spStoreSalesAuditingDataPreStage]
@StoreID int,
@StoreKey int,
@IP varchar(15),
@UName varchar(2),
...
Select @StoreQuery = 
						...
						set @Range = Char(34) + (select cast(convert(varchar, getdate()-45,121)as varchar(10)))+  Char(34)  + '''','''' +  Char(34)  +(select cast(convert(varchar, getdate(),121)as varchar(10))) +  Char(34) 
