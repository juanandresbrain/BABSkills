...
```mermaid
flowchart LR
    SP["dbo.spMergeGiftCardDetail_IDW"]
...
```sql
CREATE proc [dbo].[spMergeGiftCardDetail_IDW]
...
merge into DW.dbo.GiftCard_Detail as target
using --DWStaging.dbo.GiftCard_Detail_Stage 
(
select * from papamarttest.dw.dbo.GiftCard_Detail where FileID = 7372
)
as source 
on 
	(
