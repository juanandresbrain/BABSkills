# dbo.spFlashTrafficStoreUpload5
...
```mermaid
flowchart LR
    SP["dbo.spFlashTrafficStoreUpload5"]
...
```sql
CREATE proc [dbo].[spFlashTrafficStoreUpload5]
@StoreID int,
@StoreKey int,
@IP varchar(15)
...
begin try
	exec(@CreateTables) at [StoreServer5]
end try
...
begin
	begin try
		insert into StoreServer5.USICOAL.dbo.FlashTrafficStage
	...
	begin try
		exec(@MergeData) at [StoreServer5]
	end try
