# dbo.spFlashTrafficStoreUpload1
...
```mermaid
flowchart LR
    SP["dbo.spFlashTrafficStoreUpload1"]
...
```sql
CREATE proc [dbo].[spFlashTrafficStoreUpload1]
@StoreID int,
@StoreKey int,
@IP varchar(15)
...
begin try
	exec(@CreateTables) at [StoreServer1]
end try
...
begin
	...
	begin try
		insert into StoreServer1.USICOAL.dbo.FlashTrafficStage
	...
	begin try
		exec(@MergeData) at [StoreServer1]
	end try
