# dbo.spFlashTrafficStoreUpload6
...
```mermaid
flowchart LR
    SP["dbo.spFlashTrafficStoreUpload6"]
...
```sql
CREATE proc [dbo].[spFlashTrafficStoreUpload6]
@StoreID int,
@StoreKey int,
@IP varchar(15)
...
begin try
	exec(@CreateTables) at [StoreServer6]
end try
...
begin
	begin try
		insert into StoreServer6.USICOAL.dbo.FlashTrafficStage
	...
	begin try
		exec(@MergeData) at [StoreServer6]
	end try
