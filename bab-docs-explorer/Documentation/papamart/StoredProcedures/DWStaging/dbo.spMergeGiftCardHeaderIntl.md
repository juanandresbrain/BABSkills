...
```mermaid
flowchart LR
    SP["dbo.spMergeGiftCardHeaderIntl"]
    dbo_GiftCard_Header_International(["dbo.GiftCard_Header_International"]) --> SP
    dbo_GiftCard_Header_International_Stage(["dbo.GiftCard_Header_International_Stage"]) --> SP
```
...
```sql
CREATE proc [dbo].[spMergeGiftCardHeaderIntl]
...
merge into DW.dbo.GiftCard_Header_International as target
using DWStaging.dbo.GiftCard_Header_International_Stage as source 
on 
	(
...
When Matched and
	(		
		  isnull(target.[GroupCode],'x')<>isnull(source.[GroupCode],'x') or
