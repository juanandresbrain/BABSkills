# dbo.spMergeDiscountFactsPromotionStudioDynamics
...
```mermaid
flowchart LR
    SP["dbo.spMergeDiscountFactsPromotionStudioDynamics"]
    dbo_CouponDimPromotionStudio(["dbo.CouponDimPromotionStudio"]) --> SP
    dbo_date_dim(["dbo.date_dim"]) --> SP
    dbo_DiscountFactsPromotionStudioDynamics(["dbo.DiscountFactsPromotionStudioDynamics"]) --> SP
...
```sql
CREATE proc [dbo].[spMergeDiscountFactsPromotionStudioDynamics] -- Update to Proper Name 
...
merge into dw.[dbo].[DiscountFactsPromotionStudioDynamics]as target
...
join dw.dbo.TransactionFactsDynamics tf (nolock) on tf.register_no = s.registernumber and tf.transaction_no = s.transactionnumber  and sd.store_key = tf.store_key and  dd.date_key = tf.date_key
