# dbo.spMergeJMCslsRetailLineItem
...
```mermaid
flowchart LR
    SP["dbo.spMergeJMCslsRetailLineItem"]
    dbo_JMC_sls_retail_line_item(["dbo.JMC_sls_retail_line_item"]) --> SP
    dbo_JMC_sls_retail_line_item_stage(["dbo.JMC_sls_retail_line_item_stage"]) --> SP
...
| dbo.JMC_sls_retail_line_item_stage |
| dbo.sp_send_dbmail |

## Stored Procedure Code

```sql
CREATE proc [dbo].[spMergeJMCslsRetailLineItem] 
...
merge into dw.dbo.JMC_sls_retail_line_item as target
--using DWStaging.dbo.JMC_sls_retail_line_item_stage source 
using
(
SELECT distinct replace([device_id], '-customerdisplay','') as [device_id]
      ,[business_date],[sequence_number],[line_sequence_number],[pos_item_id],[item_id],[item_description],[item_type],[regular_unit_price],[actual_unit_price]
      ,[loyalty_unit_price],[quantity],[extended_amount],[discount_amount],[extended_discounted_amount],[rtn_extended_discounted_amount],[tax_amount],[reason_code_group_id],[reason_code]
      ,[disposition_code],[gift_receipt],[item_returnable],[item_taxable],[quantity_avail_for_return],[item_discountable],[employee_discount_allowed],[item_price_overridable]
      ,[discount_applied],[damage_discount_applied],[tax_included_in_price],[tax_group_id],[orig_line_sequence_number],[orig_sequence_number],[orig_business_date],[orig_device_id]
      ,[orig_order_id],[orig_username],[orig_business_unit_id],[return_policy_id],[item_returned],[iso_currency_code],[tare_weight],[item_weight],[item_weight_plus_tare]
      ,[weight_unit_of_measure],[weight_entry_method_code],[family_code],[item_length],[length_unit_of_measure],[quantity_modifiable],[save_value],[save_value_type],[coupon_allowed]
      ,[eletronic_coupon_allowed],[coupon_multiply_allowed],[username],[external_system_id],[product_id],[item_name],[item_long_description],[additional_classifiers]
      ,[order_line_number],[order_id],[line_item_type],[inquiry_method_code],[voided],[override_user_id],[entry_method_code],[create_time],[create_by],[last_update_time]
      ,[last_update_by],[stuff_info],[find_a_bear_id],[serialized_coupon_barcode],[classifier_class],[classifier_style],[classifier_brand],[classifier_department]
  FROM [dbo].[JMC_sls_retail_line_item_stage]
  ) as source
...
When Matched and
	(		
     ...
     --isnull(target.[line_sequence_number],0)<>isnull(source.[line_sequence_number],0)
     -- or
      ...
      or
	  isnull(target.[order_line_number],0)<>isnull(source.[order_line_number],0)
      or
	  isnull(target.[order_id],'x')<>isnull(source.[order_id],'x')
      or
	  isnull(target.[line_item_type],'x')<>isnull(source.[line_item_type],'x')
      or
	  isnull(target.[inquiry_method_code],'x')<>isnull(source.[inquiry_method_code],'x')
      or
      ...
      or
	  isnull(target.[last_update_time],'x')<>isnull(source.[last_update_time],'x')
      or
	  isnull(target.[last_update_by],'x')<>isnull(source.[last_update_by],'x')
      or
	  isnull(target.[stuff_info],'x')<>isnull(source.[stuff_info],'x')
      or
	  isnull(target.[find_a_bear_id],'x')<>isnull(source.[find_a_bear_id],'x')
      or
	  isnull(target.[serialized_coupon_barcode],'x')<>isnull(source.[serialized_coupon_barcode],'x')
      or
	  isnull(target.[classifier_class],'x')<>isnull(source.[classifier_class],'x')
      or
	  isnull(target.[classifier_style],'x')<>isnull(source.[classifier_style],'x')
      or
	  isnull(target.[classifier_brand],'x')<>isnull(source.[classifier_brand],'x')
      or
	  isnull(target.[classifier_department],'x')<>isnull(source.[classifier_department],'x')
...
Then Update
	set     
     ...
     -- target.[line_sequence_number]=source.[line_sequence_number],
      ...
      target.[inquiry_method_code]=source.[inquiry_method_code],
      ...
      target.[stuff_info]=source.[stuff_info],
      target.[find_a_bear_id]=source.[find_a_bear_id],
      target.[serialized_coupon_barcode]=source.[serialized_coupon_barcode],
      target.[classifier_class]=source.[classifier_class],
      target.[classifier_style]=source.[classifier_style],
      target.[classifier_brand]=source.[classifier_brand],
      target.[classifier_department]=source.[classifier_department],
...
Values
	(
	 source.[device_id],
      ...
      source.[order_line_number],
      source.[order_id],
      source.[line_item_type],
      source.[inquiry_method_code],
      source.[voided],
      ...
      source.[stuff_info],
      source.[find_a_bear_id],
      source.[serialized_coupon_barcode],
      source.[classifier_class],
      source.[classifier_style],
      source.[classifier_brand],
      source.[classifier_department],
	 getdate()
...
--===============================================================================================================
			exec msdb.dbo.sp_send_dbmail
				...
				@body = 'The JumpMind to Papamart Merge to JMC_sls_retail_line_item has completed',
				@subject = 'Process Completion Notice: JumpMind-to-Papamart Merge to JMC_sls_retail_line_item has completed',
				@body_format = 'HTML'
--===============================================================================================================
```
