# dbo.spAWImport_115_Build_AW_MAUnitCostBAK20230814
...
```mermaid
flowchart LR
    SP["dbo.spAWImport_115_Build_AW_MAUnitCostBAK20230814"]
...
```sql
create PROCEDURE [dbo].[spAWImport_115_Build_AW_MAUnitCostBAK20230814]
-- =============================================================================================================
...
AS
	...
	-- Drop table AW_MAUnitCost
	TRUNCATE TABLE AW_MAUnitCost
	...
	if (select count(*) from AW_MAUnitCost) = 0 --IF NO DATA, WAIT 10 MINUTES, RELOAD AND CHECK AGAIN
	begin
		...
		INSERT INTO AW_MAUnitCost
			...
			WHERE
				...
				AND (x.sales_total_units <> 0
				OR x.return_units <> 0)
		--if (select count(*) from AW_MAUnitCost) = 0
		--	begin
		--	RAISERROR ('AW_MAUnitCost is empty. Is data not in Merch? Try again.',16,1)
		--	end
	end
