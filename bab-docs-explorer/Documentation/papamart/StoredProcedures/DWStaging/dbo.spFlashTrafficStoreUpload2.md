# dbo.spFlashTrafficStoreUpload2
...
```mermaid
flowchart LR
    SP["dbo.spFlashTrafficStoreUpload2"]
...
```sql
CREATE proc [dbo].[spFlashTrafficStoreUpload2]
@StoreID int,
@StoreKey int,
@IP varchar(15)
...
begin try
	exec(@CreateTables) at [StoreServer2]
end try
...
begin
	begin try
		insert into StoreServer2.USICOAL.dbo.FlashTrafficStage
	...
	begin try
		exec(@MergeData) at [StoreServer2]
	end try
