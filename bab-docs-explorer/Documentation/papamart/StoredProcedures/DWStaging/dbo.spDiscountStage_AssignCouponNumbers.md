# dbo.spDiscountStage_AssignCouponNumbers
...
```mermaid
flowchart LR
    SP["dbo.spDiscountStage_AssignCouponNumbers"]
    dbo_coupon_dim(["dbo.coupon_dim"]) --> SP
    dbo_deal(["dbo.deal"]) --> SP
    dbo_deal_discount(["dbo.deal_discount"]) --> SP
    df(["df"]) --> SP
    ...
    dbo_vwPOSOutbound_SalePrices_V2_IncludesExpired(["dbo.vwPOSOutbound_SalePrices_V2_IncludesExpired"]) --> SP
```
...
| dbo.vwPOSOutbound_SalePrices_V2_IncludesExpired |
...
--	Tim Callahan	2023-11-16		Added union to #AptosDeal build to get Manual Jumpmind POS Black Friday Promotion 
--	Tim Callahan	2024-01-04		Added union to #AptosDeal build to get Manual Jumpmind  Promotion 
--	Tim Callahan	2024-09-09		Added Birthday Bundle Deals to #AptosDeal Build 
-- =============================================================================================================

CREATE PROCEDURE [dbo].[spDiscountStage_AssignCouponNumbers]
AS
...
--==============================================================================
	...
	union
	-- Replaced with vwPOSOutbound_SalePrices_V2_IncludesExpired on 12/10/2024
	-- The previous view was not including completed price change documents which would cause the coupon key to be 0 after the effective end date had come to pass 
		select
		deal_discount_id,
		deal_no,
		DealStartDate,
	...
	from bedrockdb02.me_01.dbo.vwPOSOutbound_SalePrices_V2_IncludesExpired sp
	group by 
		deal_discount_id,
		deal_no,
		DealStartDate,
		DealEndDate
	--11/16/2023
	-- Added Union To get capture a manual JM POS Black Friday Promotion
	union 
	select 
	cast (concat (d.deal_id, '0001') as int)  as deal_discount_id, 
	d.deal_no, 
	d.effective_from_date as DealStartDate, 
	d.effective_to_date as DealEndDate
	from bedrockdb02.me_01.dbo.deal d
	left join bedrockdb02.me_01.dbo.deal_discount dd on dd.deal_id=d.deal_id	
	where 1=1
	and d.deal_no in ('011997','011998','011999','012000') -- Black Friday 2023 BOGO for $10 - $12 Promo
	group by 
	concat (d.deal_id,'0001'), 
	deal_no, 
	d.effective_from_date, 
	d.effective_to_date
	--1/4/2024
	-- Added Union To get capture a manual JM POS  Promotion Efective Jan 11 2024
	-- Beginning 02/09/2024 -- using this last union query for all Manual Documents 
	-- Added Birthday Bundles Deals on 2/9/2024 - '012054','012055','012056','012057'
	-- Added Emp 50% off on 2/28/2024 - '012067','012068','012069','012070'
	-- Added Girl Scout Rewards - New Party Package on 3/5/2024 - '012071'
	-- Added IT - TSYL 20% OFF - TEST on 7/2/2024 = '012115'
	-- Added IT - TSYL 20% OFF - All Locations on 7/17/2024 '012116','012117','012118','012119'
	-- Added IT - 10% W/APRON  on 7/31/2024
	-- Added IT - 10% W/NFL ITEM - US on 8/29/2024
	-- Added IT - US $12 FF BOGO and IT - US $14 FF BOGO on 9/18/2024
	-- Added IT - US STORY TIME and IT - CA STORY TIME on 9/30/2024
	union 
	select 
	cast (concat (d.deal_id, '0001') as int)  as deal_discount_id, 
	d.deal_no, 
	...
	left join bedrockdb02.me_01.dbo.deal_discount dd on dd.deal_id=d.deal_id	
	where 1=1
	and d.deal_no  in ('012030','012031','012032','012033','012054','012055','012056','012057','012067','012068','012069','012070','012071','012076','012077','012078','012079','012115','012116','012117','012118','012119','012111','012112','012113','012114','012128','012129','012130','012131','012141','012142','012143','012153','012154','012175','012176','012177','012178') -- Add Manual Aptos Documents Here 
