# dbo.spFlashTrafficStoreUpload4
...
```mermaid
flowchart LR
    SP["dbo.spFlashTrafficStoreUpload4"]
...
```sql
CREATE proc [dbo].[spFlashTrafficStoreUpload4]
@StoreID int,
@StoreKey int,
@IP varchar(15)
...
begin try
	exec(@CreateTables) at [StoreServer4]
end try
...
begin
	begin try
		insert into StoreServer4.USICOAL.dbo.FlashTrafficStage
	...
	begin try
		exec(@MergeData) at [StoreServer4]
	end try
