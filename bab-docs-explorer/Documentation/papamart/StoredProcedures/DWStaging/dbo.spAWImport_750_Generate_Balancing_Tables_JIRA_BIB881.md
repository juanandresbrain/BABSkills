# dbo.spAWImport_750_Generate_Balancing_Tables_JIRA_BIB881
...
```mermaid
flowchart LR
    SP["dbo.spAWImport_750_Generate_Balancing_Tables_JIRA_BIB881"]
...
```sql
CREATE PROCEDURE [dbo].[spAWImport_750_Generate_Balancing_Tables_JIRA_BIB881]
AS
-- =============================================================================================================
...
--		Tim Callahan	2024-02-23		Replaced Balancing_SourceVAT Calculation - We do not want to compare to the LO 1150 (Sum Trans Vat) but rather the aggregate of the VAT line notes
--										In addition had to join to product dim to exclude bag fees from VAT calculation 
--		Tim Callahan	2024-04-04		Pre Deployment Check In - Changes Related to Bag Fees as related to JIRA BIB881
-- =============================================================================================================
...
(
	SUM(
		(
			(	
			case 
				when pd.style_code is not null 
					and right(pd.subclass_code,8) = ('57-01-01') -- Exclude Bag Fee Items
					and pd.style_code = '098088'
					and h.store_no in ('31','47','88','100','110','207','210','216','309','332','363','384','417','476')
				then 0
				when  
				...
				end -- New Logic as of 4/4/2024 as related to JIRA BIB881
				- 
...
-- Replaced on 2/23/2024
	...
	SELECT
		...
		h.transaction_id,
		SUM(( 			
					  cast (ln.line_note as numeric(14,4)) *
						case 
							when pd.style_code is not null 
								and right(pd.subclass_code,8) = ('57-01-01') -- Exclude Bag Fee Items
								and pd.style_code = '098088'
								and h.store_no in ('31','47','88','100','110','207','210','216','309','332','363','384','417','476')
							then 0 -- New Logic as of 4/4/2024 as related to JIRA BIB881
							when  
								pd.style_code is not null 
								and right(pd.subclass_code,8) = ('57-01-01') -- Exclude Bag Fee Items
								and pd.style_code <> '098088'
							then 0 -- New Logic as of 4/4/2024 as related to JIRA BIB881
							when [Line_action] in (1,11,95) 
								THEN -1 -- Sold, Charged, Charged On Order Line Actions
							when [Line_action] in (2,12) 							
								 THEN 1 -- Returned, Refunded Line Actions
