# dbo.spPartyFactsMergeFromStaging
...
```mermaid
flowchart LR
    SP["dbo.spPartyFactsMergeFromStaging"]
    dbo_Party_Facts(["dbo.Party_Facts"]) --> SP
    dbo_PartyFacts_Staging(["dbo.PartyFacts_Staging"]) --> SP
```
...
```sql
-- =============================================================================================================
-- Name: spPartyFactsMergeFromStaging
--
-- Description:	
--		pull current party data from staging into dw facts
--
--
-- Output: 
--		data will be loaded into dw.dbo.Party_Facts
--
-- Dependencies: 
--
-- EXAMPLE:
--		exec dw.dbo.spPartyFactsMergeFromStaging
--
-- Revision History
--		Name:			Date:			Comments:
--		Tim Bytnar		3/20/2018		created	
--		Dan Tweedie		2019-05-24		Added ThemeName
-- =============================================================================================================
CREATE PROCEDURE [dbo].[spPartyFactsMergeFromStaging]
AS
BEGIN

	SET NOCOUNT ON;

	MERGE dw.dbo.Party_Facts t
	Using DWStaging.dbo.PartyFacts_Staging s
	ON  t.PartyID = s.PartyID
	WHEN MATCHED
		AND (
					ISNULL(t.OccasionName,'') <> ISNULL(s.OccasionName,'')
				 OR ISNULL(t.PackageName,'') <> ISNULL(s.PackageName,'')
				 OR ISNULL(t.TotalGuests,'') <> ISNULL(s.TotalGuests,'')
				 OR ISNULL(t.GOHAge,'') <> ISNULL(s.GOHAge,'')
				 OR ISNULL(t.GuestAvgAge,'') <> ISNULL(s.GuestAvgAge,'')
				 OR ISNULL(t.IsCancelled,'') <> ISNULL(s.IsCancelled,'')
				 OR ISNULL(t.IsPOParty,'') <> ISNULL(s.IsPOParty,'')
				 OR ISNULL(t.ExecuteDateKey,'') <> ISNULL(s.ExecuteDateKey,'')
				 OR ISNULL(t.ExecuteTimeKey,'') <> ISNULL(s.ExecuteTimeKey,'')
				 OR ISNULL(t.store_key,'') <> ISNULL(s.store_key,'')
				 OR ISNULL(t.PMRNumber,'') <> ISNULL(s.PMRNumber,'')
				 OR ISNULL(t.ThemeName,'')<>ISNULL(s.ThemeName,'')
			)
	THEN UPDATE
		SET t.OccasionName = s.OccasionName,
			t.PackageName = s.PackageName,
			t.TotalGuests = s.TotalGuests,
			t.GOHAge = s.GOHAge,
			t.GuestAvgAge = s.GuestAvgAge,
			t.IsCancelled = s.IsCancelled,
			t.IsPOParty = s.IsPOParty,
			--The CreatedDate should never change
			t.ExecuteDateKey = s.ExecuteDateKey,
			t.ExecuteTimeKey = s.ExecuteTimeKey,
			--The CreatedBy should never change
			--The BookingMethod should never change
			t.store_key = s.store_key,
			t.UPDT_DT = GETDATE(),
			t.PMRNumber = s.PMRNumber,
...
DOMO,spGetFranchiseeTopStylesandAvailability_PastYear,-- =============================================================================================================
...
-- Revision History
--		Name:			Date:			Comments:
--		Tim Bytnar		02/26/2018		Initial creation
-- =============================================
CREATE PROCEDURE [DOMO].[spGetFranchiseeTopStylesandAvailability_PastYear] 
...
BEGIN
	...
	----------------------------------------------------------------------------------------
	--- This takes about 15 seconds to run on it's own.  I'm keeping it out of the loop
	----------------------------------------------------------------------------------------
	...
	AND dd.actual_date BETWEEN DATEADD(Year, -1, GETDATE()) and @startDate

	----------------------------------------------------------------------------------------
	--- This temp table will be where the loop inserts all of the data
	----------------------------------------------------------------------------------------
	IF (Object_ID('tempdb..##FinalRankedData') IS NOT NULL) DROP TABLE ##FinalRankedData
	CREATE TABLE ##FinalRankedData (
			WeekEndingDate Date,
			FranchiseeCountry varchar(50),
			Dept varchar(50),
			DptRnk int,
			DptRnkPW int,
			CGrpRnk int,
	...
	----------------------------------------------------------------------------------------
	--- This is the list of fiscal weeks we need to run the loop against
	----------------------------------------------------------------------------------------
	DECLARE @FiscalWeeks TABLE (fiscal_year int, fiscal_week int, WeekEnding Date)
	INSERT INTO @FiscalWeeks
	SELECT DISTINCT fiscal_year,
		   fiscal_week,
		   CAST(MAX(actual_date) as Date) as WeekEnding
	FROM date_dim
	WHERE actual_date BETWEEN DATEADD(Year, -1, GETDATE()) and GETDATE()
	GROUP BY fiscal_year, fiscal_week

	----------------------------------------------------------------------------------------
	--- The Loop
	----------------------------------------------------------------------------------------
	WHILE EXISTS (SELECT TOP 1 * FROM @FiscalWeeks)
	BEGIN
		DECLARE @CurrentWeekEnding Date
		SELECT TOP 1 @CurrentWeekEnding = WeekEnding FROM @FiscalWeeks

		----------------------------------------------------------------------------------------
		--- Creating my own ranked date_dim for determining the fiscal periods correctly
		----------------------------------------------------------------------------------------
		...
		where cast(actual_date as date) BETWEEN DATEADD(Year, -1, GETDATE()) AND @CurrentWeekEnding
		and datename(weekday, actual_date) = 'Saturday'
		and fiscal_year IS NOT NULL

		DECLARE	@MerchYearTY int, @MerchYearLY int, @MerchWeek varchar(2), @MerchPeriod varchar(2), @MerchWeekID int, @MerchWeekLW1 int, @MerchWeekLW2 int, @MerchYearLW1 int, @MerchYearLW2 int
		select 
		...
		-- Final join of all the data and insert into master table
		INSERT INTO ##FinalRankedData
		SELECT
				@CurrentWeekEnding as WeekEndingDate,
		...
		DELETE FROM @FiscalWeeks WHERE WeekEnding = @CurrentWeekEnding
	END

	SELECT * FROM ##FinalRankedData
