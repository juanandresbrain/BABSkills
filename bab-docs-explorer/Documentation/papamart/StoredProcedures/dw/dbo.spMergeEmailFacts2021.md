...
```mermaid
flowchart LR
    SP["dbo.spMergeEmailFacts2021"]
    dbo_EmailFact2021(["dbo.EmailFact2021"]) --> SP
...
```sql
CREATE proc [dbo].[spMergeEmailFacts2021]

as 

----------------------------------------------------------------------------------------------------------------
--	Ian Wallace	2021-01-28	Created proc, runs at end of SSIS package to move data from Kodiak.ESPStaging to DW
----------------------------------------------------------------------------------------------------------------
...
merge into dw.dbo.EmailFact2021 as target
Using (
		select 
			ClientID,
			SendID,
...
when matched 
	and
		(
			...
			isnull(target.clickCount,0)<>isnull(source.clickCount,0)
