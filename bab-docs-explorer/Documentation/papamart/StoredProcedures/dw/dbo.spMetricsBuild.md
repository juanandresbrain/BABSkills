...
flowchart LR
    ...
    dbo_work_tmptransrollup(["dbo.work_tmptransrollup"]) --> SP
    dbo_work_tmpTypeSummary(["dbo.work_tmpTypeSummary"]) --> SP
...
```sql
CREATE PROCEDURE [dbo].[spMetricsBuild]
-- =============================================================================================================
-- Name: spMetricsBuild
--
-- Description:	
...
-- Revision History
--		Name:			Date:			Comments:
--		Gary Murrish	11/30/2011		Prefixed work_tmp tables with dbo. so that pm_repo could run it.
--		Gary Murrish	11/4/2010		Changed GAAP Transaction Count (66) to be from vwDW_Transactions
--		Keith Missey	1/4/2010		removed references to transaction_kiosk_facts
-- =============================================================================================================
...
IF (Object_ID('dbo.work_tmptransrollup') IS NOT NULL) DROP TABLE dbo.work_tmptransrollup
...
CREATE  CLUSTERED INDEX IX_TMPTrans on dbo.work_tmptransrollup (store_key, date_key)
CREATE  INDEX IX_TMPTrans_tranID on dbo.work_tmptransrollup (transaction_id, register_no)
--select * from dbo.work_tmptransrollup where giftcardonly_y_n = 1
...
-- add sum_type column
IF (Object_ID('dbo.work_tmpunits') IS NOT NULL) DROP TABLE dbo.work_tmpunits
select u.*, sum_type
...
IF (Object_ID('dbo.work_tmpTypeSummary') IS NOT NULL) DROP TABLE dbo.work_tmpTypeSummary
SELECT  transaction_id,
	store_key,
	date_key,
	register_num,	
...
--select * from dbo.work_tmpTypeSummary ts
...
-- 
--from dbo.work_tmpTypeSummary ts
...
CREATE  INDEX IX_TMPUnits on dbo.work_tmpunits (store_key, date_key)

--select * from dbo.work_tmpunits where transaction_id = 27411280
...
FROM dbo.work_tmptransrollup a 
...
from dbo.work_tmpTypeSummary a
group by a.store_key,
	 a.date_key
...
-- select * from dbo.work_tmpunits where ttlsounds > 0
INSERT INTO [dbo].[metric_facts](metric_dim_key, [store_key], [date_key], [amount])
...
FROM dbo.work_tmptransrollup a
JOIN dbo.work_tmpTypeSummary b ON a.transaction_id = b.transaction_id
where b.barebear = 1
group by a.store_key,
	 a.date_key
...
-- IF (Object_ID('dbo.work_tmpRegRollup') IS NOT NULL) DROP TABLE dbo.work_tmpRegRollup
...
--IF (Object_ID('work_tmpRegRollup') IS NOT NULL) DROP TABLE dbo.work_tmpRegRollup
--select 	tdf.store_key,
--	tdf.date_key,
...
----select * from dbo.work_tmpRegRollup


--/* ===== CREATE  INDEX ON REG ROLLUP TABLE ===== */
--CREATE  INDEX IX_TMPRegRoll on dbo.work_tmpRegRollup (store_key, date_key)
