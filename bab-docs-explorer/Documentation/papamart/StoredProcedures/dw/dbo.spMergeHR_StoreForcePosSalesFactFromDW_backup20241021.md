# dbo.spMergeHR_StoreForcePosSalesFactFromDW_backup20241021
...
```mermaid
flowchart LR
    SP["dbo.spMergeHR_StoreForcePosSalesFactFromDW_backup20241021"]
...
```sql
create proc [dbo].[spMergeHR_StoreForcePosSalesFactFromDW_backup20241021]
@StartDate date,
@EndDate date
...
join product_dim pd on a.ProductNumber=pd.style_code and a.ProductSellingGeography=pd.jurisdiction_code;


declare @CRMProcessed int

select @CRMProcessed = case when cast(max(InsertedDate) as date) = cast(getdate() as date) then 1 else 0 end
...
BEGIN
	...
	;
				...
				left join #PartyCounts pc 
					on ss.StoreCode=pc.StoreCode --ss.LocationCode=pc.StoreCode
				...
				left join #Backpacks bp 
					on ss.StoreCode=bp.StoreCode --ss.LocationCode=pc.StoreCode
				...
				left join dwstaging.dbo.HR_StoreforceCustomerMetricsStage crm
					on ss.StoreCode=crm.StoreCode--ss.LocationCode=crm.StoreCode
					and cast(dd.actual_date as date)=crm.TransactionDateRaw
					and right((cast('00' as varchar) + cast(td.hour as varchar)),2)
						+ ':' + case when td.Minute < 30 then '00' else '30' end = crm.slot
				where (datepart(hh,getdate())>2 and cast(dd.actual_date as date) between @StartDate and @EndDate)
				or (datepart(hh,getdate())<=2 and cast(dd.actual_date as date)=cast(getdate()-1 as date))
				group by 
					ss.StoreCode,
					--ss.LocationCode,
		...
		select 
			...
			sum(SaleValue) SaleValue,
			sum(SaleUnits) SaleUnits
		from HR_StoreForcePosSalesFact
		where StoreCode = '1001'
		and Date = '01/04/2019'

		--raw cube view
		SELECT 
			sum(vw.store_transaction_flag) SaleTran,
			sum(vw.store_sales_amount) SaleValue,
			sum(vw.store_units) SaleUnits
		FROM [dbo].[vwDW_Transactions_Cube_V3] vw with (nolock) 
		join date_dim dd on vw.date_key=dd.date_key
		join store_dim sd on vw.store_key=sd.store_key
		where cast(dd.actual_date as date) = '2019-04-01'
		and sd.store_id = 1
