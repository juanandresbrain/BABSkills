# dbo.spMergeHR_StoreForcePosSalesFactFromDWDynamics
...
```mermaid
flowchart LR
    SP["dbo.spMergeHR_StoreForcePosSalesFactFromDWDynamics"]
...
```sql
CREATE proc [dbo].[spMergeHR_StoreForcePosSalesFactFromDWDynamics]
...
BEGIN
		...
		(
			...
			where (datepart(hh, getdate())>2 and sl.StoreID>0)
					OR (datepart(hh, getdate())<=2 and sl.StoreID<2000) ---EXCLUDES UK WHEN IT RUNS BETWEEN 12AM AND 2AM BECAUSE IT MESSES UP THE SALES TOTAL SOMEHOW FOR UK DURING THIS TIME
		...
		PartyCounts as
			(
				...
				and tf.party_key <> 0
				and (
						(datepart(hh,getdate())>2 and datediff(dd, dd.actual_date,getdate()) =0 )
						or (datepart(hh,getdate())<=2 and cast(dd.actual_date as date)=cast(getdate()-1 as date))
					)
				group by 
					ss.LocationCode,
		...
		Backpacks as
			(
				...
				where 1=1
					and ((datepart(hh,getdate())>2 and datediff(dd, dd.actual_date,getdate()) =0 )
							or (datepart(hh,getdate())<=2 and cast(dd.actual_date as date)=cast(getdate()-1 as date))
						)
