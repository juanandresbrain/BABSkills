# dbo.spCRMLoyaltyProductNoStyleStage
...
-- Author:		Ian Wallace
-- =============================================
CREATE PROCEDURE [dbo].[spCRMLoyaltyProductNoStyleStage]
...
SET NOCOUNT ON;
 ...
 --IF (Object_ID('tempdb..#AttrPivot') IS NOT NULL) DROP TABLE #AttrPivot;
	--	SELECT        
	--		RIGHT(CAST('000000' AS varchar(6)) + CAST(Style_Code AS varchar(6)), 6) AS style_code, 
	--		CASE 
 ...
 --           CASE 
	...
	--IF (Object_ID('tempdb..#MaxAttr') IS NOT NULL) DROP TABLE #MaxAttr;
	--	SELECT        
	--		RIGHT(CAST('000000' AS varchar(6)) + CAST(style_code AS varchar(6)), 6) AS style_code, 
	--		MIN(IDATE) AS IDATE, 
	--		MAX(ODATE) AS ODATE, 
	...
	IF (Object_ID('tempdb..#FilteredKeystories') IS NOT NULL) DROP TABLE #FilteredKeystories;
		SELECT
			CAST(style_code AS varchar(10)) AS Style_Code, 
			MIN(AttributeValue) AS KeyStory
		into #FilteredKeystories
		FROM  dbo.Attribute_Dim
		WHERE  AttributeName = 'KEYSTY'
		GROUP BY style_code


		IF (Object_ID('tempdb..#KeyStories') IS NOT NULL) DROP TABLE #KeyStories;
		SELECT        
			s.style_code, 
      ...
      GROUP BY s.style_code
		...
		--IF (Object_ID('tempdb..#unitCost') IS NOT NULL) DROP TABLE #unitCost;
		--select 
		--	ProductKey,
		...
		--IF (Object_ID('tempdb..#webInventory') IS NOT NULL) DROP TABLE #webInventory;	
		--select 
		--	  StyleCode,
	...
	-- COLUMNS NEEDED FOR LOYALTY ARE:  ProductKey,Style,KeyStory,Chain,Department,LicenseCode, Class, SubClass

	IF (Object_ID('dw..LoyaltyProductsNoStyle') IS NOT NULL) DROP TABLE dw.dbo.LoyaltyProductsNoStyle;
--truncate table dw.dbo.LoyaltyProductS
...
into dw.dbo.LoyaltyProductsNoStyle
...
WHERE pd.style_code IS NULL 
or pd.style_code = 'N/A'
