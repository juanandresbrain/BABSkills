...
```mermaid
flowchart LR
    SP["dbo.spMetricImport_SalesPlan"]
...
```sql
CREATE    PROCEDURE [dbo].[spMetricImport_SalesPlan]
AS
...
from 
	(
	select sd.store_key, dd.date_key,f.currency,ISNULL(f.sales_plan,0) AS sales_plan
	from date_dim dd
...
********************************************************/
insert metric_facts (metric_dim_key,store_key,date_key,amount)
select @MetricDimKey_GAAP, NewD.store_key, NewD.date_key, ISNULL(NewD.sales_plan,0) AS sales_plan
from 
	(
	select sd.store_key, dd.date_key,f.sales_plan
	from date_dim dd
...
********************************************************/
insert metric_facts (metric_dim_key,store_key,date_key,amount)
select @MetricDimKeyCA_GAAP, NewD.store_key, NewD.date_key, ISNULL(NewD.sales_plan,0) AS sales_plan
from 
	(
	select sd.store_key, dd.date_key,f.sales_plan
	from date_dim dd
