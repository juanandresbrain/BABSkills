...
```sql
CREATE proc [dbo].[spMergeCRMde1_V3]
...
using 
	...
	SELECT	 [CustomerNumber],
			 ...
			 isnull([MembershipPlan],'')		as [MembershipPlan],
...
-- separate merge here of just recency columns which will updat enew UpdateDateRecency column  
merge into CRMde1 as target
using 
	(
	SELECT	 [CustomerNumber],
...
then update
	set
		...
		target.[RecencyCount36m]=source.[RecencyCount36m],
		target.[RecencyCountTTL]=source.[RecencyCountTTL],
		target.UpdateDateRecency=getdate()
