# dbo.spMergeCRMLoyaltyVoucherCancelled
...
```mermaid
flowchart LR
    SP["dbo.spMergeCRMLoyaltyVoucherCancelled"]
    dbo_SerializedVoucherCancelled(["dbo.SerializedVoucherCancelled"]) --> SP
...
| Referenced Table |
|---|
| dbo.SerializedVoucherCancelled |
| dbo.SerializedVoucherStage |

## Stored Procedure Code

```sql
CREATE proc [dbo].[spMergeCRMLoyaltyVoucherCancelled]

as

set nocount on

merge into [papamart].[dw].[dbo].[SerializedVoucherCancelled] as target
using (
select distinct * from [papamart].[DWStaging].[dbo].[SerializedVoucherStage] where Status in ('Cancelled','Redeemed')
)
as source
on 
