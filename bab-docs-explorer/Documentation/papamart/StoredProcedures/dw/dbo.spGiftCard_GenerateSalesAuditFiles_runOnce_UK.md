# dbo.spGiftCard_GenerateSalesAuditFiles_runOnce_UK
...
```mermaid
flowchart LR
    SP["dbo.spGiftCard_GenerateSalesAuditFiles_runOnce_UK"]
    gcd(["gcd"]) --> SP
...
```sql
CREATE PROCEDURE [dbo].[spGiftCard_GenerateSalesAuditFiles_runOnce_UK]
AS
...
SELECT FileID INTO #UKFiles FROM dw.dbo.GiftCard_Header_International WHERE FileID = 12555
--SELECT FileID INTO #NAFiles FROM dw.dbo.GiftCard_Header WHERE FileID in (12086)	
