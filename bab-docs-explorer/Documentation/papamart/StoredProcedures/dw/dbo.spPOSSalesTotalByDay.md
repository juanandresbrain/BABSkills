...
```sql
---- =====================================================================================================
---- Name: POSSalesTotalByDay
---- Revision History
----		Name:			Date:			Comments:
----		Tim Callahan	08/15/2023		Replaces View vwPOSSalesTotalByDay
...
CREATE PROCEDURE [dbo].[spPOSSalesTotalByDay]
...
cast (sum (DISTINCT r.quantity) as int)  as Units, ----DISTINCT ADDED
sum (DISTINCT r.extended_amount)  as LineSales  ----DISTINCT ADDED
...
right (r.device_id,2), 
r.sequence_number,
r.line_sequence_number  ----ADDED GROUP BY 
...
select 
	...
	sum(DISTINCT s.subtotal) Sales
	into #Sales 
from JMC_sls_trans s with (nolock) 
where 
	(
		--cast(business_date as date) >='2023-04-12' --first day of new POs
		cast(s.create_time as date) >='2023-10-12' --first day of new POs -- -- Replaced above on 8/14/2023
...
group by
	...
	business_unit_id,
	--s.device_id,
	cast(right(s.device_id,2) as int), 
	s.trans_nbr


CREATE NONCLUSTERED INDEX [NCI_BusDateStoreTran]
ON [dbo].[#Units] ([BusinessDate],[StoreID],[TransactionNumber])
INCLUDE ([RegisterNumber],[Units],[LineSales])
...
truncate table POSSalesTotalByDay

insert into POSSalesTotalByDay
select *
from #Summary1
