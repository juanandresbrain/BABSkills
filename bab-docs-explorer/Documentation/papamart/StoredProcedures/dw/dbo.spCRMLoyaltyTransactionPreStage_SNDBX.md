# dbo.spCRMLoyaltyTransactionPreStage_SNDBX
...
```mermaid
flowchart LR
    SP["dbo.spCRMLoyaltyTransactionPreStage_SNDBX"]
    ...
    dbo_TransactionFactsDynamics(["dbo.TransactionFactsDynamics"]) --> SP
...
```sql
CREATE proc [dbo].[spCRMLoyaltyTransactionPreStage_SNDBX]
@DaysToGoBack int
...
from LoyaltyTransactionFact ctf with (nolock)
...
--join papamarttest.dw.dbo.transaction_detail_facts tdf with (nolock) on ctf.TransactionID=tdf.Transaction_ID
join  papamart.dw.dbo.TransactionDetailFactsDynamics  tdf with (nolock) on ctf.TransactionID=tdf.Transaction_ID
...
--join papamart.dw.dbo.Transaction_Facts tf on ctf.TransactionID=tf.transaction_id   
join papamart.dw.dbo.TransactionFactsDynamics tf on ctf.TransactionID=tf.transaction_id 
...
--and  ctf.transactionID =  '472160175'
--and tf.date_key >@DateKey
and ctf.DateKey >@DateKey
