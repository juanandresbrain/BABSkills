...
```sql
CREATE PROCEDURE [dbo].[sp_VoucherLookup_1_1]
...
BEGIN
	...
	FROM [stl-crmdb-p-01].crm.dbo.customer c WITH(NOLOCK)
	INNER JOIN [stl-crmdb-p-01].crm.dbo.address a WITH(NOLOCK) ON c.customer_id = a.customer_id
	WHERE c.customer_no = '708564303'
