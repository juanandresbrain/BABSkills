# dbo.spEmailSageNewHireNotificationNumeric
...
```mermaid
flowchart LR
    SP["dbo.spEmailSageNewHireNotificationNumeric"]
...
```sql
CREATE proc [dbo].[spEmailSageNewHireNotificationNumeric] 
...
select @Subj = 'Sage New Hire Instructions'
	...
	select @recip = @personalEmail     -- PROD
	--select @recip = 'ianw@buildabear.com'   -- TEST
	--select @hr_recip = 'MiriamB@buildabear.com'
	--set @cc_line = @managerEmail+';'+@hr_recip
	set @cc_line = @managerEmail       -- PROD
	--set @cc_line =  'ianw@buildabear.com'    -- TEST
...
<table><tr><td valign="top" align="left"><font face =arial size = 2 color = "blue" ><B>Employee Number: ' + @EmployeeID + ' </B></td></tr></table> 
...
set @both = @part1 + @part2 + @part3
		...
		exec msdb.dbo.sp_send_dbmail
			...
			@from_address = 'ukhr@buildabear.co.uk',    -- PROD
			--@from_address = 'ianw@buildabear.com',
			@recipients = @recip,
			...
			@blind_copy_recipients ='ian.david.wallace@gmail.com',
