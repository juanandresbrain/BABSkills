# dbo.usp_FlashGAAPSales40_20140724
...
```mermaid
flowchart LR
    SP["dbo.usp_FlashGAAPSales40_20140724"]
...
```sql
CREATE PROC [dbo].[usp_FlashGAAPSales40] 
-- =============================================================================================================
-- Name: usp_FlashGAAPSales
--
-- Description:	This is the 4.0 version with recipients changed and the data export step commented out.
...
--print @copy_recipients
    ...
    FROM    OURSBLANC.auditworks.dbo.transaction_header h
            JOIN OURSBLANC.auditworks.dbo.transaction_line l ON h.transaction_id = l.transaction_id
            JOIN OURSBLANC.auditworks.dbo.sv_store c ON h.store_no = c.store_no
...
select * from tmp_VAT return
    ...
    FROM    OURSBLANC.auditworks.dbo.transaction_header h
            INNER JOIN OURSBLANC.auditworks.dbo.transaction_line l ON h.transaction_id = l.transaction_id
            INNER JOIN OURSBLANC.auditworks.dbo.line_note ln ON l.transaction_id = ln.transaction_id
...
/*
    EXEC msdb.dbo.sp_send_dbmail @recipients = @recipients,
        @copy_recipients = @copy_recipients,
        @subject = '4.0 Flash GAAP Sales from AW',
        @attach_query_result_as_file = 'TRUE',
        @query_result_separator = @resultseparator,
        @query_attachment_filename = '40FlashGAAPSales.csv',
...
--,@query = @query
    ...
    EXEC msdb.dbo.sp_send_dbmail @recipients = @recipients_uk,
        @copy_recipients = @copy_recipients,
        @subject = '4.0 Flash Europe GAAP Sales from AW',
        @attach_query_result_as_file = 'TRUE',
        @query_result_separator = @resultseparator,
        @query_attachment_filename = '40FlashEuropeGAAPSales.csv',
...
/*
    EXEC msdb.dbo.sp_send_dbmail @recipients = @recipients,
        @copy_recipients = @copy_recipients,
        @subject = '4.0 Ridemakerz Flash GAAP Sales from AW',
        @attach_query_result_as_file = 'TRUE',
        @query_result_separator = @resultseparator,
        @query_attachment_filename = '40FlashGAAPSales_RZ.csv',
        @query_result_header = 'TRUE', @query_result_width = 80,
        @body = @str_message, @query = @query
*/

-- validation email for overnight polling consultants
    SELECT  @subject = '4.0 Stores with ZERO sales for ' + @str_salesdate
...
/* comment out this for 5.0 testing
