# dbo.spA360_trans_header_FileOutput
...
```mermaid
flowchart LR
    SP["dbo.spA360_trans_header_FileOutput"]
...
```sql
CREATE proc [dbo].[spA360_trans_header_FileOutput]
	@path varchar(200),
...
Begin 
		...
		if @allRecords = 1
		begin
			SET @selectstmnt = 'SELECT [country],[customerNumber],[purchaseChannel],[purchaseDate]'
			+ ',[purchaseStoreNumber],[purchaseRevenue],[purchaseUnitCount],[transaction_id]'
		...
		begin
				SET @selectstmnt = 'SELECT [country],[customerNumber],[purchaseChannel],[purchaseDate]'
			+ ',[purchaseStoreNumber],[purchaseRevenue],[purchaseUnitCount],[transaction_id]'
			--+ ' FROM [dbo].[A360_trans_header]'
				+ ' from DW.dbo.' + @tablename
			+ ' where cast([purchaseDate] as date) >= cast(getdate()-2 as date)'
