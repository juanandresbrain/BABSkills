# dbo.spEmailAzurePipelineNotification
...
```mermaid
flowchart LR
    SP["dbo.spEmailAzurePipelineNotification"]
...
```sql
CREATE proc [dbo].[spEmailAzurePipelineNotification] 
...
set @Subj = 'Azure Pipeline ' + @varValue + ' notification **TEST**'
...
select @body = 
'Azure Pipeline: ' + @varValue + ' (' + @varValue3 + ')'  + ' on Synapse Workspace: ' + @varValue2 + ' completed' +
    '<br><br>' +
    '<br>
