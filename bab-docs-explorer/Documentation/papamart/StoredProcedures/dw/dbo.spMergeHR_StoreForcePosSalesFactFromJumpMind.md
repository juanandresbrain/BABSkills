# dbo.spMergeHR_StoreForcePosSalesFactFromJumpMind
...
```mermaid
flowchart LR
    SP["dbo.spMergeHR_StoreForcePosSalesFactFromJumpMind"]
...
```sql
CREATE proc [dbo].[spMergeHR_StoreForcePosSalesFactFromJumpMind]
...
select 
	...
	case 
		when h.trans_type in ('SALE','REDEEM') 
			...
			then 
				case 
					when s.ProductSellingGeography in ('UK','IE') 
