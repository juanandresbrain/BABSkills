# dbo.spCRMLoyaltyTransactionPreStage_Staging
...
```mermaid
flowchart LR
    SP["dbo.spCRMLoyaltyTransactionPreStage_Staging"]
    ...
    dbo_tmpLoyaltyTransactionsStage_Staging(["dbo.tmpLoyaltyTransactionsStage_Staging"]) --> SP
...
| Store_Dim |
| dbo.STR_DIM |
| dbo.tmpLoyaltyTransactionsStage_Staging |
...
```sql
CREATE proc [dbo].[spCRMLoyaltyTransactionPreStage_Staging]
--ALTER proc [dbo].[spCRMLoyaltyTransactionPreStage]
...
--      Ian Wallace 2022-12-23  - replaced Gift Card kodiaktest join to new table on DW
--------------------------------------------------------------------------------------------------------------------------------
...
select CustomerNumber, isBonusClubMember
into #customers2
from CRMCustomerDim with (nolock)
--where isBonusClubMember=1
...
declare @DateKey2 int
select @DateKey2=date_key 
from date_dim 
where datediff(dd, actual_date, getdate()-160)=0


--IF (Object_ID('dwstaging..tmpLoyaltyTransactionsStage_Staging') IS NOT NULL) DROP TABLE dwstaging.dbo.tmpLoyaltyTransactionsStage_Staging


--INSERT INTO dwstaging.dbo.tmpLoyaltyTransactionsStage_Staging ([TransactionID],[inDiscountFacts],[LicensedOrNot])
--select TransactionID, 1,1 from CRMTransactionFact ctf with (nolock) where TransactionID = '465070170'
--union 
--select transaction_id as 'TransactionID', 1,1 from  TransactionFact with (nolock) where transaction_id = '458008969'



INSERT INTO dwstaging.dbo.tmpLoyaltyTransactionsStage_Staging ([TransactionID],[transaction_line_seq],[TransactionLineNumber]
,[DiscountReference],[inDiscountFacts],[CustomerNumber],[TransactionYear],[TransactionMonth],[TransactionDate],[StoreConcept],[StoreNumber]
,[Country],[LifetimeVisitNumber],[Units],[PurchaseRevenue],[LineItemPrice],[UnitDiscountAmount],[ParentTransactionSubTotal],[ProductKey],[SKU]
,[KeyStory],[Class],[SubClass],[ConsumerGroup],[Department],[LicensedOrNot],[currency_key],[isPointsEligible],[OrderReference],[EmployeeID])
...
select	
 ...
 --                                                          then (tdf.unit_gross_amount-(tdf.unit_disc_amount - tdf.upsell_disc_allocated) +tdf.vat_tax_amount) else 0 end)
	...
	--, ed.LineObject
--into dwstaging.dbo.tmpLoyaltyTransactionsStage_Staging
...
left join dw.dbo.LoyaltyGCranges gcr on left(tdf.reference_no, 12)  between gcr.GiftCardRangeStart and gcr.GiftCardRangeEnd
...
where 1=1
--and ctf.TransactionID = '466581506'
...
group by
	...
	,ed.LineObject
	 ...
	 -- union to acquire for non ctf transactions 
	...
	tf.transaction_id,
	tdf.transaction_line_seq,
	row_number() over(partition by tf.Transaction_id order by transaction_line_seq) TransactionLineNumber,
	tdf.reference_no DiscountReference,
	case when df.reference_no is null then 0 else 1 end as inDiscountFacts,
	--ctf.CustomerNumber,
	sv.CustomerNumber,
	datepart(yyyy, dd.Actual_Date) TransactionYear,
	datepart(mm, dd.Actual_Date) TransactionMonth,
	--ctf.TransactionDate,
	convert(varchar(10), actual_date, 23) as 'TransactionDate',
	sd.StoreConcept,
 ...
 --                                                          then (tdf.unit_gross_amount-(tdf.unit_disc_amount - tdf.upsell_disc_allocated) +tdf.vat_tax_amount) else 0 end)
	...
	case when cd.isBonusClubMember = 1 then 1 else 0 end  as 'isPointsEligible',
...
from SerializedVoucher sv with (nolock) 
join discount_facts df with (nolock) on cast(df.reference_no as varchar)=cast(sv.SerializedNumber as varchar)
join date_dim dd on df.date_key=dd.date_key 
--left join discount_facts df with (nolock) on tdf.transaction_id=df.transaction_id and tdf.reference_no=df.reference_no
join TransactionFact tf with (nolock) on df.transaction_id=tf.transaction_id
left join #Stores sd with (nolock) on tf.store_key=sd.StoreKey
left join TransactionDetailFact tdf with (nolock) on tf.transaction_id=tdf.Transaction_ID
left join line_object_dim lod on tdf.line_object_key=lod.line_object_key 
left join line_action_dim lad on tdf.line_action_key=lad.line_action_key
left join #Products pd with (nolock) on tdf.product_key=pd.ProductKey
left join #Products pd2 with (nolock) on tdf.reference_no = pd2.Style
join #Customers2 cd on sv.CustomerNumber=cd.CustomerNumber--CRMcustomerDim cd on ctf.CustomerNumber = cd.CustomerNumber 
--left join #GiftCardRanges3 gcr on left(tdf.reference_no, 12)  between gcr.GiftCardRangeStart and gcr.GiftCardRangeEnd
left join dw.dbo.LoyaltyGCranges gcr on left(tdf.reference_no, 12)  between gcr.GiftCardRangeStart and gcr.GiftCardRangeEnd
left join #WebOrders wo on tf.transaction_id=wo.TransactionID
left join #EmpDiscounts ed on tf.transaction_id=ed.TransactionID
left join CRMTransactionFact ctf with (nolock) on df.transaction_id=ctf.CRMTransactionID 

where 1=1
and ctf.TransactionID is null
--and tf.transaction_id = '458674717'

--and cd.isBonusClubMember = 1
--and ctf.TransactionDate >= cast(getdate()-45 as date)

and tf.date_key > @DateKey2
and	(
		( 
			lod.Line_Object IN (100, 102, 103, 104, 115) 
			--AND RIGHT(pd.subclass_code, 8) NOT IN ('57-01-01')  -- These are bag fees, probably do not want to exclude - 3/25/2022
...
group by
	...
	datepart(mm, dd.Actual_Date),
	convert(varchar(10), actual_date, 23),
	sd.StoreConcept,
    sd.StoreNumber,
    sd.CountryNameFull,
	--ctf.LifetimeVisitNumber,
