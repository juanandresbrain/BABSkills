...
```sql
CREATE PROCEDURE [dbo].[spIndexesRemove_SSIS] (
	@username varchar(200),
	@tablename varchar(200),
    @pkIndexes varchar(1) = 'N'
)
 AS

/*
have a conflict, this version has 3 parms and is used by the new ssis routines

the old version has 2 parms and is used by the old etl written by mike torline in informatica
...
--INSERT INTO dwstaging.dbo.tmp_Drew VALUES ('Starting...', getdate())

declare @index varchar(500)
declare @now datetime
set @now = getdate()

--INSERT INTO dwstaging.dbo.tmp_Drew VALUES ('Dropping previous #TMP', getdate())

IF (Object_ID('tempdb..#TMP') IS NOT NULL) DROP TABLE #TMP

--INSERT INTO dwstaging.dbo.tmp_Drew VALUES ('Creating new #TMP', getdate())
...
WHERE I.INDID > 0 
	...
	AND   CASE WHEN @pkIndexes = 'N' THEN INDEXPROPERTY (I.ID,I.NAME,'ISUNIQUE') ELSE 0 END = 0
	AND   CASE WHEN @pkIndexes = 'N' THEN INDEXPROPERTY (I.ID,I.NAME,'ISCLUSTERED') ELSE 0 END = 0

--INSERT INTO dwstaging.dbo.tmp_Drew SELECT '#TMP Count: ' + Convert(varchar, COUNT(1)), getdate() FROM #TMP
...
--INSERT INTO dwstaging.dbo.tmp_Drew VALUES ('Adding iSQL to #TMP', getdate())
...
--INSERT INTO dwstaging.dbo.tmp_Drew VALUES ('Removing trailing comma from COLNAMES', getdate())

  --AT THIS POINT, THE 'COLNAMES' COLUMN HAS A TRAILING COMMA
  UPDATE #TMP SET COLNAMES=LEFT(COLNAMES,LEN(COLNAMES) -1)

--INSERT INTO dwstaging.dbo.tmp_Drew VALUES ('Inserting into IndexSnapshot', getdate())
...
--INSERT INTO dwstaging.dbo.tmp_Drew SELECT 'IndexSnapshot Count: ' + Convert(varchar, COUNT(1)), getdate() FROM IndexSnapshot
...
--INSERT INTO dwstaging.dbo.tmp_Drew VALUES ('Declaring cursor for DROPs', getdate())
...
--INSERT INTO dwstaging.dbo.tmp_Drew VALUES ('Dropping [' + @username + '].[' + @tablename + '].[' + @indexname + ']' , getdate())
...
--INSERT INTO dwstaging.dbo.tmp_Drew VALUES ('Complete...', getdate())
