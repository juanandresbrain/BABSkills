# dbo.spEmail_Report_ESPPeriodicDataV6_edin
...
```mermaid
flowchart LR
    SP["dbo.spEmail_Report_ESPPeriodicDataV6_edin"]
...
```sql
CREATE PROC [dbo].[spEmail_Report_ESPPeriodicDataV6_edin]
-- =============================================================================================================
...
Exec spEmail_Report_ESPPeriodicDataV6_edin @ad_date = @date,  @reload = 1

DECLARE @date datetime
SET @date = CONVERT(VARCHAR, DATEADD(DAY, -10000, GETDATE()), 101)
Exec spEmail_Report_ESPPeriodicDataV6_edin @ad_date = @date,  @reload = 0
*/
-- =============================================================================================================
...
IF (Object_ID('tempdb.dbo.#tmpemail_pet_sfs_1') IS NOT NULL) DROP TABLE #tmpemail_pet_sfs_1
select
	e.email_addr_id,
	c.clnsd_gst_id,
	c.clnsd_addr_id,
...
CREATE INDEX IX_email_addr_id_part1
    ON #tmpemail_pet_sfs_1 (email_addr_id);
    
IF (Object_ID('tempdb.dbo.#tmpemail_pet_sfs_2') IS NOT NULL) DROP TABLE #tmpemail_pet_sfs_2
select
	ep1.*,
	ep.promo_pref AS promo_preference,
	ep.sfscert_pref AS sfscert_preference,
	ep.sfspnts_pref AS sfsacct_preference,
    DATEDIFF(wk, ep.UPDT_DT, GETDATE()) AS email_pref_ch_wkdiff	
into #tmpemail_pet_sfs_2
from #tmpemail_pet_sfs_1 ep1
...
drop table #tmpemail_pet_sfs_1

CREATE INDEX IX_clnsd_addr_id_part2
    ON #tmpemail_pet_sfs_2 (clnsd_addr_id);
    
--add distance to nearest store from addr_sum_fact table
IF (Object_ID('tempdb.dbo.#tmpemail_pet_sfs_3') IS NOT NULL) DROP TABLE #tmpemail_pet_sfs_3
select
	ep2.*,
	af.dstnc_to_str_qty AS dist_to_store
into #tmpemail_pet_sfs_3
from #tmpemail_pet_sfs_2 ep2
	LEFT JOIN dw.dbo.addr_sum_fact af WITH (NOLOCK) ON ep2.clnsd_addr_id = af.clnsd_addr_id

drop table #tmpemail_pet_sfs_2

CREATE INDEX IX_clnsd_addr_id_part3
    ON #tmpemail_pet_sfs_3 (clnsd_addr_id);
    
IF (Object_ID('tempdb.dbo.#tmpemail_pet_sfs_4') IS NOT NULL) DROP TABLE #tmpemail_pet_sfs_4
select
	ep3.*,
	ISNULL(s.store_id, 0) AS nearest_store_no,
    s.store_name AS nearest_store_name
into #tmpemail_pet_sfs_4
from #tmpemail_pet_sfs_3 ep3
	LEFT JOIN dw.dbo.[CLNSD_ADDR_DIM] a WITH ( NOLOCK ) ON ep3.clnsd_addr_id = a.clnsd_addr_id
    LEFT JOIN dw.dbo.[NRST_PSTL_CD_STR_DIM] n WITH ( NOLOCK ) ON a.nrst_str_pstl_cd = n.pstl_cd AND a.[CNTRY_ABBRV] = n.[CNTRY_ABBRV]
    LEFT JOIN dw.dbo.store_dim s WITH ( NOLOCK ) ON s.store_key = n.str_id

drop table #tmpemail_pet_sfs_3

CREATE INDEX IX_clnsd_addr_id_part4
    ON #tmpemail_pet_sfs_4 (clnsd_addr_id);

IF (Object_ID('tempdb.dbo.#tmpemail_pet_sfs_complete') IS NOT NULL) DROP TABLE #tmpemail_pet_sfs_complete
select ep4.*,
	s2.store_id AS first_store_visited
into #tmpemail_pet_sfs_complete
from #tmpemail_pet_sfs_4 ep4
...
select
	...
	'N' AS optinstatus,
	first_store_visited
from #tmpemail_pet_sfs_complete

drop table #tmpemail_pet_sfs_complete
...
*/
IF (Object_ID('tempdb.dbo.#tmpemail_pet_not_sfs_1') IS NOT NULL) DROP TABLE #tmpemail_pet_not_sfs_1
select
	e.email_addr_id,
...
CREATE INDEX IX_email_addr_id_part1
    ON #tmpemail_pet_not_sfs_1 (email_addr_id);

IF (Object_ID('tempdb.dbo.#tmpemail_pet_not_sfs_2') IS NOT NULL) DROP TABLE #tmpemail_pet_not_sfs_2
select
	ep1.*,
	ep.promo_pref AS promo_preference,
	ep.sfscert_pref AS sfscert_preference,
	ep.sfspnts_pref AS sfsacct_preference,
    DATEDIFF(wk, ep.UPDT_DT, GETDATE()) AS email_pref_ch_wkdiff	
into #tmpemail_pet_not_sfs_2
from #tmpemail_pet_not_sfs_1 ep1
	--JOIN dw.dbo.[EMAIL_ADDR_PRSNLZTN_ATTR_DIM] p WITH ( NOLOCK ) ON ep1.[EMAIL_ADDR_ID] = p.[EMAIL_ADDR_ID]
    JOIN dw.dbo.EMAIL_ADDR_PRFRNCE_DIM ep WITH (NOLOCK) ON ep1.EMAIL_ADDR_ID = ep.EMAIL_ADDR_ID

drop table #tmpemail_pet_not_sfs_1

CREATE INDEX IX_clnsd_addr_id_part2
    ON #tmpemail_pet_not_sfs_2 (clnsd_addr_id);
    
--add distance to nearest store from addr_sum_fact table
IF (Object_ID('tempdb.dbo.#tmpemail_pet_not_sfs_3') IS NOT NULL) DROP TABLE #tmpemail_pet_not_sfs_3
select
	ep2.*,
	af.dstnc_to_str_qty AS dist_to_store
into #tmpemail_pet_not_sfs_3
from #tmpemail_pet_not_sfs_2 ep2
	LEFT JOIN dw.dbo.addr_sum_fact af WITH (NOLOCK) ON ep2.clnsd_addr_id = af.clnsd_addr_id

drop table #tmpemail_pet_not_sfs_2

CREATE INDEX IX_clnsd_addr_id_part3
    ON #tmpemail_pet_not_sfs_3 (clnsd_addr_id);

IF (Object_ID('tempdb.dbo.#tmpemail_pet_not_sfs_complete') IS NOT NULL) DROP TABLE #tmpemail_pet_not_sfs_complete
select
	ep3.*,
	ISNULL(s.store_id, 0) AS nearest_store_no,
    s.store_name AS nearest_store_name
into #tmpemail_pet_not_sfs_complete
from #tmpemail_pet_not_sfs_3 ep3
	LEFT JOIN dw.dbo.[CLNSD_ADDR_DIM] a WITH ( NOLOCK ) ON ep3.clnsd_addr_id = a.clnsd_addr_id
    LEFT JOIN dw.dbo.[NRST_PSTL_CD_STR_DIM] n WITH ( NOLOCK ) ON a.nrst_str_pstl_cd = n.pstl_cd AND a.[CNTRY_ABBRV] = n.[CNTRY_ABBRV]
    LEFT JOIN dw.dbo.store_dim s WITH ( NOLOCK ) ON s.store_key = n.str_id

drop table #tmpemail_pet_not_sfs_3
...
select
	...
	'N' AS optinstatus,
	NULL as first_store_visited
from #tmpemail_pet_not_sfs_complete

drop table #tmpemail_pet_not_sfs_complete
