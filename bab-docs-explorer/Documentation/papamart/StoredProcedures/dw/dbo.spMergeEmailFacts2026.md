...
```mermaid
flowchart LR
    SP["dbo.spMergeEmailFacts2026"]
    dbo_EmailFact2026(["dbo.EmailFact2026"]) --> SP
...
```sql
CREATE proc [dbo].[spMergeEmailFacts2026]
...
----------------------------------------------------------------------------------------------------------------
	...
	select 
		...
		--where cast(SendDate as date)  >= '05/18/2026'
		group by 
			ClientID,
			SendID,
			EmailAddress,
...
merge into dw.dbo.EmailFact2026 as target
