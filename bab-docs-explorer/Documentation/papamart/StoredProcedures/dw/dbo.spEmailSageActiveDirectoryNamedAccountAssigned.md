# dbo.spEmailSageActiveDirectoryNamedAccountAssigned
...
```mermaid
flowchart LR
    SP["dbo.spEmailSageActiveDirectoryNamedAccountAssigned"]
...
```sql
CREATE proc [dbo].[spEmailSageActiveDirectoryNamedAccountAssigned] 
...
select @Subj = 'UltiPro to Active Directory Employee account name update'
	...
	SET @recip = 'MiriamB@buildabear.com;MattP@buildabear.com'
	set @copy_recip = @managerEmail
	select @ProvisionText = 'C'


select @body = 
'<font face =arial size = 2><B>Sage to Active Directory store employee account name update staged</B><br>' +
...
'The new Build-A-Bear email address is the samaccountname value plus the @buildabear.com <br> ' +
'</font>' +
	'<table border="1">' +
