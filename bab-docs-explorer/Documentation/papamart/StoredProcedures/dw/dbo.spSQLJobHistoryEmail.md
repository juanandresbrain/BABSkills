...
```sql
CREATE proc [dbo].[spSQLJobHistoryEmail]
			...
			case 
				...
				when name in 
					(
						...
						'AzureProcessing_CRMSurvey',
						'AzureProcessing__CreatePartitions'
...
dbo,spGuestLoad_Generate_DMail_EMail_Statistics_Driver,-- =============================================================================================================
-- Name: spGuestLoad_Generate_DMail_EMail_Statistics_Driver
--
-- Description:	
--		This process is the driver that updates the EMail and DMail batch statistics 
--		on all batches that have not yet been closed.
--
--
-- Input:
--		None
...
--
-- EXAMPLE:
--		exec dw.dbo.spGuestLoad_Generate_DMail_EMail_Statistics_Driver
--
-- Revision History
--		Name:				Date:			Comments:
--		Gary Murrish		2/25/2011		created
-- =============================================================================================================
CREATE PROCEDURE [dbo].[spGuestLoad_Generate_DMail_EMail_Statistics_Driver]
AS
BEGIN

DECLARE @batchID int

DECLARE eMailCursor CURSOR FAST_FORWARD
    FOR SELECT
            BATCH_ID
        FROM
            dbo.GuestLoad_CRM_Update_Batch_Email EM WITH (NOLOCK)
        WHERE
        BATCH_Status >= 90
        AND BATCH_Status < 900

OPEN eMailCursor

FETCH NEXT FROM eMailCursor INTO @batchID
WHILE @@FETCH_STATUS = 0
    BEGIN
  ---
        EXEC dbo.spGuestLoad_Generate_EMail_Statistics @batchID 
  ---
        FETCH NEXT FROM eMailCursor INTO @batchID
    END
...
DECLARE DMailCursor CURSOR FAST_FORWARD
    FOR SELECT
            BATCH_ID
        FROM
            dbo.GuestLoad_CRM_Update_Batch_DMail EM WITH (NOLOCK)
        WHERE
        BATCH_Status >= 90
        AND BATCH_Status < 900
...
FETCH NEXT FROM DMailCursor INTO @batchID
WHILE @@FETCH_STATUS = 0
    BEGIN
  ---
	--PRINT @batchID
        EXEC dbo.spGuestLoad_Generate_DMail_Statistics @batchID 
  ---
        FETCH NEXT FROM DMailCursor INTO @batchID
    END

CLOSE DMailCursor
DEALLOCATE DMailCursor 
