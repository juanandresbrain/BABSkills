...
```mermaid
flowchart LR
    SP["dbo.sp_VoucherLookup_sfs_1_1"]
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_VoucherLookup_sfs_1_1]
...
BEGIN
		...
		LEFT JOIN [kodiak].[DiscountMstrData].[dbo].[SerializationDiscountDetail] sdd (NOLOCK) ON sdd.serializedNum = c.reference_no
		LEFT JOIN [kodiak].[DiscountMstrData].[dbo].[SerializationDiscount] sd  (NOLOCK) ON sdd.serializationID = sd.serializationID
		LEFT JOIN [kodiak].[DiscountMstrData].[dbo].[Discount] d  (NOLOCK) ON  d.discountID = sd.discountID
		...
		WHERE h.store_no = 990 AND (c.reference_type = CAST(@refType AS VARCHAR(2)) OR c.reference_type = 35) AND c.customer_no = CAST(@sfs_Number AS VARCHAR(9)) AND CONVERT(VARCHAR,DATEADD(DAY,-0,GETDATE()),111)  > c.date_issued 
