# dbo.spMergeHR_StoreForcePosSalesFactFromDWDynamics_backup20230501
...
```mermaid
flowchart LR
    SP["dbo.spMergeHR_StoreForcePosSalesFactFromDWDynamics_backup20230501"]
...
```sql
CREATE proc [dbo].[spMergeHR_StoreForcePosSalesFactFromDWDynamics_backup20230501]
		...
		DataStage as
			(
				select 
					...
					right((cast('00' as varchar) + cast(td.hour as varchar)),2)
						+ ':' + case when td.Minute < 30 then '00' else '30' end as Slot,
					sum(tf.store_transaction_flag) as SaleTrans,
					sum(tf.store_sales_amount) as SaleValue,
					sum(cast(tf.store_units as int)) as SaleUnits,
...
dbo,spEmailUltiProToADValidation,-------------------------------------------------------------------------------------------
CREATE proc [dbo].[spEmailUltiProToADValidation]
...
select @Count = count(*)
				...
				where UltiProStatus = 'Active'
				and ADStageDate is not NULL
				and ADInsertDate is NULL
				and StagedProvisionEvent = 'H'
				and datediff(hh, ADStageDate, getdate()) >= 1
if @Count > 0
...
begin
	...
	set @text = '<font face =arial size = 2> ' +
					...
					'The following UltiPro employees were staged for AD but are not found in our ADEmployee table.' + 
					'<BR>' +
					'<BR>'+
					'<B>Total Employees: ' + cast(@Count as varchar) + '</B>' +
					'<BR>' + 
					'<table border="1">' +
					'<tr><th>EmployeeID</th><th>ADStageDate</th>' +
						...
						and StagedProvisionEvent = 'H'
						and datediff(hh, ADStageDate, getdate()) >= 1
						order by ADStageDate, EmployeeID
	...
	@body = @text,
	@subject= 'UltiPro to AD Stage - Employees Still Not in AD??', 
	@body_format = 'HTML'
