...
```sql
CREATE PROC [dbo].[usp_GAAPSales]
-- =============================================================================================================
-- Name: [dbo].[usp_GAAPSales]  
--
-- Description:	
...
-- Revision History
--		Name:			Date:			Comments:
--		Keith Missey	3/30/2008		Merch 4.2 Upgrades
--		Keith Missey	4/29/2008		added line object 296 for customer service
--		Keith Missey	10/31/2008		updated GAAP line objects
--		Keith Missey	12/16/2008		commented out VAT code due to performance
--		Keith Missey	5/18/2009		added line object 102 for virtual world
-- =============================================================================================================
AS 
    SET NOCOUNT ON

    IF ( OBJECT_ID('tempdb..#GAAPSales') IS NOT NULL ) 
        DROP TABLE #GAAPSales
        
    CREATE TABLE [#GAAPSales]
    (
		store_no	INT,
		store_name VARCHAR(50),
		total	NUMERIC(9,2),
		[time of last transaction polled] VARCHAR(50)
    )
    ...
    FROM    ( SELECT    a.store_no,
              ...
              WHERE     ( a.transaction_date BETWEEN CONVERT(CHAR, GETDATE(), 101)
                          ...
                          AND b.line_object IN ( 100, 102, 200, 202, 203, 204, 206,
...
--                        )
    ...
    WHERE   c.gl_company IN ( 100 ) -- US stores only
            AND c.store_status_code IN ( 1, 2, 3, 6 ) -- (1)--future, (2)--non-comparative, (3)--comparative, (6)--open
	--and c.open_date <= getdate() --commented out by Edin on 7/13/2007 so Tina can see new stores with wrong opening dates
...
--            AND gl_company IN ( 100 ) -- US stores only
--            AND store_status_code IN ( 1, 2, 3, 6 )
--    GROUP BY h.store_no
--    ORDER BY h.store_no
