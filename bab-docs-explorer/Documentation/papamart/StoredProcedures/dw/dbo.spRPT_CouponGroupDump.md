...
--                   Countries     = Comma delimited list of countries to extract
--
-- Output: Resultset 
--                   
--
...
--            Gary Murrish  10/16/2014           Changed the category for those Marketing and Distro discounts
--            Gary Murrish  12/11/2014           Changed the reference number for the 'Invalid Coupons' that are not
--                                                                   going to the G/L
--            Gary Murrish  1/26/2015            Added ISNULL to handle coupon problems
--            Gary Murrish  7/13/2015            Added Country to the results
...
--			  Kelly Farrar	2/6/2019			Adjusted logic to use dfint.reference_no not lod.Line_Object_Description for Reference Number
-- =====================================================================================================
CREATE PROCEDURE [dbo].[spRPT_CouponGroupDump]
       @startDate_key int,
       @endDate_key int,
...
BEGIN
       ...
       -- Parse the Countries
       IF OBJECT_ID('tempdb..#tmpCountry') IS NOT NULL
       BEGIN
              DROP TABLE #tmpCountry
       END

       SELECT
              LTRIM(RTRIM(ParsedItem)) AS country
       INTO #tmpCountry
       FROM
              dbo.fn_ParseString(@Countries, ',')
       ...
       FROM
              ...
              INNER JOIN #tmpCountry c WITH (NOLOCK)
              ...
              (SELECT
                           ...
                           CASE
                                  ...
                                  cd.Retail_Pro <= 0 THEN CASE
                                         WHEN cd.categoryTypeID < 0 AND LEN(dfint.reference_no) > 0 THEN dfint.reference_no --lod.Line_Object_Description
                                         ELSE CASE
...
---NEW OUTPUT CODE 11/21/2016 - DAN TWEEDIE
		...
		;with Details as
			(
				SELECT
					...
					MIN(ISNULL(cd.coupon_desc,cs.reference_no)) AS [Coupon Description],
		...
		select 
			...
			count(GAAP_sales_amount) as [No Of Trans],
...
END
       ...
       ---- Now return the result --- NO LONGER USED AS OF 11/21/2016 - DAN TWEEDIE
       --SELECT
