# dbo.spCRMdataExtension1FileOutputPSVbyGroup_ManualFile
...
```mermaid
flowchart LR
    SP["dbo.spCRMdataExtension1FileOutputPSVbyGroup_ManualFile"]
...
```sql
CREATE proc [dbo].[spCRMdataExtension1FileOutputPSVbyGroup_ManualFile]
	@path varchar(200),
...
set @groupNumString = convert(varchar, @groupNum)
		...
		SELECT @columnheaders = @columnheaders + c.name + '|'
		 ...
		 and colid not in (44,45,50,51,52,53,55,56,59)
		 ORDER BY colid
		...
		begin
			...
			--SET @filename = @filepart + @filedate + '_FILE_' + @groupNumString + '.txt'
...
SET @selectstmnt = 'select top 100 [customerNumber],NULLIF([SubscriberKey], '' '') as ''SubscriberKey'',NULLIF([status],'' '') as ''status'',[dateJoined],[LastSentDate],[LastOpenDate],'
			...
			+ '[LifetimePoints],NULLIF([FirstName],'''') as FirstName,NULLIF([LastName],'''') as LastName'
     --+ '  ,case when [LastName] = '' '' then [EmailAddress] when [LastName] is null then [EmailAddress] else [LastName] end as LastName'
	 + '  FROM DW.dbo.tmpCRMmarketingCloudDelta where [groupNum] = ' + @groupNumString
