...
| date_rpt_facts |
| metric_facts |
| dbo.spMetricRollup_52wk_company |
| dbo.vwstore_dim |
...
** 	Returns:	result set
**
**	Examples:	EXEC spMetricRollup_52wk
**			
**
**	History:	
**  Date 		Author 		Purpose
...
******************************************************************************/
CREATE                                           PROCEDURE  spMetricRollup_52wk
/* ===== ARGUMENTS ===== */	
@Bearitory as varchar(100) = null
AS
SET NOCOUNT ON

If @Bearitory is null 
Begin
Exec dbo.spMetricRollup_52wk_company
End
Else 
Begin 
...
SET @wk52TY = @wkCurTY - 52
SET @beginDate = (select min(actual_date) from date_dim where week_id = @wk52TY)

--select @wk52TY,@wkCurTY

--SET @YTDbeginDate = (select min(actual_date) from date_dim where week_id = @wk52TY)

--select * from metric_dim

--select @curDate as endDate,@beginDate as beginDate
...
from date_rpt_facts drf 		
	...
	where dd.week_id > @wk52TY and dd.week_id <= @wkCurTY 
...
where dd.date_key in (select date_key_TY from #date_key_xref)
	...
	and s.store_id <900 and s.bearritory = @Bearitory
...
where dd.date_key in (select date_key_LY from #date_key_xref)
	...
	and s.store_id <900 and s.bearritory = @Bearitory
	and s.opening_date <= getdate()
	and coalesce(s.closing_date,@CurDate+7) >= getdate()---!!!!!!!!!!!!!!!!!!!!1
...
select 		@curDate as endDate
		...
		,sd.bearritory
		,sd.region
		,max(comp_Y_N) as Comp_Y_N
		--,CASE WHEN max(sd.comp_week_id) <= min(a.week_id) THEN 1 /*=Yes*/ ELSE 0 END as Comp_Y_N
		,a.fiscal_week
		,a.fiscal_period
		,CASE   WHEN a.fiscal_period = 1 THEN 'JANUARY' 
			WHEN a.fiscal_period = 2 THEN 'FEBRUARY' 
			WHEN a.fiscal_period = 3 THEN 'MARCH' 
			WHEN a.fiscal_period = 4 THEN 'APRIL' 
			WHEN a.fiscal_period = 5 THEN 'MAY'
			WHEN a.fiscal_period = 6 THEN 'JUNE' 
			WHEN a.fiscal_period = 7 THEN 'JULY' 
			WHEN a.fiscal_period = 8 THEN 'AUGUST'
			WHEN a.fiscal_period = 9 THEN 'SEPTEMBER' 
			WHEN a.fiscal_period = 10 THEN 'OCTOBER' 
			WHEN a.fiscal_period = 11 THEN 'NOVEMBER' 
			WHEN a.fiscal_period = 12 THEN 'DECEMBER'
		END as month_name  
		,a.fiscal_quarter
		,a.fiscal_year
...
dbo,spReloadHasTraffic,-- =============================================
-- Author:		John Eck
-- Create date: 7/12/18
-- Description:	truncAtes and loads has traffic table for the has traffic flag in datawarehouse build
-- =============================================
CREATE PROCEDURE [dbo].[spReloadHasTraffic]

AS 

BEGIN
  truncate table has_traffic

  insert into has_traffic		
		select
			store_key,
			date_key,
			sum(hasTraffic) hasTraffic
		from 
			(
				select 
				...
				FROM
					...
					--where dt_key  between 7855 and 7882
				group by 
					STR_KEY,
					DT_KEY
				UNION
