...
--
-- EXAMPLE:
--		exec dbo.spStoreSalesCheck_Find_SalesDiff 
--
-- Revision History
--		Name:			Date:			Comments:
--		Dave Rice		8/16/2010		created
--		Dave Rice		10/26/2010		switched to posdbssa
--		Dave Rice		02/21/2011		added 104
--		Mike Pelikan	04/24/2012		added 105
--		Kevin Shyr		2/9/2015		Change store number logic to use a table lookup to get away from store number range
--		Kevin Shyr		2/10/2015		Added hard coded 13 and 2013 to be included
--		Anthony Delgado	07/11/2016		Added hard coded line object 106 
--		Dam Tweedie		2017-11-03		Updated polling store lookup
--		Dan Tweedie		2020-06-30		Updated sales audit query to exclude ship-from-store/pickup-from-store - registers (52,53,56,57)
-- =============================================================================================================
...
select ss.store_id
from (
	select distinct store_id from dw.dbo.StoreSalesCheck_StoreSales 
	) ss
...
from (
	...
	from StoreSalesCheck_StoreList sl
		left join (
			...
			from dw.dbo.StoreSalesCheck_StoreSales s
			group by store_id
			) s
