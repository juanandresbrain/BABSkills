# dbo.spMonthlyNewVsRepeatVisit_tdf_rpt
...
```mermaid
flowchart LR
    SP["dbo.spMonthlyNewVsRepeatVisit_tdf_rpt"]
...
******************************************************************************/
CREATE
PROCEDURE dbo.spMonthlyNewVsRepeatVisit_tdf_rpt
	/* ===== ARGUMENTS ===== */
...
--50 min
create clustered index idxC_NU_date_key on tdf_rpt(date_key)
--6 min
*/
...
JOIN dbo.tdf_rpt tdf (nolock) ON tk.hhkey = tdf.sender_address_key
JOIN date_dim dd ON tdf.date_key = dd.date_key		
...
FROM #TMPKiosk tk --(index = IX_hhkey) 
JOIN dbo.tdf_rpt tdf (nolock) ON tk.hhkey = tdf.recipient_address_key
JOIN date_dim dd ON tdf.date_key = dd.date_key		
