...
```mermaid
flowchart LR
    SP["dbo.spMetricRollup_WTDbyStore"]
...
******************************************************************************/
CREATE                                          PROCEDURE  spMetricRollup_WTDbyStore
/* ===== ARGUMENTS ===== */	
@TimePeriod 	char(1)
...
ELSE IF @TimePeriod = 'q'
	BEGIN
	SET @wkCurTYbegin = (select min(week_id) from date_dim 
				where fiscal_quarter =(select min(fiscal_quarter) from date_dim where week_id = @wkCurTYend)
				  and fiscal_year =  (select min(fiscal_year) from date_dim where week_id = @wkCurTYend))
...
ELSE 
	...
	SET @maxFW = (select max(fiscal_week) from date_dim where week_id = @wkCurTYend)
	SET @maxFP = (select max(fiscal_period) from date_dim where week_id = @wkCurTYend)
	SET @maxFQ = (select max(fiscal_quarter) from date_dim where week_id = @wkCurTYend)
	END


--select @wkCurTYbegin, @wkCurTYend
...
from date_rpt_facts drf 		
	...
	where dd.week_id >= @wkCurTYbegin and dd.week_id <= @wkCurTYend
...
--select * from #TYLYresults
	...
	select 	 @maxFW as maxFW
		...
		and dd1.fiscal_year = a.fiscal_year --= 2003 --year(@curDate)
		) as wkendingdate --changed by DanM and Cece 12/03
		,max(comp_Y_N) as Comp_Y_N
		--,CASE WHEN min(sd.comp_week_id) <= min(a.week_id) THEN 1 /*=Yes*/ ELSE 0 END as Comp_Y_N
