...
```mermaid
flowchart LR
    SP["dbo.spMergeEmailFacts2014"]
    EmailFact(["EmailFact"]) --> SP
    EmailFact2014(["EmailFact2014"]) --> SP
```
...
```sql
create proc spMergeEmailFacts2014
as
...
merge into EmailFact2014 as target
using 
	(
		select *
		from EmailFact with (nolock)
		where datepart(yyyy, SendDate) = 2014
	)
		as source 
on 
	(
