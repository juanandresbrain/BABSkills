# dbo.spReportPOS_Top20_ByDepartment
...
```mermaid
flowchart LR
    SP["dbo.spReportPOS_Top20_ByDepartment"]
...
-- =====================================================================================================
CREATE PROCEDURE [dbo].[spReportPOS_Top20_ByDepartment]
...
into #StoreLookup
from [stl-ssis-p-01].[IntegrationStaging].[ERP].[vwWarehouseIDToLocationCodeRetailInventory] -- changed view source on 9/15/2023
where 1=1
...
CREATE NONCLUSTERED INDEX [NCI_StyleLookup]
ON [dbo].[#StyleLookup] ([DepartmentCode])
INCLUDE ([ProductNumber],[ProductDescription],[Department])

CREATE NONCLUSTERED INDEX [NC_StyleProductNumber]
ON [dbo].[#StyleLookup] ([ProductNumber])
INCLUDE ([ProductDescription],[Department],[DepartmentCode])
...
--2025-03-06 - no longer need transaction facts
--DW\Historical 
--select 
----dd.actual_date, 
...
--join Transaction_Facts tf (nolock) on tf.transaction_id=tdf.transaction_id
