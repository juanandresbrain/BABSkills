# dbo.spEmailUltiProToActiveDirectoryFullNameUpdate
...
```mermaid
flowchart LR
    SP["dbo.spEmailUltiProToActiveDirectoryFullNameUpdate"]
...
```sql
CREATE proc [dbo].[spEmailUltiProToActiveDirectoryFullNameUpdate] 
...
select @Subj = 'UltiPro to Active Directory store manager full name update'
	select @recip = 'ianw@buildabear.com'
	select @ProvisionText = 'full name update'


select @body = 
'<font face =arial size = 2><B>UltiPro to Active Directory Employee Full Name update</B><br>' +
'The following employee full name was updated. <br> ' +
'</font>' +
	'<table border="1">' +
