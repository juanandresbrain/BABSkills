# dbo.spEmailUltiProToActiveDirectoryPromoteStaged
...
```mermaid
flowchart LR
    SP["dbo.spEmailUltiProToActiveDirectoryPromoteStaged"]
...
```sql
CREATE proc [dbo].[spEmailUltiProToActiveDirectoryPromoteStaged] 
...
select @Subj = 'UltiPro to Active Directory Employee promtion samaccountname update staged'
	select @recip = 'SarahMe@buildabear.com;ianw@buildabear.com'
	--select @recip = 'ianw@buildabear.com;dant@buildabear.com;'
	select @ProvisionText = 'RN'


select @body = 
'<font face =arial size = 2><B>UltiPro to Active Directory store employee promotion staged</B><br>' +
'The following employee record was sent to DataLoaderStaging table with RN event code. <br> ' +
'</font>' +
	'<table border="1">' +
		'<tr><th><font face =arial size = 2>ProvisioningEvent</font></th>' +
			...
			'<th><font face =arial size = 2>Previous samaccountname</font></th>' +
