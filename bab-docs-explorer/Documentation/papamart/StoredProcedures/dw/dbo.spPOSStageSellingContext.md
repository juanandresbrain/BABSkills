...
```mermaid
flowchart LR
    SP["dbo.spPOSStageSellingContext"]
    dbo_ATTR_MSTR_DIM(["dbo.ATTR_MSTR_DIM"]) --> SP
    ...
    dbo_LOC_TYPE_DIM(["dbo.LOC_TYPE_DIM"]) --> SP
...
```sql
CREATE proc [dbo].[spPOSStageSellingContext] 
...
WHERE	PERM_CLOSE=1
and STR_KEY not in (select STR_KEY FROM	KODIAK.BABWMstrData.dbo.STR_OPEN_DIM where CLOSE_DT = '2399-12-31' and PERM_CLOSE = 0)   -- IDW added, this was preventing 149 from adding to dataset
...
if object_id('dw..POSSellingContextStage') is not null drop table POSSellingContextStage;
with
OUN as 
	(
		select 
			...
			WarehouseID,
			OperatingUnitNumber
		from [STL-SSIS-P-01].IntegrationStaging.wms.RetailStore 
		where isnumeric(WarehouseID)=1
		and WarehouseID not in ('2301','9970','9980')
	group by 
		WarehouseLegalEntity,
		WarehouseID,
		OperatingUnitNumber
...
select
	...
	--END AS LocationID,
	l.gl_location_number as LocationID,
	...
	tsd.WebOrStore,
	cast(case 
		when l.allow_customer_order_flag =1 --or b.location_code is not null 
		then '1'
		else '0'
	end as int) as isEndlessAisleEligible,
	oun.WarehouseLegalEntity,
	oun.OperatingUnitNumber
into POSSellingContextStage
from #ps1 tsd
...
left join OUN on l.gl_location_number=oun.WarehouseID
where PermCloseStatus=0
--and (oc.ORG_CHN_TYPE_CODE in ('S', 'W') or tsd.StoreID in (549,550))
and oc.ORG_CHN_TYPE_CODE in ('S', 'W')
order by StoreID
```
