# dbo.spEmailUltiProToActiveDirectoryDemoteStaged
...
```mermaid
flowchart LR
    SP["dbo.spEmailUltiProToActiveDirectoryDemoteStaged"]
...
```sql
CREATE proc [dbo].[spEmailUltiProToActiveDirectoryDemoteStaged] 
...
select @Subj = 'UltiPro to Active Directory Employee demotion samaccountname update staged'
	--select @recip = 'ianw@buildabear.com'
	select @recip = 'ianw@buildabear.com;SarahMe@buildabear.com;'
	select @ProvisionText = 'RN'


select @body = 
'<font face =arial size = 2><B>UltiPro to Active Directory store employee demotion staged</B><br>' +
'The following employee samaccountname was set to EepEEID. <br> ' +
'</font>' +
	'<table border="1">' +
