# dbo.spCRMLoyaltyTransactionPreStage_Staging2
...
```mermaid
flowchart LR
    SP["dbo.spCRMLoyaltyTransactionPreStage_Staging2"]
    ...
    dbo_LoyaltyProducts(["dbo.LoyaltyProducts"]) --> SP
    dbo_LoyaltyProductsNoStyle(["dbo.LoyaltyProductsNoStyle"]) --> SP
    dbo_LoyaltyTransactionFactStageTest(["dbo.LoyaltyTransactionFactStageTest"]) --> SP
...
```sql
CREATE proc [dbo].[spCRMLoyaltyTransactionPreStage_Staging2]
@DaysToGoBack int, @source varchar(10)
...
-- above query from views replaced with these new tables below 
select ProductKey,Style,KeyStory,Chain,Department,LicenseCode, Class, SubClass
into #Products
from papamart.dw.dbo.LoyaltyProducts
UNION
select ProductKey,isnull(Style,('N/A' + cast(ProductKey as varchar))) as Style, KeyStory,isnull(Chain,'N/A') as Chain,isnull(Department,'N/A') as Department, LicenseCode, Class, SubClass
from papamart.dw.dbo.LoyaltyProductsNoStyle
...
--where datediff(dd, actual_date, getdate()-@DaysToGoBack)=0
...
from dwstaging.dbo.LoyaltyTransactionFactStageTest  ctf with (nolock)
...
join TransactionDetailFactsDynamics  tdf with (nolock) on ctf.TransactionID=tdf.Transaction_ID
...
join TransactionFactsDynamics tf on ctf.TransactionID=tf.transaction_id 
...
--and ctf.TransactionDate >= cast(getdate()-45 as date)
	--and (ctf.POS = case when @source = 'All' then 'Aptos' when @source = 'JumpMind' then 'JumpMind' end
	--	or
	--	ctf.POS = case when @source = 'All' then 'JumpMind' when @source = 'JumpMind' then 'JumpMind' end)
