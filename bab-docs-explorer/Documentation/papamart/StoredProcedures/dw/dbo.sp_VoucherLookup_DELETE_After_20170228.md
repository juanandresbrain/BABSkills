# dbo.sp_VoucherLookup_DELETE_After_20170228
...
```mermaid
flowchart LR
    SP["dbo.sp_VoucherLookup_DELETE_After_20170228"]
...
```sql
CREATE PROCEDURE [dbo].[sp_VoucherLookup_DELETE_After_20170228]
...
BEGIN
	...
	FROM crmdb02.crm.dbo.customer c
	INNER JOIN crmdb02.crm.dbo.address a ON c.customer_id = a.customer_id
	WHERE c.customer_no = @sfs_Number
	ORDER BY address_effective_date)
	...
	BEGIN
		...
		LEFT OUTER JOIN crmdb02.crm.dbo.customer cc (NOLOCK) ON c.customer_no = cc.customer_no
		LEFT JOIN 
			   (
		...
		WHERE h.store_no = 990 AND c.reference_type = ' + CAST(@refType AS VARCHAR(2)) + ' AND c.customer_no = ' + CAST(@sfs_Number AS VARCHAR(9)) + '
...
LEFT OUTER JOIN bedrockdb01.auditworks.dbo.cust_liability_history h (NOLOCK) ON c.reference_no = h.reference_no
--LEFT OUTER JOIN crmdb02.crm.dbo.customer cc (NOLOCK) ON c.customer_no = cc.customer_no
--INNER JOIN crmdb02.crm.dbo.customer_attribute ca (NOLOCK) ON cc.customer_id = ca.customer_id
LEFT JOIN 
       (
              SELECT MIN(h2.transaction_date) [RedeemedDate], MIN(h2.store_no) [RedeemedAt], reference_no
              FROM bedrockdb01.auditworks.dbo.cust_liability_history h2 (NOLOCK)
...
WHERE h.store_no = 990 AND c.reference_type = @refType AND c.reference_no LIKE @voucher_number
ORDER BY c.date_issued
END
