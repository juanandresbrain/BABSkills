...
```mermaid
flowchart LR
    SP["dbo.spMergeEmailFacts2017"]
    EmailFact(["EmailFact"]) --> SP
    EmailFact2017(["EmailFact2017"]) --> SP
```
...
```sql
CREATE proc spMergeEmailFacts2017
as
...
merge into EmailFact2017 as target
using 
	(
		select *
		from EmailFact with (nolock)
		where datepart(yyyy, SendDate) = 2017
	)
		as source 
on 
	(
