# dbo.spRPT_Distr_SFSAnalysis_IDW
...
```mermaid
flowchart LR
    SP["dbo.spRPT_Distr_SFSAnalysis_IDW"]
...
```sql
CREATE PROCEDURE [dbo].[spRPT_Distr_SFSAnalysis_IDW]
AS
-- =============================================
-- Author:		Gary Murrish
...
BEGIN
	...
	SELECT
		e.name,e.Email,
		--'ian wallace' as 'name', 'ianw@buildabear.com' as Email,
		--'idw' as [name],'ianw@buildabear.com' as [Email],
...
SELECT
	...
	where dl.GeogSortKey not like '0%'
	and dl.GeogSortKey like '2|S%'
