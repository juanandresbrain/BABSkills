# dbo.spGetActivationsMissingCards
...
```mermaid
flowchart LR
    SP["dbo.spGetActivationsMissingCards"]
...
```sql
-- =============================================================================================================
-- Name: [spGetActivationsMissingCards]
--
-- Description:	
--		Generate Activations for Missing Giftcards 
...
CREATE PROCEDURE [dbo].[spGetActivationsMissingCards]
@asOfDateKey int
AS
...
INSERT INTO dw.dbo.Giftcards_Activated
	...
	SELECT
		...
		-1 AS transaction_id,
		dd.date_key,
		gb.OutstandingBalance AS activate_amount,
		0 AS discount_amount,
		gb.CardNumber,
		ISNULL(cd.currency_key, -1) AS currency_key,
		gb.ActivationMid AS MID,
		@Source AS [Source]
	FROM
		Staging.BankOfAmerica gb WITH (NOLOCK)
		LEFT JOIN Staging.DWCardBalance b WITH (NOLOCK)
