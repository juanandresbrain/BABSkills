...
flowchart LR
    ...
    tmpIndexesStatistics(["tmpIndexesStatistics"]) --> SP
```
...
```sql
CREATE PROCEDURE [dbo].[spDBA_IndexOptimize] 
...
BEGIN
-- =============================================================================================================
-- Name: spDBA_IndexOptimize
--
-- Description:	Performs optimization on indexes based on parameters passed in.
--				This procedure is the Databears preferred sproc for index maintenance.
--				Works on Standard and Enterprise editions of SQL Server 2008
...
--		Gary Derikito	05/06/2009		Add logic to limit how long the procedure will run to allow staying within maintenance windows.
--		Gary Derikito	05/14/2009		Update error messages to write to log.
--		Gary Derikito	06/17/2009		Correct name of database function.
--		Gary Derikito	03/18/2010		Correct check for @FragmentationHigh_NonLOB which was preventing online rebuild. 	
--		Mike Pelikan	04/24/2012		Completely Rewrote procedure based on ola.hallengren.com's script
--										Added logic to return version date of backup script if @Databases = 'ReturnVersion'
--		Mike Pelikan	06/12/2012		Added Intermediate Node fragmentation table insert for tables with greater than 1000 pages on 
--		Mike Pelikan	03/24/2014		Changed timelimit and delay variables to be in minutes, instead of seconds.
--		Mike Pelikan	05/16/2014		Added Schema to large table repository insert
  ...
  BEGIN
	...
	----------------------------------------------------------------------------------------------------
      ...
      BEGIN
        ...
        BEGIN
			SET @CurrentExtendedInfoXML = (SELECT *
		...
		-------------------------------------------------------------------------------------------------------------------------
		--Intermediate Node fragmentation checking
		--If the leaf level has more than 1000 pages, insert it into repository for later checking 
		-------------------------------------------------------------------------------------------------------------------------
        IF @CurrentIndexID IS NOT NULL AND @CurrentPageCount > 1000
        BEGIN
			IF (SELECT COUNT(*) FROM DBAUtility.dbo.tblDBA_IndexMaintenance_LargeTable_Repository
			WHERE DatabaseID = @CurrentDatabaseID AND TableID = @CurrentObjectID AND IndexName = @CurrentIndexName) = 0 
			BEGIN
				INSERT INTO DBAUtility.dbo.tblDBA_IndexMaintenance_LargeTable_Repository(DatabaseName, SchemaName, TableName, IndexName, DatabaseID, TableID, IndexID, LastCheckDate, avg_fragmentation_in_percent, TimeSpent)
				SELECT @CurrentDatabaseName, @CurrentSchemaName, @CurrentObjectName TableName, @CurrentIndexName IndexName, @CurrentDatabaseID DatabaseID, @CurrentObjectID TableID, @CurrentIndexID IndexID, '1/1/1900', @CurrentFragmentationLevel, 0
			END
		END
		-------------------------------------------------------------------------------------------------------------------------
        ...
        BEGIN
          ...
          BEGIN
			...
			-------------------------------------------------------------------------------------------------------------------------
			--Intermediate Node fragmentation checking
			--Update the index record in the repository with today's date so it is not checked for a month (at current release)
			-------------------------------------------------------------------------------------------------------------------------

			IF @CurrentIndexID IS NOT NULL AND @CurrentPageCount > 1000
			BEGIN
				IF (SELECT COUNT(*) FROM DBAUtility.dbo.tblDBA_IndexMaintenance_LargeTable_Repository
				WHERE DatabaseID = @CurrentDatabaseID AND TableID = @CurrentObjectID) > 0 
				BEGIN
					UPDATE DBAUtility.dbo.tblDBA_IndexMaintenance_LargeTable_Repository 
					SET LastCheckDate = GETDATE()
					WHERE DatabaseID = @CurrentDatabaseID AND TableID = @CurrentObjectID  AND IndexID = @CurrentIndexID 
				END
			END
