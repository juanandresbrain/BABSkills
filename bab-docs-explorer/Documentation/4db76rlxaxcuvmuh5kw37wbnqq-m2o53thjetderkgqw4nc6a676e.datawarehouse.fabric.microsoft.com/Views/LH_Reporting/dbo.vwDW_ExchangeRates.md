...
```sql
CREATE VIEW [dbo].[vwDW_ExchangeRates]
AS
WITH ERV
AS (
	SELECT date_key
	...
	FROM LH_Mart.dbo.exchange_rate_facts
	...
	SELECT date_key
		...
		,round(CASE 
				WHEN actual_rate <> 0
					THEN 1.0 / actual_rate
				ELSE 0
				END, 5) AS actual_rate
		,round(CASE 
				WHEN fiscal_month_ave_rate <> 0
					THEN 1.0 / fiscal_month_ave_rate
				ELSE 0
				END, 5) AS fiscal_month_ave_rate
		,round(CASE 
				WHEN fiscal_month_end_rate <> 0
					THEN 1.0 / fiscal_month_end_rate
				ELSE 0
				END, 5) AS fiscal_month_end_rate
		,round(CASE 
				WHEN calendar_month_ave_rate <> 0
					THEN 1.0 / calendar_month_ave_rate
				ELSE 0
				END, 5) AS calendar_month_ave_rate
		,round(CASE 
				WHEN calendar_month_end_rate <> 0
					THEN 1.0 / calendar_month_end_rate
				ELSE 0
				END, 5) AS calendar_month_end_rate
	FROM LH_Mart.dbo.exchange_rate_facts e
	INNER JOIN (
		SELECT DISTINCT to_currency_key
			,c.currency_code
		FROM LH_Mart.dbo.exchange_rate_facts e2
		INNER JOIN LH_Mart.dbo.currency_dim c ON c.currency_key = e2.to_currency_key
	...
	FROM LH_Mart.dbo.exchange_rate_facts e
	INNER JOIN (
		SELECT DISTINCT t2.from_currency_key
			,t2.currency_code
		FROM (
			SELECT DISTINCT from_currency_key
				,c.currency_code
			FROM LH_Mart.dbo.exchange_rate_facts e2
			INNER JOIN LH_Mart.dbo.currency_dim c 
            ON c.currency_key = e2.from_currency_key
			
			UNION
			
			SELECT DISTINCT to_currency_key AS from_currency_key
				,c.currency_code
			FROM LH_Mart.dbo.exchange_rate_facts e2
			INNER JOIN LH_Mart.dbo.currency_dim c ON c.currency_key = e2.to_currency_key
			) t2
