...
SELECT employee_no = E.EMPLY_NUM,
    ...
    primary_position_code = OCP1.PSTN_CODE,
    house_account_no = E.HS_ACNT_NUM,
    date_of_hire =  convert(varchar(10),COALESCE(MIN(ESH.EFCTV_DATE), E.SNRTY_DATE, E.STS_EFCTV_DATE),101)  + ' ' + convert(varchar(8),COALESCE(MIN(ESH.EFCTV_DATE), E.SNRTY_DATE, E.STS_EFCTV_DATE),114),
...
LEFT OUTER JOIN  EMPLY_ORG_CHN_PSTN_A_HSTRY EOCPAH
    ...
    AND (EOCPAH.EXPRTN_DATE > getdate() OR EOCPAH.EXPRTN_DATE IS NULL)) 
LEFT OUTER JOIN  ORG_CHN_PSTN OCP1
    ON COALESCE(EOCPA.PSTN_CODE, E.TTL_PSTN_CODE, EOCPAH.PSTN_CODE) = OCP1.PSTN_CODE  
WHERE E.EMPLY_STS_CODE NOT IN ('TERM','SUSP','RESN','RETD')
GROUP BY E.EMPLY_NUM,E.FRST_NAME,E.LAST_NAME,E.PRMY_ORG_CHN_NUM, 
        OCP1.PSTN_CODE,E.HS_ACNT_NUM,	E.SNRTY_DATE, E.STS_EFCTV_DATE,
...
SELECT employee_no = E.EMPLY_NUM,
    ...
    primary_position_code = OCP1.PSTN_CODE,
    house_account_no = E.HS_ACNT_NUM,
    date_of_hire =  convert(varchar(10),COALESCE(MIN(ESH.EFCTV_DATE), E.SNRTY_DATE),101)  + ' ' + convert(varchar(8),COALESCE(MIN(ESH.EFCTV_DATE), E.SNRTY_DATE),114),--MIN(ESH.EFCTV_DATE),
    date_of_termination = convert(varchar(10),COALESCE(MAX(ESH1.EFCTV_DATE), E.STS_EFCTV_DATE),101)  + ' ' + convert(varchar(8),COALESCE(MAX(ESH1.EFCTV_DATE), E.STS_EFCTV_DATE),114),--ESH1.EFCTV_DATE,
    employee_dept_no = ISNULL(EOCPA.PRMRY_LOC_ID,EOCPAH.PRMRY_LOC_ID ),  
    employee_dept_desc = NULL,
    primary_position_descr = OCP1.PSTN_DESC,
    active_flag = E.ACTV
...
LEFT JOIN EMPLY_STS_HSTRY ESH 
    ON E.EMPLY_NUM = ESH.EMPLY_NUM
    AND ESH.EMPLY_STS_CODE = 'HIRE'
LEFT JOIN  EMPLY_STS_HSTRY ESH1       
...
LEFT OUTER JOIN  ORG_CHN_PSTN OCP1
    ON  COALESCE(EOCPA.PSTN_CODE, E.TTL_PSTN_CODE, EOCPAH.PSTN_CODE)= OCP1.PSTN_CODE      
WHERE E.EMPLY_STS_CODE IN ('TERM','SUSP','RESN','RETD')      
GROUP BY E.EMPLY_NUM,E.FRST_NAME,E.LAST_NAME,E.PRMY_ORG_CHN_NUM, E.SNRTY_DATE, E.STS_EFCTV_DATE,
        OCP1.PSTN_CODE,E.HS_ACNT_NUM, ISNULL(EOCPA.PRMRY_LOC_ID,EOCPAH.PRMRY_LOC_ID ) ,OCP1.PSTN_DESC,E.ACTV
```
