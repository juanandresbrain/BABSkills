...
```mermaid
flowchart LR
    VIEW["dbo.process_log_consolidated"]
    auditworks_system_flag(["auditworks_system_flag"]) --> VIEW
    ex_process_log(["ex_process_log"]) --> VIEW
    process_log(["process_log"]) --> VIEW
```
...
```sql
CREATE VIEW process_log_consolidated
AS
SELECT convert(int, s.flag_numeric_value) instance_id, p.*
  FROM process_log p, auditworks_system_flag s
 WHERE s.flag_name = 'instance_id'
UNION
SELECT -1 instance_id, p.*
  FROM ex_process_log p, auditworks_system_flag s, auditworks_system_flag e
 WHERE s.flag_name = 'instance_id'
