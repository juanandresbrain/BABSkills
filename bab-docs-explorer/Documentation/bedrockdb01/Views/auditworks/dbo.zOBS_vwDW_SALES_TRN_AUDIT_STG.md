# dbo.zOBS_vwDW_SALES_TRN_AUDIT_STG
...
```mermaid
flowchart LR
    VIEW["dbo.zOBS_vwDW_SALES_TRN_AUDIT_STG"]
    Ld_POS_Audits(["Ld_POS_Audits"]) --> VIEW
...
```sql
-- =====================================================================================================
-- Name: vwDW_SALES_TRN_AUDIT_STG
--
-- Description:	Extract information from Audit works for the evening run based upon the
...
-- =====================================================================================================
CREATE VIEW [dbo].[vwDW_SALES_TRN_AUDIT_STG]
AS
WITH vw1621
AS (
...
FROM
	Ld_POS_Audits ld WITH (NOLOCK)
	JOIN uvw_transaction_header th WITH (NOLOCK)
		ON th.transaction_id = ld.transaction_id
	...
	LEFT JOIN
		(SELECT d.transaction_id
		 ...
		 FROM
			 Ld_POS_Audits ot WITH (NOLOCK)
			 JOIN uvw_discount_detail d WITH (NOLOCK)
				 ON d.transaction_id = ot.transaction_id
		 GROUP BY
			 d.transaction_id
		   , d.line_id) dd
