...
```mermaid
flowchart LR
    VIEW["dbo.sv_media_reconciliation_3"]
...
```sql
CREATE view [dbo].[sv_media_reconciliation_3] AS
SELECT
	mrd.store_no,
	mrs.register_no,
	mrs.cashier_no,
	...
	sum(mrd.rec_amount * abs(sign(mrd.rec_side - 1)) 
		* sign(abs(mrd.rec_amount_subtype - 0)) 
		* sign(abs(mrd.rec_amount_subtype - 3))
	    * (1 - sign(mrd.rec_amount_type - 1))) as expected_media_amount,
	sum(mrd.rec_amount * abs(sign(mrd.rec_side - 1)) 
	    * sign(mrd.rec_amount_type - 1)) as expected_exchange_amount,
	...
	sum(mrd.rec_amount * abs(sign(mrd.rec_side - 1)) * (1-sign(abs(mrd.rec_amount_subtype - 3)))
	    * (1 - sign(mrd.rec_amount_type - 1))) as pickup_loan_amount,
	sum(mrd.rec_amount * (1 - abs(sign(mrd.rec_side - 1))) 
