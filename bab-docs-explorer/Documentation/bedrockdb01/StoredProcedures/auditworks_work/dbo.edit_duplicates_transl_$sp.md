# dbo.edit_duplicates_transl_$sp
...
```mermaid
flowchart LR
    SP["dbo.edit_duplicates_transl_$sp"]
...
```sql
create proc dbo.edit_duplicates_transl_$sp 
  ...
  BEGIN
   ...
   SELECT
	...
	'<transl>_merchandise_detail'
     FROM transl_merchandise_detail
    GROUP BY store_no, register_no, entry_date_time, transaction_series, transaction_no, line_id
    HAVING COUNT(line_id) > 1
  ...
  BEGIN
   ...
   SELECT
	...
	'<transl>_authorization_detail'
     FROM transl_authorization_detail
    GROUP BY store_no, register_no, entry_date_time, transaction_series, transaction_no, line_id
    HAVING COUNT(line_id) > 1
  ...
  BEGIN
   ...
   SELECT
	...
	'<transl>_special_order_detail'
     FROM transl_special_order_detail
    GROUP BY store_no, register_no, entry_date_time, transaction_series, transaction_no, line_id
    HAVING COUNT(line_id) > 1
  ...
  BEGIN
   ...
   SELECT
	...
	'<transl>_tax_override_detail'
     FROM transl_tax_override_detail
...
IF (@violated_sareject_rule = 50 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_transaction_line
      FROM transl_transaction_line al, #work_lines_edit we
...
IF (@violated_sareject_rule = 41 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_merchandise_detail
      FROM transl_merchandise_detail am, #work_lines_edit we
...
IF (@violated_sareject_rule = 42 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_authorization_detail
      FROM transl_authorization_detail ad, #work_lines_edit we
...
IF (@violated_sareject_rule = 43 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_stock_control_detail
      FROM transl_stock_control_detail ac, #work_lines_edit we
...
IF (@violated_sareject_rule = 44 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_special_order_detail
      FROM transl_special_order_detail ao, #work_lines_edit we
...
IF (@violated_sareject_rule = 45 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_post_void_detail
      FROM transl_post_void_detail ap, #work_lines_edit we
...
IF (@violated_sareject_rule = 46 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_payroll_detail
      FROM transl_payroll_detail ay, #work_lines_edit we
...
IF (@violated_sareject_rule = 48 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_tax_override_detail
      FROM transl_tax_override_detail ax, #work_lines_edit we
...
IF (@violated_sareject_rule = 49 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_return_detail
      FROM transl_return_detail ar, #work_lines_edit we
...
IF (@violated_sareject_rule = 51 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_customer
      FROM transl_customer ac, #work_lines_edit we
     WHERE ac.store_no = we.store_no
       AND ac.register_no = we.register_no
       AND ac.entry_date_time = we.entry_date_time
       AND ac.transaction_series = we.transaction_series
...
IF (@violated_sareject_rule = 52 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_customer_detail
      FROM transl_customer_detail ai, #work_lines_edit we
...
IF (@violated_sareject_rule = 53 OR @violated_sareject_rule = -1)
  BEGIN
    DELETE transl_line_note
      FROM transl_line_note an, #work_lines_edit we
...
error:
	...
	SELECT @errmsg = 'edit_duplicates_transl_$sp: ' + @errmsg
