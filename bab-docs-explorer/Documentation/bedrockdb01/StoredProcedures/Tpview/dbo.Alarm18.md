...
```sql
create proc Alarm18 -- Excessive disconnection per store.
		@StoreNumber		INT,
...
WHERE AlarmRuleNo = 18 AND ParamName = 'ACTIVE'))
BEGIN
	Set @TotalDisconnection = 0
	Set @TimeFrame = 0
	SELECT @Active = CAST(ParamValue AS INT) FROM	tblAlarmParam WHERE AlarmRuleNo = 18 AND ParamName = 'ACTIVE'
IF(@Active = 1)
BEGIN
BEGIN
	--Getting email information.
	SELECT @SendToEmail = ParamValue FROM	tblAlarmParam 
	WHERE AlarmRuleNo = 18 AND ParamName = 'EMAIL'
	...
	BEGIN
		...
		WHERE RemoteNumber = @StoreNumber AND ConnectType =1 AND RegisterNumber = @Register
		--Getting the index for the first service configured in the AlarmParam Table.
		SELECT @ThreshHold = CAST(ParamValue AS INT) FROM	tblAlarmParam 
		WHERE AlarmRuleNo = 18 AND ParamName = 'THRESHOLDHOUR'
	...
	BEGIN
		...
		WHERE RemoteNumber = @StoreNumber AND ConnectType =1 AND RegisterNumber = @Register
		SELECT @ThreshHold = CAST(ParamValue AS INT) FROM	tblAlarmParam 
		WHERE AlarmRuleNo = 18 AND ParamName = 'THRESHOLDDAY'
		SET @Date = (LTRIM(STR(DATEPART(yyyy,@LastEventTime)))+'-'+
					LTRIM(STR(DATEPART(mm,@LastEventTime)))+'-'+
	...
	BEGIN
		...
		WHERE RemoteNumber = @StoreNumber AND ConnectType =1 AND RegisterNumber = @Register
		SELECT @ThreshHold = CAST(ParamValue AS INT) FROM	tblAlarmParam 
		WHERE AlarmRuleNo = 18 AND ParamName = 'THRESHOLDWEEK'
		SET @Date = LTRIM(STR(DATEPART(yyyy,@LastEventTime)))+'-'+
					LTRIM(STR(DATEPART(mm,@LastEventTime)))+'-'+
					LTRIM(STR(DATEPART(dd,@LastEventTime)))+' 11:59:59'
		SET @Interval = 'Week'
	...
	BEGIN
		...
		+ LTRIM(STR(@StoreNumber))+ ' Register ' + LTRIM(STR(@Register)) +
