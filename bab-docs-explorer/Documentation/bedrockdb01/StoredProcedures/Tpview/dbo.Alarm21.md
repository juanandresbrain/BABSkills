...
```sql
create proc Alarm21 -- Excessive disconnection per store.
@StoreNumber	INT,
@Interval		INT,
@register		INT
AS
DECLARE @HourlyTotal 	INT,
		...
		@Active			INT
--Getting ThreshHolds
SELECT @EmailAddress = ParamValue FROM tblAlarmParam WHERE AlarmRuleNo = 21 AND ParamName = 'EMAIL'
SELECT @Active = CAST(ParamValue AS INT) FROM tblAlarmParam WHERE AlarmRuleNo = 21 AND ParamName = 'ACTIVE'
...
BEGIN
	...
	BEGIN
		...
		SET @EventDesc = 'WAN Stores: Excessive Internet Latency: The average internet latency for store '
		+LTRIM(STR(@StoreNumber))+' register ' +LTRIM(STR(@register))+' is '+LTRIM(STR(@HourlyTotal))+' milliseconds for the hour ended on ' + RTRIM(@Date) +
		...
		VALUES (GETDATE(),@EventDesc,0,0,'1900-01-01 12:01:00 AM',0,3,0,@EmailAddress,'1900-01-01 12:01:00 AM',0,5,'Permanently Connected Stores: Excessive Internet Latency: '+LTRIM(STR(@StoreNumber))+' register ' +LTRIM(STR(@register)))
	END
END
--Daily
IF(@Interval=2)
BEGIN
	...
	BEGIN
		...
		+LTRIM(STR(@StoreNumber))+' register ' +LTRIM(STR(@register))+' is '+LTRIM(STR(@DailyTotal))+' milliseconds for the day ended on ' + RTRIM(@Date) +
...
BEGIN
	...
	BEGIN
		...
		SET @EventDesc = 'Permanently Connected Stores:Excessive Internet Latency: The average internet latency for store '
		+LTRIM(STR(@StoreNumber))+' register ' +LTRIM(STR(@register))+' is '+LTRIM(STR(@WeeklyTotal))+' milliseconds for the week ended on ' + RTRIM(@Date) +
