...
FROM
    [ReportServerESellTempDB].dbo.SessionData AS SE WITH (XLOCK)
WHERE
...
FROM
    [ReportServerESellTempDB].dbo.SessionData AS SE
    INNER JOIN [ReportServerESellTempDB].dbo.SnapshotData AS SN ON SN.SnapshotDataID = SE.SnapshotDataID  
    LEFT OUTER JOIN [ReportServerESellTempDB].dbo.SnapshotData AS COMP ON SE.CompiledDefinition = COMP.SnapshotDataID      
WHERE
   SE.SessionID = @DBSessionID
