...
BEGIN
	...
	ELSE BEGIN
		-- copy non-segmented chunks
		INSERT [ReportServerESellTempDB].dbo.[ChunkData] (
			ChunkId, 
			SnapshotDataId, 
			ChunkFlags, 
			ChunkName, 
		...
		SELECT 
			...
			[c].[MimeType], 
			[c].[Content]
		FROM [ReportServerESellTempDB].dbo.[ChunkData] [c] WHERE [c].[SnapshotDataId] = @OldSnapshotId
				
		-- copy segmented chunks... real easy just add the mapping
		INSERT [ReportServerESellTempDB].[dbo].[SegmentedChunk](
			ChunkId, 
			SnapshotDataId, 
			ChunkName, 
			ChunkType,
		...
		SELECT 
			...
			ChunkFlags, 
			Machine
		FROM [ReportServerESellTempDB].dbo.[SegmentedChunk] WITH (INDEX (UNIQ_SnapshotChunkMapping))
		WHERE [SnapshotDataId] = @OldSnapshotId
	END
END
```
