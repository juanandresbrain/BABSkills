...
begin
	...
	from [ReportServerESellTempDB].dbo.Segment with (readpast)	
	where not exists (
		select 1 from [ReportServerESellTempDB].dbo.ChunkSegmentMapping CSM with (nolock)
		where CSM.SegmentId = [ReportServerESellTempDB].dbo.Segment.SegmentId
		) ;
		
	delete from [ReportServerESellTempDB].dbo.Segment with (readpast)
	where [ReportServerESellTempDB].dbo.Segment.SegmentId in (
