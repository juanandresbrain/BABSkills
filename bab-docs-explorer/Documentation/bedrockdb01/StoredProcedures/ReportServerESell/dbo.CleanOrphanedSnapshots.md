...
AS 
    ...
    begin		
        ...
        and not exists (
            select 1 from [ReportServerESellTempDB].dbo.SegmentedChunk TSC with(nolock)
    ...
    begin transaction	
    DELETE TOP (@TemporarySnapshotCount) [ReportServerESellTempDB].dbo.SnapshotData 
    output deleted.SnapshotDataID into @cleanedSnapshots(SnapshotDataId)
    FROM [ReportServerESellTempDB].dbo.SnapshotData with(readpast) 
    WHERE   [ReportServerESellTempDB].dbo.SnapshotData.PermanentRefCount <= 0 AND
            [ReportServerESellTempDB].dbo.SnapshotData.TransientRefCount <= 0 AND
            [ReportServerESellTempDB].dbo.SnapshotData.Machine = @Machine ;
    SET @SnapshotsCleaned = @SnapshotsCleaned + @@ROWCOUNT ;
    
    DELETE [ReportServerESellTempDB].dbo.ChunkData FROM [ReportServerESellTempDB].dbo.ChunkData 
	INNER JOIN @cleanedSnapshots cs
    ON [ReportServerESellTempDB].dbo.ChunkData.SnapshotDataID = cs.SnapshotDataId
    ...
    commit
   	...
   	begin		
		delete SC 
		output deleted.ChunkId into @cleanedChunks(ChunkId)
		from [ReportServerESellTempDB].dbo.SegmentedChunk SC with (readpast)	
		join @cleanedSnapshots cs on SC.SnapshotDataId = cs.SnapshotDataId ;	
		set @deleteCount = @@ROWCOUNT ; 
		set @ChunksCleaned =  @ChunksCleaned + @deleteCount ;
	end ;
	...
	begin		
		...
		output deleted.ChunkId, deleted.SegmentId into @cleanedSegments (ChunkId, SegmentId)
		from [ReportServerESellTempDB].dbo.ChunkSegmentMapping CSM with (readpast)
	...
	begin
		...
		from [ReportServerESellTempDB].dbo.Segment S with (readpast)
		join @cleanedSegments cs on S.SegmentId = cs.SegmentId
		where not exists (
			select 1 from [ReportServerESellTempDB].dbo.ChunkSegmentMapping csm
			where csm.SegmentId = cs.SegmentId ) ;
		set @deleteCount = @@ROWCOUNT ;
		set @SegmentsCleaned = @SegmentsCleaned + @deleteCount ;
	end
