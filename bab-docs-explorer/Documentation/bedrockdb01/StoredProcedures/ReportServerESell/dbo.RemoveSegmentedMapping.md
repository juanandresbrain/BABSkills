...
begin
	...
	where ChunkSegmentMapping.ChunkId in (
		...
		and not exists (
			select 1 from [ReportServerESellTempDB].dbo.SegmentedChunk TSC
	...
	from [ReportServerESellTempDB].dbo.SegmentedChunk with (readpast)
	where [ReportServerESellTempDB].dbo.SegmentedChunk.Machine = @MachineName
	and not exists (
		select 1 from [ReportServerESellTempDB].dbo.SnapshotData SD with (nolock)
		where [ReportServerESellTempDB].dbo.SegmentedChunk.SnapshotDataId = SD.SnapshotDataID
		) ;
		
	delete from [ReportServerESellTempDB].dbo.SegmentedChunk with (readpast)
	where [ReportServerESellTempDB].dbo.SegmentedChunk.SnapshotDataId in (
		select td.SnapshotDataId from @toDeleteTempChunks td
		where not exists (
			select 1 from [ReportServerESellTempDB].dbo.SnapshotData SD
			where td.SnapshotDataId = SD.SnapshotDataID
			)) ;
	...
	from [ReportServerESellTempDB].dbo.ChunkSegmentMapping with (readpast)	
	where not exists (
		select 1 from [ReportServerESellTempDB].dbo.SegmentedChunk SC with (nolock)
		where SC.ChunkId = [ReportServerESellTempDB].dbo.ChunkSegmentMapping.ChunkId
		) ;
		
	delete from [ReportServerESellTempDB].dbo.ChunkSegmentMapping with (readpast)
	output deleted.ChunkId, convert(bit, 0) into @deleted
	where [ReportServerESellTempDB].dbo.ChunkSegmentMapping.ChunkId in (
		select td.ChunkId from @toDeleteTempMappings td
		where not exists (
			select 1 from [ReportServerESellTempDB].dbo.SegmentedChunk SC
