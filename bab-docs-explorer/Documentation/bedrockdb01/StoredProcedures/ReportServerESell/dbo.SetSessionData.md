...
FROM [ReportServerESellTempDB].dbo.SessionData WITH (XLOCK) 
...
BEGIN -- Yes, update it
   ...
   UPDATE
      [ReportServerESellTempDB].dbo.SessionData
   SET
   ...
   END ELSE BEGIN
      UPDATE
         [ReportServerESellTempDB].dbo.SnapshotData
      SET
         ExpirationDate = DATEADD(n, @SnapshotTimeoutSeconds, @now)
      WHERE
         SnapshotDataID = @SnapshotDataID
...
BEGIN -- no, insert it
   UPDATE PS
    ...
    INSERT INTO [ReportServerESellTempDB].dbo.SessionData
