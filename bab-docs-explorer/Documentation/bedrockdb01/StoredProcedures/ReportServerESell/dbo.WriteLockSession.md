...
BEGIN
	...
	BEGIN
		UPDATE [ReportServerESellTempDB].dbo.SessionLock WITH (ROWLOCK)
	...
	BEGIN
		...
		BEGIN 
			...
			FROM [ReportServerESellTempDB].dbo.SessionLock WITH (ROWLOCK)
			WHERE SessionID = @SessionID;
...
ELSE
BEGIN
	INSERT INTO [ReportServerESellTempDB].dbo.SessionLock WITH (ROWLOCK) (SessionID) VALUES (@SessionID)
END
```
