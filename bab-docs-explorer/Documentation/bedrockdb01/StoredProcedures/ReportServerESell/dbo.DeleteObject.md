...
WHERE    
    (R.Path = @Path OR R.Path LIKE @Prefix ESCAPE '*')
        ...
        UPDATE [ReportServerESellTempDB].dbo.TempDataSources
           SET
              Flags = Flags & 0x7FFFFFFD, -- broken link
              Link = NULL
        FROM
           [Catalog] AS C
           INNER JOIN [ReportServerESellTempDB].dbo.TempDataSources AS DS ON C.[ItemID] = DS.Link
        WHERE
           (C.Path = @Path OR C.Path LIKE @Prefix ESCAPE '*')
...
UPDATE [ReportServerESellTempDB].dbo.TempDataSets
   SET
      [LinkID] = NULL
FROM
   [Catalog] AS C
   INNER JOIN [ReportServerESellTempDB].dbo.TempDataSets AS DS ON C.[ItemID] = DS.[LinkID]
WHERE
   (C.Path = @Path OR C.Path LIKE @Prefix ESCAPE '*')
...
BEGIN
        ...
        SET PermanentRefcount = PermanentRefcount - 1
        FROM
           [ReportServerESellTempDB].dbo.TempCatalog AS R WITH (XLOCK)
           INNER JOIN [ReportServerESellTempDB].dbo.SnapshotData AS SD ON R.Intermediate = SD.SnapshotDataID
        WHERE
           R.ContextPath = @Path
        ...
        -- Clean all data sources
        DELETE [ReportServerESellTempDB].dbo.TempDataSources
        FROM
            [ReportServerESellTempDB].dbo.TempCatalog AS R
            INNER JOIN [ReportServerESellTempDB].dbo.TempDataSources AS DS ON R.TempCatalogID = DS.ItemID
        WHERE    
            R.ContextPath = @Path
            AND R.EditSessionID = @EditSessionID
            AND R.OwnerID = @OwnerID
        ...
        DELETE [ReportServerESellTempDB].dbo.TempDataSets
        FROM
            [ReportServerESellTempDB].dbo.TempCatalog AS R
            INNER JOIN [ReportServerESellTempDB].dbo.TempDataSets AS DS ON R.TempCatalogID = DS.ItemID
        WHERE    
            R.ContextPath = @Path
            AND R.EditSessionID = @EditSessionID
            AND R.OwnerID = @OwnerID
        ...
        FROM
           [ReportServerESellTempDB].dbo.SnapshotData AS SN
           INNER JOIN [ReportServerESellTempDB].dbo.ExecutionCache AS EC on SN.SnapshotDataID = EC.SnapshotDataID
           INNER JOIN [ReportServerESellTempDB].dbo.TempCatalog AS C ON EC.ReportID = C.TempCatalogID
        WHERE
           ContextPath = @Path
        ...
        DELETE
        FROM
           [ReportServerESellTempDB].dbo.TempCatalog
        WHERE
            ContextPath = @Path
