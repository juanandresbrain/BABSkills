...
flowchart LR
    ...
    work_ecp_period_export2(["work_ecp_period_export2"]) --> SP
```
...
| work_ecp_period_export2 |

## Stored Procedure Code

```sql
create proc dbo.ecp_period_export2_$sp    @pay_period_export_date	datetime,
...
AS
/* 
Proc Name: ecp_period_export2_$sp 
...
BEGIN TRANSACTION
INSERT into work_ecp_period_export2(
...
SELECT convert(nvarchar, @pay_period_export_date, 101),
       em.PRMY_ORG_CHN_NUM, 
       q.employee_no,
       SUM(CASE WHEN etr.commission_export_column_no = 1 THEN q.commission_amount ELSE 0 END),
       SUM(CASE WHEN etr.commission_export_column_no = 2 THEN q.commission_amount ELSE 0 END),
       SUM(CASE WHEN etr.commission_export_column_no = 3 THEN q.commission_amount ELSE 0 END),
       SUM(CASE WHEN etr.commission_export_column_no = 4 THEN q.commission_amount ELSE 0 END),
 ...
 em.PRMY_ORG_CHN_NUM,
       q.employee_no
HAVING SUM(CASE WHEN etr.commission_export_column_no = 1 THEN q.commission_amount ELSE 0 END) <> 0
    OR SUM(CASE WHEN etr.commission_export_column_no = 2 THEN q.commission_amount ELSE 0 END) <> 0
    OR SUM(CASE WHEN etr.commission_export_column_no = 3 THEN q.commission_amount ELSE 0 END) <> 0
    OR SUM(CASE WHEN etr.commission_export_column_no = 4 THEN q.commission_amount ELSE 0 END) <> 0
 ORDER BY em.PRMY_ORG_CHN_NUM, q.employee_no
