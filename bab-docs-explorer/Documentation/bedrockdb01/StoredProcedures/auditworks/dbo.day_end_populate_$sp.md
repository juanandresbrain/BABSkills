...
Nov24,16 Serguei DAOM-1775 Handle possibility of store/date becoming unaccepted and/or locked by another function in between population of t_store_date_populate and lock of store/date.
                           Don't process accepted store/dates that are still in the midst of trickling in (in a a trickle audit environment store/dates are not locked and more data could be arriving after an auto-accept).
                           Since functions other than Edit hold short locks which might be released before dayend populate tries to grab its lock, don't check until then.
                           Note:  Normally the only function that would have a lock on a store/date with status 300=Accepted is the "side-affected store/date" handling of the media reconciliation posting, or an Edit of a previously auto-accepted date. Other functions downgrade status before starting.
                           Note:  Noticed that store/date being locked with 20 = Subledger and is unlocked by subledger posting, probably to let media rec "side-affected store/date" evaluations (which included dayended store/dates) move on earlier.
...
BEGIN
  ...
  --Populate the temporary table with the dayend_workload info.  Note locks will be checked later on.
  ...
  BEGIN
    ...
    SELECT
     ...
     WHERE s.store_audit_status = 300
       AND s.trickle_in_progress_flag = 0  --Edit doesn't downgrade status below 300 in trickle audit environment as of SP12
    ...
    BEGIN
      IF NOT EXISTS (SELECT 1 
        ...
        WHERE ifs.segment_id = oim.base_segment_id
		  ...
		  SELECT @errmsg = 'Unable to delete #store_date_excluded',
				 @object_name = '#store_date_excluded',
				 @operation_name = 'DELETE';      
      DELETE #store_date_excluded;
  ...
  WHILE @try_again = 1  --Since it is not ideal that a minor function like 'Media Rec for side-affected dates' might prevent a store/date from being dayended, give it a minute to release its lock and try again.
	BEGIN
		select @try_again = 0;
		...
		UPDATE store_audit_status
		 ...
		 WHERE EXISTS( SELECT 1 FROM dayend_workload dew
			 WHERE store_no = dew.store_no AND sales_date = dew.sales_date AND date_reject_id = dew.date_reject_id AND
			 batch_datetime = @current_date)
		   AND store_audit_status = 300
		   AND update_in_progress = 0  --note:  the temporary lock used by media rec when it reads the store/date over/short etc to see if it has changed can prevent the store/date from being picked up by the dayend which isn't ideal, hence the loop.
		   AND trickle_in_progress_flag = 0;  --Reminder:  Edit doesn't downgrade auto-accepted status down from 300 in trickle audit environment if it has no issues as of SP12
	    
		SELECT @lock_count = @lock_count + @@rowcount;

      IF @lock_count < @batch_count  --some store/dates in dayend_workload are not available 
        IF @wait_count < 1
		  BEGIN
			SELECT @wait_count = @wait_count + 1,
				@try_again = 1;
			WAITFOR DELAY '00:01'; -- wait 60 seconds in the hopes the locks will be released
		  END;
        ELSE  --already waited so remove unlockable store/dates from work list and move on with rest
			BEGIN
				SELECT @errmsg = ' Unable to clean up list of stores/dates which could not be dayended.  ',
					@object_name = '#store_date_excluded',
					@operation_name = 'DELETE ';
	          
				DELETE #store_date_excluded;

				SELECT @errmsg = ' Unable to determine which stores/dates could not be dayended.  ',
					@object_name = '#store_date_excluded',
					@operation_name = 'INSERT';
	          
				INSERT INTO #store_date_excluded (store_no, transaction_date)
				SELECT DISTINCT sas.store_no, sas.sales_date
				  FROM store_audit_status sas
				 WHERE EXISTS( SELECT 1 FROM dayend_workload
					 WHERE batch_datetime = @current_date AND
					 sas.store_no = store_no AND sas.sales_date = sales_date AND sas.date_reject_id = date_reject_id )
				AND sas.store_audit_status <> @new_store_audit_status;
			

				SELECT @errmsg = ' Unable to list store dates skipped by DayEnd as a result of being locked by another function or no longer accepted.  ',
					@object_name = 'process_error_log',
					@operation_name = 'INSERT';
				...
				SELECT @function_no,
					   ...
					   @process_id,
					   0,
					   'Information:  Dayend is skipping store ' + CONVERT(VARCHAR, sde.store_no) + ' , date ' + CONVERT(VARCHAR, sde.transaction_date) + ' due to posting in progress by another function.  ',
					   NULL,
					   201687,
					   @process_name,
					   'store_audit_status',
					   ...
					   sde.transaction_date,
					   1
				  FROM #store_date_excluded sde WITH (NOLOCK);

				SELECT @errmsg = ' Unable to delete excluded store/dates from dayend_workload.  ',
					@object_name = 'dayend_workload',
					@operation_name = 'DELETE';
	                      
				DELETE w from dayend_workload w
				   WHERE w.batch_datetime = @current_date AND
						EXISTS (SELECT 1 FROM #store_date_excluded sde WITH (NOLOCK)
								WHERE sde.store_no = w.store_no
								  AND sde.transaction_date = w.sales_date);
			END;  --ELSE of IF @wait_count < 1
    END; --WHILE @try_count < 2 AND @wait_count = 0
