...
HISTORY:
Date     Name         Def#  Desc
Sep13,17 Sean/TerriDAOM-2537Removed partition check that executes purge_sa_history_$sp since currently only archive tables are partitioned 
                            and not if nor media rec tables.
...
/* Check whether partitioning of archive is turned on */ 
...
/* If not using if and media rec partitioning, then purge media rec tables and sa history tables now.
   Otherwise the scheduled job partition_maint_if_susm_$sp will call it. 
   IF COALESCE(@partitioning_in_use,0) = 0 */
   
IF 1=1 -- Temporary Marker For Above Commented Statement
