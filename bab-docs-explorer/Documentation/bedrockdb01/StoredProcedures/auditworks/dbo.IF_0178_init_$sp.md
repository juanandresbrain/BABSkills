...
```sql
create proc dbo.IF_0178_init_$sp
@batch_size int 
AS
...
/* TRUNCATE Ex_Work_0178 */
TRUNCATE TABLE Ex_Work_0178
SELECT @errno = @@error 
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to Truncate Ex_Work_0178 table'
   GOTO error
   END
...
EXEC auditworks.dbo.Ex_GetMaxQueue 27, @min_serial_no, @batch_size, @max_serial_no OUTPUT
...
INSERT INTO auditworks.dbo.Ex_Work_0178
SELECT serial_no, key_1
  FROM auditworks.dbo.Ex_Queue
  WHERE queue_id = 27 AND 
        serial_no >= @min_serial_no AND 
        serial_no <= @max_serial_no AND
        (used_by_object_id = 178 OR
