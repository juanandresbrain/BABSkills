...
```sql
create proc dbo.IF_16131_init_$sp
@batch_size int 
AS
...
/* TRUNCATE Ex_Work_16131 */
TRUNCATE TABLE Ex_Work_16131
SELECT @errno = @@error 
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to Truncate Ex_Work_16131 table'
   GOTO error
   END
...
EXEC auditworks.dbo.Ex_GetMaxQueue 52, @min_serial_no, @batch_size, @max_serial_no OUTPUT
...
INSERT INTO auditworks.dbo.Ex_Work_16131
SELECT serial_no, key_1, key_2, key_9, key_10
  FROM auditworks.dbo.Ex_Queue
  WHERE queue_id = 52 AND 
        serial_no >= @min_serial_no AND 
        serial_no <= @max_serial_no AND
        (used_by_object_id = 16131 OR
