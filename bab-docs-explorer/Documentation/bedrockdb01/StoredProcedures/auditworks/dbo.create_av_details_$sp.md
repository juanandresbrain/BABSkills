...
HISTORY
Date     Name         Def# Desc
Jul17,18 Kiri    DAOM-3676 Update error handling logic till the end of SP
...
END TRY
BEGIN CATCH
	SELECT @errno = ERROR_NUMBER(), @errline = ERROR_LINE(), @errmsg2 = ERROR_MESSAGE();
END CATCH
...
error:   /* Common error handler */
    ...
    SELECT @errmsg = CONVERT(nvarchar, @errno) + '-' + @process_name + ':' + CONVERT(nvarchar, @errline) + ':'
               + COALESCE(@errmsg, ' ') + ':' + COALESCE(@errmsg2, ' ');
