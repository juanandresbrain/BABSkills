# dbo.transl_auto_complete_order_$sp
...
```mermaid
flowchart LR
    SP["dbo.transl_auto_complete_order_$sp"]
    ORG_CHN(["ORG_CHN"]) --> SP
...
```sql
CREATE proc  dbo.transl_auto_complete_order_$sp AS
/* 
PROC NAME: transl_auto_complete_order_$sp
     ...
     DESC: Determines whether any transactions require auto-completion-from-order-item-SKU-lookup (presence of line_object = -5)
           ...
           The input tables would previously have been populated by import_preloaded_input_$sp in the case of ES (see proc desc for details).        
...
Jun05,17 Sab/Kiri DAOM-2407 Support pickup of multiple ES orders in a single transaction
...
May22,15  Vicci  TFS-122962 Log subtotal discount Coupon redemptions each on a separate line with its own coupon# specified 
...
SELECT del.store_no, del.register_no, del.entry_date_time, del.transaction_series, del.transaction_no, del.line_id, 
       COALESCE(sign(m.line_id), 0) merch_flag 
...
BEGIN
  ...
  DELETE transl_stock_control_detail
   ...
   WHERE transl_stock_control_detail.store_no = del.store_no
     ...
     AND (del.merch_flag = 0 OR transl_stock_control_detail.display_def_id NOT IN (53, 68, 71, 72))  --122171 retain ref-type/refno info (gift cards) and 138105 shipment info (such as tracking ID) 
     AND (transl_stock_control_detail.line_id <> 0 OR transl_stock_control_detail.display_def_id NOT IN (71, 72)); --DAOM-1288 retain government transaction reference info (only supplied by POS not ES, at header level).
...
LEFT OUTER JOIN av_customer cq WITH (NOLOCK)
        ...
        INNER JOIN av_customer c WITH (NOLOCK)
           ...
           AND q.cust_line_id = c.line_id
           AND q.customer_role = c.customer_role
	WHERE NOT EXISTS (SELECT 1 from transl_customer tc
					   WHERE q.store_no = tc.store_no
						 AND q.register_no = tc.register_no
						 AND q.entry_date_time = tc.entry_date_time
						 AND q.transaction_series = tc.transaction_series
						 AND q.transaction_no = tc.transaction_no
						 AND CASE WHEN c.line_id = 0 THEN 0 ELSE q.item_line_id END	 = tc.line_id
						 AND c.customer_role = tc.customer_role)
  GROUP BY q.store_no,
       q.register_no,
       q.entry_date_time,
       q.transaction_series,
  ...
  SELECT q.store_no,
    ...
    FROM (SELECT wq.store_no, wq.register_no, wq.entry_date_time, wq.transaction_series, wq.transaction_no, wq.item_line_id,
         ...
         INNER JOIN av_customer_detail c WITH (NOLOCK)
           ...
           AND q.cust_line_id = c.line_id
           AND q.customer_role = c.customer_role
	WHERE NOT EXISTS (SELECT 1 from transl_customer_detail tc
					   WHERE q.store_no = tc.store_no
						 AND q.register_no = tc.register_no
						 AND q.entry_date_time = tc.entry_date_time
						 AND q.transaction_series = tc.transaction_series
          ...
          FROM (SELECT wq.originating_transaction_id, wq.originating_line_id, 
              ...
              MAX(COALESCE(cq.line_id, cql.line_id, cqh.line_id)) cust_line_id
               ...
               INNER JOIN customer c WITH (NOLOCK)
                 ...
                 AND q.customer_role = c.customer_role
			WHERE NOT EXISTS (SELECT 1 from transl_customer tc
					   WHERE tc.store_no = @store_no 
						 AND tc.register_no = @register_no 
						 AND tc.entry_date_time = @entry_date_time
						 AND tc.transaction_series = @transaction_series
						 AND tc.transaction_no = @transaction_no
						 AND tc.line_id = @max_line_id
						 AND tc.customer_role = c.customer_role)
        GROUP BY c.customer_role;
      END;
      ELSE
      BEGIN
