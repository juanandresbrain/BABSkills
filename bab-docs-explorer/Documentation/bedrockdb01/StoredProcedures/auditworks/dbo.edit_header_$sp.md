...
flowchart LR
    ...
    edit_OMS_trans_rcpt_ack(["edit_OMS_trans_rcpt_ack"]) --> SP
...
```sql
CREATE proc  dbo.edit_header_$sp @process_id binary(16),
@user_id	int,
...
HISTORY:  
Date     Name           Def# Desc
--Uncomment this line below for MSSQL2008+  SELECT @process_start_time_formatted = SYSDATETIMEOFFSET();
Sept18,17 NLM	   DAOM-2854 SA 5.1 Open to Receive Date not working as expected 
Jan20,17 Kiri      DAOM-1954 Add multiple EOM instances support for JSON event acknowlegement
Aug19,16 Vicci     DAOM-1293 Correct inconsistency in audit_status.completion_date_time setting: was always being set for register closeouts as it should
                             but was also being set for whatever registers happenned to be in the same batch as a store closeout (i.e. randomly set in a trickle environment). 
Jul12,16 Vicci      DAOM-914 Export list of transaction received from OMS to OMS if that interface is active.
May12,16 Vicci      DAOM-730 Since @errmsg3 is being passed to the create_register_status_$sp, check @errmsg3 not @errmsg to determine the output.
                             Receive register_poll_id back from create_register_status_$sp (if log parent as missing child as unused option is selected).
Mar08,16 Vicci        DAOM-2 Mark duplicates as verified if auto_verify_duplicate_trans parameter is on.
...
DECLARE
	...
	@auto_verify_duplicate_trans	tinyint,
	@register_poll_id		nvarchar(15),
	@process_start_time		datetime,
	@process_start_time_formatted	nvarchar(34);
...
SELECT  @edit_progress_flag = 0,
	...
	@process_start_time_formatted = convert(nvarchar(34),getutcdate(), 127) + '0000 +00:00',  --this is just to support SQL2005, for SQL2008+ it will be reset in next statement
	@message_id = 201068;

--Uncomment this line for MSSQL2008+
  SELECT @process_start_time_formatted = SYSDATETIMEOFFSET();

  SELECT @process_start_time_formatted = SUBSTRING(@process_start_time_formatted, 1, 10) + 'T' + SUBSTRING(@process_start_time_formatted, 12, 23)
...
SELECT @operation_name = 'SELECT',
       @errmsg         = 'Failed to determine whether or not auto-verify duplicate transactions.  ',
       @object_name    = 'auditworks_parameter';
SELECT @auto_verify_duplicate_trans = 1
  FROM auditworks_parameter
 WHERE par_name = 'auto_verify_duplicate_trans'
   AND par_value = '1';
IF COALESCE(@auto_verify_duplicate_trans, 0) <> 1 
  SELECT @auto_verify_duplicate_trans = 0;
...
SELECT
	...
	CONVERT(smalldatetime, CONVERT(nchar(8),th.entry_date_time,112)), -- removes time
	0,
 ...
 IF @errno != 0
    BEGIN
     IF @errno = 2601 /* trap duplicates. index x1 is dropped by edit_post_$sp before this insert and recreated afterwards */
       BEGIN
	SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && removing duplicated transactions : ' + CONVERT(nchar, getdate(), 8);
	PRINT @trace_msg;
...
INSERT INTO #edit_hdr_date_store_reg (
  ...
  reg_post_midnight_time )
SELECT DISTINCT
  CONVERT(smalldatetime, CONVERT(nchar(8), DATEADD(dd, -1, entry_date_time), 112)),
  store_no,
  register_no,
  date_reject_id,
  0,
...
-- Get register closeout from audit_status and set it in current batch
...
UPDATE #edit_hdr_date_store_reg
 ...
 WHERE e.store_no = a.store_no
  ...
  -- Get register closeout from current batch
    SELECT @errmsg = 'from work_tran_header_edit, for closeout_flag = 1',
...
IF @scaleout_flag = 1
  BEGIN
	SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && executing scaleout_populate_dw_tb_sp : ' + CONVERT(nchar, getdate(), 8);
	PRINT @trace_msg;
   ...
   BEGIN
	...
	SELECT @errmsg = 'Failed to EXECUTE scaleout_populate_dw_tb_$sp',
     ...
     SET @sql_command = N'EXEC ' + @dblink_name + '.' + @db_name + N'.dbo.scaleout_populate_dw_tb_$sp ' + 
		   CONVERT(nvarchar(14),@instance_id) + N',' + NCHAR(39) + CONVERT(nvarchar(30),@tran_date) + NCHAR(39);
...
IF @@fetch_status <> 0
  BREAK;
        ...
        IF @errmsg3 like '%status already exists%'
          SELECT @status_already_existed = 1;
        ELSE
          SELECT @status_already_existed = 0;

        IF @errmsg3 like '%register_poll_id=%'
          SELECT @register_poll_id = SUBSTRING(@errmsg3, charindex('register_poll_id=', @errmsg3) + 17, 15);
        ELSE
   ...
   IF @transaction_count != @duplicate_count /* not all tran in batch are duplicates */
     BEGIN
      ...
      SELECT
		...
		cashier_no,
		transaction_category,
		@auto_verify_duplicate_trans,
		CASE WHEN @auto_verify_duplicate_trans = 1 THEN getdate() ELSE NULL END,
		CASE WHEN @auto_verify_duplicate_trans = 1 THEN -1 ELSE NULL END
        FROM work_tran_header_edit WITH (NOLOCK)
	WHERE duplicate_flag = 1;
...
ELSE
  BEGIN
	...
	SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && evaluating scaleout ownership : ' + CONVERT(nchar, getdate(), 8);
	PRINT @trace_msg;
...
BEGIN
  ...
  IF @msg_counter <= 10
    BEGIN
	SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && Moving store ' + CONVERT(nvarchar,@store_no) + ' : ' + CONVERT(nchar, getdate(), 8);
	PRINT @trace_msg;
   END;
...
--If OMS export is active, log list of OMS transactions
IF NOT EXISTS (SELECT 1 FROM export_OMS_interface)
BEGIN
  SELECT @errmsg = 'Failed to log list of OMS transactions received.  ',
       @object_name = 'edit_OMS_trans_rcpt_ack',
       @operation_name = 'INSERT';
  IF EXISTS (SELECT 1 FROM interface_directory WHERE interface_id = 97 AND update_timing > 0 AND ascii_export > 0 AND (live_date IS NULL OR live_date < getdate()))
  BEGIN
    -- export_OMS_interface not defined then defaults to interfave_id 97
    INSERT into edit_OMS_trans_rcpt_ack(
         edit_timestamp, 
         edit_stream_no, 
         store_no, 
         register_no, 
    ...
    SELECT edit_timestamp, 
         ...
         @process_start_time_formatted
      FROM work_tran_header_edit
     WHERE pos_transaction_series = 'OMS';
  END;
  IF EXISTS (SELECT 1 FROM edit_OMS_trans_rcpt_ack)
  BEGIN   
    SELECT @errmsg = 'Failed to log time of last update to list of OMS transactions received.  ',
         @object_name = 'interface_status',
         @operation_name = 'UPDATE'; 
    UPDATE interface_status
       SET last_posting_datetime = getdate()
     WHERE interface_id = 97;
  END;
END
ELSE
BEGIN
  SELECT @errmsg = 'Failed to log list of OMS transactions received.  ',
       @object_name = 'edit_OMS_trans_rcpt_ack',
       @operation_name = 'INSERT';
  IF EXISTS (SELECT 1 FROM interface_directory WHERE interface_id in (SELECT DISTINCT interface_id FROM export_OMS_interface) AND update_timing > 0 AND ascii_export > 0 AND (live_date IS NULL OR live_date < getdate()))
  BEGIN
    INSERT into edit_OMS_trans_rcpt_ack(
         edit_timestamp, 
         edit_stream_no, 
  ...
  BEGIN     
    ...
    UPDATE interface_status
       SET last_posting_datetime = getdate()
     WHERE interface_id in (SELECT DISTINCT interface_id 
		FROM export_OMS_interface ei,  edit_OMS_trans_rcpt_ack eo 
		    WHERE eo.store_no = ei.store_no);
  END;
END
