...
```sql
CREATE PROCEDURE [dbo].[coupon_report_journal]
  @BeginDate datetime, @EndDate datetime, @itemlist varchar(8000), @storelist varchar(8000)
AS
-- =====================================================================================================
-- Name: coupon_report_journal
--
-- Description:	
--
-- Input:	
--			date range and items to search for
--
-- Output: Resultset with the following columns:
--			transaction data for summary report
--
-- Dependencies: None
--
-- Revision History
...
-- exec coupon_report_journal '2010-08-01', '2010-08-05',  '000000003352,000000003353', '0001,0002,0003,0004'
-- =====================================================================================================
...
Create Table #TenderTypeMax (
StoreNo int,
TransNo int,
LineObjDesc varchar(255))

Create Table #TenderCount (
StoreNo int,
TransNo int,
NumTotal smallint,
Num690 smallint,
...
a.register_no AS RegNo, a.entry_date_time AS EntryDateTime,
a.cashier_no AS CashierNo, b.line_object_type AS LineObjType,
b.line_object AS LineObject,
c.line_object_description AS LineObjDesc
FROM auditworks.dbo.transaction_header a, auditworks.dbo.transaction_line b,
  auditworks.dbo.line_object c WITH (NOLOCK) 
WHERE a.transaction_id = b.transaction_id AND  
b.line_object = c.line_object AND
...
GROUP BY a.store_no, a.transaction_no, b.reference_no, a.transaction_date, b.line_id, b.line_action, 
a.register_no, a.entry_date_time, a.cashier_no, b.line_object_type, b.line_object,c.line_object_description
ORDER BY a.store_no, a.transaction_date, a.transaction_no, b.reference_no 
...
b.line_object AS LineObject,
c.line_object_description AS LineObjDesc
FROM auditworks.dbo.av_transaction_header a, auditworks.dbo.av_transaction_line b,
  auditworks.dbo.line_object c WITH (NOLOCK) 
...
WHERE  a.transaction_id = b.transaction_id AND 
  b.line_object = c.line_object AND
  a.transaction_void_flag = 0 AND b.line_void_flag=0 AND 
...
GROUP BY a.store_no, a.transaction_no, a.transaction_date, b.line_object_type, b.line_object, c.line_object_description
...
--Count the number of tender types that are or are not 690's for each transaction
Insert into #TenderCount
Select StoreNo, TransNo, Count(*) NumTotal,
  Sum(Case LineObject When 690 Then 1 Else 0 End) Num690,
  Sum(Case LineObject When 690 Then 0 Else 1 End) NumNot690 
From #RawST 
Where LineObjType = 6
Group by StoreNo, TransNo
Order by StoreNo, TransNo

--Create one tender type record per transaction for all of the details found
Insert into #TenderType
Select #RawST.StoreNo, #RawST.TransNo, 
  Case When #TenderCount.Num690 > 0 and #TenderCount.NumNot690 = 1 and #RawST.LineObject <> 690
    Then #RawST.LineObjDesc 
  When #TenderCount.Num690 > 0 and #TenderCount.NumNot690 = 1 and #RawST.LineObject = 690
    Then ''
  When #TenderCount.NumNot690 > 1 
    Then 'Split'
    Else #RawST.LineObjDesc End UseThisOne
From #RawST Inner Join #TenderCount on #RawST.StoreNo = #TenderCount.StoreNo
  and #RawST.TransNo = #TenderCount.TransNo
Where #RawST.LineObjType = 6 
Order by #RawST.StoreNo, #RawST.TransNo

Insert into #TenderTypeMax
Select StoreNo, TransNo, Max(LineObjDesc)
From #TenderType
Group by StoreNo, TransNo
...
Select #Raw.RefNo, #Item.Desc1, #Item.Desc2, #Item.DCS, #Item.VC, #Raw.StoreNo, 
  ...
  #Raw.RegNo, 
  Substring(Convert(char(20),#Raw.EntryDateTime,0),13,5) + ' ' + Substring(Convert(char(20),#Raw.EntryDateTime,0),18,2) EntryDateTime,
  #Raw.CashierNo, IsNull(#TenderTypeMax.LineObjDesc, ' -') TenderType
From #Raw
...
Left Join #TenderTypeMax on #Raw.StoreNo = #TenderTypeMax.StoreNo and #Raw.TransNo = #TenderTypeMax.TransNo
--Order by #Raw.RefNo, #Raw.StoreNo, #Raw.TransNo
Order by --#Raw.RefNo, 
  #Raw.StoreNo, #Raw.TransDate, #Raw.TransNo, #Raw.LineID
...
drop table #SalesTax
drop table #TenderType
drop table #TenderCount
drop table #TenderTypeMax
```
