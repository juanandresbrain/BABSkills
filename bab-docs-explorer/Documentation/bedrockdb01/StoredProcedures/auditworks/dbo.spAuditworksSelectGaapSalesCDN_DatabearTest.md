# dbo.spAuditworksSelectGaapSalesCDN_DatabearTest
...
```mermaid
flowchart LR
    SP["dbo.spAuditworksSelectGaapSalesCDN_DatabearTest"]
...
```sql
CREATE proc [dbo].[spAuditworksSelectGaapSalesCDN_DatabearTest]
as
-- =============================================================================================================
-- Name: spAuditworksGaapSalesCDN
--
-- Description:	captures Gaap Sales from Auditworks for US stores.
--				Modified version of usp_GAAPSales_CDN, which called sv_store view. 
...
-- exec spAuditworksSelectGaapSalesCDN_DatabearTest
-- =============================================================================================================

set nocount on

IF (Object_ID('tempdb..#CDNGaapSalesAW') IS NOT NULL) DROP TABLE #CDNGaapSalesAW
	create table #CDNGaapSalesAW
(
...
insert #CDNGaapSalesAW
    SELECT  sa.store_no location_code, 
        ...
        left join 
			( SELECT    a.store_no,
              ...
              WHERE     ( a.transaction_date BETWEEN CONVERT(CHAR, GETDATE() - 3, 101)
                                             AND     CONVERT(CHAR, GETDATE() - 3, 101)
    ...
    WHERE   ss.gl_company IN ( 300 ) -- CDN stores only
...
insert #tmp_VAT
    ...
    WHERE   ( h.transaction_date BETWEEN CONVERT(CHAR, GETDATE() - 3, 101)
                                 AND     CONVERT(CHAR, GETDATE() - 3, 101)
			...
			AND h.store_no in (select location_code from  #CDNGaapSalesAW)
    GROUP BY h.store_no
    ORDER BY h.store_no
