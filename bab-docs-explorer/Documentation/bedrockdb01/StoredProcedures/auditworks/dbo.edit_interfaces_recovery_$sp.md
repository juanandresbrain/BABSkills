# dbo.edit_interfaces_recovery_$sp
...
```mermaid
flowchart LR
    SP["dbo.edit_interfaces_recovery_$sp"]
...
```sql
CREATE proc [dbo].[edit_interfaces_recovery_$sp] @edit_timestamp 	float,
...
/*
NAME: edit_interfaces_recovery_$sp
DESCRIPTION:
	Updates preaudit interfaces and creates interface rejections when necessary (uses tran detail tables).
	Attempts to rollforward interface posting after an error occurs in edit_interfaces_$sp (previous batch).
	Called from edit_post_$sp.

HISTORY:
Date     Name		Def# Desc
Nov20,17 Kiri	   DAOM-2815 Store number not being updated correctly on IF rejected trans
Jan26,17 Kiri      DAOM-1856 retrofit from Oracle: Bug in Edit Interfaces which causes the UPC revalidation not to kick in for non-pre/post-audit interfaces
...
May31,12 Vicci        135045 Add date_reject_id to if_transaction_header insert since default missing.
...
Jul21,09 Vicci        109078 Add missing source_store_no, fulfillment_store_no, track_tax
...
Oct04,06 Paul          77922 apply 77650, 7740 to SA5
Jul27,06 Tim           69753 Uplift defect 70769 to SA5
...
Apr29,05 Maryam      DV-1202 Insert if_transaction_line_link. Use line_id.
				expand transaction_id to use tran_id_datatype, new i/f reject type 112 (Paul)
...
Nov18,03 Phu           15801 Populate sku_id, reason, imrd, style_reference_id, display_def_id
...
Jun27,02 Phu         1-C9P5S Create entry in if_tax_detail
Jun26,02 Paul        1-DS8TD insert tax_override_flag with zero
Mar22,02 Paul        1-BUVZ9 remove commented out code
Nov26,01 Winnie	     1-969YY Add logic for R3 error handling to pass @edit_process_no to procedure
Nov01,01 Paul           8900 Author,TRANSL edit changes for Sybase, insert cashier into corrected_upc_detail
*/
...
SELECT @process_start_time = getdate(),
	...
	@process_name = 'edit_interfaces_recovery_$sp',
	@message_id = 201068;
...
CREATE TABLE #edit_if_header(transaction_id numeric(14,0) not null, -- tran_id_datatype
                             entry_date_time datetime not null,
...
DELETE work_interface_reject_edit
  FROM work_interface_reject_edit ir, transaction_header wh WITH (NOLOCK)
...
/* Don't create if rejects for void lines */
   SELECT @errmsg = 'Failed to delete work_interface_reject_edit (void lines)',
          @object_name = 'work_interface_reject_edit';
DELETE work_interface_reject_edit
  FROM work_interface_reject_edit ir, transaction_line tl WITH (NOLOCK)
  ...
  SELECT DISTINCT wh.transaction_id,
    ...
    FROM work_tran_header_edit wh WITH (NOLOCK)
         INNER JOIN transl_transaction_line tl WITH (NOLOCK)
           ON wh.transaction_id = tl.transaction_id
...
BEGIN
  ...
  SELECT DISTINCT wh.transaction_id,
	...
	0
    FROM work_tran_header_edit wh WITH (NOLOCK),
     transaction_line tl WITH (NOLOCK), interface_applicability_mass ia
    WHERE (wh.transaction_void_flag IN (0,8) OR ia.interface_voided_transactions = 1)
      ...
      AND tl.line_action = ia.line_action
      AND wh.transaction_category = ia.transaction_category
      AND (ia.live_date IS NULL OR wh.transaction_date >= ia.live_date);
END;

/*    mass insert interfaces with applicability_method > 0
      always insert for interefaces with applicability_method = 1 (all transactions apply).
...
INSERT work_interfaces_edit (
 ...
 SELECT wt.transaction_id,
  ...
  WHERE id.update_timing >= 1
    ...
    AND (id.applicability_method = 1 OR id.interface_id IN (SELECT DISTINCT interface_id
...
BEGIN
 ...
 @operation_name = 'INSERT';
  ...
  SELECT DISTINCT cl.transaction_id,
	...
	@update_timing,
	@applicability_method
    FROM #cps_transaction_list cl WITH (NOLOCK), customer c WITH (NOLOCK)
...
BEGIN
  ...
  UPDATE #settlement_list
   ...
   AND (sd.store_live_date IS NULL OR sl.transaction_date >= sd.store_live_date)))));
     ...
     SELECT @errmsg = 'Failed to delete reject type 112',
            @object_name = 'work_interface_reject_edit',
      @operation_name = 'DELETE';
  DELETE work_interface_reject_edit -- remove rows from previous abort of this proc
   WHERE if_reject_reason = 112;
  ...
  SELECT DISTINCT 112, wt.transaction_id, 0
    FROM #settlement_list sl, work_tran_header_edit wt WITH (NOLOCK)
...
IF @rows <= 0
   BEGIN
      SELECT @errmsg = 'Failed to remove entries for applicability_method > 1',
            @object_name = 'work_interfaces_edit',
            @operation_name = 'DELETE';
...
ELSE --IF @rows > 0   -- some txns are affected by if rejects
BEGIN
  ...
  SELECT @errmsg = 'Failed to remove entries for applicability_method > 1 AND interface_status_flag <> 99',
         @object_name = 'work_interfaces_edit',
         @operation_name = 'DELETE';        
  DELETE work_interfaces_edit
  WHERE applicability_method > 1
...
SELECT
	...
	wh.till_no,
	@edit_process_no
   FROM #edit_transaction_list e WITH (NOLOCK), transaction_header wh WITH (NOLOCK)
...
INSERT INTO #edit_if_header
       (transaction_id, if_entry_no, transaction_date, entry_date_time)
SELECT	wh.transaction_id, if_entry_no,	wh.transaction_date, wh.entry_date_time
...
SELECT
	...
	customer_signature_obtained,
	offline_flag 
  FROM #edit_if_header ih WITH (NOLOCK), authorization_detail ad WITH (NOLOCK)
...
SELECT
	...
	fax,
	email_address
  FROM #edit_if_header ih WITH (NOLOCK), customer cu WITH (NOLOCK)
...
SELECT
	...
	customer_info_type,
	customer_info
  FROM #edit_if_header ih WITH (NOLOCK) , customer_detail cd WITH (NOLOCK)
...
SELECT
	...
	applied_flag,
	pos_discount_serial_no
  FROM #edit_if_header ih WITH (NOLOCK), discount_detail dd WITH (NOLOCK)
...
SELECT 
	...
	note_type,
	line_note 
  FROM #edit_if_header ih WITH (NOLOCK) , line_note ln WITH (NOLOCK)
...
SELECT
	...
	originating_store_no,
     	...
     	fulfillment_store_no,
     	cost
  FROM #edit_if_header ih WITH (NOLOCK), merchandise_detail md WITH (NOLOCK)
...
SELECT
	...
	employee_type,
	payroll_entry_type
  FROM #edit_if_header ih WITH (NOLOCK), payroll_detail pd WITH (NOLOCK)
...
SELECT
	...
	post_void_successful,
	post_void_reason_code
  FROM #edit_if_header ih WITH (NOLOCK), post_void_detail pv WITH (NOLOCK)
...
SELECT
	...
	return_from_transno,
	without_receipt_flag
  FROM #edit_if_header ih WITH (NOLOCK), return_detail rd WITH (NOLOCK)
...
SELECT
	...
	spo_class_description,
	vendor_no
  FROM #edit_if_header ih WITH (NOLOCK), special_order_detail sd WITH (NOLOCK)
...
SELECT
	...
	style_reference_id,
	store_on_file_flag
  FROM #edit_if_header ih WITH (NOLOCK), stock_control_detail sc WITH (NOLOCK)
...
SELECT
	...
	reference_no,
	unit_of_measure
  FROM #edit_if_header ih WITH (NOLOCK), transaction_line tl WITH (NOLOCK)
...
SELECT
  ...
  WHERE ih.transaction_id = tod.transaction_id;
   ...
   SELECT @errmsg = 'Failed to insert tax_detail',
...
SELECT
	...
	line_id,
	linked_line_id
  FROM #edit_if_header ih WITH (NOLOCK), transaction_line_link k WITH (NOLOCK)
  WHERE ih.transaction_id = k.transaction_id;
...
IF @auto_upc_correction >= 1
  BEGIN
   ...
   SELECT
		...
		pos_identifier_type,
		th.cashier_no
     FROM work_upc_trail_edit wt WITH (NOLOCK), transaction_header th WITH (NOLOCK), merchandise_detail md WITH (NOLOCK)
