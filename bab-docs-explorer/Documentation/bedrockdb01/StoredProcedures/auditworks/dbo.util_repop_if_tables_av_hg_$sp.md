# dbo.util_repop_if_tables_av_hg_$sp
...
```mermaid
flowchart LR
    SP["dbo.util_repop_if_tables_av_hg_$sp"]
...
```sql
CREATE proc [dbo].[util_repop_if_tables_av_hg_$sp] 
...
SELECT	@interface_id = 26, -- change this
	@from_transaction_date = '05/19/2023', -- starting date
	@to_transaction_date = '05/19/2023', -- ending date or = getdate() if all
...
DECLARE reprocess_trans CURSOR FAST_FORWARD
  FOR
    SELECT DISTINCT ah.av_transaction_id
      FROM hg_temp ah /*av_transaction_header ah WITH (NOLOCK), av_interface_control ic
