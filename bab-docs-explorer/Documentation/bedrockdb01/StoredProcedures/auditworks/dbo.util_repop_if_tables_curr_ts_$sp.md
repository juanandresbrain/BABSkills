# dbo.util_repop_if_tables_curr_ts_$sp
...
```mermaid
flowchart LR
    SP["dbo.util_repop_if_tables_curr_ts_$sp"]
...
```sql
CREATE proc [dbo].[util_repop_if_tables_curr_ts_$sp]
  ...
  FOR
    SELECT DISTINCT ic.transaction_id
     ...
     WHERE ic.interface_id = @interface_id
	   ...
	   --AND th.transaction_id in (185168820,185168822)
FOR READ ONLY
