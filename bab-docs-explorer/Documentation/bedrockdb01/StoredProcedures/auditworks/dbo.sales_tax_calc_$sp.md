...
PROC NAME: sales_tax_calc_$sp
  ...
  HISTORY:
Date     Name           Def#  Desc
Sep28,16 Vicci     STOR-3540  When Tax Level B must be charged on a Tax-Level-A-inclusive-priced item but Tax Level B must be charged on the item amount EXLUDING tax, strip tax-level-A tax from item amount associated with tax-level-B before calculating tax-level-B amount.
                              Note:  relies on compound_order of Tax-Level-B being greater than that of Tax-Level-A (even though B is not charge on A) which may require facilitation in UI (currently have to turn ON tax-on-tax then turn it off again to achieve setting of compound level).
Aug15,16 Vicci      DAOM-344  Replace rounding cursor with temp table since although current 100,000 fetches would become 100,000 inserts (performance degradation), the current 50,000 updates of 1 row each would instead become 1 update of 50,000 rows (performance savings).
                              NOTE:  in-house the optimizer plan for this change was not especially pretty.  If no good in real life, try out part
                              commented out below under remark:  --DAOM-344 Another possibility
Apr20,16 Vicci      DAOM-344  Adding missing process_id = @process_id criteria to 2 work_tax_detail updates and 2 selects for penny round differences. 
Feb10,16 Vicci    TFS-156404  Give preference to tax lines with a non-zero amount when setting max_applied_by_line_id.
Feb03,16 Vicci    TFS-139985  Handle line amounts as large as numeric(18,4).
Jul31,15 Andre	      131151  Expand the colum length to support numeric(18,4).
JUL22,15 Daphna       131151  Expand the length of the column for tender total and amounts
Jul15,15 Vicci    TFS-128531  Modify statement setting work_tax_detail tax_item_group_id from tax_item to include extra join on UPC 
                              since user_upc table underlying the tax_item view is unique by UPC not by SKU and some MSSQL version were not using any index (S/B x4).
...
DECLARE
	...
	@compound_order			int,
	@max_compound_order		int,
	@cursor_open			tinyint,
	@temp_tb_created		tinyint,
	@transaction_id			tran_id_datatype,
	@line_id			numeric(5,0),
	@tax_level			tinyint,
	@tax_category			tinyint,
	@tax_paid_flag			tinyint,
	@tax_amount_collected		line_amount_18_4,
	@tran_penny_diff		numeric(18,2),
	@amount_adjustment		line_amount_18_4,
	@penny_diff_balance		line_amount_18_4,
	@start_loop 			tinyint,
	...
	UPDATE work_tax_detail
	  ...
	  FROM work_tax_detail wt
	       INNER JOIN tax_item ti
	        ...
	        AND wt.upc_no = ti.item_no  --extra join on upc required for X4 to be used in some MSSQL releases and since multiple tax item rows for the same sku may exist  (since underlying user_upc is unique by upc not sku)
...
SELECT @tax_on_tax_level_flag = SIGN(ISNULL(MAX(tax_on_tax_level), 0)),
       ...
       @max_compound_order = MAX(compound_order)
...
*/
  ...
  UPDATE work_tax_detail
     SET nontaxable_merchandise_amount = CASE WHEN tax_schedule_id IS NULL AND below_threshold_combined_rate = 0
         ...
         taxable_fee_amount = ROUND(SIGN(amount) * ABS((convert(numeric(18,4),(amount/(1+(IsNull(strip_rate, combined_tax_rate)/100)*SIGN(item_tax_strip_flag)) * tax_on_full_amount)) +
                                      (convert(numeric(18,4),(amount - CASE WHEN tax_schedule_id IS NULL THEN (threshold_amount*units) ELSE 0 END)/(1+(IsNull(strip_rate, combined_tax_rate)/100)*SIGN(item_tax_strip_flag))) * tax_on_threshold_excess))  * (1 - SIGN(ABS(line_object_type - 2 )))), 2),
         taxable_expense_amount = ROUND(SIGN(amount) * ABS((convert(numeric(18,4),(amount/(1+(IsNull(strip_rate, combined_tax_rate)/100)*SIGN(item_tax_strip_flag)) * tax_on_full_amount)) +
        (convert(numeric(18,4),(amount - CASE WHEN tax_schedule_id IS NULL THEN (threshold_amount*units) ELSE 0 END)/(1+(IsNull(strip_rate, combined_tax_rate)/100)*SIGN(item_tax_strip_flag))) * tax_on_threshold_excess)) * (1 - SIGN(ABS(line_object_type - 7 )))), 2),
         tax_amount_collected = SIGN(amount) * ABS(amount * (1 - SIGN(ABS(line_object_type - 5 ))))
   WHERE taxable = 1 
...
--STOR-3540 Remove VAT from taxable amount of other non-strip tax levels with higher compound order (Argentina IIBB on amount without IVA handling).
  IF @compound_order < @max_compound_order
  BEGIN
    UPDATE wtd --UPDATE work_tax_detail wtd
       SET amount = wtd.amount - strip.tax_amount_expected
      FROM (SELECT transaction_id, line_id, SUM(tax_amount_expected) tax_amount_expected
              FROM work_tax_detail
             WHERE process_id = @process_id
               AND line_object_type <> 5
               AND item_tax_strip_flag > 0
               AND tax_amount_expected <> 0
               AND compound_order = @compound_order
               AND taxable = 1
             GROUP BY transaction_id, line_id) strip
           INNER JOIN work_tax_detail wtd
              ON wtd.process_id = @process_id
             AND wtd.compound_order > @compound_order
             AND wtd.transaction_id = strip.transaction_id
             AND wtd.line_id = strip.line_id;
    SELECT @errno = @@error
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Failed to reduce item amount for other non-strip levels with higher compound order by the amount of tax stripped in current compound order.  ',
  	         @object_name = 'work_tax_detail',
             @operation_name = 'UPDATE'
      GOTO error
    END;
  END; --IF @compound_order < @max_compound_order
...
CREATE TABLE #line_tax_rate (
       ...
       total_tax_charged      numeric(18,4) not null,
       total_tax_refunded             numeric(18,4) not null,
       total_tax_expected_for_sold    numeric(18,4) not null,
       total_tax_expected_for_return  numeric(18,4) not null,
       total_taxable_sold             numeric(18,4) not null,
       total_taxable_returned         numeric(18,4) not null,
       taxable_items                  int    not null,
       tran_penny_diff                numeric(18,2) not null,
       has_tax_charged_line           int           not null,
       has_tax_refunded_line          int           not null,
       max_amount_with_line_id        numeric(24,0) null,
       max_non_tax_amt_with_line_id   numeric(24,0) null,
...
SELECT
  ...
  SUM (tax_amount_collected * SIGN(1 - tax_sign)), -- total_tax_charged
       ...
       MAX(CONVERT(NUMERIC(24, 0), amount * SIGN(ABS(5 - line_object_type)) * SIGN(ABS((taxable_merchandise_amount + taxable_fee_amount + taxable_expense_amount) * gl_effect))) * 100000 + line_id),
       MAX(CONVERT(NUMERIC(24, 0), amount * SIGN(ABS(5 - line_object_type)) * SIGN(ABS((nontaxable_merchandise_amount + nontaxable_fee_amount) * gl_effect))) * 100000 + line_id),

       CASE WHEN MAX(SIGN(ABS(amount)) * SIGN(1 - tax_sign) * line_id * (1 - SIGN(ABS(5 - line_object_type)))) <> 0
            THEN MAX(SIGN(ABS(amount)) * SIGN(1 - tax_sign) * line_id * (1 - SIGN(ABS(5 - line_object_type))))
            ELSE MAX(SIGN(1 - tax_sign) * line_id * (1 - SIGN(ABS(5 - line_object_type))))
       END max_tax_charged_line_id, -- max line_id of tax charged
       CASE WHEN MAX(SIGN(ABS(amount)) * SIGN(1 + tax_sign) * line_id * (1 - SIGN(ABS(5 - line_object_type)))) <> 0
            THEN MAX(SIGN(ABS(amount)) * SIGN(1 + tax_sign) * line_id * (1 - SIGN(ABS(5 - line_object_type))))
            ELSE MAX(SIGN(1 + tax_sign) * line_id * (1 - SIGN(ABS(5 - line_object_type))))
       END max_tax_refunded_line_id,  -- max line_id of tax refunded
       reference_type,
       reference_no
  FROM work_tax_detail WITH (NOLOCK)
 WHERE process_id = @process_id
...
BEGIN
  ...
  --case 40
  UPDATE work_tax_detail
     SET tax_amount_collected   = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.tax_amount_expected) / ABS(ltr.total_tax_expected_for_return - ltr.total_tax_expected_for_sold) )), 2) * (1 - max_tax_paid_flag),
         tax_amount_collected_unrounded   = CASE WHEN wt.line_object_type = 5 THEN 0 ELSE ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.tax_amount_expected) / ABS(ltr.total_tax_expected_for_return - ltr.total_tax_expected_for_sold) ))) * (1 - max_tax_paid_flag) END,
         tax_amount_paid        = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid      + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.tax_amount_expected) / ABS(ltr.total_tax_expected_for_return - ltr.total_tax_expected_for_sold) )), 2) * max_tax_paid_flag,
         max_applied_by_line_id = (  (ltr.max_tax_charged_line_id * SIGN(ABS(ltr.has_tax_charged_line))) 
                                   + (ltr.max_tax_refunded_line_id * SIGN(ABS(ltr.has_tax_refunded_line))))
  ...
  --case 50
    UPDATE work_tax_detail
       SET tax_amount_collected   = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.amount) / ABS(ltr.total_taxable_returned - ltr.total_taxable_sold) )), 2) * (1 - max_tax_paid_flag),
           tax_amount_collected_unrounded   = CASE WHEN wt.line_object_type = 5 THEN 0 ELSE ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.amount) / ABS(ltr.total_taxable_returned - ltr.total_taxable_sold) ))) * (1 - max_tax_paid_flag) END, 
           tax_amount_paid        = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid      + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.amount) / ABS(ltr.total_taxable_returned - ltr.total_taxable_sold) )), 2) * max_tax_paid_flag,
       ...
       SET tax_amount_collected
          = CASE WHEN wt.tax_sign = -1 
              THEN CASE WHEN ltr.total_taxable_sold <> 0 
                     THEN SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_charged) * wt.amount) / ABS(ltr.total_taxable_sold))), 2) * (1 - max_tax_paid_flag)
                   ELSE 0
   END
 ELSE CASE WHEN ltr.total_taxable_returned <> 0
                   THEN SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded) * wt.amount) / ABS(ltr.total_taxable_returned))), 2) * (1 - max_tax_paid_flag)
                 ELSE 0
                 END
            END,
     tax_amount_collected_unrounded
 = CASE WHEN wt.line_object_type = 5 THEN 0 
               ELSE 
                 CASE WHEN wt.tax_sign = -1 
                   THEN CASE WHEN ltr.total_taxable_sold <> 0 
                          THEN ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_charged) * wt.amount) / ABS(ltr.total_taxable_sold)))) * (1 - max_tax_paid_flag)
                        ELSE 0
                        END
                 ELSE CASE WHEN ltr.total_taxable_returned <> 0
                        THEN ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded) * wt.amount) / ABS(ltr.total_taxable_returned)))) * (1 - max_tax_paid_flag)
                      ELSE 0
                      END
                 END
               END,
         tax_amount_paid 
             = CASE WHEN wt.tax_sign = -1 
                 THEN CASE WHEN ltr.total_taxable_sold <> 0 
                        THEN SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_charged) * wt.amount) / ABS(ltr.total_taxable_sold))), 2) * max_tax_paid_flag
                      ELSE 0
                      END
               ELSE CASE WHEN ltr.total_taxable_returned <> 0
     THEN SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded) * wt.amount) / ABS(ltr.total_taxable_returned))), 2) * max_tax_paid_flag
                    ELSE 0
                    END
               END,
 ...
 AND (ROUND((ltr.total_tax_expected_for_return - ltr.total_tax_expected_for_sold), 2) = 0 -- total tax expected  --1-47UXP9
  ...
  BEGIN
    ...
    UPDATE work_tax_detail
       SET tax_amount_collected
         = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.amount) / ltr.total_taxable_returned )), 2) * (1 - max_tax_paid_flag),
           tax_amount_collected_unrounded
             = CASE WHEN wt.line_object_type = 5 THEN 0 
               ELSE ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.amount) / ltr.total_taxable_returned ))) * (1 - max_tax_paid_flag)
               END,
           tax_amount_paid
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.amount) / ltr.total_taxable_returned )), 2) * max_tax_paid_flag,
           max_applied_by_line_id
  ...
  BEGIN
    ...
    UPDATE work_tax_detail
       SET tax_amount_collected 
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.amount) / ltr.total_taxable_sold )), 2) * (1 - max_tax_paid_flag),
           tax_amount_collected_unrounded
             = CASE WHEN wt.line_object_type = 5 THEN 0
               ELSE ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.amount) / ltr.total_taxable_sold ))) * (1 - max_tax_paid_flag)
   END,
           tax_amount_paid
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid + CONVERT(NUMERIC(18,4), (ABS(ltr.total_tax_refunded - ltr.total_tax_charged) * wt.amount) / ltr.total_taxable_sold )), 2) * max_tax_paid_flag,
  ...
  BEGIN
    ...
    UPDATE work_tax_detail
       SET tax_amount_collected
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ltr.total_tax_charged * wt.tax_amount_expected) / ltr.total_tax_expected_for_sold)), 2) * (1 - max_tax_paid_flag),
     tax_amount_collected_unrounded
             = CASE WHEN wt.line_object_type = 5 THEN 0
               ELSE ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ltr.total_tax_charged * wt.tax_amount_expected) / ltr.total_tax_expected_for_sold))) * (1 - max_tax_paid_flag)
               END,
           tax_amount_paid
     = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid + CONVERT(NUMERIC(18,4), (ltr.total_tax_charged * wt.tax_amount_expected) / ltr.total_tax_expected_for_sold)), 2) * max_tax_paid_flag,
  ...
  BEGIN
    ...
    UPDATE work_tax_detail
      SET tax_amount_collected
     = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ltr.total_tax_refunded * wt.tax_amount_expected) / ltr.total_tax_expected_for_return)), 2) * (1 - max_tax_paid_flag),
           tax_amount_collected_unrounded
             = CASE WHEN wt.line_object_type = 5 THEN 0
               ELSE ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ltr.total_tax_refunded * wt.tax_amount_expected) / ltr.total_tax_expected_for_return))) * (1 - max_tax_paid_flag)
               END,
           tax_amount_paid
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid + CONVERT(NUMERIC(18,4), (ltr.total_tax_refunded * wt.tax_amount_expected) / ltr.total_tax_expected_for_return)), 2) * max_tax_paid_flag,
  ...
  BEGIN   
    ...
    UPDATE work_tax_detail
       SET tax_amount_collected
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ltr.total_tax_charged * wt.amount) / ltr.total_taxable_sold)), 2) * (1 - max_tax_paid_flag),
           tax_amount_collected_unrounded
  = CASE WHEN wt.line_object_type = 5 THEN 0
               ELSE ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ltr.total_tax_charged * wt.amount) / ltr.total_taxable_sold))) * (1 - max_tax_paid_flag)
               END,
           tax_amount_paid
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid + CONVERT(NUMERIC(18,4), (ltr.total_tax_charged * wt.amount) / ltr.total_taxable_sold)), 2) * max_tax_paid_flag,
           max_applied_by_line_id = ltr.max_tax_charged_line_id
      FROM #line_tax_rate ltr, work_tax_detail wt WITH (INDEX = work_tax_detail_x1)
     WHERE ltr.has_tax_charged_line > 0
       AND ltr.has_tax_refunded_line > 0
 ...
 -- Exchange (has tax collected and refunded), total tax refunded is prorated
  ...
  BEGIN
    ...
    UPDATE work_tax_detail
       SET tax_amount_collected
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ltr.total_tax_refunded * wt.amount) / ltr.total_taxable_returned)), 2) * (1 - max_tax_paid_flag),
           tax_amount_collected_unrounded
             = CASE WHEN wt.line_object_type = 5 THEN 0
               ELSE ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), (ltr.total_tax_refunded * wt.amount) / ltr.total_taxable_returned))) * (1 - max_tax_paid_flag)
               END,
           tax_amount_paid
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid + CONVERT(NUMERIC(18,4), (ltr.total_tax_refunded * wt.amount) / ltr.total_taxable_returned)), 2) * max_tax_paid_flag,
           max_applied_by_line_id = ltr.max_tax_refunded_line_id
      FROM #line_tax_rate ltr, work_tax_detail wt WITH (INDEX = work_tax_detail_x1)
     WHERE ltr.has_tax_charged_line > 0
       AND ltr.has_tax_refunded_line > 0
  ...
  BEGIN
    ...
    UPDATE work_tax_detail
       SET tax_amount_collected
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), ABS(ltr.total_tax_refunded - ltr.total_tax_charged) / ltr.taxable_items)), 2) * (1 - max_tax_paid_flag),
           tax_amount_collected_unrounded
             = CASE WHEN wt.line_object_type = 5 THEN 0
               ELSE ((wt.tax_amount_collected + CONVERT(NUMERIC(18,4), ABS(ltr.total_tax_refunded - ltr.total_tax_charged) / ltr.taxable_items))) * (1 - max_tax_paid_flag)
               END,
           tax_amount_paid
             = SIGN(ABS(5 - wt.line_object_type)) * ROUND((wt.tax_amount_paid + CONVERT(NUMERIC(18,4), ABS(ltr.total_tax_refunded - ltr.total_tax_charged) / ltr.taxable_items)), 2) * max_tax_paid_flag,
...
--Distribute difference between rounded and unrounded tax collected to as many lines as possible to avoid a single line being hit with an adjustment that would cause an expected vs collected variance when none exists in total.
  CREATE TABLE #tax_rounding_diff (
         process_id binary(16) NOT NULL,  --not needed but added to avoid weird optimizer decision to start with wtd instead of #trd
         row_id numeric(14,0) identity NOT NULL,
         transaction_id numeric(14,0) NOT NULL, --tran_id_datatype
         tax_level tinyint NOT NULL, 
         ...
         av_mod_flag tinyint NOT NULL,
         tax_paid_flag tinyint NULL, 
         tax_rate_code tinyint NOT NULL, 
         tax_sign smallint NOT NULL, 
         difference_in_pennies numeric(14,0) NOT NULL, 
         line_id numeric(5,0) NOT NULL,
         combined_rate numeric(6,4) NOT NULL)  
  SELECT @errno = @@error
  IF @errno <> 0
  BEGIN
    SELECT @errmsg = 'Failed to create table to hold list of rounding differences to be applied.  ',
           @object_name = '#tax_rounding_diff',
           @operation_name = 'CREATE'
    GOTO error
  END
  SELECT @temp_tb_created = 1
  
  INSERT INTO #tax_rounding_diff(process_id,
         transaction_id, tax_level, tax_jurisdiction, reference_type, reference_no, av_mod_flag,
         tax_paid_flag, tax_rate_code, tax_sign, 
         difference_in_pennies, line_id, combined_rate)
  SELECT q.process_id, q.transaction_id, q.tax_level, q.tax_jurisdiction, q.reference_type, q.reference_no, q.av_mod_flag,
         q.tax_paid_flag, q.tax_rate_code, q.tax_sign, 
         q.difference_in_pennies, w.line_id, w.combined_tax_rate
    FROM (SELECT wt.process_id, wt.transaction_id, wt.tax_level, wt.tax_jurisdiction, wt.reference_type, wt.reference_no,
           ...
           GROUP BY wt.process_id, wt.transaction_id, wt.tax_level, wt.tax_jurisdiction, wt.reference_type, wt.reference_no,
  ...
  IF @errno <> 0
  BEGIN
    SELECT @errmsg = 'Failed to build list of rounding differences to be applied.  ',
           @object_name = '#tax_rounding_diff',
           @operation_name = 'INSERT'
    GOTO error
  END

  IF @rows > 0 --rounding diff rows
  BEGIN
    CREATE UNIQUE INDEX t_tax_rounding_diff_x0
       ON #tax_rounding_diff (process_id, transaction_id, row_id)  --note, only row_id needed but rest added to influence optimizer
    SELECT @errno = @@error
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Failed to create index on #tax_rounding_diff (should not be necessary but needed to help optimizer).  ',
	     @object_name = 't_tax_rounding_diff_x0',
	     @operation_name = 'CREATE INDEX'
      GOTO error
    END
    
    UPDATE w --UPDATE work_tax_detail
       SET tax_amount_collected = tax_amount_collected - sign(r.difference_in_pennies)/100
      FROM (SELECT min(row_id) min_row_id, min(row_id) + difference_in_pennies - 1 max_row_id,
                   process_id, transaction_id --not needed but helps optimizer
              FROM #tax_rounding_diff
             WHERE process_id = @process_id  --not needed but clarifies
             GROUP BY process_id, transaction_id, tax_level, tax_jurisdiction, reference_type, reference_no, av_mod_flag,
                   tax_paid_flag, tax_rate_code, tax_sign, combined_rate,
                   difference_in_pennies) q  --note: this is the same value for all #tax_rounding_diff for a given transaction / tax-info combination
           INNER JOIN #tax_rounding_diff r
              ON r.process_id = q.process_id AND r.transaction_id = q.transaction_id  --not needed but helps optimizer
             AND r.row_id >= q.min_row_id
             AND r.row_id <= q.max_row_id
           INNER JOIN work_tax_detail w
              ON w.process_id = r.process_id  --used instead of @process_id to help optimizer
             AND w.transaction_id = r.transaction_id
             AND w.line_id = r.line_id
             AND w.tax_level = r.tax_level
             AND w.tax_jurisdiction = r.tax_jurisdiction
             AND COALESCE(w.reference_no, 'N/A') = COALESCE(r.reference_no, 'N/A')
             AND w.reference_type = r.reference_type
             AND SIGN(FLOOR(w.tax_category / 100)) = r.av_mod_flag
             AND w.tax_paid_flag = r.tax_paid_flag
             AND w.tax_rate_code = r.tax_rate_code 
             AND w.tax_sign = r.tax_sign 
             AND w.combined_tax_rate = r.combined_rate
    SELECT @errno = @@error
    IF @errno <> 0
    BEGIN
    ...
    END
/*
    --DAOM-344 Another possibility, if optimizer having trouble with update above is to replace it with the following which split its in 2 steps:
    CREATE TABLE #tax_rounding_adj (
    ...
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Failed to create table to hold list of rounding adjustments to be applied.  ',
             @object_name = '#tax_rounding_adj',
             @operation_name = 'CREATE'
      GOTO error
    END
    SELECT @temp_tb_created = 2

    INSERT INTO #tax_rounding_adj(process_id, 
           transaction_id, tax_level, tax_jurisdiction, reference_type, reference_no, av_mod_flag,
           tax_paid_flag, tax_rate_code, tax_sign, 
           adjustment_amount, line_id, combined_rate)
    SELECT r.process_id, 
           r.transaction_id, r.tax_level, r.tax_jurisdiction, r.reference_type, r.reference_no, r.av_mod_flag,
           r.tax_paid_flag, r.tax_rate_code, r.tax_sign, 
           sign(r.difference_in_pennies)/100, r.line_id, r.combined_rate
      FROM (SELECT min(row_id) min_row_id, min(row_id) + difference_in_pennies - 1 max_row_id,
                   process_id, transaction_id --not needed but helps optimizer
              FROM #tax_rounding_diff
             WHERE process_id = @process_id --note needed but helps clarify 
             GROUP BY process_id, transaction_id, tax_level, tax_jurisdiction, reference_type, reference_no, av_mod_flag,
                   tax_paid_flag, tax_rate_code, tax_sign, combined_rate,
                   difference_in_pennies) q  --note: this is the same value for all #tax_rounding_diff for a given transaction / tax-info combination
             INNER JOIN #tax_rounding_diff r
    ...
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Failed to determine to which lines a cent of the rounding difference should be allocated.  ',
  	     @object_name = '#tax_rounding_adj',
	     @operation_name = 'INSERT'
      GOTO error
    END

    CREATE INDEX t_tax_rounding_adj_x1
       ON #tax_rounding_adj(process_id, transaction_id, line_id)
    SELECT @errno = @@error
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Failed to create index on #tax_rounding_adj (should not be necessary but needed to help optimizer).  ',
	     @object_name = 't_tax_rounding_adj_x1',
	     @operation_name = 'CREATE INDEX'
      GOTO error
    END
    
    UPDATE w --UPDATE work_tax_detail
       SET tax_amount_collected = w.tax_amount_collected - r.adjustment_amount
      FROM #tax_rounding_adj r
           INNER JOIN work_tax_detail w
              ON w.process_id = r.process_id  --used instead of @process_id to help optimizer
             AND w.transaction_id = r.transaction_id
             AND w.line_id = r.line_id
             ...
             AND w.tax_rate_code = r.tax_rate_code 
             AND w.tax_sign = r.tax_sign 
             AND w.combined_tax_rate = r.combined_tax_rate
     WHERE r.process_id = @process_id --not needed but added for clarity
    SELECT @errno = @@error
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Failed to allocate a cent to the tax collected',
    ...
    DROP TABLE #tax_rounding_adj
    SELECT @errno = @@error
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Failed to drop table holding list of rounding adjustments to be applied to each line.  ',
             @object_name = '#tax_rounding_adj',
             @operation_name = 'DROP'
      GOTO error
    END
...
*/
  ...
  END --IF @rows > 0 --rounding diff rows
  
  DROP TABLE #tax_rounding_diff
  SELECT @errno = @@error
  IF @errno <> 0
  BEGIN
    SELECT @errmsg = 'Failed to drop table holding list of rounding differences to be applied.  ',
           @object_name = '#tax_rounding_diff',
           @operation_name = 'DROP'
    GOTO error
  END
  SELECT @temp_tb_created = 0
...
error:
  ...
  IF @temp_tb_created = 1
  BEGIN
    DROP TABLE #tax_rounding_diff
  END
/*
  IF @temp_tb_created = 2
  BEGIN
    DROP TABLE #tax_rounding_adj
  END
*/
