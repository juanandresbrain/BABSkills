...
```sql
/****** Object:  StoredProcedure [dbo].[util_grant_permissions_$sp]    Script Date: 5/10/2018 12:27:50 PM ******/
...
HISTORY:
10May2018 Kiri DAOM-3020  Correct Server version checking resulted in calling deprecated function in later SQL version
...
SELECT @sql_version = CHARINDEX('SQL Server', @@VERSION);
IF (@sql_version > 0)
   SELECT @sql_version =  CAST(SUBSTRING(@@VERSION, CHARINDEX('SQL Server',@@VERSION) + 11, 4) AS INT);
