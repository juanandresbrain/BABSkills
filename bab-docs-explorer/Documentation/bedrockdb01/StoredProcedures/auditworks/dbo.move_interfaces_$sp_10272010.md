# dbo.move_interfaces_$sp_10272010
...
```mermaid
flowchart LR
    SP["dbo.move_interfaces_$sp_10272010"]
...
```sql
CREATE proc [dbo].[move_interfaces_$sp_10272010] 
...
BEGIN
  ...
  SELECT DISTINCT tl.transaction_id,
      ...
      FROM work_transaction_move wt WITH (NOLOCK), 
   	   ...
   	   interface_applicability_lookup ia
     WHERE ia.interface_id = @interface_id
       AND (wt.transaction_date >= @live_date OR @live_date IS NULL) -- Def 8703
       AND wt.sa_rejection_flag = 0
       AND wt.transaction_void_flag * (wt.transaction_void_flag - 8) * (1 - @interface_voided_transactions) = 0
       AND wt.transaction_id = tl.transaction_id
