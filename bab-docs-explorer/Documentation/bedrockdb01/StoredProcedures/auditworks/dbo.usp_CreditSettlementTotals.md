# dbo.usp_CreditSettlementTotals
...
```mermaid
flowchart LR
    SP["dbo.usp_CreditSettlementTotals"]
    ...
    dbo_IFO9590_summary(["dbo.IFO9590_summary"]) --> SP
...
```sql
CREATE       proc [dbo].[usp_CreditSettlementTotals]  
  
as  
-- =====================================================================================================
-- Name: usp_CreditSettlementTotals
--
-- Description:	
--
-- Input:	
...
--		Garyd			08/30/2010		Change db name from smartview to foundation.  Change from sendmail to dbmail.
-- =====================================================================================================
...
set @sql = ''  
set @Subject = ''  
--set @recipients = 'edinp@buildabear.com'  
--set @recipients = 'paulb@buildabear.com'  
--set @recipients = 'lindak@buildabear.com; jackm@buildabear.com; IT-Retail Systems'
set @recipients = 'lindak@buildabear.com; jackm@buildabear.com'
...
if (Object_ID('tempdb..#FDMSFiles') IS NOT NULL) DROP TABLE #FDMSFiles  
create table #FDMSFiles (  
 filename varchar(100)  
)  
  
-- pull the credit settlement batch files  
set @command = 'dir /b ' + @drive + '\NSB\Settlement\*.*.*'
insert into #FDMSFiles  
exec master..xp_cmdshell @command  
delete from #FDMSFiles where filename is null or filename = 'File Not Found' or right(filename,4) = 'stat'  
...
begin  
	...
	select cs.transmission_id,
		...
		SUM(aif.amount) as Amount,
		'NO'
	from auditworks.dbo.IFO9590_summary aif,
		#cs_temp cs
	...
	if (select right(filename,4) from #FDMSFiles where substring(filename,5,4) = @transmissionid) = 'DONE'  
		begin  
...
begin  
 set @Subject = 'USA - FDMS Credit Settlement **PROBLEM**'  
end
else
	begin
		...
		begin  
	 		set @Subject = 'USA - FDMS Credit Settlement Summary'  
		end
	end
...
--exec master.dbo.xp_sendmail   
-- --@recipients = 'sarahm@buildabear.com', 
