...
May09,16 Vicci      DAOM-730  Avoid auditworks_system_flag and parameter_general triggers firing unecessarily (with potential for causing lock issues)
                              by checking whether any integrity exists before doing integrity fix code.
Jun18,15 Vicci    TFS-126651  Do not overlay business rule errors.
...
IF EXISTS (SELECT 1 FROM auditworks_system_flag --avoid firing trigger
...
general_error:
  ...
  IF COALESCE(@errno, 0) = 0  --avoid overlaying business rule errors.
