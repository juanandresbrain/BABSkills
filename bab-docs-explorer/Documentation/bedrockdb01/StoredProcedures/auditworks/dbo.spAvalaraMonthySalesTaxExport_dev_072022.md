# dbo.spAvalaraMonthySalesTaxExport_dev_072022
...
```mermaid
flowchart LR
    SP["dbo.spAvalaraMonthySalesTaxExport_dev_072022"]
...
```sql
CREATE   procedure [dbo].[spAvalaraMonthySalesTaxExport_dev_072022]
@TransactionStartDate as date
,@TransactionEndDate as date
...
--		Keith Lee			05/05/2020		Removed Zoe and Cindy from receipent list for taxadmin@buildabear.com
--		Keith Lee			05/27/2020		Added code to exclude registers 52, 53, 56, 57 for BOSFS transactions
--		Keith Lee			02/04/2022		Added code to grab store address from Kodiak/StoreMDM to be used for OrigAddress and OrigRegion as requested by Jake H and Aaron H (SR# 31963)
--		Keith lee			03/17/2022		Made changes when building the Store Info work table to only include active stores instead of only excluding warehouse locations.  This was due to the SR# 33281 reported by Aaron H.
-- 
...
IF (Object_ID('tempdb..##StartJobCheck1_test') IS NOT NULL) DROP TABLE ##StartJobCheck1_test
IF (Object_ID('tempdb..##TaxStoreData_test') IS NOT NULL) DROP TABLE ##TaxStoreData_test
IF (Object_ID('tempdb..##TaxCountryList_test') IS NOT NULL) DROP TABLE ##TaxCountryList_test
IF (Object_ID('tempdb..##TaxCodes_test') IS NOT NULL) DROP TABLE ##TaxCodes_test
IF (Object_ID('tempdb..##TaxLineCounts_test') IS NOT NULL) DROP TABLE ##TaxLineCounts_test
IF (Object_ID('tempdb..##TaxSalesDetail_test') IS NOT NULL) DROP TABLE ##TaxSalesDetail_test
IF (Object_ID('tempdb..##TaxExemptSalesDetail_test') IS NOT NULL) DROP TABLE ##TaxExemptSalesDetail_test
IF (Object_ID('tempdb..##TaxESSalesDetail_test') IS NOT NULL) DROP TABLE ##TaxESSalesDetail_test
IF (Object_ID('tempdb..##TaxSendSaleSalesDetail_test') IS NOT NULL) DROP TABLE ##TaxSendSaleSalesDetail_test
IF (Object_ID('tempdb..##TaxDetailResults_test') IS NOT NULL) DROP TABLE ##TaxDetailResults_test
IF (Object_ID('tempdb..##TaxDetailResultsFinal_test') IS NOT NULL) DROP TABLE ##TaxDetailResultsFinal_test
IF (Object_ID('tempdb..##TaxSummaryResults_test') IS NOT NULL) DROP TABLE ##TaxSummaryResults_test
IF (Object_ID('tempdb..##TaxSummaryOutput_test') IS NOT NULL) DROP TABLE ##TaxSummaryOutput_test
IF (Object_ID('tempdb..##TaxExportHeaders_test') IS NOT NULL) DROP TABLE ##TaxExportHeaders_test
IF (Object_ID('tempdb..##TaxStoreAddress_test') IS NOT NULL) DROP TABLE ##TaxStoreAddress_test
...
--SELECT '2022-01-02' AS STRT_DATE_TIME
--	,'2020-01-29' AS END_DATE_TIME
--INTO ##StartJobCheck1
...
--IF (SELECT COUNT(*) FROM ##StartJobCheck1) = 0
--GOTO FINISH
...
--SET @TransactionStartDate = (SELECT STRT_DATE_TIME FROM ##StartJobCheck1)
--SET @TransactionEndDate = (SELECT END_DATE_TIME FROM ##StartJobCheck1)

-->>>>>>   Set dates then uncomment these two SET statements when running manually   <<<<<<--
--SET @TransactionStartDate = '2022-01-02'
--SET @TransactionEndDate = '2022-01-29'
...
INSERT INTO ##TaxStoreData_test VALUES('9999',NULL,'1954 Innerbelt Business Center Drive','St Louis','MO','63114','USA','USD','9999')

UPDATE ##TaxStoreData_test
...
select	case when snumber < 2000
		...
		address1 as address, 
		city as city
into	##TaxStoreAddress_test
from KODIAK.BearData.dbo.store
...
SELECT ROW_NUMBER() OVER(ORDER BY StrCountry ASC) AS CountryID
	,StrCountry
INTO ##TaxCountryList_test
FROM ##TaxStoreData_test
...
JOIN	##TaxStoreData_test sd WITH (NOLOCK) ON th.store_no = sd.ORG_CHN_NUM
JOIN	##TaxCodes_test tc WITH (NOLOCK) ON tc.tax_item_group_id = td.tax_item_group_id
...
SELECT DISTINCT *
INTO ##TaxDetailResults_test
FROM ##TaxSalesDetail_test
...
--		,'' AS ExchangeRate  --<< Need to get for Canada stores
		...
		,sd.StrRegion AS OrigRegion  --<< State
		,sd.StrPostalCode AS OrigPostalCode
INTO	##TaxExemptSalesDetail_test
FROM 	auditworks.dbo.av_transaction_header th WITH (NOLOCK)
JOIN	auditworks.dbo.av_transaction_line tl WITH (NOLOCK) ON th.av_transaction_id=tl.av_transaction_id
LEFT JOIN	auditworks.dbo.av_tax_detail td WITH (NOLOCK) ON th.av_transaction_id = td.av_transaction_id AND tl.line_id = td.line_id
JOIN	##TaxStoreData_test sd WITH (NOLOCK) ON th.store_no = sd.ORG_CHN_NUM
...
INSERT INTO ##TaxDetailResults_test
SELECT DISTINCT *
FROM ##TaxExemptSalesDetail_test
WHERE Amount + TotalTax != 0
...
--		,'' AS ExchangeRate  --<< Need to get for Canada stores
		...
		,sd.StrRegion AS OrigRegion  --<< State
		,sd.StrPostalCode AS OrigPostalCode
INTO	##TaxSendSaleSalesDetail_test
FROM 	auditworks.dbo.av_transaction_header th WITH (NOLOCK)
JOIN	auditworks.dbo.av_transaction_line tl WITH (NOLOCK) ON th.av_transaction_id=tl.av_transaction_id 
JOIN	auditworks.dbo.av_merchandise_detail smd WITH (NOLOCK) ON th.av_transaction_id = smd.av_transaction_id AND tl.line_id = smd.line_id
LEFT JOIN	auditworks.dbo.av_tax_detail td WITH (NOLOCK) ON th.av_transaction_id = td.av_transaction_id AND tl.line_id = td.line_id
...
INSERT INTO ##TaxDetailResults_test
SELECT DISTINCT *
FROM ##TaxSendSaleSalesDetail_test
WHERE Amount + TotalTax != 0
...
INTO ##TaxLineCounts_test
FROM ##TaxDetailResults_test
...
SELECT tdr.CustomerCode
	...
	,tdr.OrigRegion
	,tdr.OrigPostalCode
INTO ##TaxDetailResultsFinal_test
FROM ##TaxDetailResults_test tdr WITH(NOLOCK)
JOIN ##TaxLineCounts_test tlc WITH(NOLOCK) ON tlc.transaction_id = tdr.Ref2 AND tlc.line_id = tdr.line_id
...
SELECT	'1' AS ProcessCode
		...
		,'' AS Email
INTO	##TaxSummaryResults_test
FROM 	##TaxDetailResultsFinal_test tsd
join	##TaxStoreAddress_test tsa on tsd.CustomerCode = tsa.store_no
...
UPDATE ##TaxSummaryResults_test
SET OrigAddress = REPLACE(OrigAddress, ',', '')
WHERE CHARINDEX(',', OrigAddress) > 0

UPDATE ##TaxSummaryResults_test
SET OrigCity = REPLACE(OrigCity, ',', '')
WHERE CHARINDEX(',', OrigCity) > 0

--select * from ##TaxSummaryResults_test
--####################################
-- Set variables
--####################################
...
SET @Copy_Recipients = 'ianwn@buildabear.com'
...
SELECT tsr.*
INTO ##TaxSummaryOutput_test
FROM ##TaxSummaryResults_test tsr WITH (NOLOCK)
JOIN ##TaxStoreData_test tsd WITH (NOLOCK) ON tsr.CustomerCode = tsd.CustomerCode
...
SET @SQL = 'SELECT * FROM ##TaxSummaryOutput_test'
...
--)
--VALUES (
--'spAvalaraMonthySalesTaxExport', --<< Stored Proc name
--GETDATE(),
...
--'Avalara Sales Tax export file has been created for import to Avalara' --<< Comment
--)
...
--	@Copy_Recipients, --<< Email TO
--	@Recipients, --<< Email CC
--	@Subject, --<< Email Subject
--	'SA export files Missing for Avalara' --<< Comment
--	)
