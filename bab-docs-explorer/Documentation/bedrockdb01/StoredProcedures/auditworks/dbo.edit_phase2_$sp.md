...
Aug18,16 Vicci     DAOM-1171 Don't unlock date_reject_id 0 if only date_reject_id > 0 is being edited and the lock is held by another function.
May27,16 Vicci      DAOM-730 Pass process_id to trickle lock so that any errors get associated with right batch.
May26,16 Vicci      DAOM-730 In trickle audit environment, re-evaluate audit status short and unrec media, 
                             since media rec posting done in phase 1 no longer locks all the store/dates trickling in.
May11,16 Vicci      DAOM-730 Log request type to process log.
                             Pass trickle polling flag to edit missing reg, otherwise it was not evaluating current date but still bumping
                             the last edit date serving as the starting point for the next run.
...
DECLARE
  ...
  @rec_process_id_reval		numeric(12,0);
...
BEGIN TRY
 ...
 PRINT @trace_msg;
  ...
  VALUES (@process_no,
          ...
          @process_start_time,
          @process_status_flag,
          'Request_type = ' + CONVERT(nvarchar, @request_type),
          NULL,
	  @batch_process_id);
...
-- So, only set the flag if this is a regular phase 2 (request_type 1).
  ...
  BEGIN
    ...
    EXEC edit_trickle_lock_store_$sp @errmsg3 OUTPUT, @process_id;
  ...
  BEGIN
    ...
    EXEC edit_missing_reg_$sp @process_id, @user_id, @errmsg3 OUTPUT, @process_no, @trickle_polling_flag;
...
@operation_name = 'OPEN';
  ...
  BEGIN
	...
	/* Batch now contains enough transactions, so start processing the current batch of store/dates
      ...
      --In a trickle audit environment, since Media Reconciliation no longer locks store/dates for each batch coming in when updating over/short, 
      --unrec media present these amounts need to be re-evaluated in phase 2.
      IF @trickle_polling_flag >= 2
      BEGIN
      
        SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG &&: Edit phase 2 - Trickle Audit re-evaluation of audit status media rec information : ' + CONVERT(nchar, getdate(), 8);
        PRINT @trace_msg;
      
        SELECT @object_name    = 'function_status_rec',
               @operation_name = 'INSERT',
               @errmsg = 'Unable to insert function_status_rec for edit to revaluate audit status media rec totals. ';
        INSERT INTO function_status_rec(
	       ...
	       edit_process_no)
        VALUES(@process_id,
               70, --media rec function
               40, --rec_status
               @batch_process_id);
        SELECT @rec_process_id_reval = @@identity;
	
        SELECT @object_name    = 'recovery_rec_audit_status',
               @errmsg = 'Failed to list store/reg/dates whose audit status media rec totals need to be re-evaluated. ';
      	INSERT INTO recovery_rec_audit_status(
	       rec_process_id, process_id, store_no, register_no, transaction_date, actual_flag,
	       unrec_flag, posting_date_time, media_rec_only_lock, locked_by_edit)
        SELECT @rec_process_id_reval, @process_id, ed.store_no, ed.register_no, ed.transaction_date, 1,
               1, getdate(), 0, 1
          FROM work_edit_batch_list ed
         WHERE ed.date_reject_id = 0
           AND NOT EXISTS(SELECT 1
                            FROM work_rec_incomplete ri
                           WHERE ed.store_no = ri.store_no
                             AND ed.transaction_date = ri.transaction_date);
	
        SELECT @object_name    = 'reconciliation_$sp',
               @operation_name = 'EXECUTE',
               @errmsg = 'Failed to perform re-evaluation of audit status media totals. ';
        EXEC reconciliation_$sp 4, @process_id, @rec_process_id_reval, 40, @errmsg3 OUTPUT, @user_id, @batch_process_id;
        ...
        BEGIN 
          ...
          END;
            ...
            SELECT @errmsg         = 'Failed to update store_audit_status (date_reject_id)',
                   ...
                   @operation_name = 'UPDATE'; 
          UPDATE store_audit_status
	     SET update_in_progress       = CASE WHEN update_in_progress = 1 THEN 0 ELSE update_in_progress END, 
                 trickle_in_progress_flag = 0
           WHERE store_no           = @store_no
             AND sales_date   = @sales_date
             AND date_reject_id     = 0
             AND (update_in_progress = 1 OR trickle_in_progress_flag > 0);        
 ...
 -- Don't delete store_audit_status if any rows remain in audit_status
  ...
  -- flag guided audit as needing a refresh
    SELECT @errmsg         = 'Failed to update ORG_CHN_HRCHY_LVL_GRP_SA (final)',
           @object_name    = 'ORG_CHN_HRCHY_LVL_GRP_SA',
           @operation_name = 'UPDATE';
  UPDATE ORG_CHN_HRCHY_LVL_GRP_SA
     SET GRP_MBR_CHNG = getdate()
