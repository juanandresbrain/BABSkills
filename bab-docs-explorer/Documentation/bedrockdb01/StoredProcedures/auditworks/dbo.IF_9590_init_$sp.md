...
```sql
create proc dbo.IF_9590_init_$sp
@batch_size int 
AS
...
/* TRUNCATE Ex_Work_9590 */
TRUNCATE TABLE Ex_Work_9590
SELECT @errno = @@error 
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to Truncate Ex_Work_9590 table'
   GOTO error
   END
...
EXEC auditworks.dbo.Ex_GetMaxQueue 50, @min_serial_no, @batch_size, @max_serial_no OUTPUT
