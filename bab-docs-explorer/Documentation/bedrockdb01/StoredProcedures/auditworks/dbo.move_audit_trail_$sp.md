...
BEGIN
  SELECT @TBL_KEY = CONVERT(VARCHAR, @to_store_no) + @sep + 
                    dbo.convdate(@to_sales_date) + @sep + 
                    CONVERT(VARCHAR, @to_register_no),
         @TBL_KEY_RSRC_NAME = 'TK_STOR_TRAN_DATE_REGI',
         @TBL_KEY_RSRC_PRMS = CONVERT(VARCHAR, @to_store_no) + ' - ' + @store_name_to + @sep + 
                              dbo.convdate(@to_sales_date) + @sep + 
                              CONVERT(VARCHAR, @to_register_no) 
END
ELSE
BEGIN
  SELECT @TBL_KEY = CONVERT(VARCHAR, @to_store_no) + @sep + 
                    dbo.convdate(@to_sales_date) + @sep + 
                    CONVERT(VARCHAR, @to_register_no) + @sep,
	...
	/* Not moving an entire store or register so need to log a detail row for each transaction that was moved.
	  ...
	  INSERT ADT_TRL_DTL (
	   ...
	   SELECT
		@ENTRY_ID,
		@TBL_NAME,
		@TBL_KEY + CONVERT(VARCHAR, transaction_no),
		@TBL_KEY_RSRC_NAME,
		@TBL_KEY_RSRC_PRMS + CONVERT(VARCHAR, transaction_no),
		'M',
		@field_name_list,
		@old_value_list,
		@new_value_list
...
SELECT 
	@ENTRY_ID,
	301,
	CONVERT(VARCHAR, @from_store_no),
	CONVERT(VARCHAR, @from_register_no),	
	dbo.convdate(@from_sales_date),
	CONVERT(VARCHAR, wt.till_no),
	CONVERT(VARCHAR, th.transaction_no),
	th.transaction_series,
	CONVERT(VARCHAR, wt.cashier_no),
	CONVERT(VARCHAR, th.transaction_id)
...
SELECT 
	@ENTRY_ID,
	301,
	CONVERT(VARCHAR, @to_store_no),
	CONVERT(VARCHAR, @to_register_no),	
	dbo.convdate(@to_sales_date),
	CONVERT(VARCHAR, th.till_no),
	CONVERT(VARCHAR, th.transaction_no),
	th.transaction_series,
	CONVERT(VARCHAR, th.cashier_no),
	CONVERT(VARCHAR, th.transaction_id)
   FROM work_transaction_move wt WITH (NOLOCK), transaction_header th WITH (NOLOCK)
  WHERE wt.process_id = @process_id
    AND wt.transaction_id = th.transaction_id
