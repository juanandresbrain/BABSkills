# dbo.user_r3_gl_interface_$sp_10062010
...
```mermaid
flowchart LR
    SP["dbo.user_r3_gl_interface_$sp_10062010"]
    audit_status(["audit_status"]) --> SP
    calendar(["calendar"]) --> SP
...
```sql
CREATE proc  [dbo].[user_r3_gl_interface_$sp_10062010] 
...
IF @grouping_level = 1
  BEGIN
	...
	SELECT
		...
		SUM(s.amount),
		ce.calendar_date
	FROM subledger s, calendar c, gl_account_cross_reference x, calendar ce
	WHERE s.posting_status = 0
	  AND s.gl_account_id = x.gl_account_id
	  AND s.transaction_date = c.calendar_date
	  AND ce.merchandise_year_no = c.merchandise_year_no
	  AND ce.merchandise_month_no = c.merchandise_month_no
	  AND ce.month_end_flag = 'M'
	  AND s.transaction_date BETWEEN @last_date_closed AND @period_ending_date
	GROUP BY SIGN(SIGN(9999-s.gl_company) + 1) * s.gl_company, x.gl_account_id, x.gl_account_no, s.store_no, ce.calendar_date
	ORDER BY SIGN(SIGN(9999-s.gl_company) + 1) * s.gl_company, x.gl_account_no, s.store_no, ce.calendar_date
...
ELSE IF @grouping_level = 2
  BEGIN
	...
	SELECT
		...
		SUM(s.amount),
		s.transaction_date
	FROM subledger s, calendar c, gl_account_cross_reference x
	WHERE s.posting_status = 0
	  AND s.gl_account_id = x.gl_account_id
	  AND s.fiscal_year = c.merchandise_year_no
	  AND s.period = c.merchandise_month_no
	  AND c.month_end_flag = 'M'
	  AND s.transaction_date BETWEEN @last_date_closed AND @period_ending_date
...
ELSE IF @grouping_level = 3
  BEGIN
	...
	SELECT
		...
		SUM(s.amount),
		s.transaction_date
	FROM subledger s, gl_account_cross_reference x, calendar c, line_object o
	WHERE s.posting_status = 0
	  AND s.gl_account_id = x.gl_account_id
	  AND s.fiscal_year = c.merchandise_year_no
	  AND s.period = c.merchandise_month_no
...
ELSE -- i_grouping_level = 0, or all others
  BEGIN
	...
	WHERE s.posting_status = 0
	  ...
	  AND ce.month_end_flag = 'M'
	  AND s.transaction_date BETWEEN @last_date_closed AND @period_ending_date
	GROUP BY SIGN(SIGN(9999-s.gl_company) + 1) * s.gl_company, x.gl_account_id, x.gl_account_no, ce.calendar_date
	ORDER BY SIGN(SIGN(9999-s.gl_company) + 1) * s.gl_company, x.gl_account_no, ce.calendar_date
