...
PROC NAME: reconciliation_$sp
    ...
    Notes: media_unreconciliation has 1 row per unreconciled media_reconciliation_detail row (hence presence of rec_id)
           carryforwards do not feed media unreconciliation, only media_reconciliation_detail.
           $0 carryforwards do not log to media_reconciliation_detail
IMPORTANT: At present, this posting does not handle the situation where there is a mix of transaction from pre and post configuration change in the same batch
           since the insert does min(rec_side), max(multiple_actual_handling_code) etc.

    Unicode Version.

HISTORY: 
Date     Name           Def# Desc
Oct18,16 Kiri/Sab  DAOM-1580 Handle situation where an entity that used to be reconciled is configured to no longer be reconciled, leaving behind a last reconciliation date time
                             in media reconciliation status that should be disregarded. See IMPORTANT comment above.
                             Handle recovery situation where recovery of edit post void for a transaction took place prior to recovery of the edit posting of the transaction in question.
Jun16,16 Vicci      DAOM-292 Since the same wra.rec_id (actual transaction) can have more than one period_to_date_time due to attachment 33 Period/Entity reconciled being different on each of its lines, include wrd.rec_date_time in join criteria from wra to wrd for over/short posting.
                             Avoid divide by 0 error when setting issue flag for over/shorts in the case of badly configured foreign currency tenders (set not to be foreign so exchange not included but exchange still given and expected + exchange = actual + exchange)
May05,16 Vicci      DAOM-607 Correct insert into work_rec_audit_status_detail from media_unreconciliation to be for DISTINCT store/reg/dates since
                             media_unreconciliation has 1 row per unreconciled media_reconciliation_detail row (hence presence of rec_id).
                             Note, no harm was done by this being missing -just unecessary duplicate rows in the table. 
                             Correct handling of balancing entity that was missing from media reconciliation status when originally looked up but had been
                             created by another process by the time the current one attempted to create it to understand that the OTHER process has the lock,
                             not the current one.
                             For option 4, handle weird scenario where the earliest date/time for transactions with tenders for a given transaction date is the following date just after midnight,
                             and these transactions are unreconciled which results in the first_unreconciled_date_time being later than corresponding transaction date.
...
BEGIN  -- recovery scenarios
  ...
  BEGIN
	...
	IF @rec_status = 5  --died after locking but before it could insert mrt
	BEGIN
      IF @process_no IN (1, 4, 5)  --Edit
        BEGIN
		  SELECT @errmsg         = 'Failed to delete work_rec_transaction (recovery).',
				 @object_name    = 'work_rec_transaction',
				 @operation_name = 'DELETE';
          DELETE w
			FROM work_rec_transaction w 
			JOIN media_reconciliation_trans mrt ON mrt.transaction_id = w.transaction_id AND mrt.line_id = w.line_id
             AND mrt.rec_amount_type = w.rec_amount_type AND mrt.rec_amount_subtype = w.rec_amount_subtype
             AND mrt.rec_side = w.rec_side
            JOIN media_reconciliation_status mrs ON mrt.balancing_entity_id = mrs.balancing_entity_id
             AND mrs.rec_type = w.rec_type
           WHERE w.rec_process_id = @rec_process_id;
        END; --IF i_process_no IN (1, 4, 5)

      IF @process_no = 103  --Edit Post Void failed and at the time it built its wrt Edit had not yet populate mrt with original trans but now it has
      BEGIN
		  SELECT @errmsg         = 'Failed to update work_rec_transaction (recovery).',
				 @object_name    = 'work_rec_transaction',
				 @operation_name = 'UPDATE';
          UPDATE w
             SET posted_flag = 3  --update
            FROM work_rec_transaction w 
            JOIN media_reconciliation_trans mrt ON mrt.transaction_id = w.transaction_id AND mrt.line_id = w.line_id
             AND mrt.rec_amount_type = w.rec_amount_type AND mrt.rec_amount_subtype = w.rec_amount_subtype
             AND mrt.rec_side = w.rec_side
            JOIN media_reconciliation_status mrs ON mrt.balancing_entity_id = mrs.balancing_entity_id
             AND mrs.rec_type = w.rec_type
             AND mrs.rec_group_line_object = w.rec_group_line_object
             AND mrs.balancing_method = w.balancing_method
             AND mrs.balancing_entity = w.balancing_entity
           WHERE w.rec_process_id = @rec_process_id 
             AND w.posted_flag = 0;  --insert
      END; --IF i_process_no = 103
	END;  --IF i_rec_status = 5
	...
	SELECT @errmsg = 'Unable to declare cursor media_status_crsr',
      ...
      /* do an if not exists first to avoid messages onscreen which would abort the frontend */
	    ...
	    END; -- @scaleout_flag = 1
 	    ...
 	    SELECT @entity_locked_count = @entity_locked_count + 1,
    ...
    BEGIN
      ...
      UPDATE media_reconciliation_trans
         SET rec_amount = ABS(SIGN(wrt.rec_amount_type - 2)) * (wrt.rec_amount + wrt.rec_exchange) + (1-ABS(SIGN(wrt.rec_amount_type - 2))) * wrt.rec_quantity, 
             ...
             deposit_slip = wrt.deposit_slip, 
             security_seal = wrt.security_seal
        FROM work_rec_entity wre WITH (NOLOCK),
             work_rec_transaction wrt WITH (NOLOCK),
             media_reconciliation_trans mrt
       WHERE wre.rec_process_id = @rec_process_id 
...
/* Build list of store/reg/dates for which unreconciled amounts may have been removed 
   ...
   by deletion of all unreconciled amounts*/
    ...
    INSERT work_rec_audit_status_detail(
           ...
           unrec_flag,
           source_rec_status)
    SELECT DISTINCT @rec_process_id,
           mu.store_no,
           mu.register_no,
           mu.transaction_date,
           0,
   ...
   Clean up previously posted reconciliations for the date/time range to be evaluated */
    ...
    DELETE media_unreconciliation
     ...
     WHERE wre.rec_process_id = @rec_process_id
       ...
       AND wre.lower_date_time <= mu.transaction_date  --note:  lower and upper are just date (not datetime) for handling code 4
  ...
  END; --IF @multiple_actual_code4_exists = 1
   ...
   -- Log over/short amounts if any
    ...
    SELECT wra.store_no,
           ...
           wra.rec_id,
           wra.transaction_date,
           SIGN(ABS((wra.rec_amount) - 
            SUM(ISNULL(wrd.rec_amount, 0) + wra.convert_to_domestic * ISNULL(wrd.rec_exchange,0)) 
                   )) *
            (SIGN((SIGN(SIGN(ABS((wra.rec_amount) - 
             SUM(ISNULL(wrd.rec_amount, 0) + wra.convert_to_domestic * ISNULL(wrd.rec_exchange,0)) ) - 
                   wra.short_tolerance_amount ) -1) + 1) + 
             (SIGN(SIGN(ABS(((wra.rec_amount) - 
                    SUM(ISNULL(wrd.rec_amount, 0) + wra.convert_to_domestic * ISNULL(wrd.rec_exchange,0))) 
                    / 
                    (SUM(ISNULL(wrd.rec_amount, 0) + wra.convert_to_domestic * ISNULL(wrd.rec_exchange,0)) + 
                     ((1-ABS(SIGN(SUM(ISNULL(wrd.rec_amount, 0) + wra.convert_to_domestic * ISNULL(wrd.rec_exchange,0))))) * wra.rec_amount )
                    )
                   )  - 
 ...
 * 2 + 2
      ...
      FROM work_rec_actual wra WITH (NOLOCK)
           LEFT JOIN work_rec_detail wrd WITH (NOLOCK)
            ...
            AND wra.rec_id = wrd.rec_id
            AND wra.transaction_date = wrd.rec_date -- to exclude unrec actual carry-forward
            AND wra.period_to_date_time = wrd.rec_date_time  --since the same wra.rec_id (actual transaction) can have more than one period_to_date_time due to attachment 33 Period/Entity reconciled being different on different each of its lines
...
/* If activity later than the last reconciliation is affected log the changes to the unreconciled amounts. 
   Use outer-join to ensure that balancing-entities which no longer have any unreconciled amounts go to zero. */
    ...
    IF @multiple_actual_code4_exists = 1
    BEGIN
      SELECT @errmsg='Failed to lower first_unreconciled_date_time in case of transaction date being earlier than entry_date_time, for multiple_actual_handling_code = 4. ',
             @object_name = 'work_rec_entity',
             @operation_name = 'UPDATE';
      --DAOM-607 handle weird scenario where the earliest date/time for transactions with tenders for a given transaction date is the following date just after midnight
      --and these transactions are unreconciled which results in the first_unreconciled_date_time being later than corresponding transaction date.
      --Just set to support the #work_unrec insert below
      --Although it is legitimate for the first_unreconciled_date_time to be later than the corresponding transaction date in mrs
      --and although the #work_unrec insert's use of the lower_date_time in that scenario (which will be the transaction date) is generally sufficient, 
      --there are exceptions, for example if an unreconciled amount arrives for a transaction date later than the transaction date of the first unreconciled transaction but before its entry_date_time.
      --this will cause the first_unreconciled_date_time to change to the timestamp of the new transaction, and without the update below, when it gets reconciled the first_unreconciled_date_time would go to NULL which is incorrect and will cause a dup key on insert if the remaining unreconciled transaction is modified.
      UPDATE wre  --UPDATE work_rec_entity
         SET first_unreconciled_date_time = (SELECT MIN(mu.transaction_date)
	                                       FROM media_unreconciliation mu WITH (NOLOCK)
	                                      WHERE mu.balancing_entity_id = wre.balancing_entity_id
 	                                        AND mu.transaction_date < CONVERT(smalldatetime, CONVERT(nchar(8), wre.first_unreconciled_date_time,112)) )
        FROM work_rec_entity wre
       WHERE wre.rec_process_id = @rec_process_id
	 AND wre.multiple_actual_handling_code = 4 
         AND wre.first_unreconciled_date_time IS NOT NULL
	 AND EXISTS (SELECT 1 
		       FROM media_unreconciliation mu
	              WHERE wre.balancing_entity_id = mu.balancing_entity_id
	                AND CONVERT(smalldatetime, CONVERT(nchar(8), wre.first_unreconciled_date_time,112)) > mu.transaction_date);
      ...
      SELECT wre.balancing_entity_id,
        ...
        FROM work_rec_entity wre WITH (NOLOCK)
             LEFT JOIN media_reconciliation_detail mrd
              ...
              AND mrd.transaction_date >= CASE WHEN CONVERT(smalldatetime, CONVERT(nchar(8), wre.first_unreconciled_date_time,112)) < wre.lower_date_time 
                                             THEN CONVERT(smalldatetime, CONVERT(nchar(8), wre.first_unreconciled_date_time,112)) 
                                             ELSE wre.lower_date_time END
