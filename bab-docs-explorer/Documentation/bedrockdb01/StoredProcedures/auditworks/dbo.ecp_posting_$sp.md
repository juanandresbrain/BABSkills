...
```sql
CREATE proc  dbo.ecp_posting_$sp 
@interface_id tinyint = 44
AS
/*  
...
HISTORY:  
Date     Name           Def#    Desc
May23,17 Kiri	    DAOM-2524   Export Job Keeps Failing - Failed to execute ecp_period_export3_$sp 
Jun30,16 Vicci       DAOM-996   Add smarload log entry for reallocation requests.
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ecp_definition_explosion_$sp: ' + CONVERT(nchar, getdate(), 8)
PRINT @trace_msg
...
BEGIN
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ecp_comms_auto_adjustment_$sp: ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
...
BEGIN
  ...
  IF @ascii_export = 1
  BEGIN
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ecp_period_export_$sp: ' + CONVERT(nchar, getdate(), 8)
    PRINT @trace_msg
  ...
  BEGIN
    ...
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ecp_period_export' + CONVERT(nvarchar, @ascii_export) + '_$sp: ' + CONVERT(nchar, getdate(), 8)
    PRINT @trace_msg
...
BEGIN
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ecp_history_cleanup_$sp: ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
...
BEGIN
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && processing loop: ' + convert(nvarchar, @loop_counter) + ', ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
  ...
  BEGIN
    ...
    BEGIN
      SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ecp_allocation_$sp: ' + CONVERT(nchar, getdate(), 8)
      PRINT @trace_msg

      IF @from_reallocation_date IS NOT NULL 
      BEGIN
        SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && Reallocation Requested for ' + CONVERT(nchar, @from_reallocation_date) + ' to ' + CONVERT(nchar, @to_reallocation_date) + ' : ' + CONVERT(nchar, getdate(), 8)
        PRINT @trace_msg
    ...
    BEGIN
      ...
      BEGIN
        SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && commission recalculation for ' + CONVERT(nchar, @from_recalculation_date) + ' to ' + CONVERT(nchar, @to_recalculation_date) + ' : ' + CONVERT(nchar, getdate(), 8)
        PRINT @trace_msg
...
--select 'TestWEETEReCalc', * from work_ecp_empl_trans_export
  ...
  BEGIN
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ECP transaction processing: ' + CONVERT(nchar, getdate(), 8)
    PRINT @trace_msg
...
*/
--Look up rate
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && Commission rate setting: ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && commission tier processing part 1: ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
...
/*  Each if entry no adds or subtracts from the count;  since ets is by item commission code, if there is more than
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ECP transaction counting: ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
...
--select 'Test6 work_ecp_empl_trans_import', * from work_ecp_empl_trans_import
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && commission tier processing part 2: ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
...
OPEN tier_change_cursor
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ECP summary/detail association: ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
...
--IF recalculation or reallocation results in same figures going to same summary id, don't post any change
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ECP hour processing: ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
...
--select 'Test8', * from work_ecp_hour_import
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ECP traffic count de-attribution resulting from payroll hour reversal: ' + CONVERT(nchar, getdate(), 8)
...
--SELECT 'Test work_ecp_hour_import traffic', * FROM work_ecp_hour_import WHERE attributed_traffic_count IS NOT NULL
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ECP summary update: ' + CONVERT(nchar, getdate(), 8)
 PRINT @trace_msg
...
AND w.transaction_commission_code = q2.transaction_commission_code
 ...
 SELECT @errno = @@error
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ECP summary insert: ' + CONVERT(nchar, getdate(), 8)
  PRINT @trace_msg
...
--select 'Test10', * from employee_hour_summary
  ...
  IF @max_serial_no > 0
  BEGIN   
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && ECP transaction expiry: ' + CONVERT(nchar, getdate(), 8)
    PRINT @trace_msg
