...
```mermaid
flowchart LR
    SP["dbo.period_end_$sp_10052010"]
...
```sql
create proc [dbo].[period_end_$sp_10052010] 
AS
...
May10,04 Maryam   DV-1071 Set @process_id and @user_name and pass it to check_dayend_running_$sp.
...
Dec06,02 Paul     1-H56TW avoid raiserror on business rule warning messages
...
DECLARE
	...
	@errmsg 			varchar(255),
	@errno 				int,
	@gl_export_procedure		varchar(30),
...
SELECT @journal_entry_description = journal_entry_description  + ' ' + CONVERT (CHAR(8), getdate(), 1),
...
IF @immediate_dayend_requested = 9  --dayend still has more batches to process
BEGIN
  SELECT @trace_msg = ':LOG ===> Deferring period end until next loop since dayend still has more batches to process on next loop first at: ' + CONVERT(CHAR, getdate(), 8)
  PRINT @trace_msg
  RETURN  --so don't run period end yet
END  --IF @immediate_dayend_requested = 9

IF @last_date_closed = @period_end_date AND
   @preliminary_period_end_date IS NULL AND
   (@gl_interface_timing <> 3  
    OR (@gl_interface_timing = 3 AND @immediate_dayend_requested = 9)
   ) --9 means subsequent runs required to finish dayend.
  RETURN
...
IF @errno <> 0
  BEGIN
   SELECT @errmsg = 'Failed to reset period_end_only flag',
             @object_name = 'auditworks_system_flag',
...
WHILE 1 = 1
  BEGIN
    ...
    BEGIN
      SELECT @trace_msg = ':LOG ===> SLEEPING in PERIOD END TIMEOUT waiting for other streams dayend at: ' + CONVERT(CHAR, getdate(), 8)
      PRINT @trace_msg
      BREAK
    ...
    IF @loop_counter = 1 
    BEGIN
      SELECT @trace_msg = ':LOG ===> SLEEPING in PERIOD END for 180 seconds at: ' + CONVERT(CHAR, getdate(), 8)
      PRINT @trace_msg
    END
...
IF @loop_counter > 1 
BEGIN
  SELECT @trace_msg = ':LOG ===> END OF SLEEPING waiting for other streams dayend in PERIOD END at: ' + CONVERT(CHAR, getdate(), 8)
  PRINT @trace_msg
...
BEGIN
  ...
  SELECT @trace_msg = ':LOG ===> Period-End NOT run because there are stores/dates ' + CONVERT(CHAR(5),@store_count) + ' in dayend_workload.'
  PRINT @trace_msg
  SELECT @trace_msg = ':LOG ===> NO GL Interface File will be created. '
  PRINT @trace_msg
...
BEGIN
  ...
  BEGIN
    ...
    SELECT @trace_msg = ':LOG ===> Period-End NOT run yet because there are ' + CONVERT(CHAR(5),@store_count) + ' store-dates which have not yet been dayended.'
    PRINT @trace_msg
...
EXEC update_process_step_log_$sp 18, 1, @step_no, 1, 0, @current_date_time
SELECT @errno = @@error
IF @errno != 0
  BEGIN
...
IF (@transaction_count1 = 0 AND @scaleout_flag IN (0,1)) -- no work to do
  ...
  BEGIN
    /* Removed since G/L export which might be running on peripherals has not yet happened and already done by reset_period_end_$sp
    IF @last_date_closed <> @period_end_date
    BEGIN
      UPDATE parameter_general
      ...
      IF @errno <> 0
        BEGIN
  	  SELECT @errmsg = 'Unable to set last_date_closed to period_end_date in parameter_general (1)',
		 @object_name = 'parameter_general',
		 @operation_name = 'UPDATE'
	  GOTO error
        END
...
IF (@period_end_only >= 1 OR @immediate_dayend_requested = 1) AND @scaleout_flag > 0
  BEGIN -- disallow request if some store-dates are not completed
   SELECT @store_count = 1
...
IF @scaleout_flag = 2 AND @gl_export_procedure IS NOT NULL -- consolidated sa db in a scaleout configuration
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT @trace_msg = ':LOG ===> SLEEPING in PERIOD END TIMEOUT waiting for peripheral period-ends at: ' + CONVERT(CHAR, getdate(), 8)
      PRINT @trace_msg
      BREAK
    ...
    IF @loop_counter = 1 
    BEGIN
      SELECT @trace_msg = ':LOG ===> SLEEPING in PERIOD END for 180 seconds waiting for peripheral period-ends at: ' + CONVERT(CHAR, getdate(), 8)
      PRINT @trace_msg
      WAITFOR DELAY '0:03:00' -- 180 seconds sleep cycle
    END  --IF @loop_counter = 1
  ...
  IF @loop_counter > 1
  BEGIN
    SELECT @trace_msg = ':LOG ===> END OF SLEEPING in PERIOD END at: ' + CONVERT(CHAR, getdate(), 8)
    PRINT @trace_msg
  END

  IF @store_count = 0
  BEGIN
    SELECT @trace_msg = ':LOG ===> SLEEPING in PERIOD END for 5 minutes at: ' + CONVERT(CHAR, getdate(), 8)
PRINT @trace_msg
...
EXEC update_process_step_log_$sp 18, 1, @step_no, NULL, 1, NULL  /* */
SELECT @errno = @@error
IF @errno != 0
  BEGIN
   SELECT @errmsg = 'Failed to execute stored proc update_process_step_log_$sp for step ' + CONVERT(varchar, @step_no),
...
IF @errno <> 0
BEGIN
  SELECT @errmsg = 'SET flag_numeric_value WHERE flag_name = "period_end_completed_by"',
         @object_name = 'auditworks_system_flag',
         @operation_name = 'UPDATE'
  GOTO error
END
...
IF @errno <> 0
BEGIN
  SELECT @errmsg = 'Unable to set period end complete date',
         @object_name = 'auditworks_system_flag',
         @operation_name = 'UPDATE'
  GOTO error
END

IF @preliminary_period_end_date IS NOT NULL --
BEGIN
  UPDATE parameter_general
  ...
  BEGIN 
    ...
    GOTO error
  END
END -- IF @preliminary_period_end_date IS NOT NULL --
*/
...
/*  Removed since must leave request outstanding if period end failed
        IF @preliminary_period_end_date IS NOT NULL --
        BEGIN
          UPDATE parameter_general
          SET preliminary_period_end_date = NULL --
