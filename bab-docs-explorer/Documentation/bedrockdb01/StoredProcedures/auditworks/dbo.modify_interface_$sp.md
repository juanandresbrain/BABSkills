...
```sql
CREATE proc  dbo.modify_interface_$sp (
...
PROC NAME: modify_interface_$sp
    ...
    work_interface_control  is the original interface_control of the transaction; Needed to rollback changes;
				 populated by copy_transaction_$sp and av_transaction_modify_$sp;
    ...
    work_control_interfaces is the work table which receives the OUT/IN which will eventually go
			    ...
			    'types' in this table are 'i'=IN, 'o'=OUT, 'c'= interface_control for new version of tran.
			    'c' is used to delete and then re-insert the final entries in table interface_control
			    including any interfaces that are newly affected due to modified lines.
...
Jul08,16 Vicci    DAOM-973 Reset @errmsg after returned by sub-proc calls if there was no error since sub-procs set it if they use try/catch 
                           even if no error occurs, and otherwise a completely irrelevant error message is logged if a untrappable failure occurs.
Jan26,15 Vicci  TFS-150357 Refix 1-4C6V8J to match its comment (i.e. take card_type ? into account) such that if an encrypted/tokenized card# 
                           has been corrected or voided by the UI the reason 2 Invalid Card rejection is removed.
May11,15 Vicci  TFS-119009 If there is no longer any card to be validated in the transaction (for example the tender was changed to cash) do not leave behind Invalid Card I/F rejects.
                           Also, fix the join from if rejection reason to transaction line for I/F Reject 2 (invalid card) to include transaction_id!
May11,15 Vicci  TFS-119009 Leave interfaces that care about invalid cards marked as having rejects when left behind Invalid card rejects (for cards that were encrypted and could not be checked again) continue to exist.
...
Feb18,14 Phu      1-4C6V8J Don't remove reject reason 2: Invalid card number if card type is ? and (card # is encrypted or manual_active_flag is off)
Sep23,13 Vicci      146826 Expand @errmsg since expanded in verify_upc_$sp.
Mar03,11 Vicci      125554 Mark interface with status 99 when an I/F reject applies to it even if its applicability_method is > 1 and
                           it therefore doesn't have any interface_control entries.
                           Prevent transaction from becoming a Tax reject if it was previously valid.
Feb03,11 Vicci      124563 Don't populate work_if_rejects since this is already done by verify_user_if_rejects_$sp.
			   Ensure that user cannot save changes that would cause a previously valid transaction to 
			   become an I/F Reject even if the latter is user-defined.
Aug31,10 Paul       120039 prevent double posting of 10 to preaudit interfaces by avoiding premature delete of
				work_interface_control, code reviewed.
...
May10,04 Maryam    DV-1071 Modified to receive @process_id and pass it to the sub procs
Apr29,04 Sab       DV-1068 remove call to verify_glc_$sp, and IF @glc_prevalidation_flag = 1
Apr25,08 Phu        100169 Fix problem of creating I/F rejects when voiding the transaction.
...
SELECT @errno = @@error, @rows = @@rowcount -- safety check
IF @errno != 0 OR @rows = 0
BEGIN
  SELECT @errmsg = 'Failed to find transaction_header',
...
WHERE ir.transaction_id = @transaction_id
AND (ir.if_reject_reason <> 2  
     OR (ir.if_reject_reason = 2  --bad card
         AND (  --card will be validated by this proc
                EXISTS (SELECT 1 FROM transaction_line l
                         WHERE l.transaction_id = @transaction_id
                           AND ir.line_id = l.line_id
                           AND LEN(COALESCE(l.invalid_reference_no, l.reference_no, '0')) <= 20
                           AND l.reference_type IN (SELECT r.reference_type
                                                      FROM reference_type_validation r
                                                     WHERE r.validation_type  = 1
                                                       AND r.manual_active_flag = 1
                                                   )
                       )
              OR --there is no longer any card to be validated in the transaction (for example the tender was changed to cash or the line was voided)
                 NOT EXISTS (SELECT 1 
                               FROM transaction_line l
                                    INNER JOIN transaction_header h
                                       ON h.transaction_id = @transaction_id
                              ...
                              WHERE l.transaction_id = @transaction_id
                                ...
                                AND l.reference_type IN (SELECT r.reference_type
                                                           FROM reference_type_validation r
                                                          WHERE r.validation_type  = 1
                                                            AND (r.manual_active_flag = 1 OR r.edit_active_flag = 1)
                                                        )
                            )
              OR --card line was modified and validated in the UI and card_type is no longer '?'
                 EXISTS (SELECT 1 
                           FROM transaction_line l
                                INNER JOIN authorization_detail a
                                   ON l.transaction_id = a.transaction_id
                  AND l.line_id = a.line_id
                                  AND a.card_type <> '?'
                          WHERE l.transaction_id = @transaction_id
                            AND ir.line_id = l.line_id
                            AND l.line_modified_flag = 1
                            AND l.reference_no <> COALESCE(ir.memo1, l.reference_no)
                            AND LEN(COALESCE(l.invalid_reference_no, l.reference_no, '0')) > 20
                            AND l.reference_type IN (SELECT r.reference_type
                                                       FROM reference_type_validation r
                                                      WHERE r.validation_type  = 1
...
IF @errno != 0
BEGIN
  SELECT @errmsg = 'Failed to DELETE work_interfaces_modify',
...
/* Determine to which interfaces the modification applies based on interface_applicability 
   for the transaction lines that were modified */
...
SELECT DISTINCT @process_id, 'o', @copy_transaction_id, wc.interface_id, 20, id.customer_liability_check
  FROM work_interface_control wc WITH (NOLOCK)
       INNER JOIN interface_directory_lookup id WITH (NOLOCK)
          ON wc.interface_id = id.interface_id
         AND id.all_modifications_relevant = 0
         AND (id.live_date IS NULL OR id.live_date <= @transaction_date)
       INNER JOIN transaction_line tl_new WITH (NOLOCK)
          ON tl_new.transaction_id = @transaction_id
         AND tl_new.line_modified_flag = 1
       INNER JOIN if_transaction_line tl_old WITH (NOLOCK)
          ON tl_old.if_entry_no = @copy_transaction_id
         AND tl_old.line_id = tl_new.line_id
       INNER JOIN interface_applicability ia WITH (NOLOCK)
          ON wc.interface_id = ia.interface_id
         AND (   (    tl_old.line_action = ia.line_action 
                  AND tl_old.line_object = ia.line_object 
                  AND ia.transaction_category = @old_transaction_category) 
              OR (    tl_new.line_action = ia.line_action
                  AND tl_new.line_object = ia.line_object
                  AND ia.transaction_category = @transaction_category))
        LEFT OUTER JOIN settlement_service ss WITH (NOLOCK)
          ON ia.interface_id = ss.interface_id
        LEFT OUTER JOIN authorization_detail ad_new WITH (NOLOCK)
          ON tl_new.transaction_id = ad_new.transaction_id
         AND tl_new.line_id = ad_new.line_id
        LEFT OUTER JOIN if_authorization_detail ad_old WITH (NOLOCK)
          ON ad_old.if_entry_no = @copy_transaction_id
         AND tl_new.line_id = ad_old.line_id         
 WHERE wc.process_id = @process_id
   ...
   AND (ss.interface_id IS NULL OR COALESCE(ad_new.other_id, '-') <> 'HostCapture' OR COALESCE(ad_old.other_id, '-') <> 'HostCapture')
SELECT @errno = @@error
IF @errno != 0
BEGIN
...
/* Check for interfaces affected by adding new transaction lines */
...
SELECT DISTINCT @process_id, 'o', @copy_transaction_id, wc.interface_id, 20, id.customer_liability_check
  FROM work_interface_control wc WITH (NOLOCK)
       ...
       INNER JOIN transaction_line tl_new WITH (NOLOCK)
         ...
         AND tl_new.line_modified_flag = 1
       INNER JOIN interface_applicability ia WITH (NOLOCK)
          ON ia.interface_id = wc.interface_id
         AND ia.line_action = tl_new.line_action
         AND ia.line_object = tl_new.line_object
         AND ia.transaction_category = @transaction_category
        LEFT OUTER JOIN settlement_service ss WITH (NOLOCK)
          ON ia.interface_id = ss.interface_id
        LEFT OUTER JOIN authorization_detail ad_new WITH (NOLOCK)
 ...
 WHERE wc.process_id = @process_id
   ...
   AND (ss.interface_id IS NULL OR COALESCE(ad_new.other_id, '-') <> 'HostCapture')
SELECT @errno = @@error
IF @errno != 0
BEGIN
  SELECT @errmsg = 'Failed to INSERT work_control_interfaces (added lines)',
         @object_name = 'work_control_interfaces',
         @operation_name = 'INSERT'
  GOTO error
END
END /* if new lines have been added */
...
IF @interface_in = 0
BEGIN /* If the new tran (void) doesn't feed any interfaces then post reversals to cust liability and then exit */
  -- 1-E2PTQ
  BEGIN TRANSACTION
...
SELECT ia.interface_id,
  ...
  FROM transaction_line tl WITH (NOLOCK)
       INNER JOIN interface_applicability ia WITH (NOLOCK)
          ON tl.line_action = ia.line_action
         AND tl.line_object = ia.line_object
         AND ia.transaction_category = @transaction_category
       INNER JOIN interface_directory_lookup id WITH (NOLOCK)
          ON id.interface_id = ia.interface_id
         AND id.applicability_method = 0
         AND (@transaction_void_flag IN (0,8) OR id.interface_voided_transactions = 1)
         AND (id.live_date IS NULL OR id.live_date <= @transaction_date)
        LEFT OUTER JOIN settlement_service ss WITH (NOLOCK)
          ON id.interface_id = ss.interface_id
        LEFT OUTER JOIN authorization_detail ad WITH (NOLOCK)
...
/* Always look up upc's but only reject if @upc_check or @cost_check >= 1 */
...
BEGIN
  UPDATE work_interfaces_modify
     SET interface_status = 99 
    FROM if_rejection_applicability ia WITH (NOLOCK)
   WHERE work_interfaces_modify.process_id = @process_id
     AND work_interfaces_modify.interface_id = ia.interface_id
...
BEGIN
  ...
  BEGIN
    ...
    IF @errno != 0
    BEGIN
     SELECT @errmsg = 'Failed to mark interfaces that care about invalid cards as having rejects. ',
            @object_name = 'work_interfaces_modify',
            @operation_name = 'UPDATE'
     GOTO error
    END
  ...
  END -- If (@execret = 1)
  ELSE
  BEGIN  --If no new rejects have been detected still check if any left behind Invalid card rejects (for cards that were encrypted and could not be checked again) exist.
    UPDATE work_interfaces_modify
      SET interface_status = 99 
      FROM if_rejection_reason r WITH (NOLOCK)
     WHERE work_interfaces_modify.credit_card_check >= 1
       AND work_interfaces_modify.process_id = @process_id
       AND r.transaction_id = @transaction_id
       AND r.if_reject_reason = 2
    SELECT @errno = @@error
    IF @errno != 0
    BEGIN
     SELECT @errmsg = 'Failed to mark interfaces that care about invalid cards as still having rejects when their cards cannot be revalidated by the back end because they are tokenized/encrypted. ',
            @object_name = 'work_interfaces_modify',
            @operation_name = 'UPDATE'
     GOTO error
    END
  END --ELSE of IF (@execret = 1)
...
/* Return Values: 0 = no rejects exist.
  ...
  BEGIN
      ...
      UPDATE work_interfaces_modify
        SET interface_status = 99 
        FROM work_interfaces_modify wm WITH (NOLOCK),
	     work_interface_reject wr WITH (NOLOCK),
      ...
      IF @errno != 0
      BEGIN
        SELECT @errmsg = 'Failed to set interface_status (store check).',
            @object_name = 'work_interfaces_modify',
            @operation_name = 'UPDATE'                
        GOTO error
      END
  ...
  END -- IF @execret > 0 
END -- IF (@store_check + @stock_origin_store_check + @merch_origin_store_check + @merch_source_store_check + @merch_fulfillment_store_check > 0)
...
IF @execret >= 1 AND @allow_saving_if_rejects = 0
   AND EXISTS (SELECT 1
                 FROM work_interfaces_modify ti WITH (NOLOCK)
                WHERE ti.process_id = @process_id
                  AND ti.interface_status = 99
                  AND (ti.exception_jurisdiction_check > 0 OR ti.tax_default_check > 0)
                  AND ti.interface_id NOT IN (SELECT wc.interface_id 
                                                FROM work_interface_control wc WITH (NOLOCK)
                                               WHERE wc.process_id = @process_id
                                                 AND wc.if_entry_no = @copy_transaction_id
                                                 AND wc.interface_status_flag = 99))
...
BEGIN
  ...
  EXEC verify_user_if_rejects_$sp @process_id, @user_id, @transaction_id, @errmsg OUTPUT, @allow_saving_if_rejects, @copy_transaction_id  
  SELECT @errno = @@error
  IF @errno != 0
  BEGIN
...
/* Insert a row for each interface that needs to exist in table interface_control after transaction save */
...
SELECT @process_id,
   ...
   FROM work_interfaces_modify WITH (NOLOCK)
  WHERE process_id = @process_id
    AND (applicability_method <= 1 OR interface_status = 99)
...
/* If the modified transaction did not previously belong to a particular preaudit interface and now does,
   or was i/f rejected from a particular preaudit interface and no longer is, or was sa rejected,
   insert an entry for it into work_control_interfaces with a status of new (10).
   ...
   For transaction add, all active preaudit interfaces will be inserted with a status of new. */
/* Note:  entry_no will be corrected in a subsequent step */
...
SELECT DISTINCT @process_id, 'i', @transaction_id, new_if.interface_id, 10, new_if.customer_liability_check 
 ...
 WHERE new_if.process_id = @process_id
   ...
   AND new_if.interface_control_flag = 1
   AND (
        (@function_no != 154  /* exclude preaudit interfaces that were originally present in interface_control */
...
-- The remaining Ex_Queue rows will be inserted by the calling procs
...
error:   /* Common error handler. */
	...
	IF @errno > 201500
	  BEGIN /* clean up work tables to avoid dead rows (ignore any errors).
	  		work_interface_control is cleaned up by restore_original_trans_$sp and by calling procs */
