...
flowchart LR
    ...
    delete_str_reg_date_asynch__sp(["delete_str_reg_date_asynch_$sp"]) --> SP
...
| delete_str_reg_date_asynch_$sp |
| dw_rebuild_request |
...
Desc : To re-evaluate most types of I/F rejects and to also revalidate some types of sa rejects. For i/f and
       ...
       with ENTY_TYPE = 'ALL' (only if they pick ALL in the S/A Reject window header) or that corresponding to the reason 
       selected otherwise (depending on user's selection).
...
Feb29,16 Vicci       TFS-148372
                    /  DAOM-166  Release single transaction manual functions to cleanup if they are more than 1/2 hour old and are not a child process of another process,
                                 since this would imply that the connection was killed before the manual function had time to complete.
Feb23,16 Vicci           DAOM-1  Handle function 33 (Mass Deletion Request).
Jan20,16 Vicci       TFS-142941  Only clean up mass correct tax if it wasn't called by the mass correct upc (otherwise the mass correct upc recovery will handle it).
...
--To handle case where connection was killed before UI could complete, release single transaciton manual functions to cleanup if they are more than 30 minutes old.
IF EXISTS (SELECT 1 
            FROM function_status WITH (NOLOCK)
           WHERE COALESCE(released_to_cleanup, 0) <> 1
             AND DATEDIFF(mi,entry_date,getdate()) > 30 -- > 30 min ago
             AND function_no in (100, 150, 35)
             AND COALESCE(transaction_id, 0) <> 0)
BEGIN
  UPDATE function_status
     SET released_to_cleanup = 1
   WHERE COALESCE(function_status.released_to_cleanup, 0) <> 1
     AND DATEDIFF(mi,function_status.entry_date,getdate()) > 30 -- > 30 min ago
     AND function_status.function_no in (100, 150, 35)
     AND COALESCE(function_status.transaction_id, 0) <> 0
     AND NOT EXISTS (SELECT 1 FROM function_status p
                      WHERE function_status.process_id = p.process_id
                        AND p.released_to_cleanup = 1); 
  SELECT @errno = @@error;
  IF @errno != 0
  BEGIN
    SELECT @errmsg = 'Failed to release any old dead manual functions to cleanup.  ',
...
IF @errno != 0
BEGIN
  SELECT @errmsg = 'Failed to determine last_date_closed.  ',
...
IF @stream_no = 1 AND 
   EXISTS (SELECT 1 
            FROM rebuild_request_detail rd
                 INNER JOIN rebuild_request r
                    ON r.request_id = rd.request_id
                   AND r.request_datetime <= @job_start_time
                   AND r.rebuild_type = 4  --current transaction tax-detail rebuild
            WHERE rd.rebuild_type = 4  --current transaction tax-detail rebuild
              AND rd.transaction_date > @last_date_closed
...
IF EXISTS (SELECT 1 
            ...
            WHERE f.function_no = 33
              AND f.status = 1  --i.e. status = "released to backend"
           )
BEGIN 
  EXEC update_chng_rspns_queue_$sp 'ASYNCH_DELETE_SRD';
  SELECT @errno = @@error
  IF @errno != 0
  BEGIN
    SELECT @errmsg = 'Failed to determine if any outstanding background mass transaction deletion requests exist.  ',
           @object_name = 'update_chng_rspns_queue_$sp',
           @operation_name = 'EXECUTE'
    GOTO error
  END
...
SELECT SUBSTRING(par_name, 1, CHARINDEX('stream', par_name) - 1) ENTY_TYPE, MAX(CONVERT(tinyint, par_value)) stream_no
 ...
 WHERE par_name LIKE '%stream'
   ...
   AND SUBSTRING(par_name, 1, CHARINDEX('stream', par_name) - 1) IN ('FUTURE_DATE', 'GENINFO', 'TAX_DFLT_EXCPT_MERGE', 'BUSINESS_DATE', 'CARD_TYPE', 'CUST_LIAB', 'EMPLY', 'LINE_OBJECT', 'ORG_CHN', 'SETTLE', 'TAX', 'TAX_ISSUE', 'TAX_REBUILD', 'UPC', 'USER_DEFINED', 'ASYNCH_DELETE_SRD')
...
SELECT @recovery_rows = COUNT(process_id)
  FROM function_status WITH (NOLOCK)
 WHERE (   function_no IN (66,78,79,80,81,82,83,88,89,91,93,94,95,96,113,124,33) -- mass correct functions
   ...
   AND (   user_id = -1 --excludes upc reassign
        OR function_no = 33) --mass deletion request has function status logged by real user
   AND (function_no <> 33 OR status > 1)  --i.e. if mass deletion request (33) only pick up stuff with status > "released to backend"
   AND (function_no NOT IN (78, 82) OR released_to_cleanup = 1)
   AND (function_no <> 89 OR lock_by_spid IS NULL OR move_flag <> 5)  --i.e. is not the mass correct tax or the mass correct tax wasn't called by the mass correct upc (move flag is revalidation type)
...
SELECT @job_complete = 0
  ...
  FOR
  SELECT DISTINCT CASE WHEN q.ENTY_TYPE IN ('FUTURE_DATE', 'GENINFO', 'TAX_DFLT_EXCPT_MERGE','ASYNCH_DELETE_SRD') THEN ' ' + q.ENTY_TYPE ELSE q.ENTY_TYPE END
   ...
   ORDER BY CASE WHEN q.ENTY_TYPE IN ('FUTURE_DATE', 'GENINFO', 'TAX_DFLT_EXCPT_MERGE','ASYNCH_DELETE_SRD') THEN ' ' + q.ENTY_TYPE ELSE q.ENTY_TYPE END /* to ensure that ENTY_TYPEs not processed by 'ALL' option are done before the 'ALL' which ends the loop.*/
  ...
  BEGIN
   ...
   IF LTRIM(@ENTY_TYPE) IN ('ASYNCH_DELETE_SRD', 'ALL')
   --When all is requested, mass deletion requests first since they may remove s/a and i/f rejects
     BEGIN
        SELECT @errmsg = 'Failed to process background mass transaction deletion requests. ',
 	   @object_name = 'delete_str_reg_date_asynch_$sp';
      EXEC delete_str_reg_date_asynch_$sp;
     END -- background asynchronous mass transaction deletion requested
