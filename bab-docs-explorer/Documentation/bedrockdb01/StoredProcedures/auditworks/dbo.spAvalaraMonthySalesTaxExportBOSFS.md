# dbo.spAvalaraMonthySalesTaxExportBOSFS
...
```mermaid
flowchart LR
    SP["dbo.spAvalaraMonthySalesTaxExportBOSFS"]
...
```sql
CREATE   procedure [dbo].[spAvalaraMonthySalesTaxExportBOSFS]
--@TransactionStartDate as int
--,@TransactionEndDate as int

AS
-- =====================================================================================================
-- Name: spAvalaraMonthySalesTaxExport_BOSFS
--
-- Description:	Monthly Sales Tax Export for BOSFS as requested by Tax Dept
--	
--
-- Input:  n/a
--			
...
--	AND notification_name = 'Avalara Tax BOSFS file generated'
--END
...
SET @TransactionStartDate = '2020-05-31'
SET @TransactionEndDate = '2020-07-04'
...
--AND		th.register_no in (52, 53, 56, 57)  -- 52 = Web Reg 2 (default sales), 53 = Web Reg 3 (PayPal), 56 = Web Reg 6 (Amazon), 57 = Web 7 (refunds/returns)
...
SET @FileName = '\AVALARA_EXPORT_BOSFS_' + @cntrycode + '_' + @FileDate + '.csv'
...
SET @CMD = 'xcopy /y /v /f /r  ' + @BackupFilePath + '\AVALARA_EXPORT_BOSFS_*_' + @FileDate + '.csv' + ' ' + @AvalaraFilePath
    select  @CMD
    exec master..xp_cmdshell @CMD
...
EXEC master..xp_cmdshell @ChkFileCMD  
SET @ChkFileCMD = 'dir /B ' + @ChkFileDrive + 'AVALARA_EXPORT_BOSFS_*_' + @FileDate + '.csv'  
...
SET @Text = 
		...
		'Avalara Sales Tax export file for BOSFS has been created for import to Avalara. <br>' +
		'Transaction date range... <br>' +
		'From: ' + CONVERT(VARCHAR(10),@TransactionStartDate) + '<br>' +
		'To: ' + CONVERT(VARCHAR(10),@TransactionEndDate) + ' <br>' +
		'<br>' +
		'(' + @ChkFileCount + ') AVALARA_EXPORT_BOSFS_*_' + @FileDate + '.csv files found in ' + @AvalaraFilePath + '. <br>' +
		'<br>' +
		'<table border="1">' + 
		'<font face =arial size = 2>' +
		'<tr bgcolor=#D5D5F7><th>Avalara Sales Tax for BOSFS export file name</th></tr>' +
		...
		'Job Name:  Avalara_Sales_Tax_BOSFS_File_Export <br>' +
		'Stored Proc:  BEDROCKDB01.auditworks.dbo.spAvalaraMonthySalesTaxExportBOSFS <br>' +
		'Created by:  Keith Lee <br>' +
		'Team Ownership:  Enterprise Systems <br>'

SET @Subject = 'Avalara Sales Tax BOSFS export file created'
...
WHERE notification_name = 'Avalara Tax BOSFS file generated'
...
VALUES (
'spAvalaraMonthySalesTaxExportBOSFS', --<< Stored Proc name
GETDATE(),
...
'Avalara Sales Tax BOSFS export file has been created for import to Avalara' --<< Comment
)
...
SET @Subject = 'WARNING - Missing all (' + CONVERT(VARCHAR(1),(SELECT COUNT(*) FROM ##TaxCountryList)-@ChkFileCount) + ') Avalara Sales Tax export files'
	...
	@Recipients, --<< Email CC
	@Subject, --<< Email Subject
	'SA export files Missing for Avalara BOSFS' --<< Comment
	)
...
SET @Subject = 'ALERT - Missing (' + CONVERT(VARCHAR(1),(SELECT COUNT(*) FROM ##TaxCountryList)-@ChkFileCount) + ') Avalara Sales Tax BOSFS export files'
	EXEC msdb.dbo.sp_send_dbmail  
	@profile_name = 'SAAdmin',
	@recipients = @Copy_Recipients,
	@copy_recipients = @Recipients,
