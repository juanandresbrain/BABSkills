# dbo.spAvalaraMonthlyVATTaxExport_IDW
...
```mermaid
flowchart LR
    SP["dbo.spAvalaraMonthlyVATTaxExport_IDW"]
    ...
    dbo_tax_detail(["dbo.tax_detail"]) --> SP
...
```sql
CREATE procedure [dbo].[spAvalaraMonthlyVATTaxExport_IDW]
...
SET @TransactionStartDate = '2025-08-03'
SET @TransactionEndDate = '2025-08-30'
...
SELECT	tl.line_id
		...
		,case when tl.reference_no = '483502' then 0 else td.taxable_amount end AS TaxableBasis
		,case when tl.reference_no = '483502' then 0 else td.tax_amount_expected end	AS ValueVAT
		,case when tl.reference_no = '483502' then 0 else td.tax_amount_expected + td.taxable_amount end AS 'TotalValueLine'		
		,CASE WHEN tl.db_cr_none = -1 THEN '0'
			ELSE '0'
...
INTO	##TaxSalesDetail
FROM 	auditworks.dbo.transaction_header th WITH (NOLOCK)
JOIN	auditworks.dbo.transaction_line tl WITH (NOLOCK) ON th.transaction_id=tl.transaction_id 
...
LEFT JOIN	auditworks.dbo.tax_detail td WITH (NOLOCK) ON th.transaction_id = td.transaction_id AND tl.line_id = td.line_id
JOIN	##TaxStoreData sd WITH (NOLOCK) ON th.store_no = sd.ORG_CHN_NUM
left join ##taxedDonations tdo on  tl.transaction_id = tdo.transaction_id and tl.line_id = tdo.line_id 
...
FROM 	auditworks.dbo.transaction_header th WITH (NOLOCK)
JOIN	auditworks.dbo.transaction_line tl WITH (NOLOCK) ON th.transaction_id=tl.transaction_id 
JOIN	auditworks.dbo.merchandise_detail smd WITH (NOLOCK) ON th.transaction_id = smd.transaction_id AND tl.line_id = smd.line_id
LEFT JOIN	auditworks.dbo.tax_detail td WITH (NOLOCK) ON th.transaction_id = td.transaction_id AND tl.line_id = td.line_id
...
JOIN	auditworks.dbo.merchandise_detail smd WITH (NOLOCK) ON th.transaction_id = smd.transaction_id AND tl.line_id = smd.line_id
LEFT JOIN	auditworks.dbo.tax_detail td WITH (NOLOCK) ON th.transaction_id = td.transaction_id AND tl.line_id = td.line_id
JOIN	auditworks.dbo.customer cs WITH (NOLOCK) ON cs.transaction_id = th.transaction_id
JOIN	##TaxStoreData sd WITH (NOLOCK) ON th.store_no = sd.ORG_CHN_NUM
--JOIN	##TaxCodes tc WITH (NOLOCK) ON tc.tax_item_group_id = td.tax_item_group_id
WHERE	sd.StrCountry IN ('GBR')
AND		th.store_no BETWEEN 1 AND 3100
...
SET @FilePath = '\\saapp01\Financials\Avalara\Test'  --<< File path
