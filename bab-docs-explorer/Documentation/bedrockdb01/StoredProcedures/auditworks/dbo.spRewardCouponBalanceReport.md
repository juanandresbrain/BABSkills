# dbo.spRewardCouponBalanceReport
...
```mermaid
flowchart LR
    SP["dbo.spRewardCouponBalanceReport"]
    cust_liability(["cust_liability"]) --> SP
    dbo_cust_liability_cancelledExcludingRedeemed(["dbo.cust_liability_cancelledExcludingRedeemed"]) --> SP
...
|---|
| cust_liability |
| dbo.cust_liability_cancelledExcludingRedeemed |
...
```sql
--exec  [dbo].[spRewardCouponBalanceReport]
...
SET @recipients = 'ChadV@buildabear.com; DavidW@buildabear.com'
...
and [reference_no] not in (select voucherCode from [dbo].[cust_liability_cancelledExcludingRedeemed]) -- duplicate vouchers mass cancelled 10/22
AND ISNULL (country,'USA') NOT IN ('UK','GB','GBR' , 'CAN' , 'CAF','CA') 
