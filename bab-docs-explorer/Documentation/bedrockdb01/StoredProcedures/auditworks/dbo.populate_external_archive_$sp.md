...
Jul05,17 Kiri       DAOM-2357 Correct recovery flag adjusting it to reflect data in header, add merch_detail
...
BEGIN TRY
  ...
  */
  ...
  BEGIN
     SELECT @errmsg = 'Unable to test ex_transaction_header for duplicates',
           @object_name = 'ex_merchandise_detail',
           @operation_name = 'SELECT'; 
  INSERT INTO work_ext_copy_trans
    SELECT DISTINCT w.av_transaction_id
	    FROM ex_merchandise_detail h, work_purge_av_detail w
     WHERE h.av_transaction_id >= @min_tran_id -- use range query to improve query plan for cross-server joins
       AND h.av_transaction_id <= @max_tran_id
       AND h.av_transaction_id = w.av_transaction_id
       AND h.transaction_date >= @min_transaction_date
  ...
  WHILE @rows_inserted = @rows_per_batch
    BEGIN    
        ...
        TRUNCATE TABLE work_ext_copy;
          ...
          SELECT @errmsg = 'Unable to get next batch from work_purge_av_detail for av_merchandise_detail',
      ...
      /* Post transaction details across to external database */
        SELECT @errmsg = 'Unable to insert into ex_merchandise_detail ',
               @object_name = 'ex_merchandise_detail',
               @operation_name = 'INSERT';      
      INSERT INTO ex_merchandise_detail
        SELECT am.* 
          FROM work_ext_copy w, av_merchandise_detail am
         WHERE w.av_transaction_id = am.av_transaction_id
           AND am.transaction_date >= @min_transaction_date
           AND am.transaction_date <= @max_transaction_date;
