...
```sql
create proc dbo.get_import_info_$sp @import_name nvarchar(60) = NULL
...
PROC NAME: get_import_info_$sp
  ...
  HISTORY:
Date     Name           Def#  Desc
Mar30,15 Vicci      DAOM-304  Handle tax_schedule_point
Oct20,15 Vicci    TFS-145937  Don't ignore changes to which import layout is active.
...
SELECT @message_id = 201068,
       ...
       @cur_date = convert(nvarchar, getdate(), 112) + convert(nvarchar, getdate(), 108),
       @cl_type = 0, @employee_type = 0, @process_no = 7,
       @store_schedule = 0, @tax_item_group = 0, @tax_class = 0, @tax_style = 0, @tax_schedule_point = 0, 
...
IF CHARINDEX('tax_schedule_point', @import_name) > 0
  SELECT @tax_schedule_point = 1;
...
BEGIN
  ...
  AND CHARINDEX(RTRIM(STUFF(import_control_file, CHARINDEX('.GO', import_control_file), 3, '   ')), @import_name) = 1
           AND ((RTRIM(STUFF(import_control_file, CHARINDEX('.GO', import_control_file), 3, '   ')) NOT IN ('class', 'employee', 'store', 'style', 'tax_item', 'tax_schedule', 'N/A', 'CL'))
                ...
                OR (RTRIM(STUFF(import_control_file, CHARINDEX('.GO', import_control_file), 3, '   ')) = 'tax_schedule' AND @tax_schedule_point = 0)
