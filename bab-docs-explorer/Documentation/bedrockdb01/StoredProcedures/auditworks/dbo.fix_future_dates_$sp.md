...
```sql
CREATE proc  dbo.fix_future_dates_$sp 
...
HISTORY:
Date     Name		   Def# Desc
Aug16,16 Vicci        DAOM-1171 Also revalidate reason 39=Store/business date in use by another process
	...
	LEFT OUTER JOIN transaction_category tc WITH (NOLOCK) 
	  ...
	  ON th.transaction_id = sr.transaction_id
         AND ((sr.violated_sareject_rule = 3  --Future date
              ...
              OR
              (sr.violated_sareject_rule IN (10, 12, 39) --Business date prior to store open to receive date, Business date prior to store opening date rejects, Store/business date in use by another process
               AND @process_no = 97)
              OR
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      IF    (@trickle_in_progress_flag > 0 AND @process_no <> 1) 
         OR (@update_in_progress > 0 AND (@update_in_progress <> 1 OR @process_no <> 1)) -- skip if store-date is locked (by a function other than the Edit when the Edit is running the revalidation)
        CONTINUE
...
--In case any store/date were locked and therefore skipped.
IF @frontend_populated = 1
BEGIN
  DELETE work_transaction_move 
   WHERE process_id = @process_id
