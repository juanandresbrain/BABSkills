...
Oct15,15 Vicci    TFS-144273 Translate Type (old Register Type) 1 means UTF8 pollfile (with or without BOM);  1252 means ASCII-CodePage1252 pollfile.
                             Don't hard-code to 1 when exporting, instead select the for the translate version in question from code_description alpha-code.
  ...
  BEGIN
    ...
    END --There is at least one mask
      ...
      IF @EXISTING_TRNSLT_VRSN_NUM IS NULL -- row doesn't exist
        BEGIN
          SELECT @pollfile_encoding_type        = NULL
          
          SELECT @pollfile_encoding_type = CONVERT(int, alpha_code)
            FROM code_description 
           WHERE code_type = 197
             AND code = @TRAN_TRNSLT_VRSN_NUM
             AND IsNumeric(alpha_code) = 1
          SELECT @errno = @@error
          IF @errno <> 0
          BEGIN
            SELECT @errmsg = 'Failed to determine pollfile encoding type expected by translate version.',
          ...
          INSERT export_register(
                 ...
                 store_no,
                 register_no,
                 register_type,  --known as Translate Type in Foundation, but represents encoding of pollfile.
                 translate_lookup_version,
                 sa_company_no)
          VALUES(@register_poll_id,
                 @ORG_CHN_NUM,
                 @WRKSTN_NUM,
                 COALESCE(@pollfile_encoding_type, 1),
                 @TRAN_TRNSLT_VRSN_NUM,
                 @sa_company_string)
          SELECT @errno = @@error
          IF @errno <> 0
