...
CREATE proc  dbo.delete_store_reg_date_$sp  (
  ...
  @status 			tinyint  = 1,  --if 0 passed in, this indicates that function status already existed (i.e. recovery) but prior failed run didn't get as far as locking store/date
...
PROC NAME: delete_store_reg_date_$sp 
     DESC: Mass delete either one valid/invalid store-date or store-date-reg.
   	   Called by frontend and function_cleanup_$sp and delete_str_reg_date_asynch_$sp.
     Unicode version.
     
HISTORY:
Date     Author    Defect# Desc
May27,16 Vicci             Add try/catch since this is called within try/catch by delete_str_reg_date_asynch_$sp and otherwise errors result in jump out to calling proc.
Feb24,16 Vicci      DAOM-1 Handle t_status = 0, i.e. being called in recovery mode but with no real recovery required since prior failed delete didn't get as far as locking store
Feb22,16 Vicci      DAOM-1 If there are still outstanding background mass deletions requests for the store, relock it with 33 instead of unlocking it.
...
IF @status = 1 -- called by frontend or asynch delete
  SELECT @recovery_flag = 0
ELSE
BEGIN -- error recovery when called from function_cleanup_$sp
  SELECT @errmsg = 'Failed to determine asynch delete request id.',
  ...
  BEGIN -- clean up work tables prior to retry
    ...
    DELETE function_status_rec
     WHERE rec_process_id = @rec_process_id
  END  --IF @rec_process_id > 0 AND @status = 2
...
BEGIN
  ...
  BEGIN
    SELECT @errmsg = 'Failed to determine @all_registers.',
...
IF @all_registers = 0
BEGIN
  SELECT @errmsg = 'Failed to determine @last_register.',
         @object_name = 'audit_status',
         @operation_name = 'SELECT';
...
IF @status = 1  
BEGIN
  SELECT @errmsg = 'Failed to determine @trickle_in_progress_flag.',
  ...
  IF @t_status = 0 
  BEGIN
    SELECT @errmsg = 'Failed to determine if status 1 entry for function satus exists.',
           @object_name = 'function_status',
           @operation_name = 'SELECT';
  ...
  IF @errno = 201550
  BEGIN
    SELECT @errmsg = 'Store' + convert(nvarchar, @store_no) + ' date ' + convert(nvarchar, @transaction_date, 106) + ' is currently in use. ',
           @message_id = 201550
    GOTO business_rule_error
  END
  ELSE
...
BEGIN
  ...
  BEGIN TRAN
	 ...
	 WHERE store_no = @store_no
	   ...
	   AND date_reject_id = @date_reject_id;
        ...
        SELECT @errmsg = 'Failed to re-lock update_in_progress with 33. ';	   
	UPDATE store_audit_status
	   SET update_in_progress = 33 --background asynchronous mass transaction deletion request.
	 WHERE store_no = @store_no
	   AND sales_date = @transaction_date
	   AND date_reject_id = @date_reject_id
	   AND EXISTS (SELECT 1 FROM rebuild_request_detail r 
                        WHERE r.rebuild_type = 5  --mass background deletion request
                          AND r.request_status < 20 --relock if status less than 20=complete
                          AND r.store_no = @store_no
                          AND r.register_no <> @register_no  --i.e. other registers still left
                          AND r.transaction_date = @transaction_date
                          AND r.date_reject_id = @date_reject_id);  
  COMMIT TRAN;
  ...
  SELECT @errmsg = 'Failed to update ORG_CHN_HRCHY_LVL_GRP_SA (1). ',
...
BEGIN
  SELECT @errmsg = 'Failed to determine @in_out_both_flag. ',
         @object_name = '#temp_audit_status',
         @operation_name = 'SELECT';
...
SELECT @errmsg = 'Failed to determine @reverse_interface. ',
       @object_name = '#trans_id',
       @operation_name = 'SELECT';  
...
BEGIN
     ...
     BEGIN 
       SELECT @errmsg = 'Failed to run calculate_timestamp_$sp. ',
              @object_name = 'calculate_timestamp_$sp',
              @operation_name = 'EXECUTE';  
       EXEC calculate_timestamp_$sp @edit_timestamp OUTPUT

       SELECT @errmsg = 'Failed to clean work_interfaces. ',
              @object_name = 'work_interfaces',
              @operation_name = 'DELETE';  
       DELETE work_interfaces
	WHERE process_id = @process_id

       SELECT @errmsg = 'Failed to clean work_if_header. ',
	...
	WHERE t.transaction_id = w.transaction_id
	 ...
	 FROM #if_trans_id i, transaction_header h
       ...
       SELECT @errmsg = 'Failed to insert into work_if_header. ',
        ...
        SELECT @errmsg = 'Failed to determin if there are any feeds to C/L. ',
               @object_name = 'work_interfaces',
               @operation_name = 'SELECT';  
        IF EXISTS (SELECT 1
	 ...
	 SELECT wi.interface_id,
	  ...
	  WHERE wi.process_id = @process_id
	    ...
	    AND wi.interface_id = 28 
          ...
          SELECT @errmsg = 'Failed clean work_interfaces. ',
                 @object_name = 'work_interfaces',
                 @operation_name = 'DELETE';  
	  DELETE work_interfaces
	    FROM work_if_header wh, work_interfaces wi
	   WHERE wi.process_id = @process_id
	     ...
	     AND wi.interface_id = 28 
          ...
          SELECT @errmsg = 'Deletion of store/reg/date would result in invalid C/L entry. ',
...
BEGIN
  ...
  SELECT @errmsg = 'Failed to determine store name. ',
         @object_name = 'ORG_CHN',
         @operation_name = 'SELECT';  
...
/*   delete to translate_error moved up before delete to tran header 
  ...
  SELECT @errmsg = 'Failed to determine if store closed. ',
         @object_name = 'ORG_CHN',
         @operation_name = 'SELECT'; 
    IF EXISTS ( SELECT 1
		   FROM ORG_CHN
   ...
   BEGIN
     ...
     IF @status_value IS NULL
       BEGIN
         SELECT @errmsg = 'Failed to determine status based on unused/missing register reporting option. ',
  ...
  /* work_transaction_list populated earlier to support rollforward after tran details already deleted */
    SELECT @errmsg = 'Failed to EXEC rec_manual_$sp. ',
...
BEGIN
    ...
    SELECT @status = 6
    SELECT @errmsg = 'Failed to set status to 6 in function_status. ',
           @object_name = 'function_status',
           @operation_name = 'UPDATE';  
    UPDATE function_status
      SET status = @status
  ...
  SELECT @errmsg = 'Failed to update ORG_CHN_HRCHY_LVL_GRP_SA (2). ',
         @object_name = 'ORG_CHN_HRCHY_LVL_GRP_SA',
         @operation_name = 'UPDATE'; 	        
  UPDATE ORG_CHN_HRCHY_LVL_GRP_SA
    SET GRP_MBR_CHNG = getdate()
  ...
  IF @rows = 0 /* invalid store */
     BEGIN
       SELECT @errmsg = 'Failed to update ORG_CHN_HRCHY_LVL_GRP_SA (3). ',
              @object_name = 'ORG_CHN_HRCHY_LVL_GRP_SA',
              @operation_name = 'UPDATE'; 	        
       UPDATE ORG_CHN_HRCHY_LVL_GRP_SA
         SET GRP_MBR_CHNG = getdate()
  ...
  SELECT @errmsg = 'Failed to deter,ome of store/date have valid transactions. ',
         @object_name = 'audit_status',
         @operation_name = 'SELECT'; 	        
