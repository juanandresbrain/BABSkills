# dbo.util_repop_if_tables_curr_GH_$sp
...
```mermaid
flowchart LR
    SP["dbo.util_repop_if_tables_curr_GH_$sp"]
    authorization_detail(["authorization_detail"]) --> SP
    crm_repop(["crm_repop"]) --> SP
...
```sql
CREATE proc [dbo].[util_repop_if_tables_curr_GH_$sp]
...
SELECT	@interface_id = 26, -- change this
	@from_transaction_date = '08/28/2023', -- starting date
	@to_transaction_date = '08/28/2023', -- ending date or = getdate() if all
...
SELECT DISTINCT ic.transaction_id
     ...
     WHERE ic.interface_id = 26
       AND ic.transaction_id = th.transaction_id
       --and th.store_no = 622
       --AND transaction_date >= '06/24/2015' 
      -- AND transaction_date <= '06/30/2015' 
	   AND th.transaction_id in (select transaction_id from crm_repop)
FOR READ ONLY
 ...
 BEGIN
  ...
  --IF applicability_method = 0
    ...
    BEGIN
     ...
     WHERE th.transaction_id in (select transaction_id from mew_222)
	    AND th.transaction_date = '11-24-2017' --@date_to_reprocess
