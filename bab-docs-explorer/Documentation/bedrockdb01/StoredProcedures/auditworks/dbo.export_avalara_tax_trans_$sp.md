...
```sql
CREATE proc  [dbo].[export_avalara_tax_trans_$sp] (
...
HISTORY:
Date     Name           Def# Desc
Jun11,18 Kiri      DAOM-3518 Corrected Export file limit - Avalara site can only receive up to 100000 lines/batch
Jul07,16 Vicci     DAOM-1058 When an "agregate" option is active, use the highest transaction_id/line_id combination for the group as the transaction_id/line_id, 
                             not the highest transaction_id and highest-line-of-any-transaction-in-group since otherwise dup-key on insert into avalara_tax_trans occurs
                             when the max transaction for a group has items from more than one tax item group and the max line for all transactions for the store/date for each of
                             the tax-item-groups is the same.
Jan15,16 Vicci    TFS-154531/154563 Correct joins to if_transaction_line_link
...
SELECT @process_no = 39,  --tax transaction export
       ...
       @max_rows_avalara_accepts = 99000,
       @one = 1,
         ...
         LEFT OUTER JOIN if_customer sndll
          ...
          AND sndll.line_id IN (SELECT ll.linked_line_id
			         ...
			         WHERE sndll.if_entry_no = ll.if_entry_no
			           AND l.line_id = ll.line_id) --not AND sndll.line_id = ll.line_id) 
         ...
         LEFT OUTER JOIN if_tax_override_detail todll
          ...
          AND todll.line_id IN (SELECT ll.linked_line_id
			         ...
			         WHERE todll.if_entry_no = ll.if_entry_no
			           AND l.line_id = ll.line_id) --not AND todll.line_id = ll.line_id) 
  ...
  SELECT @cursor_open = 1
   ...
   BEGIN
    ...
    SELECT 1 ProcessCode,
         ...
         MAX((d.transaction_id * @max_rows_avalara_accepts + d.line_id)%@max_rows_avalara_accepts)  --i.e. the max line_id of the max transaction id
    FROM avalara_tax_trans_dtl d
