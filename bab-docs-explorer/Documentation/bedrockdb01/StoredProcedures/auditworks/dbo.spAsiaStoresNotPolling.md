...
```sql
CREATE procedure [dbo].[spAsiaStoresNotPolling]
-- =====================================================================================================
-- Name: spAsiaStoresNotPolling
--
-- Description:	
--
-- Input:	
...
--		Paul B			10/26/2010		Changed from sp_EuropeStoresNotPolling to spAsiaStoresNotPolling
...
--		Paul Beckman	05/28/2016		Created spAsiaStoresNotPolling from copy of spEuropeStoresNotPolling
...
--		Lizzy Timm		02/23/2021		Added Bcc for emails sent when greater than 1 stores are not polling and added logic to only send to Bcc upto once per day
--
...
if (select count(Store_No) from ##StoresNotPolled) between 1 and 10

begin
	set @text = 
				'<font face =arial size = 2>' +
				'The following Asia stores have not reported sales to Sales Audit within the past 3 hours:<br>' +
				'<br>' +
				'<table border="1">' + 
				'<font face =arial size = 2>' +
				...
				'Job Name:  StoresNotTricklePolling Asia <br>' +
				'Stored Proc:  BEDROCKDB01.auditworks.dbo.spAsiaStoresNotPolling <br>' +
				'Created by:  Paul Beckman <br>' +
				'Team Ownership:  Enterprise Systems <br>'

	set @Subject = 'ALERT - Asia Stores Not Trickle Polling'
	...
	@body = @text,
	@body_format = 'HTML'
	--IF (SELECT COUNT(*) FROM notification_history WHERE stored_proc_name = 'spAsiaStoresNotPolling' AND action_required = 'Yes' AND convert(varchar, record_logged_datetime, 101) = convert(varchar, getdate(), 101)) < 1
	--BEGIN
	--	declare @textA nvarchar(max)
	--	SET @textA = 'Asia stores have not reported sales to Sales Audit.'
 ...
 -- ELSE
	...
	VALUES (
	'spAsiaStoresNotPolling', --<< Stored Proc name
	GETDATE(),
	...
	'Some Asia stores have not reported sales to Sales Audit within the past 3 hours' --<< Comment
	)
...
begin
	...
	'ALL Asia stores have reported sales to Sales Audit within the past 3 hours' --<< Comment
	)

end

if (select count(Store_No) from ##StoresNotPolled) > 10

begin
	set @text = 
				'<font face =arial size = 2 color="Red">' +
				'More than 10 Asia stores have not reported sales to Sales Audit within the past 3 hours:<br>' +
	...
	set @Subject = 'WARNING - Asia Stores Not Trickle Polling'
	--exec msdb.dbo.sp_send_dbmail
	--@profile_name = 'EntSysSupport',
	--@recipients = @recipients,
	--@blind_copy_recipients = 'EnterpriseSystemsAlerts@buildabear.com',
	...
	BEGIN
		--declare @textA nvarchar(max)
		--SET @textA = 'More than 10 Asia stores have not reported sales to Sales Audit.'
  ...
  ELSE
	...
	NULL, --<< Email CC
	@Subject, --<< Email Subject
	'More than 10 Asia stores have not reported sales to Sales Audit within the past 3 hours' --<< Comment
	)
