# dbo.spAuditworksEmailHIP_bak20140117
...
```mermaid
flowchart LR
    SP["dbo.spAuditworksEmailHIP_bak20140117"]
...
```sql
create proc spAuditworksEmailHIP_bak20140117
as 
-- =====================================================================================================
...
-- Input:	n/a
--
-- Output: 2 csv files to: \\posdbssa\C$\SQLFiles\HIP\ (one for UMG, one for WMG)
--
-- Dependencies: n/a
--
-- Revision History
...
--		Dan Tweedie		05/10/2013		Added code at end of procedures to move the files from posdbssa server (instead of deleting them) to \\oursmerchdb01\hip\ so the Physical Inventory team can access the files. They don't want to get an email, they just want the files as their convenience.
-- =====================================================================================================
...
join	OURSMERCHDB01.me_01.dbo.style s on tl.reference_no = s.style_code
join	OURSMERCHDB01.me_01.dbo.entity_custom_property ecp  on s.style_id = ecp.parent_id
join	OURSMERCHDB01.me_01.dbo.custom_property cp  on ecp.custom_property_id = cp.custom_property_id
join	OURSMERCHDB01.me_01.dbo.location l  on right('0000' + cast(th.store_no as varchar(4)),4) = l.location_code 
join	OURSMERCHDB01.me_01.dbo.entity_attribute_set eas  on l.location_id = eas.parent_id
join	OURSMERCHDB01.me_01.dbo.attribute_set att  on eas.attribute_set_id = att.attribute_set_id
join	OURSMERCHDB01.me_01.dbo.attribute a  on att.attribute_id = a.attribute_id
join	OURSMERCHDB01.me_01.dbo.entity_attribute_set eas_HV  on s.style_id = eas_HV.parent_id
join	OURSMERCHDB01.me_01.dbo.attribute_set att_HV  on eas_HV.attribute_set_id = att_HV.attribute_set_id
join	OURSMERCHDB01.me_01.dbo.attribute a_HV  on att_HV.attribute_id = a_HV.attribute_id
...
AND		att_HV.attribute_set_code in ('UMG', 'WMG')
ORDER BY 2
...
---export csv file for UMG data
	...
	set @file_location = '\\posdbssa\D$\HIP\'  
...
---export csv file for WMG data
	...
	set @text = 'Please see attached HIP report from Build-A-Bear Workshop for UMG and WMG.'
			+ char(10) + char(13)+
	...
	exec msdb.dbo.sp_send_dbmail
		...
		set @move = 'move ' + @fileU + ' \\oursmerchdb01\hip\HIP_Reports\'
		exec master..xp_cmdshell @move

		set @move = 'move ' + @fileW + ' \\oursmerchdb01\hip\HIP_Reports\'
		exec master..xp_cmdshell @move
