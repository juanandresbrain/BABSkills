# dbo.spDW_Extract_Transaction_Lines
...
```mermaid
flowchart LR
    SP["dbo.spDW_Extract_Transaction_Lines"]
...
```sql
CREATE proc [dbo].[spDW_Extract_Transaction_Lines]
...
IF (Object_ID('auditworks..tmpTransactionLinesStage') IS NOT NULL) DROP TABLE tmpTransactionLinesStage;
