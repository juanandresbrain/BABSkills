...
PROC NAME: fix_invalid_register_$sp
  ...
  HISTORY:
Date     Name		Def# Desc             
Apr29,16 Vicci      DAOM-664 Don't revalidate store/dates that are in the midst of being trickled in by the Edit.
                             Move population of work_transaction_move to after function status insert and store/date lock.
                             Fix oldest date before new dates (less recalculation for media rec).
                             When a register has become valid, don't leave behind Invalid Register S/A rejects for store/register/dates that were also an invalid date.
                             Remove audit_status qty updates since this was already done by move_register_$sp.
...
SELECT transaction_date = a.sales_date, a.date_reject_id, a.audit_status, CASE WHEN s.update_in_progress = 9 THEN @function_status ELSE 0 END function_status
  ...
  FROM audit_status a
       INNER JOIN store_audit_status s
         ...
         AND s.update_in_progress IN (0, 9)
 ...
 WHERE a.store_no = @store_no
   ...
   AND a.trickle_in_progress_flag = 0
...
SELECT @cursor_open = 1
	...
	BEGIN
        ...
        DELETE srr  --DELETE sa_rejection_reason
          FROM sa_rejection_reason srr
         WHERE srr.violated_sareject_rule = 1  --invalid register
           AND srr.transaction_id IN (SELECT h.transaction_id 
                                        FROM transaction_header h
  WHERE h.transaction_date = @sales_date
                                         AND h.store_no = @store_no
                                         AND h.register_no = @register_no
	  ...
	  IF @errno !=0
	  BEGIN
	    SELECT @errmsg = 'Failed to delete sa_rejection_reason invalid register for bad date when register now exists',
	...
	BEGIN -- @date_reject_id = 0
	  ...
	  INSERT INTO work_transaction_move (
	   ...
	   END
/*
      --?? This was already done by move register for function_status 8 before bumping it to 9.
