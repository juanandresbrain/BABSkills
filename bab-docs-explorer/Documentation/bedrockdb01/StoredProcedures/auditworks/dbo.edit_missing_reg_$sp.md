...
AS
  ...
  -- 3: Log server-workstation and workstations as missing.
    ...
    HISTORY:   
    Date     Name         Def# Desc
    May12,16 Vicci    DAOM-730 Do not bump last edit date past the highest date actually evaluated;  this can only happen if configured
                               for batch polling (not trickle) and phase2 run during working hours.
                               Don't assume that a pre-existing status of Missing (5) indicates that the Log parent as missing and child as unused
                               option 1 is selected since it might also indcated Log both parent and child as missing option 3.
...
DECLARE 
  ...
  SELECT @last_date_closed = last_date_closed
    ...
    SELECT @errmsg = 'Failed to select last edit date from auditworks_system_flag',
           @object_name = 'auditworks_system_flag';
  SELECT @last_edit_date  = CONVERT(smalldatetime, CONVERT(nchar(8), flag_datetime_value,112))
  ...
  BEGIN
    ...
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG &&: evaluating : ' + CONVERT(nchar, @calculation_date, 101)
	+ ' starts : ' + CONVERT(nchar, getdate(), 8);
    PRINT @trace_msg;
	...
	END; -- If @instance_id > 0
	 ...
	 SELECT st.store_no ,
                st.register_no,
                @calculation_date,
                st.register_poll_id,  --DAOM-730 part2:  this is now only set for option 1.
                st.audit_status,
                st.audit_status,
                NULL,
                r.WRKSTN_ID,
                COALESCE(r.PRNT_WRKSTN_ID, r.WRKSTN_ID),
                CASE WHEN st.register_poll_id IS NOT NULL THEN 1 ELSE 3 END  --DAOM-730 part2 cannot assume 1 given possibility of 3: Log server-workstation and workstations as missing.
           FROM audit_status st, 
                ORG_CHN_WRKSTN r WITH (NOLOCK)
    ...
    END;  --IF @date_evaluated > 0 AND @process_no = 5
    ELSE
    ...
    --
	 ...
	 SELECT rg.ORG_CHN_NUM ,
           rg.WRKSTN_NUM,
           @calculation_date,
           MAX(CASE WHEN ca.WRKSTN_ID = rg.WRKSTN_ID AND IsNull(c.RPRT_UNSD_WRKSTNS, 1) = 1  --can only set for parent register since for regular register don't know what parent register is yet -audit status insert will take care of others
                THEN RIGHT ('0000000000' + CONVERT(nvarchar, rg.ORG_CHN_NUM),10) + RIGHT ('00000' + CONVERT(nvarchar, rg.WRKSTN_NUM),5) 
                ELSE NULL END) register_poll_id,  --DAOM-730 part2
           0,
           NULL,
           ss.CLS_DATE,
           ss.OPEN_HOUR_ID,
           ...
           MAX(IsNull(c.RPRT_UNSD_WRKSTNS, 1))
	 ...
	 WHERE rg.ACTV = 1
	   ...
	   AND (@calculation_date < ss.CLS_DATE OR ss.CLS_DATE IS NULL)
         GROUP BY rg.ORG_CHN_NUM,  --handle possibility of multiple configurations
               rg.WRKSTN_NUM,
               ss.CLS_DATE,
               ss.OPEN_HOUR_ID,
               rg.WRKSTN_ID,
 ...
 -- Insert status entries inside a retry loop in order to handle possible multistream timing issues
	...
	SELECT store_no,
	           ...
	           status_flag,
	           @status_date,
	           CASE WHEN sl.rprt_unsd_wrkstns = 1  
                        THEN RIGHT ('0000000000' + CONVERT(nvarchar, r.ORG_CHN_NUM),10) + RIGHT ('00000' + CONVERT(nvarchar, r.WRKSTN_NUM),5) 
                   ELSE NULL END register_poll_id
	      FROM #store_reg_list sl
	           INNER JOIN ORG_CHN_WRKSTN r WITH (NOLOCK)
                      ON sl.prnt_wrkstn_id = r.WRKSTN_ID
	     WHERE sl.existing_status = 0
...
BEGIN
  ...
  END; -- ELSE of IF @trickle_polling_flag < 1
...
BEGIN
	  ...
	  UPDATE auditworks_system_flag
	   ...
	   WHERE flag_name = 'last_edit_date'
	     AND CONVERT(smalldatetime, CONVERT(nchar(8), getdate(),112)) <= @current_date --to avoid bumping last edit date if configured for batch polling (not trickle) and phase2 run during working hours.;

END; -- If @stores_found > 0 ...

IF @total_retries > 0 -- THEN
BEGIN
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG &&: edit_missing_reg : insert retries = ' + CONVERT(nvarchar, @total_retries);
    PRINT @trace_msg;
END;
