...
Jun02,15 Vicci     TFS-121281  Remove av_ from name of table referenced by ex_process_error_log and ex_process_log synonyms.
...
IF @schema_name IS NULL
  ...
  DECLARE c_ex_cursor CURSOR FAST_FORWARD
    FOR 
     SELECT 'ex' + SUBSTRING(name,3,99) synonym_name, name source_table_name
       FROM sys.tables
      WHERE name LIKE 'av%'
        AND substring(name, 3, 1) = '_'
     UNION
     SELECT 'ex_' + name synonym_name, name source_table_name
       FROM sys.tables
  ...
  FETCH c_ex_cursor
   INTO @synonym_name, @source_table_name;
  ...
  BEGIN
    ...
    SELECT @exists = COUNT(1) from sys.synonyms WHERE name = @synonym_name;
    ...
    BEGIN
      SELECT @errmsg = 'Unable to drop synonym ex_subledger',
             ...
             @operation_name ='DROP';
      SELECT @sql_cmd = 'DROP SYNONYM ' + @synonym_name;  

      EXEC sp_executesql @sql_cmd;
    END; -- If @exists > 0
        
    SELECT @errmsg = 'Unable to create synonym ' + @synonym_name,
           @object_name = @synonym_name,
           @operation_name ='CREATE';

    SELECT @sql_cmd = 'CREATE SYNONYM ' + @synonym_name + ' FOR ' + @db_path + @source_table_name;
