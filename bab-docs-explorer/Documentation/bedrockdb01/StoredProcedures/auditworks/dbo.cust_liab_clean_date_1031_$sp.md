# dbo.cust_liab_clean_date_1031_$sp
...
```mermaid
flowchart LR
    SP["dbo.cust_liab_clean_date_1031_$sp"]
...
```sql
create proc dbo.cust_liab_clean_date_1031_$sp 
...
**  From table store_salesaudit : field interstore_export_region
    1  -> export is used.
    2  -> online voucher tracking is used.
...
SELECT @lower_limit_date = MIN(sales_date)
  FROM store_audit_status a, store_salesaudit s
 WHERE s.store_no = a.store_no
   AND s.interstore_export_region = 2  -- stores that are online only
...
SELECT @upper_limit_date = MIN(sales_date)
  FROM audit_status a, store_salesaudit s
 WHERE s.store_no = a.store_no
   AND s.interstore_export_region = 2  -- stores that are online only
   AND (audit_status = 5
...
BEGIN
 ...
 IF @errno !=0 
 BEGIN
   SELECT @errmsg='Failed to insert clean_date @max_clean_date ',
          @object_name = 'clean_date',
