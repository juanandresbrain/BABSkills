...
flowchart LR
    ...
    IFO92750004_final(["IFO92750004_final"]) --> SP
    IFO92750005(["IFO92750005"]) --> SP
    IFO92750005_final(["IFO92750005_final"]) --> SP
```
...
```sql
create proc dbo.IF_9275_cleanup_$sp
/* Name: IF_9275_cleanup_$sp
   Generated: 05/25/04 11:40:25 AM
...
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to copy data to IFO92750001_final table.'
   GOTO error
   END


INSERT INTO IFO92750004_final
...
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to copy data to IFO92750004_final table.'
   GOTO error
   END


INSERT INTO IFO92750005_final
...
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to copy data to IFO92750005_final table.'
   GOTO error
   END
