# dbo.spAuditworksSelectGaapSalesFR_DatabearTest
...
```mermaid
flowchart LR
    SP["dbo.spAuditworksSelectGaapSalesFR_DatabearTest"]
...
```sql
CREATE proc [dbo].[spAuditworksSelectGaapSalesFR_DatabearTest]
as

-- =============================================================================================================
-- Name: spAuditworksGaapSalesFR
--
-- Description:	captures Gaap Sales from Auditworks for US stores.
--				Modified version of usp_GAAPSales_FR, which called sv_store view. 
...
-- exec [spAuditworksSelectGaapSalesFR_DatabearTest]
-- =============================================================================================================
set nocount on
IF (Object_ID('tempdb..#FRGaapSalesAW') IS NOT NULL) DROP TABLE #FRGaapSalesAW
	create table #FRGaapSalesAW
(
...
insert #FRGaapSalesAW
    ...
    WHERE   ss.gl_company IN ( 2201 ) -- FR stores only
...
insert #tmp_VAT
    ...
    WHERE   ( h.transaction_date BETWEEN CONVERT(CHAR, GETDATE() -3, 101)
			...
			AND h.store_no in (select location_code from  #FRGaapSalesAW)
    GROUP BY h.store_no
    ORDER BY h.store_no
    

--UPDATE GAAP SALES BY STRIPPING OUT VAT
    UPDATE  #FRGaapSalesAW
    SET     net_sales = net_sales + VAT
    FROM    #FRGaapSalesAW gs
            INNER JOIN #tmp_VAT v ON v.StoreNo = gs.location_code

select * from #FRGaapSalesAW return
