...
HISTORY
Date     Name           Def# Desc
Jun25,15 Vicci    TFS-127504 When "front-end warning" messages are raised by Mass Revalidate background processes, don't log them as verified.
...
IF @message_category = -1 AND @process_no IN (1, 4, 5, 78, 82, 89, 81, 124, 96, 95, 80, 111, 101, 79, 68, 97)
BEGIN
  SELECT @message_category = 10;
...
IF @message_category NOT IN (-1) -- not frontend raiserror
  ...
  SELECT @errmsg2 = CONVERT(nvarchar(2000), REPLACE(REPLACE(@error_msg, char(34), ''), @quote, ''));
