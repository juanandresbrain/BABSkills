# dbo.spAvalaraMonthySalesTaxExport_BU20190408
...
```mermaid
flowchart LR
    SP["dbo.spAvalaraMonthySalesTaxExport_BU20190408"]
...
```sql
CREATE   procedure [dbo].[spAvalaraMonthySalesTaxExport_BU20190408]
...
--		,'' AS ExchangeRate  --<< Need to get for Canada stores
		...
		,cs.postal_code AS DestPostalCode
...
--		,'' AS ExchangeRate  --<< Need to get for Canada stores
		...
		,cs.city AS DestCity
		,CASE WHEN cs.state = 'Alabama' THEN 'AL'
			WHEN cs.state = 'Alaska' THEN 'AK'
			WHEN cs.state = 'Arizona' THEN 'AZ'
			WHEN cs.state = 'Arkansas' THEN 'AR'
			WHEN cs.state = 'California' THEN 'CA'
			WHEN cs.state = 'Colorado' THEN 'CO'
			WHEN cs.state = 'Connecticut' THEN 'CT'
			WHEN cs.state = 'Delaware' THEN 'DE'
			WHEN cs.state = 'Florida' THEN 'FL'
			WHEN cs.state = 'Georgia' THEN 'GA'
			WHEN cs.state = 'Hawaii' THEN 'HI'
			WHEN cs.state = 'Idaho' THEN 'ID'
			WHEN cs.state = 'Illinois' THEN 'IL'
			WHEN cs.state = 'Indiana' THEN 'IN'
			WHEN cs.state = 'Iowa' THEN 'IA'
			WHEN cs.state = 'Kansas' THEN 'KS'
			WHEN cs.state = 'Kentucky' THEN 'KY'
			WHEN cs.state = 'Louisiana' THEN 'LA'
			WHEN cs.state = 'Maine' THEN 'ME'
			WHEN cs.state = 'Maryland' THEN 'MD'
			WHEN cs.state = 'Massachusetts' THEN 'MA'
			WHEN cs.state = 'Michigan' THEN 'MI'
			WHEN cs.state = 'Minnesota' THEN 'MN'
			WHEN cs.state = 'Mississippi' THEN 'MS'
			WHEN cs.state = 'Missouri' THEN 'MO'
			WHEN cs.state = 'Montana' THEN 'MT'
			WHEN cs.state = 'Nebraska' THEN 'NE'
			WHEN cs.state = 'Nevada' THEN 'NV'
			WHEN cs.state = 'New Hampshire' THEN 'NH'
			WHEN cs.state = 'New Jersey' THEN 'NJ'
			WHEN cs.state = 'New Mexico' THEN 'NM'
			WHEN cs.state = 'New York' THEN 'NY'
			WHEN cs.state = 'North Carolina' THEN 'NC'
			WHEN cs.state = 'North Dakota' THEN 'ND'
			WHEN cs.state = 'Ohio' THEN 'OH'
			WHEN cs.state = 'Oklahoma' THEN 'OK'
			WHEN cs.state = 'Oregon' THEN 'OR'
			WHEN cs.state = 'Pennsylvania' THEN 'PA'
			WHEN cs.state = 'Rhode Island' THEN 'RI'
			WHEN cs.state = 'South Carolina' THEN 'SC'
			WHEN cs.state = 'South Dakota' THEN 'SD'
			WHEN cs.state = 'Tennessee' THEN 'TN'
			WHEN cs.state = 'Texas' THEN 'TX'
			WHEN cs.state = 'Utah' THEN 'UT'
			WHEN cs.state = 'Vermont' THEN 'VT'
			WHEN cs.state = 'Virginia' THEN 'VA'
			WHEN cs.state = 'Washington' THEN 'WA'
			WHEN cs.state = 'West Virginia' THEN 'WV'
			WHEN cs.state = 'Wisconsin' THEN 'WI'
			WHEN cs.state = 'Wyoming' THEN 'WY'
			ELSE LEFT(cs.state,2)
			END AS DestRegion  --<< State
		,cs.post_code AS DestPostalCode
		,CASE WHEN cs.country IS NULL THEN ''
			ELSE cs.country
				END AS DestCountry
		,sd.StrRegion AS OrigRegion  --<< State
...
SELECT	'1' AS ProcessCode
		...
		,CASE WHEN tsd.transaction_date > DATEADD(MONTH, DATEDIFF(MONTH, -1, @TransactionEndDate)-1, -1) THEN CONVERT(CHAR,DATEADD(MONTH, DATEDIFF(MONTH, -1, @TransactionEndDate)-1, -1),101)
			...
			ELSE CONVERT(CHAR,tsd.transaction_date,101)
				END AS DocDate
		,tsd.CompanyCode
		,CASE WHEN tsd.CustomerCode IN ('1470','1990','1991') THEN '9999'
