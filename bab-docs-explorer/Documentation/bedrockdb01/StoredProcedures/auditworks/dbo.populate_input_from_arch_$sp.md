# dbo.populate_input_from_arch_$sp
...
```mermaid
flowchart LR
    SP["dbo.populate_input_from_arch_$sp"]
...
```sql
create proc dbo.populate_input_from_arch_$sp @input_id int,
@errmsg nvarchar(255) OUTPUT
AS

/* 
PROC NAME: populate_input_from_arch_$sp
     DESC: Populate input tables for archived transaction tables. 
HISTORY:
Date     Name           Def# Desc
Oct22,14 Vicci     TFS-81700 Copy merchandise attachment cost field.
Oct15,07 Paul          91395 uplift 89110 to SA5
Mar22,07 Paul        DV-1356 Corrected query for 2005 compatibility, cleaned up useless comments
Mar22,05 Maryam      DV-1202 Rename from_line_id to line_id, insert input_transaction_line_link
Mar18,05 David       DV-1202 Add display_def_id input_stock_control_detail.
Sep24,04 Paul        DV-1146 add columns to inserts
Jun28,04 ShuZ        DV-1071 Add without_receipt_flag when populating return_detail tables.
Jun10,07 Vicci         89110 add missing table columns
...
Sept,05  ShuZ           8673 Author, port from Oracle
...
SET ANSI_WARNINGS ON
  ...
  SELECT @process_name = 'populate_input_from_arch_$sp',
       @message_id = 201068	
  ...
  INSERT INTO input_authorization_detail(
	...
	a.offline_flag
     FROM work_input_source u, av_authorization_detail a 
    WHERE u.av_transaction_id = a.av_transaction_id	
  ...
  FROM av_customer c, 
       work_input_source u 
    WHERE u.av_transaction_id = c.av_transaction_id
    AND u.input_id = @input_id
  ...
  INSERT INTO input_discount_detail(
	...
	d.line_id,
	0,--line_id_adj,
	1, --pos_discount_level,
	d.pos_discount_type,
	d.pos_discount_amount,
	0, --pos_discount_amount_adj,
	SIGN(SIGN(l.db_cr_none) + SIGN(1 + l.db_cr_none)),		
	1, --discount_applied_flag,
	d.applied_by_line_id,
	d.pos_discount_serial_no
  FROM av_discount_detail d, 
       work_input_source u,
       av_transaction_line l
    WHERE u.av_transaction_id = d.av_transaction_id
    AND l.av_transaction_id = d.av_transaction_id
  ...
  FROM av_line_note l, 
       work_input_source u 
    WHERE u.av_transaction_id = l.av_transaction_id
    AND u.input_id = @input_id
  ...
  INSERT INTO input_merchandise_detail(
	...
	m.source_store_no,
	m.fulfillment_store_no,
	m.cost
  FROM  work_input_source u, av_merchandise_detail m
WHERE m.av_transaction_id = u.av_transaction_id
  AND u.input_id = @input_id
  ...
  FROM  work_input_source u,av_payroll_detail p
  WHERE p.av_transaction_id = u.av_transaction_id
  AND u.input_id = @input_id
  ...
  FROM  work_input_source u,av_post_void_detail p
  WHERE p.av_transaction_id = u.av_transaction_id
  AND u.input_id = @input_id
  ...
  INSERT INTO input_return_detail(
	...
	r.without_receipt_flag
  FROM  work_input_source u,av_return_detail r
  WHERE r.av_transaction_id = u.av_transaction_id
  AND u.input_id = @input_id
  ...
  INSERT INTO input_special_order_detail(
	...
	s.vendor_no
  FROM  work_input_source u,av_special_order_detail s
  WHERE s.av_transaction_id = u.av_transaction_id
  AND u.input_id = @input_id
  ...
  FROM  work_input_source u,av_stock_control_detail s
  WHERE s.av_transaction_id = u.av_transaction_id
  AND u.input_id = @input_id
  ...
  INSERT INTO input_tax_override_detail(
	...
	t.tax_exempt_no
  FROM  work_input_source u,av_tax_override_detail t
  WHERE t.av_transaction_id = u.av_transaction_id
  AND u.input_id = @input_id
  ...
  INSERT INTO input_transaction_line_link(
	...
	k.line_id,
	k.linked_line_id
  FROM  work_input_source u,av_transaction_line_link k
  WHERE k.av_transaction_id = u.av_transaction_id
  AND u.input_id = @input_id
...
FROM av_transaction_header h, 
       work_input_source u 
    WHERE u.av_transaction_id IS NOT NULL 
    AND u.av_transaction_id = h.av_transaction_id
    AND u.input_id = @input_id
  ...
  FROM av_transaction_line l,work_input_source u
  WHERE u.av_transaction_id = l.av_transaction_id
  AND u.input_id = @input_id
