...
CREATE PROC [dbo].[spCL_Import_Process]
-- =============================================================================================================
...
--		Paul Beckman	07/31/2019		Adjusted flow to delete start.upload if .tab does not exist
--		Paul Beckman	09/23/2019		Updated ERROREMAIL message body text
...
IF (SELECT COUNT(*) FROM #startupload) = 1 AND (SELECT COUNT(*) FROM cl_import_file_list) = 0
SET @command = 'del /Q ' + @drive + '\start.upload'  
EXEC master..xp_cmdshell @command

IF (SELECT COUNT(*) FROM #startupload) = 0 OR (SELECT COUNT(*) FROM cl_import_file_list) = 0
GOTO FINISH
...
IF (SELECT COUNT(*) FROM cl_import_file_list WHERE validate_status = 'Failed') > 0
GOTO ERROREMAIL
...
SET @Subject = 'Voucher Upload process STARTED'
	...
	VALUES (
	'spCL_Import_Process', --<< Stored Proc name
	GETDATE(),
	...
	'Voucher upload files have been found and the validation and upload process has been initiated' --<< Comment
	)
...
SET @text = 
		...
		'Files that FAILED validation will NOT be imported until corrected. <br>' +
		'These files have been renamed to .ERROR and can be found in \\saapp01\CL_IMPORT\Voucher_Import for correction by Enterprise Systems Support team. <br>' +
		'<br>' +
		'<table border="1">' + 
		...
		'</table>' +
		'<br>' +
		'Failed records listed below <i>(Only top 100 or less will be displayed)</i><br>' +
		'<table border="1">' + 
		'<font face =arial size = 2 color="Black">' +
...
SET @Subject = 'ALERT - Voucher Upload process files failed'
	EXEC msdb.dbo.sp_send_dbmail  
	@profile_name = 'EntSysSupport',
	@recipients = @recipients,
	@copy_recipients = @copy_recipients,
	...
	NULL, --<< Email CC
	@Subject, --<< Email Subject
	'Voucher upload files FAILED validation will NOT be imported until corrected' --<< Comment
	)
