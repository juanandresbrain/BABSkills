...
SELECT DISTINCT u.ORG_CHN_NUM
 ...
 WHERE u.APLCTN_ID = 300
   ...
   AND s.update_in_progress = 0  --don't clean up from this proc, let function cleanup do it otherwise status is incorrect.
...
BEGIN  
  ...
  UPDATE sa_rejection_reason
    ...
    FROM sa_rejection_reason r, transaction_header h, #fixed_invalid_store t, store_audit_status s
   WHERE r.violated_sareject_rule = 7  -- Invalid Store no.               
  ...
  UPDATE audit_status
     SET audit_status = 8
    FROM #fixed_invalid_store t, audit_status a, store_audit_status s
   WHERE a.store_no = t.ORG_CHN_NUM
     AND a.audit_status = 7
     AND a.trickle_in_progress_flag = 0 
     AND a.store_no = s.store_no
...
DECLARE fixed_register_crsr CURSOR FAST_FORWARD
    FOR
 SELECT DISTINCT s.store_no, s.register_no
   FROM audit_status s WITH (NOLOCK) -- no other function looks at status 8
   	       INNER JOIN store_audit_status sas WITH (NOLOCK)
	          ON s.store_no = sas.store_no
	         AND s.sales_date = sas.sales_date
	         AND s.date_reject_id = sas.date_reject_id
	         AND s.update_in_progress = 0 --don't clean up from this proc, let function cleanup do it otherwise status is incorrect.
	         AND s.trickle_in_progress_flag = 0
  WHERE s.audit_status = 8
    AND EXISTS (SELECT 1
                  FROM ORG_CHN_WRKSTN w -- register now exists
                 WHERE w.ORG_CHN_NUM = s.store_no
                   AND w.WRKSTN_NUM = s.register_no
                   AND w.ACTV = 1)
 ORDER BY s.store_no, s.register_no
