...
```sql
CREATE proc  dbo.function_cleanup_$sp 
@process_id      binary(16), -- guid
...
Jan31,17 Kiri      DAOM-2023 VD: Don't clean up the current transaction ID if it has purposefully been left behind by transction add (see i_cleanup_flag);  
                             set i_cleanup_flag to 2 (partial cleanup) for Transaction Add (150) if the function has not yet been released to cleanup
                             to avoid Transaction Add erroring out due to the loss of the transaction ID when the user corrects the issue raised by a business rule error detected by the database proc.
...
Jun15,16 Vicci      DAOM-292 Handle function 7 (Standard Import) cleanup for when called from banking import and input_ table entries left behind.
Feb23,16 Vicci        DAOM-1 Handle function 33 (Mass Background Deletion Request).
Jan20,16 Vicci    TFS-142941 for function 80 mass correct upc, check for any aborted mass_correct_tax_$sp calls made by mass_correct_upc_$sp that need to be recovered first.
...
DECLARE
  ...
  @child_process_id      	binary(16),
  @child_status			tinyint,
  @released_to_cleanup		tinyint;
...
IF @function_no = 150
BEGIN
  SELECT @errmsg = 'Failed to determine whether this is the UI issuing a self-recovery of a business rule error',
         @object_name = 'function_status',
         @operation_name = 'SELECT';
  SELECT @released_to_cleanup = MAX(released_to_cleanup) 
  ...
  IF @released_to_cleanup IS NULL
...
BEGIN
  ...
  -- @function_no = 33  Note, only gets called if status > 1, but no harm in call with status > 0
  IF @function_no = 33 AND @status > 0  -- multiple valid/invalid store_reg_date background deletion request with status > held (i.e. released by UI to back end)
  BEGIN
    SELECT @errmsg = 'Unable to recover background deletion of store/reg/dates request. ',
	   @object_name = 'delete_str_reg_date_asynch_$sp',
	   @operation_name = 'EXECUTE';
    EXEC delete_str_reg_date_asynch_$sp @process_id;

    GOTO refresh;;
  END;  --IF @function_no = 33
...
BEGIN
  ...
  IF @function_no = 80 AND @status = 2
  --if mass_correct_upc_$sp, then check for any aborted mass_correct_tax_$sp calls made by mass_correct_upc_$sp that need to be recovered first
  BEGIN
    SELECT @errmsg = 'Unable to determine current status of child process. ',
           @object_name = 'function_status',
           @operation_name = 'SELECT';
    SELECT @child_status = status,
           @child_process_id = process_id
      FROM function_status
     WHERE lock_by_spid = @process_id
       AND function_no = 89 -- mass_correct_tax_$sp
       AND move_flag = 5;  --revalidation_type 5 i.e. called by mass_correct_upc_$sp
       
    IF @child_process_id IS NOT NULL
    BEGIN
      SELECT @errmsg = 'Failed to recover mass_correct_tax_$sp revalidation_type 5. ',
   	     @object_name = 'mass_correct_tax_$sp',
	     @operation_name = 'EXECUTE';
      EXEC mass_correct_tax_$sp @child_process_id, @user_id, null, 5, @child_status;
    END  
  ...
  END  --IF @function_no = 80 AND @status = 2
...
BEGIN
  ...
  BEGIN
    ...
    IF @rows < 1
       GOTO refresh;;  --can't clean up trans mod without first cleaning up corresponding failed C/L process, but someone else is in the midst of recovering it
...
BEGIN
  ...
  --rollforward the changes
    ...
    --Note, transaction add cleans up store_audit_status lock when in recovery mode.
  ...
  BEGIN
    ...
    AND transaction_id <> @transaction_id;  --  don't clean up the current transaction ID if it has purposefully been left behind (see cleanup flag above)
...
IF (@function_no >= 241 AND @function_no <= 249) OR @function_no = 73 OR @function_no = 51 OR @function_no = 7
BEGIN
...
-- reserve_input_$sp inserts @input_id into transaction_id column of function_status
   ...
   SELECT @errmsg = 'Failed to UPDATE function_status (input_) ',
          @object_name = 'function_status',
          @operation_name = 'UPDATE';
    UPDATE function_status 
       SET status = @status 
...
refresh:
  -- make halted process visible to users without them having to manually refresh Guided Audit
  BEGIN TRY
