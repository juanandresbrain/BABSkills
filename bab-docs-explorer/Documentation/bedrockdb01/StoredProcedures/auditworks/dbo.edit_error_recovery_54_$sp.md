# dbo.edit_error_recovery_54_$sp
...
```mermaid
flowchart LR
    SP["dbo.edit_error_recovery_54_$sp"]
...
```sql
create proc [dbo].[edit_error_recovery_54_$sp] 

   
@edit_process_no tinyint = 1
 AS

/* Version:1.00 Date:2005/03/15 */
/* Author: Maryam S. on 2005/03/15 */
...
HISTORY
Date     Name		  Def#  Desc
Feb21,17 Vicci   DAOM-2070  Add linked_line_id to fetched values since it is part of unique index; also correct translate error logging to for duplicate linked lines to use translate reject                                    reason instead of 154 which is POS Tax.
Mar15,05 Maryam    DV-1202  Author
*/
...
SELECT @violated_sareject_rule = 54,
	@translate_msg = 'Duplicate lines from the translate were skipped by the edit for transaction line link',
	@process_name = 'edit_error_recovery_54_$sp',
        @message_id = 201068
...
CREATE TABLE #work_lines_edit (
	...
	linked_line_id	numeric(5,0) not null,
...
SELECT
	...
	COUNT(line_id),
	MIN(row_sequence_no)
  FROM transl_transaction_line_link WITH (NOLOCK)
...
WHILE 1=1
  ...
  DELETE transl_transaction_line_link
    WHERE store_no = @store_no
      ...
      AND line_id = @line_id
      AND linked_line_id = @linked_line_id
      AND row_sequence_no > @min_sequence_no

  SELECT @errno = @@error
  IF @errno != 0
    BEGIN
     SELECT @errmsg = 'Failed to delete duplicate rows from transl_transaction_line_link',
...
SELECT
	...
	transaction_no,
	line_id,
	@violated_sareject_rule + CASE WHEN @violated_sareject_rule = 54 THEN 102 ELSE 100 END,
	getdate(),
	@translate_msg + Convert(nvarchar,@linked_line_id)
  FROM #work_lines_edit WITH (NOLOCK)
  WHERE line_count >= 2
