...
```mermaid
flowchart LR
    SP["dbo.rprt_transaction_list_$sp"]
...
```sql
CREATE proc [dbo].[rprt_transaction_list_$sp] ( @store_group_table_name          nvarchar(40),  -- created by foundation, extracted by rdl
  @TH                              nvarchar(1000),
...
/*
Proc name: rprt_transaction_list_$sp
     Desc: Transaction List (Details) report from tran detail tables.
           This proc handles non-scaleout as well as scaleout.
           Called by Transaction List.rdl
...
Sep13,17 Sean/Terri        DAOM-2537 Removal of check / join on "transaction_date" as it is not a valid column for the table in question
...
SELECT @max_string = 3998,
          ...
          @process_no = 300,
          @report_rows = 0,
          @rprt_name = 'rprt_transaction_list_$sp', -- must match the proc name
          @trace_report = 0,
...
/* Determine whether it's pre audit archive or not in order to retrieve data from transaction_header or transaction_header.
   ...
   SELECT @errmsg = 'Unable to determine update timing for S/A Accounting -Tax interface',
...
SET @txt_terminal_capture    = dbo.get_resource_text_$fn('DDLB_TERMINAL_CAPTURE_(S/A_SETTLEMENT)', @language_id)
  ...
  /* start of report. Search for transactions that match the selection criteria clause in i_TH */
   ...
   SELECT @sql_string = 'INSERT INTO #work_trans_header( transaction_id, store_no, store_name, currency_code, register_no, cashier_no, till_no,
                                         ...
                                         SELECT DISTINCT h.transaction_id, h.store_no, w.ORG_CHN_NAME, w.DFLT_CRNCY_CODE, h.register_no, h.cashier_no,
                                                    ...
                                                    FROM #rpt_store_list w' +
                                                       ' INNER JOIN transaction_header h WITH (NOLOCK) ON (w.ORG_CHN_NUM = h.store_no) 
                       INNER JOIN transaction_line l WITH (NOLOCK) ON (h.transaction_id = l.transaction_id ' + @part_join + ')
  ...
  /* Build list of transactions */
   ...
   INSERT INTO #work_trans_line( transaction_id, line_id, line_sequence, store_no, store_name, currency_code, register_no, cashier_no,
    ...
    INNER JOIN transaction_line l WITH (NOLOCK) ON (t.transaction_id = l.transaction_id /* AND t.transaction_date = l.transaction_date */)
   ...
   SELECT t.transaction_id,  
          ...
          CASE WHEN c.title 		IS NULL THEN '' ELSE @col_title       + c.title         + '; ' END +
          CASE WHEN c.first_name 	IS NULL THEN '' ELSE @col_first_name  + c.first_name    + '; ' END +
          CASE WHEN c.last_name 	IS NULL THEN '' ELSE @col_last_name   + c.last_name     + '; ' END +
          CASE WHEN c.address_1 	IS NULL THEN '' ELSE @col_address_1   + c.address_1     + '; ' END +
          CASE WHEN c.address_2 	IS NULL THEN '' ELSE @col_address_2   + c.address_2     + '; ' END +
          CASE WHEN c.city 		IS NULL THEN '' ELSE @col_city        + c.city          + '; ' END +
          CASE WHEN c.county		IS NULL THEN '' ELSE @col_county      + c.county        + '; ' END +
          CASE WHEN c.state		IS NULL THEN '' ELSE @col_state       + c.state         + '; ' END +
          CASE WHEN c.post_code		IS NULL THEN '' ELSE @col_post_code   + c.post_code     + '; ' END +
          CASE WHEN c.country 		IS NULL THEN '' ELSE @col_country     + c.country       + '; ' END +
          CASE WHEN c.telephone_no1 	IS NULL THEN '' ELSE @col_tel_no      + c.telephone_no1 + '; ' END +
          CASE WHEN c.telephone_no2 	IS NULL THEN '' ELSE ' /='            + c.telephone_no2 + '; ' END +
          CASE WHEN c.customer_no       IS NULL THEN '' ELSE @col_customer_no + convert(nvarchar, c.customer_no) + '; ' END +
          CASE WHEN c.fax		IS NULL THEN '' ELSE @col_fax_no      + c.fax           + '; ' END +
          CASE WHEN c.email_address     IS NULL THEN '' ELSE @col_email       + c.email_address + '; ' END +
          CASE WHEN c.pos_tax_jurisdiction_code IS NULL THEN '' ELSE @col_juris_code + c.pos_tax_jurisdiction_code END
          ),3,4000)) as temp_text
     FROM #work_trans_header t
          INNER JOIN customer c WITH (NOLOCK) ON (t.transaction_id = c.transaction_id AND c.line_id = 0 /* AND t.transaction_date = c.transaction_date*/)
   ...
   INSERT INTO #attachment( transaction_id, line_id,line_sequence, attachment_type, attachment_text) 
      ...
      CASE COALESCE(c.email_address, 'null') WHEN 'null' THEN '' ELSE @col_email       + email_address + '; ' END +
          ...
          FROM #work_trans_line t
               INNER JOIN customer c WITH (NOLOCK) ON (t.transaction_id = c.transaction_id AND t.line_id = c.line_id /* AND t.transaction_date = c.transaction_date */)
               INNER JOIN #code_description cd WITH (NOLOCK) ON (c.customer_role = cd.code AND cd.code_type = 7)
         WHERE LEN(RTRIM(COALESCE(c.title,'') + COALESCE(c.first_name,'') + COALESCE(c.last_name,'') + COALESCE(c.address_1,'') + 
                         COALESCE(c.address_2,'') + COALESCE(c.city,'') + COALESCE(c.county,'') + COALESCE(c.state,'') + COALESCE(c.post_code,'') +
                         COALESCE(c.country,'') + COALESCE(c.telephone_no1,'') + COALESCE(c.telephone_no2,'') + 
   ...
   SELECT t.transaction_id,
     ...
     FROM #work_trans_header t
          INNER JOIN stock_control_detail s WITH (NOLOCK) ON (t.transaction_id = s.transaction_id AND s.line_id = 0 /* AND t.transaction_date = s.transaction_date */)
   ...
   BEGIN
     ...
     SELECT @temp_string = REVERSE(SUBSTRING(REVERSE(@temp_string),3,4000))  --to strip semi-colon off end of attachment since we do not know which non-null field will be last
  ...
  AND line_id = 0
   ...
   SELECT t.transaction_id,
          ...
          CASE COALESCE(imrd, 'null')   WHEN 'null' THEN '' ELSE '<16.resid='+convert(nvarchar,d.imrd_fe_resource_id)+ '>' + imrd + '; ' END as temp_text
     FROM #work_trans_line t
          INNER JOIN stock_control_detail s WITH (NOLOCK) ON (t.transaction_id = s.transaction_id AND s.line_id = t.line_id /* AND t.transaction_date = s.transaction_date */)
          LEFT  JOIN #code_description c WITH (NOLOCK) ON (c.code_type = 68 AND c.code = s.pos_identifier_type)
          INNER JOIN #stock_display_def d WITH (NOLOCK) ON (s.display_def_id = d.display_def_id)
    WHERE LEN(RTRIM(COALESCE(convert(nvarchar, upc_no),'') + COALESCE(convert(nvarchar, merchandise_key),'') + COALESCE(convert(nvarchar,initiated_by_host),'') +
                    COALESCE(convert(nvarchar, units),'') + COALESCE(convert(nvarchar, other_store_no),'') + COALESCE(convert(nvarchar, location_no),'') + 
   ...
   DECLARE line_note_crsr CURSOR FAST_FORWARD
    ...
    SELECT t.transaction_id, ln.note_type_description, COALESCE(ln.code_display_descr, l.line_note) as temp_text,
           l.note_type
      FROM #work_trans_header t
           INNER JOIN line_note l WITH (NOLOCK) ON (t.transaction_id = l.transaction_id AND l.line_id = 0 /* AND t.transaction_date = l.transaction_date */)
  ...
  CLOSE line_note_crsr
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text)
                     SELECT t.transaction_id,
                       ...
                       FROM #work_trans_line t
                            INNER JOIN line_note l WITH (NOLOCK) ON (t.transaction_id = l.transaction_id AND l.line_id = t.line_id /* AND t.transaction_date = l.transaction_date */)
                            LEFT  JOIN #line_note_descr ln ON (l.note_type = ln.note_type AND (l.line_note = ln.alpha_code OR l.line_note = ln.code OR ln.code_type IS NULL)) 
   ...
   DECLARE return_detail_crsr CURSOR FAST_FORWARD
    ...
    SELECT t.transaction_id,
      ...
      FROM #work_trans_header t
           INNER JOIN return_detail r WITH (NOLOCK) ON (t.transaction_id = r.transaction_id AND r.line_id = 0 /* AND t.transaction_date = r.transaction_date */)
  ...
  BEGIN
   ...
   INSERT INTO #attachment ( transaction_id, line_id, line_sequence, attachment_type, attachment_text)
    SELECT t.transaction_id,
      ...
      FROM #work_trans_line t
           INNER JOIN return_detail r WITH (NOLOCK) ON (t.transaction_id = r.transaction_id AND r.line_id = t.line_id /* AND t.transaction_date = r.transaction_date */)
           LEFT JOIN #code_description c1 ON (r.return_reason_code = c1.code AND c1.code_type = 4)
           LEFT JOIN #code_description c2 ON (r.mdse_disposition_code = c2.code AND c2.code_type = 2)
     WHERE LEN(RTRIM(COALESCE(convert(nvarchar, return_reason_code),'') + COALESCE(return_reason_message,'') + COALESCE(convert(nvarchar,mdse_disposition_code),'') +
                     COALESCE(convert(nvarchar, via_warehouse_flag),'') + COALESCE(convert(nvarchar, original_salesperson),'') + COALESCE(convert(nvarchar, original_salesperson2),'') + 
   ...
   BEGIN
      ...
      FROM #work_trans_line t
           INNER JOIN tax_detail d WITH (NOLOCK) ON (t.transaction_id = d.transaction_id AND t.line_id = d.line_id /* AND t.transaction_date = d.transaction_date */)
   ...
   DECLARE tax_override_crsr CURSOR FAST_FORWARD
    ...
    SELECT t.transaction_id,
      ...
      FROM #work_trans_header t
           INNER JOIN tax_override_detail tod WITH (NOLOCK) ON (t.transaction_id = tod.transaction_id AND tod.line_id = 0 /* AND t.transaction_date = tod.transaction_date */)
   ...
   INSERT INTO #attachment(transaction_id, line_id,line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN tax_override_detail tod WITH (NOLOCK) ON (t.transaction_id = tod.transaction_id AND tod.line_id = t.line_id /* AND t.transaction_date = tod.transaction_date */)
               LEFT JOIN tax_jurisdiction j WITH (NOLOCK) ON (tod.exception_tax_jurisdiction = j.tax_jurisdiction)
         LEFT JOIN #code_description c1 ON (tod.tax_level = c1.code AND c1.code_type = 41)
               LEFT JOIN #code_description c2 ON (tod.tax_category = c2.code AND c2.code_type = 26)
     WHERE LEN(RTRIM(COALESCE(convert(nvarchar,tod.tax_level),'') + COALESCE(convert(nvarchar,tod.tax_category),'') + COALESCE(convert(nvarchar,tod.taxable),'') +
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN if_rejection_reason i WITH (NOLOCK) ON (t.transaction_id = i.transaction_id AND t.line_id = i.line_id /* AND t.transaction_date = i.transaction_date */)
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN exception_reason e WITH (NOLOCK) ON (t.transaction_id = e.transaction_id AND t.line_id = e.line_id  /* AND t.transaction_date = e.transaction_date */)
 ...
 /**** Merchandise Detail  RECORDS ****/   
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN merchandise_detail m WITH (NOLOCK) ON (t.transaction_id = m.transaction_id AND t.line_id = m.line_id /* AND t.transaction_date = m.transaction_date */)
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id,
          ...
          FROM #work_trans_line t
               INNER JOIN post_void_detail p WITH (NOLOCK) ON (t.transaction_id = p.transaction_id AND t.line_id = p.line_id /* AND t.transaction_date = p.transaction_date */)
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT w.transaction_id, 
          ...
          FROM #work_trans_line w
               INNER JOIN authorization_detail a WITH (NOLOCK) ON (w.transaction_id = a.transaction_id AND w.line_id = a.line_id /* AND w.transaction_date = a.transaction_date */)
   ...
   INSERT INTO #attachment( transaction_id, line_id,line_sequence, attachment_type, attachment_text) 
    SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN special_order_detail s WITH (NOLOCK) ON (t.transaction_id = s.transaction_id AND t.line_id = s.line_id /* AND t.transaction_date = s.transaction_date */)
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
      ...
      CASE COALESCE(p.employee_payroll_id, 'null') WHEN 'null' THEN '' ELSE @col_payroll_id + COALESCE(SUBSTRING(@replace_string, 1, LEN(p.employee_payroll_id)), p.employee_payroll_id) + '; ' END +
          ...
          FROM #work_trans_line t
               INNER JOIN payroll_detail p WITH (NOLOCK) ON (t.transaction_id = p.transaction_id AND t.line_id = p.line_id /* AND t.transaction_date = p.transaction_date */)
