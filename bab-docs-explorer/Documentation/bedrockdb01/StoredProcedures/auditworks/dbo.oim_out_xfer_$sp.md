...
```sql
create proc dbo.oim_out_xfer_$sp 
( @entity_code                  smallint )
AS

/*
Proc name: oim_out_xfer_$sp
     Desc: To post Inter Store Transfer Send details.
...
Jan12,04 Phu              21459   Include entity code 150 - Transfer Ready To Send transactions
...
SELECT @message_id = 201068,
       @process_name = 'oim_out_xfer_$sp',
       @process_no = 209,
       @exit_loop = 0

WHILE @exit_loop = 0
BEGIN
  INSERT INTO oim_out_xfer (
    oim_out_xfer_id, document_no, inventory_move_request_no, from_location_id,
    to_location_id, routed_by_warehouse_flag, ship_date, packed_by,
    weight, unit_weight_code, no_of_containers, container_type_code,
    carrier_code, bill_of_lading, reason_code, document_source, line_id, initiated_by_host)
  SELECT
         w.transaction_id, w.reference_no, substring(w.imrd, 1, 20), w.location_id,
         w.other_location_id, 0, ISNULL(w.count_date, w.transaction_date), CONVERT(VARCHAR, w.cashier_no)
         + ' ' + ISNULL(LTRIM(e.FRST_NAME + ' ' + e.LAST_NAME),' '),
         null, null, null, null,
         null, null, SUBSTRING(w.reason, 1, 5), 8, w.min_line_id, w.initiated_by_host
  ...
  BEGIN 
    DELETE oim_out_xfer
    FROM oim_out_xfer oim, work_oim_entity w
    WHERE w.entity_code = @entity_code
    AND w.transaction_id = oim.oim_out_xfer_id

    SELECT @errno = @@error
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Unable to delete duplicate key in oim_out_xfer',
             @object_name = 'oim_out_xfer',
             @operation_name = 'DELETE'
      GOTO error
    END

    DELETE oim_out_xfer_detail
    FROM oim_out_xfer_detail oim, work_oim_entity w
    WHERE w.entity_code = @entity_code
    AND w.transaction_id = oim.oim_out_xfer_id
    ...
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Unable to delete duplicate key in oim_out_xfer_detail',
             @object_name = 'oim_out_xfer_detail',
             @operation_name = 'DELETE'
      GOTO error
    END
  ...
  IF @errno <> 0
  BEGIN
    SELECT @errmsg = 'Unable to insert oim_out_xfer',
           @object_name = 'oim_out_xfer',
...
SET no_of_containers = s.location_no,
    ...
    bill_of_lading = substring(s.vendor_no, 1, 20)
FROM oim_out_xfer oim, work_oim_entity w, if_stock_control_detail s, if_transaction_line l
WHERE w.entity_code = @entity_code
...
IF @errno <> 0
BEGIN
  SELECT @errmsg = 'Unable to update oim_out_xfer',
         @object_name = 'oim_out_xfer',
         @operation_name = 'UPDATE'
  GOTO error
END

INSERT INTO oim_out_xfer_detail (
  oim_out_xfer_id, sku_id, carton_no, reason_code,
  units_sent, line_id)
SELECT
  w.transaction_id, s.sku_id, l.reference_no, MIN(s.reason),
  SUM(CONVERT(INT, s.units * l.voiding_reversal_flag)), MIN(l.line_id)
...
IF @errno <> 0
BEGIN
  SELECT @errmsg = 'Unable to insert oim_out_xfer_detail',
         @object_name = 'oim_out_xfer_detail',
         @operation_name = 'INSERT'
  GOTO error
END
