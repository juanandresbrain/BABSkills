# dbo.rprt_ex_transaction_list_$sp
...
```mermaid
flowchart LR
    SP["dbo.rprt_ex_transaction_list_$sp"]
    ...
    ex_merchandise_detail(["ex_merchandise_detail"]) --> SP
...
```sql
CREATE proc [dbo].[rprt_ex_transaction_list_$sp] ( @store_group_table_name          nvarchar(40),  -- created by foundation, extracted by rdl
  @TH                              nvarchar(1000),
...
/*
Proc name: rprt_ex_transaction_list_$sp (5.1 / SA_PART version)
...
Jun14,14 Ian                         Copy of Archived Transaction List for External Archive tables.
...
/* Determine whether it's pre audit archive or not in order to retrieve data from transaction_header or av_transaction_header.
   ...
   BEGIN
     SELECT @errmsg         = 'Unable to execute stored procedure',
              @object_name    = 'rprt_ex_transaction_list_$sp',
...
BEGIN
   ...
   SELECT @sql_string = 'INSERT INTO #work_trans_header( transaction_id, store_no, store_name, currency_code, register_no, cashier_no, till_no,
                                         ...
                                         SELECT DISTINCT h.av_transaction_id, h.store_no, w.ORG_CHN_NAME, w.DFLT_CRNCY_CODE, h.register_no, h.cashier_no,
                                                    ...
                                                    FROM #rpt_store_list w' +
                                                       ' INNER JOIN ex_transaction_header h WITH (NOLOCK) ON (w.ORG_CHN_NUM = h.store_no) 
                                                         INNER JOIN ex_transaction_line l WITH (NOLOCK) ON (h.av_transaction_id = l.av_transaction_id ' + @part_join + ')
  ...
  '
   ...
   BEGIN
     ...
     ' INNER JOIN ex_merchandise_detail m WITH (NOLOCK) ON (l.av_transaction_id = m.av_transaction_id AND l.line_id = m.line_id'
     IF @partitioning_in_use = 1
  ...
  /* Build list of transactions */
   ...
   INSERT INTO #work_trans_line( transaction_id, line_id, line_sequence, store_no, store_name, currency_code, register_no, cashier_no,
    ...
    INNER JOIN ex_transaction_line l WITH (NOLOCK) ON (t.transaction_id = l.av_transaction_id AND t.transaction_date = l.transaction_date )
 ...
 /**************************/
   ...
   SELECT t.transaction_id,  
     ...
     FROM #work_trans_header t
          INNER JOIN ex_customer c WITH (NOLOCK) ON (t.transaction_id = c.av_transaction_id AND c.line_id = 0 AND t.transaction_date = c.transaction_date)
 ...
 COALESCE(c.pos_tax_jurisdiction_code,''))) > 0
   ...
   INSERT INTO #attachment( transaction_id, line_id,line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN ex_customer c WITH (NOLOCK) ON (t.transaction_id = c.av_transaction_id AND t.line_id = c.line_id AND t.transaction_date = c.transaction_date)
               INNER JOIN #code_description cd WITH (NOLOCK) ON (c.customer_role = cd.code AND cd.code_type = 7)
         WHERE LEN(RTRIM(COALESCE(c.title,'') + COALESCE(c.first_name,'') + COALESCE(c.last_name,'') + COALESCE(c.address_1,'') + 
                         COALESCE(c.address_2,'') + COALESCE(c.city,'') + COALESCE(c.county,'') + COALESCE(c.state,'') + COALESCE(c.post_code,'') +
                         COALESCE(c.country,'') + COALESCE(c.telephone_no1,'') + COALESCE(c.telephone_no2,'') + 
   ...
   SELECT t.transaction_id,
     ...
     FROM #work_trans_header t
          INNER JOIN ex_stock_control_detail s WITH (NOLOCK) ON (t.transaction_id = s.av_transaction_id AND s.line_id = 0 AND t.transaction_date = s.transaction_date)
   ...
   SELECT t.transaction_id,
          ...
          CASE COALESCE(imrd, 'null')   WHEN 'null' THEN '' ELSE '<16.resid='+convert(nvarchar,d.imrd_fe_resource_id)+ '>' + imrd + '; ' END as temp_text
     FROM #work_trans_line t
          INNER JOIN ex_stock_control_detail s WITH (NOLOCK) ON (t.transaction_id = s.av_transaction_id AND s.line_id = t.line_id AND t.transaction_date = s.transaction_date)
          LEFT  JOIN #code_description c WITH (NOLOCK) ON (c.code_type = 68 AND c.code = s.pos_identifier_type)
          INNER JOIN #stock_display_def d WITH (NOLOCK) ON (s.display_def_id = d.display_def_id)
    WHERE LEN(RTRIM(COALESCE(convert(nvarchar, upc_no),'') + COALESCE(convert(nvarchar, merchandise_key),'') + COALESCE(convert(nvarchar,initiated_by_host),'') +
                    COALESCE(convert(nvarchar, units),'') + COALESCE(convert(nvarchar, other_store_no),'') + COALESCE(convert(nvarchar, location_no),'') + 
   ...
   DECLARE line_note_crsr CURSOR FAST_FORWARD
    ...
    SELECT t.transaction_id, ln.note_type_description, COALESCE(ln.code_display_descr, l.line_note) as temp_text,
           l.note_type
      FROM #work_trans_header t
           INNER JOIN ex_line_note l WITH (NOLOCK) ON (t.transaction_id = l.av_transaction_id AND l.line_id = 0 AND t.transaction_date = l.transaction_date)
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text)
                     SELECT t.transaction_id,
                       ...
                       FROM #work_trans_line t
                            INNER JOIN ex_line_note l WITH (NOLOCK) ON (t.transaction_id = l.av_transaction_id AND l.line_id = t.line_id AND t.transaction_date = l.transaction_date)
                            LEFT  JOIN #line_note_descr ln ON (l.note_type = ln.note_type AND (l.line_note = ln.alpha_code OR l.line_note = ln.code OR ln.code_type IS NULL)) 
   ...
   DECLARE return_detail_crsr CURSOR FAST_FORWARD
    ...
    SELECT t.transaction_id,
      ...
      FROM #work_trans_header t
           INNER JOIN ex_return_detail r WITH (NOLOCK) ON (t.transaction_id = r.av_transaction_id AND r.line_id = 0 AND t.transaction_date = r.transaction_date)
...
/* Found a header record and already has an attachment - Add new text only if enough room */
  ...
  WHERE transaction_id = @transaction_id
   ...
   INSERT INTO #attachment ( transaction_id, line_id, line_sequence, attachment_type, attachment_text)
    SELECT t.transaction_id,
      ...
      FROM #work_trans_line t
           INNER JOIN ex_return_detail r WITH (NOLOCK) ON (t.transaction_id = r.av_transaction_id AND r.line_id = t.line_id AND t.transaction_date = r.transaction_date)
           LEFT JOIN #code_description c1 ON (r.return_reason_code = c1.code AND c1.code_type = 4)
           LEFT JOIN #code_description c2 ON (r.mdse_disposition_code = c2.code AND c2.code_type = 2)
     WHERE LEN(RTRIM(COALESCE(convert(nvarchar, return_reason_code),'') + COALESCE(return_reason_message,'') + COALESCE(convert(nvarchar,mdse_disposition_code),'') +
                     COALESCE(convert(nvarchar, via_warehouse_flag),'') + COALESCE(convert(nvarchar, original_salesperson),'') + COALESCE(convert(nvarchar, original_salesperson2),'') + 
   ...
   INSERT INTO #attachment(transaction_id, line_id,line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN ex_tax_detail d WITH (NOLOCK) ON (t.transaction_id = d.av_transaction_id AND t.line_id = d.line_id AND t.transaction_date = d.transaction_date)
   ...
   DECLARE tax_override_crsr CURSOR FAST_FORWARD
    ...
    SELECT t.transaction_id,
      ...
      FROM #work_trans_header t
           INNER JOIN ex_tax_override_detail tod WITH (NOLOCK) ON (t.transaction_id = tod.av_transaction_id AND tod.line_id = 0 AND t.transaction_date = tod.transaction_date)
   ...
   INSERT INTO #attachment(transaction_id, line_id,line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN ex_tax_override_detail tod WITH (NOLOCK) ON (t.transaction_id = tod.av_transaction_id AND tod.line_id = t.line_id AND t.transaction_date = tod.transaction_date)
               LEFT JOIN tax_jurisdiction j WITH (NOLOCK) ON (tod.exception_tax_jurisdiction = j.tax_jurisdiction)
         LEFT JOIN #code_description c1 ON (tod.tax_level = c1.code AND c1.code_type = 41)
               LEFT JOIN #code_description c2 ON (tod.tax_category = c2.code AND c2.code_type = 26)
     WHERE LEN(RTRIM(COALESCE(convert(nvarchar,tod.tax_level),'') + COALESCE(convert(nvarchar,tod.tax_category),'') + COALESCE(convert(nvarchar,tod.taxable),'') +
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN ex_if_rejection_reason i WITH (NOLOCK) ON (t.transaction_id = i.av_transaction_id AND t.line_id = i.line_id AND t.transaction_date = i.transaction_date)
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN ex_exception_reason e WITH (NOLOCK) ON (t.transaction_id = e.av_transaction_id AND t.line_id = e.line_id AND t.transaction_date = e.transaction_date)
 ...
 @temp_desc + ': ' +
          ...
          FROM #work_trans_line t
               INNER JOIN ex_merchandise_detail m WITH (NOLOCK) ON (t.transaction_id = m.av_transaction_id AND t.line_id = m.line_id AND t.transaction_date = m.transaction_date)
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT t.transaction_id,
          ...
          FROM #work_trans_line t
               INNER JOIN ex_post_void_detail p WITH (NOLOCK) ON (t.transaction_id = p.av_transaction_id AND t.line_id = p.line_id AND t.transaction_date = p.transaction_date)
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT w.transaction_id, 
          ...
          FROM #work_trans_line w
               INNER JOIN ex_authorization_detail a WITH (NOLOCK) ON (w.transaction_id = a.av_transaction_id AND w.line_id = a.line_id AND w.transaction_date = a.transaction_date)
   ...
   INSERT INTO #attachment( transaction_id, line_id,line_sequence, attachment_type, attachment_text) 
    SELECT t.transaction_id, 
          ...
          FROM #work_trans_line t
               INNER JOIN ex_special_order_detail s WITH (NOLOCK) ON (t.transaction_id = s.av_transaction_id AND t.line_id = s.line_id AND t.transaction_date = s.transaction_date)
   ...
   INSERT INTO #attachment( transaction_id, line_id, line_sequence, attachment_type, attachment_text) 
        SELECT DISTINCT 
          ...
          FROM #work_trans_line t
               INNER JOIN ex_payroll_detail p WITH (NOLOCK) ON (t.transaction_id = p.av_transaction_id AND t.line_id = p.line_id AND t.transaction_date = p.transaction_date)
