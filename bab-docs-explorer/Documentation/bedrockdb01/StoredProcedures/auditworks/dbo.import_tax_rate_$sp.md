...
```sql
create proc dbo.import_tax_rate_$sp AS
...
HISTORY
Date     Name		Def#  Desc
Feb03,16 Vicci    TFS-155914  Update the effective_until_date of the prior row BEFORE inserting the new row (instead of AFTER) to avoid 
                              error:  Cannot insert duplicate key row in object 'dbo.tax_rate' with unique index 'tax_rate_x1'
 ...
 SELECT i.entry_type,
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT @errmsg = 'Failed to INSERT imported rates for store_no = ' + CONVERT(nvarchar, COALESCE(@store_no, '')) 
