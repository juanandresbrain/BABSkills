...
HISTORY :
Date     Name              Def# Desc
May27,16 Vicci         DAOM-730 Add store/date to process error log when lock fails.  Also log right process id to error log.
...
BEGIN
 ...
 BEGIN CATCH;
        ...
        ELSE
   BEGIN
          SELECT @memo2 = convert(nvarchar, @store_no),
                 @memo_date = @transaction_date
          EXEC common_error_handling_$sp 5, @errno, @errmsg, 3, @message_id, @process_name, @object_name, @operation_name, 1, 1,
                                         0, null, 0, @memo1, @memo2, @memo3, @memo_date, null, null, 0, @process_id ;
          SELECT @memo2 = NULL,
                 @memo_date = NULL;
        END;
 END CATCH;
