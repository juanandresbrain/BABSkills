...
/* Proc Name: transaction_validate_$sp
  ...
  HISTORY:  
Date     Name		Def# Desc
Jan31,17 Kiri      DAOM-2013 VD: Avoid "ORA-14400: inserted partition key does not map to any partition Unable to insert media_reconciliation_trans" error by validating that etnry date time and period reconciled
                             specified are not a future date beyond tomorrow.
...
DECLARE
  ...
  @transaction_total		line_amount_18_4,
...
/* Check for future entry_date_time or period reconciled (beyond tomorrow) for valid business dates */
IF @date_reject_id = 0
BEGIN
   IF EXISTS (SELECT 1 FROM transaction_header
       WHERE transaction_id = @transaction_id
         AND entry_date_time > dateadd(dd,1,getdate()))
     BEGIN
        SELECT @errmsg = 'Transaction entry date time is a future date. ',
               @errno = 201508,
               @message_id = 201508
        GOTO error	
     END
   ELSE
     BEGIN
        SELECT @max_line_id = max(line_id)
         ...
         WHERE transaction_id = @transaction_id
           AND display_def_id = 33  --entity/period reconciled
           AND count_date > dateadd(dd,1,getdate());
	 IF @max_line_id > -2
	    BEGIN
	       SELECT @errmsg = 'Period Reconciled specified for line ' + STR(@max_line_id) + ' is a future date. ',
	                @errno = 201508,
	                @message_id = 201508;
	       GOTO error;
	    END	   
      END
END  --IF i_date_reject_id = 0
