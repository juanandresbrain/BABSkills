# dbo.reset_nonsale_interface_$sp
...
```mermaid
flowchart LR
    SP["dbo.reset_nonsale_interface_$sp"]
...
```sql
create proc dbo.reset_nonsale_interface_$sp 
...
/* Proc name:   reset_nonsale_interface_$sp
** Description: Reset retrieval_in_progress flag to signal that the basic
**              nonsale interface file has been completed. Called by
** 		smartload script bscintface.ict

HISTORY
Date     Name		Def#  Desc
Jan05,11 Paul         105313  Use unicode datatypes
Sep03,04 Daphna  39407/40830  add logic to set process_status_log.completed_flag 
...
SELECT @process_name = 'reset_nonsale_interface_$sp',
       @message_id = 201068
