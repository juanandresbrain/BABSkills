# dbo.util_repop_if_tables_av_BU20161018_$sp
...
```mermaid
flowchart LR
    SP["dbo.util_repop_if_tables_av_BU20161018_$sp"]
    ...
    paul_repop_av(["paul_repop_av"]) --> SP
...
```sql
CREATE proc [dbo].[util_repop_if_tables_av_BU20161018_$sp] 
...
Apr14,08 PaulS     97584 Uplift 63374 to SA5
...
Apr25,06 Daphna    63374 close and deallocate cursor for each iteration of date loop
...
SELECT	@interface_id = 52, -- change this
	@from_transaction_date = '10/24/2014', -- starting date
	@to_transaction_date = '10/24/2014', -- ending date or = getdate() if all
	@reversal = 1  -- 1 = normal entries,  -1 = reversals
--	@store_no = 126 -- specify store if required
...
DECLARE reprocess_trans CURSOR FAST_FORWARD
  FOR
    SELECT DISTINCT ah.av_transaction_id
      FROM paul_repop_av ah /*av_transaction_header ah WITH (NOLOCK), av_interface_control ic
     WHERE ah.transaction_date = @date_to_reprocess
       AND ah.av_transaction_id = ic.av_transaction_id
