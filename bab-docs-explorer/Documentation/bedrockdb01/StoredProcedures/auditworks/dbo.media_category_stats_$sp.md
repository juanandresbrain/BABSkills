  ...
  SELECT  
   ...
   WHERE mh.transaction_id = tl.transaction_id
     AND ((tender_total = 0 AND
           CONVERT(NUMERIC(18,4), SIGN((gross_line_amount - pos_discount_amount) * tl.db_cr_none * voiding_reversal_flag)) = 1)
          OR
          (tl.line_object_type NOT IN (6,20) AND
           CONVERT(NUMERIC(18,4), SIGN((gross_line_amount - pos_discount_amount) * tl.db_cr_none * voiding_reversal_flag)) = CONVERT(NUMERIC(18,4), SIGN(tender_total))))
...
-- Don't change the order of the FROM clause without running the query plan
    FROM work_media_header mh WITH (NOLOCK)
	   INNER JOIN transaction_line tl WITH (NOLOCK) ON (mh.transaction_id = tl.transaction_id AND CONVERT(NUMERIC(18,4), SIGN((gross_line_amount - pos_discount_amount) * db_cr_none * voiding_reversal_flag)) <> CONVERT(NUMERIC(18,4), SIGN(mh.tender_total)))
   ...
   WHERE  denominator <> 0
     ...
     AND  CONVERT(NUMERIC(18,4), SIGN((gross_line_amount - pos_discount_amount) * db_cr_none * voiding_reversal_flag)) <> CONVERT(NUMERIC(18,4), SIGN(mh.tender_total))
     AND  CONVERT(NUMERIC(18,4), (gross_line_amount - pos_discount_amount) * db_cr_none * voiding_reversal_flag) <> 0
...
-- Don't change the order of the FROM clause without running the query plan
    FROM work_media_header mh WITH (NOLOCK)
	 INNER JOIN transaction_line tl WITH (NOLOCK) ON (mh.transaction_id = tl.transaction_id AND (CONVERT(NUMERIC(18,4), SIGN((tl.gross_line_amount - tl.pos_discount_amount) * tl.db_cr_none * tl.voiding_reversal_flag)) = CONVERT(NUMERIC(18,4), SIGN(mh.tender_total)) OR mh.denominator = 0))
   ...
   WHERE tl.line_void_flag = 0
     ...
     AND (tl.line_action < 101 or tl.line_action > 121)
     AND  tl.line_action not in (7, 8, 95, 96, 91, 92, 3, 4, 9, 10, 78, 79, 200, 204, 210, 213, 214, 215, 223, 224, 225, 226, 227, 228, 191, 192, 193, 194, 198, 196)
     AND (CONVERT(NUMERIC(18,4), SIGN((gross_line_amount - pos_discount_amount) * db_cr_none * voiding_reversal_flag)) = CONVERT(NUMERIC(18,4), SIGN(tender_total))
                 OR denominator = 0)
    OPTION(FORCE ORDER)
