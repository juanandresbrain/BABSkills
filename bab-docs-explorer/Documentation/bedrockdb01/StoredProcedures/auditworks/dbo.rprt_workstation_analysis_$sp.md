...
'
UPDATE #ras
SET tender_total = Cast(ROUND(tender_total * COALESCE(v.exchange_rate, 1), 2) as money)
FROM #ras r
     INNER JOIN ORG_CHN s1
       ON r.store_no = s1.ORG_CHN_NUM
   ...
   ON (c.currency_id = v.currency_id
     ...
     AND v.effective_date_from <= r.transaction_date 
     AND (v.effective_date_to >= r.transaction_date OR v.effective_date_to IS NULL))
'
END
