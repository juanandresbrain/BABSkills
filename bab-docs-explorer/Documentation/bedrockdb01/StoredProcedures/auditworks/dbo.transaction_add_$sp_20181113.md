# dbo.transaction_add_$sp_20181113
...
```mermaid
flowchart LR
    SP["dbo.transaction_add_$sp_20181113"]
...
```sql
create proc [dbo].[transaction_add_$sp_20181113] @process_id		binary(16),
@user_id		int,
...
Jan11,16 Vicci       TFS-153886 For mass revalidation functions, when invalid association is now valid but transaction has become out of balance, 
                                reject it as out of balance.
JUL22,15 Daphna		131151	cannot use user-defined datatype for CONVERT
...
DECLARE
  ...
  @store_no			int,
  @tender_total			money,
  @transaction_balance		line_amount_18_4,
...
BEGIN
  ...
  BEGIN
   ...
   ELSE
   BEGIN
     IF NOT EXISTS (SELECT 1 FROM sa_rejection_reason WHERE transaction_id = @transaction_id AND violated_sareject_rule = 5)
     BEGIN
       ...
       IF @errno != 0
       BEGIN
         SELECT @errmsg = 'Failed to insert sa_rejection_reason (type 5)',
	  	@object_name = 'sa_rejection_reason',
		@operation_name = 'INSERT'
         GOTO error
       END
     END --IF not S/A Reject for Out of Balance exists
   END  --ELSE of IF @transaction_balance = 0
...
BEGIN
   --populates work_control_interfaces
