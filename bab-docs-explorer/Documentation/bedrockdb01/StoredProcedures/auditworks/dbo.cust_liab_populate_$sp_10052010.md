# dbo.cust_liab_populate_$sp_10052010
...
```mermaid
flowchart LR
    SP["dbo.cust_liab_populate_$sp_10052010"]
...
```sql
create proc [dbo].[cust_liab_populate_$sp_10052010] 
...
IF @function_no IN (4,5)
  SELECT @source_process_no = 1 
--because source_process_no = 1 for both edit phase 1 and 2
...
ELSE
BEGIN
TRUNCATE TABLE work_cust_liab_main

 SELECT @errno = @@error
 IF @errno !=0 
 BEGIN
   SELECT @errmsg='Failed to truncate work_cust_liab_main',
          @object_name = 'work_cust_liab_main',
...
INSERT work_cust_liab_main (
 ...
 SELECT
	...
	h.if_entry_no,
	tl.line_id,
	MAX((1 - tl.line_void_flag) * tl.line_action),
	MAX(tl.line_object),
	...
	(1 - tl.line_void_flag) * ISNULL(md.units,0) * voiding_reversal_flag,
	...
	SUM((1 - tl.line_void_flag) * ((ABS((SIGN(ABS(1 - cld.column_no)) - 1))) * ABS(SIGN(cld.unit_amount_flag)-1) * cld.factor * voiding_reversal_flag * ISNULL(md.units,0 * ((SIGN(ABS(x.key_2 - 20))*2) - 1)))), -- units_outstanding
	SUM((1 - tl.line_void_flag) * ((ABS((SIGN(ABS(2 - cld.column_no)) - 1))) * ABS(SIGN(cld.unit_amount_flag)-1) * cld.factor * voiding_reversal_flag * ISNULL(md.units,0 * ((SIGN(ABS(x.key_2 - 20))*2) - 1)))), -- units_2
	SUM((1 - tl.line_void_flag) * ((ABS((SIGN(ABS(3 - cld.column_no)) - 1))) * ABS(SIGN(cld.unit_amount_flag)-1) * cld.factor * voiding_reversal_flag * ISNULL(md.units,0 * ((SIGN(ABS(x.key_2 - 20))*2) - 1)))), -- units_3
	SUM((1 - tl.line_void_flag) * ((ABS((SIGN(ABS(4 - cld.column_no)) - 1))) * ABS(SIGN(cld.unit_amount_flag)-1) * cld.factor * voiding_reversal_flag * ISNULL(md.units,0 * ((SIGN(ABS(x.key_2 - 20))*2) - 1)))), -- units_4
	SUM((1 - tl.line_void_flag) * ((ABS((SIGN(ABS(5 - cld.column_no)) - 1))) * ABS(SIGN(cld.unit_amount_flag)-1) * cld.factor * voiding_reversal_flag * ISNULL(md.units,0 * ((SIGN(ABS(x.key_2 - 20))*2) - 1)))), -- units_5
	...
	(1 - tl.line_void_flag) * (Sign( IsNull(sign(so.other_store_no+1), 0) + (1-abs(IsNull(sign(so.pos_identifier_type-100),1))) )
         * SIGN( 1-ABS(sign(ISNULL(display_def_id,0)-29)) + 1-ABS(sign(ISNULL(display_def_id,0)-30)) )), --reassignment_flag, other_store_no can be 0
	MAX((1 - tl.line_void_flag) * IsNull(so.units,0)), --expiry_days
	md.sku_id,
	MAX(COALESCE(md.fulfillment_store_no, md.source_store_no, CASE WHEN cld.unit_amount_flag = 3 AND cld.column_no = 4 THEN COALESCE(so.other_store_no, h.store_no) ELSE NULL END)),
	MAX(CASE WHEN cld.unit_amount_flag = 3 AND cld.column_no = 4 THEN 1 ELSE 0 END) --location_update_flag
   FROM Ex_Queue x
        ...
        INNER JOIN cust_liability_reference_type clrt WITH (NOLOCK) ON (tl.reference_type  = clrt.reference_type)
  ...
  WHERE x.queue_id = 28
    ...
    AND tl.line_object_type NOT IN (16, 17, 18, 19, 22, 23)
  GROUP BY h.source_process_no,
	...
	(1 - tl.line_void_flag) * (Sign( IsNull(sign(so.other_store_no+1), 0) + (1-abs(IsNull(sign(so.pos_identifier_type-100),1))) )
         * SIGN( 1-ABS(sign(ISNULL(display_def_id,0)-29)) + 1-ABS(sign(ISNULL(display_def_id,0)-30)) )), --reassignment_flag, other_store_no can be 0
        md.sku_id
SELECT @errno = @@error, @inserted_rows = @@rowcount
IF @errno <> 0
BEGIN
...
BEGIN
  ...
  -- If there are no other non-void customer liability rows in the transaction then flag as 50 (posted) and do not post the transaction
  ...
  WHERE process_id = @process_id
    ...
    AND rejected_status = 1
    AND rejected_validation_id = 97 -- reference_no is null
       ...
       key_store_no = 
        CASE w.key_store_no 
          when -1 then 
            ...
            COALESCE(s.originating_store_no, w.key_store_no)
   END
...
INSERT work_cust_liab_main (
   ...
   SELECT
	...
	END, --original_key_store_no
	COALESCE(so.other_store_no, so.originating_store_no, h.store_no), --default_issuing_store
	IsNull(so.count_date, transaction_date), --default_issuance_date
	td.pos_discount_amount * tl.voiding_reversal_flag,
	x.key_2, 
	IsNull(sign(sign(employee_no)+1) * employee_tracking_id, (1-sign(abs(transaction_category-242)))
	 * IsNull(import_tracking_id, default_tracking_id) 
	 + sign(abs(transaction_category-242)) * default_tracking_id), --tracking_id
	...
	md.upc_lookup_division,
	0, -- no units
	SUM((ABS((SIGN(ABS(1 - cld.column_no)) - 1)))  * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag),
	SUM((ABS((SIGN(ABS(2 - cld.column_no)) - 1)))  * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- receivable_amount
	SUM((ABS((SIGN(ABS(3 - cld.column_no)) - 1)))  * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- amount_3
	SUM((ABS((SIGN(ABS(4 - cld.column_no)) - 1)))  * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- amount_4
	SUM((ABS((SIGN(ABS(5 - cld.column_no)) - 1)))  * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- amount_5
	SUM((ABS((SIGN(ABS(6 - cld.column_no)) - 1)))  * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- amount_6
	SUM((ABS((SIGN(ABS(7 - cld.column_no)) - 1)))  * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- amount_7
	SUM((ABS((SIGN(ABS(8 - cld.column_no)) - 1)))  * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- amount_8
	SUM((ABS((SIGN(ABS(9 - cld.column_no)) - 1))) * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- amount_9
	SUM((ABS((SIGN(ABS(10 - cld.column_no)) - 1))) * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- amount_10
	SUM((ABS((SIGN(ABS(11 - cld.column_no)) - 1))) * ABS(SIGN(ABS(cld.unit_amount_flag-1))-1) * td.pos_discount_amount * cld.factor * tl.voiding_reversal_flag), -- stocked_amount
	SIGN(SUM((ABS((SIGN(ABS(1 - cld.column_no)) - 1))) * ABS(ABS(SIGN(cld.unit_amount_flag-2)) -1) * cld.factor * tl.voiding_reversal_flag * SIGN(tl.line_sequence) * ((SIGN(ABS(x.key_2 - 20))*2 -1)) )), -- stocked_flag
	SIGN(SUM((ABS((SIGN(ABS(2 - cld.column_no)) - 1))) * ABS(ABS(SIGN(cld.unit_amount_flag-2)) -1) * cld.factor * tl.voiding_reversal_flag * SIGN(tl.line_sequence) * ((SIGN(ABS(x.key_2 - 20))*2 -1)) )), -- stocked_stolen_flag	
	SIGN(SUM((ABS((SIGN(ABS(3 - cld.column_no)) - 1))) * ABS(ABS(SIGN(cld.unit_amount_flag-2)) -1) * cld.factor * tl.voiding_reversal_flag * SIGN(tl.line_sequence) * ((SIGN(ABS(x.key_2 - 20))*2 -1)) )), -- issued_flag
	SIGN(SUM((ABS((SIGN(ABS(4 - cld.column_no)) - 1))) * ABS(ABS(SIGN(cld.unit_amount_flag-2)) -1) * cld.factor * tl.voiding_reversal_flag * SIGN(tl.line_sequence) * ((SIGN(ABS(x.key_2 - 20))*2 -1)) )), -- stolen_from_cust_flag	
	SIGN(SUM((ABS((SIGN(ABS(5 - cld.column_no)) - 1))) * ABS(ABS(SIGN(cld.unit_amount_flag-2)) -1) * cld.factor * tl.voiding_reversal_flag * SIGN(tl.line_sequence) * ((SIGN(ABS(x.key_2 - 20))*2 -1)) )), -- forfeited_flag	
	SUM((ABS((SIGN(ABS(1 - cld.column_no)) - 1))) * ABS(SIGN(cld.unit_amount_flag)-1) * (td.pos_discount_amount) * cld.factor * tl.voiding_reversal_flag ), -- amount_outstanding
	...
	1 - sign( abs(x.key_2 - 20) * (tl.voiding_reversal_flag + 1) 
	 * (sign(tl.line_sequence)+1) ), --reversal_flag
	MAX(Sign( (1-sign(abs(cld.unit_amount_flag-2))) + (1-sign(abs(cld.unit_amount_flag-3))) ) 
	 * ( 1-sign(abs(cld.column_no-3)) )), --cust_update_flag
	Sign( IsNull(sign(so.other_store_no+1), 0) + (1-abs(IsNull(sign(so.pos_identifier_type-100),1))) )
         * SIGN( 1-ABS(sign(ISNULL(display_def_id,0)-29)) + 1-ABS(sign(ISNULL(display_def_id,0)-30)) ), --reassignment_flag, other_store_no can be 0
	MAX(IsNull(so.units,0)), --expiry_days
	MAX(tl.line_object),
	md.sku_id,
	MAX(COALESCE(md.fulfillment_store_no, md.source_store_no, CASE WHEN cld.unit_amount_flag = 3 AND cld.column_no = 4 THEN COALESCE(so.other_store_no, h.store_no) ELSE NULL END)),
	MAX(CASE WHEN cld.unit_amount_flag = 3 AND cld.column_no = 4 THEN 1 ELSE 0 END) --location_update_flag
...
IF @errno <> 0
BEGIN
  SELECT @errmsg = 'Failed to INSERT discounts into work_cust_liab_main',
         @object_name = 'work_cust_liab_main',
         @operation_name = 'INSERT'
  GOTO error
END
...
/* set tracking_id */
IF @function_no = 11  -- take original for conversion
BEGIN
  UPDATE work_cust_liab_main
  SET tracking_id = tender_total
  FROM work_cust_liab_main w, if_transaction_header h WITH (NOLOCK)
  WHERE w.process_id = @process_id
  ...
  IF @errno !=0 
  BEGIN
    SELECT @errmsg='Failed to get original tracking_id from if_transaction_header.tender_total',
           @object_name = 'work_cust_liab_main',
           @operation_name = 'UPDATE'
     GOTO error
  END  
END
ELSE  -- lookup in reference_range for regular processing
BEGIN
  UPDATE work_cust_liab_main
