# dbo.util_repop_if_tables_av_ts_$sp
...
```mermaid
flowchart LR
    SP["dbo.util_repop_if_tables_av_ts_$sp"]
...
```sql
CREATE proc [dbo].[util_repop_if_tables_av_ts_$sp]
...
SELECT	@interface_id = 52, -- change this
	@from_transaction_date = '12/05/2010', -- starting date
	@to_transaction_date = '12/05/2010', -- ending date or = getdate() if all
...
DECLARE reprocess_trans CURSOR
  FOR
    SELECT DISTINCT ah.av_transaction_id
     FROM rob0105 ah/*av_transaction_header ah, av_interface_control ic
     WHERE transaction_date = '12/05/2010' --@date_to_reprocess
     AND ah.av_transaction_id = ic.av_transaction_id
     --and store_no=1513
     AND ic.interface_id = 26 --@interface_id*/
FOR READ ONLY
