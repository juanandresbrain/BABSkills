...
```sql
CREATE PROC [dbo].[usp_GAAPSales_FR]
-- =============================================================================================================
-- Name: [dbo].[usp_GAAPSales_FR]  
--
-- Description:	
...
-- Revision History
--		Name:			Date:			Comments:
--		Keith Lee	11/20/2009			Created usg_GAAPSales_FR for wf_COALITION_GAAP_Sales_V1 job
--		Garyd			08/30/2010		Initial version in source control
-- exec usp_GAAPSales_FR
-- =============================================================================================================
AS 
    IF ( OBJECT_ID('tempdb..#GAAPSales_FR') IS NOT NULL ) 
        DROP TABLE #GAAPSales_FR
    SELECT  c.store_no,
            c.store_name,
...
--                        )
    ...
    WHERE   c.gl_company IN (2201) -- FR stores only
...
--CALCULATE TOTAL VAT FOR STORES' SALES
    ...
    WHERE   ( h.transaction_date BETWEEN CONVERT(CHAR, GETDATE(), 101)
            ...
            AND gl_company IN ( 2201)-- FR stores only
