...
BEGIN
  ...
  SELECT 6 - reference_type,  
         ...
         convert(varchar,date_issued, 12),  -- FORMAT:  Feb05,2002 = '020205'
         RIGHT('000' + CONVERT(VARCHAR,issuing_store_no),3),   -- take RIGHTMOST 3 char 
         RIGHT('          ' + CONVERT(varchar(10), CONVERT(INTEGER,(pos_amount_1*100))),10)
           -- format = $17.50 = '      1750'        
    FROM work_voucher    
  ...
  UPDATE #temp_export
     SET amount = RIGHT('          ' + CONVERT(varchar(10), FLOOR(pos_amount_3*100)),10)
  ...
  INSERT export_glc_pos(
	    ...
	    store_no,       
	    amount) 
  SELECT CONVERT(CHAR,record_type),
         account_number,
         CONVERT(CHAR,status),
         customer_name,
         customer_phone,
