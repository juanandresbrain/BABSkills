# dbo.spDW_AuditworksGetWebSales
...
```mermaid
flowchart LR
    SP["dbo.spDW_AuditworksGetWebSales"]
...
```sql
CREATE proc [dbo].[spDW_AuditworksGetWebSales]
	@RangeStartDate AS DATETIME
	, @RangeEndDate AS DATETIME
as

-- =====================================================================================================
-- Name: spAuditworksSelectWebSales
--
-- Description:	Captures 1 day web sales from web cart settlement log tables.
--
-- Revision History
--		Name:					Date:			Comments:
--		Kevin Shyr				6/23/2015		Created proc.	
--
-- Testing:  [dbo].[spDW_AuditworksGetWebSales] '1/1/2015', '6/10/2015'
-- =====================================================================================================
...
select	right(('0000' + cast(sa.store_no as varchar)), 4) as store_no,
    ...
    left join
		( SELECT    a.store_no,
              ...
              WHERE     ( a.transaction_date BETWEEN @RangeStartDate AND @RangeEndDate
			...
			SELECT    a.store_no,
					a.transaction_date,
                        SUM(( (b.gross_line_amount - b.pos_discount_amount) )
                            * b.db_cr_none * b.voiding_reversal_flag) * -1 AS total,
                        LEFT(MAX(a.entry_date_time), 19) AS [time of last transaction polled]
              FROM      auditworks.dbo.av_transaction_header a WITH ( NOLOCK )
                        JOIN auditworks.dbo.av_transaction_line b WITH ( NOLOCK ) ON a.av_transaction_id = b.av_transaction_id
              WHERE     ( a.transaction_date BETWEEN @RangeStartDate AND @RangeEndDate
                          AND a.transaction_void_flag = 0
                          AND a.transaction_category IN ( 1, 2 )
                          AND b.line_void_flag = 0
