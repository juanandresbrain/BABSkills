...
```sql
CREATE proc  dbo.ecp_comms_auto_adjustment_$sp 
...
HISTORY:  
Date     Name           Def#    Desc
Aug04,16 Vicci      DAOM-1220   Handle the fact that MSSQL is ordering nvarchar data strangely (eg. -, 0, 1, -1, 2, -2, A, a, -A, -a, ab, Z, z) 
                                with resulting MAX not returning expected result, but asking for the MAX with a COLLATE Latin1_General_BIN.
Jul14,16 Vicci      DAOM-1119   Handle absence of reference amount.
Dec09,15 Vicci     TFS-151829   If an employee changes position/selling-area mid-period and qualifies for an adjustment amount based on their sales 
                                in each of position/selling area, post the sum of the adjustment amounts to the reference amount for the employee 
...
SELECT a.auto_commission_adj_id,
       ...
       MAX(aa.employee_commission_code COLLATE Latin1_General_BIN) max_ecc  --collation required since nvarchar sorts '1' before '-1'
...
SELECT aa.auto_commission_adj_id,
       ...
       CASE WHEN MAX(COALESCE(ep.PSTN_CODE, '-2') COLLATE Latin1_General_BIN) = '-2' THEN NULL ELSE MAX(COALESCE(ep.PSTN_CODE, '-2') COLLATE Latin1_General_BIN) END primary_position, 
...
HAVING    MAX(aa.employee_no) > -1  --if employee specifically selected no other criteria need be met
       OR (    (a.max_ecc = '-1' OR MAX(COALESCE(ec.employee_commission_code, '-2') COLLATE Latin1_General_BIN) <> '-2')
           AND (   (MAX(aa.home_store_no) = -1 AND MAX(aa.home_store_commission_code COLLATE Latin1_General_BIN) = '-1') 
                OR MAX(CASE WHEN aa.home_store_no = IsNull(ep.ORG_CHN_NUM, em.PRMY_ORG_CHN_NUM) THEN 1 ELSE 0 END) = 1
                OR MAX(COALESCE(sc.commission_code, scd.commission_code, '-2') COLLATE Latin1_General_BIN) <> '-2'
               )
           AND (MAX(aa.primary_position COLLATE Latin1_General_BIN) = '-1' OR MAX(CASE WHEN aa.primary_position = COALESCE(ep.PSTN_CODE, '-2') THEN 1 ELSE 0 END) = 1)
...
CREATE TABLE #ecp_comms_adj(
       ...
       commission_adj_amount  money null
)
SELECT @errno = @@error
IF @errno <> 0
BEGIN
  ...
  UPDATE work_ecp_variable_amt
    ...
    FROM (SELECT v.comparison_type,
           ...
           INNER JOIN #ecp_comms_adj a
             ...
             AND a.commission_adj_amount IS NOT NULL
...
INSERT into work_ecp_variable_amt(
  ...
  SELECT src.comparison_type,
    ...
    FROM #work_ecp_auto_adj_basis src
         ...
         INNER JOIN #ecp_comms_adj a
               ON a.auto_commission_adj_id = @prior_auto_commission_adj_id
               AND COALESCE(a.commission_adj_amount, 0) <> 0 
...
SELECT @pay_period_close_date,
 ...
 WHERE e.reference_amount_type IS NOT NULL
   AND e.commission_adj_amount IS NOT NULL
 GROUP BY e.reference_amount_type,
       e.reference_amount_type,
       cp.STRT_DATE_TIME,  
       dateadd(ss, -1, cp.END_DATE_TIME),
       e.employee_no,		
