# dbo.spCL_Import_BAD_Files_Check
...
```mermaid
flowchart LR
    SP["dbo.spCL_Import_BAD_Files_Check"]
...
```sql
--DROP PROC [dbo].[spCL_Import_BAD_Files_Check]
--GO

CREATE PROC [dbo].[spCL_Import_BAD_Files_Check]
-- =============================================================================================================
-- Name: [dbo].[spCL_Import_BAD_Files_Check]
--
-- Description:	Checks for BAD files & notifies via email accordingly
--
--
-- Output: N/A
--
...
--		Paul Beckman	05/02/2011		Added Rachael L & Carla A to CC email
...
--		Paul Beckman	09/23/2019		Updated recipient to 'EntSysSupport'
...
CLEARALERTSTATUS:
	IF (SELECT COUNT(*) FROM notification_status WHERE reported = 1 AND notification_name = 'CL bad files found') = 1
	BEGIN
		UPDATE notification_status
		SET reported = 0,reported_cleared = CONVERT(VARCHAR(19),GETDATE(),120)
		WHERE notification_name = 'CL bad files found'

		SET @text = 
		'<font face =arial size = 2>' +
		'.BAD Voucher upload files have been cleared. <br>' +
		'<br>' +
		'<table border="1">' + 
		'<font face =arial size = 2>' +
		...
		'Stored Proc:  BEDROCKDB01.auditworks.dbo.spCL_Import_BAD_Files_Check <br>' +
		'Created by:  Paul Beckman <br>' +
		'Team Ownership:  Enterprise Systems <br>'

SET @Subject = 'UPDATE - Voucher Upload BAD files cleared'
	...
	VALUES (
	'spCL_Import_BAD_Files_Check', --<< Stored Proc name
	GETDATE(),
	...
	'BAD Voucher upload files have been cleared' --<< Comment
	)
...
CHECKALERTSTATUS:
	IF (SELECT COUNT(*) FROM notification_status WHERE reported = 0 AND notification_name = 'CL bad files found') = 1
	BEGIN
		UPDATE notification_status
		...
		'.BAD Voucher upload files have been found in \\saapp01\d$\EPICOR\auditworks\ICT_IMPORT\BK and need to be corrected and resubmitted by SA Admin team. <br>' +
		'These need to be resolved for the current CL import process to complete. <br>' +
		'<br>' +
		'<table border="1">' + 
...
SET @Subject = 'WARNING - Voucher Upload BAD files found'
	EXEC msdb.dbo.sp_send_dbmail  
	@profile_name = 'EntSysSupport',
	@recipients = @recipients,
	@copy_recipients = @copy_recipients,
	...
	NULL, --<< Email CC
	@Subject, --<< Email Subject
	'BAD Voucher upload files have been found' --<< Comment
	)
