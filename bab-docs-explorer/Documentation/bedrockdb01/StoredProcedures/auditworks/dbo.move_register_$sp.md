...
Sep20,16 Vicci    DAOM-1482 In the revised transaction, set the Last Modified Date/Time and the User ID, not the Edit Timestamp.
                            In the I/F tables, continue to set the Edit Timestamp to the time of the move for both the out an the in entries,
                            but indicate the original Last Modified Date/Time and the User ID in the out entry and the 
                            new Last Modified Date/Time and User ID in the in entry for consistency with the Delete.
Aug16,16 Vicci    DAOM-1171 Also handle S/A reject reason 39=Store/business date in use by another process
Apr27,16 Vicci     DAOM-664 Don't count "unmoved" transactions for Fix Invalid Register since n/a.
Feb10,16 Vicci   TFS-155283 Merge the update of transaction date and entry date time into a single update, since otherwise duplicates may result
                            and cause a halt prior to part-2 of the update (that would have resolved it) occurring.
...
BEGIN
  SELECT @edit_timestamp = MAX(edit_timestamp)
    FROM if_transaction_header th WITH (NOLOCK), #trans_id_temp ti WITH (NOLOCK)
  ...
  BEGIN
    ...
    GOTO error
  END
  IF @edit_timestamp = 0 OR @edit_timestamp IS NULL
  BEGIN
    EXEC calculate_timestamp_$sp @edit_timestamp OUTPUT
    SELECT @errno = @@error
    IF @errno != 0
     BEGIN
       SELECT @errmsg = 'Failed to execute stored procedure calculate_timestamp_$sp (recovery mode and transaction not in if_transaction_header)',
              @object_name = 'calculate_timestamp_$sp',
              @operation_name = 'EXECUTE'  	
       GOTO error
     END
...
BEGIN
  ...
  BEGIN
	...
	transaction_remark,
	@function_no,
	th.last_modified_date_time,
	in_use_timestamp,
	th.updated_by_user_id,
        th.transaction_id,
        till_no
...
BEGIN
  ...
  BEGIN
    EXEC move_line_object_lookup_$sp @process_id, @user_id, @from_store_no, @move_flag, @to_store_no, @errmsg OUTPUT, @function_no
    SELECT @errno = @@error
    IF @errno <> 0
    BEGIN
...
register_no,
  ...
  UPDATE transaction_header
	 ...
	 entry_date_time = CASE WHEN @function_no = 109 AND @from_sales_date != @to_sales_date
	                        THEN DATEADD(dd, DATEDIFF(dd, COALESCE(move_source_date, @from_sales_date), @to_sales_date), entry_date_time)
	                        ELSE entry_date_time
	                   END,
 	 move_source_date = CASE WHEN @function_no = 109 THEN NULL ELSE COALESCE(move_source_date, transaction_date) END, --	1-3YR5DH
	 edit_progress_flag = @function_no,
...
END
  ...
  AND violated_sareject_rule IN (1,2,3,4,7,10,11,12,14,39)
