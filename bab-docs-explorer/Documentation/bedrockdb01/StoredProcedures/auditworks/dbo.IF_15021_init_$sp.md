...
```sql
create proc dbo.IF_15021_init_$sp
@batch_size int 
AS
...
/* TRUNCATE Ex_Work_15021 */
TRUNCATE TABLE Ex_Work_15021
SELECT @errno = @@error 
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to Truncate Ex_Work_15021 table'
   GOTO error
   END
...
EXEC auditworks.dbo.Ex_GetMaxQueue 51, @min_serial_no, @batch_size, @max_serial_no OUTPUT
