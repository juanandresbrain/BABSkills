# dbo.spDW_DetermineTransactionsToPull
...
```mermaid
flowchart LR
    SP["dbo.spDW_DetermineTransactionsToPull"]
    av_transaction_header(["av_transaction_header"]) --> SP
    av_transaction_line(["av_transaction_line"]) --> SP
    dwETL_Transactions_To_Pull(["dwETL_Transactions_To_Pull"]) --> SP
...
```sql
CREATE PROCEDURE [dbo].[spDW_DetermineTransactionsToPull]
...
WHERE
	th.transaction_date >= @asOfDate
	AND th.transaction_series IN ('P', '', 'D', 'F', 'W', 'A', 'C') --2016-04-19 - Added C, Customer Liability (Customer Liability Maintenance), which happens when ES order is shipped (not picked up) 
	AND th.transaction_void_flag = 0
...
TRUNCATE TABLE dwETL_Transactions_To_Pull

INSERT INTO dwETL_Transactions_To_Pull
	(	transaction_id,
		store_no,
		register_no,
		transaction_no,
...
if (select count(*) from dwETL_Transactions_To_Pull where transaction_id = 378671016) > 0
	begin

		delete 
		from dwETL_Transactions_To_Pull
