...
```sql
CREATE proc  dbo.cust_liability_revalidate_$sp   
...
May31,16 Vicci        DAOM-859 Initialize @errno in lock store date loop, otherwise if the lock of one store date fails the proc thinks that all
                               subsequent locks failed even though they succeeded and as a result never processes NOR UNLOCKS them.
