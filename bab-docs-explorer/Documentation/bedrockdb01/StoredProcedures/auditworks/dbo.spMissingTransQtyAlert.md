...
```sql
CREATE PROC [dbo].[spMissingTransQtyAlert]
-- =============================================================================================================
...
--		Paul Koudelis	09/30/2024		removed EnterpriseSystemsAlerts@buildabear.com
--
...
select 
	...
	jmc.trans_nbr,
	cast(jmc.create_time as date) as CreateDate,
	max(InsertDate) InsertDate
into #jm
from PAPAMART.dw.dbo.JMC_sls_trans jmc with (nolock)
where 1=1 
...
group by 	
	...
	cast(right(device_id,2) as int),
	jmc.trans_nbr,
	cast(jmc.create_time as date)
...
set @alertrecipients = 'EntSysSupport@buildabear.com;BIAdmin@buildabear.com;benb@buildabear.com;brandonh@buildabear.com;enjolia@buildabear.com;bradw@buildabear.com;juanp@buildabear.com'
...
select 
	m.*,
		...
		(select min(jm.InsertDate)  
			from #jm jm
			where m.Store=jm.StoreID
			and m.WS=jm.RegisterNumber
			and cast(m.Sales_Date as date)=jm.CreateDate
			and jm.trans_nbr between m.from_Trans_No and m.to_Trans_No) 
		as JMStageMinDate,
		(select max(jm.InsertDate)  
			from #jm jm
			where m.Store=jm.StoreID
			and m.WS=jm.RegisterNumber
			and cast(m.Sales_Date as date)=jm.CreateDate
			and jm.trans_nbr between m.from_Trans_No and m.to_Trans_No) 
		as JMStageMaxDate,
		(select count(distinct jm.trans_nbr) 
			from #jm jm
			where m.Store=jm.StoreID
			and m.WS=jm.RegisterNumber
			and cast(m.Sales_Date as date)=jm.CreateDate
			and jm.trans_nbr between m.from_Trans_No and m.to_Trans_No) 
		as JMTransactionCount
into #x
from ##misstransqty m
where datediff(hh, 
					(select min(jm.InsertDate)  
						...
						and jm.trans_nbr between m.from_Trans_No and m.to_Trans_No),
				getdate())>3
or (select min(jm.InsertDate)  
						from #jm jm
						where m.Store=jm.StoreID
						and m.WS=jm.RegisterNumber
						and cast(m.Sales_Date as date)=jm.CreateDate
						and jm.trans_nbr between m.from_Trans_No and m.to_Trans_No) is null



if (select count(*) from #x) > 50
begin 
	set @text = 
				...
				'<tr bgcolor=#D5D5F7><th>Store</th><th>Wksn</th><th>Sales Date</th><th>from Trans No</th><th>to Trans No</th><th>Total Trans</th><th>JM (min) Stage Date</th><th>JM (max) Stage Date</th><th>JM Tran Count</th></tr>' +
