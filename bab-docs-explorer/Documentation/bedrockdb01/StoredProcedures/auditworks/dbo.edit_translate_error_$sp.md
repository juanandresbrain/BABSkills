...
HISTORY
Date     Name              Def# Desc      
Jun15,16 Vicci         DAOM-937 Avoid error 298 when future date beyond 2079 encountered in return_detail.return_from_date by rejecting transactions with future dates in this field.
May25,16 Vicci         DAOM-730 Downgrade status if adding new translate errors for trickle audit.
Mar03,16 Vicci        TFS-33667 Mark translate errors of a type listed in the translate_error_auto_verify config as verified by system.
...
BEGIN TRY
  /* Create translate error for invalid date in return attachment that was not addressed by the translate to avoid edit crashing on attempt to insert date beyond 2079 into a smalldatetime in return_detail */
   SELECT @errmsg = 'Failed to insert rows into transl_error',
...
SELECT store_no,
       ...
       13,
       @now,
       convert(nvarchar, return_from_date),
       'Date of original purchase in return attachment cannot be a future date'
  FROM transl_return_detail
 WHERE return_from_date > dateadd(dd, 1, @now)       
SELECT @rows = @@rowcount;

IF @rows > 0
BEGIN
  SELECT @errmsg = 'Failed to remove invalid dates from transl_return_detail',
	 @object_name = 'transl_return_detail',
	 @operation_name = 'UPDATE';
  UPDATE transl_return_detail
     SET return_from_date = NULL
   WHERE return_from_date > dateadd(dd, 1, @now);
END
...
INSERT translate_error (
	...
	verification_remark,
	verified_date,
	file_size)  --used as auto-verification indicator only
SELECT
	le.store_no,
	le.register_no,
	le.entry_date_time,
	...
	le.transl_error_msg,
	wh.transaction_id,
	CASE WHEN (wh.transaction_void_flag >= 1 AND wh.transaction_void_flag != 8) 
	     THEN 1 
	     ELSE CASE WHEN v.transl_reject_reason IS NOT NULL THEN 1 ELSE 0 END 
	END verified,
	CASE WHEN v.transl_reject_reason IS NOT NULL THEN v.verified_by_user_id ELSE NULL END verified_by_user_id,  
	v.verification_remark,
	CASE WHEN v.transl_reject_reason IS NOT NULL THEN getdate() ELSE NULL END verified_date, 
	CASE WHEN v.transl_reject_reason IS NOT NULL THEN -1 ELSE NULL END file_size --the file_size for errors is only used as an auto-verify indicator set to -1 to tell auto-verifications apart from regular verification in order to offer the option to deactivate auto-verify upon un-verification.
  FROM  transl_error le WITH (NOLOCK)
	 ...
	 AND wh.transaction_void_flag = 0 )
        LEFT OUTER JOIN translate_error_auto_verify v
          ON v.transl_reject_reason = le.transl_reject_reason
         AND v.output_file_code = COALESCE(le.output_file_code, N'?')
         AND v.output_file_column = COALESCE(le.output_file_column, 0)
         AND v.transaction_series = le.transaction_series
         AND v.effective_from_date_time <= @now
         AND (v.effective_until_date_time IS NULL OR v.effective_until_date_time >= @now)
...
IF @rows >= 1
 BEGIN
   /* Create SA rejection reason 8 (error in Translate). Only do this for Edit Phase1.
      ...
      nor for auto-verified translate errors.
   ...
   IF @process_timestamp != 0
	...
	FROM transl_error le WITH (NOLOCK), work_tran_header_edit wh WITH (NOLOCK)
	 ...
	 AND NOT EXISTS (SELECT 1
	                  ...
	                  WHERE v.transl_reject_reason = le.transl_reject_reason
	                    AND v.output_file_code = COALESCE(le.output_file_code, N'?')
	                    AND v.output_file_column = COALESCE(le.output_file_column, 0)
	                    AND v.transaction_series = le.transaction_series
	                    AND v.effective_from_date_time <= @now
  ...
  BEGIN
   ...
   UPDATE audit_status
      SET translate_error_qty = @reject_count,
          ...
          audit_status = CASE WHEN audit_status IN (200, 300) AND @reject_count > 0 AND @verified = 0  --can only happen if trickle_audit
	                      THEN 100
	                      ELSE audit_status
	                 END,
	  status_date = CASE WHEN audit_status IN (200, 300) AND @reject_count > 0 AND @verified = 0  --can only happen if trickle_audit
	                     THEN getdate()
	                     ELSE status_date
	                END
    WHERE store_no = @store_no
   ...
   IF @reject_count > 0 AND @verified = 0 AND @trickle_polling_flag >= 2
   BEGIN
     SELECT @errmsg = 'Failed to downgrade store_audit_status status. ',
     ...
     UPDATE store_audit_status
      ...
      WHERE store_no = @store_no
        AND sales_date = @sales_date
        AND date_reject_id = @date_reject_id
        AND store_audit_status IN (200, 300);
    END; --IF @reject_count > 0 AND @verified = 0 AND @trickle_polling_flag >= 2
