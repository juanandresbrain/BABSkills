...
HISTORY:
Date     Name         Def#  Desc
Mar07,18 Geoff   DAOM-3353  Fix syntax error in datetime conversion statement.
Apr01,16 Vicci    DAOM-292  Ensure pos_deptclass (auto-set period reconciled request) is set to 0 (although it doesn't matter since count_date is set, this is to avoid confusion).
...
BEGIN
  SELECT @errmsg = 'Store No = ' + convert(nvarchar,@store_no) +
                   ...
                   ', Effective Date = ' + convert(nvarchar, @effective_date, 101) + ' not found', --DAOM-3353
         @object_name = 'media_parameter_selection',
         @operation_name = 'SELECT'
  GOTO error
END 
