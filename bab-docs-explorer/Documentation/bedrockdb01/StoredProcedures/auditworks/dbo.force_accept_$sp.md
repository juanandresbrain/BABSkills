  ...
  BEGIN
	...
	FROM processing_request_log
      ...
      SELECT w.entry_id,
	     ...
	     300,
	     @TBL_NAME,
	     convert(varchar, @store_no) + @sep + dbo.convdate(@sales_date) + @sep + convert(varchar, w.register_no) + @sep + convert(varchar, @date_reject_id),
	     @TBL_KEY_RSRC_NAME,
	     convert(varchar, @store_no) + ' - ' + @store_name + @sep + dbo.convdate(@sales_date) + @sep + convert(varchar, w.register_no) + @sep + convert(varchar, @date_reject_id),
	     @process_no
        FROM work_adt_trl w
       WHERE process_id = @process_id 
      SELECT @errno = @@error 
