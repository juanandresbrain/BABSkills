...
```sql
create proc dbo.get_export_info_$sp (@stream_no                      tinyint = 1)
AS
/*
...
Jun15,15  Vicci   125654   Add add_BOM.
...
BEGIN
  ...
  BEGIN
    SELECT @errmsg = 'Maximum number of retries (' + CONVERT(nvarchar, @max_retry) + 
                     ') has been reached. Skipping interface id ' + CONVERT(nvarchar, @interface_id),
           @errno = 201685,
           @message_id = 201685,
...
IF @autogen_inprogress > 0 
 ...
 BEGIN
  ...
  VALUES(@process_no,
         ...
         @@spid,
         0,
         'Warning:  tax-item-group auto-generation has been in progress since ' + convert(nvarchar, @autogen_inprogress_since) + '!  Export to NSB POS (Coalition) will be held as a result.  Please verify.',
         null,
         @process_name,
         'interface_status',
         'SELECT',
...
SELECT @process_no,
         ...
         @@spid,
         0,
         'Warning:  interface ' + d.interface_description + ' (' + convert(nvarchar, s.interface_id) + ') export has been placed on hold since ' + convert(nvarchar, @autogen_inprogress_since) + ' !',
 null,
         @process_name,
         'interface_status',
         'SELECT',
         202512,
         s.hold_datetime,
         d.interface_description + ' (' + convert(nvarchar, s.interface_id) + ')'
...
AND d.interface_description + ' (' + convert(nvarchar, s.interface_id) + ')' NOT IN 
               (SELECT memo1
                  FROM process_error_log
...
FROM interface_status s
     ...
     LEFT OUTER JOIN smartload_var v
      ...
      AND SUBSTRING(v.var_name, 1, 3) = RIGHT('000' + CONVERT(nvarchar, f.interface_id), 3)
