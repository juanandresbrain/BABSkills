...
```mermaid
flowchart LR
    SP["dbo.ENCRYPTN_HSTRY_CONV_$SP"]
    EMPLY(["EMPLY"]) --> SP
...
```sql
create proc dbo.ENCRYPTN_HSTRY_CONV_$SP 

(
 @restart 	tinyint = 0, -- ( 0: continue )
 @action 	tinyint = 0,  -- ( 5: estimate rows, 6: convert )
...
/* 
  Name: ENCRYPTN_HSTRY_CONV_$SP
  Desc: Convert SDS V2.1 tokens to SDS V2.2 format
		[Prototype version] Called from C# History conversion utility repeatedly until there is no more work to do.
        This proc populates a batch of rows to be converted into a work table and then updates the actual table from the work table.
        
        The @batch_size must not be set too large since that could adversely affect the query plan of the update statement.
        This proc will populate 2 tables and will drop and recreate those tables to get the correct number of columns:
        ENCRYPTN_HSTRY_DATA_MAIN - containing information about the table and columns that need to be converted; 
        ENCRYPTN_HSTRY_DATA_WORK - containing actual row values that require conversion and the keys to the table.
        The proc will return 0 if there is nothing to process, otherwise it will return the number of key columns converted.
        When converting, exclude ref numbers < 3 nchar to avoid non-numerics such as a dash that pass the isnumeric test.
        ...
        The @batch_size variable limits the max number of rows to be converted per batch.

        For SA5, the old audit trail table would still exist for inquiry purposes after upgrading to SA5, but would no
        longer be updated. After upgrade to SA5, the EMPLY table might need to be converted. To convert EMPLY, insert a
        row into restricted_field (field_name = 'HS_ACNT_NUM', active_flag = 1, restriction_level = 1). For new installs of
        SA5, the EMPLY table would be converted by CRDM tm and the old audit trail would never require conversion.
        
		This proc supports only actions 5 and 6 
		The return code for this proc will return zero if there are no rows left to convert, will return 2 if more rows 
		exist (i.e. proc needs to be called again). Will return -1 if there is an unexpected error.

		**** Must script with ansi_warnings and ansi_nulls on to support scaleout

History:
Date     Name           Defect#  Desc
Apr15,13 Kiri           142743   author
...
SELECT @process_name = 'ENCRPTN_HSTRY_CONV_$SP',
       @file_name = 'SDS token V2.2 conversion in progress... ',
       @process_no = 11,
...
-- Only Estimating rows or Conversion is supported.
IF @action NOT IN (5,6)
  RETURN -1

-- Vefify token version 2.2 validity
IF LEN(@token_string) <> 32
BEGIN
  SELECT @errmsg = 'Token string is not of v2.2 valid type',
         @object_name = '@token_string',
         @operation_name = 'PARAMETER'
  GOTO error
END
/* Since only the reference_no column can be converted in transaction_line, build a delimited list of reference types
   that are to be converted. This will be used as an in clause to avoid a join to table restricted_field on each line */
...
IF  @ref_type_list = ' '
  BEGIN
   SELECT @errno = 201613,
	  @errmsg = 'WARNING: SA Token Conversion Utility did not find any active rows in master table restricted_field.',
...
/* if more rows still exist to be converted, then the return code would be > 0 */
/* search for rows that have not been converted by looking at length of column data when @action = 6, similar to @action = 0 */
...
IF @action = 5 
  BEGIN
	...
	IF EXISTS (SELECT 1 
        FROM restricted_field
            WHERE field_name = 'HS_ACNT_NUM'  --Note:  this row does not exist for new 5.0 implementations, only for clients upgrading from pre-4.1 where EMPLY was populated from employee since CRDM enforces conversion with no option to deactivate it.
              AND active_flag = 1
              AND restriction_level = 1) 
		SELECT @row_count = COUNT(1) FROM  EMPLY 
			WHERE LEN(HS_ACNT_NUM) = 26;
	SELECT @rows = COUNT(1) FROM   sos_credit_information 
		WHERE LEN(account_no) = 26;	
	SELECT @row_count = @row_count + @rows;
	
	SELECT  @rows = COUNT(1) FROM   av_transaction_line 
		WHERE LEN(reference_no) = 26 AND reference_type in (1,2);
	SELECT @row_count = @row_count + @rows;
	SELECT  @rows = COUNT(1) FROM    transaction_line 
		WHERE LEN(reference_no) = 26 AND reference_type in (1,2);       
	SELECT @row_count = @row_count + @rows;
	SELECT  @rows = COUNT(1) FROM   media_reconciliation_trans 
		WHERE LEN(reference_no) = 26 AND reference_type in (1,2);
	SELECT @row_count = @row_count + @rows;
	RETURN @row_count
  END

-- Ensure that the work_encr_batch table is up to date
IF EXISTS (SELECT 1
	FROM sysobjects
		WHERE type = 'U' AND name = 'work_encr_batch')
BEGIN
	DROP TABLE work_encr_batch
END
	CREATE TABLE dbo.work_encr_batch (
		tbl_name nvarchar(255) not null,
...
-- Check for data that needs to be converted
...
-- Table ENCRYPTN_HSTRY_DATA_MAIN will be truncated when there is no conversion work left to do for any table

-- **** Start of EMPLOYEE logic ****

IF @action > 0
  SET ROWCOUNT 0 -- one batch to avoid batching logic problems
...
BEGIN
  ...
  SELECT @sql_command = 'INSERT INTO ENCRYPTN_HSTRY_DATA_WORK (KEY_VAL, EMPLY_NUM)
			SELECT HS_ACNT_NUM, EMPLY_NUM
			 ...
			 AND HS_ACNT_NUM LIKE ''==SDS01%''
			 AND LEN(HS_ACNT_NUM) = 26'
  ...
  -- Update EMPLY token V2.2 
  SELECT @sql_command = 'UPDATE EMPLY SET HS_ACNT_NUM = ' +  '''' + '==SDS02' + @token_string + 
                                          ''' + RIGHT(t.KEY_VAL , 15) FROM EMPLY o  
			INNER JOIN ENCRYPTN_HSTRY_DATA_WORK t ON (o.HS_ACNT_NUM = t.KEY_VAL
				AND o.EMPLY_NUM = t.EMPLY_NUM)'
  SELECT @sql_command = @sql_command + '	 SELECT @rows = @@rowcount'
  ...
  IF @errno != 0
    BEGIN
      SELECT @errmsg = 'Failed to update Token2 (EMPLY).',
             @object_name = 'EMPLY',
...
-- check for last transaction_date that was processed
  ...
  SELECT @sql_command = 'INSERT INTO ENCRYPTN_HSTRY_DATA_WORK (KEY_VAL, transaction_date, account_no, cashier_no, store_no, register_no, transaction_no, entry_date_time)
			 SELECT account_no, transaction_date, account_no, cashier_no, store_no, register_no, transaction_no, entry_date_time
			  ...
			  '''' + ' AND transaction_date <= ''' + CONVERT(nvarchar, @end_tran_date,112) + '''
			     AND account_no LIKE  ''==SDS01%''
				 AND LEN(account_no) = 26 '
  ...
  BEGIN
   ...
   -- Update SOS token V2.2  
  SELECT @sql_command = 'UPDATE sos_credit_information SET account_no = ' +  '''' + '==SDS02' + @token_string + 
                                          ''' + RIGHT(t.account_no , 15) FROM sos_credit_information o  
			INNER JOIN ENCRYPTN_HSTRY_DATA_WORK t ON  ( o.account_no = t.KEY_VAL
				AND o.transaction_date = t.transaction_date 
				AND o.account_no = t.account_no
				AND o.cashier_no = t.cashier_no
				AND o.store_no = t.store_no 
				AND o.register_no = t.register_no
				AND o.transaction_no = t.transaction_no
				AND o.entry_date_time = t.entry_date_time)'
  SELECT @sql_command = @sql_command + '	 SELECT @rows = @@rowcount'
  EXEC sp_executesql @sql_command, N'@rows int OUT', @rows OUT
  ...
  IF @errno != 0
    BEGIN
      SELECT @errmsg = 'Failed to update Token2 (sos_credit_information).',
...
BEGIN
  ...
  -- set search end value to a high number since many lines will not require conversion
  ...
  'INSERT INTO ENCRYPTN_HSTRY_DATA_WORK (KEY_VAL, av_transaction_id, line_id)
   SELECT l.reference_no, l.av_transaction_id, l.line_id
	  ...
	  ') AND l.reference_no LIKE ''==SDS01%''' +
	  ' AND LEN(l.reference_no) = 26 '
...
-- Update av_transaction_line with token v2.2
  SELECT @sql_command = 'UPDATE av_transaction_line SET reference_no = ' +  '''' + '==SDS02' + @token_string + 
                                          ''' + RIGHT(t.KEY_VAL , 15) FROM av_transaction_line o  
			INNER JOIN ENCRYPTN_HSTRY_DATA_WORK t ON  (o.reference_no = t.KEY_VAL
				AND o.av_transaction_id = t.av_transaction_id 
  ...
  IF @errno != 0
    BEGIN
      SELECT @errmsg = 'Failed to update Token2 (av_transaction_line).',
...
-- Update transaction_line with token v2.2
  SELECT @sql_command = 'UPDATE transaction_line SET reference_no = ' +  '''' + '==SDS02' + @token_string + 
            ''' + RIGHT(t.KEY_VAL , 15) FROM transaction_line o  
			INNER JOIN ENCRYPTN_HSTRY_DATA_WORK t ON  (o.reference_no = t.KEY_VAL
  ...
  IF @errno != 0
    BEGIN
      SELECT @errmsg = 'Failed to update Token2 (transaction_line).',
...
-- KK Update media_rec with Token v2.2
  SELECT @sql_command = 'UPDATE media_reconciliation_trans SET reference_no = ' +  '''' + '==SDS02' + @token_string + 
            ''' + RIGHT(t.KEY_VAL , 15) FROM media_reconciliation_trans o  
			INNER JOIN ENCRYPTN_HSTRY_DATA_WORK t ON  ( o.reference_no = t.KEY_VAL
				AND o.transaction_id = t.transaction_id 
				AND o.line_id = t.line_id
				AND o.balancing_entity_id =  t.balancing_entity_id
				AND o.rec_side = t.rec_side
				AND o.rec_amount_type = t.rec_amount_type
				AND o.rec_amount_subtype = t.rec_amount_subtype
				AND o.entry_date_time = t.entry_date_time )'
  SELECT @sql_command = @sql_command + '   SELECT @rows = @@rowcount'
  ...
  IF @errno != 0
    BEGIN
      SELECT @errmsg = 'Failed to update Token2 (media_reconciliation_trans).',
...
BEGIN
  ...
  INSERT INTO #temp_audit_trail (entry_id,
	...
	SELECT entry_id,
	 ...
	 WHERE table_name = 'transaction_line'
	   ...
	   AND function_no IN (100,150) -- only care about add and modify functions for conversion purposes
 ...
 AND tl.line_id = CONVERT(NUMERIC, RIGHT(ah.table_key, ncharINDEX('/',REVERSE(ah.table_key))-1))
...
SET ROWCOUNT @batch_size
IF @action > 0
  SET ROWCOUNT 0 -- one batch to avoid batching logic problems when re-converting
...
-- Exclude ref numbers < 3 nchar to avoid non-numerics such as a dash that pass the isnumeric test
  ...
  IF @action = 0 -- only check for numeric if converting for the first time
	SELECT @sql_command = @sql_command + 
	  'AND LEN(before_value) <= 26) = 1 '
...
BEGIN
  ...
  IF @action = 0 -- only check for already converted if converting for the first time
	SELECT @sql_command = @sql_command + 
	  'AND LEN(ad.after_value) <= 26) = 1 '
...
SELECT @rows = 0,
       @return_code = 0,
       @file_name = 'No SDS V2.1 history found to convert',
       @table_name = 'No SDS V2.1 found',
       @key_start = NULL,
       @key_end = NULL
...
UPDATE process_log
     SET process_end_time = getdate(),
         ...
         file_name = @file_name + ' - ' + @token_string
...
BEGIN CATCH
	...
	SELECT @errno = ERROR_NUMBER(),
		@errmsg = COALESCE(@errmsg, ' ') + ' ' + ERROR_MESSAGE() + ' Line: ' + CONVERT(nvarchar, ERROR_LINE()) 
	EXEC common_error_handling_$sp 0, @errno, @errmsg, 0, @message_id, 
                                 @process_name, @object_name, @operation_name, 1
