...
PROC NAME: exec partition_clone_tb_$sp
  ...
  HISTORY:
Date     Name       Def#      Desc
Sep13,17 Sean/Terri DAOM-2537 Changes to the column order on the table and index and handling of "(MAX)" size
...
', @ndx_drop_sql = ''
  ...
  SELECT @tb_sql = @tb_sql + substring('CREATE TABLE dbo.' + @table_name + @clone_suffix + '(
        ...
        c.name + ' ' + 
        convert(nvarchar(45), CASE
								WHEN type_name(c.xtype) like '%char%' OR type_name(c.xtype) like '%binary%' THEN type_name(c.xtype) + '(' + 
										CASE WHEN type_name(c.xtype) IN (N'nvarchar', N'varbinary', N'varchar') AND c.prec = -1 THEN N'MAX'
										ELSE convert(nvarchar,c.prec)
										END + ')'
...
'
  ...
  SELECT @ndx_sql= @ndx_sql + CASE WHEN c.key_ordinal = h.min_key_ordinal 
              THEN 'CREATE ' 
              ...
              ELSE ', ' + tc.name
              END + 
         CASE WHEN c.key_ordinal = max_key_ordinal 
  ...
  '
    ...
    FROM sys.indexes i
         ...
         INNER JOIN (SELECT object_id, index_id, MAX(CASE WHEN htc.name = @partition_col THEN 1 ELSE 0 END)transaction_date_exists, MIN(key_ordinal) min_key_ordinal, MAX(key_ordinal) max_key_ordinal
   ...
   ORDER BY i.name, i.index_id, c.key_ordinal
  SELECT @errno = @@error
  IF @errno != 0
  BEGIN
