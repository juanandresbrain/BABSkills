# dbo.spAuditworksSelectGaapSalesIR_DatabearTest
...
```mermaid
flowchart LR
    SP["dbo.spAuditworksSelectGaapSalesIR_DatabearTest"]
...
```sql
CREATE proc [dbo].[spAuditworksSelectGaapSalesIR_DatabearTest]
as

-- =============================================================================================================
-- Name: spAuditworksGaapSalesIR
--
-- Description:	captures Gaap Sales from Auditworks for US stores.
--				Modified version of usp_GAAPSales_IR, which called sv_store view. 
...
-- exec spAuditworksSelectGaapSalesIR_DatabearTest
-- =============================================================================================================
set nocount on
IF (Object_ID('tempdb..#IRGaapSalesAW') IS NOT NULL) DROP TABLE #IRGaapSalesAW
	create table #IRGaapSalesAW
(
...
insert #IRGaapSalesAW
    ...
    WHERE   ss.gl_company IN ( 2103 ) -- IR stores only
...
insert #tmp_VAT
    ...
    WHERE   ( h.transaction_date BETWEEN CONVERT(CHAR, GETDATE() -3, 101)
			...
			AND h.store_no in (select location_code from  #IRGaapSalesAW)
    GROUP BY h.store_no
    ORDER BY h.store_no
    

--UPDATE GAAP SALES BY STRIPPING OUT VAT
    UPDATE  #IRGaapSalesAW
    SET     net_sales = net_sales + VAT
    FROM    #IRGaapSalesAW gs
            INNER JOIN #tmp_VAT v ON v.StoreNo = gs.location_code

select * from #IRGaapSalesAW return
