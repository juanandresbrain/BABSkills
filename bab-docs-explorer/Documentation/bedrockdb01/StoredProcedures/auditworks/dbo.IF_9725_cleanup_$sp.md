# dbo.IF_9725_cleanup_$sp

**Database:** auditworks  
**Server:** bedrockdb01  

## Architecture Diagram

```mermaid
flowchart LR
    SP["dbo.IF_9725_cleanup_$sp"]
    dbo_Ex_Execution(["dbo.Ex_Execution"]) --> SP
    dbo_Ex_Work_9725(["dbo.Ex_Work_9725"]) --> SP
    IFO97250001(["IFO97250001"]) --> SP
    IFO97250001_final(["IFO97250001_final"]) --> SP
    IFO97250004(["IFO97250004"]) --> SP
    IFO97250004_final(["IFO97250004_final"]) --> SP
    IFO97250005(["IFO97250005"]) --> SP
    IFO97250005_final(["IFO97250005_final"]) --> SP
```

## Table Dependencies

| Referenced Table |
|---|
| dbo.Ex_Execution |
| dbo.Ex_Work_9725 |
| IFO97250001 |
| IFO97250001_final |
| IFO97250004 |
| IFO97250004_final |
| IFO97250005 |
| IFO97250005_final |

## Stored Procedure Code

```sql
create proc dbo.IF_9725_cleanup_$sp
/* Name: IF_9725_cleanup_$sp
   Generated: 07/08/04 4:13:07 PM
   Automatically Generated by SmartView Exports Builder
   Called by IF_9725_main_$sp.
Update rows as being processed..
   *** DO NOT MODIFY!!! ***
*/
@executionid int 
AS
DECLARE @errmsg               varchar(255), 
        @errno                int, 
        @transaction_count    numeric(12,0), 
        @process_no           smallint, 
        @process_log_entry    bit, 
        @process_timestamp    float, 
        @row                  int, 
        @return               tinyint, 
        @from_serial_no       numeric(14,0), 
        @to_serial_no         numeric(14,0) 

SELECT @errmsg = NULL, 
       @transaction_count = 0, 
       @process_no = 19, 
       @process_timestamp = 0, 
       @return = 1, 
       @to_serial_no = 0, 
       @from_serial_no = 0 


SELECT @from_serial_no = MIN(serial_no),
       @to_serial_no = MAX(serial_no)
  FROM auditworks.dbo.Ex_Work_9725

Begin Transaction

INSERT INTO IFO97250001_final
SELECT C1_HdrID, C34_Ky_2, C2_TrnsID, C3_POSTrnsN, C4_TrnsSrc, C5_Str, C6_Rgstr, C7_Cshr, C8_TrnsTyp, C9_TrnsDt, C10_CstmrN, C11_MtchKy, C12_Tlphn, C13_TndrTyp, C14_FlgAEmplySl, C15_FlgBBrBcksRdmd, C16_FlgCMllCrtfctRdmd, C17_FlgDLyltyRwrdRdmd, C30_FlgEECrtfctRdmd, C29_FlgFByStffCrdRdmd, C28_FlgGCbCsh, C27_SgFlg8, C22_TrnsAmnt, C33_CrrncyCd
FROM IFO97250001

SELECT @errno = @@error 
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to copy data to IFO97250001_final table.'
   GOTO error
   END


INSERT INTO IFO97250004_final
SELECT C1_HdrID, C2_DtlLnN, C3_MrchndsngStylCd, C4_ClrCd, C5_SzDscrptn, C6_Qntty, C7_ItmAmntLcl, C14_DmmyNtCst, C9_SlsAssctN, C10_MrkdwnPrcnt, C11_CpnCd, C12_PrdctCd, C13_Cmmnt
FROM IFO97250004

SELECT @errno = @@error 
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to copy data to IFO97250004_final table.'
   GOTO error
   END


INSERT INTO IFO97250005_final
SELECT C1_HdrID, C2_Ln#, C3_TndrTyp, C4_Idntfr, C5_Amnt
FROM IFO97250005

SELECT @errno = @@error 
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to copy data to IFO97250005_final table.'
   GOTO error
   END


/* Insert into ex_execution the entries we have processed */
INSERT INTO auditworks.dbo.Ex_Execution
 (queue_id, object_id, execution_id, from_serial_no, to_serial_no)
 VALUES (26, 9725, @executionid, 
 @from_serial_no, @to_serial_no)
SELECT @errno = @@error 
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to insert into auditworks.dbo.Ex_Execution'
   GOTO error
   END




Commit Transaction
endofproc: /* End of Procedure */ 
RETURN @return

error: /* Error Handler */ 

If @@trancount > 0 
   ROLLBACK TRANSACTION 

SELECT @errmsg = 'IF_9725:' + @errmsg + ' - ' + convert(varchar, @errno) 

RAISERROR (@errmsg, 16, 1)
RETURN 


dbo,dt_dropuserobjectbyid,/*
**	Drop an object from the dbo.dtproperties table
*/
create procedure dbo.dt_dropuserobjectbyid
	@id int
as
	set nocount on
	delete from dbo.dtproperties where objectid=@id
```

