...
Jan17,18 Sean/Terri    DAOM-2866 Removed the time on the date comparison methods
Sep13,17 Sean/Terri    DAOM-2537 Added a check on table before dropping and modify the date comparison method that was comparing date
                          (string type) to another date (string type) instead of smalldatetime type on the work_history_tran_id table.
...
SELECT @trace_msg = ':LOG ***> Starting Purge of Archive partitions : ' + CONVERT(NCHAR, getdate(), 8);
PRINT @trace_msg;
        ...
        SELECT @exec_sql = N'
               ...
               SELECT DISTINCT h.av_transaction_id, h.transaction_date 
                 FROM av_transaction_header h WITH (NOLOCK)
                      LEFT OUTER JOIN av_exception_reason x WITH (NOLOCK)  --find the ones to keep
                        ON ''' + REPLACE (convert(nvarchar (10), @max_tran_date, 121), '-', '') + ''' > ''' + REPLACE (convert(nvarchar (10), @oldest_date_exception, 121), '-', '') + '''	--Note, this will avoid the join altogether if extended arch exceptions not used since @oldest_date_exception would then be NULL
                       ...
                       AND x.transaction_date > ''' + convert(nvarchar, @oldest_date_exception, 101) + '''
			...
			AND (COALESCE(r.history_cleanup_criteria, 1) <> 5  --5: do not retain transactions beyond history days even if still referenced by C/L
                ...
                WHERE $partition.' + @partition_function + '(h.transaction_date) = ' + CONVERT(nvarchar, @partition_no) + '--Note:  MSSQL will go to the correct partition when this function is applied.
                  AND ((tax_override_flag = 0) OR h.transaction_date <= ''' + convert(nvarchar, @tax_date, 101) + ''')
                  AND ((employee_no IS NULL) OR h.transaction_date <= ''' + convert(nvarchar, @employee_purchase_date, 101) + ''')
...
SELECT @errmsg = 'Failed to insert sa_partition_log (del hdr)',
      ...
      BEGIN
        ...
        BEGIN
          ...
          SELECT h.*
           ...
           WHERE $partition.' + @partition_function + '(h.transaction_date) = ' + CONVERT(nvarchar, @partition_no) + '--Note:  MSSQL will go to the correct partition when this function is applied.
             AND (   (h.transaction_date >= ''' + CONVERT(nvarchar, @employee_purchase_date, 101) + ''' AND h.employee_no IS NOT NULL)
                  OR (h.transaction_date >= ''' + CONVERT(nvarchar, @tax_date, 101) + ''' AND h.tax_override_flag > 0))  --Note:  sales tax exception updates this flag to 0 (not an exception), 1 (expected vs collected variance) or 2 (send-sale, return from another jurisdiction, etc) correctly.
  ...
  BEGIN 
    SELECT @exec_sql =
		...
		N'
			IF OBJECT_ID (N''av_transaction_header' + @clone_suffix + N''', N''U'') IS NOT NULL
			BEGIN

				DROP TABLE av_transaction_header' + @clone_suffix + '
