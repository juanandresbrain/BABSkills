...
flowchart LR
    ...
    translate_error_auto_verify(["translate_error_auto_verify"]) --> SP
...
```sql
CREATE proc  dbo.verify_transl_error_$sp 
 @process_id binary(16),
 @user_id    int,
 @action tinyint = 1 /* 1 = verify, 0 = unverify, 2 = verify and auto-verify going forward, 3 = unverify and stop auto-verifying going forward, 11 = verify called by force-accept */
...
HISTORY
Date	 Name      Def#   Desc
Sep27,16 Vicci  DAOM-1172 Correct the general_error go-to to handle the possibility of receiving a business rule error instead of a system error.
Sep06,16 Vicci  DAOM-1172 Flag Guided Audit as needing refreshing to make halted process visible to user.
Aug09,16 Vicci  DAOM-1172 Compensate for UI bug (not resetting process ID following a failure) by preventing a call when there is an outstanding transaction add in progress
                          for the same process_id.
Apr19,16 Vicci  DAOM-592  Remove the code which sets the Auto-verification Since Disabled indicator (-2), since 1) there is overhead to setting it and 2) the UI will not be using it to disable the "stop auto-verifying" option anyhow because the overhead of doing so outweighs the benefits to the end user.
Mar17,16 Vicci  DAOM-184  Add Audit Trail logging for auto-verification list master table update.
Mar03,16 Vicci TFS-33667  If @action = 2, list these types of translate errors in the translate_error_auto_verify config to have them auto-verified by the system going forward.
                          If @action is 3, stop auto-verifying these types of translate errors going forward.
...
DELETE function_status
 WHERE (function_no = @function_no OR function_no = 117)
   ...
   AND NOT EXISTS (SELECT 1 FROM function_status WHERE function_no = 112 AND process_id = @process_id);  --DAOM-1172:  to compensate for UI bug where they aren't resetting the process ID following an error.

--Note:  1) null store/date/date-rej/reg indicate no store date locked, but work-table would still need cleaning up.
--       2) we want this to return an error if there is already an outstanding function 112 for this process_id:  it needs to be recovered separately first.
...
IF @action = 2  --auto-verify going forward
BEGIN
  SELECT @errmsg = 'Failed to mark translate errors to be auto-verified going forward. ',
         @object_name = 'translate_error_auto_verify',
         @operation_name = 'INSERT';
  INSERT into translate_error_auto_verify(
         transl_reject_reason,
         output_file_code,
         output_file_column,
         transaction_series,
         effective_from_date_time,
         effective_until_date_time,
         verification_remark,
         verified_by_user_id)
  SELECT q.transl_reject_reason,
         q.output_file_code,
         q.output_file_column,
         q.transaction_series,
         @now,
         NULL,
         q.verification_remark,
         @user_id
    FROM (SELECT e.transl_reject_reason,
                 COALESCE(e.output_file_code, '?') output_file_code,
                 COALESCE(e.output_file_column, 0) output_file_column,
                 e.transaction_series,
                 MAX(e.verification_remark) verification_remark
            FROM work_translate_error w
                 INNER JOIN translate_error e
                    ON e.translate_error_id = w.translate_error_id
           WHERE w.process_id = @process_id
           GROUP BY e.transl_reject_reason,
                 COALESCE(e.output_file_code, '?'),
                 COALESCE(e.output_file_column, 0),
                 e.transaction_series) q
   WHERE NOT EXISTS (SELECT 1 FROM translate_error_auto_verify v 
                      WHERE v.transl_reject_reason = q.transl_reject_reason
                        AND v.output_file_code = q.output_file_code
                        AND v.output_file_column = q.output_file_column
                        AND v.transaction_series = q.transaction_series
                        AND v.effective_until_date_time IS NULL);  --currently in effect;  note, future effective dates are not supported so this is good enough.
  SELECT @rows = @@rowcount, @action = 1;

  IF @rows > 0
  BEGIN  
    SELECT @ENTRY_ID = NEWID();

    SELECT @errmsg = 'Cannot log translate error auto-verification activation info to audit trail header. ',
    ...
    VALUES (@ENTRY_ID,
      	   ...
      	   NULL,
      	   NULL,
      	   0);  --Master Table Maintenance

    SELECT @errmsg = 'Cannot log translate error auto-verification activation info to audit trail detail. ',
    ...
    SELECT @ENTRY_ID,
	   'TRANSLATE_ERROR_AUTO_VERIFY',
	     CONVERT(nvarchar, v.transl_reject_reason) + @sep + 
             v.output_file_code  + @sep + convert(nvarchar, v.output_file_column) + @sep + v.transaction_series + @sep + 
             dbo.convdate(v.effective_from_date_time) 
           TBL_KEY,
           'TK_TRAN_REJE_REAS_OUTP_FILE_OUTP_COLU_TRAN_SERI' TBL_KEY_RSRC_NAME,
             SUBSTRING(CASE WHEN c.code_display_descr IS NULL THEN CONVERT(nvarchar, v.transl_reject_reason) ELSE c.code_display_descr + ' (' + convert(nvarchar, v.transl_reject_reason) + ')' END + @sep + 
             CASE WHEN o.memo IS NULL THEN v.output_file_code  + @sep + CONVERT(nvarchar, v.output_file_column) ELSE o.memo + ' (' + v.output_file_code  + convert(nvarchar, v.output_file_column) + ')' END + @sep +
             CASE WHEN s.transaction_series IS NULL THEN v.transaction_series ELSE s.description + ' (' + v.transaction_series + ')' END + @sep + 
             dbo.convdate(v.effective_from_date_time), 1, 255) 
           TBL_KEY_RSRC_PRMS,
           'A',
           'EFFECTIVE_UNTIL_DATE'  + @sep + 'VERIFICATION_REMARK',
           NULL,
           COALESCE(dbo.convdate(v.effective_until_date_time), '') + @sep + COALESCE(v.verification_remark, '')
      FROM translate_error_auto_verify v
           LEFT OUTER JOIN code_description c
             ON c.code_type = 30
            AND c.code = v.transl_reject_reason
           LEFT OUTER JOIN translate_output_param o
             ON v.output_file_code = o.output_file
            AND v.output_file_column = o.output_column
           LEFT OUTER JOIN transaction_series s
             ON v.transaction_series = s.transaction_series
     WHERE v.effective_from_date_time = @now;
  END; --IF @rows > 0

END;

IF @action = 3  --stop auto-verifying going forward
BEGIN
  SELECT @errmsg = 'Failed to stop auto-verifying going forward. ',
         @object_name = 'translate_error_auto_verify',
         @operation_name = 'UPDATE';
  UPDATE v  --UPDATE translate_error_auto_verify
     SET effective_until_date_time = @now
    FROM translate_error_auto_verify v
         INNER JOIN (SELECT DISTINCT 
                            e.transl_reject_reason,
                            COALESCE(e.output_file_code, '?') output_file_code,
                            COALESCE(e.output_file_column, 0) output_file_column,
                            e.transaction_series
                       FROM work_translate_error w
                            INNER JOIN translate_error e
                               ON e.translate_error_id = w.translate_error_id
                      WHERE w.process_id = @process_id) q
            ON v.transl_reject_reason = q.transl_reject_reason
           AND v.output_file_code = q.output_file_code
           AND v.output_file_column = q.output_file_column
           AND v.transaction_series = q.transaction_series
   WHERE v.effective_until_date_time IS NULL;
  SELECT @rows = @@rowcount;

/*  
--Removed since UI has decided to offer the Remove from Auto-verification unconditionally (not worth the overhead of checking the file size of each row).
  SELECT @errmsg = 'Failed to mark other auto-verified rows as no longer needing "remove from auto-verification" prompt if ever unverified via UI. ',
	 @object_name = 'translate_error',
	 @operation_name = 'UPDATE';
  UPDATE e  --UPDATE translate_error
     SET file_size = -2  --file_size is only an auto-verify indicator (-1 = auto-verified, -2=auto-verified but auto-verification since turned off, null otherwise). 
    FROM translate_error_auto_verify v
         INNER JOIN translate_error e
            ON e.verified = 1
           AND e.verified_date >= CONVERT(smalldatetime, v.effective_from_date_time)
           AND e.file_size = -1 --i.e. auto-verified
           AND e.transl_reject_reason = v.transl_reject_reason
           AND COALESCE(e.output_file_code, '?') = v.output_file_code
           AND COALESCE(e.output_file_column, 0) = v.output_file_column
           AND e.transaction_series = v.transaction_series          
   WHERE v.effective_until_date_time = @now;
*/
  ...
  BEGIN  
    ...
    SELECT @errmsg = 'Cannot log translate error auto-verification deactivation infoto audit trail header. ',
	   @object_name = 'ADT_TRL_HDR',
	   @operation_name = 'INSERT';
    INSERT INTO ADT_TRL_HDR (
           ENTRY_ID,
    ...
    SELECT @errmsg = 'Cannot log translate error auto-verification deactivation info to audit trail detail. ',
	   @object_name = 'ADT_TRL_DTL',
	   @operation_name = 'INSERT';
    INSERT INTO ADT_TRL_DTL (
	   ENTRY_ID,
	   ...
	   'translate_error_auto_verify',
	     CONVERT(nvarchar, v.transl_reject_reason) + @sep + 
           ...
           'TK_TRAN_REJE_REAS_OUTP_FILE_OUTP_COLU_TRAN_SERI' TBL_KEY_RSRC_NAME,
             ...
             CASE WHEN o.memo IS NULL THEN v.output_file_code  + @sep + CONVERT(nvarchar, v.output_file_column) ELSE o.memo + ' (' + v.output_file_code  + convert(nvarchar, v.output_file_column) + ')' END + @sep +
             CASE WHEN s.transaction_series IS NULL THEN v.transaction_series ELSE s.description + ' (' + v.transaction_series + ')' END + @sep + 
             dbo.convdate(effective_from_date_time), 1, 255)
           TBL_KEY_RSRC_PRMS,
           'M',
           'EFFECTIVE_UNTIL_DATE',
           NULL,
           dbo.convdate(v.effective_until_date_time)
      FROM translate_error_auto_verify v
           LEFT OUTER JOIN code_description c
             ON c.code_type = 30
            AND c.code = v.transl_reject_reason
	...
	WHERE wt.process_id = @process_id
     ...
     BEGIN -- update flags for locked store-register
	  ...
	  SET verified = @action_value,
	      ...
	      file_size = CASE WHEN @action_passed = 2 THEN -1 ELSE NULL END  --file_size is only an auto-verify indicator (-1 = auto-verified, -2=auto-verified but auto-verification since turned off, null otherwise).  It is set to -1 even though this technically is only a auto-verify request to allow for UI "unverify" to prompt for removal of on-going auto-verification.
	 FROM work_translate_error wt, translate_error te
	WHERE process_id = @process_id
	  AND wt.translate_error_id = te.translate_error_id
...
general_error:
  IF NOT EXISTS (SELECT 1 FROM message WHERE id = @errno)
  BEGIN
    SELECT @errno = ERROR_NUMBER();
  END;
  
  SELECT @errmsg2 = @process_name + ':  ' + COALESCE(@errmsg, '') + COALESCE(' Line: ' + CONVERT(nvarchar, ERROR_LINE()) + ', ' + ERROR_MESSAGE(), '');
