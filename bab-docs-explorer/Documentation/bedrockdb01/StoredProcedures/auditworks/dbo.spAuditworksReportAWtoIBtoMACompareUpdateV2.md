# dbo.spAuditworksReportAWtoIBtoMACompareUpdateV2
...
```mermaid
flowchart LR
    SP["dbo.spAuditworksReportAWtoIBtoMACompareUpdateV2"]
...
```sql
CREATE proc [dbo].[spAuditworksReportAWtoIBtoMACompareUpdateV2]
...
--STEP 3 --- Compare data and send emails
		...
		if (select count(*) from tblCompareAWtoIB where aw_ib_diff > 20 or ib_store is null) <> 0
			begin
				...
				set @body = 'Discrepancies exist between Auditworks and Merchandising. Sales Posting and Validations will run again once more automatically.'
							+ char(10) + char(13) 
				set @query = 'set nocount on 
								...
								order by aw_store 
								select ''Controlled by stored procedure: BEDROCKDB01.Auditworks.spAuditworksReportAWtoIBtoMACompareUpdate'''
			end
		ELSE
			begin
		...
		begin					
			...
			@recipients = 'EntSysSupport@buildabear.com; JeffK@buildabear.com; biadmin@buildabear.com',
			@body = @body,
			@query = @query,
			@subject = @subject,
			@profile_name = 'SQLServices' 
			    IF @subject = 'AW to IB comparison - PROBLEM'
    BEGIN
        EXEC msdb.dbo.sp_send_dbmail 
...
BEGIN
    ...
    EXEC msdb.dbo.sp_start_job @job_name = 'Validation AutoRerun'
END
		end
...
-----------
		...
		if (select count(*) from tblCompareIbPeriod b, dbo.tblCompareMevPeriod m where b.ib_store = m.mev_store	and ABS(b.ib_sales) <> m.mev_sales) <> 0
			begin
				...
				set @body = 'Discrepancies exist between Merchandising and Merchandise Analytics. Sales Posting and Validations will run again once more automatically.'
				set @query = 'set nocount on 
		...
		begin					
			...
			@subject = @subject,
			@profile_name = 'SQLServices'
			  -- Only send to entsyssupport@buildabear.com if the subject is 'All Sales Have not Posted - PROBLEM'
    IF @subject = 'Mev to IB comparison - PROBLEM'
    BEGIN
        EXEC msdb.dbo.sp_send_dbmail 
            @recipients = 'justincr@buildabear.com',
            @body = @body,
...
END
		...
		IF (SELECT COUNT(*) FROM tblCompareIbPeriod b, dbo.tblCompareMevPeriod m WHERE b.ib_store = m.mev_store AND ABS(b.ib_sales) <> m.mev_sales) <> 0
BEGIN
    SET @subject = 'All Sales Have not Posted - PROBLEM'
    SET @body = 'Discrepancies exist between Merchandising and Merchandise Analytics. Sales Posting and Validations will run again once more automatically.'
    SET @query = 'SET NOCOUNT ON;
                  SELECT b.ib_store, b.ib_sales, m.mev_sales 
                  FROM auditworks.dbo.tblCompareIbPeriod b
                  INNER JOIN auditworks.dbo.tblCompareMevPeriod m ON b.ib_store = m.mev_store
                  WHERE ABS(b.ib_sales) <> m.mev_sales
                  ORDER BY b.ib_store;
                  SELECT ''Controlled by stored procedure: BEDROCKDB01.Auditworks.spAuditworksReportAWtoIBtoMACompareUpdate'''
    ...
    -- Only send to entsyssupport@buildabear.com if the subject is 'All Sales Have not Posted - PROBLEM'
    IF @subject = 'All Sales Have not Posted - PROBLEM'
    BEGIN
        EXEC msdb.dbo.sp_send_dbmail 
            @recipients = 'entsyssupport@buildabear.com',
            @body = @body,
...
ELSE
BEGIN
    SET @subject = 'All Sales have posted - NO PROBLEM'
    SET @body = 'Discrepancies DO NOT exist between Merchandising and Merchandise Analytics.'
    SET @query = 'SET NOCOUNT ON;
                  SELECT ''Controlled by stored procedure: BEDROCKDB01.Auditworks.spAuditworksReportAWtoIBtoMACompareUpdate'''
...
END
		...
		-----Check to make sure the file directory exists, send an alert to call posAdmin if directory does not exist
			...
			begin
				EXEC msdb.dbo.sp_send_dbmail
				@recipients = 'Poll@buildabear.com; entsyssupport@buildabear.com',
		...
		begin	
			...
			set @query3 = 'set nocount on select b.ib_store STORE, b.ib_sales IB_SALES, m.mev_sales MEV_SALES from auditworks.dbo.tblCompareIbPeriod b INNER JOIN auditworks.dbo.tblCompareMevPeriod m ON b.ib_store = m.mev_store where ABS(b.ib_sales) <> m.mev_sales 
order by b.ib_store'
