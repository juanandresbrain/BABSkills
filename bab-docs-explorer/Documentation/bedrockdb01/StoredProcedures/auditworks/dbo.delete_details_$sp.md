...
Jul14,16 Vicci        DAOM-1126 Clean up geninfo detail too.
...
DELETE geninfo_detail
  FROM geninfo_detail d, work_delete_detail w WITH (NOLOCK)
 WHERE d.transaction_id = w.transaction_id
   AND w.process_id = @process_id
SELECT @errno = @@error
IF @errno != 0
  BEGIN
	SELECT @errmsg = 'Unable to delete geninfo_detail',
...
error:
	...
	RETURN
dbo,spUpdateStatisticsForVATLoad,-- =============================================
-- Author:	Rick Caminiti
-- Create date: 3/6/2012
-- Description:	Could not execute update statistics over a databaselink so we had to 
--                  put it in a proc.  This is called from BABWSCORE01 SQL Agent Job AuditWorksImport_Transactions_VAT
-- =============================================
CREATE PROCEDURE spUpdateStatisticsForVATLoad
AS
BEGIN

update statistics dbo.line_object
update statistics dbo.merchandise_detail
update statistics dbo.discount_detail
update statistics dbo.transaction_header
update statistics dbo.transaction_line
