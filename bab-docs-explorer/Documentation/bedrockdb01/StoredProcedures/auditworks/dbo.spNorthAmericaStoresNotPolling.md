# dbo.spNorthAmericaStoresNotPolling
...
```mermaid
flowchart LR
    SP["dbo.spNorthAmericaStoresNotPolling"]
...
```sql
CREATE  procedure [dbo].[spNorthAmericaStoresNotPolling]

-- =====================================================================================================
-- Name: spNorthAmericaStoresNotPolling
--
-- Description:	
--
-- Input:	
...
--		Garyd			08/30/2010		Modify query for SA 5.0 tables and use db_mail.
--		Paul B			10/26/2010		Changed from sp_NorthAmericaStoresNotPolling to spNorthAmericaStoresNotPolling
--		Paul B			07/26/2011		Added Translate version 17 for Store 6.2 translate stores
--		Paul B			01/29/2015		Modified Store exclusion range high end from 2099 to 2399 for DNK
...
--		Paul Beckman	06/02/2016		Modified Store exclusion range high end from 2399 to 3099 for CHN
...
--		Lizzy Timm		02/23/2021		Added Bcc for emails sent when over 99 stores are not polling and added logic to only send to Bcc upto once per day
...
set @recipients = 'BIAdmin@buildabear.com;benb@buildabear.com;brandonh@buildabear.com;enjolia@buildabear.com;bradw@buildabear.com;BIAdmin@buildabear.com;benb@buildabear.com;brandonh@buildabear.com;enjolia@buildabear.com;bradw@buildabear.com;juanp@buildabear.com'
--'poll@buildabear.com'
...
begin
	...
	VALUES (
	'spNorthAmericaStoresNotPolling', --<< Stored Proc name
	GETDATE(),
	'No', --<< Issues found - Yes / No
	'No', --<< Action required - Yes / No
	'No', --<< Notification sent - Yes / No
	'North American Stores ALL Trickle Polling' --<< Comment
	)
end

--=======================================
if (select count(Store_No) from ##StoresNotPolled) between 1 and 14
begin
	INSERT INTO notification_history
	(stored_proc_name,
	record_logged_datetime,
	...
	'No', --<< Action required - Yes / No
	'No', --<< Notification sent - Yes / No
	'North American stores ' + CONVERT(VARCHAR(8000),SUBSTRING(@listStr , 1, LEN(@listStr)-1)) + ' have not reported sales to Sales Audit within the past 3 hours' --<< Comment
	)
end

--=======================================
if (select count(Store_No) from ##StoresNotPolled) between 15 and 99
begin
set @text = 
				'<font face =arial size = 2>' +
				'The following North American stores have not reported sales to Sales Audit within the past 3 hours:<br>' +
				'<br>' +
				'<table border="1">' + 
				'<font face =arial size = 2>' +
				...
				'Job Name:  StoresNotTricklePolling North America <br>' +
				'Stored Proc:  BEDROCKDB01.auditworks.dbo.spNorthAmericaStoresNotPolling <br>' +
				'Created by:  Paul Beckman <br>' +
				'Team Ownership:  Enterprise Systems <br>'

	
	set @Subject = 'ALERT - North American Stores Not Trickle Polling'
	...
	'Many North American stores have not reported sales to Sales Audit within the past 3 hours' --<< Comment
	)
end

--=======================================
if (select count(Store_No) from ##StoresNotPolled) > 99
begin
	set @text = 
				'<font face =arial size = 2 color="Red">' +
				'More than 99 North American stores have not reported sales to Sales Audit within the past 3 hours:<br>' +
	...
	set @Subject = 'WARNING - North American Stores Not Trickle Polling'
	...
	--@body = @text,
	--@body_format = 'HTML'
	IF (SELECT COUNT(*) FROM notification_history WHERE stored_proc_name = 'spNorthAmericaStoresNotPolling' AND action_required = 'Yes' AND convert(varchar, record_logged_datetime, 101) = convert(varchar, getdate(), 101)) < 1
	BEGIN
		declare @textA nvarchar(max)
		SET @textA = 'More than 99 North American stores have not reported sales to Sales Audit.'
  ...
  ELSE
	...
	NULL, --<< Email CC
	@Subject, --<< Email Subject
	'Too many North American stores have not reported sales to Sales Audit within the past 3 hours' --<< Comment
	)
end
