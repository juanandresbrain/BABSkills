# dbo.spAuditworksReportAWtoIBtoMACompareUpdate
...
```mermaid
flowchart LR
    SP["dbo.spAuditworksReportAWtoIBtoMACompareUpdate"]
...
```sql
CREATE proc [dbo].[spAuditworksReportAWtoIBtoMACompareUpdate]
...
--STEP 3 --- Compare data and send emails
		...
		begin					
			...
			@recipients = 'EntSysSupport@buildabear.com; BIAdmin@buildabear.com; JeffK@buildabear.com',
			@body = @body,
			@query = @query,
			@subject = @subject,
...
-----------
		...
		begin					
			...
			@subject = @subject,
			@profile_name = 'SQLServices'
			  -- Only send to enterprisesystemsalerts@buildabear.com if the subject is 'All Sales Have not Posted - PROBLEM'
...
END
		...
		-----Check to make sure the file directory exists, send an alert to call posAdmin if directory does not exist
			...
			begin
				EXEC msdb.dbo.sp_send_dbmail
				@recipients = 'Poll@buildabear.com; Enterprisesystemsalerts@buildabear.com',
