...
```mermaid
flowchart LR
    SP["dbo.reset_mdse_interface_$sp"]
...
```sql
create proc dbo.reset_mdse_interface_$sp 
...
/* Proc name:   reset_mdse_interface_$sp
** Description: Reset retrieval_in_progress flag to signal that the basic
**              merchandise interface file has been completed. Called by
...
SELECT @process_name = 'reset_mdse_interface_$sp',
       @message_id = 201068
