...
```sql
create proc dbo.force_accept_0102_$sp 
...
IF EXISTS (SELECT 1 FROM processing_request_log
             WHERE user_name = @name_of_user
               AND request_datetime = @datetime_of_request
               AND processing_status != 0
               AND processing_status <= 20
               AND register_no <= 0)
   SELECT @all_register = 1
ELSE SELECT @all_register = 0
...
BEGIN   
  ...
  SELECT getdate(), 
  	 ...
  	 + '/' + CONVERT(VARCHAR(11), @sales_date), 
         ...
         @sales_date,
         @date_reject_id
   FROM processing_request_log ps, audit_status st
  WHERE st.audit_status IN (100, 200)
     AND @store_no        = ps.store_no
     AND @sales_date      = ps.sales_date
     AND @date_reject_id  = ps.date_reject_id
     AND st.store_no 	  = @store_no
     AND st.sales_date    = @sales_date
     AND st.date_reject_id= @date_reject_id
     AND processing_status != 0
     AND processing_status <= 20   
  ...
  SELECT @errno = @@error
    IF @errno !=0
      BEGIN
        SELECT @errmsg = 'Failed to INSERT audit_trail_detail (@all_register=1).'
        GOTO error
      END                 
...
BEGIN   
  ...
  SELECT getdate(), 
  	 ...
  	 + '/' + CONVERT(VARCHAR(11), @sales_date), 
         ...
         @sales_date,
         @date_reject_id
   FROM processing_request_log ps , register r, audit_status st
  WHERE st.audit_status IN (100, 200)
     AND @store_no        = ps.store_no
     AND @sales_date      = ps.sales_date
     AND @date_reject_id  = ps.date_reject_id
     ...
     AND r.store_no       = st.store_no
     AND r.register_no    = st.register_no
     AND st.sales_date    = ps.sales_date
     AND st.date_reject_id = ps.date_reject_id
     AND ps.processing_status != 0
     AND ps.processing_status <= 20
...
INSERT audit_trail_detail
    ...
    IF @errno !=0
      BEGIN
        SELECT @errmsg = 'Failed to INSERT audit_trail_detail (@all_register=0).'
        GOTO error
      END                        
END
