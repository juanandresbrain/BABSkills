...
Sep22,16 Vicci        DAOM-1501 Correct updates which reference interface_reject_move to be limited to the current process ID.
Sep20,16 Vicci        DAOM-1482 In the I/F tables, set Last Modified Date/Time of the 'in' entry to match that in the newly revised transaction header.
...
BEGIN
  UPDATE #work_interfaces
    ...
    FROM #work_interfaces wi,
         ...
         if_rejection_applicability ir
   WHERE rm.process_id = @process_id
     AND rm.transaction_id = wi.transaction_id
     AND wi.interface_id = ir.interface_id
     AND rm.if_reject_reason = ir.if_reject_reason
