...
flowchart LR
    ...
    dbo_Ex_Work_9725(["dbo.Ex_Work_9725"]) --> SP
```
...
```sql
create proc dbo.IF_9725_init_$sp
@batch_size int 
AS
...
/* TRUNCATE Ex_Work_9725 */
TRUNCATE TABLE Ex_Work_9725
SELECT @errno = @@error 
IF @errno <> 0 
   BEGIN
   SELECT @errmsg = 'Unable to Truncate Ex_Work_9725 table'
   GOTO error
   END
...
INSERT INTO auditworks.dbo.Ex_Work_9725
