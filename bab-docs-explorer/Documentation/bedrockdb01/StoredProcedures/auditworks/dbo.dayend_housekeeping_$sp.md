...
flowchart LR
    ...
    clean_work_tables__sp(["clean_work_tables_$sp"]) --> SP
...
HISTORY
Date     Name       Def# Desc
Jun17,16 Vicci  DAOM-916 Clean up any orphaned process_step_log entries resulting from failed processes and reset the corresponding process_status_log entries.
Mar18,16 Vicci  DAOM-191 Clean up any orphaned work table rows resulting from killed processes.
...
--PURGE OF WORK TABLES
SELECT @trace_msg = ':LOG ===> clean_work_tables_$sp begins at: ' + CONVERT(NCHAR, getdate(), 8)
PRINT @trace_msg

EXEC clean_work_tables_$sp
SELECT @errno = @@error
IF @errno != 0
BEGIN
    SELECT @errmsg = 'Failed to execute stored procedure clean_work_tables_$sp',
           @object_name = 'clean_work_tables_$sp',
           @operation_name = 'EXECUTE'
    GOTO error
END
...
UPDATE process_status_log
   SET expected_workload = 1,
       completed_workload = 1,
       completed_flag = 1
 WHERE (completed_flag = 0 OR expected_workload > completed_workload)
   AND (process_no IN (7, 73) OR (process_no >= 241 AND process_no <= 249))
   AND EXISTS (SELECT 1
                 FROM process_step_log s
                WHERE s.process_step_start_time < dateadd(mm, -1, getdate())  --safety check, should not be required.
                  AND s.process_no = process_status_log.process_no
                  AND process_status_log.process_start_time <= s.process_step_start_time
                  AND s.stream_no > 0
                  AND s.stream_no NOT IN (SELECT input_id FROM input_processing_status)
              ) 
SELECT @errno = @@error
IF @errno != 0
BEGIN
    SELECT @errmsg = 'Failed to reset dead processes in process_status_log.  ',
...
DELETE FROM process_step_log 
 WHERE process_step_start_time < dateadd(mm, -1, getdate())  --safety check, should not be required.
   AND (process_no IN (7, 73) OR (process_no >= 241 AND process_no <= 249)) 
   AND stream_no > 0
   AND stream_no NOT IN (SELECT input_id FROM input_processing_status)
SELECT @errno = @@error
IF @errno != 0
BEGIN
    SELECT @errmsg = 'Failed to remove dead processes in process_step_log.  ',
