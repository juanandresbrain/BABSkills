...
```sql
CREATE proc  dbo.rec_update_audit_status_$sp 
@process_id		    binary(16),       --current spid or spid being recovered
@user_id                    int,
@process_no                 smallint,
...
PROC NAME: rec_update_audit_status_$sp
     ...
     Note: media_rec_only_lock: 0=store/date already naturally locked by current process, 
				1=store/date side-affected, so temporarily locked to do update, 
				2=store/date can't be locked since already locked by another process, 
				3=store/date already naturally locked by current process but side-affected register not in batch

     Unicode version.
     
HISTORY: 
Date      Name          Def# Desc
Nov24,16 Serguei   DAOM-1775 Since build_subledger_$sp prematurely unlocks store/date being dayended (perhaps to allow
                             media rec to do lock anyhow since dayend doesn't change the figures?), ensure status verification is not executed against
                             store/dates in the midst of being dayended.
May26,16 Vicci      DAOM-730 Remove setting of trickle_counts_flag to 3 since stuff trickling in is skipped in the cursor doing the register verify.
                             Downgrade audit_status/store_audit_status from 200/300 to 100 if introducing an over/short beyond tolerance.
                             If called by the Edit in trickle audit mode, don't lock the store/dates trickling in:  phase2 will double check the
                             audit status media rec figures.
May13,16 Vicci      DAOM-730 Set trickle_counts_flag = 3 (current batch verified by media rec) edit_store_date_list so that trickle audit knows 
                             which store/reg/date have already been verified and which it has to do itself.
                             Don't prematurely unlock ALL balancing entities just because 1 store/date is unlockable and being skipped.
...
Apr05,10  Vicci       115860 Don't assume that if a store/date is locked by Edit then all registers used on that str/date 
			     ...
			     that are not in the batch but whose stats are side-affected by the reg in batch get verified.
...
Apr27,04  Maryam     DV-1071 Receive @process_id and pass it to the sub procs.
                             (Paul) Modified the where clause for needs_locking_crsr.
...
Mar10,04  Maryam       25346 Add s.date_reject_id = 0 to the where clause when updating audit_status for side-affected 
...
BEGIN
  ...
  SELECT @verify_store_status = 0;  --don't do for each register, do for each store
END;
...
BEGIN  
  ...
  IF @process_no IN (1,2,4,5,103) -- Edit
  BEGIN  
      SELECT @errmsg='Failed to indicate that register was side-affected by Edit (not in its current batch)',
             @object_name = 'work_rec_audit_status',
  ...
  BEGIN
      SELECT @errmsg='Failed to indicate that register was side-affected by manual function (not in its current batch)',
             @object_name = 'work_rec_audit_status',
             @operation_name = 'UPDATE';
    UPDATE work_rec_audit_status 
       SET media_rec_only_lock = 3  --3=store/date already naturally locked by current process but side-affected register not in batch
     WHERE work_rec_audit_status.rec_process_id = @rec_process_id
       AND NOT EXISTS (SELECT 1
...
BEGIN
  ...
  Which are not already locked and lock them using an update_in_progress flag = @process_no */
      ...
      SELECT @errmsg = 'Unable to declare cursor needs_locking_crsr',
             @object_name = 'needs_locking_crsr',
        @operation_name = 'OPEN';
  DECLARE needs_locking_crsr CURSOR FAST_FORWARD
  FOR 
  SELECT DISTINCT sas.store_no,
   ...
   WHERE wras.rec_process_id = @rec_process_id
     ...
     AND ((@process_no IN (4,103) AND sas.update_in_progress NOT IN (1,4,103) AND sas.trickle_in_progress_flag <> 1) -- not already locked by edit nor in edit trickle audit batch
  ...
  BEGIN
    ...
    BEGIN
          SELECT @errmsg='Failed to set media_rec_only_lock.',
                 ...
                 @operation_name = 'UPDATE';
      UPDATE work_rec_audit_status
         SET media_rec_only_lock = 1,  --1=side-affected-store_date.temp-locked, 3=side-affected-reg.store-date-naturally-locked
    ...
    END;  -- @errno <> 0 
  END; -- WHILE 1=1 (needs_locking_crsr)          
  ...
  CLOSE needs_locking_crsr;
  DEALLOCATE needs_locking_crsr;
...
BEGIN -- cleanup work table
  ...
  UPDATE audit_status
     SET media_short = ISNULL(wras.media_short, s.media_short), 
         ...
         audit_status = CASE WHEN s.audit_status IN (200, 300) AND wras.short_by_tender_over_limit > 0 AND wras.media_rec_verified = 0
		             THEN 100 ELSE s.audit_status END,   --this change would only ever be possible for trickle audit
	 status_date = CASE WHEN s.audit_status IN (200, 300) AND wras.short_by_tender_over_limit > 0 AND wras.media_rec_verified = 0
		            THEN getdate() ELSE s.status_date END --this change would only ever be possible for trickle audit
  ...
  IF @process_no IN (1,2,4,5,103) AND @trickle_polling_flag >= 2
  BEGIN
    --For trickle audit, the update above may have downgraded the audit_status so the store_audit_status might need downgrading too.
    SELECT @errmsg='Failed to downgrade store_audit_status.',
    ...
    UPDATE sas  --UPDATE store_audit_status
      ...
      FROM (SELECT DISTINCT store_no, transaction_date
              FROM work_rec_audit_status
             WHERE rec_process_id = @rec_process_id
               AND media_rec_only_lock NOT IN (1, 2)  --do not evaluate 1 because will be fixed in edit_side_lock_str_date_crsr, nor 2 because its audit status wasn't touched since it couldn't be locked
               AND short_by_tender_over_limit > 0
               AND media_rec_verified = 0) wras
           INNER JOIN store_audit_status sas
              ON sas.store_no = wras.store_no
             AND sas.sales_date = wras.transaction_date
             AND sas.store_audit_status IN (200, 300)
             AND sas.date_reject_id = 0;
  END;  --IF i_process_no IN (1,2,4,5,103) AND i_trickle_polling_flag >= 2

    
  /* Update audit_status for side-affected store/register/dates that were only locked for media-rec purposes */
    SELECT @errmsg='Failed to open side_affected_crsr cursor.',
           @object_name = 'side_affected_crsr',
           @operation_name = 'OPEN';
  DECLARE side_affected_crsr CURSOR FAST_FORWARD
      FOR
   SELECT wr.store_no,
          wr.register_no,
          wr.transaction_date,
    ...
    WHERE wr.rec_process_id = @rec_process_id
      AND wr.media_rec_only_lock in (1, 3)  --(1=store/date side-affected and locked, 3=store/date already naturally locked by current process but side-affected register not in batch)
      AND (wr.prior_audit_status IS NULL OR wr.prior_audit_status < 301 OR wr.prior_audit_status > 500)
      AND wr.store_no = s.store_no
      AND wr.register_no = s.register_no
      AND wr.transaction_date = s.sales_date
      AND s.date_reject_id = 0
      AND s.trickle_in_progress_flag = 0;  --trickle audit starts with verified and downgrades if necessary;  if the register isn't in my natural batch (i.e. only_lock 1,3)  I didn't downgrade it so no need to re-verify it;  if it is in my batch it will be verified when the rest of my job is done.

  OPEN side_affected_crsr;

  SELECT @cursor_open = 2;

  WHILE 2 = 2
  BEGIN
    FETCH side_affected_crsr
     INTO @store_no,
          @register_no,
          @transaction_date,
          @prior_audit_status,
    ...
    --Note, this proc downgrades status to 100 if necessary
  ...
  CLOSE side_affected_crsr;
  DEALLOCATE side_affected_crsr;
  SELECT @cursor_open = 0;
  
  IF @process_no IN (1,2,4,5,103) --Edit  
  BEGIN
        SELECT @errmsg='Failed to open edit_side_lock_str_date_crsr cursor.',
               @object_name = 'edit_side_lock_str_date_crsr',
               @operation_name = 'OPEN';
    DECLARE edit_side_lock_str_date_crsr CURSOR FAST_FORWARD
    FOR
    ...
    WHERE rec_process_id = @rec_process_id
      ...
      AND (prior_audit_status < 301 OR prior_audit_status > 500);

    OPEN edit_side_lock_str_date_crsr;
    ...
    WHILE 3 = 3
    BEGIN
      FETCH edit_side_lock_str_date_crsr
       INTO @edit_store_no,
    ...
    CLOSE edit_side_lock_str_date_crsr;
    DEALLOCATE edit_side_lock_str_date_crsr;
    SELECT @cursor_open = 0;
...
BEGIN
  ...
  /* Request any subledger rebuilds required by side-affected store/dates which were already complete */
  ...
  BEGIN
   ...
   store_no,
    ...
    COMMIT;
  END; -- If @rows > 0 i.e. subledger rebuild required for side-affected store/dates which were already complete

  BEGIN TRANSACTION;

  /* Unlock the side-affected store/dates that were only locked for media-rec purposes */
