# dbo.import_taxability_by_style_$sp
...
```mermaid
flowchart LR
    SP["dbo.import_taxability_by_style_$sp"]
    ORG_CHN(["ORG_CHN"]) --> SP
    common_error_handling__sp(["common_error_handling_$sp"]) --> SP
    import_taxability_by_style(["import_taxability_by_style"]) --> SP
...
```sql
create proc dbo.import_taxability_by_style_$sp 

AS

/* 
Description: This program posts updates to taxability exceptions by style
		received from a client or 3rd party to the AW taxability_by_style
...
DECLARE
  @item_datatype 	varchar(255),
  @errmsg		varchar(255),
  @errno 		int,
  @sales_date 		smalldatetime,
  @entry_type 		char,
  @store_no 		int,
  @tax_jurisdiction	char(5),
  @tax_level 		tinyint,
  @style_reference_id 	numeric(12,0),
...
SELECT @process_name = 'import_taxability_by_style_$sp',
       @message_id = 201068,
  ...
  UPDATE import_taxability_by_style
  ...
  IF @errno != 0
    BEGIN
      SELECT @errmsg = 'Failed to set effective_from_date.',
	     @object_name = 'import_taxability_by_style',
  ...
  IF @item_datatype = 'varchar'
    BEGIN
      UPDATE import_taxability_by_style
         SET style_reference_id = t.style_reference_id 
        FROM import_taxability_by_style bcp, tax_item t
       WHERE bcp.item_id = t.item_id 

      SELECT @errno = @@error
      IF @errno != 0
        BEGIN
          SELECT @errmsg = 'Failed to set style_reference_id in import_taxability_by_style (1)',
		 @object_name = 'import_taxability_by_style',
		 @operation_name = 'UPDATE'
          GOTO error
        END
  ...
  ELSE
    BEGIN
      ...
      IF @errno != 0
        BEGIN
          SELECT @errmsg = 'Failed to set item_no in import_taxability_by_style',
		 @object_name = 'import_taxability_by_style',
		 @operation_name = 'UPDATE'
          GOTO error
        END
      ...
      IF @errno != 0
        BEGIN
          SELECT @errmsg = 'Failed to set style_reference_id in import_taxability_by_style (2)',
		 @object_name = 'import_taxability_by_style',
		 @operation_name = 'UPDATE'
          GOTO error
        END
  ...
  UPDATE import_taxability_by_style
    ...
    FROM import_taxability_by_style bcp, ORG_CHN s 
  ...
  IF @errno != 0
    BEGIN
      SELECT @errmsg = 'Failed to set tax-jurisdiction for import_taxability_by_style rows of subtype S',
	     @object_name = 'import_taxability_by_style',
	     @operation_name = 'UPDATE'
      GOTO error
    END
  ...
  IF EXISTS(SELECT entry_type
              FROM import_taxability_by_style
             WHERE UPPER(entry_type) = 'R')
    TRUNCATE TABLE taxability_by_style

  SELECT @errno = @@error
  IF @errno != 0
    BEGIN
      SELECT @errmsg = 'Failed to truncate taxability_by_style table in preparation for replacement',
	     @object_name = 'taxability_by_style',
	     @operation_name = 'TRUNCATE'
      GOTO error
    END

  DECLARE tax_style_crsr CURSOR
  FOR
  SELECT entry_type,
         store_no,
         tax_jurisdiction,
...
ORDER BY effective_from_date   
  ...
  IF @errno != 0
    BEGIN
      SELECT @errmsg = 'Failed to open cursor tax_style_crsr.',
	     @object_name = 'tax_style_crsr',
	     @operation_name = 'OPEN'
      GOTO error
    END
  ...
  IF UPPER(@entry_type) NOT IN ('I', 'R', 'D', 'U')
    BEGIN
      SELECT @errmsg = 'Invalid entry_type in tax_style data file. Please verify the |1 table.',
	     @errno = 201066,
	     @message_id2 = 201066,
	     @object_name = 'tax_style_crsr',
	     @memo1 = 'tax_style_crsr',
	     @operation_name = 'SELECT'
  ...
  IF UPPER(@entry_type) = 'D'
    BEGIN
      ...
      IF @rows > 0 --i.e. row(s) to be deleted exist
      BEGIN
        ...
        IF @max_effective_from_date IS NOT NULL AND @rows = 1  --Note: @rows = 2 means all effective dates have already been deleted
        BEGIN
          UPDATE taxability_by_style
  ...
  IF UPPER(@entry_type) IN ('I', 'U', 'R')
    BEGIN
      ...
      IF @errno != 0
        BEGIN
          SELECT @errmsg = 'Failed to UPDATE taxability_by_style from import_taxability_by_style',
		 @object_name = 'taxability_by_style',
		 @operation_name = 'UPDATE'
          GOTO error
        END
          ...
          IF @errno != 0
            BEGIN
              SELECT @errmsg = 'Failed to INSERT newly imported exceptions for store_no = ' +
				CONVERT(VARCHAR, @store_no) +', tax_jurisdiciton = ' + @tax_jurisdiction +
				', style_reference_id = ' + CONVERT(VARCHAR, @style_reference_id) + ', tax_level = ' + CONVERT(VARCHAR, @tax_level)+
				', tax_rate_code = ' + CONVERT(VARCHAR,@tax_rate_code) + ', upc_lookup_division = ' + CONVERT(VARCHAR,@upc_lookup_division) + ', effective_from_date = '
				+ CONVERT(VARCHAR(11), @effective_from_date)+' into the taxability_by_style table ',
		     @object_name = 'taxability_by_style',
...
CLOSE tax_style_crsr
SELECT @errno = @@error
IF @errno != 0
  BEGIN
    SELECT @errmsg = 'Failed to CLOSE cursor tax_style_crsr.',
	   @object_name = 'tax_style_crsr',
