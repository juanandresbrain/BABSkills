# dbo.transl_stock_control_vendor_$sp
...
```mermaid
flowchart LR
    SP["dbo.transl_stock_control_vendor_$sp"]
    ORG_CHN(["ORG_CHN"]) --> SP
...
```sql
create proc dbo.transl_stock_control_vendor_$sp 
...
/* 
PROC NAME: transl_stock_control_vendor_$sp
     DESC: This proc will try to replace any vendor_no of '-3' with alpha_code found in code_description with a code_type 
...
SELECT 
       @process_name = 'transl_stock_control_vendor_$sp',
       @message_id   = 201068,       
       @process_id = @@spid,
       @fixed_count = 0,
       @issue_count = 0,
...
-- Find out how many rows exist with vendor_no = '-3'
SELECT @issue_count = count(*)
  FROM transl_stock_control_detail
 WHERE vendor_no = '-3'
   AND lookup_pos_code_vendor IS NOT NULL   
...
AND NOT EXISTS (SELECT 1 FROM transl_stock_control_detail
                 WHERE pos_description_vendor IS NOT NULL)  
  RETURN
...
UPDATE transl_stock_control_detail
   SET vendor_no = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
  FROM transl_stock_control_detail t, stock_control_display_def d,  code_description c
 WHERE t.vendor_no = '-3'
   AND t.lookup_pos_code_vendor IS NOT NULL   
   AND t.lookup_pos_code_vendor = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
   AND t.display_def_id = d.display_def_id
   AND d.vendor_no_code_type <> 5
   AND c.code_type = d.vendor_no_code_type

SELECT @errno = @@error,
       @fixed_count = @fixed_count + @@rowcount
IF @errno != 0
  BEGIN
    SELECT @errmsg = 'Failed to set the vendor_no.',
           @object_name = 'transl_stock_control_detail',
  ...
  BEGIN
    ...
    SELECT @request_id,
     ...
     WHERE t.vendor_no <> '-3'
       ...
       AND d.vendor_no_code_type <> 5
       AND c.code_type = d.vendor_no_code_type
       AND t.vendor_no = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
       AND c.resource_id = l.resource_id
       AND t.pos_description_vendor IS NOT NULL
       AND (t.pos_description_vendor <> l.system_description OR l.system_description IS NULL)
    ...
    SELECT @request_id,
           'code_description',
           d.vendor_no_code_type,
           t.lookup_pos_code_vendor,
           t.pos_description_vendor,
           ISNULL(o.PRMRY_LANG_ID, @base_language_id),
           NULL,
           1,
      ...
      FROM transl_stock_control_detail t
           ...
           LEFT JOIN ORG_CHN o ON (store_no = ORG_CHN_NUM)
     WHERE t.vendor_no = '-3'
       AND t.lookup_pos_code_vendor IS NOT NULL
       AND d.vendor_no_code_type <> 5
     GROUP BY d.vendor_no_code_type,
           t.lookup_pos_code_vendor,
           t.pos_description_vendor,
           ISNULL(o.PRMRY_LANG_ID, @base_language_id)
  ...
  BEGIN
    ...
    SELECT @request_id, 
      ...
      WHERE t.vendor_no <> '-3'
        ...
        AND d.vendor_no_code_type <> 5
        AND t.vendor_no = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
        AND c.code_type = vendor_no_code_type
        AND t.pos_description_vendor IS NOT NULL       
        AND (t.pos_description_vendor <> c.code_system_descr OR c.code_system_descr IS NULL)
      GROUP BY c.code_type,
           c.code,
           t.pos_description_vendor,
           c.resource_id
