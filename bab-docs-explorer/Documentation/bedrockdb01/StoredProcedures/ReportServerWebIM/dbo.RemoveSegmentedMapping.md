...
begin
	...
	where ChunkSegmentMapping.ChunkId in (
		...
		and not exists (
			select 1 from [ReportServerWebIMTempDB].dbo.SegmentedChunk TSC
	...
	from [ReportServerWebIMTempDB].dbo.SegmentedChunk with (readpast)
	where [ReportServerWebIMTempDB].dbo.SegmentedChunk.Machine = @MachineName
	and not exists (
		select 1 from [ReportServerWebIMTempDB].dbo.SnapshotData SD with (nolock)
		where [ReportServerWebIMTempDB].dbo.SegmentedChunk.SnapshotDataId = SD.SnapshotDataID
		) ;
		
	delete from [ReportServerWebIMTempDB].dbo.SegmentedChunk with (readpast)
	where [ReportServerWebIMTempDB].dbo.SegmentedChunk.SnapshotDataId in (
		select td.SnapshotDataId from @toDeleteTempChunks td
		where not exists (
			select 1 from [ReportServerWebIMTempDB].dbo.SnapshotData SD
			where td.SnapshotDataId = SD.SnapshotDataID
			)) ;
	...
	from [ReportServerWebIMTempDB].dbo.ChunkSegmentMapping with (readpast)	
	where not exists (
		select 1 from [ReportServerWebIMTempDB].dbo.SegmentedChunk SC with (nolock)
		where SC.ChunkId = [ReportServerWebIMTempDB].dbo.ChunkSegmentMapping.ChunkId
		) ;
		
	delete from [ReportServerWebIMTempDB].dbo.ChunkSegmentMapping with (readpast)
	output deleted.ChunkId, convert(bit, 0) into @deleted
	where [ReportServerWebIMTempDB].dbo.ChunkSegmentMapping.ChunkId in (
		select td.ChunkId from @toDeleteTempMappings td
		where not exists (
			select 1 from [ReportServerWebIMTempDB].dbo.SegmentedChunk SC
