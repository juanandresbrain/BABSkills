...
FROM [ReportServerWebIMTempDB].dbo.SessionData WITH (XLOCK) 
...
BEGIN -- Yes, update it
   ...
   UPDATE
      [ReportServerWebIMTempDB].dbo.SessionData
   SET
   ...
   END ELSE BEGIN
      UPDATE
         [ReportServerWebIMTempDB].dbo.SnapshotData
      SET
         ExpirationDate = DATEADD(n, @SnapshotTimeoutSeconds, @now)
      WHERE
         SnapshotDataID = @SnapshotDataID
...
BEGIN -- no, insert it
   UPDATE PS
    ...
    INSERT INTO [ReportServerWebIMTempDB].dbo.SessionData
