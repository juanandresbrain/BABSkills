...
begin
  ...
  from [ReportServerWebIMTempDB].dbo.SessionLock sl with(rowlock, readpast)
  join [ReportServerWebIMTempDB].dbo.SessionData s with(readpast) on sl.SessionID = s.SessionID
  ...
  delete top(20) l
  from [ReportServerWebIMTempDB].dbo.SessionData l
...
begin
  ...
  from [ReportServerWebIMTempDB].dbo.SessionData s with(readpast)
  left join [ReportServerWebIMTempDB].dbo.SessionLock sl with(nolock) on sl.SessionID = s.SessionID
...
delete top(20) ps
from [ReportServerWebIMTempDB].dbo.PersistedStream as ps
