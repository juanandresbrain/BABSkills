...
flowchart LR
    ...
    CheckSessionLock(["CheckSessionLock"]) --> SP
...
FROM
    [ReportServerWebIMTempDB].dbo.SessionData AS SE WITH (XLOCK)
WHERE
...
FROM
    [ReportServerWebIMTempDB].dbo.SessionData AS SE
    INNER JOIN [ReportServerWebIMTempDB].dbo.SnapshotData AS SN ON SN.SnapshotDataID = SE.SnapshotDataID  
    LEFT OUTER JOIN [ReportServerWebIMTempDB].dbo.SnapshotData AS COMP ON SE.CompiledDefinition = COMP.SnapshotDataID      
WHERE
   SE.SessionID = @DBSessionID
