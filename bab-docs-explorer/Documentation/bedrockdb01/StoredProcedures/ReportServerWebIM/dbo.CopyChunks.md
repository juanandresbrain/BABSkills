...
BEGIN
	...
	ELSE BEGIN
		-- copy non-segmented chunks
		INSERT [ReportServerWebIMTempDB].dbo.[ChunkData] (
			ChunkId, 
			SnapshotDataId, 
			ChunkFlags, 
			ChunkName, 
		...
		SELECT 
			...
			[c].[MimeType], 
			[c].[Content]
		FROM [ReportServerWebIMTempDB].dbo.[ChunkData] [c] WHERE [c].[SnapshotDataId] = @OldSnapshotId
				
		-- copy segmented chunks... real easy just add the mapping
		INSERT [ReportServerWebIMTempDB].[dbo].[SegmentedChunk](
			ChunkId, 
			SnapshotDataId, 
			ChunkName, 
			ChunkType,
		...
		SELECT 
			...
			ChunkFlags, 
			Machine
		FROM [ReportServerWebIMTempDB].dbo.[SegmentedChunk] WITH (INDEX (UNIQ_SnapshotChunkMapping))
		WHERE [SnapshotDataId] = @OldSnapshotId
	END
END
```
