...
AS 
    ...
    begin		
        ...
        and not exists (
            select 1 from [ReportServerWebIMTempDB].dbo.SegmentedChunk TSC with(nolock)
    ...
    begin transaction	
    DELETE TOP (@TemporarySnapshotCount) [ReportServerWebIMTempDB].dbo.SnapshotData 
    output deleted.SnapshotDataID into @cleanedSnapshots(SnapshotDataId)
    FROM [ReportServerWebIMTempDB].dbo.SnapshotData with(readpast) 
    WHERE   [ReportServerWebIMTempDB].dbo.SnapshotData.PermanentRefCount <= 0 AND
            [ReportServerWebIMTempDB].dbo.SnapshotData.TransientRefCount <= 0 AND
            [ReportServerWebIMTempDB].dbo.SnapshotData.Machine = @Machine ;
    SET @SnapshotsCleaned = @SnapshotsCleaned + @@ROWCOUNT ;
    
    DELETE [ReportServerWebIMTempDB].dbo.ChunkData FROM [ReportServerWebIMTempDB].dbo.ChunkData 
	INNER JOIN @cleanedSnapshots cs
    ON [ReportServerWebIMTempDB].dbo.ChunkData.SnapshotDataID = cs.SnapshotDataId
    ...
    commit
   	...
   	begin		
		delete SC 
		output deleted.ChunkId into @cleanedChunks(ChunkId)
		from [ReportServerWebIMTempDB].dbo.SegmentedChunk SC with (readpast)	
		join @cleanedSnapshots cs on SC.SnapshotDataId = cs.SnapshotDataId ;	
		set @deleteCount = @@ROWCOUNT ; 
		set @ChunksCleaned =  @ChunksCleaned + @deleteCount ;
	end ;
	...
	begin		
		...
		output deleted.ChunkId, deleted.SegmentId into @cleanedSegments (ChunkId, SegmentId)
		from [ReportServerWebIMTempDB].dbo.ChunkSegmentMapping CSM with (readpast)
	...
	begin
		...
		from [ReportServerWebIMTempDB].dbo.Segment S with (readpast)
		join @cleanedSegments cs on S.SegmentId = cs.SegmentId
		where not exists (
