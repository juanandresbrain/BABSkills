...
```sql
create proc dbo.Ex_OutputStat_Work 

@ExecutionID int, @SequenceNo int, @OutputType int, @RecordCount int, 
 @ReturnCode int, @FileSize int, @FileName varchar(255), @WorkDateTime varchar(30) 
...
DECLARE @errno int,
	...
	INSERT INTO Ex_OutputStat 
		   (execution_id, sequence_no, output_type, record_count, 
	  	    work_file_name, work_file_size, work_return_code, work_date_time)
	VALUES (@ExecutionID, @SequenceNo, @OutputType, @RecordCount, 
			@FileName, @FileSize, @ReturnCode, @WorkDateTime)
...
SELECT @errmsg = 'Ex_OutputStat_Work ' + @errmsg 
if @errno < 100000 
