...
IF @FromIsPermanent != 0 AND @ToIsPermanent = 0 BEGIN
    ...
    INSERT INTO [ReportServerSATempDB].dbo.ChunkData
    ...
    NOT EXISTS(
        SELECT T.ChunkName
        FROM [ReportServerSATempDB].dbo.ChunkData AS T -- exclude the ones in the target
        WHERE
	...
	select @Machine = Machine from [ReportServerSATempDB].dbo.SnapshotData SD where SD.SnapshotDataID = @ToSnapshotID
		
	INSERT INTO [ReportServerSATempDB].dbo.SegmentedChunk
	...
	WHERE 
		...
		NOT EXISTS(
			...
			FROM [ReportServerSATempDB].dbo.SegmentedChunk TSC
			-- JOIN [ReportServerSATempDB].dbo.SnapshotChunkMapping TSCM ON (TSC.ChunkId = TSCM.ChunkId)
			WHERE 
 ...
 END ELSE IF @FromIsPermanent = 0 AND @ToIsPermanent = 0 BEGIN	
	...
	select @Machine = Machine from [ReportServerSATempDB].dbo.SnapshotData SD where SD.SnapshotDataID = @FromSnapshotID
    ...
    SELECT
        newid(), @ToSnapshotID, COALESCE(@TargetChunkName, S.ChunkName), S.ChunkType, S.MimeType, S.Version, S.ChunkFlags, S.Content
    FROM
        [ReportServerSATempDB].dbo.ChunkData AS S
    WHERE   
        S.SnapshotDataID = @FromSnapshotID AND
        (S.ChunkType = @ChunkType OR @ChunkType IS NULL) AND
        (S.ChunkName = @ChunkName OR @ChunkName IS NULL) AND
	...
	FROM [ReportServerSATempDB].dbo.SegmentedChunk C WITH(INDEX (UNIQ_SnapshotChunkMapping))
	WHERE	C.SnapshotDataId = @FromSnapshotID AND
			...
			NOT EXISTS(
				...
				SELECT T.ChunkId
				FROM [ReportServerSATempDB].dbo.SegmentedChunk T
...
END ELSE IF @FromIsPermanent = 0 AND @ToIsPermanent != 0 BEGIN
    ...
    WHERE   
     ...
     FROM [ReportServerSATempDB].dbo.ChunkSegmentMapping CM 
     ...
     FROM [ReportServerSATempDB].dbo.Segment CS
