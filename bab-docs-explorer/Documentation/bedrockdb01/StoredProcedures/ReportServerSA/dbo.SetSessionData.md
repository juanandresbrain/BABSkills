...
FROM [ReportServerSATempDB].dbo.SessionData WITH (XLOCK) 
...
BEGIN -- Yes, update it
   ...
   UPDATE
      [ReportServerSATempDB].dbo.SessionData
   SET
   ...
   END ELSE BEGIN
      UPDATE
         [ReportServerSATempDB].dbo.SnapshotData
      SET
         ExpirationDate = DATEADD(n, @SnapshotTimeoutSeconds, @now)
      WHERE
         SnapshotDataID = @SnapshotDataID
...
BEGIN -- no, insert it
   UPDATE PS
    SET PS.RefCount = 1
    FROM
        [ReportServerSATempDB].dbo.PersistedStream as PS
    WHERE
        PS.SessionID = @SessionID	
        
    INSERT INTO [ReportServerSATempDB].dbo.SessionData
