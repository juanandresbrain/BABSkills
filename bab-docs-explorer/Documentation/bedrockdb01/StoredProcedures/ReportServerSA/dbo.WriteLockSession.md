...
BEGIN
	...
	BEGIN
		UPDATE [ReportServerSATempDB].dbo.SessionLock WITH (ROWLOCK)
	...
	BEGIN
		...
		BEGIN 
			...
			FROM [ReportServerSATempDB].dbo.SessionLock WITH (ROWLOCK)
			WHERE SessionID = @SessionID;
...
ELSE
BEGIN
	INSERT INTO [ReportServerSATempDB].dbo.SessionLock WITH (ROWLOCK) (SessionID) VALUES (@SessionID)
END
```
