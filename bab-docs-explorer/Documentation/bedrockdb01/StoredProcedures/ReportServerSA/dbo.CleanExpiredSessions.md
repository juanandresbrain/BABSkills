...
begin
  ...
  from [ReportServerSATempDB].dbo.SessionLock sl with(rowlock, readpast)
  join [ReportServerSATempDB].dbo.SessionData s with(readpast) on sl.SessionID = s.SessionID
  ...
  delete top(20) l
  from [ReportServerSATempDB].dbo.SessionData l
...
begin
  ...
  from [ReportServerSATempDB].dbo.SessionData s with(readpast)
  left join [ReportServerSATempDB].dbo.SessionLock sl with(nolock) on sl.SessionID = s.SessionID
...
delete top(20) ps
from [ReportServerSATempDB].dbo.PersistedStream as ps
