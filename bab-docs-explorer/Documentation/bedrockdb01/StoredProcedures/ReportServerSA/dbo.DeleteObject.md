...
WHERE    
    (R.Path = @Path OR R.Path LIKE @Prefix ESCAPE '*')
        ...
        UPDATE [ReportServerSATempDB].dbo.TempDataSources
           SET
              Flags = Flags & 0x7FFFFFFD, -- broken link
              Link = NULL
        FROM
           [Catalog] AS C
           INNER JOIN [ReportServerSATempDB].dbo.TempDataSources AS DS ON C.[ItemID] = DS.Link
        WHERE
           (C.Path = @Path OR C.Path LIKE @Prefix ESCAPE '*')
...
UPDATE [ReportServerSATempDB].dbo.TempDataSets
   SET
      [LinkID] = NULL
FROM
   [Catalog] AS C
   INNER JOIN [ReportServerSATempDB].dbo.TempDataSets AS DS ON C.[ItemID] = DS.[LinkID]
WHERE
   (C.Path = @Path OR C.Path LIKE @Prefix ESCAPE '*')
...
FROM
   ...
   INNER JOIN [ReportServerSATempDB].dbo.ExecutionCache AS EC on SN.SnapshotDataID = EC.SnapshotDataID
...
BEGIN
        ...
        SET PermanentRefcount = PermanentRefcount - 1
        FROM
           [ReportServerSATempDB].dbo.TempCatalog AS R WITH (XLOCK)
           INNER JOIN [ReportServerSATempDB].dbo.SnapshotData AS SD ON R.Intermediate = SD.SnapshotDataID
        WHERE
           R.ContextPath = @Path
        ...
        -- Clean all data sources
        DELETE [ReportServerSATempDB].dbo.TempDataSources
        FROM
            [ReportServerSATempDB].dbo.TempCatalog AS R
            INNER JOIN [ReportServerSATempDB].dbo.TempDataSources AS DS ON R.TempCatalogID = DS.ItemID
        WHERE    
            R.ContextPath = @Path
            AND R.EditSessionID = @EditSessionID
            AND R.OwnerID = @OwnerID
        ...
        DELETE [ReportServerSATempDB].dbo.TempDataSets
        FROM
            [ReportServerSATempDB].dbo.TempCatalog AS R
            INNER JOIN [ReportServerSATempDB].dbo.TempDataSets AS DS ON R.TempCatalogID = DS.ItemID
        WHERE    
            R.ContextPath = @Path
            AND R.EditSessionID = @EditSessionID
            AND R.OwnerID = @OwnerID
        ...
        FROM
           [ReportServerSATempDB].dbo.SnapshotData AS SN
           INNER JOIN [ReportServerSATempDB].dbo.ExecutionCache AS EC on SN.SnapshotDataID = EC.SnapshotDataID
           INNER JOIN [ReportServerSATempDB].dbo.TempCatalog AS C ON EC.ReportID = C.TempCatalogID
        WHERE
           ContextPath = @Path
        ...
        DELETE
        FROM
           [ReportServerSATempDB].dbo.TempCatalog
        WHERE
            ContextPath = @Path
