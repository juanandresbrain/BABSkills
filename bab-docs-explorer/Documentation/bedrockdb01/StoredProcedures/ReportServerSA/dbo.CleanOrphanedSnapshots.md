...
AS 
    ...
    begin		
        ...
        and not exists (
            select 1 from [ReportServerSATempDB].dbo.SegmentedChunk TSC with(nolock)
    ...
    begin transaction	
    DELETE TOP (@TemporarySnapshotCount) [ReportServerSATempDB].dbo.SnapshotData 
    output deleted.SnapshotDataID into @cleanedSnapshots(SnapshotDataId)
    FROM [ReportServerSATempDB].dbo.SnapshotData with(readpast) 
    WHERE   [ReportServerSATempDB].dbo.SnapshotData.PermanentRefCount <= 0 AND
            [ReportServerSATempDB].dbo.SnapshotData.TransientRefCount <= 0 AND
            [ReportServerSATempDB].dbo.SnapshotData.Machine = @Machine ;
    SET @SnapshotsCleaned = @SnapshotsCleaned + @@ROWCOUNT ;
    
    DELETE [ReportServerSATempDB].dbo.ChunkData FROM [ReportServerSATempDB].dbo.ChunkData 
	INNER JOIN @cleanedSnapshots cs
    ON [ReportServerSATempDB].dbo.ChunkData.SnapshotDataID = cs.SnapshotDataId
    ...
    commit
   	...
   	begin		
		delete SC 
		output deleted.ChunkId into @cleanedChunks(ChunkId)
		from [ReportServerSATempDB].dbo.SegmentedChunk SC with (readpast)	
		join @cleanedSnapshots cs on SC.SnapshotDataId = cs.SnapshotDataId ;	
		set @deleteCount = @@ROWCOUNT ; 
		set @ChunksCleaned =  @ChunksCleaned + @deleteCount ;
	end ;
	...
	begin		
		...
		output deleted.ChunkId, deleted.SegmentId into @cleanedSegments (ChunkId, SegmentId)
		from [ReportServerSATempDB].dbo.ChunkSegmentMapping CSM with (readpast)
	...
	begin
		...
		from [ReportServerSATempDB].dbo.Segment S with (readpast)
		join @cleanedSegments cs on S.SegmentId = cs.SegmentId
		where not exists (
			select 1 from [ReportServerSATempDB].dbo.ChunkSegmentMapping csm
			where csm.SegmentId = cs.SegmentId ) ;
		set @deleteCount = @@ROWCOUNT ;
		set @SegmentsCleaned = @SegmentsCleaned + @deleteCount ;
	end
