...
begin
	...
	where ChunkSegmentMapping.ChunkId in (
		...
		and not exists (
			select 1 from [ReportServerSATempDB].dbo.SegmentedChunk TSC
	...
	from [ReportServerSATempDB].dbo.SegmentedChunk with (readpast)
	where [ReportServerSATempDB].dbo.SegmentedChunk.Machine = @MachineName
	and not exists (
		select 1 from [ReportServerSATempDB].dbo.SnapshotData SD with (nolock)
		where [ReportServerSATempDB].dbo.SegmentedChunk.SnapshotDataId = SD.SnapshotDataID
		) ;
		
	delete from [ReportServerSATempDB].dbo.SegmentedChunk with (readpast)
	where [ReportServerSATempDB].dbo.SegmentedChunk.SnapshotDataId in (
		select td.SnapshotDataId from @toDeleteTempChunks td
		where not exists (
			select 1 from [ReportServerSATempDB].dbo.SnapshotData SD
			where td.SnapshotDataId = SD.SnapshotDataID
			)) ;
	...
	from [ReportServerSATempDB].dbo.ChunkSegmentMapping with (readpast)	
	where not exists (
		select 1 from [ReportServerSATempDB].dbo.SegmentedChunk SC with (nolock)
		where SC.ChunkId = [ReportServerSATempDB].dbo.ChunkSegmentMapping.ChunkId
		) ;
		
	delete from [ReportServerSATempDB].dbo.ChunkSegmentMapping with (readpast)
	output deleted.ChunkId, convert(bit, 0) into @deleted
	where [ReportServerSATempDB].dbo.ChunkSegmentMapping.ChunkId in (
		select td.ChunkId from @toDeleteTempMappings td
		where not exists (
			select 1 from [ReportServerSATempDB].dbo.SegmentedChunk SC
