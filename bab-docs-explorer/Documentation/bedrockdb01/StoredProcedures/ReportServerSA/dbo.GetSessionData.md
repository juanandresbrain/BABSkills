...
FROM
    [ReportServerSATempDB].dbo.SessionData AS SE WITH (XLOCK)
WHERE
...
FROM
    [ReportServerSATempDB].dbo.SessionData AS SE
    INNER JOIN [ReportServerSATempDB].dbo.SnapshotData AS SN ON SN.SnapshotDataID = SE.SnapshotDataID  
    LEFT OUTER JOIN [ReportServerSATempDB].dbo.SnapshotData AS COMP ON SE.CompiledDefinition = COMP.SnapshotDataID      
WHERE
   SE.SessionID = @DBSessionID
