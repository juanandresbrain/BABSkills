...
```sql
create proc dbo.mew_sales_export_$sp 
...
Desc: Standard interface to NSB Merchandising. Replaces SVExport MeW Export.
  ...
  Can use same version for SA5.0 and SA5.1
...
CREATE TABLE #mew_extract(
	...
	sold_at_price numeric(12,4) NULL,
    ...
    BEGIN
     -- extract the merch data for Merch 4.0 and less
      ...
      SELECT w.serial_no,
	     ...
	     h.transaction_date,
	     h.transaction_no,
	     CASE WHEN l.line_action = 99 THEN s.EXTRNL_RFRNC_NUM ELSE COALESCE(f.EXTRNL_RFRNC_NUM, s.EXTRNL_RFRNC_NUM) END location_id,
	     h.register_no,
	     l.reference_no,
