...
AND (ir.if_reject_reason <> 2  
     OR (ir.if_reject_reason = 2  --bad card
         AND (  --card will be validated by this proce
              ...
              OR --there is no longer any card to be validated in the transaction (for example the tender was changed to cash)
                 NOT EXISTS (SELECT 1 FROM transaction_line l
                              WHERE l.transaction_id = @transaction_id
                                AND ir.line_id = l.line_id
                                AND l.reference_type IN (SELECT r.reference_type
                                                           FROM reference_type_validation r
