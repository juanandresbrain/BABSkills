...
BEGIN
  ...
  SELECT 'Store/Dates Processed : ' + convert(nvarchar,@session_store_count) + 
         '  Transactions Processed : ' + convert(nvarchar,@session_tran_count)
  SELECT 'Transactions/hour : ' + convert(nchar(10), @session_per_hour) 
  ...
  BEGIN
    ...
    BEGIN
     ...
     SELECT SUBSTRING((@process_desc + @blanks),1,32) + ' ' + 
                 convert(nchar(6),@process_duration) + ' Seconds ' + 
                 convert(nchar(6), convert(decimal(5,2),(@process_duration/(0.0000001+@stream_duration)*100)) ) + ' % ' + 
                 convert(nvarchar,@start_time,8) + '  ' + 
                 convert(nvarchar,@end_time,8)

        --Stats for the purging functions
        IF @process_no IN (16,41,29)
          SELECT SUBSTRING((@process_desc + @blanks),1,32) + ' ' + 
                 convert(nchar(6),@process_duration) + ' Seconds.  ' + 
                 convert(nchar(7),@transaction_count) + '  ' + 
                 convert(nvarchar,@start_time,8) + '  ' + 
                 convert(nvarchar,@end_time,8) + ' ' + 
                 convert(nchar(7),convert(decimal(7,0),(@transaction_count/(0.0000001+@process_duration))*60*60)) + '/hour'

        IF @process_no IN (18) 
          SELECT SUBSTRING((@process_desc + @blanks),1,32) + ' ' + 
                 convert(nchar(6),@stream_duration) + ' Seconds.  ' + 
                 convert(nvarchar,@start_time,8) + '  ' +
                 convert(nvarchar,@end_time,8)
...
SELECT 'Overall Dayend (transactions/hour) : ' + convert(nchar(7),@overall)
