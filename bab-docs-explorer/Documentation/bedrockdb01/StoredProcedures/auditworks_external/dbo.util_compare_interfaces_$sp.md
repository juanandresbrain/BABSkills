...
SELECT @process_name = 'util_compare_interfaces_$sp',
       @process_no = 36,
       @message_id = 201068,
       @to_interface_posting_date = convert(datetime, '12/31/' + convert(nvarchar,datepart(yy, getdate()))),
...
UPDATE util_comparison_key
   SET comparison_key = convert(nvarchar, h.store_no) + ' _ ' + convert(nvarchar, h.register_no) + ' _ ' + 
...
SELECT @process_id, @comparison_id, 'if_transaction_header', 'Header Data', 
	 ...
	 pos_tax_jurisdiction + ' _ ' + convert(nvarchar, employee_no) + ' _ ' + 
	 convert(nvarchar, source_process_no) + ' _ ' + convert(nvarchar,sign(transaction_id)),
       transaction_remark,
...
SELECT @process_id, @comparison_id, 'if_interface_control', 'Interface Control', 
       ...
       convert(nvarchar, interface_control_flag) + ' _ ' + convert(nvarchar, effective_date),
       null,
       null  
