# dbo.edit_error_recovery_44_$sp
...
```mermaid
flowchart LR
    SP["dbo.edit_error_recovery_44_$sp"]
...
```sql
create proc [dbo].[edit_error_recovery_44_$sp] 
...
SELECT @violated_sareject_rule = 44,
	@translate_msg = 'Duplicate lines from the translate were skipped by the edit for special orders',
	@process_name = 'edit_error_recovery_44_$sp',
        @message_id = 201068
...
SELECT
	...
	COUNT(line_id),
	MIN(row_sequence_no)
  FROM transl_special_order_detail WITH (NOLOCK)
...
WHILE 1=1
  ...
  IF @errno != 0
    BEGIN
     SELECT @errmsg = 'Failed to delete duplicate rows from transl_special_order_detail',
