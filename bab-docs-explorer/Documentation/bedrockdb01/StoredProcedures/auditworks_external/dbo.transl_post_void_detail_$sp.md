# dbo.transl_post_void_detail_$sp
...
```mermaid
flowchart LR
    SP["dbo.transl_post_void_detail_$sp"]
    ORG_CHN(["ORG_CHN"]) --> SP
...
```sql
create proc dbo.transl_post_void_detail_$sp @process_id		binary(16),
@process_no		smallint,
@lookup_pass		tinyint,
...
/* 
PROC NAME: transl_post_void_detail_$sp
     DESC: This proc will try to replace any post_void_reason_code of -3 with code found in code_description 
           for code_type 27. If code is not found, a new one will be created by transl_auto_configure_$sp.
...
May02,05  David      DV-1202 Return if no rows to configure and no description change.
...
SELECT @process_name = 'transl_post_void_detail_$sp',
       @message_id   = 201068,       
       @process_id = @@spid,
       @fixed_count = 0,
       @issue_count = 0,
...
-- Find out how many rows exist with post_void_reason_code = -3
SELECT @issue_count = count(*)
  FROM transl_post_void_detail
 WHERE post_void_reason_code = -3
...
IF @issue_count = 0 
AND NOT EXISTS (SELECT 1 FROM transl_post_void_detail
...
UPDATE transl_post_void_detail
   SET post_void_reason_code = code
  FROM transl_post_void_detail t, code_description c
 WHERE t.post_void_reason_code = -3
...
IF @errno != 0
  BEGIN
    SELECT @errmsg = 'Failed to set the post_void_reason_code.',
           @object_name = 'transl_post_void_detail',
...
IF EXISTS (SELECT 1
  ...
  BEGIN
    ...
    SELECT @request_id,
      ...
      FROM transl_post_void_detail t,
           code_description c,
           ORG_CHN o,
     ...
     WHERE t.post_void_reason_code <> -3
       ...
       AND c.code_type = 27
       AND t.post_void_reason_code = c.code
    ...
    SELECT @request_id,
           ...
           0,
           MIN(convert(nvarchar, t.entry_date_time, 109) + '|' + convert(nvarchar, t.store_no)  + '|' + convert(nvarchar, t.register_no)  + '|' + t.transaction_series  + '|' +  convert(nvarchar, t.transaction_no) + '|' + convert(nvarchar, t.line_id))
      FROM transl_post_void_detail t LEFT JOIN ORG_CHN o ON (store_no = ORG_CHN_NUM)
     WHERE t.post_void_reason_code = -3
