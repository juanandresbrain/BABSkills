...
IF @rows != 0 AND @edit_timestamp < 1000
  SELECT @trickle_in_progress_flag = @prev_trickle_flag;

IF ( @rows = 0
   OR @store_audit_status IN ( 5, 7, 8, 200, 900, 901, 902, 903, 904, 905, 906))
...
IF @auto_accepted > 0 AND @store_audit_status = 300
  ...
  SELECT @store_audit_status = 100, @auto_accepted = 0, @status_reject_reason = 0, @date_reject_flag = 0;
...
BEGIN
  ...
  BEGIN
     ...
     IF @locked_by_process IN (1,4) AND @update_in_progress IN (1,4)
...
BEGIN
  ...
  SELECT @trickle_already_in_progress = ISNULL(trickle_in_progress_flag,0),
   ...
   IF @date_reject_id < 255
     AND @current_store_audit_status NOT IN (100, 7, 8, 5)  -- not edited, bad store, bad reg, missing
