...
```sql
create proc [dbo].[edit_post_$sp] @edit_process_no	tinyint = 1
...
BEGIN
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && Edit cannot execute edit_post_$sp:  the S/A database is currently being upgraded.  Please try again later. ' + CONVERT(char, getdate(), 8);
  PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_post_$sp starts : ' + CONVERT(char, getdate(), 8);
PRINT @trace_msg;
...
BEGIN
  ...
  BEGIN  
    ...
    /* check last time that edit_missing_reg_$sp was run on this server/peripheral.
       If more than 3 hours ago then run again */
    ...
    IF DATEDIFF(hh,@last_edit_date,getdate()) > 3 -- THEN
    BEGIN
      SELECT @step_no = 29;
...
BEGIN
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_cleanup_interfaces_$sp: ' + CONVERT(char, getdate(), 8);
  PRINT @trace_msg;
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_interfaces_recovery_$sp: ' + CONVERT(char, getdate(), 8);
  PRINT @trace_msg;
  ...
  UPDATE edit_store_date_list
   ...
   WHERE batch_process_no = @edit_process_no
     ...
     AND NOT EXISTS (SELECT 1 FROM edit_status WHERE edit_status = 1 AND edit_process_no = @edit_process_no AND edit_function_no IN (5, 1, 70));
...
BEGIN
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_cleanup_details_$sp: ' + CONVERT(char, getdate(), 8);
   PRINT @trace_msg;
...
BEGIN
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && Attempting rec_edit recovery: ' + CONVERT(char, getdate(), 8),
	 @rec_error_no = 1;
  PRINT @trace_msg;
  ...
  END TRY
  BEGIN CATCH
    /* Trap any error that might occur during media rec posting and log a warning message to process_error_log (stream_no is in memo1)
...
BEGIN
  ...
  BEGIN
    ...
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_post ends: ' + CONVERT(char, getdate(), 8);
    PRINT @trace_msg;

    RETURN;
  END; -- IF @edit_process_no > 1

  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_phase2 starts: ' + CONVERT(char, getdate(), 8);
  PRINT @trace_msg;
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        IF @loop_count < 2
        BEGIN
          SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && RETRYING: edit_phase2_$sp: count (' + RTRIM(CONVERT(char, @loop_count,2)) + ')  ' + RTRIM(CONVERT(char, getdate(), 8));
          PRINT @trace_msg;
        ...
        BEGIN   
		 ...
		 @operation_name = 'SELECT';
          ...
          SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG EXECWARN: Maximum number of retries exceeded. Could not process ' + RTRIM(CONVERT(char, @stores_left, 2)) + ' store(s).';
          PRINT @trace_msg;
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_phase2 ends : ' + CONVERT(char, getdate(), 8);
  PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_header: ' + CONVERT(char, getdate(), 8),
       @step_no = 1;
PRINT @trace_msg;
...
ELSE
BEGIN
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && Warning: work_tran_header.work_tran_header_edit_x1 index not available to be dropped.  ' + CONVERT(char, getdate(), 8);
  PRINT @trace_msg;
END;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_post_void: ' + CONVERT(char, getdate(), 8),
       @step_no = 2;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_return: ' + CONVERT(char, getdate(), 8),
       @step_no = 4;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_customer: ' + CONVERT(char, getdate(), 8),
       @step_no = 5;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_customer_detail: ' + CONVERT(char, getdate(), 8),
       @step_no = 6;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_special_order_detail: ' + CONVERT(char, getdate(), 8),
       @step_no = 7
PRINT @trace_msg
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_payroll: ' + CONVERT(char, getdate(), 8),
       @step_no = 9;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_transaction_line_link: ' + CONVERT(char, getdate(), 8),
       @step_no = 10;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_lines: ' + CONVERT(char, getdate(), 8),
       @step_no = 11;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_stock_control: ' + CONVERT(char, getdate(), 8),
       @step_no = 8;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_authorization: ' + CONVERT(char, getdate(), 8),
       @step_no = 12;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_discount_detail: ' + CONVERT(char, getdate(), 8),
       @step_no = 13;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_merchandise: ' + CONVERT(char, getdate(), 8),
       @step_no = 14;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_tax: ' + CONVERT(char, getdate(), 8),
       @step_no = 15;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit employee attributes: ' + CONVERT(char, getdate(), 8),
       @step_no = 72;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_lines_validation: ' + CONVERT(char, getdate(), 8),
       @step_no = 17;
PRINT @trace_msg;
...
BEGIN
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_verify_attachments: ' + CONVERT(char, getdate(), 8),
	 @step_no = 18
  PRINT @trace_msg
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && insert_header_lines: ' + CONVERT(char, getdate(), 8);
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_translate_error: ' + CONVERT(char, getdate(), 8),
       @step_no = 3;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && User-Defined IF Rejects: ' + CONVERT(char, getdate(), 8),
       @step_no = 16;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && rec_edit: ' + CONVERT(char, getdate(), 8);
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_interfaces: ' + CONVERT(char, getdate(), 8),
       @step_no = 20;
PRINT @trace_msg;
...
SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && reconciliation: ' + CONVERT(char, getdate(), 8),
       @step_no = 25;
PRINT @trace_msg;
...
BEGIN
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && glc_recovery_summary: ' + CONVERT(char, getdate(), 8),
         @step_no = 23;
  PRINT @trace_msg;
  ...
  BEGIN
    ...
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && trickle_sa_rejects: ' + CONVERT(char, getdate(), 8),
           @step_no = 27;
    PRINT @trace_msg;
    ...
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_trickle_exceptions: ' + CONVERT(char, getdate(), 8),
           @step_no = 66;
    PRINT @trace_msg;
    ...
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_trickle_unlock: ' + CONVERT(char, getdate(), 8);
    PRINT @trace_msg;
  ...
  SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_trickle_unlock ends: ' + CONVERT(char, getdate(), 8);
  PRINT @trace_msg;
...
BEGIN
  ...
  BEGIN
    ...
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_phase2 of current batch starts: ' + CONVERT(char, getdate(), 8);
    PRINT @trace_msg;
    ...
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && edit_phase2 of current batch ends: ' + CONVERT(char, getdate(), 8);
    PRINT @trace_msg;
...
BEGIN
  ...
  IF @request_type > 1
  BEGIN
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && recovery of edit_phase2 starts: ' + CONVERT(char, getdate(), 8);
    PRINT @trace_msg;
    ...
    SELECT @trace_msg = NCHAR(13) + NCHAR(10) + ':LOG && recovery of edit_phase2 ends: ' + CONVERT(char, getdate(), 8);
    PRINT @trace_msg;
  END; -- If @request_type > 1
