...
/* Proc_name: purge_queue_and_execution_$sp
   ...
   Note : interface interface partitioning is only available in SA5.1 for mssql. Non-existence of the partitioning parameter in table
...
Jun21,11 Paul    116538 If partitioning is turned on then update if_interface_status
Apr23,10 Paul    117332 When using partitioning, do not log a warning message in this proc.
Mar24,10 Paul    115834 If partitioning is turned on, then do not delete from the if* tables. Populate transaction_id in process_error_log.
Aug31,09 Paul    111900 update if_cleanup_status to handle the scaleout consolidated environment,
				handle possible timing issues
...
-- Clean up interfaces which have been turned off - Defect 8372 

SELECT @rows = @rows_per_batch
WHILE @rows = @rows_per_batch -- LOOP 2
  BEGIN
    BEGIN TRANSACTION
    UPDATE if_interface_control_deadlock
       SET function_no = @process_no,
    ...
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Failed to update if_interface_control_deadlock (2)',
             @object_name = 'if_interface_control_deadlock',
             @operation_name = 'UPDATE'
      GOTO error
    END
    ...
    IF @errno <> 0
    BEGIN
      SELECT @errmsg = 'Unable to delete from Ex_Queue (2)',
             @object_name = 'Ex_Queue',
             @operation_name = 'DELETE'
      GOTO error
    END
  ...
  END -- LOOP2 - WHILE @rows = @rows_per_batch 
...
/* Now delete the interface detail attachments (if not using partitioning).
     If partitioning is turned on, then avoid deleting tran details because partitioning maintenance
