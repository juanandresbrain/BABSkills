...
```sql
create proc dbo.transaction_modify_$sp @process_id			binary(16),
@user_id  			int, 
...
SELECT @trickle_in_progress_flag = 0,
       ...
       @process_name = 'transaction_modify_$sp',
       @message_id = 201068,
...
IF @errno != 0
BEGIN
  SELECT @errmsg = 'Failed to select @trickle_in_progress_flag',
...
-- DEF: 1-EIRG1: reset voiding_reversal_flag
  ...
  /* Check if register is currently tricking in (trickle audit) */
...
BEGIN
  ...
  /* Unconditionally set audit_status to 100 and then re-evaluate if not trickle audit */
  UPDATE audit_status
  ...
  ELSE
   BEGIN -- leave audit_status = 100 when modifying a register that is trickling in
    EXEC verify_store_status_$sp @process_id, NULL, @store_no, @transaction_date, @date_reject_id,
				@errmsg OUTPUT
  SELECT @errno = @@error
    IF @errno != 0
      BEGIN
        ...
        GOTO error
      END  
   END -- else @register_trickle_flag <> 0
