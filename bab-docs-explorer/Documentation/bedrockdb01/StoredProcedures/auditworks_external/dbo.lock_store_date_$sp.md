...
WHILE @lock_counter < @max_retries
  ...
  IF @existing_status > 0 AND @update_in_progress IN (75,76,77,78,82,140,223,224)
	BREAK; /* while */
...
BEGIN
  SELECT @errmsg = CASE WHEN @existing_status IS NULL THEN 'Store/Date has no status record' ELSE 'Store/Date has invalid status ' + convert(varchar, @existing_status) END,
