...
```sql
create proc [dbo].[get_max_serial_no_$sp]   
 @QueueID		tinyint,
 @MinSerialNo		numeric(14,0),
 @DesiredCount		int,
 @ActualMaxSerialNo	numeric(14,0) OUTPUT
...
/* 
PROC NAME: get_max_serial_no_$sp
     DESC: To determine the max serial_no for the next batch of transactions
	   based on the i_DesiredCount (desired max batch size) input parameter.
	   Used output variable instead of return variable because of arithmetic overflow.

  HISTORY:
Date     Name     Def# Desc
Oct27,09 Phu  1-43SZMZ If the last loop finds no more trans, zero is incorrectly returned.
Apr19,02 ShuZ  1-CD0IX Standardize  R3.5 Common error handling
Jan30,01 Paul     7271 copied SmartView proc from Chris C.
*/

SELECT @process_name = 'get_max_serial_no_$sp',
      @message_id = 201068
