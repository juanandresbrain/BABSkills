...
```sql
create proc dbo.move_merchandise_$sp  @process_id            binary(16),
 @user_id               int,
 @store_no              int,
...
SELECT wt.process_id,
       ...
       md.units,
       price_override,
       CONVERT (numeric(12,4), gross_line_amount / units), -- ticket_price
       CONVERT (numeric(12,4),(gross_line_amount - pos_discount_amount)/units), -- sold_at_price
       CONVERT (numeric(12,4),gross_line_amount / units), -- plu_price
...
BEGIN 
  ...
  SET plu_price = CONVERT (numeric(12,4),(mt.gross_line_amount -
...
BEGIN
  ...
  SELECT wt.process_id,
	 ...
	 pos_identifier,
	 pos_deptclass,
	 units = COALESCE(sc.units, 0),
         tl.gross_line_amount,
	 upc_lookup_division, -- def 8663 = @tinyint1_filler,
	 line_void_flag = (1 - tl.line_void_flag) * ABS(voiding_reversal_flag), /* > 0 if not void */
         line_object_type,
  ...
  BEGIN
    ...
    UPDATE work_merchandise_move
     ...
     WHERE mt.transaction_id = th.transaction_id
       ...
       AND th.transaction_category = COALESCE(la.transaction_category, th.transaction_category)
       AND attachment_type = 3   -- stock control
