...
```sql
create proc dbo.build_subledger_$sp ( @process_id				binary(16),
	...
	SELECT h.transaction_id,
	  ...
	  FROM #sub_trans_line h WITH (NOLOCK)
	       INNER JOIN discount_detail d WITH (NOLOCK)
	         ...
	         AND CONVERT(NUMERIC(12,4), d.pos_discount_amount * d.applied_flag) <> 0
  ...
  ELSE IF @rebuild_flag = 2
   ...
   IF @do_breakout = 1  -- there are tax rows to breakout
	...
	SELECT
	       ...
	       '     ',
	       @store_deposit_destination,
	       ABS(SIGN(mrd.rec_amount_type - 2)), /* discounted_line_object set to 0 for units or 1 for amount */
	       mrd.store_no,
	       ' ',
	 ...
	 lo.transaction_category,  --101672
	       ...
	       ABS(SIGN(mrd.rec_amount_type - 2)),
	       mrd.transaction_date,
	       lo.store_balance_group,
               mrd.balancing_entity_id,
               mrd.rec_side,
        ...
        IF @errno = 515 /* Trap error and identify which rows are causing the error 515 */
              BEGIN /* 515 error can occur if row not found in line_object_action_association or in media_parameter_rec_calc */
                        ...
                        SELECT @errmsg = 'Failed to open cursor for error_crsr',
      			       @object_name = 'error_crsr',
                               @operation_name = 'OPEN';
 	          ...
 	          WHERE mrd.store_no = @store_no
                ...
                SELECT @errmsg = 'Failed to close error_crsr',
                                @object_name = 'error_crsr',
	 ...
	 PRINT ':LOG EXECERR: Aborting subledger posting due to missing media rec configuration or error. Check process_error_log for details.';
          GOTO business_error;
        END;
    ...
    /* Add transfer of expected amounts in case of fund transfer and over/shortout of their original accounts */
 	    ...
 	    SELECT
	           ...
	           a.discounted_line_object, /* discounted_line_object set to 0 for units or 1 for amount */
	           mrd.store_no,
	           ' ',
	           a.transaction_date,
		   lo.store_balance_group,
		   @merchandise_year_no,
	           @merchandise_month_no,
	           @gl_company,
                   (1-a.discounted_line_object) * (mrd.rec_amount * -1 * (1-SIGN(rec_side))) + (a.units * -1 * SIGN(rec_side) * (2*abs(sign(a.class_code-25))-1) ),
                   a.discounted_line_object * (mrd.rec_amount * -1 * (1-SIGN(rec_side))) + (a.amount * -1 * SIGN(rec_side) * (2*abs(sign(a.class_code-25))-1) ),  /* i.e. post the expected amount if doing a fund transfer or post the over/short amount if just removing the over/short */
		   0,
	           0,
	           1,
	           0,
	           mrd.store_no, -- originating_store_no
                   mrd.balancing_entity_id,
                   a.empl_purch_flag,
                   lo.unit_of_measure,
	    ...
	    FROM #sub_trans_line a WITH (NOLOCK)
                   INNER JOIN media_reconciliation_detail mrd WITH (NOLOCK) ON (a.transaction_date = mrd.rec_date
                                                            ...
                                                            AND a.discounted_line_object = abs(SIGN(mrd.rec_amount_type - 2)))
                   INNER JOIN ORG_CHN s WITH (NOLOCK) ON (mrd.store_no = s.ORG_CHN_NUM)
                   LEFT JOIN line_object_action_association lo WITH (NOLOCK) ON (mrd.line_object = lo.line_object
                                                                                 AND mrd.line_action = lo.line_action
                               AND mrd.transaction_category = lo.transaction_category)
  	     WHERE a.class_code IN (24, 5, 25)
               AND ((mrd.rec_side = 0 AND a.class_code = 24)
                     OR (a.class_code IN (5, 25) AND mrd.rec_amount_subtype IN (4, 24) AND rec_amount_type = 1))
   ...
   /*	Perform integrity check */
        ...
        SELECT @trans_line_total = SUM(amount)
         FROM #sub_trans_line
        WHERE transaction_category <> 250 OR class_code <> 25; -- this where clause MUST be the same as one below in order to avoid out of balance incorrectly.
	  ...
	  BEGIN
            SELECT @errno = 201048,
             ...
             @process_timestamp, @transaction_count, @memo1, null, null, @sales_date;
          END;
	...
	SELECT
	 ...
	 WHERE transaction_category <> 250 OR class_code <> 25 --over/short already included under class_code rec_amount_subtype 5
      GROUP BY
	       store_no,
	       register_no,
	       transaction_category,
