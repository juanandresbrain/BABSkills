...
SELECT  @process_id, @comparison_id, 'audit_status', 'Reg Status', 
	...
	convert(nvarchar, a.sales_date) + ' _ ' + convert(nvarchar, a.date_reject_id),
        convert(nvarchar, a.audit_status) + ' _ ' + convert(nvarchar, status_set_by_user_id) + ' _ ' +
        convert(nvarchar, a.update_in_progress) + ' _ ' + 
        convert(nvarchar, a.trickle_in_progress_flag) + ' _ ' + 
        convert(nvarchar, a.completion_date_time, 9) + ' _ ' + 
        convert(nvarchar, a.archived_flag),
	a.register_poll_id,
        a.status_remark
  FROM audit_status a
...
SELECT  @process_id, @comparison_id, 'audit_status', 'Statistics', 
        convert(nvarchar, a.store_no) + ' _ ' + convert(nvarchar, a.register_no) + ' _ ' + 
	convert(nvarchar, a.sales_date) + ' _ ' + convert(nvarchar, a.date_reject_id),
       convert(nvarchar, a.sa_reject_qty) + ' _ ' + 
       convert(nvarchar, a.if_reject_qty) + ' _ ' + 
       convert(nvarchar, a.exception_qty) + ' _ ' +
       convert(nvarchar, a.missing_qty) + ' _ '  +
       convert(nvarchar, a.translate_error_qty) + ' _ ' +
       convert(nvarchar, a.duplicate_qty) + ' _ '  +
       convert(nvarchar, a.valid_qty),
        convert(nvarchar, a.exceptions_verified) + ' _ ' +
        convert(nvarchar, a.missing_verified) + ' _ '  + 
        convert(nvarchar, a.translate_error_verified) + ' _ ' +
        convert(nvarchar, a.duplicate_verified),
       ' '  
  FROM audit_status a
 WHERE (a.status_date >= @from_posting_date OR a.edited_date >= @from_posting_date)
   AND (a.status_date < @to_posting_date OR a.edited_date < @to_posting_date)
...
SELECT  @process_id, @comparison_id, 'store_audit_status', 'Status', 
        convert(nvarchar, a.store_no) + ' _ ' + 
	convert(nvarchar, a.sales_date) + ' _ ' + convert(nvarchar, a.date_reject_id),
        convert(nvarchar, a.store_audit_status) + ' _ ' + convert(nvarchar, status_set_by_user_id) + ' _ ' +
        convert(nvarchar, a.update_in_progress) + ' _ ' + 
        convert(nvarchar, a.trickle_in_progress_flag) + ' _ ' + 
        convert(nvarchar, a.archived_flag),
       convert(nvarchar, a.short_by_tender_over_limit) + ' _ ' + 
       convert(nvarchar, a.opening_drawer_discrepancy) + ' _ ' + 
       convert(nvarchar, a.media_rec_verified) + ' _ ' + 
       convert(nvarchar, a.media_short),
        a.status_remark
