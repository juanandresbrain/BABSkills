...
```sql
create proc [dbo].[util_posted_unposted_intf_$sp] 
...
SELECT interface_id, sv.object_id, SUBSTRING(ISNULL(description,interface_description),1,30)
  FROM interface_directory id WITH (NOLOCK), smartview_interface sv WITH (NOLOCK)
 WHERE id.object_id IS NOT NULL
   AND update_timing > 0
   AND sv.queue_id = id.interface_id
