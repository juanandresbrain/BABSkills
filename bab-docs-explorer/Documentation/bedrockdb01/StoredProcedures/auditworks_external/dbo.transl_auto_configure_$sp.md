       ...
       WHERE v.line_object_type = @line_object_type
         AND (v.default_reference_type = @reference_type_type
             OR (v.line_action IN (32,234,246) AND @line_object_type = 6) -- inserts all tender counted. See changes in media_parameter_object_$trI.
                ...
                OR (v.default_reference_type = 1 AND @reference_type_type IN (2, 3, 218, 227)) /*handle debit and house card, room charge and paypal like credit card*/	
...
SELECT CASE table_name WHEN 'line_object' THEN 1 
       ...
       w.desc_update_flag,
       w.store_no, w.register_no, w.entry_date_time, w.transaction_series, w.transaction_no, w.line_id, CONVERT(smalldatetime, CONVERT(char(8), w.entry_date_time,112)) transaction_date
  FROM work_config_pos_code w
