...
flowchart LR
    ...
    work_ecp_period_export3(["work_ecp_period_export3"]) --> SP
```
...
| work_ecp_period_export3 |

## Stored Procedure Code

```sql
create proc dbo.ecp_period_export3_$sp    @pay_period_export_date	datetime,
...
AS
/* 
Proc Name: ecp_period_export3_$sp 
Desc:   Performs period-end export of commission amounts with rows by date/employee/transaction-role-group to payroll

HISTORY:  
Date     Name           Def#    Desc
Jan19,15 Vicci     TFS-100881   Support the export of multiple pay-periods (refix 85597).
Jul22,14 Vicci      TFS-78522   Author
*/
...
SELECT @errno = 0,
       ...
       @process_name = 'ecp_period_export2_$sp',
       @process_no = 282,
       @stream_no = 1
...
SELECT @pay_period_start_date = dateadd(dd, 1, convert(datetime, flag_alpha_value))  --note the alpha value is the period last exported, the date value is the period export requested.
...
BEGIN TRANSACTION
INSERT into work_ecp_period_export3(
       pay_period_end_datetime,
       home_store_no,
       employee_no,
       commission_export_group_code,
       commission_amount,
...
SELECT convert(nvarchar, @pay_period_export_date, 101),
       em.PRMY_ORG_CHN_NUM, 
       q.employee_no,
       COALESCE(etr.commission_export_group_code, etr.employee_transaction_role) commission_export_group_code,
       SUM(q.commission_amount) commission_amount,
       convert(nvarchar, @pay_period_start_date, 101),
       MAX(em.LAST_NAME),
       MAX(em.FRST_NAME),
       CASE WHEN etr.commission_export_group_code IS NULL THEN employee_transaction_role_desc ELSE NULL END
  FROM (SELECT ets.employee_no,
         ...
         GROUP BY ets.employee_no, ets.employee_transaction_role
        ...
        SELECT eca.employee_no,
               '..ADJ' + IsNull(convert(nvarchar, eca.auto_commission_adj_id), '') employee_transaction_role,
         ...
         GROUP BY eca.employee_no,  '..ADJ' + IsNull(convert(nvarchar, eca.auto_commission_adj_id), '')
        HAVING SUM(eca.commission_adj_amount) <> 0
      ) q
   LEFT OUTER JOIN EMPLY em
     ON q.employee_no = em.EMPLY_NUM
   LEFT OUTER JOIN employee_transaction_role etr
     ON q.employee_transaction_role = etr.employee_transaction_role
 GROUP BY 
       em.PRMY_ORG_CHN_NUM, 
       q.employee_no,
       COALESCE(etr.commission_export_group_code, etr.employee_transaction_role),
       CASE WHEN etr.commission_export_group_code IS NULL THEN employee_transaction_role_desc ELSE NULL END
HAVING SUM(q.commission_amount) <> 0
 ORDER BY em.PRMY_ORG_CHN_NUM, q.employee_no, COALESCE(etr.commission_export_group_code, etr.employee_transaction_role)
SELECT @errno = @@error, @current_rows = @@rowcount
IF @errno <> 0
BEGIN
  SELECT @errmsg = 'Failed to build export of commission amounts for payroll',
         @object_name = 'work_ecp_period_export2',
