...
SELECT @alert_limit = CASE WHEN IsNumeric(par_value) = 1 THEN convert(numeric(12,4), par_value) ELSE NULL END
  FROM ecp_parameter
...
SELECT COALESCE(ep.ORG_CHN_NUM, 
       ...
       employee_transaction_role,
       convert(char(5), era.auto_adj_id) + '-' + h.comment_indicator comment_indicator,
       CASE WHEN h.comment_indicator = ' AUTO' 
            THEN RIGHT('              ' + convert(nvarchar, convert(numeric(12,2), era.reference_amount)), 14) + ':  ' 
 ...
 GROUP BY COALESCE(ep.ORG_CHN_NUM, 
       ...
       COALESCE(CASE WHEN @compare_role = 1 
	             ...
	             ELSE convert(nvarchar(20), NULL) 
	        END, tr.employee_transaction_role, @employee_transaction_role),
       convert(char(5), era.auto_adj_id) + '-' + h.comment_indicator,
       CASE WHEN h.comment_indicator = ' AUTO' THEN RIGHT('              ' + convert(nvarchar, convert(numeric(12,2), era.reference_amount)), 14) + ':  ' ELSE '' END + h.comment 
       ) q
  INNER JOIN #select_calendar_period cl
          ON q.comment_level = convert(nvarchar, cl.CLNDR_LVL_SEQ) + '.' + convert(nvarchar, cl.period_start_datetime)
         AND cl.add_subtract_flag = 1
...
SELECT DISTINCT 7, '', 0, 0, 0, 0,  --employee detail auto-adjustment variable line
       ...
       '''' + 
	RIGHT('              ' + convert(nvarchar, convert(numeric(12,2), variable_amount)), 14) + ':  '
...
/*
       ...
       convert(char(5), var.accumulation_basis) + convert(char(5), var.comparison_type) + convert(nvarchar, var.auto_adj_period_end_datetime, 101) + ' ' + convert(nvarchar, var.auto_adj_period_end_datetime, 108) comment_indicator,
 ...
 GROUP BY COALESCE(ep.ORG_CHN_NUM, 
       ...
       COALESCE(CASE WHEN @compare_role = 1 AND era.auto_adj_id IS NOT NULL 
	             ...
	             ELSE convert(nvarchar(20), NULL) 
	        END, tr.employee_transaction_role, @employee_transaction_role),
       convert(char(5), var.accumulation_basis) + convert(char(5), var.comparison_type) + convert(nvarchar, var.auto_adj_period_end_datetime, 101) + ' ' + convert(nvarchar, var.auto_adj_period_end_datetime, 108),
       var.auto_adj_period_end_datetime,
       var.comp_employee_no,
       var.comp_primary_position,
       var.comp_primary_selling_area_no,
...
--SELECT 'Test, Column is in list'
 ...
 SELECT @group_no = 7
  ...
  BEGIN
        ...
        BEGIN
        SELECT @sql_command = '''' + @KPI_desc + ':  '' + convert(char(15), ' + @sort_column_name + ') 
          FROM #ecp_prod_rpt2 r'      
        END
        ELSE
        BEGIN
          SELECT @sql_command = '''' + @KPI_desc + ':  '' + convert(char(15), IsNull(q.sort_col, 0.00)) 
           FROM #ecp_prod_rpt2 r
      ...
      BEGIN
        IF @sort_column_name = 'rcap_alert' 
          SELECT @sql_command = @sort_column_name + '
            FROM #ecp_prod_rpt2 r'      
     ELSE
          SELECT @sql_command = '''' + @KPI_desc + ':  '' + right(convert(char(15), ' + @sort_column_name + '), 6) 
 FROM #ecp_prod_rpt2 r'      
      END
      SELECT @kpi_done = 1
...
BEGIN
    ...
    SELECT @sql_command = 'SET ROWCOUNT ' + convert(nvarchar, @top_X) + '
  			   INSERT INTO #filter_employee(employee_no, group_desc_1, group_desc_2, group_desc_3, group_desc_4, group_desc_5, group_desc_6, group_desc_7)
                         ...
                         AND ( ' + @filter_clause  + ' )
                           ...
                           SET ROWCOUNT 0'
    EXEC sp_executesql @sql_command, N'@alert_limit numeric(12,4)', @alert_limit 
    SELECT @errno = @@error
    IF @errno <> 0
    BEGIN
      PRINT @sql_command
  ...
  BEGIN
    ...
    BEGIN
      ...
      EXEC sp_executesql @sql_command, N'@alert_limit numeric(12,4),
                                         @group_desc_1 nvarchar(400),
                                         @group_desc_2 nvarchar(400),
                                         @group_desc_3 nvarchar(400),
                                         @group_desc_4 nvarchar(400),
