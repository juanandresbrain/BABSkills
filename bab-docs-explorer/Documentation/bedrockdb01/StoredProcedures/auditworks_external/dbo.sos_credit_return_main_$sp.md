...
BEGIN
  ...
  SELECT th.transaction_id,
   ...
   WHERE th.store_no = @store_no
     ...
     AND ( (CONVERT(NUMERIC(12,4), (tl.gross_line_amount - tl.pos_discount_amount) * db_cr_none) < 0
	AND transaction_void_flag IN (0,8))
	OR (CONVERT(NUMERIC(12,4), (tl.gross_line_amount - tl.pos_discount_amount) * db_cr_none) > 0 AND transaction_void_flag = 1) )
