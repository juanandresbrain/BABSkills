# dbo.transl_stock_control_imrd_$sp
...
```mermaid
flowchart LR
    SP["dbo.transl_stock_control_imrd_$sp"]
    ORG_CHN(["ORG_CHN"]) --> SP
...
```sql
create proc dbo.transl_stock_control_imrd_$sp 
...
/* 
PROC NAME: transl_stock_control_imrd_$sp
     DESC: This proc will try to replace any imrd of '-3' with alpha_code found in code_description with a code_type 
...
Apr29,05  Maryam     DV-1202 Author
*/
...
SELECT @process_name = 'transl_stock_control_imrd_$sp',
       @message_id   = 201068,       
       @process_id = @@spid,
       @fixed_count = 0,
       @issue_count = 0,
...
-- Find out how many rows exist with imrd = '-3'
SELECT @issue_count = count(*)
  FROM transl_stock_control_detail
 WHERE imrd = '-3'
   AND lookup_pos_code_imrd IS NOT NULL
...
AND NOT EXISTS (SELECT 1 FROM transl_stock_control_detail
                 WHERE pos_description_imrd IS NOT NULL)  
  RETURN
...
UPDATE transl_stock_control_detail
   SET imrd = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
  FROM transl_stock_control_detail t, stock_control_display_def d, code_description c
 WHERE t.imrd = '-3'
   AND t.lookup_pos_code_imrd IS NOT NULL
   AND t.display_def_id = d.display_def_id
   AND t.lookup_pos_code_imrd = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
   AND d.imrd_code_type <> 5
   AND c.code_type = d.imrd_code_type

SELECT @errno = @@error,
       @fixed_count = @fixed_count + @@rowcount
IF @errno != 0
  BEGIN
    SELECT @errmsg = 'Failed to set the imrd.',
           @object_name = 'transl_stock_control_detail',
...
AND t.display_def_id = d.display_def_id
       AND d.imrd_code_type <> 5
       AND c.code_type = d.imrd_code_type
       AND t.imrd = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
       AND c.resource_id = l.resource_id
       AND t.pos_description_imrd IS NOT NULL
       AND (t.pos_description_imrd <> l.system_description OR l.system_description IS NULL)
  ...
  BEGIN
    ...
    SELECT @request_id, 
      ...
      WHERE t.imrd <> '-3'
        ...
        AND c.code_type = d.imrd_code_type  
        AND t.pos_description_imrd IS NOT NULL     
        AND (t.pos_description_imrd <> c.code_system_descr OR c.code_system_descr IS NULL)
      GROUP by c.code_type,
           c.code,
           t.pos_description_imrd,
           c.resource_id
