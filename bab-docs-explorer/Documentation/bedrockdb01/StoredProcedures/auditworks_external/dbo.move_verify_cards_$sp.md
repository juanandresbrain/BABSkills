...
INSERT #credit_cards
 ...
 SELECT tl.transaction_id,
  ...
  WHERE wt.process_id = @process_id
    ...
    AND r.validation_type  = 1
    AND r.edit_active_flag = 1
    AND (tl.reference_no IS NULL OR IsNumeric(tl.reference_no) = 0 OR len(tl.reference_no) > 20) -- exclude rows already inserted
    AND tl.invalid_reference_no IS NULL --
...
-- cleanup invalid_reference_no if card no is valid
UPDATE transaction_line
...
-- Change card_type to '?' for valid card_no that needs to be encrypted.
-- In this case, log it as I/F reject and the mass correct process will do the encryption.
UPDATE #credit_cards
   SET calculated_card_type = '?'
  FROM #credit_cards ec, restricted_field rf
 WHERE ec.card_no IS NOT NULL --
