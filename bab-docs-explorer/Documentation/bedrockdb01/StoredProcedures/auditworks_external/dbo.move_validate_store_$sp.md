...
IF @errno != 0
    BEGIN
      SELECT @errmsg = 'unable to select @reg_count_from from audit_status',
...
IF @errno != 0
    BEGIN
      SELECT @errmsg = 'unable to select @reg_count_to from audit_status',
             @object_name = 'audit_status',
             @operation_name = 'SELECT'
      GOTO error
    END 
...
IF @reg_count_from = @reg_count_to
  BEGIN /* Def 8447: Look for store/reg not on file (needs to be corrected first),
   ...
   SELECT @rows = COUNT(store_no)
    ...
    IF @errno != 0
      BEGIN
        SELECT @errmsg = 'unable to select @rows from audit_status',
               @object_name = 'audit_status',
               @operation_name = 'SELECT'
        GOTO error
   END 
