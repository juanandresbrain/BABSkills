...
```sql
create proc dbo.verify_register_$sp @process_id             binary(16),
@user_id                int,
@store_no 		int,
...
BEGIN
  ...
  BEGIN
    ...
    END -- @valid_qty > 0
    ELSE
    BEGIN -- @valid_qty = 0 and no audit concerns. Need to check for missing/unused/closed.
      ...
      BEGIN
        ...
        -- If the edit is not currently processing the store-reg-date, then re-evaluate the status if register is a server
        ...
        AND EXISTS (SELECT 1 FROM ORG_CHN_WRKSTN
	  	     WHERE ORG_CHN_NUM = @store_no
		       AND WRKSTN_NUM = @register_no
		       AND ISNULL(PRNT_WRKSTN_ID, WRKSTN_ID) = PRNT_WRKSTN_ID) -- server registers only
