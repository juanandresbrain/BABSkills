...
(@i_seq integer, 
 ...
 @i_lcid integer,
 @i_period_label varchar(500) OUT)
...
BEGIN
  ...
  SELECT @i_start_date_token  = "'" +convert(char(20),@i_startdatetime,113) + "'"

  SELECT @i_period_date_token = "'" + convert(char(20),@i_perioddatetime,113) + "'"
  ...
  ELSE
    BEGIN
      ...
      BEGIN
        ...
        SELECT @i_algrthm = SUBSTRING(@i_algrthm,1,@i_cnt-1) + 'substring(' +
                            SUBSTRING(@i_algrthm,@i_strt_bracket+1,@i_end_bracket-@i_strt_bracket-1) + ',4,3)' +
                            SUBSTRING(@i_algrthm,@i_end_bracket+1,500)   
