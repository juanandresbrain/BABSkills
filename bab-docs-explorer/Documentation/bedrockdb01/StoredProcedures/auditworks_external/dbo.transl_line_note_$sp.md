...
```sql
create proc dbo.transl_line_note_$sp @process_id		binary(16),
@process_no		smallint,
@lookup_pass		tinyint,
...
/* 
PROC NAME: transl_line_note_$sp
     DESC: This proc will try to replace any line_note of '-3' with alpha_code found in code_description with a code_type 
           that is not a free from entry and found in note_type for the note type in question. 
...
May02,05  Maryam     DV-1202 Author
*/
...
SELECT
       @process_name = 'transl_line_note_$sp',
       @message_id   = 201068,       
       @process_id = @@spid,
       @fixed_count = 0,
       @issue_count = 0,
...
-- Find out how many rows exist with line_note = '-3'
...
IF @issue_count = 0
AND NOT EXISTS (SELECT 1 FROM transl_line_note
...
UPDATE transl_line_note
   SET line_note = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
  FROM transl_line_note t, note_type n, code_description c
 WHERE t.line_note = '-3'
   AND t.lookup_pos_code IS NOT NULL
   AND t.lookup_pos_code = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
   AND t.note_type = n.note_type
   AND n.code_type <> 5
   AND n.code_type = c.code_type

SELECT @errno = @@error,
       @fixed_count = @fixed_count + @@rowcount
IF @errno != 0
  BEGIN
    SELECT @errmsg = 'Failed to set the line_note.',
           @object_name = 'transl_line_note',
  ...
  BEGIN
    ...
    SELECT @request_id,
     ...
     WHERE t.line_note <> '-3'
       ...
       AND n.code_type <> 5
       AND n.code_type = c.code_type
       AND t.line_note = ISNULL(c.alpha_code, CONVERT(nvarchar, code))
    ...
    SELECT @request_id,
           ...
           MIN(convert(nvarchar, t.entry_date_time, 109) + '|' + convert(nvarchar, t.store_no)  + '|' + convert(nvarchar, t.register_no)  + '|' + t.transaction_series  + '|' +  convert(nvarchar, t.transaction_no) + '|' + convert(nvarchar, t.line_id))
      FROM transl_line_note t
           INNER JOIN note_type n ON (t.note_type = n.note_type)
           LEFT JOIN ORG_CHN oc ON (store_no = oc.ORG_CHN_NUM)
     WHERE t.line_note = '-3'
       AND t.lookup_pos_code IS NOT NULL
       AND n.code_type <> 5
     GROUP by n.code_type,
...
ELSE -- Multi language is off
  BEGIN
    ...
    SELECT @request_id,
           ...
           0,
           MIN(convert(nvarchar, t.entry_date_time, 109) + '|' + convert(nvarchar, t.store_no)  + '|' + convert(nvarchar, t.register_no)  + '|' + t.transaction_series  + '|' +  convert(nvarchar, t.transaction_no) + '|' + convert(nvarchar, t.line_id))
      FROM transl_line_note t, note_type n
     WHERE t.line_note = '-3'
       AND t.lookup_pos_code IS NOT NULL
       AND t.note_type = n.note_type
       AND n.code_type <> 5
