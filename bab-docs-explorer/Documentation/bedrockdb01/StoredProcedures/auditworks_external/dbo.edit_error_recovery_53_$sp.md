# dbo.edit_error_recovery_53_$sp
...
```mermaid
flowchart LR
    SP["dbo.edit_error_recovery_53_$sp"]
...
```sql
create proc [dbo].[edit_error_recovery_53_$sp] 
...
SELECT @violated_sareject_rule = 53,
	@translate_msg = 'Duplicate lines from the translate were skipped by the edit for line notes',
	@process_name = 'edit_error_recovery_53_$sp',
        @message_id = 201068
...
SELECT
	...
	MIN(row_sequence_no),
      note_type
  FROM transl_line_note WITH (NOLOCK)
...
WHILE 1=1
  ...
  IF @errno != 0
    BEGIN
     SELECT @errmsg = 'Failed to delete duplicate rows from transl_line_note',
...
SELECT
	...
	getdate(),
	@translate_msg + 'note_type: '+ convert(nvarchar,note_type)
  FROM #work_lines_edit WITH (NOLOCK)
  WHERE line_count >= 2
