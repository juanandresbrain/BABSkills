...
```sql
create proc dbo.edit_insert_header_lines_$sp @errmsg			nvarchar(2000) OUTPUT,
...
SELECT
	 ...
	 sku_id,
	   ...
	   pos_deptclass,
	   CONVERT(numeric(12,4), gross_line_amount / units),
	   CONVERT(numeric(12,4), net_line_amount / units),
	   scanned,
	   pos_identifier,
	   pos_identifier_type,
           CONVERT(numeric(12,4), plu_amount / units),
           originating_store_no,
           source_store_no,
	   fulfillment_store_no,
	   cost
...
SELECT wh.transaction_id,
       ...
       gd.field_data_date,
       gd.field_data_num
  FROM transl_geninfo_detail gd, work_tran_header_edit wh WITH (NOLOCK)
