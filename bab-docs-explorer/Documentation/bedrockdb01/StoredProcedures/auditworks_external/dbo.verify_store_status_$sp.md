...
```sql
create proc dbo.verify_store_status_$sp 
...
(@process_id            binary(16),
 ...
 @called_by_edit        tinyint = 0) -- 2= called by accept/force accept, 3 = called by unaccept
...
IF @called_by_edit <> 3   ---  not called by unaccept
BEGIN
  -- set verified reg to accepted based on store-level closeout txn
    ...
    BEGIN       
      ...
      UPDATE audit_status
       ...
       WHERE store_no = @store_no
         ...
         AND (@auto_accept_flag IN (3,4) 
              OR ISNULL(unreconciled_media_present,0) =0
              OR EXISTS (SELECT 1 FROM media_reconciliation_status m 
  ...
  END; --IF @auto_accept_flag > 0
END; -- not called by unaccept  
  ...
  IF (@min_audit_status >= 200 AND @min_audit_status <= 300)
      ...
      AND ISNULL(@errmsg,' ') != 'force accept'
   SELECT @min_audit_status = 100;

  IF @min_audit_status < 300
