...
SELECT  @process_id, @comparison_id, 'audit_status', 'Media Rec', 
        convert(nvarchar, a.store_no) + ' _ ' + convert(nvarchar, a.register_no) + ' _ ' + 
	convert(nvarchar, a.sales_date) + ' _ ' + convert(nvarchar, a.date_reject_id),
       convert(nvarchar, a.short_by_tender_over_limit) + ' _ ' + 
       convert(nvarchar, a.opening_drawer_discrepancy) + ' _ ' + 
       convert(nvarchar, a.media_rec_verified) + ' _ ' + 
       convert(nvarchar, a.media_short) + ' _ ' +
       convert(nvarchar, a.unreconciled_media_present), 
       ' ',
       ' '
  FROM audit_status a
...
SELECT @process_id, @comparison_id, 'media_reconciliation_detail', 'Media Rec', 
       convert(nvarchar, m.store_no) + ' _ ' + convert(nvarchar, m.register_no) + ' _ ' + 
       convert(nvarchar, m.transaction_date) + ' _ ' + 
       convert(nvarchar, m.cashier_no) + ' _ ' + 
       convert(nvarchar, m.transaction_category) + ' _ ' + 
       convert(nvarchar, m.line_object) + ' _ ' + 
       convert(nvarchar, m.line_action) + ' _ ' + 
       convert(nvarchar,s.rec_type) + '.' + convert(nvarchar, s.balancing_method) + '.' + 
       convert(nvarchar, s.rec_group_line_object) + '.' + s.balancing_entity + ' _ ' +
       convert(nvarchar, m.rec_side) + ' _ ' + 
       convert(nvarchar, m.rec_amount_type) + ' _ ' + 
       convert(nvarchar, m.rec_amount_subtype) + ' _ ' + 
       convert(nvarchar, IsNull(h.store_no, a.store_no)) + ' ' + convert(nvarchar, IsNull(h.register_no, a.register_no)) + ' ' + 
       convert(nvarchar, m.rec_date) + ' ' + convert(nvarchar, IsNull(h.entry_date_time, a.entry_date_time), 9) + ' ' + 
       convert(nvarchar, IsNull(h.transaction_no, a.transaction_no)) + IsNull(h.transaction_series, a.transaction_series),
	convert(nvarchar, rec_amount) + ' _ ' + convert(nvarchar, period_from_date_time, 9) + ' _ ' + 
	convert(nvarchar, period_to_date_time, 9) + ' _ ' + convert(nvarchar, convert_to_domestic), 
        convert(nvarchar, issue_flag) + ' _ ' + convert(nvarchar, audit_activity_flag),
        null
...
SELECT @process_id, @comparison_id, 'media_reconciliation_status', 'Media Rec', 
       convert(nvarchar,s.rec_type) + '.' + convert(nvarchar, s.balancing_method) + '.' + 
       convert(nvarchar, s.rec_group_line_object) + '.' + s.balancing_entity,
	convert(nvarchar, first_unreconciled_date_time, 9) + ' _ ' + 
	convert(nvarchar, last_activity_date_time, 9) + ' _ ' + 
	convert(nvarchar, last_reconciliation_date_time, 9) + ' _ ' + 
	convert(nvarchar, unreconciled_activity_amount) + ' _ ' + 
	convert(nvarchar, unreconciled_exchange_amount) + ' _ ' + 
	convert(nvarchar, current_balance_amount) + ' _ ' + 
	convert(nvarchar, current_balance_exchange_amt),
       convert(nvarchar, unrec_tolerance_days) + ' _ ' + 
       convert(nvarchar, unrec_tolerance_amount) + ' _ ' + 
       convert(nvarchar, media_parameter_set_no) + ' _ ' + 
       convert(nvarchar, foreign_currency_id),
        null
...
SELECT @process_id, @comparison_id, 'media_unreconciliation', 'Media Rec', 
       ...
       convert(nvarchar,s.rec_type) + '.' + convert(nvarchar, s.balancing_method) + '.' + 
       convert(nvarchar, s.rec_group_line_object) + '.' + s.balancing_entity + ' _ ' +
       convert(nvarchar, IsNull(h.entry_date_time, a.entry_date_time), 9) + ' ' + 
       convert(nvarchar, IsNull(h.transaction_no, a.transaction_no)) + IsNull(IsNull(h.transaction_series, a.transaction_series), 'Integrity!!! rec_id not on file: ' + convert(nvarchar, m.rec_id)),
	convert(nvarchar, m.unrec_activity_flag),
	null,
        null
...
SELECT @process_id, @comparison_id, 'media_reconciliation_trans', 'Media Rec', 
       convert(nvarchar, m.entry_date_time, 9) + ' _ ' + 
       convert(nvarchar,s.rec_type) + '.' + convert(nvarchar, s.balancing_method) + '.' + 
       convert(nvarchar, s.rec_group_line_object) + '.' + s.balancing_entity + ' _ ' +
       convert(nvarchar, m.rec_side) + ' _ ' + 
       convert(nvarchar, m.rec_amount_type) + ' _ ' + 
       convert(nvarchar, m.rec_amount_subtype) + ' _ ' + 
       convert(nvarchar, m.store_no) + ' ' + convert(nvarchar, m.register_no) + ' ' + 
       convert(nvarchar, m.transaction_date) + ' ' + 
       convert(nvarchar, IsNull(h.transaction_no, a.transaction_no)) + IsNull(IsNull(h.transaction_series, a.transaction_series), 'Integrity!!! rec_id not on file: ' + convert(nvarchar, m.transaction_id)) + ' _ ' +
       convert(nvarchar, m.line_id),
	convert(nvarchar, m.rec_amount) + ' _ ' + 
	convert(nvarchar, m.void_flag) + ' _ ' + 
	convert(nvarchar, m.transaction_category) + ' _ ' + 
	convert(nvarchar, m.line_object) + ' _ ' + convert(nvarchar, m.line_action) + ' _ ' + 
	m.reference_no,
       convert(nvarchar, m.cashier_no) + ' _ ' + 
       convert(nvarchar, m.tender_total) + ' _ ' + 
       convert(nvarchar, m.audit_activity_flag),
       null
