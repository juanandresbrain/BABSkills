...
DECLARE
	...
	@comparison_key			nvarchar(255),
	@rows				int,
	@prev_comparison_key		nvarchar(255),
...
SELECT @process_id, @comparison_id, 'cust_liability', 'Customer Liability Data', 
       u.comparison_key + ' _ ' + convert(nvarchar,u.ref_type) +' _ '+convert(nvarchar,u.transaction_key),
       convert(nvarchar,c.date_issued) + ' _ '+ convert(nvarchar,c.issuing_store_no) + ' _ ' + convert(nvarchar,c.tracking_id) + ' _ '+ 
       convert(nvarchar,c.liability_amount) + ' _ ' + convert(nvarchar,c.receivable_amount) + ' _ '+ 
       convert(nvarchar,c.amount_3) + ' _ ' + convert(nvarchar,c.amount_4) + ' _ '+ 
       convert(nvarchar,c.amount_5) + ' _ ' + convert(nvarchar,c.amount_6) + ' _ '+ 
       convert(nvarchar,c.amount_7) + ' _ ' + convert(nvarchar,c.amount_8) + ' _ '+ 
       convert(nvarchar,c.amount_9) + ' _ ' + convert(nvarchar,c.amount_10) + ' _ '+ 
       convert(nvarchar,c.stocked_amount) + ' _ ' + convert(nvarchar,c.stocked_flag) + ' _ '+ 
       convert(nvarchar,c.stocked_stolen_flag) + ' _ ' + convert(nvarchar,c.issued_flag),
       
       convert(nvarchar,c.stolen_from_cust_flag) + ' _ ' + convert(nvarchar,c.forfeited_flag) + ' _ '+ 
       convert(nvarchar,c.assumed_completion_date) + ' _ ' + convert(nvarchar,c.reopen_date) + ' _ '+ 
       convert(nvarchar,c.expiry_date) +' _ '+ convert(nvarchar,c.original_amount) + ' _ ' + 
       convert(nvarchar,c.pos_status) + ' _ ' + convert(nvarchar,c.pos_amount_1)+ ' _ ' + convert(nvarchar,c.pos_amount_2)+ ' _ ' +
       convert(nvarchar,c.pos_amount_3)+ ' _ ' + convert(nvarchar,c.employee_no),
       null
...
SELECT @process_id, @comparison_id, 'cust_liability', 'Customer Liab Customer Data', 
       ...
       convert(nvarchar,c.customer_no) + ' _ ' + c.pos_tax_jurisdiction_code + ' _ ' + 
...
SELECT @process_id, @comparison_id, 'cust_liability_detail', 'Customer Liability Detail', 
       u.comparison_key + ' _ ' + convert(nvarchar,u.ref_type) +' _ ' + 
       convert(nvarchar,u.transaction_key) + ' _ ' + convert(nvarchar,d.line_object) + ' _ ' + 
       convert(nvarchar,d.upc_no) + ' _ ' + convert(nvarchar,d.upc_lookup_division) + ' _ ' + 
       convert(nvarchar,d.discount_line_object),
       
       convert(nvarchar, d.location_store_no) + ' _ ' + convert(nvarchar, d.location_no) + ' _ ' +
       convert(nvarchar, d.amount_outstanding) + ' _ ' + convert(nvarchar, d.units_outstanding) + ' _ ' +
       convert(nvarchar, d.units_2) + ' _ ' + convert(nvarchar, d.units_3) + ' _ ' +
       convert(nvarchar, d.units_4) + ' _ ' + convert(nvarchar, d.units_5),
       null,
       null  
...
BEGIN
  ...
  SELECT @comparison_key = @reference_no + ' _ ' + convert(nvarchar,@reference_type) + ' _ ' + 
         convert(nvarchar,@key_store_no) + ' _ ' + convert(nvarchar,@transaction_date) + ' _ ' + 
         convert(nvarchar,@process_no) + ' _ ' + convert(nvarchar,store_no) + ' _ ' + 
         convert(nvarchar,register_no) + ' _ ' + convert(nvarchar,transaction_no) + ' _ ' + transaction_series
  ...
  IF @rows = 0 
    SELECT @comparison_key = @reference_no + ' _ ' + convert(nvarchar,@reference_type) + ' _ ' + 
             convert(nvarchar,@key_store_no) + ' _ ' + convert(nvarchar,@transaction_date) + ' _ ' + 
             convert(nvarchar,@process_no) + ' _ _ _ ' + convert(nvarchar,@process_key) + ' _ '
  ...
  SELECT @comparison_key = @comparison_key + convert(nvarchar, @sequence_no)
  ...
  SELECT @process_id, @comparison_id, 'cust_liability_history', 'Customer Liability History', 
         ...
         convert(nvarchar,h.transaction_category) + ' _ ' +
         convert(nvarchar, h.transaction_void_flag) + ' _ ' + convert(nvarchar, h.interface_control_flag) + ' _ ' +
         convert(nvarchar, h.liability_amount) + ' _ ' + convert(nvarchar, h.receivable_amount) + ' _ ' + 
         convert(nvarchar, h.amount_3) + ' _ ' + convert(nvarchar, h.amount_4) + ' _ ' + 
         convert(nvarchar, h.amount_5) + ' _ ' + convert(nvarchar, h.amount_6) + ' _ ' + 
         convert(nvarchar, h.amount_7) + ' _ ' + convert(nvarchar, h.amount_8) + ' _ ' + 
         convert(nvarchar, h.amount_9) + ' _ ' + convert(nvarchar, h.amount_10) + ' _ ' + 
         convert(nvarchar, h.stocked_amount) + ' _ ' + convert(nvarchar, h.stocked_flag),
         
         convert(nvarchar, h.stocked_stolen_flag) + ' _ ' + convert(nvarchar, h.issued_flag) + ' _ ' + 
         convert(nvarchar, h.stolen_from_cust_flag) + ' _ ' + convert(nvarchar, h.forfeited_flag), 
         
         convert(nvarchar, h.entry_date_time)
...
BEGIN
  ...
  SELECT @comparison_key = @reference_no + ' _ ' + convert(nvarchar,@reference_type) + ' _ ' + 
         convert(nvarchar,@key_store_no) + ' _ ' + convert(nvarchar,@line_object) + ' _ ' + 
         convert(nvarchar,@upc_no) + ' _ ' + convert(nvarchar,@upc_lookup_division) + ' _ ' + 
         convert(nvarchar,@discount_line_object) + ' _ ' + convert(nvarchar,@transaction_date) + ' _ ' + 
         convert(nvarchar,@process_no) + ' _ ' + convert(nvarchar,store_no) + ' _ ' + 
         convert(nvarchar,register_no) + ' _ ' + convert(nvarchar,transaction_no) + ' _ ' + 
         transaction_series
    FROM transaction_header
   WHERE transaction_id = @process_key         
...
SELECT @process_id, @comparison_id, 'cust_liability_detail_history', 'Customer Liab Detail Hist', 
         ...
         convert(nvarchar, dh.location_store_no) + ' _ ' + convert(nvarchar, dh.location_no) + ' _ ' +
         convert(nvarchar, dh.transaction_category) + ' _ ' + convert(nvarchar, dh.transaction_void_flag) + ' _ ' + 
         convert(nvarchar, dh.interface_control_flag) + ' _ ' + convert(nvarchar, dh.amount_outstanding) + ' _ ' + 
         convert(nvarchar, dh.units_outstanding) + ' _ ' + convert(nvarchar, dh.units_2) + ' _ ' + 
         convert(nvarchar, dh.units_3) + ' _ ' + convert(nvarchar, dh.units_4) + ' _ ' + convert(nvarchar, dh.units_5),
         NULL,
         convert(nvarchar, dh.entry_date_time) 
