# ERP.spMergePurchaseOrderHeader_Bak20210125
...
```mermaid
flowchart LR
    SP["ERP.spMergePurchaseOrderHeader_Bak20210125"]
    ERP_PurchaseOrderHeader(["ERP.PurchaseOrderHeader"]) --> SP
    ERP_PurchaseOrderHeaderStage(["ERP.PurchaseOrderHeaderStage"]) --> SP
```
...
```sql
CREATE proc [ERP].[spMergePurchaseOrderHeader_Bak20210125]
...
Using (select * from ERP.PurchaseOrderHeaderStage where isCurrent = 1) as source
On (
