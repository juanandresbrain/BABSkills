# WMS.spSelectWMSPurchaseOrderReceiptsWIP20220525
...
```mermaid
flowchart LR
    SP["WMS.spSelectWMSPurchaseOrderReceiptsWIP20220525"]
...
```sql
CREATE proc [WMS].[spSelectWMSPurchaseOrderReceiptsWIP20220525] 
...
select	
		Warehouse,
		convert(varchar, cast(dateadd(hh,-@offset, r.MessageQueueDateUTC) as smalldatetime), 101) as receipt_date,
		...
		'' as BOL
from #stage
group by convert(varchar, cast(dateadd(hh,-@offset, r.MessageQueueDateUTC) as smalldatetime), 101)
--convert(varchar, cast(MessageQueueDateUTC as smalldatetime), 101), AptosPONumber, ASN, ItemID, Warehouse
...
---prepare data for printing
	...
	declare header cursor for
		select distinct receipt_date, convert(varchar, receipt_date, 112) + convert(varchar, datepart(hh, receipt_date)) + convert(varchar, datepart(mi, receipt_date)) + convert(varchar, datepart(ss, receipt_date)) + convert(varchar, max(id)) doc_nbr, isnull(bol, 'NO_BOL') bol, po, asn, case when Warehouse='9980' then '0980' when Warehouse='1013' then '0013' else Warehouse end as Warehouse
		from #wms_po_receipts
		group by receipt_date, po, asn, bol, case when Warehouse='9980' then '0980' when Warehouse='1013' then '0013' else Warehouse end
		order by receipt_date, po, asn, bol

	open header
	while @counterH <= @totalH
		begin
			fetch next from header into @date, @docnbr, @bol, @po, @asn, @Warehouse
