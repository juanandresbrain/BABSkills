...
```mermaid
flowchart LR
    SP["dbo.spAmex_Bank_Export"]
    dbo_babw_amexUS(["dbo.babw_amexUS"]) --> SP
    dbo_tmp_amex_bank(["dbo.tmp_amex_bank"]) --> SP
    dbo_tmp_amex_bank_control(["dbo.tmp_amex_bank_control"]) --> SP
    dbo_tmp_amex_bank_out(["dbo.tmp_amex_bank_out"]) --> SP
```
...
| dbo.tmp_amex_bank_out |

## Stored Procedure Code

```sql
CREATE PROCEDURE [dbo].[spAmex_Bank_Export] 
...
truncate table [dbo].[tmp_amex_bank]
truncate table [dbo].[tmp_amex_bank_out]
...
INSERT INTO [dbo].[tmp_amex_bank] ([As Of],[Currency],[BankID Type],[BankID],[Account],[Data Type],[BAI Code],
...
update  [dbo].[tmp_amex_bank]  set [rowNumNew] = [rowNum] + 3
...
--=============
 ...
 IF NOT EXISTS (SELECT * FROM [dbo].[tmp_amex_bank_control] WHERE CreatedDate = cast(getdate() as date))
		INSERT INTO [dbo].[tmp_amex_bank_control] ([CreatedDate],[lastBatchNum],[DayString],[TimeString])  
		VALUES (cast(getdate() as date), '01',@DayString, @TimeString)
    ELSE
       UPDATE [dbo].[tmp_amex_bank_control] SET lastBatchNum = FORMAT(lastBatchNum + 1,'00'), DayString = @DayString, TimeString = @TimeString WHERE CreatedDate = cast(getdate() as date)

SET @FilePath = '\\stl-dynsnc-p-01\oData\amex\bankStatementFiles'
set @TimeString = (SELECT REPLACE(CONVERT(varchar(5), GETDATE(), 108), ':', ''))
set @DayString = (SELECT FORMAT(GETDATE(), 'yy') +  RIGHT('00' + CONVERT(NVARCHAR(2), DATEPART(MONTH, GETDATE())), 2) + RIGHT('00' + CONVERT(NVARCHAR(2), DATEPART(DAY, GETDATE())), 2))


set @DayStringGroup1 = (select min(LEFT([Bank Reference], 8)) from  [dbo].[tmp_amex_bank])
set @DayStringLine2 = RIGHT(@DayStringGroup1, 6)
set @DayStringGroup2 = (select max(LEFT([Bank Reference], 8)) from  [dbo].[tmp_amex_bank])
set @UniqueSeq = (select DayString + lastBatchNum from [dbo].[tmp_amex_bank_control] where CreatedDate = cast(getdate() as date))
set @RowCountTrans = (select count(*)+2 from [dbo].[tmp_amex_bank])
set @RowCountGroup = (select count(*)+4 from [dbo].[tmp_amex_bank])
set @RowCountTotal = (select count(*)+6 from [dbo].[tmp_amex_bank])
...
insert [dbo].[tmp_amex_bank_out] (rowNum, col1) select 1,  '01,AMEXCLEAR,,' + '' + @DayString + '' +  ',' + '' + @TimeString + '' + ',' + '' + @UniqueSeq + '' +  ',,,2' 
insert [dbo].[tmp_amex_bank_out] (rowNum, col1) select 2,  '02,,AMEXCLEAR,1,' + '' + @DayStringLine2 + '' +  ',' + '' + @TimeString + '' + ',' + 'USD,1'
insert [dbo].[tmp_amex_bank_out] (rowNum, col1) select 3,  '03,1100AMEXCLEAR,,010,0,,,015,0,,,045,0,,,040,0,,,072,0,,,074,0,,,075,0,,'
insert [dbo].[tmp_amex_bank_out] (rowNum, col1)
select  rowNumNew, '16,' + [BAI Code] + ',' +  replace(cast([Amount] as varchar),'.','') + ',' + [Description] + ',' + [Bank Reference] + ',' + [Customer Reference] + ',' + [Bank Reference]
from  [dbo].[tmp_amex_bank] where left([Bank Reference],8) = @DayStringGroup1 and Description = 'Deposit' 
insert [dbo].[tmp_amex_bank_out] (rowNum, col1)
select rowNumNew, '16,' + [BAI Code] + ',' +  replace(cast([Amount] as varchar),'.','') + ',' + [Description] + ',' + [Bank Reference] + ',' + [Customer Reference] + ',' + [Bank Reference] 
from  [dbo].[tmp_amex_bank] where left([Bank Reference],8) = @DayStringGroup1  and Description = 'Disbursement'
insert [dbo].[tmp_amex_bank_out] (rowNum, col1) select 99997,  '49,0,' + '' + @RowCountTransString + '' 
insert [dbo].[tmp_amex_bank_out] (rowNum, col1) select 99998,  '98,0,1,' + '' + @RowCountGroupString + '' 
insert [dbo].[tmp_amex_bank_out] (rowNum, col1) select 99999,  '99,0,1,' + '' + @RowCountTotalString + ''

SET @SQL = 'SELECT col1 FROM [stl-ssis-p-01].IntegrationStaging.dbo.tmp_amex_bank_out order by rowNum asc'

SELECT  @CMD = 'bcp "' + @SQL + '" queryout "' + @FilePath + '\bankAmex_' + @FileDate + '.txt" -T -c -t,'
--SELECT  @CMD = 'bcp "' + @SQL + '" queryout "' + @FilePath + '\bankamex2.csv" -T -c -t,'
exec master..xp_cmdshell @CMD



dbo,spConvertCouponStagingtoHTML,-- =============================================
-- Author:		Tim Bytnar
-- Create date: 10/18/2017
-- Description:	Converts the staging table for Discount Coupon exports into an HTML formatted string to be emailed.
-- =============================================
CREATE PROC [dbo].[spConvertCouponStagingtoHTML] 
AS
BEGIN
  declare @textTitle varchar(100),
       @tableHTML nvarchar(max)

	select @textTitle = 'Export Results'
	...
	set @tableHTML = '<html><head><style>' +
	   ...
	   '<tr bgcolor=#4b6c9e>' +
	   '<td align=center><font face="calibri" color=White><b>Discount Amount</b></font></td>' +    -- Manually type headers
	   '<td align=center><font face="calibri" color=White><b>Country</b></font></td>' +     -- Manually type headers
	   '<td align=center><font face="calibri" color=White><b>Ending Date</b></font></td>' + -- Manually type headers
	   '<td align=center><font face="calibri" color=White><b>Discount ID</b></font></td>' +    -- Manually type headers
	   '<td align=center><font face="calibri" color=White><b>Total Coupons</b></font></td>' +     -- Manually type headers
	   '<td align=center><font face="calibri" color=White><b>Coupon Number</b></font></td></tr>'   -- Manually type headers

	declare @body varchar(max)
	select @body =
	(
	   select  td = discountAmount,     -- Here we put the column names
			   td = cntryAbbr,      -- Here we put the column names
			   td = endingDate,        -- Here we put the column names
			   td = discountID,        -- Here we put the column names
			   td = totalCoupons,        -- Here we put the column names
			   td = couponNumber        -- Here we put the column names
	...
	set @tableHTML = @tableHTML + '<br><br>The coupons file can be found here: \\stl-sftp-p-01\ecommerce\to-sfcc\coupons'

	--SELECT CAST(@tableHTML as varchar(8000)) as result

	exec msdb.dbo.sp_send_dbmail
	   @profile_name = 'BIAdmin',
	   @recipients = 'timb@buildabear.com;BrysonA@buildabear.com;RochelleP@buildabear.com;NataliaD@buildabear.com;NoelleM@buildabear.com;RachelS@buildabear.com',
	   @body = @tableHTML,
	   @subject = 'Coupons.xml was successfully generated',
	   @body_format = 'HTML'
END

dbo,spDBA_WhoIsActive,create PROC [dbo].[spDBA_WhoIsActive]
...
-- =============================================================================================================
-- Name: spDBA_WhoIsActive
--
-- Description:	replacement for sp_who, sp_who2
--
-- Output: see below
-- 
-- Available actions:
