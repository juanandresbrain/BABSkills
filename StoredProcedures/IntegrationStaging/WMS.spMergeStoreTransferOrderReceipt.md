# WMS.spMergeStoreTransferOrderReceipt
...
```mermaid
flowchart LR
    SP["WMS.spMergeStoreTransferOrderReceipt"]
...
```sql
CREATE proc [WMS].[spMergeStoreTransferOrderReceipt] -- Update to Proper Name 
...
using
	(
		...
		where 1=1
		--and ISNUMERIC(left(TargetLicensePlateNumber,1)) = 1 -- Remarked out on 10/11/2023		
		and s.TargetLicensePlateNumber <> '' -- Replaced above on 10/11/2023
		group by 
		s.InventorySiteId,
		s.WarehouseId, 
		s.DataAreaId , 
		...
		s.ContainerId,
		--s.TargetLicensePlateNumber,
		isnull(l.ContainerID,s.TargetLicensePlateNumber),
		s.LoadId, 
		s.ShipmentId, 
		s.WarehouseWorkId, 
		s.WarehouseWorkOrderType, 
