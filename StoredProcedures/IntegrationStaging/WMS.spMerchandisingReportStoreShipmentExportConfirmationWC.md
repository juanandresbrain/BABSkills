# WMS.spMerchandisingReportStoreShipmentExportConfirmationWC
...
```mermaid
flowchart LR
    SP["WMS.spMerchandisingReportStoreShipmentExportConfirmationWC"]
    dbo_sp_send_dbmail(["dbo.sp_send_dbmail"]) --> SP
    dbo_WCExport(["dbo.WCExport"]) --> SP
...
```sql
CREATE proc [WMS].[spMerchandisingReportStoreShipmentExportConfirmationWC]
...
IF (Object_ID('IntegrationStaging..WCExport') IS NOT NULL) DROP TABLE WCExport
...
--and left(document_number,10)  = '2900257796' -- Testing Purposes Only
...
begin
	...
	SET @1file_location = '\\stl-ssis-p-01\IntegrationStaging\3PW\WC_Distro\OUTBOUND\StoreShipmentConfirmation\'
	SET @1file_name = 'WC_store_shipments.csv'
	SET @1server = 'stl-ssis-p-01'
	SET @1database = 'IntegrationStaging'
	SET @1sqlcmd = 'sqlcmd -S' + @1server + ' -d' + @1database + ' -Q' + '"' + @1query + '"' + ' -o' + '"' + @1file_location + @1file_name + '"' + ' -s"," -w1000 -W'
	--SET @1sqlcmd = 'sqlcmd -S' + @1server + ' -E' +' -d' + @1database + ' -Q' + '"' + @1query + '"' + ' -o' + '"' + @1file_location + @1file_name + '"' + ' -s"," -w1000 -W'
	...
	EXEC [stl-ssis-p-01].msdb.dbo.sp_send_dbmail
		...
		@file_attachments ='\\stl-ssis-p-01\IntegrationStaging\3PW\WC_Distro\OUTBOUND\StoreShipmentConfirmation\WC_store_shipments.csv'
